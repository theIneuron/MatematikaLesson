// Assembler for Dars22 (frac_5_14 — Aralash son <-> noto'g'ri kasr), keep-visible rebuild.
...
// Re-run after editing _dars22_middle.jsx:  node build_dars22.cjs
const fs = require('fs');
...
let mid = fs.readFileSync(path.join(root, 'src/components/_dars22_middle.jsx'), 'utf8').replace(/\r\n/g, '\n');
if (mid.indexOf('/*__BASE__*/') === -1) throw new Error('base-styles placeholder /*__BASE__*/ missing in _dars22_middle.jsx');
mid = mid.replace('/*__BASE__*/', stylesBody);

const out = infra + mid;
fs.writeFileSync(path.join(root, 'src/components/Dars22.jsx'), out, 'utf8');
console.log('Dars22.jsx built:', out.length, 'chars,', out.split('\n').length, 'lines');

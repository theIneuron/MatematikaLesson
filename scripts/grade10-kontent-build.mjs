...
function parse(src) {
  ...
  for (const raw of src.split('\n')) {
    ...
    if (c.length === 4) {
      ...
      // ТРУБА ВНУТРИ ЗНАЧЕНИЯ. `|a| > 1` в таблице формул это не модуль, а два
      // лишних разделителя: строка разваливается на три столбца и попадает в
      // ветку текста как трёхъязычная. В уроке 9 из-за этого шпаргалка
      // собралась битой, а урок не проходился на последнем экране — причём
      // сборка отработала «успешно». Теперь это ошибка, а не сюрприз.
      problems.push(
        `экран ${cur.n}, ключ ${c[0]}: в значении есть труба. `
        + 'В таблице это разделитель столбцов — запиши без неё',
      )
    }
  }
  return screens

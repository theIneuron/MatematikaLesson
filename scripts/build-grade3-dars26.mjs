// build-grade3-dars26.mjs — собирает Dars26.jsx из донора Dars25.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars26.jsx`.
// Запуск: node scripts/build-grade3-dars26.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars25.jsx');
const OUT = path.resolve('src/components/grade3/Dars26.jsx');
const BLK = path.resolve(process.env.D26_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d26-blocks.txt');
const screens = sections('d26-screens.txt');
blocks.CONTENT = read('d26-content.txt').trim();
...
// у донора первая строка узла идёт без отступа: секции блоков обрезаются по краям
cut('{/* --- MARKAZIY STELA', '    {/* --- POL:', blocks.NODE + '\n', 'узел сцены');

cut('// --- ULUSH FIGURASI', 'const MCOne = ', blocks.FIGS + '\n', 'фигуры');

cut('// s2 — BIR NECHTA ULUSH', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
cut("// s4 — RASM BO'YICHA YOZUV", '// s5 — ', screens.S4 + '\n\n', 'экран s4');

// герой факткарды и префикс урока: имена донора меняем на свои.
// `d25` меняется целиком, а не только `d25-`: так же названы id градиентов сцены.
const head = s.slice(0, s.indexOf('const STYLES = BASE_STYLES + `'));
const tail = s.slice(s.indexOf('const STYLES = BASE_STYLES + `'));
s = head.split('EgyptFig').join('DialFig').split('d25').join('d26') + tail;

if (!s.includes('export default function DenomFamilyLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function DenomFamilyLesson({', 'export default function CompareSharesLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d24-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d25-').join('d26-');
const left = (keep.match(/\.d2[45]-/g) || []).length;
...
for (const dead of ['d25', 'EgyptFig', 'DenomFamilyLesson']) {

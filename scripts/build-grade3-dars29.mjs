// build-grade3-dars29.mjs — собирает Dars29.jsx из донора Dars28.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars29.jsx`.
// Запуск: node scripts/build-grade3-dars29.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars28.jsx');
const OUT = path.resolve('src/components/grade3/Dars29.jsx');
const BLK = path.resolve(process.env.D29_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d29-blocks.txt');
const screens = sections('d29-screens.txt');
blocks.CONTENT = read('d29-content.txt').trim();
...
cut('// s2 — IKKI LAGAN', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
// маркер берётся длиннее: короткий совпал бы с комментарием внутри CONTENT
cut("// s4 — RASM BO'YICHA: ikki doira, javob", '// s5 — ', screens.S4 + '\n\n', 'экран s4');
...
s = head.split('ClockFig').join('HalfFig').split('d28').join('d29') + tail;

if (!s.includes('export default function ImproperFracLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function ImproperFracLesson({', 'export default function CompareFracLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d27-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d28-').join('d29-');
const left = (keep.match(/\.d2[78]-/g) || []).length;
...
for (const dead of ['d28', 'ClockFig', 'ImproperFracLesson']) {

// build-grade3-dars34.mjs — собирает Dars34.jsx из донора Dars33.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars34.jsx`.
// Запуск: node scripts/build-grade3-dars34.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars33.jsx');
const OUT = path.resolve('src/components/grade3/Dars34.jsx');
const BLK = path.resolve(process.env.D34_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d34-blocks.txt');
const screens = sections('d34-screens.txt');
blocks.CONTENT = read('d34-content.txt').trim();
...
cut('// --- KRISTALL QATLAMI (D33)', 'const NumPad = ({ value', blocks.SCENE + '\n', 'сцена');

cut('// --- FACTCARD QAHRAMONI', 'const MCOne = ', blocks.FIGS + '\n', 'фигуры');

cut('// s2 — MODEL: chegara', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
// маркер берётся длиннее: короткий совпал бы с комментарием внутри CONTENT
cut("// s4 — RASM BO'YICHA: chizmadagi", '// s5 — ', screens.S4 + '\n\n', 'экран s4');
...
s = head.split('SamePerimFig').join('DmFig').split('d33').join('d34') + tail;

if (!s.includes('export default function PerimeterLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function PerimeterLesson({', 'export default function AreaUnitLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d32-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d33-').join('d34-');
const left = (keep.match(/\.d(32|33)-/g) || []).length;
...
for (const dead of ['d33', 'SamePerimFig', 'PerimeterLesson']) {

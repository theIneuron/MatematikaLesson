// build-grade3-dars31.mjs — собирает Dars31.jsx из донора Dars30.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars31.jsx`.
// Запуск: node scripts/build-grade3-dars31.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars30.jsx');
const OUT = path.resolve('src/components/grade3/Dars31.jsx');
const BLK = path.resolve(process.env.D31_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d31-blocks.txt');
const screens = sections('d31-screens.txt');
blocks.CONTENT = read('d31-content.txt').trim();
...
cut('// s2 — MODEL: avval ikkita', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
...
s = head.split('FruitFig').join('ScaleFig').split('d30').join('d31') + tail;

if (!s.includes('export default function AddFracLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function AddFracLesson({', 'export default function DecimalLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d29-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d30-').join('d31-');
const left = (keep.match(/\.d(29|30)-/g) || []).length;
...
for (const dead of ['d30', 'FruitFig', 'AddFracLesson']) {

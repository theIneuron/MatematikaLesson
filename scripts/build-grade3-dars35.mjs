// build-grade3-dars35.mjs — собирает Dars35.jsx из донора Dars34.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars35.jsx`.
// Запуск: node scripts/build-grade3-dars35.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars34.jsx');
const OUT = path.resolve('src/components/grade3/Dars35.jsx');
const BLK = path.resolve(process.env.D35_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d35-blocks.txt');
const screens = sections('d35-screens.txt');
blocks.CONTENT = read('d35-content.txt').trim();
...
s = head.split('DmFig').join('SplitFig').split('d34').join('d35') + tail;

if (!s.includes('export default function AreaUnitLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function AreaUnitLesson({', 'export default function RectAreaLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d33-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d34-').join('d35-');
const left = (keep.match(/\.d(33|34)-/g) || []).length;
...
for (const dead of ['d34', 'DmFig', 'AreaUnitLesson']) {

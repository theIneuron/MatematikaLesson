// build-grade3-dars28.mjs — собирает Dars28.jsx из донора Dars27.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars28.jsx`.
// Запуск: node scripts/build-grade3-dars28.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars27.jsx');
const OUT = path.resolve('src/components/grade3/Dars28.jsx');
const BLK = path.resolve(process.env.D28_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d28-blocks.txt');
const screens = sections('d28-screens.txt');
blocks.CONTENT = read('d28-content.txt').trim();
...
cut('// s2 — MODEL', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
// маркер берётся длиннее: короткий совпал бы с комментарием внутри CONTENT
cut("// s4 — RASM BO'YICHA: tasma butunligicha", '// s5 — ', screens.S4 + '\n\n', 'экран s4');
...
s = head.split('WholeFig').join('ClockFig').split('d27').join('d28') + tail;

if (!s.includes('export default function ShareOfNumberLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function ShareOfNumberLesson({', 'export default function ImproperFracLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d26-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d27-').join('d28-');
const left = (keep.match(/\.d2[67]-/g) || []).length;
...
for (const dead of ['d27', 'WholeFig', 'ShareOfNumberLesson']) {

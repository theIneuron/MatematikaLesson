// build-grade3-dars21.mjs — собирает Dars20.jsx из донора Dars20.jsx (этап 3, сборка).
//
...
// Запуск: node scripts/build-grade3-dars21.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars20.jsx');
const OUT = path.resolve('src/components/grade3/Dars21.jsx');
const BLK = path.resolve(process.env.D21_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d21-blocks.txt');
const screens = sections('d21-screens.txt');
blocks.CONTENT = read('d21-content.txt').trim();
...
cut('// --- NAZORAT TERMINALI (D20)', 'const NumPad = ({ value', blocks.SCENE + '\n', 'сцена');
cut('// --- TEKSHIRUV KARTASI', 'const MCOne = ', blocks.FIGS + '\n', 'фигуры');

// экраны: у уроков 20 и 21 совпадают все, кроме двух первых объяснений

cut('// s1 — IKKI TANISH TEKSHIRUV', '// s2 — ', screens.S1 + '\n\n', 'экран s1');

cut('// s2 — QOLDIQ BILAN TEKSHIRISH', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
...
s = head.split('ControlScene').join('ColumnScene').split('BarcodeFig').join('AlgoFig').split('d20-').join('d21-') + tail;

if (!s.includes('export default function CheckDivisionLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function CheckDivisionLesson({', 'export default function WrittenColumnLesson({');

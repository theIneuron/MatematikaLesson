// build-grade3-dars27.mjs — собирает Dars27.jsx из донора Dars26.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars27.jsx`.
// Запуск: node scripts/build-grade3-dars27.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars26.jsx');
const OUT = path.resolve('src/components/grade3/Dars27.jsx');
const BLK = path.resolve(process.env.D27_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d27-blocks.txt');
const screens = sections('d27-screens.txt');
blocks.CONTENT = read('d27-content.txt').trim();
...
cut('// s2 — BELGI', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
cut("// s4 — RASM BO'YICHA BELGI", '// s5 — ', screens.S4 + '\n\n', 'экран s4');
...
s = head.split('DialFig').join('WholeFig').split('d26').join('d27') + tail;

if (!s.includes('export default function CompareSharesLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function CompareSharesLesson({', 'export default function ShareOfNumberLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d25-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d26-').join('d27-');
const left = (keep.match(/\.d2[56]-/g) || []).length;
...
for (const dead of ['d26', 'DialFig', 'CompareSharesLesson', 'SharePair']) {

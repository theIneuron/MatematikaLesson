// build-grade3-dars30.mjs — собирает Dars30.jsx из донора Dars29.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars30.jsx`.
// Запуск: node scripts/build-grade3-dars30.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars29.jsx');
const OUT = path.resolve('src/components/grade3/Dars30.jsx');
const BLK = path.resolve(process.env.D30_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d30-blocks.txt');
const screens = sections('d30-screens.txt');
blocks.CONTENT = read('d30-content.txt').trim();
...
cut('// s2 — IKKI TASMA', '// s3 — ', screens.S2 + '\n\n', 'экран s2');
...
s = head.split('HalfFig').join('FruitFig').split('d29').join('d30') + tail;

if (!s.includes('export default function CompareFracLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function CompareFracLesson({', 'export default function AddFracLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d28-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d29-').join('d30-');
const left = (keep.match(/\.d2[89]-/g) || []).length;
...
for (const dead of ['d29', 'HalfFig', 'CompareFracLesson']) {

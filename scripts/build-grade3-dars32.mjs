// build-grade3-dars32.mjs — собирает Dars32.jsx из донора Dars31.jsx (этап 3, сборка).
//
...
//
// Одноразовый генератор: после сборки источник правды — сам `src/components/grade3/Dars32.jsx`.
// Запуск: node scripts/build-grade3-dars32.mjs
import fs from 'node:fs';
import path from 'node:path';

const SRC = path.resolve('src/components/grade3/Dars31.jsx');
const OUT = path.resolve('src/components/grade3/Dars32.jsx');
const BLK = path.resolve(process.env.D32_BLOCKS || 'C:/Users/USER/AppData/Local/Temp/claude/c--Users-USER-MatematikaLesson/f093dee5-b110-48ec-8639-7805bc86f15f/scratchpad');
...
const blocks = sections('d32-blocks.txt');
const screens = sections('d32-screens.txt');
blocks.CONTENT = read('d32-content.txt').trim();
...
cut("// s2 — MODEL: tasma o'nga", '// s3 — ', screens.S2 + '\n\n', 'экран s2');
...
s = head.split('ScaleFig').join('CheckLoopFig').split('d31').join('d32') + tail;

if (!s.includes('export default function DecimalLesson({')) throw new Error('корневой компонент не найден');
s = s.replace('export default function DecimalLesson({', 'export default function ShareTasksLesson({');
...
const keep = donorTail.replace(/(^|\n)([^\n{}]*\{[^}]*\})/g, (m, pre, rule) => {
  ...
  return /\.d30-/.test(sel) ? pre.replace(/\n$/, '') : m;
})
  .replace(/(^|\n)@media[^\n]*\.d24-[^\n]*/g, (m, pre) => pre.replace(/\n$/, ''))
  .split('d31-').join('d32-');
const left = (keep.match(/\.d(30|31)-/g) || []).length;
...
for (const dead of ['d31', 'ScaleFig', 'DecimalLesson']) {

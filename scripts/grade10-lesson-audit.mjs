...
function checkStrings(screens) {
  screens.forEach((s, idx) => {
    ...
    walkL(s, (l, trail) => {
      ...
      // Озвучка: строки из массива audio.
      if (audioIds.has(l)) {
        for (const k of LANGS) {
          const m = (l[k] || '').match(BAD_AUDIO)
          if (m) bad(`экран ${n}, озвучка ${k.toUpperCase()}: символ «${m[0]}» — надо словами`)
        }
      }
    })
  })
...
function checkProbes(screens) {
  screens.forEach((s, idx) => {
    ...
    const collect = (v, trail) => {
      ...
      if (Array.isArray(v.items) && v.items.some((it) => it && ('correct' in it || 'label' in it))) found.push([trail, v])

...
async function runFirstTryPersistence(browser) {
  const lesson = lessons.find((item) => isNoGateTheory(item) && item.file !== 'Dars51.jsx');
  if (!lesson) return;
  const context = await browser.newContext({ viewport: { width: 1366, height: 768 }, reducedMotion: 'reduce' });
  const page = await context.newPage();
  const diagnostics = monitorPage(page);
  const reachFirstQuestion = async () => {
    for (let expected = 1; expected < 9; expected += 1) {
      const before = await currentScreenCount(page);
      const next = await theoryNextButton(page);
      if (!(await next.isEnabled())) throw new Error('Continue savolsiz bloklangan');
      await next.click();
      await waitForScreenChange(page, before.text);
    }
  };
  try {
    await openLesson(page, lesson, 'en');
    await reachFirstQuestion();
    const options = inLesson(page, '.option');
    const optionCount = await options.count();
    let correctIndex = -1;
    for (let index = 0; index < optionCount; index += 1) {
      await options.nth(index).click();
      await sleep(40);
      if ((await options.nth(index).getAttribute('class'))?.split(/\s+/).includes('right')) {
        correctIndex = index;
        break;
      }
    }
    if (correctIndex < 0) throw new Error('to\'g\'ri variant aniqlanmadi');

    diagnostics.reset();
    await openLesson(page, lesson, 'en');
    await reachFirstQuestion();
    const freshOptions = inLesson(page, '.option');
    const wrongIndex = (correctIndex + 1) % optionCount;
    await freshOptions.nth(correctIndex).click();
    await sleep(40);
    await freshOptions.nth(wrongIndex).click();
    await sleep(40);

    for (let expected = 9; expected < 15; expected += 1) {
      const before = await currentScreenCount(page);
      const next = await theoryNextButton(page);
      if (!(await next.isEnabled())) throw new Error('Continue javobdan keyin bloklangan');
      await next.click();
      await waitForScreenChange(page, before.text);
    }
    ...
    await finish.click();
    await waitForCompletion(diagnostics);
    if (diagnostics.completionCalls.length !== 1) throw new Error('onFinished bir marta chaqirilmadi');
    const payload = diagnostics.completionCalls[0];
    const answer = payload?.answers?.find((item) => item.screenIdx === 8);
    if (answer?.firstTry !== true || answer?.attempts !== 2) {
      throw new Error('birinchi to\'g\'ri urinish keyingi bosishda saqlanmadi');
    }
    if (payload?.correctAnswers !== 1 || payload?.totalQuestions !== 6) {
      throw new Error('firstTry LMS hisobi noto\'g\'ri: ' + JSON.stringify({ correctAnswers: payload?.correctAnswers, totalQuestions: payload?.totalQuestions }));
    }
  } catch (error) {
    failures.push('firstTry persistence ' + lesson.file + ': ' + error.message);
  } finally {
    await context.close();
  }
}
...
try {
  ...
  try {
    ...
    await runFirstTryPersistence(browser);

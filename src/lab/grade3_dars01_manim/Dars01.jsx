...
import manimVideo from './YuzlikOnlikBirlik.mp4';   // LAB: Manim video-recap (QOIDA'dan keyin)
...
const TOTAL_SCREENS = 16;   // LAB: QOIDA'dan keyin Manim video-ekrani qo'shildi (+1)
const LESSON_META = {
...
const SCREEN_META = [
  ...
  { id: 'sManim', type: 'exploration', template: 'video', scored: false, scope: null },        // LAB: QOIDA'dan keyin Manim video-recap
...
// ============================================================
// ============================================================
// LAB: Manim video-recap ekrani (QOIDA'dan keyin) — izolyatsiya tajribasi.
// Passiv klip: kashf qilingan qoidani bitta silliq animatsiyada jamlaydi.
// ============================================================
const ScreenManim = (props) => {
  const t = useT();
  const eyebrow = { ru: 'Разбор (видео)', uz: "Ko'rib chiqamiz (video)" };
  const lead = { ru: 'Посмотрим всё вместе', uz: "Hammasini birga ko'ramiz" };
  const caption = { ru: 'Сотни, десятки, единицы — место решает.', uz: "Yuzlik, o'nlik, birlik — o'rin hal qiladi." };
  const navContent = (
    <>
      <NavBack onPrev={props.onPrev} label={<BackLabel/>}/>
  ...
  return (
    <Stage eyebrow={eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 16px)', alignItems: 'center', justifyContent: 'center' }}>
        <h1 className="title h-sub fade-up">{t(lead)}</h1>
        <div className="frame fade-up delay-1" style={{ padding: 0, overflow: 'hidden', width: '100%', maxWidth: 720, borderRadius: 18, lineHeight: 0 }}>
          <video src={manimVideo} controls autoPlay muted loop playsInline
            style={{ width: '100%', display: 'block', borderRadius: 18, background: '#0a0e1a' }}/>
        </div>
        <p className="fade-up delay-1" style={{ textAlign: 'center', color: '#5A5A60', fontWeight: 600, fontSize: 'clamp(14px, 1.9vw, 17px)', margin: 0 }}>
          {t(caption)}
        </p>
      </div>
    </Stage>
...
export default function TensUnitsLesson({
  ...
  const screens = [Screen0, Screen1, Screen3, Screen4, Screen5, Screen6, ScreenMing, Screen7, ScreenManim, Screen8, Screen9, Screen10, Screen11, ScreenCase, Screen14, Screen15];

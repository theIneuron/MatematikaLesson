// Dars 9 · Amaliyot 10 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D10_OPTS = { uz: ["O‘nliklar ustuniga", 'Birliklar ustuniga', 'Javob yoniga alohida'], ru: ['В столбец десятков', 'В столбец единиц', 'Отдельно рядом с ответом'] };
const D10_CORRECT = 0;
const D10_T = {
  uz: {
    eyebrow: "Ko‘chuvchi qayerga", setup: 'Birliklar: 9 + 7 = 16. Bitta o‘nlik ko‘chdi.',
    ask: 'Ko‘chuvchi «1» qaysi ustunga qo‘shiladi?',
    correct: "To'g'ri. Ko‘chuvchi 1 — bu bitta o‘nlik, u o‘nliklar ustuniga qo‘shiladi.",
    wrong: "Maslahat: ko‘chuvchi «1» — bu 1 ta o‘nlik. U qaysi razryadga tegishli?",
    rule: "Ko‘chuvchi 1 — bitta o‘nlik. U o‘nliklar ustuniga qo‘shiladi.",
  },
  ru: {
    eyebrow: 'Куда идёт перенос', setup: 'Единицы: 9 + 7 = 16. Один десяток перешёл.',
    ask: 'В какой столбец прибавляется перенос «1»?',
    correct: 'Верно. Перенос 1 — это один десяток, он прибавляется к столбцу десятков.',
    wrong: 'Подсказка: перенос «1» — это 1 десяток. К какому разряду он относится?',
    rule: 'Перенос 1 — один десяток. Он прибавляется к столбцу десятков.',
  },
};
function D09_10Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: opts[picked] }, correctAnswer: { idx: 0, label: opts[0] }, correct, meta: { tag: 'carrywhere', level: '🔴' } });

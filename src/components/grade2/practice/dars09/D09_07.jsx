// Dars 9 · Amaliyot 07 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D07_CHIPS = [65, 55, 56, 515], D07_CORRECT = 0;
const D07_T = {
  uz: {
    eyebrow: 'Masala', setup: 'Mars bazasida ikki konteyner yuk bor.',
    ask: 'Bir konteynerda 38 quti, boshqasida 27 quti. Jami nechta quti?',
    correct: "To'g'ri. 38 + 27 = 65 (8 + 7 = 15 → 5 yozamiz, 1 ko‘chadi; 3 + 2 + 1 = 6).",
    wrong: "Maslahat: «jami» — qo‘shish. Birliklar 8 + 7 o‘ndan oshadi, ko‘chgan o‘nlikni unutmang.",
    rule: "«Jami» — qo‘shish. O‘tishni hisobga oling: 38 + 27 = 65.",
  },
  ru: {
    eyebrow: 'Задача', setup: 'На базе Марса два контейнера с грузом.',
    ask: 'В одном контейнере 38 ящиков, в другом 27. Сколько всего ящиков?',
    correct: 'Верно. 38 + 27 = 65 (8 + 7 = 15 → пишем 5, 1 переносим; 3 + 2 + 1 = 6).',
    wrong: 'Подсказка: «всего» — сложение. Единицы 8 + 7 больше десяти, не забудь перенос.',
    rule: '«Всего» — сложение. Учитывай переход: 38 + 27 = 65.',
  },
};
function D09_07Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D07_CHIPS.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { idx: picked, label: String(D07_CHIPS[picked]) }, correctAnswer: { idx: 0, label: '65' }, correct, meta: { tag: 'wordcarry', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: 32 }}>📦</div><div style={{ ...S.mono, fontSize: 24, fontWeight: 800, color: C.mars }}>38</div></div>
          <div style={{ ...S.mono, fontSize: 26, color: C.sink2 }}>+</div>
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: 32 }}>📦</div><div style={{ ...S.mono, fontSize: 24, fontWeight: 800, color: C.one }}>27</div></div>
          <div style={{ ...S.mono, fontSize: 26, color: C.sink2 }}>=</div>
          <div style={{ ...S.mono, fontSize: 32, fontWeight: 800, color: C.acc }}>?</div>
        </div>
      </Stage>
      <p style={{ ...S.ask, fontSize: 20 }}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {order.map((i) => { const on = picked === i; let tone = null; if (checked && on) tone = i === D07_CORRECT ? 'ok' : 'no'; return <Chip key={i} on={on && !checked} tone={tone} disabled={isReview || checked} onClick={() => setPicked(i)} w={72}>{D07_CHIPS[i]}</Chip>; })}
      </div>

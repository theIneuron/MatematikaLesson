// Dars 9 · Amaliyot 01 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const Cassette = ({ s = 26, dim, lit = true }) => (
  <svg width={s} height={s * 66 / 48} viewBox="0 0 48 66" aria-hidden="true" style={{ display: 'inline-block', verticalAlign: 'middle', opacity: dim ? 0.28 : 1, transition: 'opacity .2s' }}>
    <defs><linearGradient id="d9cass" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#4E5E82" /><stop offset="52%" stopColor="#8C9EC4" /><stop offset="100%" stopColor="#4E5E82" /></linearGradient></defs>
    <rect x="1" y="4" width="46" height="61" rx="7" fill="url(#d9cass)" stroke="#33415F" strokeWidth="1.4" />
...
const Battery = ({ s = 18, dim }) => (
  <svg width={s * 0.66} height={s} viewBox="0 0 22 34" aria-hidden="true" style={{ display: 'inline-block', verticalAlign: 'middle', opacity: dim ? 0.28 : 1, transition: 'opacity .2s' }}>
    <defs><linearGradient id="d9batt" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#0E6E96" /><stop offset="50%" stopColor="#8FE0F4" /><stop offset="100%" stopColor="#0A5876" /></linearGradient></defs>
    <rect x="8" y="0.6" width="6" height="3.4" rx="1.4" fill="#8FA0AE" /><rect x="1.4" y="4" width="19.2" height="29.4" rx="4.2" fill="url(#d9batt)" stroke="#093F55" strokeWidth="1" />
...
// 7 + 5 = 12 batareya. 10 tasini bosib kassetaga o'tkazish. Qoldi 2.
const D01_T = {
  uz: {
    eyebrow: "O'tkazing", setup: 'Birliklar to‘plandi: 7 + 5 = 12 batareya.',
    ask: '12 batareyadan 10 tasini bosib bitta kassetaga o‘tkazing.',
    moved: 'Kassetaga', left: 'Qoldi',
    correct: "To'g'ri. 10 batareya = 1 kasseta (o‘nlik) ko‘chdi, 2 batareya qoldi.",
    wrong: "Maslahat: aynan 10 batareyani bosing — ular bitta o‘nlikka (kasseta) birlashadi.",
    rule: "10 birlik = 1 o‘nlik. Birliklar 10 ga yetsa, 1 o‘nlik ko‘chadi.",
  },
  ru: {
    eyebrow: 'Перенеси', setup: 'Единицы собрались: 7 + 5 = 12 батареек.',
    ask: 'Нажми 10 из 12 батареек — перенеси их в одну кассету.',
    moved: 'В кассету', left: 'Осталось',
    correct: 'Верно. 10 батареек = 1 кассета (десяток) перешла, осталось 2 батарейки.',
    wrong: 'Подсказка: нажми ровно 10 батареек — они объединятся в один десяток (кассету).',
    rule: '10 единиц = 1 десяток. Если единиц 10 или больше, один десяток переходит.',
  },
};
function D09_01Impl(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D01_T[lang] || D01_T.uz;
  const isReview = mode === 'review';
  const [moved, setMoved] = useState([]); // battery indices moved into cassette
  const [fb, setFb] = useState(null);
  const [checked, setChecked] = useState(false);
  useEffect(() => { const sa = initialAnswer?.studentAnswer; if (sa?.moved) { setMoved(sa.moved); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); } } }, [initialAnswer]);
  const touched = moved.length > 0;
  useEffect(() => { onReady?.(touched && !checked); }, [touched, checked, onReady]);
  const locked = isReview || checked;
  const tap = (i) => { if (locked) return; setMoved((m) => m.includes(i) ? m.filter((x) => x !== i) : [...m, i]); };
  const bundled = moved.length >= 10;
  const check = useCallback(() => {
    const correct = moved.length === 10;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { moved, count: moved.length }, correctAnswer: { moved: 10, left: 2 }, correct, meta: { tag: 'bundle', level: '🟢' } });
  }, [moved, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const leftCol = checked ? (fb?.correct ? C.ok : C.no) : C.mars;
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 18, justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, minWidth: 74 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 800, color: bundled ? '#9df0bd' : C.sink2 }}>{bundled ? '10 = 1 o‘nlik' : t.moved + ': ' + moved.length}</span>
          </div>
        </div>

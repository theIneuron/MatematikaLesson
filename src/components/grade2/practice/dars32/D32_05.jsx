// Dars 32 · Amaliyot 05 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 32;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN SCENE — ulushlar ============================== */
function sector(cx, cy, r, a0, a1) { const p=(a)=>[cx+r*Math.cos(a), cy+r*Math.sin(a)]; const [x0,y0]=p(a0),[x1,y1]=p(a1); const big=(a1-a0)>Math.PI?1:0; return `M ${cx} ${cy} L ${x0} ${y0} A ${r} ${r} 0 ${big} 1 ${x1} ${y1} Z`; }
// Ulush rasmi (variantlar uchun) — doira n bo'lak, ba'zilari bo'yalgan
const FracFig = ({ parts, shaded, r=40, box=100 }) => {
  const cx=box/2, cy=box/2;
  return (<svg width={box} height={box} viewBox={`0 0 ${box} ${box}`}>
    {Array.from({length:parts}).map((_,i)=>{ const a0=-Math.PI/2+i*2*Math.PI/parts, a1=-Math.PI/2+(i+1)*2*Math.PI/parts;
      return <path key={i} d={sector(cx,cy,r,a0,a1)} fill={shaded.includes(i)?'#FFC23C':'rgba(255,255,255,.06)'} stroke="#0b1030" strokeWidth="2" />; })}
    <circle cx={cx} cy={cy} r={r} fill="none" stroke="#FFC23C" strokeWidth="2.5" />
  </svg>);
};

/* ---- IMZO: ULUSH-BO'YASH (ShadeFraction) ---- */
function makeShade(cfg) {
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review'; const n=cfg.parts;
    const [sh,setSh]=useState([]),[fb,setFb]=useState(null),[checked,setChecked]=useState(false);
    useEffect(()=>{ const sa=initialAnswer?.studentAnswer; if(sa?.sh){ setSh(sa.sh); if(typeof initialAnswer.correct==='boolean'){ setFb({correct:initialAnswer.correct}); setChecked(true); } } },[initialAnswer]);
    useEffect(()=>{ onReady?.(sh.length>0 && !checked); },[sh,checked,onReady]);
    const toggle=(i)=>{ if(isReview||checked) return; setSh(s=>s.includes(i)?s.filter(x=>x!==i):[...s,i]); };
    const check=useCallback(()=>{ const correct=sh.length===cfg.need; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:[], studentAnswer:{sh,count:sh.length}, correctAnswer:{count:cfg.need}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[sh,t,playCorrect,playWrong,onSubmit]);
    const cref=useRef(check); cref.current=check; useEffect(()=>{ registerCheck?.(()=>cref.current()); },[registerCheck]);
    const col = checked?(fb?.correct?C.ok:C.no):'#FFC23C';
    const box=200, cx=box/2, cy=100, r=78;
    return (
      <div style={S.wrap}>
        <div style={S.eyebrow}>{t.eyebrow}</div>
        <p style={S.setup}>{t.setup}</p>
        ...
        <Stage>
          {cfg.shape==='bar' ? (
            <svg width="100%" viewBox={`0 0 ${box} 90`} style={{maxWidth:box,display:'block',margin:'0 auto'}}>
              {Array.from({length:n}).map((_,i)=>(<rect key={i} x={20+i*(box-40)/n} y={26} width={(box-40)/n-4} height={44} rx={5} fill={sh.includes(i)?col:'rgba(255,255,255,.06)'} stroke="#FFC23C" strokeWidth="2.5" style={{cursor:(isReview||checked)?'default':'pointer'}} onClick={()=>toggle(i)} />))}
            </svg>
          ) : (
            <svg width="100%" viewBox={`0 0 ${box} 200`} style={{maxWidth:box,display:'block',margin:'0 auto'}}>
              {Array.from({length:n}).map((_,i)=>{ const a0=-Math.PI/2+i*2*Math.PI/n, a1=-Math.PI/2+(i+1)*2*Math.PI/n;
                return <path key={i} d={sector(cx,cy,r,a0,a1)} fill={sh.includes(i)?col:'rgba(255,255,255,.06)'} stroke="#0b1030" strokeWidth="2.5" style={{cursor:(isReview||checked)?'default':'pointer'}} onClick={()=>toggle(i)} />; })}
              <circle cx={cx} cy={cy} r={r} fill="none" stroke="#FFC23C" strokeWidth="3" />
            </svg>
          )}
          <div style={{ textAlign:'center', marginTop:4, fontSize:13.5, fontWeight:700, color:C.goldSoft }}>{t.hint}: {sh.length} / {n}</div>
        </Stage>
        {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
        {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
      </div>
...
/* ===== 05 · TrueFalse · teng bo'lmagan ===== */
const D05 = makeTrueFalse({ tag:'tf_equal', level:'🟡', answer:false, stage:(t)=>(<div style={{display:'flex',justifyContent:'center'}}><svg width="120" height="80" viewBox="0 0 120 80"><rect x="10" y="20" width="30" height="40" fill="rgba(255,255,255,.06)" stroke="#FFC23C" strokeWidth="2.5"/><rect x="40" y="20" width="70" height="40" fill="rgba(255,255,255,.06)" stroke="#FFC23C" strokeWidth="2.5"/></svg></div>) });
D05.T = {
  uz: { eyebrow:"To'g'ri yoki noto'g'ri", setup:'Shakl ikki bo‘lakka bo‘lingan, lekin bo‘laklar har xil kattalikda.', ask:'Bu shakl teng bo‘laklarga bo‘linganmi? (bo‘laklarning kattaligiga qarang)', yes:'Ha', no:'Yo‘q',
    correct:"To'g'ri javob: Yo‘q. Bo‘laklar har xil — ular teng emas.", wrong:'Ikkala bo‘lak bir xil kattalikdami? Biri kattaroq ko‘rinadi.', rule:'Ulush uchun bo‘laklar TENG bo‘lishi kerak.' },
  ru: { eyebrow:'Верно или неверно', setup:'Фигура разделена на две части, но части разного размера.', ask:'Разделена ли фигура на равные части? (посмотри на размер частей)', yes:'Да', no:'Нет',
    correct:'Правильно: Нет. Части разные — они не равны.', wrong:'Одинакового ли размера обе части? Одна больше.', rule:'Для доли части должны быть РАВНЫМИ.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_32_05(props) {
  return (
    <React.Fragment>

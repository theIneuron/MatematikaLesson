// Dars 12 · Amaliyot 06 — mustaqil jsx-question (per-task split).
...
const Box = ({ val, checked }) => (
  <span style={{ minWidth:52, height:48, border:'2px dashed '+(checked?C.ok:C.gold), borderRadius:10, display:'inline-flex', alignItems:'center', justifyContent:'center', color:C.gold, background:C.stile }}>{val!=null&&val!==''?val:'⬜'}</span>
);
// Bir qatorli zanjir: a op1 b op2 ⬜ = r  (yashirin qo'shiluvchi/kamayuvchi)
const MissOp = ({ a, op1, b, op2, r, val, checked }) => (
  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:9, ...S.mono, fontSize:28, fontWeight:800, color:C.sink, flexWrap:'wrap' }}>
    <span>{a}</span><span style={{ color:C.gold }}>{op1}</span><span>{b}</span>
    <span style={{ color:C.gold }}>{op2}</span><Box val={val} checked={checked} /><span style={{ color:C.gold }}>=</span><span>{r}</span>
  </div>
);
/* ===== 06 · missing_add · numpad · missing_add · YASHIRIN QO'SHILUVCHI (yangi tip) ===== */
const D06 = makeNumpad({ tag:'missing_add', level:'🟡', answer:10, max:2,
  stage:(t,st)=>(<div><MissOp a={50} op1="−" b={20} op2="+" r={40} val={st.val} checked={st.checked} /><CargoCap text={t.scene} /></div>) });

D06.T = {
  uz: { eyebrow:"Yashirin qo'shiluvchi", setup:"Zanjirdagi ikkinchi qo'shiluvchi yashiringan.", scene:'50 − 20 + ⬜ = 40', ask:'Yashirin sonni toping: 50 − 20 + ⬜ = 40.',
    correct:"To'g'ri. 50 − 20 = 30; 30 + 10 = 40.", wrong:"Avval 50 − 20 = 30 ni toping. 30 ga qancha qo'shsak 40 bo'ladi?", rule:'50 − 20 = 30; 30 + 10 = 40.' },
  ru: { eyebrow:'Скрытое слагаемое', setup:'Второе слагаемое в цепочке скрыто.', scene:'50 − 20 + ⬜ = 40', ask:'Найди скрытое число: 50 − 20 + ⬜ = 40.',
    correct:'Верно. 50 − 20 = 30; 30 + 10 = 40.', wrong:'Сначала найди 50 − 20 = 30. Сколько прибавить к 30, чтобы вышло 40?', rule:'50 − 20 = 30; 30 + 10 = 40.' },
};

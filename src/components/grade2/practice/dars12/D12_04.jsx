// Dars 12 · Amaliyot 04 — mustaqil jsx-question (per-task split).
...
/* ===== 04 · pickexpr · MC (full) · pickexpr · IFODANI TANLASH (yangi tip) ===== */
const D04 = makeMC({ tag:'pickexpr', level:'🟡', correctIdx:0, half:false, fs:19, stage:marsMC });

D04.T = {
  uz: { eyebrow:'Mos ifodani tanlash', setup:'Kemada 30 quti bor edi. 25 tasi ortildi, keyin 15 tasi tushirildi.', scene:'Mars bazasi · ifoda', ask:'Qaysi ifoda masalaga mos keladi?', opts:['30 + 25 − 15','30 − 25 + 15','30 + 25 + 15','30 − 25 − 15'],
    correct:"To'g'ri. Ortildi — qo'shish (+25), tushirildi — ayirish (−15): 30 + 25 − 15.", wrong:"Har o'zgarishni belgiga aylantiring: ortildi — qo'shish, tushirildi — ayirish.", rule:'30 + 25 − 15 (ortildi +, tushirildi −).' },
  ru: { eyebrow:'Выбери выражение', setup:'На корабле было 30 ящиков. 25 загрузили, затем 15 выгрузили.', scene:'База Марс · выражение', ask:'Какое выражение подходит к задаче?', opts:['30 + 25 − 15','30 − 25 + 15','30 + 25 + 15','30 − 25 − 15'],
    correct:'Верно. Загрузили — плюс (+25), выгрузили — минус (−15): 30 + 25 − 15.', wrong:'Переведи каждое изменение в знак: загрузили — плюс, выгрузили — минус.', rule:'30 + 25 − 15 (загрузили +, выгрузили −).' },
};

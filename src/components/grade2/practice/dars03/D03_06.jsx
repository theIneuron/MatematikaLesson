// Dars 3 · Amaliyot 06 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D06_BANK = [40, 9, 49, 90, 4];
const D06_T = {
  uz: {
    eyebrow: 'Tenglama yig‘ing', setup: 'Razryadlardan to‘liq tenglama tuzamiz.',
    ask: '40 va 9 dan kodni yig‘ing: ⬜ + ⬜ = ⬜.',
    correct: "To'g'ri. 40 + 9 = 49. O'nliklar 40, birliklar 9.",
    wrong: "Maslahat: 40 (o'nliklar) + 9 (birliklar) = 49. 90 — o'rin almashgan.",
    rule: "40 + 9 = 49. Razryad qo'shiluvchilaridan son yig'iladi.",
  },
  ru: {
    eyebrow: 'Собери равенство', setup: 'Строим полное равенство из разрядов.',
    ask: 'Собери код из 40 и 9: ⬜ + ⬜ = ⬜.',
    correct: 'Верно. 40 + 9 = 49. Десятки 40, единицы 9.',
    wrong: 'Подсказка: 40 (десятки) + 9 (единицы) = 49. 90 — перестановка.',
    rule: '40 + 9 = 49. Число собирается из разрядных слагаемых.',
  },
};
function D03_06Impl(props) {
  ...
  const clickSlot = (i) => () => {
    if (locked) return;
    if (pick != null) { setSlots((s) => { const n = s.slice(); n[i] = { v: D06_BANK[pick], bi: pick }; return n; }); setPick(null); }
    else if (slots[i] != null) { setSlots((s) => { const n = s.slice(); n[i] = null; return n; }); }
  };
  const check = useCallback(() => {
    const a = slots[0] && slots[0].v, b = slots[1] && slots[1].v, c = slots[2] && slots[2].v;
    const correct = c === 49 && ((a === 40 && b === 9) || (a === 9 && b === 40));
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { slots }, correctAnswer: { eq: '40+9=49' }, correct, meta: { tag: 'eqnbuild', level: '🟡' } });
  ...
  const slot = (i) => {
    ...
    return <span onClick={clickSlot(i)} style={{ minWidth: 60, height: 58, borderRadius: 12, border: '2px ' + (v != null ? 'solid' : 'dashed') + ' ' + bd, background: C.stile, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', ...S.mono, fontSize: 26, fontWeight: 800, color: col, cursor: locked ? 'default' : 'pointer' }}>{v != null ? v : '⬜'}</span>;
  };
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, ...S.mono, fontSize: 26, fontWeight: 800, color: C.sink2, flexWrap: 'wrap' }}>
          {slot(0)}<span>+</span>{slot(1)}<span>=</span>{slot(2)}
        </div>
      </Stage>
      <p style={{ ...S.ask, fontSize: 21 }}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D06_BANK.map((n, i) => usedSet.has(i)

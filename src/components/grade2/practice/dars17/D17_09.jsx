// Dars 17 · Amaliyot 09 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D09_T = {
  uz: {
    eyebrow: 'Masala', setup: "Har antennada 8 kanal bor. Bit 6 ta antennani ishga tushirdi.",
    ask: 'Jami nechta kanal ushlanadi?',
    opts: ['14', '42', '48', '54'],
    correct: "To'g'ri. Har antennada 8 kanal, 6 antenna: 8 × 6 = 48.",
    wrong: "Maslahat: 8 ni 6 marta oling. 8,16,24,32,40,48. 8+6=14 — bu qo'shish.",
    rule: "«Har birida A, jami B ta» — bu A × B ko'paytma.",
  },
  ru: {
    eyebrow: 'Задача', setup: 'В каждой антенне 8 каналов. Бит включил 6 антенн.',
    ask: 'Сколько каналов держится всего?',
    opts: ['14', '42', '48', '54'],
    correct: 'Верно. По 8 каналов в 6 антеннах: 8 × 6 = 48.',
    wrong: 'Подсказка: возьми 8 шесть раз. 8,16,24,32,40,48. 8+6=14 — это сложение.',
    rule: '«По A в каждом, всего B» — это произведение A × B.',
  },
};
function D17_09Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 2, label: '48' }, correct, meta: { tag: 'mul_word89', level: '🔴' } });
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap' }}>
          {Array.from({ length: 6 }).map((_, g) => (
            <div key={g} style={{ padding: 5, borderRadius: 9, border: '1.5px solid ' + C.sig, background: 'rgba(91,214,242,.10)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 3 }}>{Array.from({ length: 8 }).map((_, i) => <Dish key={i} s={9} delay={(g * 8 + i) * 0.02} />)}</div>
            </div>
          ))}
        </div>
      </Stage>

// Dars 17 · Amaliyot 06 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D06_T = {
  uz: {
    eyebrow: 'To‘qqiz sirri', setup: "Sir: ×9 da javobning raqamlari yig'indisi doim 9 bo'ladi.",
    ask: '9 × 4 nechaga teng? (Raqamlar yig‘indisi 9 bo‘lganini toping.)',
    opts: ['36', '34', '45', '32'],
    correct: "To'g'ri. 9 × 4 = 36, va 3 + 6 = 9. Sir ishladi!",
    wrong: "Maslahat: 9, 18, 27, 36 deb sanang. Raqamlari yig'indisi 9 bo'lsin: 3+6=9.",
    rule: "×9 sirri: javob raqamlari yig'indisi 9 (36→3+6=9, 45→4+5=9).",
  },
  ru: {
    eyebrow: 'Секрет девятки', setup: 'Секрет: у ×9 сумма цифр ответа всегда равна 9.',
    ask: 'Чему равно 9 × 4? (Найди, где сумма цифр равна 9.)',
    opts: ['36', '34', '45', '32'],
    correct: 'Верно. 9 × 4 = 36, и 3 + 6 = 9. Секрет сработал!',
    wrong: 'Подсказка: считай 9, 18, 27, 36. Сумма цифр должна быть 9: 3+6=9.',
    rule: 'Секрет ×9: сумма цифр ответа равна 9 (36→3+6=9, 45→4+5=9).',
  },
};
function D17_06Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '36' }, correct, meta: { tag: 'nine_pattern', level: '🟡' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div className="d17-float" style={{ textAlign: 'center', ...S.mono, fontSize: 40, fontWeight: 800, color: C.gold }}>9 × 4</div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D06_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

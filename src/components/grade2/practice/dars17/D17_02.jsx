// Dars 17 · Amaliyot 02 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D02_T = {
  uz: {
    eyebrow: '×8 jadvali', setup: "Har antennada 8 kanal. Bit 5 ta antennani yoqdi.",
    ask: '8 × 5 nechaga teng?', expr: '8 × 5',
    opts: ['13', '32', '40', '48'],
    correct: "To'g'ri. 8 × 5 = 8+8+8+8+8 = 40.",
    wrong: "Maslahat: 8 ni 5 marta oling: 8, 16, 24, 32, 40. 8+5=13 — bu qo'shish.",
    rule: "8 × 5 — 8 ni 5 marta olish, 8+5 qo'shish emas.",
  },
  ru: {
    eyebrow: 'Таблица ×8', setup: 'В каждой антенне 8 каналов. Бит включил 5 антенн.',
    ask: 'Чему равно 8 × 5?', expr: '8 × 5',
    opts: ['13', '32', '40', '48'],
    correct: 'Верно. 8 × 5 = 8+8+8+8+8 = 40.',
    wrong: 'Подсказка: возьми 8 пять раз: 8, 16, 24, 32, 40. 8+5=13 — это сложение.',
    rule: '8 × 5 — это 8, взятое 5 раз, а не 8+5.',
  },
};
function D17_02Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 2, label: '40' }, correct, meta: { tag: 'mul8_mc', level: '🟢' } });
      ...
      <Stage>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'center' }}>
          {Array.from({ length: 5 }).map((_, r) => (
            <div key={r} style={{ padding: '4px 8px', borderRadius: 9, border: '1.5px solid ' + C.sig, background: 'rgba(91,214,242,.10)' }}><div style={{ display: 'flex', gap: 5 }}>{Array.from({ length: 8 }).map((_, c) => <Dish key={c} s={12} delay={(r * 8 + c) * 0.03} />)}</div></div>
          ))}
        </div>
        <div className="d17-float" style={{ textAlign: 'center', marginTop: 8, ...S.mono, fontSize: 34, fontWeight: 800, color: C.gold }}>{t.expr}</div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {t.opts.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D02_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o}</button>)}
      </div>
...
export default function D17_02(props) {
  return (<><style>{FX_CSS}</style><D17_02Impl {...props} /></>);
}

/* v-audio (auto) — TTS-toza narratsiya (ovoz). UZ = draft, uz-metodist validatsiyasi kerak. */
D17_02.audio = {
  uz: { intro: "Har antennada 8 kanal. Bit 5 ta antennani yoqdi. 8 ko'paytirish 5 nechaga teng?", on_correct: "To'g'ri. 8 ko'paytirish 5 teng 8 qo'shish 8 qo'shish 8 qo'shish 8 qo'shish 8 teng 40.", on_wrong: "Maslahat. 8 ni 5 marta oling. 8, 16, 24, 32, 40. 8 qo'shish 5 teng 13, bu qo'shish." },
  ru: { intro: "В каждой антенне 8 каналов. Бит включил 5 антенн. Чему равно 8 умножить на 5?", on_correct: "Верно. 8 умножить на 5 равно 8 плюс 8 плюс 8 плюс 8 плюс 8 равно 40.", on_wrong: "Подсказка. Возьми 8 пять раз. 8, 16, 24, 32, 40. 8 плюс 5 равно 13, это сложение." },
};

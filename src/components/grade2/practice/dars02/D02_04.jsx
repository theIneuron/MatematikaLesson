// Dars 2 · Amaliyot 04 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D04_T = {
  uz: {
    eyebrow: 'Bo‘sh katak', setup: 'Kodni razryadlarga yoyamiz: 45 = 40 + ?',
    ask: '45 = 40 + ⬜. Bo‘sh katakka qaysi son tushadi?',
    opts: ['5', '40', "to'qqiz", '50'],
    correct: "To'g'ri. 45 = 40 + 5. 40 — o'nliklar, 5 — birliklar.",
    wrong: "Maslahat: 45 ning birlik raqami nechta? 40 ga qancha qo'shsak 45 bo'ladi?",
    rule: "Ikki xonali sonni yoyish: 45 = 40 + 5 (o'nliklar + birliklar).",
  },
  ru: {
    eyebrow: 'Пустая клетка', setup: 'Раскладываем код по разрядам: 45 = 40 + ?',
    ask: '45 = 40 + ⬜. Какое число в пустой клетке?',
    opts: ['5', '40', 'девять', '50'],
    correct: 'Верно. 45 = 40 + 5. 40 — десятки, 5 — единицы.',
    wrong: 'Подсказка: сколько единиц в 45? Сколько прибавить к 40, чтобы вышло 45?',
    rule: 'Разложение двузначного: 45 = 40 + 5 (десятки + единицы).',
  },
};
function D02_04Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '5' }, correct, meta: { tag: 'expand_blank', level: '🟡' } });
  }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const boxVal = picked != null ? t.opts[picked] : '⬜';
  const boxCol = checked ? (fb?.correct ? C.one : '#FF9A82') : (picked != null ? C.one : C.sink2);
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div>
      <p style={S.setup}>{t.setup}</p>
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, ...S.mono, fontSize: 34, fontWeight: 800, color: C.sink, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 44, color: C.ten }}>45</span><span style={{ color: C.sink2 }}>=</span>
          <span style={{ color: C.ten }}>40</span><span style={{ color: C.sink2 }}>+</span>
          <span className={picked == null && !checked ? 'd02-pulse' : undefined} style={{ minWidth: 52, height: 58, border: '2px dashed ' + boxCol, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: boxCol, background: C.stile, fontSize: 30 }}>{boxVal}</span>
        </div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D04_CORRECT, checked, isReview, { half: true, center: true, fs: 20 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

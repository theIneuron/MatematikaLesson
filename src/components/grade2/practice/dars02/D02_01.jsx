// Dars 2 · Amaliyot 01 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D01_T = {
  uz: {
    eyebrow: 'Kodni o‘qing', setup: 'Kema displeyida 45 kodi yonyapti.',
    ask: '45 kodi qanday o‘qiladi?',
    opts: ['qirq besh', 'besh qirq', "to'qqiz", 'qirq'],
    correct: "To'g'ri. 45 = 4 o'nlik va 5 birlik — «qirq besh».",
    wrong: "Maslahat: chap raqam (4) — o'nliklar, o'ng (5) — birliklar. Avval o'nlikni o'qiymiz.",
    rule: "Ikki xonali son: avval o'nlik, keyin birlik o'qiladi. 45 = qirq besh.",
  },
  ru: {
    eyebrow: 'Прочитай код', setup: 'На дисплее корабля светится код 45.',
    ask: 'Как читается код 45?',
    opts: ['сорок пять', 'пять сорок', 'девять', 'сорок'],
    correct: 'Верно. 45 = 4 десятка и 5 единиц — «сорок пять».',
    wrong: 'Подсказка: левая цифра (4) — десятки, правая (5) — единицы. Сначала читаем десятки.',
    rule: 'Двузначное число: сначала десятки, потом единицы. 45 = сорок пять.',
  },
};
function D02_01Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: t.opts[0] }, correct, meta: { tag: 'read_mc', level: '🟢' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: C.sink2, letterSpacing: '.1em', marginBottom: 4 }}>KOD</div>
          <div className="d02-float" style={{ ...S.mono, fontSize: 56, fontWeight: 800, letterSpacing: 6 }}>
            <span style={{ color: C.ten }}>4</span><span style={{ color: C.one }}>5</span>
          </div>
        </div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D01_CORRECT, checked, isReview, { half: true, center: true, fs: 18 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

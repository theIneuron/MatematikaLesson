// Dars 34 · Amaliyot 09 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 09 · Story MC · Payshanbadan oldin ===== */
const D09 = makeMC({ tag:'story_day', level:'🔴', correctIdx:0, half:true, fs:17, mono:false });
D09.T = {
  uz: { eyebrow:'Masala', setup:'Jasurning tug‘ilgan kuni Payshanbaga to‘g‘ri keladi.', ask:'Jasurning tug‘ilgan kunidan bir kun OLDIN haftaning qaysi kuni edi?', opts:['Chorshanba','Juma','Seshanba','Shanba'],
    correct:"To'g'ri. Payshanbadan bir kun oldin — Chorshanba.", wrong:'Payshanbadan ORQAGA bir qadam: undan oldin qaysi kun keladi?', rule:'Payshanbadan oldin — Chorshanba.' },
  ru: { eyebrow:'Задача', setup:'День рождения Джасура приходится на четверг.', ask:'Какой день недели был за день ДО дня рождения Джасура?', opts:['Среда','Пятница','Вторник','Суббота'],
    correct:'Верно. За день до четверга — среда.', wrong:'Шаг НАЗАД от четверга: какой день перед ним?', rule:'Перед четвергом — среда.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_34_09(props) {
  return (
    <React.Fragment>

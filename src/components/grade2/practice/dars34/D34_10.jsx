// Dars 34 · Amaliyot 10 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 10 · Order · oylar ===== */
const D10 = makeOrder({ tag:'order_months', level:'🔴' });
D10.T = {
  uz: { eyebrow:'Tartiblash', setup:'To‘rt oy aralashib ketgan.', ask:'Oylarni yil bo‘yicha to‘g‘ri tartibda bosing (1, 2, 3, 4):',
    cards:[{ txt:'Mart', pos:2 }, { txt:'Yanvar', pos:0 }, { txt:'Aprel', pos:3 }, { txt:'Fevral', pos:1 }],
    correct:"To'g'ri. Yanvar, Fevral, Mart, Aprel.", wrong:'Yilning birinchi oyi — Yanvar. Keyin tartib bilan davom eting.', rule:'Yanvar → Fevral → Mart → Aprel.' },
  ru: { eyebrow:'Упорядочи', setup:'Четыре месяца перемешаны.', ask:'Нажми месяцы по порядку года (1, 2, 3, 4):',
    cards:[{ txt:'Март', pos:2 }, { txt:'Январь', pos:0 }, { txt:'Апрель', pos:3 }, { txt:'Февраль', pos:1 }],
    correct:'Верно. Январь, февраль, март, апрель.', wrong:'Первый месяц — январь. Дальше по порядку.', rule:'Январь → февраль → март → апрель.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_34_10(props) {
  return (
    <React.Fragment>

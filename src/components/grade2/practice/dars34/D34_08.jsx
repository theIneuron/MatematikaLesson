// Dars 34 · Amaliyot 08 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 08 · MC · Chorshanba +2 ===== */
const D08 = makeMC({ tag:'wed_plus2', level:'🔴', correctIdx:0, half:true, fs:17, mono:false });
D08.T = {
  uz: { eyebrow:'Kun hisobi', setup:'Bugun Chorshanba kuni.', ask:'Bugun Chorshanba. Oradan 2 kun o‘tgach, haftaning qaysi kuni bo‘ladi?', opts:['Juma','Payshanba','Shanba','Seshanba'],
    correct:"To'g'ri. Chorshanba → Payshanba → Juma. 2 kundan keyin Juma.", wrong:'Chorshanbadan sanang: birinchi kun Payshanba, ikkinchi kun qaysi?', rule:'Chorshanba + 2 kun = Juma.' },
  ru: { eyebrow:'Счёт дней', setup:'Сегодня среда.', ask:'Сегодня среда. Через 2 дня какой будет день недели?', opts:['Пятница','Четверг','Суббота','Вторник'],
    correct:'Верно. Среда → четверг → пятница. Через 2 дня — пятница.', wrong:'Считай от среды: первый день четверг, второй какой?', rule:'Среда + 2 дня = пятница.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_34_08(props) {
  return (
    <React.Fragment>

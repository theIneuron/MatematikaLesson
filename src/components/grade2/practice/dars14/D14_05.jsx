// Dars 14 · Amaliyot 05 — mustaqil jsx-question (per-task split).
...
const MissFactor = ({ k, total, val }) => (
  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:10, ...S.mono, fontSize:34, fontWeight:800, color:C.sink }}>
    <span style={{ minWidth:56, height:56, border:'2px '+(val!=null?'solid '+C.acc:'dashed '+C.gold), borderRadius:12, display:'inline-flex', alignItems:'center', justifyContent:'center', color: val!=null?'#fff':C.gold, background: val!=null?'rgba(255,79,40,.18)':C.stile }}>{val!=null?val:'⬜'}</span>
    <span style={{ color:C.gold }}>×</span><span>{k}</span><span style={{ color:C.gold }}>=</span><span>{total}</span>
  </div>
);
/* ===== 05 · fillblank · MC (bo'shliqni to'ldirish) · fillblank ===== */
const D05 = makeMC({ tag:'fillblank', level:'🟡', correctIdx:0, half:true, fs:24,
  stage:(t,st)=>(<MissFactor k={3} total={18} val={st.picked!=null?t.opts[st.picked]:null} />) });

D05.T = {
  uz: { eyebrow:"Bo'shliqni to'ldirish", setup:"Ko'paytuvchilardan biri yashirin: ⬜ × 3 = 18.", ask:"Bo'sh katakka mos sonni tanlang: ⬜ × 3 = 18.", opts:['6','5','7','4'],
    correct:"To'g'ri. 6 × 3 = 18.", wrong:'3 tadan sanang: 3, 6, 9, 12, 15, … 18 gacha nechta qadam?', rule:"6 × 3 = 18 → bo'sh katak 6." },
  ru: { eyebrow:'Заполни пропуск', setup:'Один из множителей скрыт: ⬜ × 3 = 18.', ask:'Выбери число для пустой клетки: ⬜ × 3 = 18.', opts:['6','5','7','4'],
    correct:'Верно. 6 × 3 = 18.', wrong:'Считай по 3: 3, 6, 9, 12, 15, … Сколько шагов до 18?', rule:'6 × 3 = 18 → пустая клетка 6.' },
};

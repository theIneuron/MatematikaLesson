// Dars 14 · Amaliyot 01 — mustaqil jsx-question (per-task split).
...
/* ============================== JUPITER SCENE (yo'ldosh guruhlari · skip-strip) ============================== */
const Moon = ({ s = 18, delay = 0 }) => (
  <span className="px-drop" style={{ width:s, height:s, borderRadius:'50%', display:'inline-block', background:'radial-gradient(circle at 34% 30%, #eaf2ff, #9fb4dc 60%, #63779f)', boxShadow:'0 0 5px rgba(159,180,220,.6)', animationDelay:delay+'s' }} />
);
// n guruh, har birida k ta yo'ldosh
const GroupsViz = ({ n, k, s = 18 }) => (
  <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
    {Array.from({ length:n }).map((_,gi)=>(
      <div key={gi} style={{ display:'flex', gap:5, padding:'7px 8px', border:'1.5px solid '+C.ring, borderRadius:12, background:'rgba(159,180,220,.12)' }}>
        {Array.from({ length:k }).map((_,i)=><Moon key={i} s={s} delay={(gi*k+i)*0.03} />)}
      </div>
    ))}
  </div>
);
...
/* ===== 01 · skip2 · MC (groups) · skip2 ===== */
const D01 = makeMC({ tag:'skip2', level:'🟢', correctIdx:0, half:true, fs:22, stage:groups(4,2) });

D01.T = {
  uz: { eyebrow:'2 tadan sanash', setup:"4 guruh, har birida 2 tadan yo'ldosh. 2 tadan sanaymiz: 2, 4, 6, …", scene:'4 guruh × 2 tadan', ask:"Jami nechta yo'ldosh? (4 marta 2)", opts:['8','6','10','4'],
    correct:"To'g'ri. 2, 4, 6, 8 — 4 marta 2 = 4 × 2 = 8.", wrong:"2 tadan sanang: 2, 4, 6, … To'rtinchi qadam qayerda? Qo'shmang — sanang.", rule:'4 × 2 = 8 (2 tadan: 2, 4, 6, 8).' },
  ru: { eyebrow:'Счёт по 2', setup:'4 группы по 2 спутника. Считаем по 2: 2, 4, 6, …', scene:'4 группы × по 2', ask:'Сколько всего спутников? (4 раза по 2)', opts:['8','6','10','4'],
    correct:'Верно. 2, 4, 6, 8 — 4 раза по 2 = 4 × 2 = 8.', wrong:'Считай по 2: 2, 4, 6, … Где четвёртый шаг?', rule:'4 × 2 = 8 (по 2: 2, 4, 6, 8).' },
};
...
D14_01.audio = {
  uz: { intro: "4 guruh, har birida 2 tadan yo'ldosh. 2 tadan sanaymiz. 2, 4, 6, va hokazo Jami nechta yo'ldosh? 4 marta 2", on_correct: "To'g'ri. 2, 4, 6, 8, 4 marta 2 teng 4 ko'paytirish 2 teng 8.", on_wrong: "2 tadan sanang. 2, 4, 6, va hokazo To'rtinchi qadam qayerda? Qo'shmang, sanang." },
  ru: { intro: "4 группы по 2 спутника. Считаем по 2. 2, 4, 6, и так далее Сколько всего спутников? 4 раза по 2", on_correct: "Верно. 2, 4, 6, 8, 4 раза по 2 равно 4 умножить на 2 равно 8.", on_wrong: "Считай по 2. 2, 4, 6, и так далее Где четвёртый шаг?" },
};

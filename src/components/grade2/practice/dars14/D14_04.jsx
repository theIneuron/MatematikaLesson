// Dars 14 · Amaliyot 04 — mustaqil jsx-question (per-task split).
...
// Skip-strip: ketma-ket kataklar, bittasi bo'sh. filled/tone bilan javob ko'rinadi.
const SkipStrip = ({ seq, blankIdx, filled, tone }) => (
  <div style={{ display:'flex', gap:6, justifyContent:'center', alignItems:'center', flexWrap:'wrap' }}>
    {seq.map((v,i)=>{
      const isBlank = i===blankIdx;
      const bd = isBlank ? (tone==='ok'?C.ok:tone==='no'?C.no:(filled!=null?C.acc:C.gold)) : C.ring;
      const col = isBlank ? (tone==='ok'?'#9df0bd':tone==='no'?'#ffb4a8':(filled!=null?'#fff':C.gold)) : C.gold;
      return (
        <React.Fragment key={i}>
          <div style={{ minWidth:46, height:46, borderRadius:10, border:'2px '+(isBlank&&filled==null?'dashed':'solid')+' '+bd, background: isBlank?(filled!=null?'rgba(255,79,40,.18)':C.stile):'rgba(255,194,60,.12)', display:'flex', alignItems:'center', justifyContent:'center', ...S.mono, fontSize:20, fontWeight:800, color:col }}>
            {isBlank ? (filled!=null?filled:'⬜') : v}
          </div>
          {i<seq.length-1 && <span style={{ color:C.acc, fontWeight:800, fontSize:15 }}>›</span>}
        </React.Fragment>
      );
    })}
  </div>
);
const stripStage = (seq, blankIdx) => ((t,st)=>(<div><SkipStrip seq={seq} blankIdx={blankIdx} filled={st.picked!=null?t.opts[st.picked]:null} tone={st.checked?(st.fb?.correct?'ok':'no'):null} /><GCap text={t.scene} /></div>));
/* ===== 04 · tablefill3 · MC (strip) · tablefill3 ===== */
const D04 = makeMC({ tag:'tablefill3', level:'🟡', correctIdx:0, half:true, fs:22, stage:stripStage([3,0,9,12,15],1) });

D04.T = {
  uz: { eyebrow:'Jadval · ×3', setup:"3 ga ko'paytirish jadvali qatori. Har katak avvalgidan 3 ko'p.", scene:'3 tadan: 3, ⬜, 9, 12, 15', ask:"Bo'sh katakni to'ping (2 marta 3):", opts:['6','5','9','8'],
    correct:"To'g'ri. 3, 6, 9, 12, 15 — bo'sh katak 6 (2 × 3).", wrong:"Avvalgi katakka 3 qo'shing: 3 dan keyin nechchi?", rule:"×3: 3, 6, 9, 12, 15. Bo'sh — 6." },
  ru: { eyebrow:'Таблица · ×3', setup:'Строка таблицы ×3. Каждая клетка на 3 больше.', scene:'по 3: 3, ⬜, 9, 12, 15', ask:'Заполни пустую клетку (2 раза по 3):', opts:['6','5','9','8'],
    correct:'Верно. 3, 6, 9, 12, 15 — пустая клетка 6 (2 × 3).', wrong:'Прибавь 3 к предыдущей: что после 3?', rule:'×3: 3, 6, 9, 12, 15. Пустая — 6.' },
};
...
D14_04.audio = {
  uz: { intro: "3 ga ko'paytirish jadvali qatori. Har katak avvalgidan 3 ko'p. Bo'sh katakni to'ping 2 marta 3.", on_correct: "To'g'ri. 3, 6, 9, 12, 15, bo'sh katak 6 2 ko'paytirish 3.", on_wrong: "Avvalgi katakka 3 qo'shing. 3 dan keyin nechchi?" },
  ru: { intro: "Строка таблицы умножить на 3. Каждая клетка на 3 больше. Заполни пустую клетку 2 раза по 3.", on_correct: "Верно. 3, 6, 9, 12, 15, пустая клетка 6 2 умножить на 3.", on_wrong: "Прибавь 3 к предыдущей. Что после 3?" },
};

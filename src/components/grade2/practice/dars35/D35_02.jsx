// Dars 35 · Amaliyot 02 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 35;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN SCENE — pul ============================== */
const Coin = ({ v, size=44 }) => {
  const col = v>=500?'#FFC23C':v>=200?'#7CE0A3':v>=100?'#5BD6F2':'#FF9A6B';
  return (<svg width={size} height={size} viewBox="0 0 44 44"><circle cx="22" cy="22" r="20" fill={col} stroke="#0b1030" strokeWidth="2"/><circle cx="22" cy="22" r="15" fill="none" stroke="rgba(11,16,48,.35)" strokeWidth="1.5"/><text x="22" y="26" textAnchor="middle" fontSize="12" fontWeight="800" fill="#0b1030" fontFamily="monospace">{v}</text></svg>);
};
/* ---- IMZO: TANGA-SAVAT (CoinTray) — tangalarni bosib summa yig'ish ---- */
function makeCoinTray(cfg) {
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review';
    const [tray,setTray]=useState([]),[fb,setFb]=useState(null),[checked,setChecked]=useState(false);
    useEffect(()=>{ const sa=initialAnswer?.studentAnswer; if(sa?.tray){ setTray(sa.tray); if(typeof initialAnswer.correct==='boolean'){ setFb({correct:initialAnswer.correct}); setChecked(true); } } },[initialAnswer]);
    const sum=tray.reduce((s,v)=>s+v,0);
    useEffect(()=>{ onReady?.(tray.length>0 && !checked); },[tray,checked,onReady]);
    const add=(v)=>{ if(isReview||checked) return; setTray(a=>[...a,v]); };
    const back=()=>{ if(isReview||checked) return; setTray(a=>a.slice(0,-1)); };
    const check=useCallback(()=>{ const correct=sum===cfg.target; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:[], studentAnswer:{tray,sum}, correctAnswer:{value:cfg.target}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[tray,sum,t,playCorrect,playWrong,onSubmit]);
    ...
    return (
      <div style={S.wrap}>
        ...
        <p style={S.ask}>{t.ask}</p>
        <Stage>
          <div style={{ minHeight:56, borderRadius:12, border:'2px dashed '+C.stageBd, background:'rgba(255,255,255,.04)', padding:'8px', display:'flex', flexWrap:'wrap', gap:6, alignItems:'center', justifyContent:'center' }}>
            {tray.length===0 && <span style={{ fontSize:13, color:C.sink2 }}>{t.empty}</span>}
            <AnimatePresence>{tray.map((v,i)=><motion.span key={i} initial={{scale:0,y:-16}} animate={{scale:1,y:0}} exit={{scale:0}}><Coin v={v} size={38} /></motion.span>)}</AnimatePresence>
          </div>
          <div style={{ textAlign:'center', marginTop:8, ...S.mono, fontSize:24, fontWeight:800, color:done?C.leaf:bad?'#ffb4a8':C.gold }}>{sum} {t.som}</div>
        </Stage>
        <div style={{ display:'flex', gap:10, justifyContent:'center', alignItems:'center', marginTop:6, flexWrap:'wrap' }}>
          {cfg.coins.map((v,i)=><motion.button key={i} type="button" whileTap={{scale:.9}} disabled={isReview||checked} onClick={()=>add(v)} style={{ border:'none', background:'transparent', cursor:(isReview||checked)?'default':'pointer', padding:0 }}><Coin v={v} size={52} /></motion.button>)}
          <button type="button" disabled={isReview||checked} onClick={back} style={{ width:52, height:52, borderRadius:26, border:'2px solid '+C.line, background:C.paper, fontSize:20, fontWeight:800, color:C.no, cursor:(isReview||checked)?'default':'pointer' }}>⌫</button>
        </div>
        {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
        {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
      </div>
...
/* ===== 02 · IMZO CoinTray · 100 ===== */
const D02 = makeCoinTray({ tag:'tray100', level:'🟢', target:100, coins:[50,100,200] });
D02.T = {
  uz: { eyebrow:'Tanga-savat', setup:"Anvar do‘konda 100 so'mlik narsa olmoqchi.", ask:'Aynan 100 so‘m yig‘ish uchun tangalarni savatga soling.', empty:'Tangalarni bu yerga soling', som:"so'm", correct:"To'g'ri. 100 so‘m yig‘ildi (100 yoki 50 + 50).", wrong:'Yig‘indi aynan 100 so‘m bo‘lsin. Ortiqcha bo‘lsa, ⌫ bilan oling.', rule:'100 so‘m = 100 yoki 50 + 50.' },
  ru: { eyebrow:'Копилка монет', setup:'Анвар хочет купить вещь за 100 сумов.', ask:'Собери ровно 100 сумов: клади монеты в копилку.', empty:'Клади монеты сюда', som:'сум', correct:'Верно. Собрано 100 сумов (100 или 50 + 50).', wrong:'Сумма должна быть ровно 100. Лишнее убери кнопкой ⌫.', rule:'100 сумов = 100 или 50 + 50.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_35_02(props) {
  return (
    <React.Fragment>

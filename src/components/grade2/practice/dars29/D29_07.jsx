// Dars 29 · Amaliyot 07 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 07 · VisualMC · qaysi noto'g'ri (ochiq) ===== */
const D07 = makeVisualMC({ tag:'pick_open', level:'🟡', correctIdx:2, items:[{n:3,c:true},{n:5,c:true},{n:4,c:false},{n:6,c:true}], render:(it,i)=><PolyFig n={it.n} closed={it.c} stroke={['#FFC23C','#7CE0A3','#FF9A6B','#5BD6F2'][i]} /> });
D07.T = {
  uz: { eyebrow:'Xatoni top', setup:'To‘g‘ri yasalgan shakl yopiq bo‘ladi.', ask:'Quyidagilardan qaysi shakl noto‘g‘ri yasalgan (yopilmagan)?',
    correct:"To'g'ri. Bu shaklning bir joyi ochiq — u yopilmagan.", wrong:'Qaysi shaklning boshi oxiriga ulanmagan? O‘shani toping.', rule:'Noto‘g‘ri shakl — yopilmagan (ochiq).' },
  ru: { eyebrow:'Найди ошибку', setup:'Верно построенная фигура замкнута.', ask:'Какая фигура построена неверно (не замкнута)?',
    correct:'Верно. У этой фигуры есть открытое место — она не замкнута.', wrong:'У какой начало не соединено с концом? Её и найди.', rule:'Неверная фигура — не замкнута.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_29_07(props) {
  return (
    <React.Fragment>

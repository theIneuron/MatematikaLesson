// Dars 29 · Amaliyot 08 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 08 · VisualMC · qaysi buyum to'g'ri to'rtburchak (rasmli) ===== */
const D08 = makeVisualMC({ tag:'real_rect', level:'🔴', correctIdx:3,
  items:[{kind:'chess'},{kind:'ball'},{kind:'pizza'},{kind:'tv'}],
  render:(it,i,st)=>{ if(it.kind==='chess') return <ChessIcon/>; if(it.kind==='ball') return <BallIcon/>; if(it.kind==='pizza') return <PizzaIcon/>; return <TvIcon lit={!!(st&&st.picked&&st.checked&&st.correct)} />; } });
D08.T = {
  uz: { eyebrow:'Hayotdan', setup:'To‘g‘ri to‘rtburchakning qarama-qarshi tomonlari teng, lekin bo‘yi va eni har xil. (Kvadratning esa hamma tomoni teng, doira yumaloq.)', ask:'Quyida ko‘rsatilgan buyumlardan qaysi biri to‘g‘ri to‘rtburchak shaklida?', opts:['Shaxmat doskasi','Koptok','Pitsa','Televizor'],
    correct:"To'g'ri. Televizor ekrani to‘g‘ri to‘rtburchak — bo‘yi va eni har xil.", wrong:'Shaxmat doskasi — kvadrat (hamma tomoni teng), koptok va pitsa — yumaloq. Bo‘yi va eni har xil buyumni toping.', rule:'To‘g‘ri to‘rtburchak — bo‘yi va eni har xil (masalan, televizor).' },
  ru: { eyebrow:'Из жизни', setup:'У прямоугольника противоположные стороны равны, но длина и ширина разные. (У квадрата все стороны равны, круг — круглый.)', ask:'Какой из показанных предметов имеет форму прямоугольника?', opts:['Шахматная доска','Мяч','Пицца','Телевизор'],
    correct:'Верно. Экран телевизора — прямоугольник: длина и ширина разные.', wrong:'Шахматная доска — квадрат (все стороны равны), мяч и пицца — круглые. Найди предмет с разными длиной и шириной.', rule:'Прямоугольник — длина и ширина разные (например, телевизор).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_29_08(props) {
  return (
    <React.Fragment>

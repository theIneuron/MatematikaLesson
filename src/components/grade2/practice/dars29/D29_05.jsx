// Dars 29 · Amaliyot 05 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 05 · VisualMC · qaysi yopiq ===== */
const D05 = makeVisualMC({ tag:'pick_closed', level:'🟡', correctIdx:1, items:[{n:4,c:false},{n:4,c:true},{n:3,c:false},{n:5,c:false}], render:(it,i)=><PolyFig n={it.n} closed={it.c} stroke={['#FF9A6B','#7CE0A3','#5BD6F2','#c792ea'][i]} /> });
D05.T = {
  uz: { eyebrow:'To‘g‘ri yasalgani', setup:'To‘liq (yopiq) shaklda chiziq boshi oxiriga ulanadi.', ask:'Quyidagilardan qaysi biri to‘g‘ri, ya‘ni to‘liq yopiq shakl?',
    correct:"To'g'ri. Bu shaklning boshi oxiriga ulangan — u yopiq.", wrong:'Har shaklga qarang: qaysi birining boshi oxiriga to‘liq ulangan?', rule:'To‘g‘ri shakl — boshi oxiriga ulangan (yopiq).' },
  ru: { eyebrow:'Верно построено', setup:'В замкнутой фигуре начало соединено с концом.', ask:'Какая из фигур построена верно — полностью замкнута?',
    correct:'Верно. У этой фигуры начало соединено с концом — она замкнута.', wrong:'Посмотри: у какой начало полностью соединено с концом?', rule:'Верная фигура — замкнутая.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_29_05(props) {
  return (
    <React.Fragment>

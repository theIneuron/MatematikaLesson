// Dars 29 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 06 · VisualMC · shakl naqshi (uchburchak,to'rtburchak,?,oltiburchak) ===== */
const D06 = makeVisualMC({ tag:'shape_pattern', level:'🟡', correctIdx:3,
  stage:(t,st)=>(<div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, flexWrap:'nowrap' }}>
    <PolyFig n={3} r={30} box={78} stroke="#FFC23C" />
    <PolyFig n={4} r={30} box={78} stroke="#7CE0A3" />
    {st && st.pickedCorrect
      ? <motion.div key="penta" initial={{ scale:0, rotate:-45 }} animate={{ scale:1, rotate:0 }} transition={{ type:'spring', stiffness:210, damping:13 }} style={{ width:66, height:78, display:'flex', alignItems:'center', justifyContent:'center' }}><PolyFig n={5} r={30} box={78} stroke="#c792ea" /></motion.div>
      : <div style={{ width:66, height:78, display:'flex', alignItems:'center', justifyContent:'center', border:'2.5px dashed '+C.acc, borderRadius:14, background:C.accSoft, fontSize:38, fontWeight:900, color:C.acc }}>?</div>}
    <PolyFig n={6} r={30} box={78} stroke="#5BD6F2" />
  </div>),
  items:[{n:3},{n:6},{n:4},{n:5}], render:(it,i)=><PolyFig n={it.n} r={32} box={92} stroke={['#FFC23C','#5BD6F2','#7CE0A3','#c792ea'][i]} /> });
D06.T = {
  uz: { eyebrow:'Shakl naqshi', setup:'Ra‘no shakllarni bir qatorga terib chiqdi.', ask:'3-o‘rindagi shakl qaysi shakl ekanligini toping. Maslahat: tomonlar soniga e‘tibor bering.',
    correct:"To'g'ri. Tartib 3–4–5–6 tomon: yetishmayotgani beshburchak (5 tomon).", wrong:'Tomonlar sonini sanang: 3, 4, ?, 6. Yetishmayotgan son — 5, ya‘ni beshburchak.', rule:'Naqsh: 3–4–5–6 tomon. Bo‘sh joy — beshburchak.' },
  ru: { eyebrow:'Узор фигур', setup:'Рано разложила фигуры в один ряд.', ask:'Определи, какая фигура стоит на 3-м месте. Подсказка: обрати внимание на число сторон.',
    correct:'Верно. Порядок 3–4–5–6 сторон: пропущен пятиугольник (5 сторон).', wrong:'Посчитай стороны: 3, 4, ?, 6. Пропущено число 5 — это пятиугольник.', rule:'Узор: 3–4–5–6 сторон. Пропуск — пятиугольник.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_29_06(props) {
  return (
    <React.Fragment>

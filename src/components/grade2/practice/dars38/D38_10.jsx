// Dars 38 · Amaliyot 10 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 38;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN — ma'lumotlar bilan ishlash ============================== */
const Fruit = ({ c='#FF6B6B', s=26, on=false, delay=0 }) => (
  <motion.svg width={s} height={s} viewBox="0 0 26 26" initial={{scale:0}} animate={{scale:1}} transition={{delay,type:'spring'}}>
    <circle cx="13" cy="15" r="9" fill={on?C.acc:c} stroke="#0b1030" strokeWidth="1.5" />
    <rect x="12" y="3" width="2" height="6" rx="1" fill="#7CE0A3" />
  </motion.svg>
);
// Ustunli diagramma (MC sahnalari uchun)
const BarChart = ({ data, max=8 }) => {
  const bw=44, gap=18, H=120, x0=20;
  return (<svg width="100%" viewBox={`0 0 ${x0*2+data.length*(bw+gap)} ${H+34}`} style={{maxWidth:x0*2+data.length*(bw+gap),display:'block',margin:'0 auto'}}>
    {data.map((d,i)=>{ const h=d.val/max*H; const x=x0+i*(bw+gap);
      return (<g key={i}>
        <motion.rect initial={{height:0,y:H}} animate={{height:h,y:H-h}} transition={{delay:i*0.15,type:'spring',stiffness:90}} x={x} width={bw} rx={5} fill={d.color} />
        <text x={x+bw/2} y={H-h-6} textAnchor="middle" fontSize="14" fontWeight="800" fill={C.gold} fontFamily="monospace">{d.val}</text>
        <text x={x+bw/2} y={H+22} textAnchor="middle" fontSize="12.5" fontWeight="700" fill={C.sink}>{d.label}</text>
      </g>); })}
  </svg>);
};

/* ---- IMZO: PIKTOGRAMMA (PictoCount) — ikonkalarni bosib sanash ---- */
function makePictoCount(cfg) {
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review'; const n=cfg.n;
    ...
    const check=useCallback(()=>{ const correct=tapped.length===n; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:[], studentAnswer:{tapped,count:tapped.length}, correctAnswer:{count:n}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[tapped,t,playCorrect,playWrong,onSubmit]);
    ...
    return (<div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display:'flex', gap:6, justifyContent:'center', flexWrap:'wrap', alignItems:'center' }}>
          {Array.from({length:n}).map((_,i)=>(<button key={i} type="button" disabled={isReview||checked} onClick={()=>tap(i)} style={{ border:'none', background:'transparent', cursor:(isReview||checked)?'default':'pointer', padding:2 }}><Fruit c={cfg.color||'#FF6B6B'} s={34} on={tapped.includes(i)} delay={i*0.05} /></button>))}
        </div>
        <div style={{ textAlign:'center', marginTop:8, ...S.mono, fontSize:22, fontWeight:800, color:done?C.leaf:bad?'#ffb4a8':C.goldSoft }}>{t.counter}: {tapped.length}</div>
      </Stage>
      {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
      {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
    </div>);
...
const barStage = (data,max)=>((t)=>(<BarChart data={data} max={max||8} />));

/* ===== 10 · MC · umumlashtirish ===== */
const D10 = makeMC({ tag:'gen_data', level:'🔴', correctIdx:0, half:false, fs:15, mono:false });
D10.T = {
  uz: { eyebrow:'Umumlashtirish', setup:'Piktogramma o‘qishda muhim qoida bor.', ask:'Piktogrammani to‘g‘ri o‘qish uchun nima muhim?', opts:['1 rasm nechta birlikni bildirishini bilish','Rasmlar rangi','Rasmlar kattaligi','Rasm chiroyliligi'],
    correct:"To'g'ri. Har rasm nechta birlikni bildirishini (masshtabni) bilish muhim.", wrong:'Ba‘zan 1 rasm 1 emas, 2 (yoki ko‘proq) birlikni bildiradi. Nimani bilish kerak?', rule:'Piktogrammada masshtab (1 rasm = nechta) muhim.' },
  ru: { eyebrow:'Обобщение', setup:'При чтении пиктограммы есть важное правило.', ask:'Что важно, чтобы верно прочитать пиктограмму?', opts:['Знать, сколько единиц в 1 рисунке','Цвет рисунков','Размер рисунков','Красоту рисунка'],
    correct:'Верно. Важно знать масштаб — сколько единиц в одном рисунке.', wrong:'Иногда 1 рисунок = не 1, а 2 (или больше). Что нужно знать?', rule:'В пиктограмме важен масштаб (1 рисунок = сколько).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_38_10(props) {
  return (
    <React.Fragment>

// Dars 22 · Amaliyot 02 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 02 · Nechta vagoncha (÷5) · 🟢 · groups5 =================== */
const D02_CORRECT = 0;
const D02_T = {
  uz: { eyebrow: 'Nechta vagoncha', setup: "20 kristal. Har vagonchaga 5 tadan solamiz.", ask: '20 kristalni har vagonchaga 5 tadan solsak, nechta vagoncha kerak?', opts: ['4', '5', '3', '15'], correct: "To'g'ri. 20 ÷ 5 = 4. 4 ta vagoncha to'ladi.", wrong: "Maslahat: 20 da nechta 5lik bor? 20 − 5 = 15 emas.", rule: "Har vagonchada 5 tadan: 20 ÷ 5 = 4 vagoncha." },
  ru: { eyebrow: 'Сколько вагонеток', setup: '20 кристаллов. По 5 в каждую вагонетку.', ask: 'Если класть по 5, сколько вагонеток нужно для 20?', opts: ['4', '5', '3', '15'], correct: 'Верно. 20 ÷ 5 = 4. Наполнятся 4 вагонетки.', wrong: 'Подсказка: сколько пятёрок в 20? 20 − 5 = 15 — неверно.', rule: 'По 5 в каждой: 20 ÷ 5 = 4 вагонетки.' },
};
function D22_02(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D02_T[lang] || D02_T.uz; const isReview = mode === 'review';
  ...
  const check = useCallback(() => { const correct = picked === D02_CORRECT; setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '4' }, correct, meta: { tag: 'groups5', level: '🟢' } }); }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 300, margin: '0 auto' }}>{Array.from({ length: 20 }).map((_, i) => <Crystal key={i} s={16} delay={i * 0.02} />)}</div><div className="d22-float" style={{ textAlign: 'center', marginTop: 8, fontSize: 13, fontWeight: 700, color: C.goldSoft }}>20 kristal · har vagonchaga 5 tadan</div></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>{t.opts.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D02_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 22 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o}</button>)}</div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_22_02(props) {
  return (
    <React.Fragment>

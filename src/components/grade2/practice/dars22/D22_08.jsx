// Dars 22 · Amaliyot 08 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
const D08_CORRECT = 0;
const D08_T = {
  uz: { eyebrow: 'Masala', setup: "Anvar 30 ta kristalni har vagonchaga 5 tadan joyladi.", ask: 'Nechta vagoncha to‘ldi?', opts: ['6', '5', '25', '7'], correct: "To'g'ri. 30 ÷ 5 = 6. 6 ta vagoncha.", wrong: "Maslahat: 30 da nechta 5lik bor? 30 − 5 = 25 emas — bu ayirish.", rule: "Har vagonchaga 5 tadan: 30 ÷ 5 = 6." },
  ru: { eyebrow: 'Задача', setup: 'Анвар клал по 5 кристаллов в каждую вагонетку, всего 30.', ask: 'Сколько вагонеток наполнилось?', opts: ['6', '5', '25', '7'], correct: 'Верно. 30 ÷ 5 = 6. 6 вагонеток.', wrong: 'Подсказка: сколько пятёрок в 30? 30 − 5 = 25 — вычитание.', rule: 'По 5 в каждой: 30 ÷ 5 = 6.' },
};
function D22_08(props) {
  ...
  const check = useCallback(() => { const correct = picked === D08_CORRECT; setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '6' }, correct, meta: { tag: 'word5', level: '🔴' } }); }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ display: 'flex', gap: 8, justifyContent: 'center', alignItems: 'flex-end', flexWrap: 'wrap' }}>{[5, 5, 5, 5, 5, 5].map((n, i) => <Cart key={i} n={n} w={50} />)}</div></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>{t.opts.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D08_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 22 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o}</button>)}</div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_22_08(props) {
  return (
    <React.Fragment>

// Dars 22 · Amaliyot 07 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 07 · Yashirin bo'luvchi · 🟡 · missing_divisor =================== */
const D07_CORRECT = 0;
const D07_T = {
  uz: { eyebrow: 'Yashirin bo‘luvchi', setup: "Nechaga bo'lingan? 20 ÷ ⬜ = 5.", ask: 'Yashirin bo‘luvchini toping: 20 ÷ ⬜ = 5.', opts: ['4', '5', '15', '25'], correct: "To'g'ri. 20 ÷ 4 = 5. (4 × 5 = 20)", wrong: "Maslahat: 20 ni nechaga bo'lsak 5 chiqadi? 5 ni nechaga ko'paytirsak 20 bo'ladi?", rule: "20 ÷ ⬜ = 5 → ⬜ = 20 ÷ 5 = 4." },
  ru: { eyebrow: 'Скрытый делитель', setup: 'На сколько поделили? 20 ÷ ⬜ = 5.', ask: 'Найди скрытый делитель: 20 ÷ ⬜ = 5.', opts: ['4', '5', '15', '25'], correct: 'Верно. 20 ÷ 4 = 5. (4 × 5 = 20)', wrong: 'Подсказка: на что поделить 20, чтобы вышло 5? На что умножить 5, чтобы вышло 20?', rule: '20 ÷ ⬜ = 5 → ⬜ = 20 ÷ 5 = 4.' },
};
function D22_07(props) {
  ...
  const check = useCallback(() => { const correct = picked === D07_CORRECT; setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '4' }, correct, meta: { tag: 'missing_divisor', level: '🟡' } }); }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const boxCol = checked ? (fb?.correct ? C.gold : '#FF9A82') : (picked != null ? C.gold : C.sink2);
  const boxVal = picked != null ? t.opts[picked] : '⬜';
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, ...S.mono, fontSize: 40, fontWeight: 800, color: C.sink }}><span>20</span><span style={{ color: C.gold }}>÷</span><span className={picked == null && !checked ? 'd22-pulse' : undefined} style={{ minWidth: 60, height: 62, border: '2px dashed ' + boxCol, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: boxCol, background: C.stile }}>{boxVal}</span><span style={{ color: C.gold }}>=</span><span>5</span></div></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>{t.opts.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D07_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o}</button>)}</div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_22_07(props) {
  return (
    <React.Fragment>

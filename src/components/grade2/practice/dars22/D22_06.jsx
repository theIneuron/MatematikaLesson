// Dars 22 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 06 · Javobni ter (÷4) · 🟡 · numpad4 =================== */
const D06_ANS = 6;
const D06_T = {
  uz: { eyebrow: 'Javobni ter', setup: "24 kristalni 4 vagonchaga teng ulang.", ask: '24 ÷ 4 nechchi? Raqamni bosib tering:', correct: "To'g'ri. 24 ÷ 4 = 6. Har vagonchada 6 tadan.", wrong: "Maslahat: 4 ni nechaga ko'paytirsak 24 bo'ladi? 24 − 4 = 20 emas.", rule: "24 ÷ 4 = 6. Yordam: 4 × 6 = 24." },
  ru: { eyebrow: 'Набери ответ', setup: '24 кристалла раздели в 4 вагонетки поровну.', ask: 'Сколько 24 ÷ 4? Набери цифру:', correct: 'Верно. 24 ÷ 4 = 6. По 6 в каждой.', wrong: 'Подсказка: на что умножить 4, чтобы вышло 24? 24 − 4 = 20 — неверно.', rule: '24 ÷ 4 = 6. Подсказка: 4 × 6 = 24.' },
};
function D22_06(props) {
  ...
  const check = useCallback(() => { const correct = parseInt(val, 10) === D06_ANS; setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value: parseInt(val, 10) }, correctAnswer: { value: D06_ANS }, correct, meta: { tag: 'numpad4', level: '🟡' } }); }, [val, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 300, margin: '0 auto' }}>{Array.from({ length: 24 }).map((_, i) => <Crystal key={i} s={14} delay={i * 0.02} />)}</div><div className="d22-float" style={{ textAlign: 'center', marginTop: 8, ...S.mono, fontSize: 30, fontWeight: 800, color: C.gold }}>24 ÷ 4</div></Stage>
...
const __FX_CSS = `
      ...
      `;
export default function Task_22_06(props) {
  return (
    <React.Fragment>

// Dars 22 · Amaliyot 10 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 10 · To'g'rimi? · 🔴 · truefalse =================== */
const D10_CORRECT = 1; // Yo'q (20 ÷ 5 = 5 xato, to'g'risi 4)
const D10_T = {
  uz: { eyebrow: 'To‘g‘rimi?', setup: "Bit yozdi: 20 ÷ 5 = 5.", ask: 'Bu to‘g‘rimi? 20 ÷ 5 = 5?', yes: 'Ha, to‘g‘ri', no: "Yo'q, xato", correct: "To'g'ri! 20 ÷ 5 = 4, 5 emas. (5 × 4 = 20)", wrong: "Maslahat: 5 ni nechaga ko'paytirsak 20 bo'ladi? 5 × 5 = 25, 20 emas.", rule: "20 ÷ 5 = 4, chunki 5 × 4 = 20." },
  ru: { eyebrow: 'Верно ли?', setup: 'Бит написал: 20 ÷ 5 = 5.', ask: 'Это верно? 20 ÷ 5 = 5?', yes: 'Да, верно', no: 'Нет, ошибка', correct: 'Верно! 20 ÷ 5 = 4, а не 5. (5 × 4 = 20)', wrong: 'Подсказка: на что умножить 5, чтобы вышло 20? 5 × 5 = 25, не 20.', rule: '20 ÷ 5 = 4, ведь 5 × 4 = 20.' },
};
function D22_10(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D10_T[lang] || D10_T.uz; const isReview = mode === 'review';
  ...
  const check = useCallback(() => { const correct = picked === D10_CORRECT; setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: [{ id: '0', label: t.yes }, { id: '1', label: t.no }], studentAnswer: { idx: picked }, correctAnswer: { idx: 1 }, correct, meta: { tag: 'truefalse', level: '🔴' } }); }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const btn = (i, label) => { const on = picked === i, show = checked && on; let bg = C.paper, bd = C.line, col = C.ink; if (on && !checked) { bg = C.accSoft; bd = C.acc; } if (show) { const ok = i === D10_CORRECT; bg = ok ? C.okSoft : C.noSoft; bd = ok ? C.ok : C.no; col = ok ? C.ok : C.no; } return <button key={i} type="button" disabled={isReview || checked} onClick={() => setPicked(i)} style={{ flex: 1, minHeight: 66, borderRadius: 14, border: '2px solid ' + bd, background: bg, color: col, fontSize: 16, fontWeight: 800, fontFamily: 'inherit', cursor: (isReview || checked) ? 'default' : 'pointer' }}>{label}</button>; };
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ textAlign: 'center', ...S.mono, fontSize: 44, fontWeight: 800, color: C.gold }}>20 ÷ 5 = 5</div></Stage>
      <p style={S.ask}>{t.ask}</p>
...
const __FX_CSS = `
      ...
      `;
export default function Task_22_10(props) {
  return (
    <React.Fragment>

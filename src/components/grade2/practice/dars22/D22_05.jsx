// Dars 22 · Amaliyot 05 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 05 · Ortiqchani top · 🟡 · oddone =================== */
const D05_OPTS = [{ e: '20 ÷ 4', v: 5 }, { e: '25 ÷ 5', v: 5 }, { e: '24 ÷ 4', v: 6 }];
const D05_CORRECT = 2;
const D05_T = {
  uz: { eyebrow: 'Ortiqchani top', setup: "Uch ifoda. Ikkitasi 5 ga teng.", ask: 'Qaysi ifoda javobi 5 GA TENG EMAS?', correct: "To'g'ri. 24 ÷ 4 = 6, u 5 ga teng emas. 20 ÷ 4 = 5, 25 ÷ 5 = 5.", wrong: "Maslahat: har birini hisoblang. 24 ÷ 4 nechchi?", rule: "20 ÷ 4 = 5, 25 ÷ 5 = 5, 24 ÷ 4 = 6. Ortiqchasi — 24 ÷ 4." },
  ru: { eyebrow: 'Найди лишнее', setup: 'Три выражения. Два равны 5.', ask: 'Какое выражение НЕ равно 5?', correct: 'Верно. 24 ÷ 4 = 6, не 5. 20 ÷ 4 = 5, 25 ÷ 5 = 5.', wrong: 'Подсказка: посчитай каждое. Сколько 24 ÷ 4?', rule: '20 ÷ 4 = 5, 25 ÷ 5 = 5, 24 ÷ 4 = 6. Лишнее — 24 ÷ 4.' },
};
function D22_05(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D05_T[lang] || D05_T.uz; const isReview = mode === 'review';
  ...
  const check = useCallback(() => { const correct = picked === D05_CORRECT; setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: D05_OPTS.map((o, i) => ({ id: String(i), label: o.e })), studentAnswer: { idx: picked }, correctAnswer: { idx: 2, label: '24 ÷ 4' }, correct, meta: { tag: 'oddone', level: '🟡' } }); }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ textAlign: 'center', ...S.mono, fontSize: 22, fontWeight: 800, color: C.gold }}>= 5 ?</div></Stage>
      <p style={S.ask}>{t.ask}</p>
      {D05_OPTS.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D05_CORRECT, checked, isReview, { center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o.e}</button>)}
...
const __FX_CSS = `
      ...
      `;
export default function Task_22_05(props) {
  return (
    <React.Fragment>

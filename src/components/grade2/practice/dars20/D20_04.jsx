// Dars 20 · Amaliyot 04 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D04_ROWS = [{ expr: '2 × 5', twin: '10 ÷ 5' }, { expr: '3 × 4', twin: '12 ÷ 3' }, { expr: '4 × 5', twin: '20 ÷ 4' }, { expr: '2 × 6', twin: '12 ÷ 6' }];
const D04_CARDS = ['10 ÷ 5', '20 ÷ 4', '12 ÷ 3', '12 ÷ 6', '10 ÷ 2', '12 ÷ 4'];
const D04_T = {
  uz: {
    eyebrow: 'Oila juftlari', setup: "Har ko'paytmaning bo'lish juftini toping (bitta oiladan).",
    ask: 'Kartani bosing, keyin mos ifodaning bo‘sh joyini bosing:',
    correct: "To'g'ri. 2×5→10÷5, 3×4→12÷3, 4×5→20÷4, 2×6→12÷6.",
    wrong: "Maslahat: har ko'paytma bo'lishni tekshiradi. 3×4=12 → 12÷3.",
    rule: "Bir oilada: a × b = c, va c ÷ a = b.",
  },
  ru: {
    eyebrow: 'Пары семьи', setup: 'Найди для каждого произведения его деление (из той же семьи).',
    ask: 'Нажми карточку, затем пустое место нужного выражения:',
    correct: 'Верно. 2×5→10÷5, 3×4→12÷3, 4×5→20÷4, 2×6→12÷6.',
    wrong: 'Подсказка: каждое произведение проверяет деление. 3×4=12 → 12÷3.',
    rule: 'В одной семье: a × b = c, и c ÷ a = b.',
  },
};
function D20_04Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { slots }, correctAnswer: { twins: D04_ROWS.map((r) => r.twin) }, correct, meta: { tag: 'fact_match', level: '🟡' } });
  ...
  return (
    <div style={S.wrap}>
        ...
        {D04_ROWS.map((r, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'center' }}>
              <div style={{ width: 54, textAlign: 'right', ...S.mono, fontSize: 17, fontWeight: 800, color: cols[i] }}>{r.expr}</div>
              <span style={{ ...S.mono, fontSize: 15, fontWeight: 800, color: C.ink3 }}>→</span>
              <div onClick={clickSlot(i)} style={{ width: 78, height: 52, borderRadius: 12, border: '2px ' + (v != null ? 'solid' : 'dashed') + ' ' + bd, background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: locked ? 'default' : 'pointer', ...S.mono, fontSize: 17, fontWeight: 800, color: col }}>{v != null ? v : ''}</div>
            </div>
          );
        })}
      </div>
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D04_CARDS.map((n, idx) => {
          if (usedSet.has(idx)) return <span key={idx} style={{ width: 78, height: 50, borderRadius: 12, border: '2px dashed ' + C.line, background: '#fafafa' }} />;
          const on = pick === idx;
          return <button key={idx} type="button" disabled={locked} onClick={() => setPick(on ? null : idx)} style={{ width: 78, height: 50, borderRadius: 12, border: '2px solid ' + (on ? C.acc : C.line), background: on ? C.accSoft : C.paper, ...S.mono, fontSize: 17, fontWeight: 800, color: C.ink, cursor: locked ? 'default' : 'pointer', boxShadow: on ? '0 0 0 4px #FFE0D6' : 'none' }}>{n}</button>;
        })}
      </div>

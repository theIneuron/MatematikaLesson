// Dars 20 · Amaliyot 05 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D05_T = {
  uz: {
    eyebrow: 'Oila a‘zosi', setup: "Kristal oilasi: 2 × 5 = 10, 10 ÷ 2 = 5.",
    ask: 'Oilaning bo‘sh a‘zosini toping: 10 ÷ 5 = ⬜',
    opts: ['5', '2', '10', '7'],
    correct: "To'g'ri. 10 ÷ 5 = 2. Oila to'ldi: 2 × 5 = 10, 10 ÷ 2 = 5, 10 ÷ 5 = 2.",
    wrong: "Maslahat: 10 ni 5 ga bo'ling. 2 × 5 = 10 bo'lgani uchun 10 ÷ 5 = 2.",
    rule: "Bir massivdan bitta × va ikkita ÷ chiqadi — bir oila.",
  },
  ru: {
    eyebrow: 'Член семьи', setup: 'Семья кристаллов: 2 × 5 = 10, 10 ÷ 2 = 5.',
    ask: 'Найди пропущенного члена: 10 ÷ 5 = ⬜',
    opts: ['5', '2', '10', '7'],
    correct: 'Верно. 10 ÷ 5 = 2. Семья готова: 2 × 5 = 10, 10 ÷ 2 = 5, 10 ÷ 5 = 2.',
    wrong: 'Подсказка: раздели 10 на 5. Так как 2 × 5 = 10, то 10 ÷ 5 = 2.',
    rule: 'Из одного массива — одно × и два ÷: одна семья.',
  },
};
function D20_05Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 1, label: '2' }, correct, meta: { tag: 'family_fill', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'center' }}>
          <div style={{ ...S.mono, fontSize: 20, fontWeight: 800, color: C.goldSoft }}>2 × 5 = 10</div>
          <div style={{ ...S.mono, fontSize: 20, fontWeight: 800, color: C.goldSoft }}>10 ÷ 2 = 5</div>
          <div className="d20-float" style={{ ...S.mono, fontSize: 26, fontWeight: 800, color: C.gold }}>10 ÷ 5 = ⬜</div>
        </div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D05_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

// Dars 20 · Amaliyot 01 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D01_T = {
  uz: {
    eyebrow: 'Bir oila', setup: "Kristallar 3 qatorda, har qatorda 4 tadan: 3 × 4 = 12.",
    ask: 'Unda 12 ÷ 4 = ⬜?',
    opts: ['8', '3', '4', '12'],
    correct: "To'g'ri. 3 × 4 = 12, demak 12 ÷ 4 = 3. Bir oila!",
    wrong: "Maslahat: 12 ni 4 ga bo'ling, ayirmang. 12 ÷ 4 = 3, 12 − 4 = 8 emas.",
    rule: "3 × 4 = 12 → 12 ÷ 4 = 3. Ko'paytirish va bo'lish — bir oila.",
  },
  ru: {
    eyebrow: 'Одна семья', setup: 'Кристаллы в 3 рядах по 4: 3 × 4 = 12.',
    ask: 'Тогда 12 ÷ 4 = ⬜?',
    opts: ['8', '3', '4', '12'],
    correct: 'Верно. 3 × 4 = 12, значит 12 ÷ 4 = 3. Одна семья!',
    wrong: 'Подсказка: раздели 12 на 4, а не вычитай. 12 ÷ 4 = 3, а не 12 − 4 = 8.',
    rule: '3 × 4 = 12 → 12 ÷ 4 = 3. Умножение и деление — одна семья.',
  },
};
function D20_01Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 1, label: '3' }, correct, meta: { tag: 'family_mc', level: '🟢' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <CrystalArray r={3} c={4} />
        <div style={{ textAlign: 'center', ...S.mono, fontSize: 22, fontWeight: 800, color: C.goldSoft }}>3 × 4 = 12</div>
        <div className="d20-float" style={{ textAlign: 'center', marginTop: 6, ...S.mono, fontSize: 30, fontWeight: 800, color: C.gold }}>12 ÷ 4 = ⬜</div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D01_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

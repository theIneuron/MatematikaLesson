// Dars 20 · Amaliyot 02 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const BoxSub = ({ val, boxCol, picked, pulse }) => (
  <span className={pulse ? 'd20-pulse' : undefined} style={{ minWidth: 56, height: 58, border: '2px dashed ' + boxCol, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: boxCol, background: C.stile }}>{val}</span>
);
...
const D02_T = {
  uz: {
    eyebrow: 'Bo‘sh katak', setup: "Kristal oilasida bir a'zo yashiringan.",
    ask: 'Bo‘sh katakni to‘ldiring:',
    opts: ['3', '4', '8', '6'],
    correct: "To'g'ri. 3 × 4 = 12. 3 ni 4 ga ko'paytirsak 12 bo'ladi.",
    wrong: "Maslahat: qaysi son 4 ga ko'paytirilsa 12 bo'ladi? 3 × 4 = 12.",
    rule: "⬜ × 4 = 12 — 4 ga ko'paytirilsa 12 beradigan sonni toping.",
  },
  ru: {
    eyebrow: 'Пустая клетка', setup: 'В семье кристаллов спрятан один член.',
    ask: 'Заполни пустую клетку:',
    opts: ['3', '4', '8', '6'],
    correct: 'Верно. 3 × 4 = 12. Если 3 умножить на 4, будет 12.',
    wrong: 'Подсказка: какое число умножить на 4, чтобы вышло 12? 3 × 4 = 12.',
    rule: '⬜ × 4 = 12 — найди число, которое умножить на 4 даёт 12.',
  },
};
function D20_02Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '3' }, correct, meta: { tag: 'family_sub', level: '🟢' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, ...S.mono, fontSize: 40, fontWeight: 800, color: C.sink }}>
          ...
          <span style={{ color: C.gold }}>×</span><span>4</span><span style={{ color: C.gold }}>=</span><span>12</span>
        </div>
      </Stage>

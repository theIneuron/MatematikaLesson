// Dars 20 · Amaliyot 08 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D08_T = {
  uz: {
    eyebrow: 'Bo‘sh katak', setup: "Yordam: qaysi son 5 ga ko'paytirilsa 15 bo'ladi?",
    ask: 'Bo‘sh katakni to‘ldiring:',
    opts: ['5', '3', '10', '4'],
    correct: "To'g'ri. 3 × 5 = 15, demak 15 ÷ 3 = 5. Bir oila!",
    wrong: "Maslahat: 3 × 5 = 15. Shundan 15 ÷ 3 = 5.",
    rule: "15 ÷ ⬜ = 5 — ko'paytirishdan foydalaning: 3 × 5 = 15.",
  },
  ru: {
    eyebrow: 'Пустая клетка', setup: 'Подсказка: какое число умножить на 5, чтобы вышло 15?',
    ask: 'Заполни пустую клетку:',
    opts: ['5', '3', '10', '4'],
    correct: 'Верно. 3 × 5 = 15, значит 15 ÷ 3 = 5. Одна семья!',
    wrong: 'Подсказка: 3 × 5 = 15. Отсюда 15 ÷ 3 = 5.',
    rule: '15 ÷ ⬜ = 5 — используй умножение: 3 × 5 = 15.',
  },
};
function D20_08Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 1, label: '3' }, correct, meta: { tag: 'div_sub_fact', level: '🔴' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, ...S.mono, fontSize: 40, fontWeight: 800, color: C.sink }}>
          <span>15</span><span style={{ color: C.gold }}>÷</span>
          <BoxSub val={boxVal} boxCol={boxCol} picked={picked != null} pulse={picked == null && !checked} />
          <span style={{ color: C.gold }}>=</span><span>5</span>
        </div>
      </Stage>

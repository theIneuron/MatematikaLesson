// Dars 6 · Amaliyot 03 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D03_CHIPS = [45, 90, 54, 40], D03_CORRECT = 0;
const D03_T = {
  uz: {
    eyebrow: 'Ikki belgi orasi', setup: 'Kema 40 va 50 belgilar ORASIDA, aynan o‘rtasida.',
    ask: 'Kema 40 va 50 ning aynan o‘rtasida. Bu qaysi son?',
    correct: "To'g'ri. 40 va 50 o‘rtasi — 45.",
    wrong: "Maslahat: 40 dan besh birlik oldinga sanang — ikki o‘nlikning o‘rtasini toping.",
    rule: "Ikki o‘nlik o‘rtasi — 5 birlik: 40 va 50 orasi 45.",
  },
  ru: {
    eyebrow: 'Между отметками', setup: 'Корабль МЕЖДУ 40 и 50, ровно посередине.',
    ask: 'Корабль ровно между 40 и 50. Какое это число?',
    correct: 'Верно. Середина 40 и 50 — это 45.',
    wrong: 'Подсказка: отсчитай пять единиц от 40 — найди середину двух десятков.',
    rule: 'Середина двух десятков — 5 единиц: между 40 и 50 это 45.',
  },
};
function D06_03Impl(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D03_T[lang] || D03_T.uz;
  const order = React.useMemo(() => { const a = D03_CHIPS.map((_, i) => i); for (let k = a.length - 1; k > 0; k--) { const j = Math.floor(Math.random() * (k + 1)); const tmp = a[k]; a[k] = a[j]; a[j] = tmp; } return a; }, []);
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D03_CHIPS.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { idx: picked, label: String(D03_CHIPS[picked]) }, correctAnswer: { idx: 0, label: '45' }, correct, meta: { tag: 'between', level: '🟢' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <LineBase max={100}>
          <StaticTicks max={100} labels={[40, 50]} />
          <div className="d06-float" style={{ position: 'absolute', left: '45%', top: -2, transform: 'translateX(-50%)', fontSize: 28 }}>🚀</div>
          <div style={{ position: 'absolute', left: '45%', top: 52, transform: 'translateX(-50%)', ...S.mono, fontSize: 13, fontWeight: 800, color: C.acc }}>?</div>
        </LineBase>
      </Stage>
      <p style={{ ...S.ask, fontSize: 21 }}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {order.map((i) => { const on = picked === i; let tone = null; if (checked && on) tone = i === D03_CORRECT ? 'ok' : 'no'; return <Chip key={i} on={on && !checked} tone={tone} disabled={isReview || checked} onClick={() => setPicked(i)} w={64}>{D03_CHIPS[i]}</Chip>; })}
      </div>

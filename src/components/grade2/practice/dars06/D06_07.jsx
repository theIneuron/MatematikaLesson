  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D07_CHIPS.map((n, i) => { const on = picked === i; let tone = null; if (checked && on) tone = i === D07_CORRECT ? 'ok' : 'no'; return <Chip key={i} on={on && !checked} tone={tone} disabled={isReview || checked} onClick={() => setPicked(i)} w={64}>{n}</Chip>; })}
      </div>

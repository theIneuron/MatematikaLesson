// Dars 6 · Amaliyot 10 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D10_CORRECT = 40;
const D10_T = {
  uz: {
    eyebrow: 'Teng ikkiga bo‘ling', setup: 'Trassa 0 dan 80 gacha.',
    ask: '0 dan 80 gacha trassani teng ikkiga bo‘ladigan belgini bosing.',
    correct: "To'g'ri. 40 — o‘rtasi: 0–40 va 40–80 teng (har biri 40).",
    wrong: "Maslahat: teng ikki bo‘lak — o‘rta nuqta 0 va 80 dan teng uzoqlikda.",
    rule: "Teng ikkiga bo‘lish — o‘rta nuqta: 0 va 80 orasi 40.",
  },
  ru: {
    eyebrow: 'Раздели пополам', setup: 'Трасса от 0 до 80.',
    ask: 'Нажми отметку, которая делит трассу от 0 до 80 ровно пополам.',
    correct: 'Верно. 40 — середина: 0–40 и 40–80 равны (по 40).',
    wrong: 'Подсказка: две равные части — середина одинаково далеко от 0 и 80.',
    rule: 'Деление пополам — середина: между 0 и 80 это 40.',
  },
};
function D06_10Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value: picked }, correctAnswer: { value: D10_CORRECT }, correct, meta: { tag: 'halve', level: '🔴' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <LineBase max={80} h={78}>
          <ClickTicks max={80} labels={[0, 10, 20, 30, 40, 50, 60, 70, 80]} picked={picked} correctVal={D10_CORRECT} checked={checked} disabled={isReview || checked} onPick={setPicked} h={78} />
        </LineBase>
      </Stage>

// Dars 5 · Amaliyot 07 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D07_CHIPS = [8, 80, 18, 0], D07_CORRECT = 0;
const D07_T = {
  uz: {
    eyebrow: 'Nechta o‘nlik', setup: 'Displeyda yaxlit o‘nlik kod.',
    ask: '80 kodida nechta o‘nlik (dasta) bor?',
    correct: "To'g'ri. 80 = 8 o'nlik (8 dasta). Har dasta — 10.",
    wrong: "Maslahat: 80 = 8 ta o'ntalik. Nechta dasta 80 ni beradi?",
    rule: "Yaxlit o'nlik: 80 = 8 o'nlik. Birinchi raqam — o'nliklar soni.",
  },
  ru: {
    eyebrow: 'Сколько десятков', setup: 'На дисплее круглый код десятков.',
    ask: 'Сколько десятков (пачек) в коде 80?',
    correct: 'Верно. 80 = 8 десятков (8 пачек). Каждая пачка — 10.',
    wrong: 'Подсказка: 80 = 8 десятков. Сколько пачек дают 80?',
    rule: 'Круглый десяток: 80 = 8 десятков. Первая цифра — число десятков.',
  },
};
function D05_07Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D07_CHIPS.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { idx: picked, label: String(D07_CHIPS[picked]) }, correctAnswer: { idx: 0, label: '8' }, correct, meta: { tag: 'howmanytens', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ ...S.mono, fontSize: 52, fontWeight: 800, color: C.ten }}>80</div>
          <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', maxWidth: 180, justifyContent: 'center' }}>{Array.from({ length: 8 }).map((_, i) => <Cassette key={i} s={22} />)}</div>
        </div>
      </Stage>

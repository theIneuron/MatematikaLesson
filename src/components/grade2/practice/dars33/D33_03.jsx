// Dars 33 · Amaliyot 03 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 03 · TrueFalse · strelkalar ===== */
const D03 = makeTrueFalse({ tag:'tf_hands', level:'🟢', answer:true, stage:clockStage(2,0) });
D03.T = {
  uz: { eyebrow:"To'g'ri yoki noto'g'ri", setup:'Soatda ikkita strelka bor: biri uzun, biri kalta.', ask:'Soatning katta (uzun) strelkasi minutni, kichik (kalta) strelkasi soatni ko‘rsatadi. Bu fikr to‘g‘rimi?', yes:'Ha', no:'Yo‘q',
    correct:"To'g'ri javob: Ha. Uzun strelka — minut, kalta — soat.", wrong:'Qaysi strelka uzunroq? Aynan o‘sha minutni ko‘rsatadi.', rule:'Uzun strelka — minut, kalta strelka — soat.' },
  ru: { eyebrow:'Верно или неверно', setup:'На часах две стрелки: длинная и короткая.', ask:'Большая (длинная) стрелка показывает минуты, а маленькая (короткая) — часы. Верно?', yes:'Да', no:'Нет',
    correct:'Правильно: Да. Длинная — минуты, короткая — часы.', wrong:'Какая стрелка длиннее? Именно она показывает минуты.', rule:'Длинная — минуты, короткая — часы.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_33_03(props) {
  return (
    <React.Fragment>

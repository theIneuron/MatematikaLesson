// Dars 33 · Amaliyot 04 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 33;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN SCENE — vaqt / soat ============================== */
const ClockFace = ({ h, m, size=150, hourHand=null }) => {
  const cx=size/2, cy=size/2, R=size/2-8;
  const hh = hourHand!=null ? hourHand : h;
  const ha = ((hh%12)+ (m||0)/60)*30 - 90, ma = (m||0)*6 - 90;
  const hx=cx+Math.cos(ha*Math.PI/180)*R*0.5, hy=cy+Math.sin(ha*Math.PI/180)*R*0.5;
  const mx=cx+Math.cos(ma*Math.PI/180)*R*0.78, my=cy+Math.sin(ma*Math.PI/180)*R*0.78;
  return (<svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
    <circle cx={cx} cy={cy} r={R} fill="#0b1030" stroke="#FFC23C" strokeWidth="4" />
    {Array.from({length:12}).map((_,i)=>{ const a=(i+1)*30-90; const tx=cx+Math.cos(a*Math.PI/180)*R*0.82, ty=cy+Math.sin(a*Math.PI/180)*R*0.82;
      return <text key={i} x={tx} y={ty+5} fontSize="13" fontWeight="800" textAnchor="middle" fill="#EAF0F8" fontFamily="monospace">{i+1}</text>; })}
    <motion.line x1={cx} y1={cy} x2={hx} y2={hy} stroke="#5BD6F2" strokeWidth="5" strokeLinecap="round" initial={{opacity:0}} animate={{opacity:1}} />
    <motion.line x1={cx} y1={cy} x2={mx} y2={my} stroke="#FF9A6B" strokeWidth="3.5" strokeLinecap="round" initial={{opacity:0}} animate={{opacity:1}} />
    <circle cx={cx} cy={cy} r={5} fill="#FFC23C" />
  </svg>);
};

/* ---- IMZO: SOAT-STRELKA (ClockSet) — kerakli soatni bosib qo'yish ---- */
function makeClockSet(cfg) {
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review';
    ...
    const check=useCallback(()=>{ const correct=pick===cfg.hour; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:[], studentAnswer:{value:pick}, correctAnswer:{value:cfg.hour}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[pick,t,playCorrect,playWrong,onSubmit]);
    const cref=useRef(check); cref.current=check; useEffect(()=>{ registerCheck?.(()=>cref.current()); },[registerCheck]);
    const size=210, cx=size/2, cy=size/2, R=size/2-10;
      ...
      <div style={S.wrap}>
        ...
        <Stage>
          <svg width="100%" viewBox={`0 0 ${size} ${size}`} style={{maxWidth:size,display:'block',margin:'0 auto'}}>
            <circle cx={cx} cy={cy} r={R} fill="#0b1030" stroke="#FFC23C" strokeWidth="4" />
            {/* minut strelkasi 12 da (00 daqiqa) */}
            <line x1={cx} y1={cy} x2={cx} y2={cy-R*0.78} stroke="#FF9A6B" strokeWidth="3.5" strokeLinecap="round" />
            {/* soat strelkasi tanlangan songa */}
            {pick!=null && (()=>{ const a=(pick%12)*30-90; return <motion.line x1={cx} y1={cy} x2={cx+Math.cos(a*Math.PI/180)*R*0.5} y2={cy+Math.sin(a*Math.PI/180)*R*0.5} stroke={col} strokeWidth="6" strokeLinecap="round" animate={{rotate:0}} />; })()}
            <circle cx={cx} cy={cy} r={5} fill="#FFC23C" />
            {Array.from({length:12}).map((_,i)=>{ const hn=i+1; const a=hn*30-90; const tx=cx+Math.cos(a*Math.PI/180)*R*0.82, ty=cy+Math.sin(a*Math.PI/180)*R*0.82; const on=pick===(hn%12);
              return (<g key={i} style={{cursor:(isReview||checked)?'default':'pointer'}} onClick={()=>{ if(!isReview&&!checked) setPick(hn%12); }}>
                <circle cx={tx} cy={ty} r={15} fill={on?col:'transparent'} />
                <text x={tx} y={ty+5} fontSize="14" fontWeight="800" textAnchor="middle" fill={on?'#fff':'#EAF0F8'} fontFamily="monospace">{hn}</text>
              </g>); })}
          </svg>
          <div style={{ textAlign:'center', marginTop:4, fontSize:13.5, fontWeight:700, color:C.goldSoft }}>{t.hint}</div>
        </Stage>
        {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
        {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
      </div>
...
const clockStage = (h,m) => ((t)=>(<div style={{display:'flex',justifyContent:'center'}}><ClockFace h={h} m={m} /></div>));

/* ===== 04 · IMZO ClockSet · 6:00 ===== */
const D04 = makeClockSet({ tag:'set6', level:'🟡', hour:6 });
D04.T = {
  uz: { eyebrow:'Soatni qo‘yish', setup:'Katta (minut) strelka 12 da turibdi. Endi soat strelkasini to‘g‘ri qo‘yamiz.', ask:'Soatni 6:00 ga qo‘ying: soat strelkasi ko‘rsatishi kerak bo‘lgan sonni bosing.', hint:'6 raqamini bosing', correct:"To'g'ri. Soat strelkasi 6 da, minut strelkasi 12 da — 6:00.", wrong:'6:00 da soat strelkasi qaysi son ustida turadi? O‘sha sonni bosing.', rule:'6:00 → soat strelkasi 6 da, minut 12 da.' },
  ru: { eyebrow:'Установи часы', setup:'Большая (минутная) стрелка на 12. Теперь поставим часовую стрелку.', ask:'Установи 6:00: нажми число, на которое должна указывать часовая стрелка.', hint:'нажми число 6', correct:'Верно. Часовая на 6, минутная на 12 — 6:00.', wrong:'На каком числе часовая стрелка при 6:00? Нажми его.', rule:'6:00 → часовая на 6, минутная на 12.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_33_04(props) {
  return (
    <React.Fragment>

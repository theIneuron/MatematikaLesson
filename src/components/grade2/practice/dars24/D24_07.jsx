// Dars 24 · Amaliyot 07 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 07 · Javobni ter · 🟡 · numpad =================== */
const D07_ANS = 4;
const D07_T = {
  uz: { eyebrow: 'Javobni ter', setup: "«Jasur 28 ta kristalni 7 do'stiga teng uladi.»", ask: 'Har do‘stga nechta tegadi? Raqamni bosib tering:', correct: "To'g'ri. 28 ÷ 7 = 4. (7 × 4 = 28)", wrong: "Maslahat: teng ulash — bo'lish. 7 ni nechaga ko'paytirsak 28 bo'ladi?", rule: "28 ÷ 7 = 4." },
  ru: { eyebrow: 'Набери ответ', setup: '«Джасур разделил 28 кристаллов между 7 друзьями поровну.»', ask: 'Сколько достанется каждому? Набери цифру:', correct: 'Верно. 28 ÷ 7 = 4. (7 × 4 = 28)', wrong: 'Подсказка: поровну — деление. На что умножить 7, чтобы вышло 28?', rule: '28 ÷ 7 = 4.' },
};
function D24_07(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  ...
  const check = useCallback(() => { const correct = parseInt(val, 10) === D07_ANS; setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value: parseInt(val, 10) }, correctAnswer: { value: D07_ANS }, correct, meta: { tag: 'numpad_word', level: '🟡' } }); }, [val, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage>{CryRow(7, 4)}<div className="d24-float" style={{ textAlign: 'center', marginTop: 8, ...S.mono, fontSize: 26, fontWeight: 800, color: C.gold }}>28 ÷ 7 = ?</div></Stage>
...
const __FX_CSS = `
      ...
      `;
export default function Task_24_07(props) {
  return (
    <React.Fragment>

// Dars 24 · Amaliyot 08 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 08 · Qaysi masala bo'lish? · 🔴 · whichdiv (multiselect) =================== */
const D08_OPTS = [
  { e: '24 kristalni 4 do‘stga teng ulash', ok: true },
  { e: 'Har qutiga 5 tadan, 30 ta uchun nechta quti', ok: true },
  { e: 'Har vagonchada 6, 3 vagoncha — jami qancha', ok: false },
  { e: '18 kristalga yana 6 ta qo‘shildi', ok: false },
];
const D08_T = {
  uz: { eyebrow: 'Qaysi bo‘lish?', setup: "To'rt masala. Qaysilari BO'LISH bilan yechiladi?", ask: 'BO‘LISH masalalarini belgilang (bir nechta):', correct: "To'g'ri. «Teng ulash» va «har qutiga N tadan» — bo'lish. «Jami» — ×, «qo'shildi» — +.", wrong: "Maslahat: teng ulash va teng guruhlarga bo'lish — bo'lish. «Jami» — ko'paytirish, «qo'shildi» — qo'shish.", rule: "Bo'lish kalitlari: «teng ulash», «har biriga/qutiga N tadan»." },
  ru: { eyebrow: 'Где деление?', setup: 'Четыре задачи. Какие решаются ДЕЛЕНИЕМ?', ask: 'Отметь задачи на ДЕЛЕНИЕ (несколько):', correct: 'Верно. «Поровну» и «по N в коробку» — деление. «Всего» — ×, «добавили» — +.', wrong: 'Подсказка: поровну и на равные группы — деление. «Всего» — умножение, «добавили» — сложение.', rule: 'Ключи деления: «поровну», «по N в каждую».' },
};
function D24_08(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D08_T[lang] || D08_T.uz; const isReview = mode === 'review';
  ...
  const check = useCallback(() => { const want = D08_OPTS.map((o, i) => o.ok ? i : -1).filter((x) => x >= 0); const correct = sel.length === want.length && want.every((i) => sel.includes(i)); setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: D08_OPTS.map((o, i) => ({ id: String(i), label: o.e })), studentAnswer: { sel }, correctAnswer: { sel: want }, correct, meta: { tag: 'whichdiv', level: '🔴' } }); }, [sel, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const box = (i) => { const on = sel.includes(i), o = D08_OPTS[i]; let bd = C.line, bg = C.paper, col = C.ink; if (on) { bd = C.acc; bg = C.accSoft; } if (checked && on) { bd = o.ok ? C.ok : C.no; bg = o.ok ? C.okSoft : C.noSoft; col = o.ok ? C.ok : C.no; } return (<button key={i} type="button" disabled={isReview || checked} onClick={() => toggle(i)} style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', padding: '13px 14px', borderRadius: 13, border: '2px solid ' + bd, background: bg, color: col, fontSize: 15, fontWeight: 700, fontFamily: 'inherit', textAlign: 'left', cursor: (isReview || checked) ? 'default' : 'pointer', marginBottom: 9 }}><span style={{ flex: '0 0 auto', width: 24, height: 24, borderRadius: 7, border: '2px solid ' + (on ? bd : C.ink3), background: on ? bd : '#fff', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15 }}>{on ? '✓' : ''}</span>{o.e}</button>); };
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <p style={S.ask}>{t.ask}</p>
      {D08_OPTS.map((_, i) => box(i))}
      {fb && <FB ok={fb.correct} text={fb.correct ? t.correct : t.wrong} />}{checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
    </div>
  );
}
...
const __FX_CSS = `
      ...
      `;
export default function Task_24_08(props) {
  return (
    <React.Fragment>

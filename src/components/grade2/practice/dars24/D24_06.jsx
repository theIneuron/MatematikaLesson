// Dars 24 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 06 · Ifodani yig' · 🟡 · buildeq (drag) =================== */
const D06_SLOTS = [18, 3, 6]; // 18 ÷ 3 = 6
const D06_POOL = [18, 3, 6, 24];
const D06_T = {
  uz: { eyebrow: 'Ifodani yig‘', setup: "«18 ta kristal 3 vagonchaga teng ulanadi.» Ifodani tuzing.", ask: 'Raqamni bosing, keyin bo‘sh joyga qo‘ying: ⬜ ÷ ⬜ = ⬜', correct: "To'g'ri. 18 ÷ 3 = 6. Har vagonchada 6 tadan.", wrong: "Maslahat: bo'linuvchi (jami) — 18, bo'luvchi (vagoncha) — 3, javob — 6.", rule: "18 ÷ 3 = 6." },
  ru: { eyebrow: 'Составь выражение', setup: '«18 кристаллов делят поровну в 3 вагонетки.» Составь выражение.', ask: 'Нажми число, затем поставь в пустое место: ⬜ ÷ ⬜ = ⬜', correct: 'Верно. 18 ÷ 3 = 6. По 6 в каждой.', wrong: 'Подсказка: делимое (всего) — 18, делитель (вагонетки) — 3, ответ — 6.', rule: '18 ÷ 3 = 6.' },
};
function D24_06(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D06_T[lang] || D06_T.uz; const isReview = mode === 'review';
  const [slots, setSlots] = useState([null, null, null]); const [pick, setPick] = useState(null);
  const [fb, setFb] = useState(null); const [checked, setChecked] = useState(false);
  ...
  const used = {}; slots.forEach((v) => { if (v != null) used[v] = (used[v] || 0) + 1; });
  const clickSlot = (k) => { if (locked) return; if (pick != null) { setSlots((s) => { const n = s.slice(); n[k] = pick; return n; }); setPick(null); } else if (slots[k] != null) { setSlots((s) => { const n = s.slice(); n[k] = null; return n; }); } };
  const check = useCallback(() => { const correct = slots.every((v, i) => v === D06_SLOTS[i]); setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.(); onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { slots }, correctAnswer: { slots: D06_SLOTS }, correct, meta: { tag: 'buildeq', level: '🟡' } }); }, [slots, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const slotBox = (k) => {
    const v = slots[k]; let bd = v != null ? C.acc : C.sink2, cl = v != null ? '#fff' : C.sink2, bg = v != null ? 'rgba(255,79,40,.18)' : C.stile;
    if (checked) { const okv = v === D06_SLOTS[k]; bd = okv ? C.ok : C.no; cl = okv ? '#9df0bd' : '#ffb4a8'; bg = okv ? 'rgba(31,122,77,.25)' : 'rgba(192,57,43,.25)'; }
    return <button type="button" disabled={locked} onClick={() => clickSlot(k)} style={{ width: 60, height: 60, borderRadius: 12, border: '2px ' + (v != null ? 'solid' : 'dashed') + ' ' + bd, background: bg, ...S.mono, fontSize: 26, fontWeight: 800, color: cl, cursor: locked ? 'default' : 'pointer' }}>{v != null ? v : ''}</button>;
  };
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div><p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, ...S.mono, fontSize: 34, fontWeight: 800, color: C.gold }}>{slotBox(0)}<span>÷</span>{slotBox(1)}<span>=</span>{slotBox(2)}</div></Stage>
      <p style={{ ...S.ask, fontSize: 15 }}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D06_POOL.map((n, i) => { const cnt = slots.filter((x) => x === n).length; const avail = cnt < 1; if (!avail) return <span key={i} style={{ width: 58, height: 54, borderRadius: 12, border: '2px dashed ' + C.line, background: '#fafafa', display: 'inline-block' }} />; return <button key={i} type="button" disabled={locked} onClick={() => setPick(pick === n ? null : n)} style={{ width: 58, height: 54, borderRadius: 12, border: '2px solid ' + (pick === n ? C.acc : C.line), background: pick === n ? C.accSoft : C.paper, ...S.mono, fontSize: 22, fontWeight: 800, color: C.ink, cursor: locked ? 'default' : 'pointer', boxShadow: pick === n ? '0 0 0 4px #FFE0D6' : 'none' }}>{n}</button>; })}
      </div>
      {fb && <FB ok={fb.correct} text={fb.correct ? t.correct : t.wrong} />}{checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
    </div>
  );
...
const __FX_CSS = `
      ...
      `;
export default function Task_24_06(props) {
  return (
    <React.Fragment>

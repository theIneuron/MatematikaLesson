// Dars 31 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 31;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN SCENE — tenglama / tarozi ============================== */
/* ---- IMZO: TAROZI-MUVOZANAT (Balance) ---- */
function makeBalance(cfg) {
  const leftVal=(x)=> cfg.rev ? (cfg.k - x) : (cfg.op==='-'? x-cfg.k : x+cfg.k);
  const _qi = _QSEQ++;
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review';
    ...
    useEffect(()=>{ if(initialAnswer?.studentAnswer?.idx!=null){ setPicked(initialAnswer.studentAnswer.idx); if(typeof initialAnswer.correct==='boolean'){ setFb({correct:initialAnswer.correct}); setChecked(true); } } },[initialAnswer]);
    useEffect(()=>{ onReady?.(picked!=null && !checked); },[picked,checked,onReady]);
    const _corig = cfg.xvals.findIndex(x=>leftVal(x)===cfg.target); const xvals = orderTarget(cfg, cfg.xvals.length, _corig, _qi).map((i)=>cfg.xvals[i]); const correctIdx = xvals.findIndex(x=>leftVal(x)===cfg.target);
    const check=useCallback(()=>{ const correct=picked===correctIdx; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:xvals.map(String), studentAnswer:{idx:picked,value:picked!=null?xvals[picked]:null}, correctAnswer:{value:xvals[correctIdx]}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[picked,correctIdx,t,playCorrect,playWrong,onSubmit]);
    const cref=useRef(check); cref.current=check; useEffect(()=>{ registerCheck?.(()=>cref.current()); },[registerCheck]);
    const diff = picked==null?0:(leftVal(xvals[picked])-cfg.target);
    const angle = checked&&fb?.correct?0:Math.max(-16,Math.min(16,diff*5));
    const balanced = picked!=null && diff===0;
    return (
      <div style={S.wrap}>
        <div style={S.eyebrow}>{t.eyebrow}</div>
        <p style={S.setup}>{t.setup}</p>
        <Stage>
          <svg width="100%" viewBox="0 0 260 150" style={{ maxWidth:260, display:'block', margin:'0 auto' }}>
            <line x1="130" y1="24" x2="130" y2="120" stroke={C.sink2} strokeWidth="4" />
            <polygon points="112,124 148,124 130,140" fill={C.sink2} />
            <motion.g animate={{ rotate:angle }} transition={{ type:'spring', stiffness:120 }} style={{ originX:'130px', originY:'28px' }}>
              <line x1="40" y1="28" x2="220" y2="28" stroke={C.gold} strokeWidth="5" strokeLinecap="round" />
              <circle cx="130" cy="28" r="6" fill={C.gold} />
              <line x1="40" y1="28" x2="40" y2="52" stroke={C.sink2} strokeWidth="2" />
              <line x1="220" y1="28" x2="220" y2="52" stroke={C.sink2} strokeWidth="2" />
              <rect x="8" y="52" width="64" height="30" rx="8" fill={(balanced||!checked)?'rgba(255,79,40,.2)':'rgba(192,57,43,.2)'} stroke={C.acc} strokeWidth="2" />
              <text x="40" y="72" textAnchor="middle" fill={C.sink} fontSize="16" fontWeight="800" fontFamily="monospace">{cfg.leftLabel(picked!=null?xvals[picked]:null)}</text>
              <rect x="188" y="52" width="64" height="30" rx="8" fill="rgba(124,224,163,.18)" stroke={C.leaf} strokeWidth="2" />
              <text x="220" y="72" textAnchor="middle" fill={C.sink} fontSize="16" fontWeight="800" fontFamily="monospace">{cfg.target}</text>
            </motion.g>
          </svg>
          <div style={{ textAlign:'center', marginTop:4, fontSize:13.5, fontWeight:700, color: balanced?C.leaf:C.goldSoft }}>{balanced?t.level:t.hint}</div>
        </Stage>
        <p style={S.ask}>{t.ask}</p>
        <div style={{ display:'flex', flexWrap:'wrap', gap:9 }}>
          {xvals.map((v,i)=><button key={i} type="button" className="px-rise px-press" style={{ ...optStyle(picked,i,correctIdx,checked,isReview,{half:true,center:true,mono:true,fs:22}), animationDelay:(i*0.05)+'s' }} disabled={isReview||checked} onClick={()=>setPicked(i)}>x = {v}</button>)}
        </div>
        {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
        {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
      </div>
...
const eqStage = (str) => ((t)=>(<div style={{ textAlign:'center', ...S.mono, fontSize:36, fontWeight:800, color:C.gold }}>{str}</div>));

/* ===== 06 · Find-wrong · x+5=12 ===== */
const D06 = makeFindwrong({ tag:'eq_wrong', level:'🟡', colors:['#FFC23C','#5BD6F2','#7CE0A3','#FF9A6B'] });
D06.T = {
  uz: { eyebrow:'Xatoni top', setup:'x + 5 = 12 tenglamasini bir necha o‘quvchi yechdi.', ask:'x + 5 = 12 ni yechishda qaysi qatorda XATO qilingan?',
    rows:[{ txt:'x = 12 − 5', ok:true }, { txt:'x = 7', ok:true }, { txt:'x = 12 + 5', ok:false }, { txt:'tekshir: 7 + 5 = 12', ok:true }],
    correct:"To'g'ri. x = 12 + 5 xato — qo‘shish o‘rniga ayirish kerak. To‘g‘risi x = 12 − 5 = 7.", wrong:'x ni topish uchun teskari amal: qo‘shilgan sonni AYIRING, qo‘shmang.', rule:'x + 5 = 12 → x = 12 − 5 = 7 (12 + 5 emas).' },
  ru: { eyebrow:'Найди ошибку', setup:'Уравнение x + 5 = 12 решали несколько учеников.', ask:'В какой строке при решении x + 5 = 12 допущена ОШИБКА?',
    rows:[{ txt:'x = 12 − 5', ok:true }, { txt:'x = 7', ok:true }, { txt:'x = 12 + 5', ok:false }, { txt:'проверка: 7 + 5 = 12', ok:true }],
    correct:'Верно. x = 12 + 5 — ошибка: нужно вычитать, а не прибавлять. Верно x = 12 − 5 = 7.', wrong:'Чтобы найти x, обратное действие: прибавленное число ВЫЧТИ.', rule:'x + 5 = 12 → x = 12 − 5 = 7 (не 12 + 5).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_31_06(props) {
  return (
    <React.Fragment>

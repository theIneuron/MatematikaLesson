// Dars 39 · Amaliyot 10 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 39;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN — yakuniy takrorlash ============================== */
const bigStage = (str, col) => ((t)=>(<div style={{ textAlign:'center', ...S.mono, fontSize:34, fontWeight:800, color: col||C.gold }}>{str}</div>));
const SqStage = (t)=>(<div style={{display:'flex',justifyContent:'center'}}><svg width="130" height="120" viewBox="0 0 130 120"><rect x="30" y="20" width="70" height="70" fill="rgba(255,194,60,.14)" stroke="#FFC23C" strokeWidth="3"/><text x="65" y="14" fill="#EAF0F8" fontSize="14" fontWeight="800" textAnchor="middle">5 sm</text></svg></div>);

/* ===== 10 · Find-wrong · aralash ===== */
const D10 = makeFindwrong({ tag:'r_wrong', level:'🔴', colors:['#FFC23C','#5BD6F2','#7CE0A3','#FF9A6B'] });
D10.T = {
  uz: { eyebrow:'Xatoni top', setup:'To‘rt xil hisob, bittasi noto‘g‘ri.', ask:'Qaysi qatorda XATO bor?',
    rows:[{ txt:'1 hafta = 7 kun', ok:true }, { txt:'1 m = 100 sm', ok:true }, { txt:'6 × 2 = 12', ok:true }, { txt:'x + 5 = 12 → x = 17', ok:false }],
    correct:"To'g'ri. x = 12 − 5 = 7 bo‘lishi kerak, 17 emas.", wrong:'Har birini tekshiring. Tenglamada x ni topish uchun qo‘shilganni AYIRING.', rule:'x + 5 = 12 → x = 7 (17 emas).' },
  ru: { eyebrow:'Найди ошибку', setup:'Четыре разных расчёта, один неверен.', ask:'В какой строке ОШИБКА?',
    rows:[{ txt:'1 неделя = 7 дней', ok:true }, { txt:'1 м = 100 см', ok:true }, { txt:'6 × 2 = 12', ok:true }, { txt:'x + 5 = 12 → x = 17', ok:false }],
    correct:'Верно. Должно быть x = 12 − 5 = 7, а не 17.', wrong:'Проверь каждую. Чтобы найти x, прибавленное ВЫЧТИ.', rule:'x + 5 = 12 → x = 7 (не 17).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_39_10(props) {
  return (
    <React.Fragment>

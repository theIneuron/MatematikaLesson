// Dars 7 · Amaliyot 05 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
// 3⬜ + 24 = 59 -> ⬜ = 5
const D05_TILES = [5, 9, 4, 3];
const D05_T = {
  uz: {
    eyebrow: 'Yetishmagan raqam', setup: 'Stolbik: 3⬜ + 24 = 59. Bir raqam yashiringan.',
    ask: 'Birinchi sonning yetishmagan birlik raqamini toping: 3⬜ + 24 = 59.',
    correct: "To'g'ri. 35 + 24 = 59 (birlik 5 + 4 = 9).",
    wrong: "Maslahat: birliklar ustuniga qarang. ⬜ + 4 = 9 bo‘lishi uchun ⬜ nechchi?",
    rule: "Bir ustun ma'lum bo‘lsa, yetishgan raqamni topamiz: ⬜ + 4 = 9 → 5.",
  },
  ru: {
    eyebrow: 'Недостающая цифра', setup: 'Столбик: 3⬜ + 24 = 59. Одна цифра скрыта.',
    ask: 'Найди недостающую цифру единиц первого числа: 3⬜ + 24 = 59.',
    correct: 'Верно. 35 + 24 = 59 (единицы 5 + 4 = 9).',
    wrong: 'Подсказка: смотри на столбец единиц. Чтобы ⬜ + 4 = 9, чему равно ⬜?',
    rule: 'Если столбец известен, находим цифру: ⬜ + 4 = 9 → 5.',
  },
};
function D07_05Impl(props) {
  ...
  const check = useCallback(() => {
    const correct = picked != null && D05_TILES[picked] === 5;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: D05_TILES.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { idx: picked, digit: picked != null ? D05_TILES[picked] : null }, correctAnswer: { digit: 5 }, correct, meta: { tag: 'missingdigit', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <Colon>
            <ColHeader />
            <Row sign="" tenNode={<DC ch="3" c={C.ten} />} oneNode={<BoxCell ch={picked != null ? D05_TILES[picked] : '?'} tone={boxTn} />} />
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D05_TILES.map((n, i) => { const on = picked === i; let tone = null; if (checked && on) tone = D05_TILES[i] === 5 ? 'ok' : 'no'; return <Chip key={i} on={on && !checked} tone={tone} disabled={isReview || checked} onClick={() => setPicked(i)} w={60}>{n}</Chip>; })}
      </div>

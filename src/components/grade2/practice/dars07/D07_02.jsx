// Dars 7 · Amaliyot 02 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
// 34 + 2⬜ = 59. Yashirin birlik = 5. Variantlar IFODA ko'rinishida (2+3=5).
const D02_OPTS = ['2 + 3', '3 + 3', '2 + 2', '1 + 2'];
const D02_VALS = [5, 6, 4, 3];
const D02_CORRECT = 0;
const D02_T = {
  uz: {
    eyebrow: 'Yashirin birlik', setup: 'Stolbik: 34 + 2⬜ = 59. Birlik raqami yashiringan.',
    ask: 'Yashirin birlik raqamini toping. Variantlar — ifoda ko‘rinishida.',
    correct: "To'g'ri. Birliklar: 4 + 5 = 9, demak yashirin raqam 5. (2 + 3 = 5)",
    wrong: "Maslahat: birliklar ustuniga qarang — 4 + ⬜ = 9. Qaysi ifoda mos qiymat beradi?",
    rule: "Birliklar: 4 + 5 = 9. Ifoda 2 + 3 = 5 to‘g‘ri keladi.",
  },
  ru: {
    eyebrow: 'Скрытая единица', setup: 'Столбик: 34 + 2⬜ = 59. Цифра единиц скрыта.',
    ask: 'Найди скрытую цифру единиц. Варианты — в виде выражений.',
    correct: 'Верно. Единицы: 4 + 5 = 9, значит скрытая цифра 5. (2 + 3 = 5)',
    wrong: 'Подсказка: смотри на столбец единиц — 4 + ⬜ = 9. Какое выражение даёт нужное значение?',
    rule: 'Единицы: 4 + 5 = 9. Выражение 2 + 3 = 5 подходит.',
  },
};
function D07_02Impl(props) {
  ...
  const check = useCallback(() => {
    const correct = picked != null && D02_VALS[picked] === 5;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: D02_OPTS.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: picked != null ? D02_OPTS[picked] : null, value: picked != null ? D02_VALS[picked] : null }, correctAnswer: { digit: 5, label: '2 + 3' }, correct, meta: { tag: 'exprdigit', level: '🟢' } });
  }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const boxTn = checked ? (fb?.correct ? 'ok' : 'no') : (picked != null ? 'sel' : null);
  const boxVal = picked != null ? D02_VALS[picked] : '?';
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <Colon>
            ...
            <Row sign="+" tenNode={<DC ch="2" c={C.ten} />} oneNode={<BoxCell ch={boxVal} tone={boxTn} />} />
            <HR />
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D02_OPTS.map((o, i) => {
          ...
          if (show) { const okv = i === D02_CORRECT; bd = okv ? C.ok : C.no; bg = okv ? C.okSoft : C.noSoft; col = okv ? C.ok : C.no; }

// Dars 16 · Amaliyot 09 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D09_T = {
  uz: {
    eyebrow: 'Masala', setup: "Madina issiqxonaga lotoklar tayyorladi: har lotokda 7 ta ko'chat, jami 6 ta lotok.",
    ask: 'Hammasi bo‘lib nechta ko‘chat bor?',
    opts: ['13', '36', '42', '48'],
    correct: "To'g'ri. 7 ta ko'chatdan 6 lotok: 7 × 6 = 42.",
    wrong: "Maslahat: 7 ni 6 marta oling. 7,14,21,28,35,42. 7+6=13 — bu qo'shish.",
    rule: "«Har birida A, jami B ta» — bu A × B ko'paytma.",
  },
  ru: {
    eyebrow: 'Задача', setup: 'Мадина приготовила лотки в теплицу: в каждом 7 саженцев, всего 6 лотков.',
    ask: 'Сколько всего саженцев?',
    opts: ['13', '36', '42', '48'],
    correct: 'Верно. По 7 саженцев в 6 лотках: 7 × 6 = 42.',
    wrong: 'Подсказка: возьми 7 шесть раз. 7,14,21,28,35,42. 7+6=13 — это сложение.',
    rule: '«По A в каждом, всего B» — это произведение A × B.',
  },
};
function D16_09Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 2, label: '42' }, correct, meta: { tag: 'mul_word', level: '🔴' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 6, justifyContent: 'center', flexWrap: 'wrap' }}>
          {Array.from({ length: 6 }).map((_, g) => (
            <div key={g} style={{ padding: 5, borderRadius: 9, border: '1.5px solid ' + C.green, background: 'rgba(110,242,155,.10)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 2 }}>{Array.from({ length: 7 }).map((_, i) => <span key={i} className="d16-sprout" style={{ width: 8, height: 8, borderRadius: '50% 50% 50% 0', background: C.green, transform: 'rotate(45deg)', animationDelay: ((g * 7 + i) * 0.02) + 's' }} />)}</div>
            </div>
          ))}
        </div>
      </Stage>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D09_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 22 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

// Dars 16 · Amaliyot 02 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D02_T = {
  uz: {
    eyebrow: '×6 jadvali', setup: "Bit har lotokka 6 tadan urug' ekdi — 4 ta lotok.",
    ask: '6 × 4 nechaga teng?', expr: '6 × 4',
    opts: ['10', '20', '24', '30'],
    correct: "To'g'ri. 6 × 4 = 6+6+6+6 = 24.",
    wrong: "Maslahat: 6 ni 4 marta oling: 6, 12, 18, 24. 6+4=10 — bu qo'shish.",
    rule: "6 × 4 — 6 ni 4 marta olish, 6+4 qo'shish emas.",
  },
  ru: {
    eyebrow: 'Таблица ×6', setup: 'Бит посадил по 6 семян в лоток — 4 лотка.',
    ask: 'Чему равно 6 × 4?', expr: '6 × 4',
    opts: ['10', '20', '24', '30'],
    correct: 'Верно. 6 × 4 = 6+6+6+6 = 24.',
    wrong: 'Подсказка: возьми 6 четыре раза: 6, 12, 18, 24. 6+4=10 — это сложение.',
    rule: '6 × 4 — это 6, взятое 4 раза, а не 6+4.',
  },
};
function D16_02Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 2, label: '24' }, correct, meta: { tag: 'mul6_mc', level: '🟢' } });
      ...
      <Stage>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'center' }}>
          {Array.from({ length: 4 }).map((_, r) => (
            <div key={r} style={{ padding: '5px 8px', borderRadius: 9, border: '1.5px solid ' + C.green, background: 'rgba(110,242,155,.10)' }}><div style={{ display: 'flex', gap: 4 }}>{Array.from({ length: 6 }).map((_, c) => <span key={c} className="d16-sprout" style={{ width: 13, height: 13, borderRadius: '50% 50% 50% 0', background: C.green, transform: 'rotate(45deg)', animationDelay: ((r * 6 + c) * 0.04) + 's' }} />)}</div></div>
          ))}
        </div>
        <div className="d16-float" style={{ textAlign: 'center', marginTop: 8, ...S.mono, fontSize: 34, fontWeight: 800, color: C.gold }}>{t.expr}</div>
      </Stage>

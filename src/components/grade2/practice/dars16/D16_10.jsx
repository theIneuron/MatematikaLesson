// Dars 16 · Amaliyot 10 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
// tap raqam-plita (klaviaturasiz). value — string.
...
const D10_T = {
  uz: {
    eyebrow: 'Javobni ter', setup: "Eng katta lotok: 6 ta qator, har qatorda 7 ta ko'chat.",
    ask: '6 × 7 nechaga teng? Raqamlarni bosib javobni tering:',
    correct: "To'g'ri. 6 × 7 = 42. (7 ni 6 marta: 7,14,21,28,35,42)",
    wrong: "Maslahat: 7 ni 6 marta sanang — 7,14,21,28,35,42.",
    rule: "6 × 7 = 7 × 6 = 42. Ikkala jadvaldan ham chiqadi.",
  },
  ru: {
    eyebrow: 'Набери ответ', setup: 'Самый большой лоток: 6 рядов, в каждом по 7 саженцев.',
    ask: 'Чему равно 6 × 7? Набери ответ, нажимая цифры:',
    correct: 'Верно. 6 × 7 = 42. (7 шесть раз: 7,14,21,28,35,42)',
    wrong: 'Подсказка: посчитай 7 шесть раз — 7,14,21,28,35,42.',
    rule: '6 × 7 = 7 × 6 = 42. Выходит из обеих таблиц.',
  },
};
function D16_10Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value: parseInt(val, 10) }, correctAnswer: { value: D10_ANS }, correct, meta: { tag: 'mul_numpad', level: '🔴' } });
      ...
      <Stage>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: 'center' }}>
          {Array.from({ length: 6 }).map((_, r) => (
            <div key={r} style={{ display: 'flex', gap: 3 }}>{Array.from({ length: 7 }).map((_, c) => <span key={c} className="d16-sprout" style={{ width: 12, height: 12, borderRadius: '50% 50% 50% 0', background: C.green, transform: 'rotate(45deg)', animationDelay: ((r * 7 + c) * 0.02) + 's' }} />)}</div>
          ))}
        </div>
        <div className="d16-float" style={{ textAlign: 'center', marginTop: 8, ...S.mono, fontSize: 30, fontWeight: 800, color: C.gold }}>6 × 7</div>
      </Stage>

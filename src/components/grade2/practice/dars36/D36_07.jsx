// Dars 36 · Amaliyot 07 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 36;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN — kattaliklarga masala ============================== */
/* ---- IMZO: BIRLIKLI JAVOB (UnitBuild) — son (raqam-plita) + birlik (chip) ---- */
function makeUnitBuild(cfg) {
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review'; const cells=cfg.cells;
    const [built,setBuilt]=useState(''),[unit,setUnit]=useState(null),[fb,setFb]=useState(null),[checked,setChecked]=useState(false);
    useEffect(()=>{ const sa=initialAnswer?.studentAnswer; if(sa){ if(sa.value!=null) setBuilt(String(sa.value)); if(sa.unit!=null) setUnit(sa.unit); if(typeof initialAnswer.correct==='boolean'){ setFb({correct:initialAnswer.correct}); setChecked(true); } } },[initialAnswer]);
    useEffect(()=>{ onReady?.(built.length===cells && unit!=null && !checked); },[built,unit,checked,onReady]);
    const tap=(d)=>{ if(isReview||checked) return; setBuilt(b=>b.length>=cells?b:b+d); };
    const back=()=>{ if(isReview||checked) return; setBuilt(b=>b.slice(0,-1)); };
    const check=useCallback(()=>{ const correct=built!=='' && Number(built)===cfg.answer && unit===cfg.unitIdx; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:t.units, studentAnswer:{value:built===''?null:Number(built),unit}, correctAnswer:{value:cfg.answer,unit:cfg.unitIdx}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[built,unit,t,playCorrect,playWrong,onSubmit]);
    ...
    return (<div style={S.wrap}>
      ...
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14 }}>
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          {Array.from({length:cells}).map((_,i)=>(<motion.div key={i} animate={built[i]?{scale:[1,1.15,1]}:{}} style={{ width:50, height:60, borderRadius:12, border:'2px solid '+(built[i]?cBd:C.line), background:built[i]?cBg:C.paper, display:'flex', alignItems:'center', justifyContent:'center', ...S.mono, fontSize:28, fontWeight:800, color:cCol }}>{built[i]||''}</motion.div>))}
          <span style={{ ...S.mono, fontSize:20, fontWeight:800, color:C.ink2, minWidth:70 }}>{unit!=null?t.units[unit]:'…'}</span>
        </div>
        <div style={{ display:'flex', gap:7, flexWrap:'wrap', justifyContent:'center' }}>
          {cfg.digits.map((d,i)=>(<motion.button key={i} type="button" whileTap={{scale:.9}} disabled={isReview||checked} onClick={()=>tap(d)} style={{ width:52, height:50, borderRadius:12, border:'2px solid '+C.line, background:C.paper, ...S.mono, fontSize:22, fontWeight:800, color:C.ink, cursor:(isReview||checked)?'default':'pointer' }}>{d}</motion.button>))}
          <button type="button" disabled={isReview||checked} onClick={back} style={{ width:52, height:50, borderRadius:12, border:'2px solid '+C.line, background:C.paper, ...S.mono, fontSize:18, fontWeight:800, color:C.no, cursor:(isReview||checked)?'default':'pointer' }}>⌫</button>
        </div>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', justifyContent:'center' }}>
          {t.units.map((u,i)=>{ const on=unit===i; let bd=on?C.acc:C.line, bg=on?C.accSoft:C.paper, col=C.ink; if(checked&&on){ const ok=i===cfg.unitIdx; bd=ok?C.ok:C.no; bg=ok?C.okSoft:C.noSoft; col=ok?C.ok:C.no; }
            return <button key={i} type="button" disabled={isReview||checked} onClick={()=>setUnit(i)} style={{ padding:'10px 16px', borderRadius:999, border:'2px solid '+bd, background:bg, color:col, fontSize:15, fontWeight:800, cursor:(isReview||checked)?'default':'pointer' }}>{u}</button>; })}
        </div>
      </div>
      {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
      {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
...
/* ===== 07 · Find-wrong · kattaliklar ===== */
const D07 = makeFindwrong({ tag:'qty_wrong', level:'🟡', colors:['#FFC23C','#5BD6F2','#7CE0A3','#FF9A6B'] });
D07.T = {
  uz: { eyebrow:'Xatoni top', setup:'To‘rt kattalik hisoblangan, bittasi noto‘g‘ri.', ask:'Qaysi qatorda hisob NOTO‘G‘RI?',
    rows:[{ txt:'1 m = 100 sm', ok:true }, { txt:'1 soat = 60 minut', ok:true }, { txt:'1 dm = 100 sm', ok:false }, { txt:'yarim soat = 30 minut', ok:true }],
    correct:"To'g'ri. 1 dm = 10 sm bo‘lishi kerak, 100 emas.", wrong:'Birliklarni tekshiring: 1 dm nechta sm ga teng?', rule:'1 dm = 10 sm (100 emas).' },
  ru: { eyebrow:'Найди ошибку', setup:'Посчитаны четыре величины, одна неверна.', ask:'В какой строке расчёт НЕВЕРЕН?',
    rows:[{ txt:'1 м = 100 см', ok:true }, { txt:'1 час = 60 минут', ok:true }, { txt:'1 дм = 100 см', ok:false }, { txt:'полчаса = 30 минут', ok:true }],
    correct:'Верно. 1 дм = 10 см, а не 100.', wrong:'Проверь единицы: сколько см в 1 дм?', rule:'1 дм = 10 см (не 100).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_36_07(props) {
  return (
    <React.Fragment>

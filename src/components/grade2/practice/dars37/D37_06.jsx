// Dars 37 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 06 · MC · kun ketma-ketligi ===== */
const D06 = makeMC({ tag:'day_logic', level:'🟡', correctIdx:0, half:true, fs:17, mono:false });
D06.T = {
  uz: { eyebrow:'Mantiqiy ketma-ketlik', setup:'Kunlar doim bir xil tartibda keladi.', ask:'Agar bugun Seshanba bo‘lsa, ertaga haftaning qaysi kuni bo‘ladi?', opts:['Chorshanba','Dushanba','Payshanba','Juma'],
    correct:"To'g'ri. Seshanbadan keyin Chorshanba keladi.", wrong:'Seshanbadan bir qadam oldinga: undan keyin qaysi kun?', rule:'Seshanba → Chorshanba.' },
  ru: { eyebrow:'Логическая цепочка', setup:'Дни всегда идут в одном порядке.', ask:'Если сегодня вторник, какой день недели будет завтра?', opts:['Среда','Понедельник','Четверг','Пятница'],
    correct:'Верно. После вторника — среда.', wrong:'Шаг вперёд от вторника: какой день после него?', rule:'Вторник → среда.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_37_06(props) {
  return (
    <React.Fragment>

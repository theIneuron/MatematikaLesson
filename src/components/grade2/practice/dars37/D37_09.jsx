// Dars 37 · Amaliyot 09 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 37;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN — mantiq ============================== */
/* ---- IMZO: NAQSH DAVOMI (PatternNext) ---- */
function makePatternNext(cfg) {
  const _qi = _QSEQ++;
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review';
    ...
    useEffect(()=>{ onReady?.(picked!=null && !checked); },[picked,checked,onReady]);
    const check=useCallback(()=>{ const correct=picked===dCorrect; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:dispOpts.map(String), studentAnswer:{idx:picked}, correctAnswer:{idx:dCorrect}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[picked,t,playCorrect,playWrong,onSubmit]);
    const cref=useRef(check); cref.current=check; useEffect(()=>{ registerCheck?.(()=>cref.current()); },[registerCheck]);
    const slotVal = picked!=null ? dispOpts[picked] : '?';
    const slotCol = checked?(fb?.correct?C.ok:C.no):(picked!=null?C.acc:C.gold);
    return (<div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div>
      <p style={S.setup}>{t.setup}</p>
      <p style={S.ask}>{t.ask}</p>
      <Stage>
        <div style={{ display:'flex', gap:8, justifyContent:'center', alignItems:'center', flexWrap:'wrap' }}>
          {t.seq.map((v,i)=>(<motion.div key={i} initial={{scale:0}} animate={{scale:1}} transition={{delay:i*0.12}} style={{ minWidth:48, height:52, borderRadius:12, background:'rgba(255,194,60,.14)', border:'2px solid '+C.gold, display:'flex', alignItems:'center', justifyContent:'center', ...S.mono, fontSize:22, fontWeight:800, color:C.gold }}>{v}</motion.div>))}
          <span style={{ color:C.acc, fontWeight:800, fontSize:20 }}>→</span>
          <motion.div animate={picked!=null?{scale:[1,1.15,1]}:{scale:[1,1.08,1]}} transition={{repeat:picked!=null?0:Infinity,duration:1.2}} style={{ minWidth:48, height:52, borderRadius:12, background:C.stile, border:'2px dashed '+slotCol, display:'flex', alignItems:'center', justifyContent:'center', ...S.mono, fontSize:22, fontWeight:800, color:slotCol }}>{slotVal}</motion.div>
        </div>
      </Stage>
      <div style={{ display:'flex', flexWrap:'wrap', gap:9 }}>
        {dispOpts.map((o,i)=><button key={i} type="button" className="px-rise px-press" style={{ ...optStyle(picked,i,dCorrect,checked,isReview,{half:true,center:true,mono:true,fs:22}), animationDelay:(i*0.05)+'s' }} disabled={isReview||checked} onClick={()=>setPicked(i)}>{o}</button>)}
      </div>
      {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
      {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
    </div>);
...
/* ===== 09 · Story MC · og'irlik mantiq ===== */
const D09 = makeMC({ tag:'weight_logic', level:'🔴', correctIdx:0, half:true, fs:16, mono:false });
D09.T = {
  uz: { eyebrow:'Masala', setup:'Uchta quti bor: qizil, ko‘k va yashil. Qizil quti ko‘kdan og‘irroq. Yashil quti esa qizildan og‘irroq.', ask:'Qaysi quti eng og‘ir?', opts:['Yashil','Qizil','Ko‘k','Aniqlab bo‘lmaydi'],
    correct:"To'g'ri. Yashil > qizil > ko‘k, demak eng og‘iri — yashil.", wrong:'Taqqoslashlarni tartiblang: yashil qizildan, qizil ko‘kdan og‘ir. Eng tepasida kim?', rule:'Yashil > Qizil > Ko‘k → yashil eng og‘ir.' },
  ru: { eyebrow:'Задача', setup:'Три коробки: красная, синяя и зелёная. Красная тяжелее синей. Зелёная тяжелее красной.', ask:'Какая коробка самая тяжёлая?', opts:['Зелёная','Красная','Синяя','Нельзя узнать'],
    correct:'Верно. Зелёная > красная > синяя, значит самая тяжёлая — зелёная.', wrong:'Выстрой сравнения: зелёная тяжелее красной, красная — синей. Кто наверху?', rule:'Зелёная > красная > синяя → зелёная тяжелее всех.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_37_09(props) {
  return (
    <React.Fragment>

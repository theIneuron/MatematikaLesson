// Dars 37 · Amaliyot 02 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 02 · MC · toq son ortiqcha ===== */
const D02 = makeMC({ tag:'odd_out', level:'🟢', correctIdx:0, half:true, fs:22, stage:(t)=>(<div style={{textAlign:'center',...S.mono,fontSize:30,fontWeight:800,color:C.gold}}>2  4  5  6  8</div>) });
D02.T = {
  uz: { eyebrow:'Ortiqchasini top', setup:'Bu qatorda deyarli hamma sonlar juft.', ask:'2, 4, 5, 6, 8 — qaysi son juft EMAS (ortiqcha)?', opts:['5','4','6','8'],
    correct:"To'g'ri. 5 — toq son, qolganlari juft.", wrong:'Juft sonlar 2 ga bo‘linadi. Qaysi son ikkiga teng bo‘linmaydi?', rule:'5 — toq son (juft emas).' },
  ru: { eyebrow:'Найди лишнее', setup:'Здесь почти все числа чётные.', ask:'2, 4, 5, 6, 8 — какое число НЕ чётное (лишнее)?', opts:['5','4','6','8'],
    correct:'Верно. 5 — нечётное, остальные чётные.', wrong:'Чётные делятся на 2. Какое не делится на два?', rule:'5 — нечётное (не чётное).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_37_02(props) {
  return (
    <React.Fragment>

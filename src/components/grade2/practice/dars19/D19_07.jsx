// Dars 19 · Amaliyot 07 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D07_T = {
  uz: {
    eyebrow: 'Urug‘', setup: "Bit sirni aytdi: 3 × 4 = 12.",
    ask: 'Unda 12 ÷ 3 = ⬜? Bo‘sh katakni to‘ldiring:',
    opts: ['4', '3', '12', '9'],
    correct: "To'g'ri. 3 × 4 = 12, demak 12 ÷ 3 = 4. Ko'paytirish va bo'lish — bir oila.",
    wrong: "Maslahat: 3 ni nechaga ko'paytirsak 12 bo'ladi? O'sha son — 12 ÷ 3.",
    rule: "3 × 4 = 12 → 12 ÷ 3 = 4. Ko'paytirishni bilsang, bo'lishni topasan.",
  },
  ru: {
    eyebrow: 'Подсказка', setup: 'Бит открыл секрет: 3 × 4 = 12.',
    ask: 'Тогда 12 ÷ 3 = ⬜? Заполни клетку:',
    opts: ['4', '3', '12', '9'],
    correct: 'Верно. 3 × 4 = 12, значит 12 ÷ 3 = 4. Умножение и деление — одна семья.',
    wrong: 'Подсказка: на что умножить 3, чтобы вышло 12? Это число — 12 ÷ 3.',
    rule: '3 × 4 = 12 → 12 ÷ 3 = 4. Знаешь умножение — найдёшь деление.',
  },
};
function D19_07Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '4' }, correct, meta: { tag: 'fact_seed', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
          <div style={{ ...S.mono, fontSize: 26, fontWeight: 800, color: C.goldSoft, background: C.stile, borderRadius: 10, padding: '8px 16px', border: '1px solid ' + C.stageBd }}>3 × 4 = 12</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, ...S.mono, fontSize: 34, fontWeight: 800, color: C.sink }}>
            <span>12 ÷ 3</span><span style={{ color: C.gold }}>=</span>
            <span className={picked == null && !checked ? 'd19-pulse' : undefined} style={{ minWidth: 56, height: 58, border: '2px dashed ' + boxCol, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: boxCol, background: C.stile }}>{boxVal}</span>
          </div>
        </div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D07_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

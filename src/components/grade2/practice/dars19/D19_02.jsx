// Dars 19 · Amaliyot 02 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D02_T = {
  uz: {
    eyebrow: 'Bo‘lish', setup: "12 ta kristalni 3 sandiqqa teng joyladik.", expr: '12 ÷ 3',
    ask: '12 ÷ 3 nechaga teng?',
    opts: ['9', '4', '3', '6'],
    correct: "To'g'ri. 12 ÷ 3 = 4. Har sandiqda 4 tadan.",
    wrong: "Maslahat: 12 ni 3 ga teng ulang. 12 ÷ 3 = 4. 12 − 3 = 9 — bu ayirish.",
    rule: "Bo'lish — teng ulash, ayirish emas. 12 ÷ 3 = 4.",
  },
  ru: {
    eyebrow: 'Деление', setup: '12 кристаллов разложили в 3 сундука поровну.', expr: '12 ÷ 3',
    ask: 'Чему равно 12 ÷ 3?',
    opts: ['9', '4', '3', '6'],
    correct: 'Верно. 12 ÷ 3 = 4. В каждом сундуке по 4.',
    wrong: 'Подсказка: раздели 12 на 3 поровну. 12 ÷ 3 = 4. 12 − 3 = 9 — это вычитание.',
    rule: 'Деление — раздать поровну, а не вычесть. 12 ÷ 3 = 4.',
  },
};
function D19_02Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 1, label: '4' }, correct, meta: { tag: 'div_mc', level: '🟢' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          {[0, 1, 2].map((b) => (
            <div key={b} style={{ minWidth: 60, borderRadius: '10px 10px 14px 14px', border: '2px solid ' + C.ring, background: 'rgba(255,194,60,.06)', padding: 6, display: 'flex', flexWrap: 'wrap', gap: 3, justifyContent: 'center' }}>
              {Array.from({ length: 4 }).map((_, i) => <Crystal key={i} s={14} delay={(b * 4 + i) * 0.04} />)}
            </div>
          ))}
        </div>
        <div className="d19-float" style={{ textAlign: 'center', marginTop: 8, ...S.mono, fontSize: 36, fontWeight: 800, color: C.gold }}>{t.expr}</div>
      </Stage>
...
export default function D19_02(props) {
  return (<><style>{FX_CSS}</style><D19_02Impl {...props} /></>);
}

/* v-audio (auto) — TTS-toza narratsiya (ovoz). UZ = draft, uz-metodist validatsiyasi kerak. */
D19_02.audio = {
  uz: { intro: "12 ta kristalni 3 sandiqqa teng joyladik. 12 bo'lish 3 nechaga teng?", on_correct: "To'g'ri. 12 bo'lish 3 teng 4. Har sandiqda 4 tadan.", on_wrong: "Maslahat. 12 ni 3 ga teng ulang. 12 bo'lish 3 teng 4. 12 ayirish 3 teng 9, bu ayirish." },
  ru: { intro: "12 кристаллов разложили в 3 сундука поровну. Чему равно 12 делить на 3?", on_correct: "Верно. 12 делить на 3 равно 4. В каждом сундуке по 4.", on_wrong: "Подсказка. Раздели 12 на 3 поровну. 12 делить на 3 равно 4. 12 минус 3 равно 9, это вычитание." },
};

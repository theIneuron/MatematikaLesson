// Dars 19 · Amaliyot 03 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D03_T = {
  uz: {
    eyebrow: 'Bo‘sh katak', setup: "12 kristal teng ulandi — har biriga 4 tadan tegdi.",
    ask: 'Nechta sandiqqa ulangan? Bo‘sh katakni to‘ldiring:',
    opts: ['3', '4', '6', '2'],
    correct: "To'g'ri. 12 ÷ 3 = 4. 3 sandiqqa ulasak, har biriga 4 tadan.",
    wrong: "Maslahat: har biriga 4 tadan tegishi uchun nechta sandiq kerak? 12 ÷ 3 = 4.",
    rule: "12 ÷ ⬜ = 4 — nechta guruhda har biriga 4 tadan bo'ladi.",
  },
  ru: {
    eyebrow: 'Пустая клетка', setup: '12 кристаллов раздали поровну — каждому по 4.',
    ask: 'На сколько сундуков поделили? Заполни клетку:',
    opts: ['3', '4', '6', '2'],
    correct: 'Верно. 12 ÷ 3 = 4. На 3 сундука — по 4 каждому.',
    wrong: 'Подсказка: сколько сундуков, чтобы в каждом было по 4? 12 ÷ 3 = 4.',
    rule: '12 ÷ ⬜ = 4 — на сколько групп, чтобы по 4 в каждой.',
  },
};
function D19_03Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '3' }, correct, meta: { tag: 'div_sub', level: '🟢' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, ...S.mono, fontSize: 40, fontWeight: 800, color: C.sink }}>
          <span>12</span><span style={{ color: C.gold }}>÷</span>
          <span className={picked == null && !checked ? 'd19-pulse' : undefined} style={{ minWidth: 60, height: 62, border: '2px dashed ' + boxCol, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: boxCol, background: C.stile }}>{boxVal}</span>
          <span style={{ color: C.gold }}>=</span><span>4</span>
        </div>
      </Stage>

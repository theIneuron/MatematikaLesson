// Dars 19 · Amaliyot 06 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D06_TOTAL = 15, D06_BASK = 5, D06_CORRECT = 2;
const D06_T = {
  uz: {
    eyebrow: 'Teng ulash', setup: "15 ta kristal bor. Ularni 5 sandiqqa teng ulash kerak.",
    ask: 'Kristallarni bosib ulang. Har sandiqqa nechtadan tegadi?',
    opts: ['5', '10', '3', '4'],
    correct: "To'g'ri. 15 ÷ 5 = 3. Har sandiqda 3 tadan.",
    wrong: "Maslahat: hammaga teng ulang. 15 ÷ 5 = 3.",
    rule: "15 ni 5 ga teng ulash: har biriga 3 tadan.",
  },
  ru: {
    eyebrow: 'Поровну', setup: '15 кристаллов. Их надо разложить в 5 сундуков поровну.',
    ask: 'Раздай кристаллы. Сколько в каждый сундук?',
    opts: ['5', '10', '3', '4'],
    correct: 'Верно. 15 ÷ 5 = 3. В каждом сундуке по 3.',
    wrong: 'Подсказка: раздай всем поровну. 15 ÷ 5 = 3.',
    rule: '15 на 5 поровну — по 3 в каждый.',
  },
};
function D19_06Impl(props) {
  ...
  useEffect(() => { if (initialAnswer?.studentAnswer?.idx != null) { setPicked(initialAnswer.studentAnswer.idx); setPlaced(D06_TOTAL); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); } } }, [initialAnswer]);
  useEffect(() => { onReady?.(placed === D06_TOTAL && picked != null && !checked); }, [placed, picked, checked, onReady]);
  const check = useCallback(() => {
    const correct = picked === D06_CORRECT;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 2, label: '3' }, correct, meta: { tag: 'share_tap5', level: '🟡' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage><ShareBoard total={D06_TOTAL} baskets={D06_BASK} placed={placed} setPlaced={setPlaced} locked={isReview || checked} /></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D06_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 22 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

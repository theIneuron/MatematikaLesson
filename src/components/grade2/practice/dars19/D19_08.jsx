// Dars 19 · Amaliyot 08 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D08_T = {
  uz: {
    eyebrow: 'Masala', setup: "Jasur 18 ta kristalni 3 sandiqqa teng joyladi.",
    ask: 'Har sandiqda nechta kristal bor?',
    opts: ['15', '6', '3', '9'],
    correct: "To'g'ri. 18 ÷ 3 = 6. Har sandiqda 6 tadan.",
    wrong: "Maslahat: 18 ni 3 ga teng ulang. 18 ÷ 3 = 6. 18 − 3 = 15 emas.",
    rule: "«Jamini teng guruhga ulash» — bu bo'lish: 18 ÷ 3 = 6.",
  },
  ru: {
    eyebrow: 'Задача', setup: 'Джасур разложил 18 кристаллов в 3 сундука поровну.',
    ask: 'Сколько кристаллов в каждом сундуке?',
    opts: ['15', '6', '3', '9'],
    correct: 'Верно. 18 ÷ 3 = 6. В каждом сундуке по 6.',
    wrong: 'Подсказка: раздели 18 на 3 поровну. 18 ÷ 3 = 6. 18 − 3 = 15 — неверно.',
    rule: '«Разложить всё поровну» — это деление: 18 ÷ 3 = 6.',
  },
};
function D19_08Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 1, label: '6' }, correct, meta: { tag: 'div_word', level: '🔴' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 320, margin: '0 auto' }}>
          {Array.from({ length: 18 }).map((_, i) => <Crystal key={i} s={16} delay={i * 0.03} />)}
        </div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D08_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 22 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

// Dars 13 · Amaliyot 05 — mustaqil jsx-question (per-task split).
...
/* ===== 05 · equalrows · multiselect · equalrows ===== */
const D05 = makeMultiselect({ tag:'equalrows', level:'🟡' });

D05.T = {
  uz: { eyebrow:'Teng qatorlar', setup:"Ko'paytirish faqat qatorlar TENG bo'lsa mumkin.", ask:"Qaysi qatorlar teng (ko'paytirsa bo'ladi)? Belgilang:",
    opts:[{ label:'3 + 3 + 3', ok:true }, { label:'2 + 4 + 2', ok:false }, { label:'5 + 5', ok:true }, { label:'3 + 1 + 3', ok:false }],
    correct:"To'g'ri. 3 + 3 + 3 va 5 + 5 — qatorlar teng, ko'paytirsa bo'ladi.", wrong:"Har qatorda son BIR XIL bo'lsa — teng. 2 + 4 + 2 da sonlar har xil.", rule:'Teng qatorlar: 3 + 3 + 3 = 3 × 3, 5 + 5 = 2 × 5.' },
  ru: { eyebrow:'Равные ряды', setup:'Умножать можно, только если ряды РАВНЫ.', ask:'Где ряды равны (можно умножить)? Отметь:',
    opts:[{ label:'3 + 3 + 3', ok:true }, { label:'2 + 4 + 2', ok:false }, { label:'5 + 5', ok:true }, { label:'3 + 1 + 3', ok:false }],
    correct:'Верно. 3 + 3 + 3 и 5 + 5 — ряды равны, можно умножить.', wrong:'Ряды равны, если число ОДИНАКОВО. В 2 + 4 + 2 числа разные.', rule:'Равные ряды: 3 + 3 + 3 = 3 × 3, 5 + 5 = 2 × 5.' },
};

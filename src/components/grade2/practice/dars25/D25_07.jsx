// Dars 25 · Amaliyot 07 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 07 · VisualMC · o'lchash mumkin (kesma) ===== */
const D07 = makeVisualMC({ tag:'measurable', level:'🟡', correctIdx:1, items:[{kind:'togri'},{kind:'kesma'},{kind:'nur'},{kind:'togri'}], render:(it,i)=>(<LineSVG kind={it.kind} w={150} delay={i*0.1} />) });
D07.T = {
  uz: { eyebrow:'O‘lchash mumkinmi?', setup:'Faqat ikkala uchi chegaralangan chiziqni to‘liq o‘lchash mumkin.', ask:'Qaysi chiziqni lineyka bilan to‘liq o‘lchash mumkin? (ikkala uchi ham chegaralangan)',
    correct:"To'g'ri. Kesmaning ikkala uchi bor — uni o‘lchash mumkin.", wrong:'Cheksiz (o‘qli) chiziqni o‘lchab bo‘lmaydi. Ikkala uchi nuqta bo‘lganini tanlang.', rule:'Faqat kesmani (ikki uchi nuqta) to‘liq o‘lchash mumkin.' },
  ru: { eyebrow:'Можно ли измерить?', setup:'Полностью измерить можно только линию с двумя ограниченными концами.', ask:'Какую линию можно полностью измерить линейкой? (оба конца ограничены)',
    correct:'Верно. У отрезка оба конца есть — его можно измерить.', wrong:'Бесконечную (со стрелкой) линию измерить нельзя. Выбери с двумя точками.', rule:'Полностью измерить можно только отрезок (две точки).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_25_07(props) {
  return (
    <React.Fragment>

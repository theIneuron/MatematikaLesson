// Dars 25 · Amaliyot 09 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 09 · Story MC · Anvar nur (ikki chiziq alohida) ===== */
const twoLineStage = (t)=>(
  <div style={{ display:'flex', flexDirection:'column', gap:14, alignItems:'center' }}>
    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
      <span style={{ color:C.acc, fontWeight:800, fontSize:14, width:64, textAlign:'right' }}>{t.anvar}:</span>
      <span style={{ border:'1.5px solid '+C.stageBd, borderRadius:10, padding:'2px 6px', background:'rgba(255,255,255,.04)' }}><LineSVG kind="nur" w={170} tone="light" /></span>
    </div>
    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
      <span style={{ color:C.leaf, fontWeight:800, fontSize:14, width:64, textAlign:'right' }}>{t.rano}:</span>
      <span style={{ border:'1.5px solid '+C.stageBd, borderRadius:10, padding:'2px 6px', background:'rgba(255,255,255,.04)' }}><LineSVG kind="kesma" w={170} tone="light" /></span>
    </div>
  </div>
);
const D09 = makeMC({ tag:'story_nur', level:'🔴', correctIdx:0, half:true, fs:16, mono:false, stage:twoLineStage });
D09.T = {
  uz: { eyebrow:'Masala', anvar:'Anvar', rano:"Ra'no", setup:"Anvarning chizig'i A nuqtadan boshlanib, o‘q bilan cheksizlikka ketmoqda. Ra'noning chizig'i esa A va B nuqtalari orasida yotibdi. Ikkala chiziq yuqorida alohida ko'rsatilgan.", ask:'Anvarning chizig‘i (yuqoridagi birinchi chiziq) qanday ataladi?', opts:['Nur','Kesma',"To'g'ri chiziq",'Burchak'],
    correct:"To'g'ri. Anvarning chizig'i bir uchi (A) nuqta, ikkinchi tomoni cheksiz — bu nur. (Ra'noniki esa kesma.)", wrong:'Anvarning chizig‘i bir nuqtadan boshlanib, cheksiz ketmoqda. Bu qaysi tur?', rule:'Bir uchi nuqta + cheksiz tomon = nur.' },
  ru: { eyebrow:'Задача', anvar:'Анвар', rano:'Рано', setup:'Линия Анвара начинается в точке A и уходит стрелкой в бесконечность. Линия Рано лежит между точками A и B. Обе линии показаны выше по отдельности.', ask:'Как называется линия Анвара (первая линия выше)?', opts:['Луч','Отрезок','Прямая','Угол'],
    correct:'Верно. Линия Анвара: один конец (A) — точка, другая сторона бесконечна — это луч. (У Рано — отрезок.)', wrong:'Линия Анвара начинается в точке и уходит в бесконечность. Какой это тип?', rule:'Точка + бесконечная сторона = луч.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_25_09(props) {
  return (
    <React.Fragment>

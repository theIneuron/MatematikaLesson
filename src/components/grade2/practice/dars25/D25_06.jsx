// Dars 25 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 06 · VisualMC · nur EMAS ===== */
const D06 = makeVisualMC({ tag:'find_nur', level:'🟡', correctIdx:2, items:[{kind:'nur'},{kind:'nur'},{kind:'kesma'},{kind:'nur'}], render:(it,i)=>(<LineSVG kind={it.kind} w={150} delay={i*0.1} />) });
D06.T = {
  uz: { eyebrow:'Xatoni top', setup:'To‘rtta chiziqdan uchtasi nur, biri esa boshqa turdagi chiziq.', ask:'Quyidagilardan qaysi biri NUR emas? (ikkala uchida ham nuqta bor)',
    correct:"To'g'ri. Ikkala uchida nuqta bo‘lgani — kesma, u nur emas.", wrong:'Nurning bir uchi nuqta, bir tomoni o‘q. Qaysi rasmda ikkala uch ham nuqta?', rule:'Nur emas — ikkala uchi nuqta bo‘lgan kesma.' },
  ru: { eyebrow:'Найди лишнее', setup:'Из четырёх линий три — лучи, одна — другого типа.', ask:'Какая из линий НЕ луч? (у неё обе точки на концах)',
    correct:'Верно. С двумя точками на концах — это отрезок, а не луч.', wrong:'У луча один конец точка, другой стрелка. Где обе точки?', rule:'Не луч — отрезок с двумя точками на концах.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_25_06(props) {
  return (
    <React.Fragment>

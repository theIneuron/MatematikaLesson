// Dars 25 · Amaliyot 01 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 01 · MC · kesma id ===== */
const D01 = makeMC({ tag:'id_kesma', level:'🟢', correctIdx:2, half:true, fs:16, mono:false, stage:lineStage('kesma') });
D01.T = {
  uz: { eyebrow:'Chiziq turi', setup:"Bit ekranga chiziq chizdi. Uning ikkala uchi ham nuqta bilan chegaralangan.", ask:'Ikkala uchi nuqta bilan chegaralangan bu chiziq qanday ataladi?', opts:['Nur',"To'g'ri chiziq",'Kesma','Egri chiziq'],
    correct:"To'g'ri. Ikki uchi nuqta — bu kesma.", wrong:'Uchlariga qarang: ikkalasida ham nuqta bor, o‘q yo‘q. Bu qaysi chiziq?', rule:'Kesma — ikki uchi nuqta bilan chegaralangan.' },
  ru: { eyebrow:'Тип линии', setup:'Бит начертил линию. Оба её конца ограничены точками.', ask:'Как называется линия, оба конца которой ограничены точками?', opts:['Луч','Прямая','Отрезок','Кривая'],
    correct:'Верно. Две точки на концах — это отрезок.', wrong:'Смотри на концы: на обоих точки, стрелок нет. Какая это линия?', rule:'Отрезок — оба конца ограничены точками.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_25_01(props) {
  return (
    <React.Fragment>

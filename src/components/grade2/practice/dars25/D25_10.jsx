// Dars 25 · Amaliyot 10 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 10 · TrueFalse · to'g'ri chiziqni o'lchab bo'lmaydi ===== */
const D10 = makeTrueFalse({ tag:'tf_measure', level:'🔴', answer:true, swap:true, stage:lineStage('togri') });
D10.T = {
  uz: { eyebrow:'Umumlashtirish', setup:'To‘g‘ri chiziq ikkala tomonga cheksiz davom etadi.', ask:'To‘g‘ri chiziqni lineyka bilan to‘liq o‘lchab bo‘lmaydi, chunki uning uchi yo‘q. Bu fikr to‘g‘rimi?', yes:'To‘g‘ri', no:'Noto‘g‘ri',
    correct:"To'g'ri javob: To‘g‘ri. Cheksiz chiziqni o‘lchab bo‘lmaydi.", wrong:'Uning ikkala tomoni cheksiz — boshi ham, oxiri ham yo‘q. Uni o‘lchab bo‘ladimi?', rule:'To‘g‘ri chiziq cheksiz — uni o‘lchab bo‘lmaydi. Faqat kesma o‘lchanadi.' },
  ru: { eyebrow:'Обобщение', setup:'Прямая бесконечно продолжается в обе стороны.', ask:'Прямую нельзя полностью измерить линейкой, потому что у неё нет концов. Верно ли это?', yes:'Верно', no:'Неверно',
    correct:'Правильно: Верно. Бесконечную линию измерить нельзя.', wrong:'Обе её стороны бесконечны — нет ни начала, ни конца. Можно ли её измерить?', rule:'Прямая бесконечна — её нельзя измерить. Измеряют только отрезок.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_25_10(props) {
  return (
    <React.Fragment>

// Dars 30 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
let _QBASE = 30;          // dars raqami — pozitsiya boshlanishi
...
/* ============================== NEPTUN SCENE — harfli ifodalar ============================== */
/* ---- IMZO: HARF->SON QO'YISH (Substitute) ---- */
function makeSubstitute(cfg) {
  const _qi = _QSEQ++;
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review';
    const _ord = orderTarget(cfg, t.opts.length, cfg.correctIdx, _qi); const dispOpts = _ord.map((i)=>t.opts[i]); const dCorrect = _ord.indexOf(cfg.correctIdx);
    const [filled,setFilled]=useState(false),[picked,setPicked]=useState(null),[fb,setFb]=useState(null),[checked,setChecked]=useState(false);
    useEffect(()=>{ const sa=initialAnswer?.studentAnswer; if(sa?.idx!=null){ setFilled(true); setPicked(sa.idx); if(typeof initialAnswer.correct==='boolean'){ setFb({correct:initialAnswer.correct}); setChecked(true); } } },[initialAnswer]);
    useEffect(()=>{ onReady?.(filled && picked!=null && !checked); },[filled,picked,checked,onReady]);
    const check=useCallback(()=>{ const correct=picked===dCorrect; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:dispOpts.map((l,i)=>({id:String(i),label:l})), studentAnswer:{idx:picked,filled}, correctAnswer:{idx:dCorrect}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[picked,filled,t,playCorrect,playWrong,onSubmit]);
    const cref=useRef(check); cref.current=check; useEffect(()=>{ registerCheck?.(()=>cref.current()); },[registerCheck]);
    const slot = (
      <span style={{ display:'inline-flex', width:52, height:52, border:'2px '+(filled?'solid '+C.acc:'dashed '+C.gold), borderRadius:12, alignItems:'center', justifyContent:'center', color:filled?'#fff':C.gold, background:filled?'rgba(255,79,40,.2)':C.stile, margin:'0 4px' }}>
        {filled ? <motion.span key="v" initial={{scale:0,y:-14}} animate={{scale:1,y:0}}>{cfg.value}</motion.span> : cfg.letter}
      </span>
    );
    return (
      <div style={S.wrap}>
        ...
        <p style={S.setup}>{t.setup}</p>
        <Stage>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6, ...S.mono, fontSize:34, fontWeight:800, color:C.sink }}>
            {cfg.tpl.map((tok,i)=> tok===cfg.letter ? <span key={i}>{slot}</span> : <span key={i} style={{ color:['+','-','−','='].includes(tok)?C.gold:C.sink, margin:'0 2px' }}>{tok}</span>)}
          </div>
          {!filled && !checked && <div style={{ textAlign:'center', marginTop:12 }}>
            <span style={{ fontSize:13, color:C.goldSoft, marginRight:8 }}>{cfg.letter} =</span>
            <motion.button type="button" whileTap={{scale:.9}} onClick={()=>setFilled(true)} style={{ width:52, height:52, borderRadius:12, border:'2px solid '+C.gold, background:C.paper, ...S.mono, fontSize:24, fontWeight:800, color:C.ink, cursor:'pointer' }}>{cfg.value}</motion.button>
            <div style={{ fontSize:12.5, color:C.sink2, marginTop:6 }}>{t.place}</div>
          </div>}
        </Stage>
        <p style={S.ask}>{t.ask}</p>
        <div style={{ display:'flex', flexWrap:'wrap', gap:9, opacity: filled?1:0.45, pointerEvents: filled?'auto':'none' }}>
          {dispOpts.map((o,i)=><button key={i} type="button" className="px-rise px-press" style={{ ...optStyle(picked,i,dCorrect,checked,isReview,{half:true,center:true,mono:true,fs:22}), animationDelay:(i*0.05)+'s' }} disabled={isReview||checked||!filled} onClick={()=>setPicked(i)}>{o}</button>)}
        </div>
        {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
        {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
      </div>
...
const exprStage = (str) => ((t)=>(<div style={{ textAlign:'center', ...S.mono, fontSize:34, fontWeight:800, color:C.gold }}>{str}</div>));

/* ===== 06 · Find-wrong · a+5, a=2 ===== */
const D06 = makeFindwrong({ tag:'sub_wrong', level:'🟡', colors:['#FFC23C','#5BD6F2','#7CE0A3','#FF9A6B'] });
D06.T = {
  uz: { eyebrow:'Xatoni top', setup:'a + 5 ifodasida a = 2. Bir necha o‘quvchi javob berdi.', ask:'Qaysi qatorda a + 5 (a = 2) NOTO‘G‘RI hisoblangan?',
    rows:[{ txt:'2 + 5 = 7', ok:true }, { txt:'a → 2', ok:true }, { txt:'a + 5 = 25', ok:false }, { txt:'javob: 7', ok:true }],
    correct:"To'g'ri. a + 5 = 25 xato — harfni yonma-yon yozib qo‘ygan. To‘g‘risi 2 + 5 = 7.", wrong:'Harfni son bilan yonma-yon yozib bo‘lmaydi. a o‘rniga 2 ni qo‘yib qo‘shing.', rule:'a + 5 (a=2) = 2 + 5 = 7 (25 emas).' },
  ru: { eyebrow:'Найди ошибку', setup:'В a + 5 значение a = 2. Несколько учеников ответили.', ask:'В какой строке a + 5 (a = 2) посчитано НЕВЕРНО?',
    rows:[{ txt:'2 + 5 = 7', ok:true }, { txt:'a → 2', ok:true }, { txt:'a + 5 = 25', ok:false }, { txt:'ответ: 7', ok:true }],
    correct:'Верно. a + 5 = 25 — ошибка: букву приписали к числу. Верно 2 + 5 = 7.', wrong:'Нельзя приписывать букву к числу. Подставь 2 вместо a и сложи.', rule:'a + 5 (a=2) = 2 + 5 = 7 (не 25).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_30_06(props) {
  return (
    <React.Fragment>

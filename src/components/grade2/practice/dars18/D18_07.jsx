// Dars 18 · Amaliyot 07 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D07_T = {
  uz: {
    ...
    opts: ['3', '9', '6', '12'],
    correct: "To'g'ri. 3 × 9 = 9 × 3. O'rin almashdi, natija — 27.",
    wrong: "Maslahat: o'ng tomonda 9 × 3. O'rin almashsa, chapda ⬜ × 9 = 3 × 9.",
    rule: "a × b = b × a. O'ng tomondagi 9 × 3 — chapda 3 × 9.",
  },
  ru: {
    eyebrow: 'Пустая клетка', setup: 'Эти два выражения равны. Заполни пустую клетку.',
    ask: 'Поставь в пустую клетку верное число:',
    opts: ['3', '9', '6', '12'],
    correct: 'Верно. 3 × 9 = 9 × 3. Поменяли местами, итог — 27.',
    wrong: 'Подсказка: справа 9 × 3. При перестановке слева ⬜ × 9 = 3 × 9.',
    rule: 'a × b = b × a. Справа 9 × 3 — значит слева 3 × 9.',
  },
};
function D18_07Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '3' }, correct, meta: { tag: 'commute_sub2', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, ...S.mono, fontSize: 34, fontWeight: 800, color: C.sink }}>
          ...
          <span>× 9</span><span style={{ color: C.gold }}>=</span><span>9 × 3</span>
        </div>
      </Stage>

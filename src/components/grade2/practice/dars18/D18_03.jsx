// Dars 18 · Amaliyot 03 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const BoxSub = ({ val, boxCol, picked, pulse }) => (
  <span className={pulse ? 'd18-pulse' : undefined} style={{ minWidth: 56, height: 58, border: '2px dashed ' + boxCol, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: boxCol, background: C.stile }}>{val}</span>
);
...
const D03_T = {
  uz: {
    eyebrow: 'Bo‘sh katak', setup: "Bu ikki ifoda teng. Bo'sh katakni to'ldiring.",
    ask: 'Bo‘sh katakka to‘g‘ri sonni qo‘ying:',
    opts: ['2', '7', '9', '14'],
    correct: "To'g'ri. 7 × 2 = 2 × 7. O'rin almashdi, natija — 14.",
    wrong: "Maslahat: o'sha sonlarni o'rin almashtiring: 7 × 2 = 2 × 7.",
    rule: "a × b = b × a. Chap tomondagi sonlar o'ngda o'rin almashadi.",
  },
  ru: {
    eyebrow: 'Пустая клетка', setup: 'Эти два выражения равны. Заполни пустую клетку.',
    ask: 'Поставь в пустую клетку верное число:',
    opts: ['2', '7', '9', '14'],
    correct: 'Верно. 7 × 2 = 2 × 7. Поменяли местами, итог — 14.',
    wrong: 'Подсказка: поменяй те же числа местами: 7 × 2 = 2 × 7.',
    rule: 'a × b = b × a. Числа слева меняются местами справа.',
  },
};
function D18_03Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '2' }, correct, meta: { tag: 'commute_sub', level: '🟢' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, ...S.mono, fontSize: 34, fontWeight: 800, color: C.sink }}>
          <span>7 × 2</span><span style={{ color: C.gold }}>=</span>
          <BoxSub val={boxVal} boxCol={boxCol} picked={picked != null} pulse={picked == null && !checked} />
          <span>× 7</span>
        </div>
      </Stage>
...
export default function D18_03(props) {
  return (<><style>{FX_CSS}</style><D18_03Impl {...props} /></>);
}

/* v-audio (auto) — TTS-toza narratsiya (ovoz). UZ = draft, uz-metodist validatsiyasi kerak. */
D18_03.audio = {
  uz: { intro: "Bu ikki ifoda teng. Bo'sh katakni to'ldiring. Bo'sh katakka to'g'ri sonni qo'ying.", on_correct: "To'g'ri. 7 ko'paytirish 2 teng 2 ko'paytirish 7. O'rin almashdi, natija, 14.", on_wrong: "Maslahat. O'sha sonlarni o'rin almashtiring. 7 ko'paytirish 2 teng 2 ko'paytirish 7." },
  ru: { intro: "Эти два выражения равны. Заполни пустую клетку. Поставь в пустую клетку верное число.", on_correct: "Верно. 7 умножить на 2 равно 2 умножить на 7. Поменяли местами, итог, 14.", on_wrong: "Подсказка. Поменяй те же числа местами. 7 умножить на 2 равно 2 умножить на 7." },
};

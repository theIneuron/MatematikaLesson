// Dars 18 · Amaliyot 08 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D08_T = {
  uz: {
    eyebrow: 'Jadval katagi', setup: "3 × 4 = 12 ni bilamiz. O'rin almashishdan foydalaning.",
    ask: 'Bo‘sh katakni to‘ldiring: 4 × 3 = ⬜',
    opts: ['7', '12', '10', '16'],
    correct: "To'g'ri. 4 × 3 = 3 × 4 = 12. O'rin almashish yordam berdi.",
    wrong: "Maslahat: 3 × 4 = 12 ni bilsangiz, 4 × 3 ham 12. O'rin almashdi, xolos.",
    rule: "Bitta ko'paytmani bilsang, o'rin almashganini ham bilasan: 4 × 3 = 3 × 4.",
  },
  ru: {
    eyebrow: 'Клетка таблицы', setup: 'Мы знаем 3 × 4 = 12. Используй перестановку.',
    ask: 'Заполни пустую клетку: 4 × 3 = ⬜',
    opts: ['7', '12', '10', '16'],
    correct: 'Верно. 4 × 3 = 3 × 4 = 12. Перестановка помогла.',
    wrong: 'Подсказка: если знаешь 3 × 4 = 12, то 4 × 3 тоже 12. Просто поменяли местами.',
    rule: 'Знаешь одно произведение — знаешь и переставленное: 4 × 3 = 3 × 4.',
  },
};
function D18_08Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 1, label: '12' }, correct, meta: { tag: 'table_fill', level: '🔴' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
          <div style={{ ...S.mono, fontSize: 24, fontWeight: 800, color: C.sink2, background: C.stile, borderRadius: 10, padding: '8px 16px', border: '1px solid ' + C.stageBd }}>3 × 4 = 12</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, ...S.mono, fontSize: 34, fontWeight: 800, color: C.sink }}>
            <span>4 × 3</span><span style={{ color: C.gold }}>=</span>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D08_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

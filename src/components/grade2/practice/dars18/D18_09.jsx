// Dars 18 · Amaliyot 09 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D09_T = {
  uz: {
    eyebrow: 'Masala', setup: "Anvar 4 qatorda 7 tadan quti terdi. Zuhra 7 qatorda 4 tadan terdi.",
    ask: 'Kimda ko‘proq quti bor?',
    opts: ['Anvarda', 'Zuhrada', 'Teng'],
    correct: "To'g'ri. Anvar: 4 × 7 = 28. Zuhra: 7 × 4 = 28. Ikkalasida teng!",
    wrong: "Maslahat: 4 × 7 va 7 × 4 ni sanang. O'rin almashish — natija bir xil.",
    rule: "a × b = b × a — o'rin almashish hayotda ham: qutilar soni teng.",
  },
  ru: {
    eyebrow: 'Задача', setup: 'Анвар сложил 4 ряда по 7 ящиков. Зухра — 7 рядов по 4.',
    ask: 'У кого больше ящиков?',
    opts: ['У Анвара', 'У Зухры', 'Поровну'],
    correct: 'Верно. Анвар: 4 × 7 = 28. Зухра: 7 × 4 = 28. Поровну!',
    wrong: 'Подсказка: посчитай 4 × 7 и 7 × 4. Перестановка — итог одинаков.',
    rule: 'a × b = b × a — перестановка и в жизни: ящиков поровну.',
  },
};
function D18_09Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 2, label: t.opts[2] }, correct, meta: { tag: 'compare_word', level: '🔴' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 24, justifyContent: 'center' }}>
          <div style={{ textAlign: 'center' }}><BoxArray r={4} c={7} s={13} /><div style={{ ...S.mono, fontSize: 15, fontWeight: 800, color: C.gold, marginTop: 5 }}>4 × 7</div></div>
          <div style={{ textAlign: 'center' }}><BoxArray r={7} c={4} s={13} /><div style={{ ...S.mono, fontSize: 15, fontWeight: 800, color: C.gold, marginTop: 5 }}>7 × 4</div></div>
        </div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {order.map((i) => <button key={i} type="button" style={optStyle(picked, i, D09_CORRECT, checked, isReview, { center: true, fs: 17 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>)}
      </div>

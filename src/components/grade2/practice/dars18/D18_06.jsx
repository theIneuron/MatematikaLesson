...
function D18_06Impl(props) {
  ...
  const order = React.useMemo(() => { const a = D06_ROWS.map((_, i) => i); for (let k = a.length - 1; k > 0; k--) { const j = Math.floor(Math.random() * (k + 1)); const tmp = a[k]; a[k] = a[j]; a[j] = tmp; } return a; }, []);
  ...
  return (
    <div style={S.wrap}>
      ...
      {order.map((i) => { const r = D06_ROWS[i]; return (

// Dars 18 · Amaliyot 02 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D02_T = {
  uz: {
    eyebrow: 'O‘rin almashish', setup: "Anvar ikki qutini teng dedi: 4 × 6 = 6 × ⬜.",
    ask: 'Bo‘sh katakka qaysi son tushadi?',
    opts: ['4', '6', '10', '24'],
    correct: "To'g'ri. 4 × 6 = 6 × 4. Ko'paytuvchilar o'rin almashdi, natija o'sha — 24.",
    wrong: "Maslahat: o'rin almashishda o'sha sonlar qoladi. 4 × 6 = 6 × 4.",
    rule: "a × b = b × a. Sonlar o'rin almashadi, natija o'zgarmaydi.",
  },
  ru: {
    eyebrow: 'Перестановка', setup: 'Анвар сказал, что два ящика равны: 4 × 6 = 6 × ⬜.',
    ask: 'Какое число встанет в пустую клетку?',
    opts: ['4', '6', '10', '24'],
    correct: 'Верно. 4 × 6 = 6 × 4. Множители поменялись местами, итог тот же — 24.',
    wrong: 'Подсказка: при перестановке остаются те же числа. 4 × 6 = 6 × 4.',
    rule: 'a × b = b × a. Числа меняются местами, итог не меняется.',
  },
};
function D18_02Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '4' }, correct, meta: { tag: 'commute_mc', level: '🟢' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div className="d18-float" style={{ textAlign: 'center', ...S.mono, fontSize: 34, fontWeight: 800, color: C.sink }}>4 × 6 <span style={{ color: C.gold }}>=</span> 6 × <span style={{ color: C.gold }}>⬜</span></div>
      </Stage>

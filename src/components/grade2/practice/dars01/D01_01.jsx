// Dars 1 · Amaliyot 01 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const Stepper = ({ label, color, value, set, max, locked, render }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: 8, borderRadius: 12, border: '2px solid ' + C.stageBd, background: C.stile, minWidth: 120 }}>
    <div style={{ fontSize: 12, fontWeight: 800, color, textTransform: 'uppercase', letterSpacing: '.04em' }}>{label}</div>
    <div style={{ minHeight: 44, display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center', justifyContent: 'center' }}>{render(value)}</div>
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <button type="button" disabled={locked || value <= 0} onClick={() => set(Math.max(0, value - 1))} style={{ width: 34, height: 34, borderRadius: 9, border: 'none', background: value <= 0 ? '#2a3a58' : color, color: '#08111f', fontSize: 20, fontWeight: 800, cursor: (locked || value <= 0) ? 'default' : 'pointer' }}>−</button>
      <span style={{ ...S.mono, fontSize: 22, fontWeight: 800, color: C.sink, minWidth: 22, textAlign: 'center' }}>{value}</span>
      <button type="button" disabled={locked || value >= max} onClick={() => set(Math.min(max, value + 1))} style={{ width: 34, height: 34, borderRadius: 9, border: 'none', background: value >= max ? '#2a3a58' : color, color: '#08111f', fontSize: 20, fontWeight: 800, cursor: (locked || value >= max) ? 'default' : 'pointer' }}>+</button>
    </div>
  </div>
);
...
const D01_T = {
  uz: {
    ...
    wrong: "Maslahat: 3 ta kasseta — 30, 6 ta batareya — 6. 30 + 6 = 36.",
  ...
  ru: {
    ...
    wrong: 'Подсказка: 3 кассеты — 30, 6 батареек — 6. 30 + 6 = 36.',
    rule: 'Число = десятки × 10 + единицы. 3 десятка 6 единиц = 36.',
  },
};
function D01_01Impl(props) {
  ...
  const check = useCallback(() => {
    const correct = tens === D01_TENS && ones === D01_ONES;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { tens, ones, value: num }, correctAnswer: { tens: D01_TENS, ones: D01_ONES, value: D01_NUM }, correct, meta: { tag: 'pv_compose', level: '🟢' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          <Stepper label={t.ten} color={C.ten} value={tens} set={setTens} max={9} locked={locked} render={(v) => Array.from({ length: v }).map((_, i) => <Cassette key={i} s={48} delay={i * 0.03} />)} />
          <Stepper label={t.one} color={C.one} value={ones} set={setOnes} max={9} locked={locked} render={(v) => Array.from({ length: v }).map((_, i) => <Battery key={i} s={40} delay={i * 0.03} />)} />
        </div>
        <div style={{ textAlign: 'center', marginTop: 10, color: C.sink2, fontSize: 13, fontWeight: 700 }}>{t.formed}</div>
        <div className="d01-float" style={{ textAlign: 'center', ...S.mono, fontSize: 40, fontWeight: 800, color: C.ten }}>{num}</div>
      </Stage>

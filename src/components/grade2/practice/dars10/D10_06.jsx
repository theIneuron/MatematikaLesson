// Dars 10 · Amaliyot 06 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D06_CHIPS = [52, 42, 79, 25], D06_CORRECT = 0;
const D06_T = {
  uz: {
    eyebrow: 'Qo‘shish bilan tekshiring', setup: '52 − 27 = 25. Tekshirish: ayirma + ayriluvchi = kamayuvchi.',
    ask: '25 + 27 nechchi bo‘lsa, ayirish to‘g‘ri bo‘ladi?',
    correct: "To'g'ri. 25 + 27 = 52 = kamayuvchi. Demak 52 − 27 = 25 to‘g‘ri.",
    wrong: "Maslahat: ayirmaga ayriluvchini qo‘shing. Natija yuqoridagi kamayuvchiga teng bo‘lishi kerak.",
    rule: "Ayirishni tekshirish: ayirma + ayriluvchi = kamayuvchi. 25 + 27 = 52.",
  },
  ru: {
    eyebrow: 'Проверь сложением', setup: '52 − 27 = 25. Проверка: разность + вычитаемое = уменьшаемое.',
    ask: 'Каким должно быть 25 + 27, чтобы вычитание было верным?',
    correct: 'Верно. 25 + 27 = 52 = уменьшаемое. Значит 52 − 27 = 25 верно.',
    wrong: 'Подсказка: прибавь к разности вычитаемое. Результат должен равняться уменьшаемому, что сверху.',
    rule: 'Проверка вычитания: разность + вычитаемое = уменьшаемое. 25 + 27 = 52.',
  },
};
function D10_06Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D06_CHIPS.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { idx: picked, label: String(D06_CHIPS[picked]) }, correctAnswer: { idx: 0, label: '52' }, correct, meta: { tag: 'checkadd', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ textAlign: 'center' }}>
          <div style={{ ...S.mono, fontSize: 30, fontWeight: 800, color: C.sink, marginBottom: 8 }}><span style={{ color: C.mars }}>52</span> − <span style={{ color: C.one }}>27</span> = <span style={{ color: C.ten }}>25</span></div>
          <div style={{ ...S.mono, fontSize: 36, fontWeight: 800, color: C.sink }}><span style={{ color: C.ten }}>25</span> + <span style={{ color: C.one }}>27</span> = <span style={{ color: C.acc }}>?</span></div>
        </div>
      </Stage>
      <p style={{ ...S.ask, fontSize: 20 }}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {order.map((i) => { const on = picked === i; let tone = null; if (checked && on) tone = i === D06_CORRECT ? 'ok' : 'no'; return <Chip key={i} on={on && !checked} tone={tone} disabled={isReview || checked} onClick={() => setPicked(i)} w={68}>{D06_CHIPS[i]}</Chip>; })}
      </div>

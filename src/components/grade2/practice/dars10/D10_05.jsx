// Dars 10 · Amaliyot 05 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
// qarz belgili raqam: chizilgan + tepada kichik yangi qiymat (top)
const DCb = ({ ch, c, strike, top }) => (
  <div style={{ width: 44, height: 50, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    {top != null && <span className="d10-pop" style={{ position: 'absolute', top: -3, ...S.mono, fontSize: 16, fontWeight: 800, color: C.acc }}>{top}</span>}
    <span style={{ ...S.mono, fontSize: 32, fontWeight: 800, color: strike ? C.sink2 : c, textDecoration: strike ? 'line-through' : 'none', textDecorationColor: C.acc, opacity: strike ? 0.75 : 1 }}>{ch}</span>
  </div>
);
...
const D05_T = {
  uz: {
    eyebrow: 'Qarz bilan yeching', setup: 'Stolbik 52 − 27. O‘nlik 5 chizilgan (4 bo‘lgan), birlik 2 ustida 12 ko‘rinadi.',
    ask: 'Birlik 12 − 7, o‘nlik qarzdan keyin 4 − 2. Javobni tering.',
    correct: "To'g'ri. 12 − 7 = 5, 4 − 2 = 2. Javob 25.",
    wrong: "Maslahat: birlik 12 − 7 ni yeching. So‘ng o‘nliklarni: qarzdan keyin 4 − 2.",
    rule: "Qarz bilan ayirish: 12 − 7 = 5, 4 − 2 = 2. 52 − 27 = 25.",
  },
  ru: {
    eyebrow: 'Реши с займом', setup: 'Столбик 52 − 27. Десяток 5 зачёркнут (стало 4), над 2 видно 12.',
    ask: 'Единицы 12 − 7, десятки после займа 4 − 2. Набери ответ.',
    correct: 'Верно. 12 − 7 = 5, 4 − 2 = 2. Ответ 25.',
    wrong: 'Подсказка: реши единицы 12 − 7. Потом десятки: после займа 4 − 2.',
    rule: 'Вычитание с займом: 12 − 7 = 5, 4 − 2 = 2. 52 − 27 = 25.',
  },
};
function D10_05Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value: parseInt(val, 10) }, correctAnswer: { value: D05_ANS }, correct, meta: { tag: 'borrowsolve', level: '🔴' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}>
          <Colon>
            <ColHeader />
            <Row sign="" tenNode={<DCb ch="5" c={C.ten} strike top="4" />} oneNode={<DCb ch="2" c={C.one} top="12" />} />
            <Row sign="−" tenNode={<DC ch="2" c={C.ten} />} oneNode={<DC ch="7" c={C.one} />} />
            <HR />

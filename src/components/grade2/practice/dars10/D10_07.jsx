// Dars 10 · Amaliyot 07 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D07_CHIPS = [27, 37, 33, 63], D07_CORRECT = 0;
const D07_T = {
  uz: {
    eyebrow: 'Masala', setup: 'Mars bazasida 45 batareya bor edi.',
    ask: '45 batareyadan 18 tasi ishlatildi. Nechta batareya qoldi?',
    correct: "To'g'ri. 45 − 18 = 27 (birlik 5<8, qarz: 15 − 8 = 7; 3 − 1 = 2).",
    wrong: "Maslahat: «qoldi» — ayirish. Birlik 5 dan 8 ni ayirib bo‘lmaydi, qarz oling.",
    rule: "«Qoldi» — ayirish. Qarzni unutmang: 45 − 18 = 27.",
  },
  ru: {
    eyebrow: 'Задача', setup: 'На базе Марса было 45 батареек.',
    ask: 'Из 45 батареек израсходовали 18. Сколько осталось?',
    correct: 'Верно. 45 − 18 = 27 (единицы 5<8, заём: 15 − 8 = 7; 3 − 1 = 2).',
    wrong: 'Подсказка: «осталось» — вычитание. Из 5 нельзя вычесть 8, займи десяток.',
    rule: '«Осталось» — вычитание. Не забудь заём: 45 − 18 = 27.',
  },
};
function D10_07Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D07_CHIPS.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { idx: picked, label: String(D07_CHIPS[picked]) }, correctAnswer: { idx: 0, label: '27' }, correct, meta: { tag: 'wordborrow', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: 30 }}>🔋</div><div style={{ ...S.mono, fontSize: 24, fontWeight: 800, color: C.mars }}>45</div></div>
          <div style={{ ...S.mono, fontSize: 26, color: C.sink2 }}>−</div>
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: 30 }}>⚙️</div><div style={{ ...S.mono, fontSize: 24, fontWeight: 800, color: C.one }}>18</div></div>
          <div style={{ ...S.mono, fontSize: 26, color: C.sink2 }}>=</div>
      ...
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {order.map((i) => { const on = picked === i; let tone = null; if (checked && on) tone = i === D07_CORRECT ? 'ok' : 'no'; return <Chip key={i} on={on && !checked} tone={tone} disabled={isReview || checked} onClick={() => setPicked(i)} w={68}>{D07_CHIPS[i]}</Chip>; })}
      </div>

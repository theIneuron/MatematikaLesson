// Dars 28 · Amaliyot 01 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ---- IMZO: PERIMETR-YURISH — tomonlarni bosib yig'indini to'plash ---- */
function makePerimeter(cfg) {
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review'; const pts=cfg.pts, lens=cfg.lens, n=pts.length;
    const [tapped,setTapped]=useState([]),[picked,setPicked]=useState(null),[fb,setFb]=useState(null),[checked,setChecked]=useState(false);
    useEffect(()=>{ const sa=initialAnswer?.studentAnswer; if(sa){ if(sa.tapped) setTapped(sa.tapped); if(sa.idx!=null) setPicked(sa.idx); if(typeof initialAnswer.correct==='boolean'){ setFb({correct:initialAnswer.correct}); setChecked(true); } } },[initialAnswer]);
    useEffect(()=>{ onReady?.(picked!=null && !checked); },[picked,checked,onReady]);
    const allTapped=tapped.length===n;
    const tap=(i)=>{ if(isReview||checked) return; setTapped(a=>a.includes(i)?a:[...a,i]); };
    const check=useCallback(()=>{ const correct=picked===cfg.correctIdx; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:cfg.opts, studentAnswer:{tapped,idx:picked}, correctAnswer:{idx:cfg.correctIdx}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[tapped,picked,t,playCorrect,playWrong,onSubmit]);
    ...
    return (
      <div style={S.wrap}>
        ...
        <p style={S.ask}>{t.ask}</p>
        <Stage>
          <svg width="100%" viewBox={`0 0 ${cfg.box||200} ${cfg.h||190}`} style={{ maxWidth:cfg.box||200, display:'block', margin:'0 auto' }}>
            <polygon points={pts.map(p=>p.join(',')).join(' ')} fill="rgba(255,255,255,.05)" stroke={C.stageBd} strokeWidth="2" />
            {pts.map((p,i)=>{ const q=pts[(i+1)%n]; const on=tapped.includes(i); const mx=(p[0]+q[0])/2, my=(p[1]+q[1])/2;
              return (<g key={i}>
                <line x1={p[0]} y1={p[1]} x2={q[0]} y2={q[1]} stroke={on?C.acc:C.sink2} strokeWidth={on?7:5} strokeLinecap="round" style={{cursor:(isReview||checked)?'default':'pointer'}} onClick={()=>tap(i)} />
                <circle cx={mx} cy={my} r={13} fill={on?C.acc:'#16223c'} stroke="#fff" strokeWidth="1.5" style={{cursor:(isReview||checked)?'default':'pointer'}} onClick={()=>tap(i)} />
                <text x={mx} y={my+5} fontSize="14" fontWeight="800" textAnchor="middle" fill="#fff" style={{pointerEvents:'none'}} fontFamily="monospace">{lens[i]}</text>
              </g>); })}
          </svg>
          <div style={{ textAlign:'center', marginTop:6, ...S.mono, fontSize:24, fontWeight:800, color:done?C.leaf:bad?'#ffb4a8':C.gold, minHeight:30 }}>{tapped.length===0 ? t.tapHint : tapped.map((i,k)=>(k?' + ':'')+lens[i]).join('') + (allTapped ? ' = ?' : '')}</div>
        </Stage>
        <p style={{ ...S.ask, fontSize:15, margin:'6px 0 10px', color:allTapped?C.ink:C.ink3 }}>{allTapped ? t.pickAsk : t.walkFirst}</p>
        <div style={{ display:'flex', flexWrap:'wrap', gap:9 }}>
          {cfg.opts.map((o,i)=><button key={i} type="button" style={optStyle(picked,i,cfg.correctIdx,checked,isReview,{half:true,center:true,mono:true,fs:22})} disabled={isReview||checked||!allTapped} onClick={()=>setPicked(i)}>{o}</button>)}
        </div>
        {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
        {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
      </div>
...
const PENTA = polyPoints(5, 100, 95, 72);

/* ===== 01 · MC · uchburchak perimetr ===== */
const D01 = makeMC({ tag:'tri_perim', level:'🟢', correctIdx:2, half:true, fs:22, stage:(t)=>(<div style={{display:'flex',justifyContent:'center'}}><svg width="180" height="130" viewBox="0 0 180 130"><polygon points="90,20 30,110 150,110" fill="rgba(255,194,60,.14)" stroke="#FFC23C" strokeWidth="3"/><text x="52" y="60" fill="#EAF0F8" fontSize="15" fontWeight="800">3</text><text x="128" y="60" fill="#EAF0F8" fontSize="15" fontWeight="800">4</text><text x="90" y="125" fill="#EAF0F8" fontSize="15" fontWeight="800" textAnchor="middle">5</text></svg></div>) });
D01.T = {
  uz: { eyebrow:'Perimetr', setup:'Uchburchakning tomonlari 3, 4 va 5 birlik.', ask:'Uchburchakning barcha tomonlarini qo‘shsak, perimetri nechchi bo‘ladi?', opts:['15','9','12','7'],
    correct:"To'g'ri. 3 + 4 + 5 = 12.", wrong:'Perimetr — barcha tomonlar yig‘indisi. Uchala tomonni ham qo‘shing, birontasini tashlab qo‘ymang.', rule:'Perimetr = 3 + 4 + 5 = 12.' },
  ru: { eyebrow:'Периметр', setup:'Стороны треугольника — 3, 4 и 5 единиц.', ask:'Если сложить все стороны треугольника, каким будет периметр?', opts:['15','9','12','7'],
    correct:'Верно. 3 + 4 + 5 = 12.', wrong:'Периметр — сумма всех сторон. Сложи все три, не пропусти.', rule:'Периметр = 3 + 4 + 5 = 12.' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_28_01(props) {
  return (
    <React.Fragment>

// Dars 28 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ===== 06 · Find-wrong · perimetr hisobi ===== */
const D06 = makeFindwrong({ tag:'perim_wrong', level:'🟡', colors:['#FFC23C','#5BD6F2','#7CE0A3','#FF9A6B'] });
D06.T = {
  uz: { eyebrow:'Xatoni top', setup:'To‘rtta perimetr hisoblangan: uchtasi to‘g‘ri, bittasi noto‘g‘ri (bir tomon tashlab qo‘yilgan).', ask:'Qaysi perimetr NOTO‘G‘RI hisoblangan?',
    rows:[{ txt:'2 + 3 + 4 = 9', ok:true }, { txt:'5 + 5 + 5 = 15', ok:true }, { txt:'4 + 4 + 4 = 8', ok:false }, { txt:'6 + 2 + 6 + 2 = 16', ok:true }],
    correct:"To'g'ri. 4 + 4 + 4 = 12 bo‘lishi kerak, 8 emas (bir tomon tashlangan).", wrong:'Har birini qayta qo‘shib chiqing: barcha tomonlar qo‘shilganmi?', rule:'4 + 4 + 4 = 12 (8 emas).' },
  ru: { eyebrow:'Найди ошибку', setup:'Посчитаны четыре периметра: три верно, один неверно (пропущена сторона).', ask:'Какой периметр посчитан НЕВЕРНО?',
    rows:[{ txt:'2 + 3 + 4 = 9', ok:true }, { txt:'5 + 5 + 5 = 15', ok:true }, { txt:'4 + 4 + 4 = 8', ok:false }, { txt:'6 + 2 + 6 + 2 = 16', ok:true }],
    correct:'Верно. 4 + 4 + 4 = 12, а не 8 (пропущена сторона).', wrong:'Пересчитай каждый: все ли стороны сложены?', rule:'4 + 4 + 4 = 12 (не 8).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_28_06(props) {
  return (
    <React.Fragment>

// Dars 8 · Amaliyot 01 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const Cassette = ({ s = 26, dim }) => (
  <svg width={s} height={s * 66 / 48} viewBox="0 0 48 66" aria-hidden="true" style={{ display: 'inline-block', verticalAlign: 'middle', opacity: dim ? 0.28 : 1, transition: 'opacity .2s' }}>
    <defs><linearGradient id="d8cass" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#4E5E82" /><stop offset="52%" stopColor="#8C9EC4" /><stop offset="100%" stopColor="#4E5E82" /></linearGradient></defs>
    <rect x="1" y="4" width="46" height="61" rx="7" fill="url(#d8cass)" stroke="#33415F" strokeWidth="1.4" />
...
const Battery = ({ s = 18, dim }) => (
  <svg width={s * 0.66} height={s} viewBox="0 0 22 34" aria-hidden="true" style={{ display: 'inline-block', verticalAlign: 'middle', opacity: dim ? 0.28 : 1, transition: 'opacity .2s' }}>
    <defs><linearGradient id="d8batt" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#0E6E96" /><stop offset="50%" stopColor="#8FE0F4" /><stop offset="100%" stopColor="#0A5876" /></linearGradient></defs>
    <rect x="8" y="0.6" width="6" height="3.4" rx="1.4" fill="#8FA0AE" /><rect x="1.4" y="4" width="19.2" height="29.4" rx="4.2" fill="url(#d8batt)" stroke="#093F55" strokeWidth="1" />
...
// 59 (5 kasseta + 9 batareya). 24 ni olib tashla: 2 kasseta + 4 batareya. Qoldi 35.
const D01_T = {
  uz: {
    eyebrow: 'Olib tashlang', setup: 'Bazada 59 quvvat: 5 kasseta va 9 batareya.',
    ask: '59 dan 24 ni olib tashlang: 2 kasseta va 4 batareyani bosing.',
    left: 'Qoldi',
    correct: "To'g'ri. 5 − 2 = 3 kasseta, 9 − 4 = 5 batareya. Qoldi 35.",
    wrong: "Maslahat: 24 = 2 o‘nlik (kasseta) va 4 birlik (batareya). Aynan shuncha olib tashlang.",
    rule: "Ayirish — olib tashlash: 59 − 24 = 35 (kasseta va batareyani alohida).",
  },
  ru: {
    eyebrow: 'Убери', setup: 'На базе 59 заряда: 5 кассет и 9 батареек.',
    ask: 'Убери из 59 число 24: нажми 2 кассеты и 4 батарейки.',
    left: 'Осталось',
    correct: 'Верно. 5 − 2 = 3 кассеты, 9 − 4 = 5 батареек. Осталось 35.',
    wrong: 'Подсказка: 24 = 2 десятка (кассеты) и 4 единицы (батарейки). Убери ровно столько.',
    rule: 'Вычитание — это убрать: 59 − 24 = 35 (кассеты и батарейки отдельно).',
  },
};
function D08_01Impl(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D01_T[lang] || D01_T.uz;
  const isReview = mode === 'review';
  const [remC, setRemC] = useState([]); // removed cassette indices
  const [remB, setRemB] = useState([]);
  const [fb, setFb] = useState(null);
  const [checked, setChecked] = useState(false);
  useEffect(() => { const sa = initialAnswer?.studentAnswer; if (sa?.remC) { setRemC(sa.remC); setRemB(sa.remB || []); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); } } }, [initialAnswer]);
  const touched = remC.length + remB.length > 0;
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { remC, remB, left }, correctAnswer: { value: 35 }, correct, meta: { tag: 'takeaway', level: '🟢' } });
  }, [remC, remB, left, t, playCorrect, playWrong, onSubmit]);

// Dars 8 · Amaliyot 06 — mustaqil topshiriq fayli (grade1/grade5 uslubi).
...
const D06_CHIPS = [43, 34, 93, 53], D06_CORRECT = 0;
const D06_T = {
  uz: {
    eyebrow: 'Nechta qoldi', setup: 'Mars bazasida 68 quti bor edi.',
    ask: '68 qutidan 25 tasi kemaga jo‘natildi. Nechta quti qoldi?',
    correct: "To'g'ri. 68 − 25 = 43 (8 − 5 = 3, 6 − 2 = 4).",
    wrong: "Maslahat: «qoldi» — ayirish. 68 dan 25 ni ayiring: birlik va o‘nlik alohida.",
    rule: "«Qoldi» — ayirish: 68 − 25 = 43.",
  },
  ru: {
    eyebrow: 'Сколько осталось', setup: 'На базе Марса было 68 ящиков.',
    ask: 'Из 68 ящиков 25 отправили на корабль. Сколько осталось?',
    correct: 'Верно. 68 − 25 = 43 (8 − 5 = 3, 6 − 2 = 4).',
    wrong: 'Подсказка: «осталось» — это вычитание. Из 68 вычти 25 по разрядам.',
    rule: '«Осталось» — вычитание: 68 − 25 = 43.',
  },
};
function D08_06Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D06_CHIPS.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { idx: picked, label: String(D06_CHIPS[picked]) }, correctAnswer: { idx: 0, label: '43' }, correct, meta: { tag: 'wordsub', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: 30 }}>📦</div><div style={{ ...S.mono, fontSize: 22, fontWeight: 800, color: C.mars }}>68</div></div>
          <div style={{ ...S.mono, fontSize: 24, color: C.sink2 }}>−</div>
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: 30 }}>🚀</div><div style={{ ...S.mono, fontSize: 22, fontWeight: 800, color: C.one }}>25</div></div>
          <div style={{ ...S.mono, fontSize: 24, color: C.sink2 }}>=</div>
          <div style={{ ...S.mono, fontSize: 30, fontWeight: 800, color: C.acc }}>?</div>
        </div>
      </Stage>
      <p style={{ ...S.ask, fontSize: 20 }}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
        {order.map((i) => { const on = picked === i; let tone = null; if (checked && on) tone = i === D06_CORRECT ? 'ok' : 'no'; return <Chip key={i} on={on && !checked} tone={tone} disabled={isReview || checked} onClick={() => setPicked(i)} w={64}>{D06_CHIPS[i]}</Chip>; })}
      </div>

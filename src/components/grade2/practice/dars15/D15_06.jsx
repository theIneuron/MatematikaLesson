// Dars 15 · Amaliyot 06 — mustaqil jsx-question (per-task split).
    ...
    {seq.map((v,i)=>{
      ...
      return (
        <React.Fragment key={i}>
          <div style={{ minWidth:48, height:48, borderRadius:10, border:'2px '+(isBlank&&filled==null?'dashed':'solid')+' '+bd, background: isBlank?(filled!=null?'rgba(255,79,40,.18)':C.stile):'rgba(255,194,60,.12)', display:'flex', alignItems:'center', justifyContent:'center', ...S.mono, fontSize:20, fontWeight:800, color:col }}>
...
/* ===== 06 · stripfill4 · jadval katagi (tap) · stripfill4 ===== */
const D06 = makeMC({ tag:'stripfill4', level:'🟡', correctIdx:0, half:true, fs:22, stage:stripStage([4,8,0,16,20],2) });

D06.T = {
  uz: { eyebrow:'Jadval katagi · ×4', setup:"4 ga ko'paytirish qatori. Har katak avvalgidan 4 ko'p.", scene:'4 tadan: 4, 8, ⬜, 16, 20', ask:"Bo'sh katakka mos sonni tanlang (3 marta 4):", opts:['12','10','14','16'],
    correct:"To'g'ri. 4, 8, 12, 16, 20 — bo'sh katak 12.", wrong:"Avvalgi katakka 4 qo'shing: 8 dan keyin nechchi?", rule:"×4: 4, 8, 12, 16, 20. Bo'sh — 12." },
  ru: { eyebrow:'Клетка таблицы · ×4', setup:'Строка ×4. Каждая клетка на 4 больше.', scene:'по 4: 4, 8, ⬜, 16, 20', ask:'Выбери число для пустой клетки (3 раза по 4):', opts:['12','10','14','16'],
    correct:'Верно. 4, 8, 12, 16, 20 — пустая клетка 12.', wrong:'Прибавь 4 к предыдущей: что после 8?', rule:'×4: 4, 8, 12, 16, 20. Пустая — 12.' },
};

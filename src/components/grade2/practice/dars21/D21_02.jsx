// Dars 21 · Amaliyot 02 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 02 · Jadval qatori (÷2) · 🟢 · tablerow2 =================== */
const D02_POOL = [5, 4, 6, 3], D02_ANS = 5, D02_BLANK = 4; // 10 ÷ 2 = 5
const D02_T = {
  uz: {
    eyebrow: 'Jadval qatori', setup: "÷2 jadvali qatori. Bo'sh katakni to'ldiring.",
    ask: 'Bo‘sh katakka mos raqamni bosing (10 : 2 = ⬜):',
    correct: "To'g'ri. 10 ÷ 2 = 5. Bo'linmalar tartib bilan: 1, 2, 3, 4, 5.",
    wrong: "Maslahat: qatordagi tartibga qarang — 1, 2, 3, 4, keyin nechchi? 2 × 5 = 10.",
    rule: "÷2 jadvali: 2→1, 4→2, 6→3, 8→4, 10→5.",
  },
  ru: {
    eyebrow: 'Строка таблицы', setup: 'Строка таблицы ÷2. Заполни пустую клетку.',
    ask: 'Нажми число для пустой клетки (10 : 2 = ⬜):',
    correct: 'Верно. 10 ÷ 2 = 5. Частные по порядку: 1, 2, 3, 4, 5.',
    wrong: 'Подсказка: смотри на порядок — 1, 2, 3, 4, а дальше? 2 × 5 = 10.',
    rule: 'Таблица ÷2: 2→1, 4→2, 6→3, 8→4, 10→5.',
  },
};
function D21_02(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D02_T[lang] || D02_T.uz;
  const isReview = mode === 'review';
  const [pick, setPick] = useState(null); const [fb, setFb] = useState(null); const [checked, setChecked] = useState(false);
  useEffect(() => { if (initialAnswer?.studentAnswer?.value != null) { const i = D02_POOL.indexOf(initialAnswer.studentAnswer.value); setPick(i >= 0 ? i : null); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); } } }, [initialAnswer]);
  useEffect(() => { onReady?.(pick != null && !checked); }, [pick, checked, onReady]);
  const check = useCallback(() => {
    const correct = pick != null && D02_POOL[pick] === D02_ANS;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: D02_POOL.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { value: pick != null ? D02_POOL[pick] : null }, correctAnswer: { value: 5 }, correct, meta: { tag: 'tablerow2', level: '🟢' } });
  }, [pick, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const tone = checked ? (fb?.correct ? 'ok' : 'no') : null;
  const filled = pick != null ? D02_POOL[pick] : null;
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div>
      <p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ display: 'flex', justifyContent: 'center' }}><TableRow step={2} count={5} blankIdx={D02_BLANK} filled={filled} tone={tone} /></div></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D02_POOL.map((n, i) => { const on = pick === i; let tn = null; if (checked && on) tn = D02_POOL[i] === D02_ANS ? 'ok' : 'no'; let bd = C.line, bg = C.paper, col = C.ink; if (on && !checked) { bd = C.acc; bg = C.accSoft; } if (tn === 'ok') { bd = C.ok; bg = C.okSoft; col = C.ok; } if (tn === 'no') { bd = C.no; bg = C.noSoft; col = C.no; } return <button key={i} type="button" disabled={isReview || checked} onClick={() => setPick(i)} style={{ width: 62, height: 58, borderRadius: 13, border: '2px solid ' + bd, background: bg, ...S.mono, fontSize: 24, fontWeight: 800, color: col, cursor: (isReview || checked) ? 'default' : 'pointer', boxShadow: (on && !checked) ? '0 0 0 4px #FFE0D6' : 'none' }}>{n}</button>; })}
      </div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_21_02(props) {
  return (
    <React.Fragment>

// Dars 21 · Amaliyot 05 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 05 · Jadval qatori (÷3) · 🟡 · tablerow3 =================== */
const D05_POOL = [6, 5, 7, 4], D05_ANS = 6, D05_BLANK = 5; // 18 ÷ 3 = 6
const D05_T = {
  uz: {
    eyebrow: 'Jadval qatori', setup: "÷3 jadvali qatori. Oxirgi katak bo'sh.",
    ask: 'Bo‘sh katakka mos raqamni bosing (18 : 3 = ⬜):',
    correct: "To'g'ri. 18 ÷ 3 = 6. Bo'linmalar: 1, 2, 3, 4, 5, 6.",
    wrong: "Maslahat: 1, 2, 3, 4, 5, keyin nechchi? 3 × 6 = 18.",
    rule: "÷3 jadvali: 3→1, 6→2 … 18→6.",
  },
  ru: {
    eyebrow: 'Строка таблицы', setup: 'Строка таблицы ÷3. Последняя клетка пуста.',
    ask: 'Нажми число для пустой клетки (18 : 3 = ⬜):',
    correct: 'Верно. 18 ÷ 3 = 6. Частные: 1, 2, 3, 4, 5, 6.',
    wrong: 'Подсказка: 1, 2, 3, 4, 5, а дальше? 3 × 6 = 18.',
    rule: 'Таблица ÷3: 3→1, 6→2 … 18→6.',
  },
};
function D21_05(props) {
  ...
  useEffect(() => { if (initialAnswer?.studentAnswer?.value != null) { const i = D05_POOL.indexOf(initialAnswer.studentAnswer.value); setPick(i >= 0 ? i : null); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); } } }, [initialAnswer]);
  useEffect(() => { onReady?.(pick != null && !checked); }, [pick, checked, onReady]);
  const check = useCallback(() => {
    const correct = pick != null && D05_POOL[pick] === D05_ANS;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: D05_POOL.map((n, i) => ({ id: String(i), label: String(n) })), studentAnswer: { value: pick != null ? D05_POOL[pick] : null }, correctAnswer: { value: 6 }, correct, meta: { tag: 'tablerow3', level: '🟡' } });
  }, [pick, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const tone = checked ? (fb?.correct ? 'ok' : 'no') : null;
  const filled = pick != null ? D05_POOL[pick] : null;
  return (
    <div style={S.wrap}>
      <div style={S.eyebrow}>{t.eyebrow}</div>
      <p style={S.setup}>{t.setup}</p>
      <Stage><div style={{ display: 'flex', justifyContent: 'center', overflow: 'auto' }}><TableRow step={3} count={6} blankIdx={D05_BLANK} filled={filled} tone={tone} /></div></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D05_POOL.map((n, i) => { const on = pick === i; let tn = null; if (checked && on) tn = D05_POOL[i] === D05_ANS ? 'ok' : 'no'; let bd = C.line, bg = C.paper, col = C.ink; if (on && !checked) { bd = C.acc; bg = C.accSoft; } if (tn === 'ok') { bd = C.ok; bg = C.okSoft; col = C.ok; } if (tn === 'no') { bd = C.no; bg = C.noSoft; col = C.no; } return <button key={i} type="button" disabled={isReview || checked} onClick={() => setPick(i)} style={{ width: 62, height: 58, borderRadius: 13, border: '2px solid ' + bd, background: bg, ...S.mono, fontSize: 24, fontWeight: 800, color: col, cursor: (isReview || checked) ? 'default' : 'pointer', boxShadow: (on && !checked) ? '0 0 0 4px #FFE0D6' : 'none' }}>{n}</button>; })}
      </div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_21_05(props) {
  return (
    <React.Fragment>

// Dars 21 · Amaliyot 08 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
const D08_T = {
  uz: {
    eyebrow: 'Masala', setup: "Zuhra 24 ta kristalni 3 vagonchaga teng ulaydi.",
    ask: 'Har vagonchada nechta kristal bo‘ladi?',
    opts: ['8', '7', '12', '21'],
    correct: "To'g'ri. 24 ÷ 3 = 8. Har vagonchada 8 tadan.",
    wrong: "Maslahat: 24 ni 3 ga teng ulang. 24 − 3 = 21 emas — bu ayirish.",
    rule: "«Teng ulash» — bu bo'lish: 24 ÷ 3 = 8.",
  },
  ru: {
    eyebrow: 'Задача', setup: 'Зухра делит 24 кристалла в 3 вагонетки поровну.',
    ask: 'Сколько кристаллов будет в каждой вагонетке?',
    opts: ['8', '7', '12', '21'],
    correct: 'Верно. 24 ÷ 3 = 8. В каждой вагонетке по 8.',
    wrong: 'Подсказка: раздели 24 на 3 поровну. 24 − 3 = 21 — это вычитание.',
    rule: '«Разделить поровну» — это деление: 24 ÷ 3 = 8.',
  },
};
function D21_08(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '8' }, correct, meta: { tag: 'word3', level: '🔴' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', alignItems: 'flex-end', flexWrap: 'wrap' }}><Cart n={0} w={64} /><Cart n={0} w={64} /><Cart n={0} w={64} /></div>
        <div style={{ textAlign: 'center', marginTop: 6, fontSize: 13, fontWeight: 700, color: C.goldSoft }}>24 kristal · 3 vagoncha · teng</div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {t.opts.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D08_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 22 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o}</button>)}
      </div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_21_08(props) {
  return (
    <React.Fragment>

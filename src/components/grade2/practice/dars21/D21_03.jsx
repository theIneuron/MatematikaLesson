// Dars 21 · Amaliyot 03 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
const D03_T = {
  uz: {
    eyebrow: 'Yarmi', setup: "14 kristalni 2 vagonchaga teng ulaymiz.",
    ask: 'Har biriga nechtadan kristal ketadi?',
    opts: ['7', '6', '8', '12'],
    correct: "To'g'ri. 14 ÷ 2 = 7. Har vagonchada 7 tadan.",
    wrong: "Maslahat: 14 ni ikkiga teng ajrating. 14 − 2 = 12 emas — bu ayirish.",
    rule: "÷2 — yarmi. 14 ÷ 2 = 7.",
  },
  ru: {
    eyebrow: 'Половина', setup: '14 кристаллов делим в 2 вагонетки поровну.',
    ask: 'Сколько кристаллов достанется в каждую?',
    opts: ['7', '6', '8', '12'],
    correct: 'Верно. 14 ÷ 2 = 7. В каждой вагонетке по 7.',
    wrong: 'Подсказка: раздели 14 на две равные части. 14 − 2 = 12 — это вычитание.',
    rule: '÷2 — половина. 14 ÷ 2 = 7.',
  },
};
function D21_03(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '7' }, correct, meta: { tag: 'half2', level: '🟢' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage><div style={{ display: 'flex', gap: 16, justifyContent: 'center', alignItems: 'flex-end' }}><Cart n={7} /><Cart n={7} /></div></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {t.opts.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D03_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 22 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o}</button>)}
      </div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_21_03(props) {
  return (
    <React.Fragment>

// Dars 21 · Amaliyot 07 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 07 · To'g'ri qator (16:2) · 🟡 · rowpick =================== */
const D07_OPTS = ['16 : 2 = 8', '16 : 2 = 6', '16 : 2 = 4', '16 : 2 = 14'], D07_CORRECT = 0;
const D07_T = {
  uz: {
    eyebrow: 'To‘g‘ri qator', setup: "16 kristal 2 vagonchaga teng ulanadi.",
    ask: '16 : 2 ning javobi qaysi qatorda TO‘G‘RI ko‘rsatilgan?',
    correct: "To'g'ri. 16 : 2 = 8. Har vagonchada 8 tadan.",
    wrong: "Maslahat: 16 ning yarmi nechchi? 2 ni nechaga ko‘paytirsak 16 bo‘ladi?",
    rule: "16 : 2 = 8. Yordam: 2 × 8 = 16.",
  },
  ru: {
    eyebrow: 'Верная строка', setup: '16 кристаллов делят в 2 вагонетки поровну.',
    ask: 'В какой строке ВЕРНО показан ответ 16 : 2?',
    correct: 'Верно. 16 : 2 = 8. В каждой вагонетке по 8.',
    wrong: 'Подсказка: сколько половина от 16? На что умножить 2, чтобы вышло 16?',
    rule: '16 : 2 = 8. Подсказка: 2 × 8 = 16.',
  },
};
function D21_07(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D07_OPTS.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: D07_OPTS[picked] }, correctAnswer: { idx: 0, label: '16 : 2 = 8' }, correct, meta: { tag: 'rowpick', level: '🟡' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'center', maxWidth: 300, margin: '0 auto' }}>
          {Array.from({ length: 16 }).map((_, i) => <Crystal key={i} s={15} delay={i * 0.025} />)}
        </div>
        <div className="d21-float" style={{ textAlign: 'center', marginTop: 8, ...S.mono, fontSize: 30, fontWeight: 800, color: C.gold }}>16 : 2 = ?</div>
      </Stage>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        {D07_OPTS.map((o, i) => {
          ...
          if (show) { const okv = i === D07_CORRECT; bd = okv ? C.ok : C.no; bg = okv ? C.okSoft : C.noSoft; col = okv ? C.ok : C.no; }
          return <button key={i} type="button" disabled={isReview || checked} onClick={() => setPicked(i)} style={{ minHeight: 60, borderRadius: 13, border: '2px solid ' + bd, background: bg, ...S.mono, fontSize: 24, fontWeight: 800, color: col, cursor: (isReview || checked) ? 'default' : 'pointer', boxShadow: (on && !checked) ? '0 0 0 4px #FFE0D6' : 'none' }}>{o}</button>;
        })}
      </div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_21_07(props) {
  return (
    <React.Fragment>

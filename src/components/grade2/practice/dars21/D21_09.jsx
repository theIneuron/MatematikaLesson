// Dars 21 · Amaliyot 09 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
const D09_T = {
  uz: {
    eyebrow: 'Yashirin son', setup: "Bit sonni yashirdi: ⬜ ÷ 2 = 8.",
    ask: 'Yashirin bo‘linuvchini toping: ⬜ ÷ 2 = 8.',
    opts: ['16', '10', '6', '4'],
    correct: "To'g'ri. 8 × 2 = 16, demak 16 ÷ 2 = 8.",
    wrong: "Maslahat: 8 ni 2 ga ko'paytiring. Qaysi son 2 ga bo'linganda 8 chiqadi?",
    rule: "⬜ ÷ 2 = 8 → ⬜ = 8 × 2 = 16.",
  },
  ru: {
    eyebrow: 'Скрытое число', setup: 'Бит спрятал число: ⬜ ÷ 2 = 8.',
    ask: 'Найди скрытое делимое: ⬜ ÷ 2 = 8.',
    opts: ['16', '10', '6', '4'],
    correct: 'Верно. 8 × 2 = 16, значит 16 ÷ 2 = 8.',
    wrong: 'Подсказка: умножь 8 на 2. Какое число при делении на 2 даёт 8?',
    rule: '⬜ ÷ 2 = 8 → ⬜ = 8 × 2 = 16.',
  },
};
function D21_09(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 0, label: '16' }, correct, meta: { tag: 'missing_dividend', level: '🔴' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, ...S.mono, fontSize: 40, fontWeight: 800, color: C.sink }}>
          <span className={picked == null && !checked ? 'd21-pulse' : undefined} style={{ minWidth: 64, height: 62, border: '2px dashed ' + boxCol, borderRadius: 12, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: boxCol, background: C.stile }}>{boxVal}</span>
          <span style={{ color: C.gold }}>÷</span><span>2</span><span style={{ color: C.gold }}>=</span><span>8</span>
        </div>
      </Stage>
      <p style={S.ask}>{t.ask}</p>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {t.opts.map((o, i) => <button key={i} type="button" style={optStyle(picked, i, D09_CORRECT, checked, isReview, { half: true, center: true, mono: true, fs: 24 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{o}</button>)}
      </div>
...
const __FX_CSS = `
      ...
      `;
export default function Task_21_09(props) {
  return (
    <React.Fragment>

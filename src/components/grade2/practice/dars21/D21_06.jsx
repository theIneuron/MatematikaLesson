// Dars 21 · Amaliyot 06 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* =================== 06 · Bir xil javob · 🟡 · samevalue (multiselect) =================== */
const D06_OPTS = [{ e: '12 ÷ 2', v: 6, ok: true }, { e: '18 ÷ 3', v: 6, ok: true }, { e: '14 ÷ 2', v: 7, ok: false }, { e: '15 ÷ 3', v: 5, ok: false }];
const D06_T = {
  uz: {
    eyebrow: 'Bir xil javob', setup: "Zuhra javobi 6 ga teng ifodalarni izlaydi.",
    ask: 'Javobi 6 ga teng 2 ta ifodani belgilang:',
    correct: "To'g'ri. 12 ÷ 2 = 6 va 18 ÷ 3 = 6. Boshqalari 7 va 5.",
    wrong: "Maslahat: har birini hisoblang. 14 ÷ 2 = 7, 15 ÷ 3 = 5 — 6 emas.",
    rule: "12 ÷ 2 = 6 va 18 ÷ 3 = 6 — ikkalasi 6.",
  },
  ru: {
    eyebrow: 'Один ответ', setup: 'Зухра ищет выражения, равные 6.',
    ask: 'Отметь 2 выражения, равные 6:',
    correct: 'Верно. 12 ÷ 2 = 6 и 18 ÷ 3 = 6. Остальные 7 и 5.',
    wrong: 'Подсказка: посчитай каждое. 14 ÷ 2 = 7, 15 ÷ 3 = 5 — не 6.',
    rule: '12 ÷ 2 = 6 и 18 ÷ 3 = 6 — оба равны 6.',
  },
};
function D21_06(props) {
  ...
  const check = useCallback(() => {
    const want = D06_OPTS.map((o, i) => o.ok ? i : -1).filter((x) => x >= 0);
    const correct = sel.length === want.length && want.every((i) => sel.includes(i));
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({ questionText: t.ask, options: D06_OPTS.map((o, i) => ({ id: String(i), label: o.e })), studentAnswer: { sel }, correctAnswer: { sel: want }, correct, meta: { tag: 'samevalue', level: '🟡' } });
  }, [sel, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const box = (i) => {
    const on = sel.includes(i), o = D06_OPTS[i];
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage><div style={{ textAlign: 'center', ...S.mono, fontSize: 40, fontWeight: 800, color: C.gold }}>= 6 ?</div></Stage>
      <p style={S.ask}>{t.ask}</p>
      {D06_OPTS.map((_, i) => box(i))}
...
const __FX_CSS = `
      ...
      `;
export default function Task_21_06(props) {
  return (
    <React.Fragment>

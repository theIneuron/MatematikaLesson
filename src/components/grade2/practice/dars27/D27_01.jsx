// Dars 27 · Amaliyot 01 — mustaqil topshiriq fayli (monolitdan bo'lindi).
...
/* ============================== URAN SCENE — uzunlik/lineyka ============================== */
/* ---- IMZO: LINEYKA-O'LCHASH — obyekt oxiridagi bo'linmani bosib o'lchash ---- */
function makeRuler(cfg) {
  function Comp(props) {
    const { lang='uz', mode='answer', initialAnswer=null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props||{};
    const t = Comp.T[lang]||Comp.T.uz; const isReview = mode==='review';
    ...
    const max=cfg.max, step=42, x0=26, W=x0*2+max*step, yR=70;
    const xx=(i)=>x0+i*step;
    const check=useCallback(()=>{ const correct=pick===cfg.len; setFb({correct}); setChecked(true); correct?playCorrect?.():playWrong?.();
      onSubmit?.({ questionText:t.ask, options:[], studentAnswer:{value:pick}, correctAnswer:{value:cfg.len}, correct, meta:{tag:cfg.tag,level:cfg.level} }); },[pick,t,playCorrect,playWrong,onSubmit]);
    const cref=useRef(check); cref.current=check; useEffect(()=>{ registerCheck?.(()=>cref.current()); },[registerCheck]);
    const tone=(i)=> checked&&pick===i ? (i===cfg.len?'ok':'no') : (pick===i?'on':null);
    return (
      <div style={S.wrap}>
        <div style={S.eyebrow}>{t.eyebrow}</div>
        <p style={S.setup}>{t.setup}</p>
        <p style={S.ask}>{t.ask}</p>
        <Stage>
          <svg width="100%" viewBox={`0 0 ${W} 118`} style={{ maxWidth:W, display:'block', margin:'0 auto' }}>
            {/* obyekt */}
            <motion.rect initial={{ width:0 }} animate={{ width:xx(cfg.len)-xx(0) }} transition={{ duration:.7 }} x={xx(0)} y={26} height={20} rx={5} fill={cfg.color||'#FFC23C'} opacity=".92" />
            {/* lineyka tanasi */}
            <rect x={x0} y={yR} width={max*step} height={30} rx={4} fill="#EAF0F8" />
            {Array.from({length:max+1}).map((_,i)=>(<g key={i}>
              <line x1={xx(i)} y1={yR} x2={xx(i)} y2={yR+(i%1===0?16:10)} stroke="#5A5A60" strokeWidth="2" />
              <text x={xx(i)} y={yR+28} fontSize="13" fontWeight="700" textAnchor="middle" fill="#0E0E10" fontFamily="monospace">{i}</text>
              {/* bosiladigan bo'linma nuqtasi */}
              <circle cx={xx(i)} cy={yR-14} r={tone(i)==='ok'?11:tone(i)==='no'?11:9}
                fill={tone(i)==='ok'?C.ok:tone(i)==='no'?C.no:tone(i)==='on'?C.acc:'#B6C7D2'} stroke="#fff" strokeWidth="2"
                style={{cursor:(isReview||checked)?'default':'pointer'}} onClick={()=>{ if(!isReview&&!checked) setPick(i); }} />
            </g>))}
            <text x={W-30} y={yR-8} fontSize="13" fontWeight="800" fill={C.goldSoft} textAnchor="end" fontFamily="monospace">{cfg.unit}</text>
          </svg>
          <div style={{ textAlign:'center', marginTop:6, fontSize:14, fontWeight:700, color:C.goldSoft }}>{t.hint}</div>
        </Stage>
        {fb && <FB ok={fb.correct} text={fb.correct?t.correct:t.wrong} />}
        {checked && fb?.correct && t.rule && <RuleChip text={t.rule} />}
      </div>
...
/* ===== 01 · IMZO Ruler (sm) ===== */
const D01 = makeRuler({ tag:'ruler_sm', level:'🟢', max:8, len:6, unit:'sm', color:'#FFC23C' });
D01.T = {
  uz: { eyebrow:'Lineyka bilan o‘lchash', setup:"Ra'no qalamni lineykaning 0 bo‘linmasidan boshlab qo‘ydi.", ask:'Qalamning uchi qaysi bo‘linmada tugayapti? Shu bo‘linmani bosing.', hint:'Qalam uchi turgan sonni bosing', correct:"To'g'ri. Qalamning uzunligi — 6 sm.", wrong:'Qalam noldan boshlanadi. Uning uchi qaysi son ustida tugaydi — shuni bosing.', rule:'Uzunlik = qalam uchi turgan bo‘linma (noldan hisoblab).' },
  ru: { eyebrow:'Измерение линейкой', setup:'Рано положила карандаш от деления 0 на линейке.', ask:'На каком делении заканчивается кончик карандаша? Нажми это деление.', hint:'Нажми число, где кончик карандаша', correct:'Верно. Длина карандаша — 6 см.', wrong:'Карандаш начинается с нуля. На каком числе его кончик — то и нажми.', rule:'Длина = деление, где кончик (считая от нуля).' },
};
...
const __FX_CSS = `
      ...
      `;
export default function Task_27_01(props) {
  return (
    <React.Fragment>

...
const SCREEN_META = [
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },      // 0  hook: 3×4=12 → 12÷4=? (distraktor 8=12−4)
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },        // 1  ×↔÷ OILA: 3×4 massiv → 3×4=12, 12÷3=4, 12÷4=3
  { id: 's2',  type: 'exploration', template: 'custom',   scored: false, scope: null },        // 2  2-misol oila: 2×5=10 → 10÷2=5, 10÷5=2
  { id: 's3',  type: 'rule',        template: 'custom',   scored: false, scope: null },        // 3  QOIDA: ×ni bilsang ÷ni ham bilasan + check
  { id: 's4',  type: 'exploration', template: 'custom',   scored: false, scope: null },        // 4  qo'llash: 15÷3 ni «3×?=15» orqali + check
  { id: 'sTBL', type: 'exploration', template: 'custom',  scored: false, scope: null },        // 5  ×↔÷ oila kartasi (bir massiv → 2× va 2÷)
  { id: 's5',  type: 'test',        template: 'custom',   scored: true,  scope: 'practice' },  // 6  mashq OILA-A'ZO-TOP (MC) single
  { id: 's6',  type: 'test',        template: 'custom',   scored: true,  scope: 'practice' },  // 7  mashq MOSLASH single
  { id: 's7',  type: 'test',        template: 'custom',   scored: true,  scope: 'practice' },  // 8  mashq OILA-A'ZO-TOP: 3 round
  { id: 's8',  type: 'test',        template: 'custom',   scored: true,  scope: 'practice' },  // 9  mashq MOSLASH: 3 juft
  { id: 's9',  type: 'test',        template: 'custom',   scored: true,  scope: 'practice' },  // 10 mashq OILA-A'ZO-TOP: 3 round
  { id: 's10', type: 'test',        template: 'custom',   scored: true,  scope: 'practice' },  // 11 mashq MOSLASH: 3 juft
  { id: 's11', type: 'test',        template: 'custom',   scored: true,  scope: 'practice' },  // 12 mashq OILA-A'ZO-TOP: 3 round
  { id: 'sCASE', type: 'case',      template: 'custom',   scored: true,  scope: 'practice' },  // 13 MASALA: oila orqali ÷ topish
  { id: 's14',  type: 'test',       template: 'custom',   scored: true,  scope: 'final' },     // 14 FINAL: 3 round oila-a'zo-top + FactCard
  { id: 's15',  type: 'summary',    template: 'custom',   scored: false, scope: 'final' }      // 15 yakun + QOIDA recap (→ ÷2/÷3 jadvali)
];
...
const CONTENT = {
  ...
  s0: {
    ...
    q: { ru: 'Анвар и Зухра сортируют кристаллы. Они знают: 3 × 4 = 12. Тогда 12 ÷ 4 = ?', uz: "Anvar va Zuhra kristallarni saralashyapti. Ular biladi: 3 × 4 = 12. Unda 12 ÷ 4 = ?" },
    ...
    audio: {
      intro: {
        ru: [
          'Мы знаем, что такое деление. Корабль Бита у шахты Сатурна, экипаж сортирует кристаллы.',
          'Анвар и Зухра разложили двенадцать кристаллов в три ряда по четыре. Это три умножить на четыре — двенадцать.',
          'Бит спрашивает: если знаешь, что три на четыре двенадцать, то сколько будет двенадцать разделить на четыре?',
          'Послушай два ответа. Первый — восемь. Второй — три. Или ты пока не знаешь. Выбери свой ответ.'
        ],
        uz: [
          "Bo'lish nimaligini bilamiz. Bitning kemasi Saturn koni yonida, ekipaj kristallarni saralayapti.",
          "Anvar va Zuhra o'n ikkita kristallni uch qatorga, to'rttadan terdi. Bu uch marta to'rt — o'n ikki.",
          "Bit so'raydi: uch marta to'rt o'n ikki ekanini bilsang, o'n ikkini to'rtga bo'lsak nechta bo'ladi?",
      ...
      on_wrong: { ru: 'Тут не отнимают. Смотри на умножение: три на четыре двенадцать, поэтому двенадцать на четыре это три. Сейчас поймём.', uz: "Bu yerda ayirilmaydi. Ko'paytirishga qara: uch marta to'rt o'n ikki, shuning uchun o'n ikkini to'rtga bo'lsak uch. Hozir tushunamiz." },
  ...
  s1: {
    ...
    info: { ru: 'Знаешь умножение — знаешь и деление: 3 × 4 = 12 → 12 ÷ 3 = 4, 12 ÷ 4 = 3.', uz: "Ko'paytirishni bilsang — bo'lishni ham bilasan: 3 × 4 = 12 → 12 ÷ 3 = 4, 12 ÷ 4 = 3." },
    audio: {
      ru: [
        'Вот три ряда по четыре. Это три умножить на четыре — двенадцать.',
        'Тот же массив показывает деление. Двенадцать разделить на три это четыре, а двенадцать на четыре это три.',
        'Одна картинка — целая семья: одно умножение и два деления. Знаешь умножение — знаешь и деление.'
      ],
      uz: [
        ...
        "Bitta rasm — butun oila: bitta ko'paytirish va ikki bo'lish. Ko'paytirishni bilsang — bo'lishni ham bilasan."
      ]
    }
  },
  ...
  s2: {
    ...
    audio: {
      ru: [
        ...
        'Тот же массив: десять разделить на два это пять, десять на пять это два.',
        'Снова целая семья из одного массива. Одно умножение и два деления.'
      ],
      uz: [
  ...
  s3: {
    ...
    rule: { ru: 'Умножение помогает делить. Одна семья: 3 × 4 = 12 → 12 ÷ 3 = 4, 12 ÷ 4 = 3. Чтобы найти деление, вспомни умножение.', uz: "Ko'paytirish bo'lishga yordam beradi. Bitta oila: 3 × 4 = 12 → 12 ÷ 3 = 4, 12 ÷ 4 = 3. Bo'lishni topish uchun ko'paytirishni esla." },
    check_q: { ru: '2 × 5 = 10. Тогда 10 ÷ 5 = ?', uz: "2 × 5 = 10. Unda 10 ÷ 5 = ?" },
    opts: [{ ru: '2', uz: '2', ok: true }, { ru: '5', uz: '5' }, { ru: '7', uz: '7' }],
    wrong: { ru: 'Смотри на семью: 2 × 5 = 10, значит 10 ÷ 5 = 2.', uz: "Oilaga qara: 2 × 5 = 10, demak 10 ÷ 5 = 2." },
    check_ok: { ru: 'Верно! 2 × 5 = 10, значит 10 ÷ 5 = 2.', uz: "To'g'ri! 2 × 5 = 10, demak 10 ÷ 5 = 2." },
    audio: {
      ru: [
        'Запишем правило. Слушай и запомни.',
        'Умножение помогает делить. Знаешь умножение — знаешь и деление.',
        'Три на четыре двенадцать, значит двенадцать на три четыре, а двенадцать на четыре три.',
        'Проверь. Два на пять это десять. Тогда сколько десять разделить на пять?'
      ],
      uz: [
        "Qoidani yozamiz. Tinglang va yodlang.",
        "Ko'paytirish bo'lishga yordam beradi. Ko'paytirishni bilsang — bo'lishni ham bilasan.",
  ...
  s4: {
    ...
    lead: { ru: 'Дели через умножение.', uz: "Ko'paytirish orqali bo'l." },
    ...
    audio: {
      ru: [
        ...
        'Как найти пятнадцать разделить на три? Спроси: на сколько умножить три, чтобы вышло пятнадцать?',
        'Три на пять пятнадцать. Значит пятнадцать разделить на три это пять.',
        'Проверь. Три на пять это пятнадцать. Тогда сколько пятнадцать разделить на пять?'
      ],
      uz: [
        "Bo'lishni ko'paytirish orqali topish qulay.",
  ...
  sTBL: {
    ...
    info_badge: { ru: 'Главное', uz: 'Asosiy' },
    info: { ru: 'Одна семья: одно умножение и два деления. Знаешь умножение — находишь деление.', uz: "Bitta oila: bitta ko'paytirish va ikkita bo'lish. Ko'paytirishni bilsang — bo'lishni topasan." },
    audio: {
      ru: [
        'Каждый массив рассказывает целую семью равенств.',
        'Четыре ряда по шесть — это четыре на шесть, двадцать четыре.',
        'И то же двадцать четыре: двадцать четыре на четыре это шесть, а на шесть это четыре.'
      ],
      uz: [
  ...
  s5: {
    ...
    lead: { ru: 'Найди пропавшего в семье.', uz: "Oiladagi yo'qni top." },
    ...
    a: 3, b: 4, by: 4,
    wrong: { ru: 'Смотри на умножение семьи: 3 × 4 = 12, значит 12 ÷ 4 = 3.', uz: "Oila ko'paytmasiga qara: 3 × 4 = 12, demak 12 ÷ 4 = 3." },
    done_text: { ru: 'Верно! 3 × 4 = 12, значит 12 ÷ 4 = 3.', uz: "To'g'ri! 3 × 4 = 12, demak 12 ÷ 4 = 3." },
    audio: {
      intro: { ru: 'Тренировка. В семье пропало одно равенство. Смотри на умножение и найди деление. Можешь открыть таблицу.', uz: "Mashq. Oilada bitta tenglik yo'qoldi. Ko'paytirishga qarab bo'lishni top. Jadvalni ochsang bo'ladi." },
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
      on_wrong: { ru: 'Вспомни умножение этой семьи.', uz: "Shu oilaning ko'paytmasini esla." }
    }
  },
  ...
  s6: {
    ...
    audio: {
      ...
      on_wrong: { ru: 'Смотри на результат: он должен совпадать.', uz: "Natijaga qara: u mos kelishi kerak." }
    }
  },
  ...
  s7: {
    ...
    audio: {
      intro: { ru: 'В семье пропало равенство. Смотри на умножение и найди деление.', uz: "Oilada tenglik yo'qoldi. Ko'paytirishga qarab bo'lishni top." },
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
  ...
  s8: {
    ...
    audio: {
      ...
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
      on_wrong: { ru: 'Смотри на результат.', uz: "Natijaga qara." }
    }
  },
  ...
  s9: {
    ...
    done_text: { ru: 'Верно!', uz: "To'g'ri!" },
    audio: {
      intro: { ru: 'Снова семья с пропажей. Найди деление через умножение.', uz: "Yana yo'qolgan oila. Ko'paytirish orqali bo'lishni top." },
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
  ...
  s11: {
    ...
    done_text: { ru: 'Верно!', uz: "To'g'ri!" },
    audio: {
      intro: { ru: 'Последняя тренировка перед задачей. Найди деление через умножение.', uz: "Masaladan oldingi oxirgi mashq. Ko'paytirish orqali bo'lishni top." },
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
      on_wrong: { ru: 'На сколько умножить, чтобы вышло это число?', uz: "Nechaga ko'paytirsak shu son chiqadi?" }
    }
  },
  ...
  s13: {
    ...
    story: { ru: 'Анвар разложил 24 кристалла в 4 равных ряда. Сколько в каждом ряду? Вспомни: 4 × ? = 24.', uz: "Anvar 24 ta kristallni 4 teng qatorga terdi. Har qatorda nechta? Esla: 4 × ? = 24." },
    a: 4, b: 6, by: 4,
  ...
  s14: {
    ...
    audio: {
      intro: { ru: 'Финальная проверка. В семье пропало равенство. Найди деление через умножение.', uz: "Yakuniy tekshiruv. Oilada tenglik yo'qoldi. Ko'paytirish orqali bo'lishni top." },
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
      on_wrong: { ru: 'Вспомни умножение этой семьи.', uz: "Shu oilaning ko'paytmasini esla." }
    }
  },
  ...
  s15: {
    ...
    rule_recap: { ru: 'Умножение и деление — одна семья: 3 × 4 = 12 → 12 ÷ 3 = 4, 12 ÷ 4 = 3. Знаешь одно — знаешь всё.', uz: "Ko'paytirish va bo'lish — bitta oila: 3 × 4 = 12 → 12 ÷ 3 = 4, 12 ÷ 4 = 3. Birini bilsang — hammasini bilasan." },
    ...
    audio: {
      ...
      uz: "Missiya bajarildi. Ko'paytirish va bo'lish bitta oila ekanini bildik. Uch marta to'rt o'n ikki bo'lsa, o'n ikkini uchga bo'lsak to'rt, to'rtga bo'lsak uch. Ko'paytirishni bilsang — bo'lishni oson topasan. Anvar va Zuhra kristallarni oilalarga saraladi. Keyin ikki va uchga bo'lishni o'rganamiz."
    }
  }
};
...
const BRIDGES = {
  ...
  s4:  { ru: 'Дели через умножение.', uz: "Ko'paytirish orqali bo'l." },
  ...
  s7:  { ru: 'Ещё раз найди пропажу.', uz: "Yana yo'qni top." },
  s8:  { ru: 'Снова соединяем семьи.', uz: "Yana oilalarni ulaymiz." },
  s9:  { ru: 'Найди деление через умножение.', uz: "Ko'paytirish orqali bo'lishni top." },
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(16px, 3vw, 24px)', minHeight: 'clamp(200px, 46vw, 280px)' }}>
          <FamilyViz a={2} b={5} reveal={reveal}/>
        </div>
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <FamilyViz a={2} b={5} reveal={done ? 2 : 0}/>
        </div>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <FamilyViz a={3} b={5} reveal={done ? 2 : 0}/>
        </div>
...
// Dars20 wrapper'lari — ×↔÷ oila mexanikasiga ulanadi (DealStage/ArrayRevStage/ArrayStage/CommuteStage endi o'lik kod).
const A5 = (props) => <FamilyFindStage props={props} cKey="s5"/>;
...
const A9 = (props) => <FamilyFindStage props={props} cKey="s9"/>;
const A10 = (props) => <MatchStage props={props} cKey="s10"/>;
const A11 = (props) => <FamilyFindStage props={props} cKey="s11"/>;
const ACase = (props) => <FamilyFindStage props={props} cKey="s13"/>;
...
@media (prefers-reduced-motion: reduce) { .d20-wire { animation: none; stroke-dasharray: none; } }
@media (prefers-reduced-motion: reduce) {

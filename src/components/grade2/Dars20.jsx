...
const SCREEN_META = [
  ...
  { id: 's3',  type: 'rule',        template: 'custom',   scored: false, scope: null },        // 3  QOIDA: ×ni bilsangiz ÷ni ham bilasiz + check
...
// SwipeHand — sudrash ishorasi: chap katakni o'ng katakka surish (bolaga vizual ko'rsatma)
const SwipeHand = () => (
        ...
        <div className="frame fade-up delay-1" style={{ padding: 'clamp(12px, 2.4vw, 18px)' }}>
          <div ref={boxRef} style={{ position: 'relative' }}>
            ...
            {canAct && matched.size === 0 && !drag && (
              <div aria-hidden="true" className="d20-swipe" style={{ position: 'absolute', top: 'clamp(16px,3.4vw,22px)', left: '31%', zIndex: 5, pointerEvents: 'none' }}>
                <SwipeHand/>
              </div>
            )}
          </div>
...
/* SWIPE-HINT: sudrab javob berish ishorasi (qo'l chapdan o'ngga suriladi) */
@keyframes d20swipe { 0%,8% { transform: translateX(0) scale(1); opacity: 0; } 16% { transform: translateX(0) scale(1); opacity: .95; } 28% { transform: translateX(0) scale(.8); opacity: .95; } 72% { transform: translateX(var(--sw,110px)) scale(.8); opacity: .95; } 84% { transform: translateX(var(--sw,110px)) scale(1); opacity: 0; } 100% { opacity: 0; } }
.d20-swipe { --sw: clamp(74px,30vw,130px); animation: d20swipe 2.4s ease-in-out infinite; will-change: transform, opacity; }
@media (prefers-reduced-motion: reduce) { .d20-swipe { animation: none; opacity: .85; transform: none; } }
@media (prefers-reduced-motion: reduce) {

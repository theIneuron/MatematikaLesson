...
// ============================================================================
// ░░ 2-SINF · Dars08 — "Ayirish (o'tishsiz)" / "Вычитание без перехода" (sub-2-08-v1) · Б2 MARS · spec: ETALON_2SINF.md §11 ░░
// MEXANIKA: ustunlab (столбик) ayirish — xonama-xona; birlikdan birlik, o'nlikdan o'nlik (o'tishsiz).
// Vizual: ColumnCard/ColumnSub (столбик karta, − amali), DropColumnStage (raqam-plita drag-drop), MarsBase (s0 sahna).
...
const CONTENT = {
  ...
  s0: {
    ...
    audio: {
      intro: {
        ru: [
          'Сегодня тема урока, вычитание чисел столбиком. Научимся вычитать по разрядам.',
          'Корабль Бита остался на красной планете Марс. На грузовой базе считают ящики.',
          'На базе было пятьдесят девять ящиков. Двадцать пять погрузили на корабль. Сколько осталось?',
          'Послушай два ответа. Первый, сорок три. Второй, тридцать четыре. Или ты пока не знаешь. Выбери свой ответ.'
        ],
        uz: [
          "Bugungi dars mavzusi, sonlarni ustunlab ayirish. Xonalab ayirishni o'rganamiz.",
  ...
  s1: {
    ...
    audio: {
      ru: [
        ...
        'Пятьдесят девять это пятьдесят и девять. Двадцать пять это двадцать и пять.',
  ...
  s2: {
    ...
    audio: {
      ru: [
        ...
        'Всегда начинаем справа, с единиц. Девять минус пять это четыре. Пишем четыре в столбик единиц.',
        'Теперь идём налево, к десяткам. Пять минус два это три. Пишем три в столбик десятков.',
        'Получилось тридцать четыре. Пятьдесят девять минус двадцать пять это тридцать четыре.'
      ],
      uz: [
        "Ellik to'qqizdan yigirma beshni ustunlab ayiramiz.",
  ...
  s3: {
    ...
    audio: {
      ru: [
        ...
        'Вычитаем по разрядам, каждый столбик отдельно. Ведь число состоит из десятков и единиц.',
        'Сначала вычитаем единицы, потом десятки.',
        'Единицы пишем под единицами, десятки под десятками.',
        'А теперь сам. Вычти единицы: девять минус пять.'
      ],
      uz: [
        "Ustunlab ayirish qoidasini yozamiz. Tinglang va yodlang.",
        "Xonalab ayiramiz, har ustun alohida. Chunki son o'nlik va birliklardan tuzilgan.",
  ...
  s4: {
    ...
    audio: {
      ru: [
        ...
        'Единицы: восемь минус шесть это два.',
        'Десятки: шесть минус один это пять. Получилось пятьдесят два.',
        'Вычти десятки сам: шесть минус один.'
      ],
      uz: [
        "Yana bir misol. Oltmish sakkizdan o'n oltini ustunlab ayiramiz.",
  ...
  s5: {
    ...
    audio: {
      ru: [
        ...
        'Единицы: семь минус один это шесть. Десятки: четыре минус два это два.',
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ padding: 'clamp(8px, 1.8vw, 14px)', overflow: 'hidden' }}>
          <MarsBase answer={picked !== null ? 34 : null}/>
        </div>
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(16px, 3vw, 24px)', minHeight: 'clamp(180px, 40vw, 240px)' }}>
          <ColumnSub a={59} b={25} sum={done ? 34 : null} emph={done ? null : emph}/>
        </div>
...
const Screen3 = (props) => {
  ...
  const curAns = isUnits ? 4 : 3;              // 9−5=4 (birlik), 5−2=3 (o'nlik)
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <ColumnSub a={59} b={25} sum={done ? 34 : null} emph={done ? null : phase}/>
        </div>
...
// ColumnSub — Dars08 star-vizual: ustunlab ayirish (столбик). O'nlik ustuni sariq, birlik ko'k.
...
const ColumnSub = ({ a, b, sum = null, emph = null }) => {
...
// RazryadBreak — s1 «NEGA ustunlab» tushuntirishi: har sonni razryadga ajratadi (59 = 50 + 5).
  ...
  return (
    <div className="d2-scene" style={{ background: 'radial-gradient(ellipse at 50% 20%, #34180f 0%, #140a08 78%)' }}>
      ...
      <div style={{ position: 'absolute', left: '48%', bottom: '17%', transform: 'translateX(-50%)', display: 'flex', alignItems: 'center', gap: 'clamp(5px,1.4vw,10px)', zIndex: 5 }}>
        <CargoCrate n={59} ca="#FF9166" cb="#fe5b1a" br="#C1381A"/>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <ColumnSub a={68} b={16} sum={done ? 52 : null} emph={done ? null : 'tens'}/>
        </div>
...
// natija (qoldiq) yashil tabloda ✓ bilan chiqadi. Eski «MarsApproach» (kemaning Marsga
...
const MarsCargoDone = ({ total, label }) => {
  ...
  return (
    <div className="d2-scene" style={{ background: 'radial-gradient(ellipse at 50% 20%, #34180f 0%, #140a08 78%)' }}>
      ...
      {/* qolgan yuk (dekor, ketma-ket pop-in) */}

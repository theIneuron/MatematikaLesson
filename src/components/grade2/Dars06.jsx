...
const FinalMixed = (props) => {
  ...
  const allDone = solved && isLast;
  const revealRef = useRevealScroll(solved, 400);
  const correctIdx = isPlace ? -1 : opts.findIndex((o) => o.ok);
  const report = (correct, studentAns) => {
    if (!meta.scored || !props.onAnswer) return;
    props.onAnswer({ stage: meta.scope, screenIdx: props.screen, subIndex: ri, question: t(cur.q), options: isPlace ? null : opts.map((o) => o[lang]), correctIndex: isPlace ? null : correctIdx, correctAnswer: isPlace ? String(cur.target) : opts[correctIdx][lang], studentAnswerIndex: null, studentAnswer: studentAns, correct, firstTry: correct, attempts: attemptsRef.current, solved: true });
  };
  const finishRound = () => {
    sfx.playCorrect();
    setSolved(true);

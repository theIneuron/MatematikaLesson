    ...
    <div className="d2-scene" style={{ background: 'radial-gradient(ellipse at 50% 16%, #3a1a10 0%, #120806 80%)' }}>
      ...
      <div className="d12-rocket" style={{ position: 'absolute', left: '50%', bottom: '30%', width: 'clamp(58px,15.5vw,90px)', zIndex: 6 }}>
        <div className="d12-shake" style={{ position: 'relative' }}>
          {/* olov gulxani */}
          <div className="d12-flame" aria-hidden="true" style={{ position: 'absolute', left: '50%', top: '90%', width: '44%', height: 'clamp(42px,12vw,72px)', background: 'linear-gradient(180deg,#FFF3C0 0%,#FF9A3C 46%,#fe5b1a 80%, rgba(254,91,26,0) 100%)', borderRadius: '46% 46% 50% 50% / 30% 30% 100% 100%', filter: 'blur(0.6px)', transformOrigin: '50% 0%' }}/>
    ...
    <>
      ...
      {!solved1 && (
        <>
          ...
          <div className="fade-up" style={{ display: 'flex', justifyContent: 'center' }}>
            <span style={{ ...CHAIN_BOX, minWidth: 'clamp(64px,16vw,84px)', fontSize: 'clamp(26px,6vw,36px)', background: flash ? '#FFF1EA' : '#FFF1E2', color: flash ? '#fe5b1a' : '#E8891C', border: `2.5px ${typed ? 'solid' : 'dashed'} ${flash ? '#fe5b1a' : '#E8891C'}` }}>{typed || '?'}</span>
          </div>
...
const OraliqErrorBody = ({ round, canAct, onSolved, onWrongTry, revealRef, wrongText, doneText }) => {
  ...
  const errCell = (cell, disp, correct) => {
    ...
    const bd = errRed || flashRed ? '#fe5b1a' : '#B9C4D2';
    const bg = errRed || flashRed ? '#FFF1EA' : '#F2F5F8';
    return (
      <button onClick={() => tap(cell)} disabled={!canAct || solved}
        style={{ ...CHAIN_BOX, background: bg, border: `2.5px dashed ${bd}`, cursor: solved ? 'default' : 'pointer', color: errRed ? T.success : (flashRed ? '#fe5b1a' : T.ink), transition: 'all .2s' }}>

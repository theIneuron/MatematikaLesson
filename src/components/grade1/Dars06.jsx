...
// ============================================================================
// ░░ 1-SINF · Dars06 — "Son tarkibi 6–10" (num-1-06-v1) · syujet:
// Dars05 davomi: endi sonlar kattaroq (6–10). Olmalarni qizil va yashilga ajratish —
// son ikki qismdan iborat. Oxirida o'nning juftlari (до-10). ░░
// METOD: Dars05'dan 1:1 ko'chirildi — ikki rangli ten-frame (BondFrame) + Singapur
// number-bond (NumberBond) + CPA (konkret -> tasviriy -> belgi). Faqat sonlar 6–10 ga
// kengaytirildi; ten-frame 10 katak (ikki qator, "besh va yana").
//
...
const LESSON_META = {
  lessonId: 'num-1-06-v1',
  lessonTitle: { ru: 'Состав числа 6–10', uz: "Son tarkibi 6–10" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // hikoya: Anvar olma bilan; endi sonlar kattaroq (6–10)
  { id: 's0',     type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: 8 ni necha xil ikki guruhga bo'lish mumkin?
  { id: 's1',     type: 'exploration', template: 'custom',   scored: false, scope: null },            // tap-to-fill: 6 ramkasini qizil/yashil bilan to'ldirish
  { id: 's2',     type: 'exploration', template: 'custom',   scored: false, scope: null },            // berk qism: 5 qizil ochiq, jami 8 -> berk yashilni ochib topish
  { id: 's3',     type: 'rule',        template: 'custom',   scored: false, scope: null },            // qoida: son ikki qismdan (avto-aylanuvchi juftlar)
  { id: 's4',     type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 7 = 4 va ?
  { id: 's5',     type: 'exploration', template: 'custom',   scored: false, scope: null },            // yangi son 8: tap-to-fill splitlari
  { id: 's6',     type: 'rule',        template: 'custom',   scored: false, scope: null },            // qoida: o'rin almashinuvi (avto rang almashinuvi)
  { id: 's7',     type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 9 = ? va 2
  { id: 's8',     type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // ha/yo'q: 5 qizil va 2 yashil birga 8mi? (yo'q, 7)
  { id: 's9',     type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qaysi juft birga 9 ni beradi?
  { id: 's10',    type: 'test',        template: 'custom',   scored: true,  scope: 'module-mikro' },  // juftlash: qizil qism -> yetishmagan yashil son (o'ngacha)
  ...
  { id: 's11',    type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: 10 = 7 va ? + fakt
...
const CONTENT = {
  // ---- sIntro HIKOYA — Bit boshlovchi; Dars05'dan davom: endi sonlar kattaroq (6–10) ----
  sIntro: {
    eyebrow: { ru: 'История', uz: 'Hikoya' },
    title: { ru: 'Теперь яблок больше', uz: "Endi olma ko'proq" },
    bit_label: { ru: 'Бит', uz: 'Bit' },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    audio: {
      ru: [
        'Привет, друг! В прошлый раз Анвар разделил пять яблок на красные и зелёные.',
        'Сегодня яблок больше, от шести до десяти.',
        'Рано говорит: большие числа тоже делятся на две части. Слушай до конца и нажимай дальше.'
      ],
      uz: [
        "Salom, do'stim! O'tgan safar Anvar beshta olmani qizil va yashilga ajratdi.",
        "Bugun olma ko'proq, oltidan o'ngacha.",
        "Ra'no aytadi: katta sonlar ham ikki qismga bo'linadi. Oxirigacha tinglang va davom bosing."
      ]
    }
  },

  // ---- s0 HOOK — jumboq: 8 ni necha xil ikki qismga bo'lish mumkin? (to'g'ri javob yo'q) ----
  s0: {
    eyebrow: { ru: 'Загадка', uz: 'Topishmoq' },
    title_part1: { ru: 'Восемь яблок на', uz: 'Sakkizta olmani' },
    ...
    audio: {
      intro: {
        ru: 'Восемь яблок можно разделить на две части. А как думаешь ты, способ один или их много? Нажми любой ответ.',
        uz: "Sakkizta olmani ikki qismga ajratish mumkin. Sizningcha-chi, yo'l bittami yoki ko'pmi? Qaysi javob bo'lsa ham bosing."
      },
  ...
  // ---- s1 EXPLORATION — tap-to-fill: 6 ramkasini qizil bilan to'ldirish, qolgani yashil ----
  s1: {
    ...
    done_text: { ru: 'Видишь? Шесть — это красные и зелёные вместе. Частей бывает по-разному.', uz: "Ko'rdingizmi? Olti — bu qizil va yashil birga. Qismlar har xil bo'ladi." },
    done_audio: { ru: 'Видишь? Шесть — это красные и зелёные вместе. Частей бывает по-разному.', uz: "Ko'rdingizmi? Olti — bu qizil va yashil birga. Qismlar har xil bo'ladi." },
    audio: {
      ru: [
        'Здесь шесть яблок. Нажимай на яблоко, оно станет красным.',
        'Сколько яблок ты не тронул, столько останется зелёных. Красные и зелёные вместе всегда шесть.'
      ],
      uz: [
        "Bu yerda oltita olma bor. Olmani bosing, olma qizil bo'ladi.",
        "Nechta olmani bosmasangiz, shuncha yashil qoladi. Qizil va yashil birga doim olti."
      ]
    }
  },

  // ---- s2 EXPLORATION — berk qism: 5 qizil ochiq, jami 8 -> ochib yashilni topish (3) ----
  s2: {
    eyebrow: { ru: 'Закрытая часть', uz: 'Berk qism' },
    instruction: { ru: 'Пять яблок красные. Всего восемь. Угадай, сколько зелёных закрыто, и открой', uz: "Beshta olma qizil. Hammasi sakkiz. Nechta yashil berk — toping va oching" },
    done_text: { ru: 'Открыли! Три зелёных. Восемь это пять и ещё три.', uz: "Ochdik! Uchta yashil. Sakkiz, bu besh va yana uch." },
    audio: {
      ru: [
        'Пять яблок уже красные. А всего у нас восемь.',
        'Значит, зелёные яблоки спрятались. Сколько их? Открой и проверь. Восемь это пять и ещё три.'
      ],
      uz: [
        "Beshta olma qizil bo'ldi. Hammasi esa sakkizta.",
        "Demak, yashil olmalar yashiringan. Ular nechta? Oching va tekshiring. Sakkiz, bu besh va yana uch."
      ]
    }
  },

  // ---- s3 RULE — son ikki qismdan (avto-aylanuvchi juftlar, 7) ----
  s3: {
    ...
    tip: {
      ru: 'Семь — это два и пять, три и четыре, четыре и три, пять и два. Части разные, а вместе всегда семь.',
      uz: "Yetti — bu ikki va besh, uch va to'rt, to'rt va uch, besh va ikki. Qismlar har xil, birga esa doim yetti."
    },
    audio: {
      ru: 'Любое число можно разделить на две части. Смотри. Два красных и пять зелёных. Три красных и четыре зелёных. Четыре красных и три зелёных. Части меняются, а вместе всегда семь.',
      uz: "Har qanday sonni ikki qismga bo'lish mumkin. Qarang. Ikki qizil va besh yashil. Uch qizil va to'rt yashil. To'rt qizil va uch yashil. Qismlar o'zgaradi, birga esa doim yetti."
    }
  },

  // ---- s4 TEST MC — 7 = 4 va ? · plitalar [7,4,3,5], to'g'ri = uch (idx2) ----
  s4: {
    eyebrow: { ru: 'Тренировка · 1 / 6', uz: 'Mashq · 1 / 6' },
    title: { ru: 'Четыре яблока красные. Всего семь. Сколько зелёных?', uz: "To'rtta olma qizil. Hammasi yetti. Nechta yashil?" },
    correct_text: { ru: 'Верно. Четыре и ещё три — это семь.', uz: "To'g'ri. To'rt va yana uch — bu yetti." },
    wrong_0: {
      ru: 'Это всё вместе. Это не одна часть. Часть меньше целого.',
      uz: "Bu hammasi birga. Bu bitta qism emas. Qism butundan kichik."
    },
    wrong_1: {
      ru: 'Столько уже красных. Зелёных другое число. Считай от четырёх до семи.',
      uz: "Bu qizil olmalar soni. Yashillari boshqa son. To'rtdan yettigacha sanang."
    },
    wrong_3: {
      ru: 'Это на два больше. Посчитай зелёные по одному.',
      uz: "Bu ikkitaga ko'p. Yashil olmalarni bittadan sanang."
    },
    wrong_default: {
      ru: 'Не совсем. Считай от четырёх до семи и узнаешь, сколько зелёных.',
      uz: "Unchalik emas. To'rtdan yettigacha sanang va nechta yashil ekanini bilasiz."
    },
    audio: {
      intro: { ru: 'Четыре яблока красные. Всего семь. Сколько яблок зелёные? Нажми число.', uz: "To'rtta olma qizil. Hammasi yetti. Nechta olma yashil? Sonni bosing." },
      on_correct: { ru: 'Верно. Их три.', uz: "To'g'ri. Ular uchta." },
      on_wrong: { ru: 'Не совсем. Посчитай ещё раз.', uz: "Unchalik emas. Yana sanang." }
    }
  },

  // ---- s5 EXPLORATION — yangi son 8: tap-to-fill splitlari ----
  s5: {
    eyebrow: { ru: 'Другое число', uz: 'Boshqa son' },
    instruction: { ru: 'Теперь яблок восемь. Нажимай — красные и зелёные, а вместе восемь', uz: "Endi olma sakkizta. Bosing — qizil va yashil, birga esa sakkiz" },
    done_text: { ru: 'Восемь тоже состоит из частей. Три и пять, четыре и четыре, шесть и два.', uz: "Sakkiz ham qismlardan iborat. Uch va besh, to'rt va to'rt, olti va ikki." },
    done_audio: { ru: 'Восемь тоже состоит из частей. Три и пять, четыре и четыре, шесть и два.', uz: "Sakkiz ham qismlardan iborat. Uch va besh, to'rt va to'rt, olti va ikki." },
    audio: {
      ru: [
        'Не только шесть состоит из частей. Возьмём восемь яблок.',
        'Нажимай и смотри. Красные и зелёные вместе всегда восемь.'
      ],
      uz: [
        "Faqat olti emas, har son qismlardan iborat. Sakkizta olmani olamiz.",
        "Bosing va qarang. Qizil va yashil birga doim sakkiz."
      ]
    }
  },

  // ---- s6 RULE — o'rin almashinuvi (avto, 4 va 3 = 3 va 4 = 7) ----
  s6: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Поменяй части местами —', uz: 'Qismlarni almashtiring —' },
    title_part2_em: { ru: 'число не изменится', uz: "son o'zgarmaydi" },
    tip: {
      ru: 'Четыре красных и три зелёных — это семь. Три красных и четыре зелёных — тоже семь. Части поменялись, а всего столько же.',
      uz: "To'rt qizil va uch yashil — bu yetti. Uch qizil va to'rt yashil — ham yetti. Qismlar almashdi, hammasi esa o'sha."
    },
    audio: {
      ru: 'Смотри на яблоки. Четыре красных и три зелёных, вместе семь. Теперь поменяем местами. Три красных и четыре зелёных, снова семь. Части можно менять местами, число от этого не меняется.',
      uz: "Olmalarga qarang. To'rt qizil va uch yashil, birga yetti. Endi o'rin almashtiramiz. Uch qizil va to'rt yashil, yana yetti. Qismlarni almashtirsa bo'ladi, son bundan o'zgarmaydi."
    }
  },

  // ---- s7 TEST MC — 9 = ? va 2 (2 yashil ko'rinadi, qizil berk) · plitalar [7,9,2,6], to'g'ri = yetti (idx0) ----
  s7: {
    eyebrow: { ru: 'Тренировка · 2 / 6', uz: 'Mashq · 2 / 6' },
    title: { ru: 'Два яблока зелёные. Всего девять. Сколько красных?', uz: "Ikkita olma yashil. Hammasi to'qqiz. Nechta qizil?" },
    correct_text: { ru: 'Верно. Семь и ещё два — это девять.', uz: "To'g'ri. Yetti va yana ikki — bu to'qqiz." },
    wrong_1: {
      ru: 'Это всё вместе. Это не одна часть. Часть меньше целого.',
      uz: "Bu hammasi birga. Bu bitta qism emas. Qism butundan kichik."
    },
    wrong_2: {
      ru: 'Столько зелёных. Красных другое число. Считай от двух до девяти.',
      uz: "Bu yashil olmalar soni. Qizillari boshqa son. Ikkidan to'qqizgacha sanang."
    },
    wrong_3: {
      ru: 'Это на одно меньше. Посчитай красные яблоки.',
      uz: "Bu bittaga kam. Qizil olmalarni sanang."
    },
    wrong_default: {
      ru: 'Не совсем. Считай от двух до девяти и узнаешь, сколько красных.',
      uz: "Unchalik emas. Ikkidan to'qqizgacha sanang va nechta qizil ekanini bilasiz."
    },
    audio: {
      intro: { ru: 'Два яблока зелёные. Всего девять. Сколько яблок красные? Нажми число.', uz: "Ikkita olma yashil. Hammasi to'qqiz. Nechta olma qizil? Sonni bosing." },
      on_correct: { ru: 'Верно. Их семь.', uz: "To'g'ri. Ular yettita." },
      on_wrong: { ru: 'Не совсем. Посчитай ещё раз.', uz: "Unchalik emas. Yana sanang." }
    }
  },

  // ---- s8 TEST ha/yo'q — 5 qizil + 2 yashil birga 8mi? (yo'q, bu 7). To'g'ri = Yo'q (idx1) ----
  s8: {
    eyebrow: { ru: 'Тренировка · 3 / 6', uz: 'Mashq · 3 / 6' },
    title: { ru: 'Пять красных и две зелёных. Бит говорит: вместе восемь. Это верно?', uz: "Besh qizil va ikki yashil. Bit aytadi: birga sakkiz. Bu to'g'rimi?" },
    opt0: { ru: 'Да, верно', uz: "Ha, to'g'ri" },
    opt1: { ru: 'Нет, неверно', uz: "Yo'q, noto'g'ri" },
    correct_text: { ru: 'Верно. Здесь пять и две — это семь, а не восемь.', uz: "To'g'ri. Bu yerda besh va ikki — yetti, sakkiz emas." },
    wrong_0: {
      ru: 'Посчитай вместе. Пять и ещё две, вместе семь. До восьми не хватает одного.',
      uz: "Birga sanang. Besh va yana ikki, birga yetti. Sakkizgacha bittasi yetmaydi."
    },
    wrong_default: {
      ru: 'Посчитай обе части вместе. Получится семь, а не восемь.',
      uz: "Ikkala qismni birga sanang. Yetti chiqadi, sakkiz emas."
    },
    audio: {
      intro: { ru: 'Здесь пять красных и две зелёных яблока. Бит говорит, что вместе восемь. Посчитай и нажми да или нет.', uz: "Bu yerda besh qizil va ikki yashil olma bor. Bit birga sakkiz deydi. Sanang va ha yoki yo'q bosing." },
      on_correct: { ru: 'Верно. Это семь.', uz: "To'g'ri. Bu yetti." },
      on_wrong: { ru: 'Не совсем. Посчитай обе части.', uz: "Unchalik emas. Ikkala qismni sanang." }
    }
  },

  // ---- s9 TEST MC — qaysi juft birga 9? juftlar [(6,2),(6,3),(5,3)], to'g'ri = idx1 ----
  s9: {
    eyebrow: { ru: 'Тренировка · 4 / 6', uz: 'Mashq · 4 / 6' },
    title: { ru: 'У какой пары вместе ровно девять?', uz: "Qaysi juftda birga aniq to'qqiz bor?" },
    correct_text: { ru: 'Верно. Шесть и три — это ровно девять.', uz: "To'g'ri. Olti va uch — aniq to'qqiz." },
    wrong_0: {
      ru: 'Здесь вместе восемь. До девяти не хватает одного. Посчитай обе части.',
      uz: "Bu yerda birga sakkiz. To'qqizgacha bittasi yetmaydi. Ikkala qismni sanang."
    },
    wrong_2: {
      ru: 'Здесь тоже восемь. До девяти не хватает одного. Посчитай обе части.',
      uz: "Bu yerda ham sakkiz. To'qqizgacha bittasi yetmaydi. Ikkala qismni sanang."
    },
    wrong_default: {
      ru: 'Не совсем. Считай обе части вместе и ищи девять.',
      uz: "Unchalik emas. Ikkala qismni birga sanang va to'qqizni qidiring."
    },
    audio: {
      intro: { ru: 'Перед тобой три пары. Найди ту, где красные и зелёные вместе ровно девять. Посчитай и нажми.', uz: "Oldingizda uchta juft bor. Qizil va yashil birga aniq to'qqiz bo'lgan juftni toping. Sanang va bosing." },
      on_correct: { ru: 'Верно. Вместе девять.', uz: "To'g'ri. Birga to'qqiz." },
      on_wrong: { ru: 'Не совсем. Посчитай обе части.', uz: "Unchalik emas. Ikkala qismni sanang." }
    }
  },

  // ---- s10 TEST juftlash — qizil qism -> yetishmagan yashil son (o'ngacha, до-10) ----
  s10: {
    eyebrow: { ru: 'Тренировка · 5 / 6', uz: 'Mashq · 5 / 6' },
    instruction: { ru: 'Сколько зелёных не хватает до десяти? Нажми число, потом его яблоки', uz: "O'ngacha nechta yashil yetishmaydi? Sonni bosing, keyin uning olmalarini bosing" },
    correct_text: { ru: 'Верно. Вместе ровно десять.', uz: "To'g'ri. Birga aniq o'n." },
    done_text: { ru: 'Молодец! Каждая часть нашла свою пару до десяти.', uz: "Barakalla! Har qism o'ngacha o'z juftini topdi." },
    wrong_default: { ru: 'Здесь вместе не десять. Посчитай красные и подбери зелёные до десяти.', uz: "Bu yerda birga o'n emas. Qizillarni sanang va o'ngacha yashilni toping." },
    audio: {
      intro: { ru: 'У каждой части свои красные яблоки. Подбери, сколько зелёных не хватает до десяти. Сначала нажми число, потом его яблоки.', uz: "Har qismda qizil olmalar bor. O'ngacha nechta yashil yetishmasligini toping. Avval sonni bosing, keyin uning olmalarini bosing." },
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
      on_wrong: { ru: 'Не совсем. Посчитай до десяти.', uz: "Unchalik emas. O'ngacha sanang." }
    }
  },

  // ---- sDrill TRENIROVKA — 6 mashq ketma-ket (yetishmagan qism, 6–10), ball yo'q ----
  sDrill: {
  ...
  // ---- sd MINI-O'YIN — maqsadli raundlar: qizil yasang (tap, ball yo'q). Jami 8. ----
  sd: {
    ...
    audio: {
      intro: { ru: 'Поиграем. Здесь восемь яблок. Бит просит сделать несколько красных. Нажимай, яблоко станет красным, остальные будут зелёными.', uz: "O'ynaymiz. Bu yerda sakkizta olma bor. Bit bir nechta qizil olma so'raydi. Bosing, olma qizil bo'ladi, qolgani yashil bo'ladi." }
    }
  },

  // ---- sGuest KO'PRIK — Anvar tan oladi; до-10 juftlariga ishora ----
  sGuest: {
    eyebrow: { ru: 'Возвращаемся к Анвару', uz: 'Anvarga qaytamiz' },
    title: { ru: 'Анвар собрал десять', uz: "Anvar o'nni yig'di" },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    audio: {
      ru: [
        'Помнишь, в начале Анвар думал, что число можно показать только одним способом?',
        'Теперь он сам разделил большие числа по-разному. И запомнил пары для десяти. Пять и пять. Семь и три.',
        'Анвар понял. И большое число состоит из двух частей. Осталось одно задание.'
      ],
      uz: [
        "Esingizdami, boshida Anvar sonni faqat bitta yo'l bilan ko'rsatiladi deb o'ylagandi?",
        "Endi u katta sonlarni o'zi turlicha ajratdi. Va o'nning juftlarini esladi. Besh va besh. Yetti va uch.",
        "Anvar tushundi. Katta son ham ikki qismdan iborat. Bitta topshiriq qoldi."
      ]
    }
  },

  // ---- s11 TEST final + FAKT — 10 = 7 va ? (7 qizil + berk yashil) · plitalar [7,10,4,3], to'g'ri = uch (idx3) ----
  s11: {
    eyebrow: { ru: 'Итог', uz: 'Yakun' },
    title: { ru: 'Семь яблок красные. Всего десять. Сколько зелёных?', uz: "Yettita olma qizil. Hammasi o'n. Nechta yashil?" },
    correct_text: { ru: 'Верно. Семь и ещё три — это десять.', uz: "To'g'ri. Yetti va yana uch — bu o'n." },
    wrong_0: {
      ru: 'Столько уже красных. Зелёных другое число. Считай от семи до десяти.',
      uz: "Bu qizil olmalar soni. Yashillari boshqa son. Yettidan o'ngacha sanang."
    },
    wrong_1: {
      ru: 'Это всё вместе. Это не одна часть. Часть меньше целого.',
      uz: "Bu hammasi birga. Bu bitta qism emas. Qism butundan kichik."
    ...
    wrong_2: {
      ru: 'Это на одно больше. Посчитай зелёные яблоки.',
      uz: "Bu bittaga ko'p. Yashil olmalarni sanang."
    },
    wrong_default: {
      ru: 'Не совсем. Считай от семи до десяти и узнаешь, сколько зелёных.',
      uz: "Unchalik emas. Yettidan o'ngacha sanang va nechta yashil ekanini bilasiz."
    },
    fact_badge: { ru: 'А знаешь? · Игра', uz: "Bilasizmi? · O'yin" },
    fact_text: {
      ru: 'На косточке домино всегда две части с точками. Сложишь их — получишь число косточки. Кто помнит пары до десяти, тот играет быстрее.',
      uz: "Domino toshida doim ikki qism — nuqtalar bor. Ularni qo'shsangiz, tosh soni chiqadi. O'ngacha juftlarni bilgan tezroq o'ynaydi."
    },
    fact_audio: {
      ru: 'А знаешь, на косточке домино всегда две части с точками. Сложишь их, получишь число косточки.',
      uz: "Bilasizmi, domino toshida doim nuqtali ikki qism bor. Ularni qo'shsangiz, tosh soni chiqadi."
    },
    audio: {
      intro: { ru: 'Последнее задание. Семь яблок красные. Всего десять. Сколько яблок зелёные? Нажми число.', uz: "Oxirgi topshiriq. Yettita olma qizil. Hammasi o'n. Nechta olma yashil? Sonni bosing." },
      on_correct: { ru: 'Верно. Их три.', uz: "To'g'ri. Ular uchta." },
      on_wrong: { ru: 'Не совсем. Посчитай от семи до десяти.', uz: "Unchalik emas. Yettidan o'ngacha sanang." }
    }
  },
  ...
  s12: {
    ...
    main_2_em: { ru: 'делишь большие числа на две части', uz: "katta sonlarni ikki qismga bo'lasiz" },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    conn_label_refs: { ru: 'Опирается на', uz: 'Tayanadi' },
    conn_refs: { ru: 'Состав числа 2–5', uz: "Sonning tarkibi 2–5" },
    conn_label_next: { ru: 'Следующий урок', uz: 'Keyingi dars' },
    conn_next: { ru: 'Смысл сложения.', uz: "Qo'shishning ma'nosi." },
    audio: {
      ru: 'Сегодня Анвар увидел, что большие числа тоже делятся на части. А ты научился делить числа от шести до десяти и запомнил пары для десяти. Части бывают разные, а всего остаётся столько же. На следующем уроке начнём складывать. Молодец!',
      uz: "Bugun Anvar katta sonlar ham qismlarga bo'linishini ko'rdi. Siz esa oltidan o'ngacha sonlarni bo'lishni va o'nning juftlarini o'rgandingiz. Qismlar har xil, hammasi esa o'sha bo'lib qoladi. Keyingi darsda qo'shishni boshlaymiz. Barakalla!"
    }
  }
};
// ============================================================
...
// BondFrame — 10 katakli ramka (ikki qator, "besh va yana"); 0..a-1 qizil olma, a..total-1 yashil, qolgani bo'sh.
...
// ============================================================
// EKRANLAR — Dars06 (sonning tarkibi 6–10)
// ============================================================
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(22px, 4.4vw, 32px)' }}>
          <div className="g1-scene">
            ...
            <div className="g1-cast-row">
              ...
              <div className={`g1-cast g1-comp-frame ${step >= 2 ? 'in' : ''}`}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ padding: 'clamp(14px, 2.8vw, 20px)' }}>
          ...
          <p className="g1-explore-hint">
            {lang === 'uz' ? "Olmani bosing — rangi o'zgaradi" : 'Нажми клетку — цвет поменяется'}
...
const Screen1 = (props) => <FillExplore c={CONTENT.s1} total={6} needWays={3} {...props}/>;
const Screen5 = (props) => <FillExplore c={CONTENT.s5} total={8} needWays={3} {...props}/>;
...
const Screen2 = (props) => {
  ...
  const reveal = () => {
    if (open) return;
    setOpen(true);
    if (!audio.muted) { const e = getAudioEngine(); if (e) { e.pushOneOff(NUM_WORDS[lang][3]); e.pushOneOff(c.done_text[lang]); } }
  };
  const navContent = (
    <>
      <NavBack onPrev={props.onPrev} label={<BackLabel/>}/>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <p className="h-sub title fade-up">{t(c.instruction)}</p>
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <BondFrame total={8} a={5} hide={!open} anim={open}/>
        </div>
        {!open && (
...
// s4 — TEST MC: 7 = 4 va ? (ramka 4 qizil + berk yashil; javobda ochiladi). Plitalar [7,4,3,5], to'g'ri idx2.
const Screen4 = (props) => {
  const c = CONTENT.s4;
  const t = useT();
  return (
    <QuestionScreen
      ...
      figure={(solved) => <BondFrame total={7} a={4} hide={!solved} anim={solved}/>}
      options={[<NumTile d={7}/>, <NumTile d={4}/>, <NumTile d={3}/>, <NumTile d={5}/>]}
      correctIdx={2}
      mascot={false}
...
// s7 — TEST MC: 9 = ? va 2 (2 yashil ko'rinadi, qizil berk). Plitalar [7,9,2,6], to'g'ri idx0.
const Screen7 = (props) => {
  const c = CONTENT.s7;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub">{t(c.title)}</h2>}
      figure={(solved) => <BondFrame total={9} a={7} hide={!solved} hideSide="red" anim={solved}/>}
      options={[<NumTile d={7}/>, <NumTile d={9}/>, <NumTile d={2}/>, <NumTile d={6}/>]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// s8 — TEST ha/yo'q: 5 qizil + 2 yashil birga 8mi? To'g'ri = Yo'q, bu 7 (idx1).
const Screen8 = (props) => {
  const c = CONTENT.s8;
  const t = useT();
  return (
...
const Screen9 = (props) => {
  ...
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub">{t(c.title)}</h2>}
      options={[<PairCard a={6} b={2}/>, <PairCard a={6} b={3}/>, <PairCard a={5} b={3}/>]}
      correctIdx={1}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// s10 — TEST juftlash: qizil qism -> yetishmagan yashil son (o'ngacha, до-10). Tap son, keyin ramka.
const S10_ITEMS = [{ red: 6, miss: 4 }, { red: 8, miss: 2 }, { red: 7, miss: 3 }];
            ...
            {S10_ITEMS.map((it) => {
              ...
              return (
                <button key={it.red} className={`g1-m-frame ${done ? 'g1-m-frame-ok' : ''} ${nudge === it.red ? 'g1-nudge' : ''}`} disabled={done || allDone || !canAns} onClick={() => tapFrame(it)}>
                  <BondFrame total={10} a={it.red} hide={!done} anim={done}/>
                </button>
              );
            })}
          </div>
...
const DRILL = [
  { total: 6, a: 4, hide: 'blue', ans: 2, tiles: [3, 2, 4] },
  { total: 8, a: 5, hide: 'blue', ans: 3, tiles: [3, 2, 4] },
  { total: 7, a: 2, hide: 'red', ans: 2, tiles: [2, 3, 5] },
  { total: 9, a: 6, hide: 'blue', ans: 3, tiles: [2, 3, 4] },
  { total: 10, a: 7, hide: 'blue', ans: 3, tiles: [4, 3, 2] },
  { total: 8, a: 3, hide: 'red', ans: 3, tiles: [3, 5, 4] },
];
...
const ScreenD = (props) => {
  ...
  const [cells, setCells] = useState(() => Array.from({ length: SD_TOTAL }, () => false));
  ...
  const nextRound = () => { setRi(ri + 1); setCells(Array.from({ length: SD_TOTAL }, () => false)); setRoundDone(false); };
  const navContent = (
    <>
      <NavBack onPrev={props.onPrev} label={<BackLabel/>}/>
      <NavNext disabled={!done} onClick={props.onNext} label={<NextLabel/>}/>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          ...
          <div className="g1-bc-parts">
            ...
            <span className="g1-bc-and">{lang === 'uz' ? 'va' : 'и'}</span>
            <span className="g1-bc-part g1-bc-b">{SD_TOTAL - redCount}</span>
          </div>
        </div>
        {roundDone && (
          <div ref={revealRef} className="frame-success fade-up" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
...
// s11 — TEST final + FAKT: 10 = 7 va ? (7 qizil + berk yashil). Plitalar [7,10,4,3], to'g'ri idx3.
const Screen11 = (props) => {
  ...
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub">{t(c.title)}</h2>}
      figure={(solved) => <BondFrame total={10} a={7} hide={!solved} anim={solved}/>}
      options={[<NumTile d={7}/>, <NumTile d={10}/>, <NumTile d={4}/>, <NumTile d={3}/>]}
      correctIdx={3}
      mascot={false}
      factOnCorrect={(
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame-success fade-up delay-1" style={{ display: 'flex', alignItems: 'center', gap: 'clamp(12px, 3vw, 22px)', flexWrap: 'wrap' }}>
          ...
          <div className="g1-nb-sm"><NumberBond total={10} a={7}/></div>
        </div>
        ...
        <div className="frame fade-up delay-1" style={{ padding: 'clamp(8px, 1.8vw, 14px)', overflow: 'hidden' }}>
          <div className="g1-scene" style={{ minHeight: 'clamp(150px, 30vw, 230px)' }}>
            ...
            <div className="g1-cast-row">
              ...
              <div className="g1-cast g1-comp-frame in"><BondFrame total={10} a={7}/></div>
              <div className="g1-cast in">
...
/* Mobil (tor ekran): hikoya sahnasi (Ra'no · olma ramkasi · Anvar) bitta qatorga
   sig'sin — personaj + ramka kichrayadi, qator o'ralmaydi (aks holda personajlar
   chetga chiqib qirqilardi). 10 katakli ramka ham sig'adi. */
@media (max-width: 640px) {
  .g1-scene .g1-cast-row { flex-wrap: nowrap; gap: clamp(4px,2vw,10px); }
  .g1-scene .g1-cast-fig { height: clamp(84px,22vw,116px); }
  .g1-scene .g1-cast-name { font-size: clamp(11px,3vw,13px); }
  .g1-comp-frame .g1-bf { padding: clamp(3px,0.8vw,5px); gap: clamp(2px,0.6vw,3px); }
  .g1-comp-frame .g1-bf-cell { width: clamp(14px,3.4vw,20px); height: clamp(14px,3.4vw,20px); border-radius: 6px; }
}
...
@media (prefers-reduced-motion: reduce) {
  .d4-mount, .d4-mount-r, .d4-croc-anim, .d4-fruit-bob, .d4-tapfruit, .d4-fruit, .d4-tfruit-bob, .d4-tfruit, .d4-taphand { animation: none; }
}
...
.g1-comp-frame { align-self: flex-end; }

...
// ============================================================================
// ░░ 1-SINF · Dars23 — "Числовая последовательность: счёт по 5 и по 10" (num-1-23-v1) · Б8 · spec: ETALON_1SINF.md ░░
// Baza: Dars21 (BARQAROR — infra + AppleCrate/RabbitHop/cast/saxnalar string-in-string). Dars15 EMAS (uni boshqa chat o'zgartiradi).
// YADRO: skip-count — 10 lab, 5 lab, oldinga va orqaga. Qonuniyat: qadam bir xil.
// MEXANIKA: tap-skip (s1: 10 lab savatlar, s2: 5 lab guruhlar), orqaga stepper + RabbitHop (s5), SeqTrack (tushgan son MC).
// Zich 0-100 son o'qi ISHLATILMAYDI (1-sinf uchun yaroqsiz) — konkret AppleCrate + tile-track. Столбик/typing YO'Q.
// Syujet: Dars21-22 davomi — savatlar qatorda, butun hosilni tez sanaymiz (CastScene-bog').
// Misconception'lar: M1 bittalab sanash · M2 qadamni adashtirish · M3 orqaga yo'nalish · M4 o'nlik chegarasi.
//
...
const LESSON_META = {
  lessonId: 'num-1-23-v1',
  lessonTitle: { ru: 'Числовая последовательность: счёт по 5 и по 10', uz: "Sonli ketma-ketlik: beshlab va o'nlab" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: tez sanaymiz
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: qanday tezroq
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // 10 lab (savatlar)
  { id: 's2',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // 5 lab (guruhlar)
  { id: 's3',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // bir xil qadam
  { id: 's4',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // davom 10 lab: ?=40
  { id: 's5',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // orqaga 10 lab 100 dan
  { id: 's6',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // orqaga: kichrayadi
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // orqaga davom: ?=20
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 5 lab davom: ?=30
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: tushgan son (3 raund)
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik: tez sanaldi
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: tushgan son ?=80 + fakt
...
const CONTENT = {
  sIntro: {
    ...
    title: { ru: 'Считаем быстро', uz: 'Tez sanaymiz' },
    body: {
      ru: 'Корзины с яблоками стоят в ряд. Считать по одному долго. Научимся считать по десять и по пять — вперёд и назад.',
      uz: "Olmali savatlar qatorda turibdi. Bittalab sanash uzoq. O'nlab va beshlab — oldinga va orqaga sanashni o'rganamiz."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! Корзины стоят в длинный ряд.',
        'Считать каждое яблоко по одному очень долго.',
        'Есть способ быстрее: считать по десять и по пять.',
        'Научимся считать вперёд и назад. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! Savatlar uzun qator bo'lib turibdi.",
        "Har olmani bittalab sanash juda uzoq.",
        "Tezroq usul bor: o'nlab va beshlab sanash.",
        "Oldinga va orqaga sanashni o'rganamiz. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: 'Пять корзин в ряд. Как сосчитать их', uz: "Besh savat qatorda. Ularni qanday" },
    title_part2_em: { ru: 'быстрее', uz: 'tezroq' },
    ...
    opt1: { ru: 'По десять', uz: "O'nlab" },
    opt2: { ru: 'На глаз', uz: "Ko'z bilan" },
    audio: {
      intro: {
        ru: 'Пять корзин стоят в ряд. В каждой десять яблок. Как сосчитать их быстрее? Нажми, как думаешь.',
        uz: "Besh savat qatorda turibdi. Har birida o'nta olma. Ularni qanday tezroq sanaymiz? O'ylab, bosing."
      },
      on_correct: { ru: 'Хорошо. Будем считать по десять. Так получится быстро.', uz: "Yaxshi. O'nlab sanaymiz. Shunda tez bo'ladi." },
      on_wrong: { ru: 'Хорошо. Будем считать по десять. Так получится быстро.', uz: "Yaxshi. O'nlab sanaymiz. Shunda tez bo'ladi." }
    }
  },

  s1: {
    eyebrow: { ru: 'Считаем по десять', uz: "O'nlab sanaymiz" },
    instruction: { ru: 'Нажимай на корзины по очереди. В каждой десять', uz: "Savatlarni navbat bilan bosing. Har birida o'nta" },
    count_label: { ru: 'Сосчитано', uz: 'Sanaldi' },
    full_text: { ru: 'Пять корзин — пятьдесят яблок.', uz: "Besh savat — ellik olma." },
    full_audio: { ru: 'Готово. Пять корзин по десять — это пятьдесят. Считать по десять быстро.', uz: "Tayyor. Besh savat o'ntadan — bu ellik. O'nlab sanash tez." },
    audio: {
      ru: [
        'В каждой корзине десять яблок. Будем считать корзины по десять.',
        'Нажимай по очереди и слушай. Десять, двадцать, тридцать, сорок, пятьдесят.'
      ],
      uz: [
        "Har savatda o'nta olma. Savatlarni o'nlab sanaymiz.",
        "Navbat bilan bosing va tinglang. O'n, yigirma, o'ttiz, qirq, ellik."
      ]
    }
  },

  s2: {
    eyebrow: { ru: 'Считаем по пять', uz: 'Beshlab sanaymiz' },
    instruction: { ru: 'Нажимай на группы по очереди. В каждой пять', uz: "Guruhlarni navbat bilan bosing. Har birida beshta" },
    count_label: { ru: 'Сосчитано', uz: 'Sanaldi' },
    full_text: { ru: 'Четыре группы — двадцать яблок.', uz: "To'rt guruh — yigirma olma." },
    full_audio: { ru: 'Готово. Четыре группы по пять — это двадцать. Считать по пять тоже удобно.', uz: "Tayyor. To'rt guruh beshtadan — bu yigirma. Beshlab sanash ham qulay." },
    audio: {
      ru: [
        'Теперь яблоки в группах по пять. Будем считать по пять.',
        'Нажимай по очереди и слушай. Пять, десять, пятнадцать, двадцать.'
      ],
      uz: [
        "Endi olmalar beshtadan guruhda. Beshlab sanaymiz.",
        "Navbat bilan bosing va tinglang. Besh, o'n, o'n besh, yigirma."
      ]
    }
  },
  ...
  s3: {
    ...
    title_part1: { ru: 'Каждый раз прибавляем', uz: 'Har safar bir xil' },
    title_part2_em: { ru: 'одинаковый шаг', uz: 'qadam' },
    title_part3: { ru: '', uz: " qo'shamiz" },
    tip: {
      ru: 'При счёте по десять каждый раз прибавляем десять: 10, 20, 30. По пять прибавляем пять: 5, 10, 15. Шаг всегда одинаковый.',
      uz: "O'nlab sanaganda har safar o'n qo'shamiz: 10, 20, 30. Beshlab beshta qo'shamiz: 5, 10, 15. Qadam doim bir xil."
    },
    audio: {
      ru: 'Когда считаем по десять, каждый раз прибавляем десять. Когда по пять, прибавляем пять. Шаг всегда одинаковый, поэтому числа идут по порядку.',
      uz: "O'nlab sanaganda har safar o'n qo'shamiz. Beshlab sanaganda besh qo'shamiz. Qadam doim bir xil, shuning uchun sonlar tartib bilan boradi."
    }
  },

  s4: {
    eyebrow: { ru: 'Тренировка · 1 / 4', uz: 'Mashq · 1 / 4' },
    title: { ru: 'Какое число пропущено в ряду?', uz: 'Qatorda qaysi son tushib qolgan?' },
    opt0: { ru: '40', uz: '40' },
    opt1: { ru: '35', uz: '35' },
    opt2: { ru: '31', uz: '31' },
    opt3: { ru: '45', uz: '45' },
    ...
    wrong_default: {
      ...
      uz: "O'nlab sanang: o'ttizga o'n qo'shing."
    },
    audio: {
      intro: { ru: 'Посмотри на ряд чисел. Какое число пропущено? Выбери.', uz: "Sonlar qatoriga qarang. Qaysi son tushib qolgan? Tanlang." },
      on_correct: { ru: 'Верно. После тридцати по десять будет сорок.', uz: "To'g'ri. O'ttizdan keyin o'nlab qirq bo'ladi." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор и считай по десять.', uz: "Unchalik emas. Tushuntirishga qarab, o'nlab sanang." }
    }
  },

  s5: {
    eyebrow: { ru: 'Считаем назад', uz: 'Orqaga sanaymiz' },
    instruction: { ru: 'Считаем назад от ста по десять. Нажимай кнопку', uz: "Yuzdan orqaga o'nlab sanaymiz. Tugmani bosing" },
    count_label: { ru: 'Сейчас', uz: 'Hozir' },
    full_text: { ru: 'Сто, девяносто, восемьдесят, семьдесят, шестьдесят.', uz: "Yuz, to'qson, sakson, yetmish, oltmish." },
    full_audio: { ru: 'Назад числа становятся меньше. Сто, девяносто, восемьдесят, семьдесят, шестьдесят.', uz: "Orqaga sonlar kichrayadi. Yuz, to'qson, sakson, yetmish, oltmish." },
    audio: {
      ru: [
        'Теперь считаем назад. Начинаем со ста и убираем по десять.',
        'Нажимай и слушай. Сто, девяносто, восемьдесят, семьдесят, шестьдесят.'
      ],
      uz: [
        "Endi orqaga sanaymiz. Yuzdan boshlab o'ntadan kamaytiramiz.",
        "Bosing va tinglang. Yuz, to'qson, sakson, yetmish, oltmish."
      ]
    }
  },
  ...
  s6: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Назад число', uz: 'Orqaga son' },
    title_part2_em: { ru: 'становится меньше', uz: 'kichrayadi' },
    title_part3: { ru: '', uz: '' },
    tip: {
      ru: 'Назад шаг тоже десять, но каждый раз убираем десять, и число становится меньше: 100, 90, 80, 70.',
      uz: "Orqaga qadam ham o'n, lekin har safar o'n kamayadi, son kichrayadi: 100, 90, 80, 70."
    },
    audio: {
      ru: 'Когда считаем назад, шаг тоже десять. Но мы убираем десять, и число становится меньше. Сто, девяносто, восемьдесят.',
      uz: "Orqaga sanaganda qadam ham o'n. Lekin o'n kamaytiramiz, son kichrayadi. Yuz, to'qson, sakson."
    }
  },
  ...
  s7: {
    ...
    wrong_default: {
      ...
      uz: "Orqaga o'nlab sanang: o'ttizdan o'n kamaytiring."
    },
    audio: {
      intro: { ru: 'Ряд идёт назад. Какое число пропущено? Выбери.', uz: "Qator orqaga boradi. Qaysi son tushib qolgan? Tanlang." },
      on_correct: { ru: 'Верно. После тридцати назад по десять будет двадцать.', uz: "To'g'ri. O'ttizdan keyin orqaga o'nlab yigirma bo'ladi." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор и считай назад по десять.', uz: "Unchalik emas. Tushuntirishga qarab, orqaga o'nlab sanang." }
    }
  },
  ...
  s8: {
    ...
    wrong_default: {
      ...
      uz: "Beshlab sanang: yigirma beshga besh qo'shing."
    },
    audio: {
      intro: { ru: 'Числа идут по пять. Какое пропущено? Выбери.', uz: "Sonlar beshlab boradi. Qaysi biri tushib qolgan? Tanlang." },
      on_correct: { ru: 'Верно. После двадцати пяти по пять будет тридцать.', uz: "To'g'ri. Yigirma beshdan keyin beshlab o'ttiz bo'ladi." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор и считай по пять.', uz: "Unchalik emas. Tushuntirishga qarab, beshlab sanang." }
    }
  },

  sg: {
    eyebrow: { ru: 'Игра', uz: "O'yin" },
    instruction: { ru: 'Найди пропущенное число', uz: "Tushib qolgan sonni toping" },
    correct_text: { ru: 'Верно! Идём дальше.', uz: "To'g'ri! Davom etamiz." },
    done_text: { ru: 'Молодцы! Все ряды собраны.', uz: "Barakalla! Hamma qator yig'ildi." },
    retry_audio: { ru: 'Ничего страшного. Посмотри на шаг и сосчитай.', uz: "Hech qisi yo'q. Qadamga qarab sanang." },
    audio: {
      intro: { ru: 'Поиграем с рядами. Я показываю числа с пропуском, а ты находишь нужное. Начинаем.', uz: "Qatorlar bilan o'ynaymiz. Men tushib qolgan sonli qator ko'rsataman, siz kerakli sonni topasiz. Boshladik." }
    }
  },

  sGuest: {
    eyebrow: { ru: 'Всё сосчитано', uz: 'Hammasi sanaldi' },
    title: { ru: 'Быстро и без ошибок', uz: 'Tez va xatosiz' },
    body: {
      ru: 'Рано, Анвар и Зухра сосчитали весь урожай по десять. Получилось быстро и без ошибок.',
      uz: "Ra'no, Anvar va Zuhra butun hosilni o'nlab sanashdi. Tez va xatosiz bo'ldi."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, весь урожай сосчитан по десять.',
        'Так получилось намного быстрее, чем по одному.'
      ],
      uz: [
        "Tinglang, butun hosil o'nlab sanaldi.",
        "Bittalab sanagandan ko'ra ancha tez bo'ldi."
      ]
    }
  },
  ...
  s9: {
    ...
    wrong_default: {
      ...
      uz: "O'nlab sanang: yetmishga o'n qo'shing."
    },
    fact_badge: { ru: 'А знаешь? · Счёт', uz: 'Bilasizmi? · Sanoq' },
    fact_text: {
      ru: 'Считать по десять придумали давно. Так быстро считают деньги, шаги и большие кучки.',
      uz: "O'nlab sanashni ko'p yil oldin o'ylab topishgan. Shunday qilib pul, qadam va katta uyumlarni tez sanashadi."
    },
    fact_audio: {
      ru: 'Считать по десять очень удобно. Так быстро считают деньги, шаги и большие кучки.',
      uz: "O'nlab sanash juda qulay. Shunday qilib pul, qadam va katta uyumlarni tez sanashadi."
    },
    audio: {
      intro: { ru: 'Последнее задание. Какое число пропущено в ряду? Выбери.', uz: "Oxirgi topshiriq. Qatorda qaysi son tushib qolgan? Tanlang." },
      on_correct: { ru: 'Верно. После семидесяти по десять будет восемьдесят.', uz: "To'g'ri. Yetmishdan keyin o'nlab sakson bo'ladi." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор и считай по десять.', uz: "Unchalik emas. Tushuntirishga qarab, o'nlab sanang." }
    }
  },
  ...
  s10: {
    ...
    main_2_em: { ru: 'считать по пять и по десять', uz: "beshlab va o'nlab sanay olasiz" },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке научимся складывать и вычитать круглые десятки.',
      uz: "Keyingi darsda yumaloq o'nliklarni qo'shish va ayirishni o'rganamiz."
    },
    audio: {
      ru: [
        'Молодец! Ты считал по десять и по пять, вперёд и назад.',
        'Теперь это получается быстро. На следующем уроке будем складывать круглые десятки. До встречи!'
      ],
      uz: [
        "Barakalla! O'nlab va beshlab, oldinga va orqaga sanadingiz.",
        "Endi bu tez chiqadi. Keyingi darsda yumaloq o'nliklarni qo'shamiz. Ko'rishguncha!"
      ]
    }
  }
};
...
// ============================================================
// SCREEN-KOMPONENTLAR — Dars23 (14 ekran)
// ============================================================

// Skip-count son so'zlari (5 lab, 5..100). Per-tap pushOneOff uchun.
const SEQW = {
  ru: { 5:'пять',10:'десять',15:'пятнадцать',20:'двадцать',25:'двадцать пять',30:'тридцать',35:'тридцать пять',40:'сорок',45:'сорок пять',50:'пятьдесят',55:'пятьдесят пять',60:'шестьдесят',65:'шестьдесят пять',70:'семьдесят',75:'семьдесят пять',80:'восемьдесят',85:'восемьдесят пять',90:'девяносто',95:'девяносто пять',100:'сто' },
  uz: { 5:'besh',10:"o'n",15:"o'n besh",20:'yigirma',25:'yigirma besh',30:"o'ttiz",35:"o'ttiz besh",40:'qirq',45:'qirq besh',50:'ellik',55:'ellik besh',60:'oltmish',65:'oltmish besh',70:'yetmish',75:'yetmish besh',80:'sakson',85:'sakson besh',90:"to'qson",95:"to'qson besh",100:'yuz' }
};

// SeqTrack — sonlar ketma-ketligi plitkalari, bitta tushgan (?) joy bilan; yechilgach javob ko'rinadi.
const SeqTrack = ({ seq, missingIdx, answer, solved = false }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(6px, 2vw, 12px)', flexWrap: 'wrap' }}>
    {seq.map((v, i) => (
      <span key={i} className={`d4-numtile${i === missingIdx && solved ? ' g1-pop-in' : ''}`} style={i === missingIdx ? { color: solved ? T.accent : T.ink3 } : undefined} aria-hidden="true">
        {i === missingIdx ? (solved ? answer : '?') : v}
      </span>
    ))}
  </div>
);
...
// s0 — HOOK (soft): 5 savat qatori (=50), qanday tezroq, har javob OK.
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.6vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ padding: 'clamp(16px, 3vw, 24px)' }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 'clamp(8px, 2vw, 16px)', flexWrap: 'wrap' }}>
            {[0, 1, 2, 3, 4].map((i) => <AppleCrate key={i}/>)}
          </div>
        </div>
        {!picked && (
...
// SkipForwardExplore — navbat bilan bosib skip-count (s1: 10 lab, s2: 5 lab).
const SkipForwardExplore = ({ props, c, vals, renderItem }) => {
  const lang = useLang();
  const t = useT();
  const audio = useAudio(makeAutoSegments(c, lang));
  const N = vals.length;
  ...
  const cur = k > 0 ? vals[k - 1] : 0;
  const tap = (i) => {
    if (i !== k || full) return;
    const nv = vals[i];
    setK(i + 1);
    if (!audio.muted) { const e = getAudioEngine(); if (e) { e.pushOneOff(SEQW[lang][nv]); if (i + 1 === N) e.pushOneOff(c.full_audio[lang]); } }
  };
  const navContent = (
    <>
      <NavBack onPrev={props.onPrev} label={<BackLabel/>}/>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 'clamp(8px, 2vw, 14px)', flexWrap: 'wrap', alignItems: 'flex-end' }}>
            {vals.map((v, i) => (
              <button key={i} className={`g1-item ${i < k ? 'g1-item-on' : ''}`} disabled={i !== k || full} onClick={() => tap(i)}
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                {renderItem()}
                {i < k && <span className="mono" style={{ fontWeight: 800 }}>{v}</span>}
              </button>
            ))}
          </div>
...
// s1 — EXPLORATION: 10 lab (5 savat -> 10,20,30,40,50).
const Screen1 = (props) => (
  <SkipForwardExplore props={props} c={CONTENT.s1} vals={[10, 20, 30, 40, 50]} renderItem={() => <AppleCrate/>}/>
);

// s2 — EXPLORATION: 5 lab (4 guruh -> 5,10,15,20).
const Screen2 = (props) => (
  <SkipForwardExplore props={props} c={CONTENT.s2} vals={[5, 10, 15, 20]} renderItem={() => <span style={{ display: 'inline-flex', maxWidth: 90 }}><Pips n={5} kind="apple" wrap={true}/></span>}/>
);

// s3 — RULE: bir xil qadam.
const Screen3 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s3;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          <SeqTrack seq={[10, 20, 30, 40, 50]} missingIdx={-1}/>
          <SeqTrack seq={[5, 10, 15, 20, 25]} missingIdx={-1}/>
        </div>
...
// s4 — TEST MC: davom 10 lab 10,20,30,?,50 -> 40 (idx0).
const Screen4 = (props) => {
  const c = CONTENT.s4;
  const t = useT();
  return (
    <QuestionScreen
      ...
      figure={(solved) => <SeqTrack seq={[10, 20, 30, 40, 50]} missingIdx={3} answer={40} solved={solved}/>}
      options={[<DigitGlyph d={40} size="mid"/>, <DigitGlyph d={35} size="mid"/>, <DigitGlyph d={31} size="mid"/>, <DigitGlyph d={45} size="mid"/>]}
      correctIdx={0}
      mascot={false}
...
// s5 — EXPLORATION: orqaga 10 lab 100 dan (100,90,80,70,60) — stepper + RabbitHop.
const Screen5 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s5;
  const audio = useAudio(makeAutoSegments(c, lang));
  const canAct = useCanAnswer(audio);
  const VALS = [100, 90, 80, 70, 60];
  const [k, setK] = useState(1);
  const full = k === VALS.length;
  const revealRef = useRevealScroll(full);
  const cur = VALS[k - 1];
  const step = () => {
    if (full || !canAct) return;
    const nv = VALS[k];
    setK(k + 1);
    if (!audio.muted) { const e = getAudioEngine(); if (e) { e.pushOneOff(SEQW[lang][nv]); if (k + 1 === VALS.length) e.pushOneOff(c.full_audio[lang]); } }
  };
  const navContent = (
    <>
      <NavBack onPrev={props.onPrev} label={<BackLabel/>}/>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <p className="h-sub title fade-up">{t(c.instruction)}</p>
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          <div style={{ width: 'clamp(70px, 16vw, 110px)' }}><RabbitHop/></div>
          <span className="d4-numtile" style={{ color: T.accent }} aria-hidden="true">{cur}</span>
          <SeqTrack seq={VALS.slice(0, k)} missingIdx={-1}/>
          {!full && (
            <button className="btn" disabled={!canAct} onClick={step}
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 2vw, 14px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          <span className="mono" style={{ fontSize: 'clamp(18px, 3vw, 26px)', color: T.ink3 }}>←</span>
          <SeqTrack seq={[100, 90, 80, 70, 60]} missingIdx={-1}/>
        </div>
        <BitSays text={t(c.tip)}/>
      </div>
    </Stage>
...
// s7 — TEST MC: orqaga davom 50,40,30,?,10 -> 20 (idx0).
const Screen7 = (props) => {
  const c = CONTENT.s7;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <SeqTrack seq={[50, 40, 30, 20, 10]} missingIdx={3} answer={20} solved={solved}/>}
      options={[<DigitGlyph d={20} size="mid"/>, <DigitGlyph d={25} size="mid"/>, <DigitGlyph d={29} size="mid"/>, <DigitGlyph d={40} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// s8 — TEST MC: 5 lab davom 15,20,25,?,35 -> 30 (idx0).
const Screen8 = (props) => {
  const c = CONTENT.s8;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <SeqTrack seq={[15, 20, 25, 30, 35]} missingIdx={3} answer={30} solved={solved}/>}
      options={[<DigitGlyph d={30} size="mid"/>, <DigitGlyph d={26} size="mid"/>, <DigitGlyph d={35} size="mid"/>, <DigitGlyph d={40} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// sg — MINI-O'YIN (3 raund, ballsiz): tushgan sonni top.
const SEQ_GAME = [
  { seq: [20, 30, 40, 50], mi: 2, ans: 40, opts: [40, 35, 45] },
  { seq: [80, 70, 60, 50], mi: 2, ans: 60, opts: [60, 65, 55] },
  { seq: [10, 15, 20, 25], mi: 2, ans: 20, opts: [20, 21, 16] },
];
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <SeqTrack seq={round.seq} missingIdx={round.mi} answer={round.ans} solved={solvedItem}/>
          {!solvedItem && (
...
// s9 — TEST final + FactCard: tushgan son 60,70,?,90 -> 80 (idx0).
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <SeqTrack seq={[60, 70, 80, 90]} missingIdx={2} answer={80} solved={solved}/>}
      options={[<DigitGlyph d={80} size="mid"/>, <DigitGlyph d={75} size="mid"/>, <DigitGlyph d={71} size="mid"/>, <DigitGlyph d={85} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      factOnCorrect={(
...
// ============================================================
export default function NumberSequenceLesson({

...
// ============================================================================
// ░░ 1-SINF · Dars26 — "Двузначное + двузначное (без перехода)" (num-1-26-v1) · Б9 · spec: ETALON_1SINF.md ░░
// Baza: Dars21 (BARQAROR — infra + AppleViz/AppleCrate/cast/saxnalar string-in-string). Dars15 EMAS (boshqa chat o'zgartiradi).
// YADRO: razryad bo'yicha qo'shish — o'nlikni o'nlikка, birlikni birlikка (34+25: 3+2=5 o'nlik, 4+5=9 birlik → 59). O'tishsiz, столбиксиз.
// MEXANIKA: AddTwoFig (ikki AppleViz birlashadi + yozuv). CombineExplore (tap-combine).
// Syujet: Dars24-25 davomi — ikki olma-uyumini razryad bo'yicha birlashtiramiz (CastScene-bog').
// Misconception'lar: M1 hamma raqamni qo'shish (34+25=14) · M2 faqat bitta razryad (=50) · M3 razryadlarni almashtirish (=95).
//
...
const LESSON_META = {
  lessonId: 'num-1-26-v1',
  lessonTitle: { ru: 'Двузначное + двузначное (без перехода)', uz: "Ikki xonali + ikki xonali (o'tishsiz)" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: uyumlarni birlashtiramiz
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: 34 va 25
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // 34+25 razryad bo'yicha
  { id: 's2',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // o'nlikni o'nlikка, birlikni birlikка
  { id: 's3',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 34+25=59
  { id: 's4',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // 23+45 (ikkala razryad)
  { id: 's5',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // ikkala razryadni unutma
  { id: 's6',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 52+13=65
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 43+21=64
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 23+45=68
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: 34+25, 61+27, 45+32
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: 52+34=86 + fakt
...
const CONTENT = {
  sIntro: {
    ...
    title: { ru: 'Объединяем кучки', uz: 'Uyumlarni birlashtiramiz' },
    body: {
      ru: 'Две кучки яблок нужно объединить. Десятки сложим с десятками, а отдельные яблоки — с отдельными. Так считать удобно.',
      uz: "Ikki uyum olmani birlashtirish kerak. O'nliklarni o'nlik bilan, yakka olmalarni yakka bilan qo'shamiz. Shunday sanash qulay."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! Здесь две кучки яблок.',
        'Их нужно объединить в одну.',
        'Десятки сложим с десятками, а отдельные яблоки — с отдельными.',
        'Научимся складывать по разрядам. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! Bu yerda ikki uyum olma.",
        "Ularni bitta qilib birlashtirish kerak.",
        "O'nliklarni o'nlik bilan, yakka olmalarni yakka bilan qo'shamiz.",
        "Razryad bo'yicha qo'shishni o'rganamiz. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: '34 яблока и 25 яблок. Сколько их', uz: '34 olma va 25 olma. Birga' },
    title_part2_em: { ru: 'вместе', uz: 'nechta' },
    title_part3: { ru: '?', uz: '?' },
    ...
    audio: {
      intro: {
        ru: 'В одной кучке тридцать четыре яблока, в другой двадцать пять. Сколько вместе? Нажми, как думаешь.',
        uz: "Bir uyumda o'ttiz to'rt olma, ikkinchisida yigirma besh. Birga nechta? O'ylab, bosing."
      },
      on_correct: { ru: 'Хорошо. Сложим десятки и единицы отдельно.', uz: "Yaxshi. O'nlik va birlikni alohida qo'shamiz." },
      on_wrong: { ru: 'Хорошо. Сложим десятки и единицы отдельно.', uz: "Yaxshi. O'nlik va birlikni alohida qo'shamiz." }
    }
  },

  s1: {
    eyebrow: { ru: 'Складываем', uz: "Qo'shamiz" },
    instruction: { ru: 'Объединим 34 и 25 по разрядам', uz: "34 va 25 ni razryad bo'yicha birlashtiramiz" },
    full_text: { ru: 'Пять корзин и девять яблок — 59.', uz: "Besh savat va to'qqiz olma — 59." },
    full_audio: { ru: 'Три десятка и два — пять десятков. Четыре и пять — девять единиц. Получилось пятьдесят девять.', uz: "Uch o'nlik va ikki — besh o'nlik. To'rt va besh — to'qqiz birlik. Ellik to'qqiz bo'ldi." },
    audio: {
      ru: [
        'Слева тридцать четыре, справа двадцать пять.',
        'Сложим десятки. Три и два — пять корзин. Сложим единицы. Четыре и пять — девять. Пятьдесят девять.'
      ],
      uz: [
        "Chapda o'ttiz to'rt, o'ngda yigirma besh.",
        "O'nliklarni qo'shamiz. Uch va ikki — besh savat. Birliklarni qo'shamiz. To'rt va besh — to'qqiz. Ellik to'qqiz."
      ]
    }
  },
  ...
  s2: {
    ...
    title_part1: { ru: 'Десятки — к десяткам,', uz: 'Razryadni' },
    title_part2_em: { ru: 'единицы — к единицам', uz: "o'z juftiga" },
    title_part3: { ru: '', uz: " qo'shamiz" },
    tip: {
      ru: 'Десятки складываем с десятками, единицы — с единицами. 34 и 25: пять десятков и девять единиц, это 59.',
      uz: "O'nlikni o'nlik bilan, birlikni birlik bilan qo'shamiz. 34 va 25: besh o'nlik va to'qqiz birlik, bu 59."
    },
    audio: {
      ru: 'Десятки складываем с десятками, единицы с единицами. Каждый разряд со своим. Пять десятков и девять единиц — это пятьдесят девять.',
      uz: "O'nlikni o'nlik bilan, birlikni birlik bilan qo'shamiz. Har razryad o'z jufti bilan. Besh o'nlik va to'qqiz birlik — bu ellik to'qqiz."
    }
  },
  ...
  s3: {
    ...
    audio: {
      intro: { ru: 'Тридцать четыре и двадцать пять. Сколько получится? Выбери.', uz: "O'ttiz to'rt va yigirma besh. Nechta bo'ladi? Tanlang." },
  ...
  s4: {
    eyebrow: { ru: 'Складываем', uz: "Qo'shamiz" },
    instruction: { ru: 'Объединим 23 и 45 по разрядам', uz: "23 va 45 ni razryad bo'yicha birlashtiramiz" },
    full_text: { ru: 'Шесть корзин и восемь яблок — 68.', uz: "Olti savat va sakkiz olma — 68." },
    full_audio: { ru: 'Два десятка и четыре — шесть десятков. Три и пять — восемь единиц. Шестьдесят восемь.', uz: "Ikki o'nlik va to'rt — olti o'nlik. Uch va besh — sakkiz birlik. Oltmish sakkiz." },
    audio: {
      ru: [
        'Слева двадцать три, справа сорок пять.',
        'Сложим десятки. Два и четыре — шесть. Сложим единицы. Три и пять — восемь. Шестьдесят восемь.'
      ],
      uz: [
        "Chapda yigirma uch, o'ngda qirq besh.",
        "O'nliklarni qo'shamiz. Ikki va to'rt — olti. Birliklarni qo'shamiz. Uch va besh — sakkiz. Oltmish sakkiz."
      ]
    }
  },
  ...
  s5: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Считаем', uz: 'Ikkala' },
    title_part2_em: { ru: 'оба разряда', uz: 'razryadni' },
    title_part3: { ru: '', uz: ' ham sanaymiz' },
    tip: {
      ru: 'Нельзя забыть ни один разряд. Сначала десятки, потом единицы. Оба нужны, чтобы получить верное число.',
      uz: "Birorta razryadni unutib bo'lmaydi. Avval o'nlik, keyin birlik. To'g'ri son uchun ikkalasi ham kerak."
    },
    audio: {
      ru: 'Запомни. Складываем оба разряда. Сначала десятки, потом единицы. Нельзя забыть ни один.',
      uz: "Eslab qoling. Ikkala razryadni ham qo'shamiz. Avval o'nlik, keyin birlik. Birortasini unutib bo'lmaydi."
    }
  },
  ...
  s6: {
    ...
    audio: {
      intro: { ru: 'Пятьдесят два и тринадцать. Сколько получится? Выбери.', uz: "Ellik ikki va o'n uch. Nechta bo'ladi? Tanlang." },
  ...
  s8: {
    ...
    audio: {
      intro: { ru: 'Двадцать три и сорок пять. Сколько получится? Выбери.', uz: "Yigirma uch va qirq besh. Nechta bo'ladi? Tanlang." },
  ...
  sGuest: {
    eyebrow: { ru: 'Кучки объединены', uz: 'Uyumlar birlashdi' },
    title: { ru: 'Каждый разряд со своим', uz: "Har razryad o'z jufti bilan" },
    body: {
      ru: 'Рано, Анвар и Зухра объединили кучки по разрядам: десятки к десяткам, единицы к единицам. Всё сошлось.',
      uz: "Ra'no, Anvar va Zuhra uyumlarni razryad bo'yicha birlashtirdi: o'nlik o'nlikka, birlik birlikka. Hammasi to'g'ri keldi."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, мы складывали по разрядам.',
        'Десятки к десяткам, единицы к единицам. Всё сошлось верно.'
      ],
      uz: [
        "Tinglang, biz razryad bo'yicha qo'shdik.",
        "O'nlik o'nlikka, birlik birlikka. Hammasi to'g'ri keldi."
      ]
    }
  },
  ...
  s9: {
    ...
    fact_text: {
      ru: 'Складывая две цены или две корзины, считаешь десятки с десятками, единицы с единицами. Так быстро и без ошибок.',
      uz: "Ikki narx yoki ikki savatni qo'shganda o'nlikni o'nlikka, birlikni birlikka qo'shasiz. Shunday tez va xatosiz."
    },
    fact_audio: {
      ru: 'А знаешь, складывая две цены, считаешь десятки с десятками, единицы с единицами. Так быстро и без ошибок.',
      uz: "Bilasizmi, ikki narxni qo'shganda o'nlikni o'nlikka, birlikni birlikka qo'shasiz. Shunday tez va xatosiz."
    },
    audio: {
      intro: { ru: 'Последнее задание. Пятьдесят два и тридцать четыре. Сколько получится? Выбери.', uz: "Oxirgi topshiriq. Ellik ikki va o'ttiz to'rt. Nechta bo'ladi? Tanlang." },
  ...
  s10: {
    ...
    main_2_em: { ru: 'складывать два двузначных числа', uz: "ikkita ikki xonali sonni qo'sha olasiz" },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке будем вычитать двузначные числа.',
      uz: "Keyingi darsda ikki xonali sonlarni ayiramiz."
    },
    audio: {
      ru: [
        'Молодец! Ты складывал по разрядам: десятки с десятками, единицы с единицами.',
        'Получались верные числа. На следующем уроке будем вычитать. До встречи!'
      ],
      uz: [
        "Barakalla! Razryad bo'yicha qo'shdingiz: o'nlik o'nlik bilan, birlik birlik bilan.",
        "To'g'ri sonlar chiqdi. Keyingi darsda ayirishni o'rganamiz. Ko'rishguncha!"
      ]
    }
  }
};
...
// ============================================================
// SCREEN-KOMPONENTLAR — Dars26 (14 ekran)
// ============================================================

const D26_OP = { fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, color: '#5A5A60' };

// AddTwoFig — ikki ikki xonali son razryad bo'yicha: ikki AppleViz + yozuv; yechilgach birlashgan AppleViz + natija.
const AddTwoFig = ({ a, b, solved = false, result }) => {
  const at = Math.floor(a / 10), ao = a % 10, bt = Math.floor(b / 10), bo = b % 10;
  const rt = Math.floor(result / 10), ro = result % 10;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.6vw, 18px)' }}>
      {!solved ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 2.4vw, 16px)', flexWrap: 'wrap' }}>
          <AppleViz tens={at} ones={ao}/>
          <span className="mono" style={{ ...D26_OP, fontSize: 'clamp(20px, 3.6vw, 28px)' }}>+</span>
          <AppleViz tens={bt} ones={bo}/>
        </div>
      ) : (
        <AppleViz tens={rt} ones={ro} pop={true}/>
      )}
...
// s0 — HOOK (soft): 34 va 25, birga nechta? har javob OK.
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(8px, 2.4vw, 16px)', flexWrap: 'wrap', justifyContent: 'center' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(3px, 0.8vw, 6px)' }}>
              <div style={{ height: 'clamp(68px, 16vw, 92px)', display: 'flex', alignItems: 'flex-end' }}><RanoSVG mood="pointing"/></div>
              <BasketTop n={12} kind="apple"/>
            </div>
            <span className="mono" style={{ ...D26_OP, fontSize: 'clamp(20px, 3.6vw, 28px)' }}>+</span>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(3px, 0.8vw, 6px)' }}>
              <div style={{ height: 'clamp(68px, 16vw, 92px)', display: 'flex', alignItems: 'flex-end' }}><AnvarSVG pose="coming"/></div>
              <BasketTop n={11} kind="apple"/>
            </div>
          </div>
        </div>
        {!picked && (
...
// CombineExplore — ikki ikki xonali sonni razryad bo'yicha birlashtirish (s1, s4).
const CombineExplore = ({ props, c, a, b, result }) => {
  const lang = useLang();
  const t = useT();
  const audio = useAudio(makeAutoSegments(c, lang));
  const canAct = useCanAnswer(audio);
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 26px)' }}>
          <AddTwoFig a={a} b={b} solved={done} result={result}/>
          {!done && (
...
// s1 — EXPLORATION: 34+25 razryad bo'yicha.
const Screen1 = (props) => (
  <CombineExplore props={props} c={CONTENT.s1} a={34} b={25} result={59}/>
);

// s2 — RULE: o'nlikni o'nlikка, birlikni birlikка.
const Screen2 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s2;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <AddTwoFig a={34} b={25} solved={true} result={59}/>
        </div>
...
const Screen3 = (props) => {
  ...
  return (
    <QuestionScreen
      ...
      figure={(solved) => <AddTwoFig a={34} b={25} solved={solved} result={59}/>}
      options={[<DigitGlyph d={59} size="mid"/>, <DigitGlyph d={14} size="mid"/>, <DigitGlyph d={50} size="mid"/>, <DigitGlyph d={95} size="mid"/>]}
      correctIdx={0}
      mascot={false}
...
// s4 — EXPLORATION: 23+45 (ikkala razryad).
const Screen4 = (props) => (
  <CombineExplore props={props} c={CONTENT.s4} a={23} b={45} result={68}/>
);

// s5 — RULE: ikkala razryadni ham unutma.
const Screen5 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s5;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <AddTwoFig a={23} b={45} solved={true} result={68}/>
        </div>
        <BitSays text={t(c.tip)}/>
      </div>
    </Stage>
...
const Screen6 = (props) => {
  ...
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <AddTwoFig a={52} b={13} solved={solved} result={65}/>}
      options={[<DigitGlyph d={65} size="mid"/>, <DigitGlyph d={11} size="mid"/>, <DigitGlyph d={60} size="mid"/>, <DigitGlyph d={56} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// s7 — TEST MC: 43+21=64 (idx0).
// s7 — TEST (interaktiv): 43+21 razryad bo'yicha. Bola "O'nlik qo'sh" (2) va "Birlik qo'sh" (1) bilan
// o'nlikni o'nlikka, birlikni birlikka qo'shadi -> 64. "Tekshirish" bilan tasdiqlaydi (erta/chala -> xato yo'li).
const Screen7 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s7;
  ...
  const canAct = useCanAnswer(audio);
  const BASE_T = 4, BASE_O = 3, ADD_T = 2, ADD_O = 1; // 43 + 21
  const [aTens, setATens] = useState(props.storedAnswer?.solved ? ADD_T : 0);
  const [aOnes, setAOnes] = useState(props.storedAnswer?.solved ? ADD_O : 0);
  ...
  const tens = BASE_T + aTens;
  const ones = BASE_O + aOnes;
  const total = tens * 10 + ones;
  const addTen = () => { if (solved || aTens >= ADD_T || !canAct) return; setATens(aTens + 1); if (status === 'wrong') setStatus('none'); };
  const addOne = () => { if (solved || aOnes >= ADD_O || !canAct) return; setAOnes(aOnes + 1); if (status === 'wrong') setStatus('none'); };
  const check = () => {
    if (solved || !canAct) return;
    if (aTens === ADD_T && aOnes === ADD_O) {
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          ...
          <span className="mono" style={{ fontSize: 'clamp(14px, 1.9vw, 17px)', color: T.ink3 }}>{BASE_T * 10 + BASE_O} + {ADD_T * 10 + ADD_O}</span>
          ...
          {!solved && (
            <div style={{ display: 'flex', gap: 'clamp(8px, 1.8vw, 14px)', flexWrap: 'wrap', justifyContent: 'center' }}>
              <button className="btn-white-accent" disabled={!canAct || aTens >= ADD_T} onClick={addTen}
                style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(16px, 2.4vw, 24px)', fontSize: 'clamp(13px, 1.7vw, 15px)' }}>
                {t(c.btn_tens)}
              </button>
              <button className="btn-white-accent" disabled={!canAct || aOnes >= ADD_O} onClick={addOne}
                style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(16px, 2.4vw, 24px)', fontSize: 'clamp(13px, 1.7vw, 15px)' }}>
                {t(c.btn_ones)}
              </button>
...
const Screen8 = (props) => {
  ...
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <AddTwoFig a={23} b={45} solved={solved} result={68}/>}
      options={[<DigitGlyph d={68} size="mid"/>, <DigitGlyph d={14} size="mid"/>, <DigitGlyph d={60} size="mid"/>, <DigitGlyph d={86} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// sg — MINI-O'YIN (3 raund, ballsiz): 34+25, 61+27, 45+32.
const ADD2_GAME = [
  { a: 34, b: 25, ans: 59, opts: [59, 14, 50] },
  { a: 61, b: 27, ans: 88, opts: [88, 16, 80] },
  { a: 45, b: 32, ans: 77, opts: [77, 14, 70] },
];
const ScreenGame = (props) => {
  ...
  const total = ADD2_GAME.length;
  const [ri, setRi] = useState(0);
  const [solvedItem, setSolvedItem] = useState(false);
  const [wrong, setWrong] = useState(() => new Set());
  const [praiseWord, setPraiseWord] = useState('');
  const [encWord, setEncWord] = useState('');
  const round = ADD2_GAME[ri];
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <AddTwoFig key={ri} a={round.a} b={round.b} solved={solvedItem} result={round.ans}/>
          {!solvedItem && (
...
// s9 — TEST final + FactCard: 52+34=86 (idx0).
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <AddTwoFig a={52} b={34} solved={solved} result={86}/>}
      options={[<DigitGlyph d={86} size="mid"/>, <DigitGlyph d={14} size="mid"/>, <DigitGlyph d={80} size="mid"/>, <DigitGlyph d={68} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      factOnCorrect={(
...
// ============================================================
export default function AddTwoDigitLesson({

...
// ============================================================================
// ░░ 1-SINF · Dars30 — "Составные задачи: всего и осталось (с таблицей)" (num-1-30-v1) · Б10 · spec: ETALON_1SINF.md ░░
// Baza: Dars29 (masala-kit — infra + SolTile/OpTile/PQ/RemoveRow/cast string-in-string).
// YADRO: masalani jadvalga yozamiz — "edi / keldi / jami" (qo'shish) va "edi / ketdi / qoldi" (ayirish); noma'lum katakni topamiz; amalni MAZMUNDAN tanlash.
// MEXANIKA: TableFig (uch ustunli jadval — bitta katak '?'), SolTile (yechim yozuvi), OpTile (+/−), PQ (savol+masala matni).
// Matn ovozда to'liq o'qiladi (1-sinf), ekranда qisqa tayanch. Typing YO'Q, son kichik (10 ichida).
// Misconception'lar: M1 "keldi" uchun ayirish / "ketdi" uchun qo'shish (amalni chalkashtirish) · M2 jamini noto'g'ri ustunga · M3 javobda noto'g'ri son (off-by-one).
//
...
const LESSON_META = {
  lessonId: 'num-1-30-v1',
  lessonTitle: { ru: 'Составные задачи: всего и осталось (с таблицей)', uz: "Tarkibli masala: jadval bilan (jami va qoldi)" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: masalani jadvalga yozamiz
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: jadvaldagi '?' katak
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // jadval: edi/keldi/jami
  { id: 's2',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // jami = edi + keldi
  { id: 's3',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // jami katagini topish 4+3=7
  { id: 's4',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // jadval: edi/ketdi/qoldi
  { id: 's5',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // qoldi = edi − ketdi
  { id: 's6',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qoldi katagini topish 8−3=5
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qaysi amal (keldi -> +)
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // noma'lum 'edi' katagi 7−3=4
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: 3 jadval masala
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik -> Dars31
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: jadval jami 5+4=9 + fakt
...
const CONTENT = {
  lab: {
    edi: { ru: 'Было', uz: 'Edi' },
    keldi: { ru: 'Пришло', uz: 'Keldi' },
    jami: { ru: 'Всего', uz: 'Jami' },
    ketdi: { ru: 'Ушло', uz: 'Ketdi' },
    qoldi: { ru: 'Осталось', uz: 'Qoldi' }
  },

  sIntro: {
    eyebrow: { ru: 'История', uz: 'Hikoya' },
    title: { ru: 'Запишем задачу в таблицу', uz: 'Masalani jadvalga yozamiz' },
    body: {
      ru: 'В прошлый раз мы решали задачи на остаток. Сегодня будем записывать задачу в таблицу — так удобно и понятно.',
      uz: "O'tgan safar qoldiqqa masala yechdik. Bugun masalani jadvalga yozamiz — bunday qulay va tushunarli."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! В прошлый раз мы решали задачи на остаток.',
        'Сегодня запишем задачу в таблицу.',
        'В таблице видно, что было, что пришло и сколько всего.',
        'Одна клетка пустая — её мы и найдём. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! O'tgan safar qoldiqqa masala yechdik.",
        "Bugun masalani jadvalga yozamiz.",
        "Jadvalda nima edi, nima keldi va jami nechta ekani ko'rinadi.",
        "Bitta katak bo'sh — o'shani topamiz. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: 'В таблице одна клетка пустая. Что нам надо', uz: "Jadvalda bitta katak bo'sh. Bizga nima" },
    title_part2_em: { ru: 'найти', uz: 'topish' },
    title_part3: { ru: '?', uz: ' kerak?' },
    question: { ru: 'Как думаешь? Нажми ответ.', uz: "Qanday o'ylaysiz? Javobni bosing." },
    opt0: { ru: 'Сколько всего', uz: 'Jami nechta' },
    opt1: { ru: 'Кто старше', uz: 'Kim kattaroq' },
    opt2: { ru: 'Трудно сказать', uz: 'Aytish qiyin' },
    audio: {
      intro: {
        ru: 'Посмотри на таблицу. Было три, пришло два, а всего — пустая клетка. Что нам надо найти? Нажми, как думаешь.',
        uz: "Jadvalga qarang. Uch edi, ikki keldi, jami esa — bo'sh katak. Bizga nima topish kerak? O'ylab, bosing."
      },
      on_correct: { ru: 'Хорошо. Найдём, сколько всего.', uz: "Yaxshi. Jami nechta ekanini topamiz." },
      on_wrong: { ru: 'Хорошо. Найдём, сколько всего.', uz: "Yaxshi. Jami nechta ekanini topamiz." }
    }
  },

  s1: {
    eyebrow: { ru: 'Было и пришло', uz: 'Edi va keldi' },
    instruction: { ru: 'Было 3 яблока, пришло ещё 2. Сколько всего?', uz: "3 olma edi, yana 2 keldi. Jami nechta?" },
    reveal_label: { ru: 'Посчитать', uz: 'Hisoblash' },
    full_text: { ru: 'Пришло — сложили: 3 + 2 = 5.', uz: "Keldi — qo'shdik: 3 + 2 = 5." },
    full_audio: { ru: 'К трём прибавим два — пять. Всего пять.', uz: "Uchga ikkini qo'shamiz — besh. Jami besh." },
    audio: {
      ru: [
        'Послушай задачу. Было три яблока, пришло ещё два.',
        'Сколько яблок стало всего? Это надо найти.'
      ],
      uz: [
        "Masalani tinglang. Uch olma edi, yana ikki keldi.",
        "Jami nechta olma bo'ldi? Buni topish kerak."
      ]
    }
  },
  ...
  s2: {
    ...
    title_part1: { ru: 'Всего —', uz: 'Jami —' },
    title_part2_em: { ru: 'было и пришло вместе', uz: 'edi va keldi birga' },
    title_part3: { ru: '', uz: '' },
    tip: {
      ru: 'Когда что-то приходит или добавляется, мы складываем. Всего — это было и пришло вместе. Здесь три и два — это пять.',
      uz: "Bir narsa kelsa yoki qo'shilsa, biz qo'shamiz. Jami — bu edi va keldi birga. Bu yerda uch va ikki — bu besh."
    },
    audio: {
      ru: 'Запомни. Когда что-то приходит, мы складываем. Всего это было и пришло вместе. Три и два пять.',
      uz: "Eslab qoling. Bir narsa kelsa, biz qo'shamiz. Jami — bu edi va keldi birga. Uch va ikki besh."
    }
  },
  ...
  s3: {
    ...
    wrong_1: {
      ru: 'Яблоки пришли, их стало больше. Значит складываем, а не вычитаем. Четыре и три — семь.',
      uz: "Olmalar keldi, ular ko'paydi. Demak ayirmaymiz, qo'shamiz. To'rt va uch — yetti."
    },
    wrong_2: {
      ru: 'Четыре и три — это семь, не восемь. Всего семь.',
      uz: "To'rt va uch — bu yetti, sakkiz emas. Jami yetti."
    },
    wrong_default: {
  ...
  s4: {
    eyebrow: { ru: 'Было и ушло', uz: 'Edi va ketdi' },
    instruction: { ru: 'Было 6 яблок, 2 ушло. Сколько осталось?', uz: "6 olma edi, 2 tasi ketdi. Nechta qoldi?" },
    reveal_label: { ru: 'Посчитать', uz: 'Hisoblash' },
    full_text: { ru: 'Ушло — вычли: 6 − 2 = 4.', uz: "Ketdi — ayirdik: 6 − 2 = 4." },
    full_audio: { ru: 'Ушло — вычитаем. Из шести вычесть два — осталось четыре.', uz: "Oltidan ikkini ayiramiz — to'rt qoldi." },
    audio: {
      ru: [
        'Послушай задачу. Было шесть яблок, два ушло.',
        'Сколько яблок осталось? Это надо найти.'
      ],
      uz: [
        "Masalani tinglang. Olti olma edi, ikkitasi ketdi.",
        "Nechta olma qoldi? Buni topish kerak."
      ]
    }
  },
  ...
  s5: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Осталось —', uz: 'Qoldi —' },
    title_part2_em: { ru: 'из было убрать ушло', uz: 'edidan ketganni ayiramiz' },
    title_part3: { ru: '', uz: '' },
    tip: {
      ru: 'Когда что-то уходит или отдают, мы вычитаем. Осталось — это из было убрать ушло. Здесь из шести убрать два — четыре.',
      uz: "Bir narsa ketsa yoki berilsa, biz ayiramiz. Qoldi — bu edidan ketganni ayirish. Bu yerda oltidan ikki — to'rt."
    },
    audio: {
      ru: 'Запомни. Когда что-то уходит, мы вычитаем. Осталось это из было убрать ушло. Из шести убрать два четыре.',
      uz: "Eslab qoling. Bir narsa ketsa, biz ayiramiz. Qoldi — bu edidan ketganni ayirish. Oltidan ikki to'rt."
    }
  },
  ...
  s7: {
    ...
    problem: { ru: 'Было 5 яблок, пришло 3. Найти всего.', uz: "5 olma edi, 3 keldi. Jamini topamiz." },
    correct_text: { ru: 'Правильно. Пришло — значит плюс.', uz: "To'g'ri. Keldi — demak qo'shish." },
    wrong_1: {
      ru: 'Яблоки пришли, их стало больше. Значит складываем, ставим плюс.',
      uz: "Olmalar keldi, ular ko'paydi. Demak qo'shamiz, qo'shish belgisi."
    },
    wrong_default: {
      ru: 'Пришло — выбираем плюс.',
      uz: "Keldi — qo'shishni tanlaymiz."
    },
    audio: {
      intro: { ru: 'Было пять яблок, пришло три, надо найти всего. Какое действие подходит? Выбери.', uz: "Besh olma edi, uch keldi, jamini topish kerak. Qaysi amal mos? Tanlang." },
      on_correct: { ru: 'Верно. Пришло — складываем, ставим плюс.', uz: "To'g'ri. Keldi — qo'shamiz, qo'shish belgisi." },
      on_wrong: { ru: 'Не совсем. Подумай, что с яблоками.', uz: "Unchalik emas. Olmalar bilan nima bo'lganini o'ylang." }
    }
  },
  ...
  sg: {
    ...
    q_round1: { ru: 'Было 3, пришло 4.', uz: '3 edi, 4 keldi.' },
    q_round2: { ru: 'Было 8, ушло 3.', uz: '8 edi, 3 ketdi.' },
    q_round3: { ru: 'Было 5, пришло 2.', uz: '5 edi, 2 keldi.' },
    audio: {
      intro: { ru: 'Поиграем с таблицами. Я читаю задачу, а ты находишь пустую клетку. Начинаем.', uz: "Jadvallar bilan o'ynaymiz. Men masalani o'qiyman, siz bo'sh katakni topasiz. Boshladik." }
    }
  },

  sGuest: {
    eyebrow: { ru: 'Задача решена', uz: 'Masala yechildi' },
    title: { ru: 'Таблица помогла', uz: 'Jadval yordam berdi' },
    body: {
      ru: 'Рано, Анвар и Зухра записали задачи в таблицу и нашли пустую клетку. Пришло — складываем, ушло — вычитаем.',
      uz: "Ra'no, Anvar va Zuhra masalalarni jadvalga yozib, bo'sh katakni topdi. Keldi — qo'shamiz, ketdi — ayiramiz."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, мы записали задачи в таблицу.',
        'Пришло складываем, ушло вычитаем. Таблица помогает не запутаться.'
      ],
      uz: [
        "Tinglang, biz masalalarni jadvalga yozdik.",
        "Keldi — qo'shamiz, ketdi — ayiramiz. Jadval adashmaslikka yordam beradi."
      ]
    }
  },
  ...
  s9: {
    ...
    wrong_1: {
      ru: 'Яблоки пришли, складываем. Пять и четыре — девять.',
      uz: "Olmalar keldi, qo'shamiz. Besh va to'rt — to'qqiz."
    },
    wrong_2: {
      ru: 'Пять и четыре — это девять, не восемь. Всего девять.',
      uz: "Besh va to'rt — bu to'qqiz, sakkiz emas. Jami to'qqiz."
    },
    wrong_default: {
      ru: 'Сложи было и пришло.',
      uz: "Edi va keldini qo'shing."
    ...
    fact_text: {
      ru: 'Таблица показывает, что было, что изменилось и сколько стало. С ней решать легче.',
  ...
  s10: {
    ...
    main_2_em: { ru: 'решать задачи с таблицей', uz: "jadval bilan masala yecha olasiz" },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке будем решать задачи в два шага.',
      uz: 'Keyingi darsda ikki qadamli masala yechamiz.'
    },
    audio: {
      ru: [
        'Молодец! Ты записывал задачи в таблицу и находил пустую клетку.',
        'Пришло складываем, ушло вычитаем. На следующем уроке решим задачи в два шага. До встречи!'
      ],
      uz: [
        "Barakalla! Masalalarni jadvalga yozdingiz va bo'sh katakni topdingiz.",
        "Keldi — qo'shamiz, ketdi — ayiramiz. Keyingi darsda ikki qadamli masala yechamiz. Ko'rishguncha!"
      ]
    }
  }
};
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.6vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <TableFig cells={[{ label: t(L.edi), value: 3 }, { label: t(L.keldi), value: 2 }, { label: t(L.jami), unknown: true }]}/>
        </div>
        {!picked && (
...
// s1 — EXPLORATION: jadval edi/keldi/jami — jamini hisoblaymiz (qo'shish).
const Screen1 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s1;
  ...
  const go = () => {
    ...
    setDone(true);   // avval "3 + 2 = ?" (son gapi), keyin javob materiallashadi
    setTimeout(() => {
      setShowRes(true);
  ...
  const cells = [{ label: t(L.edi), value: 3 }, { label: t(L.keldi), value: 2 }, done ? { label: t(L.jami), value: 5 } : { label: t(L.jami), unknown: true }];
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 2vw, 14px)', flexWrap: 'wrap', padding: 'clamp(18px, 3.4vw, 28px)' }}>
          <TableFig pop cells={[{ label: t(L.edi), value: 3 }, { label: t(L.keldi), value: 2 }, { label: t(L.jami), value: 5 }]}/>
        </div>
...
// s3 — TEST MC: jami 4+3=7 (idx0). Variantlar — SolTile.
const Screen3 = (props) => {
  const c = CONTENT.s3;
  const t = useT();
  const L = CONTENT.lab;
...
// s4 — EXPLORATION: jadval edi/ketdi/qoldi — qoldini hisoblaymiz (ayirish).
const Screen4 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s4;
  ...
  const go = () => {
    if (done || !canAct) return;
    setDone(true);   // avval "6 − 2 = ?" (son gapi), keyin javob materiallashadi
    setTimeout(() => {
      setShowRes(true);
      if (!audio.muted) { const e = getAudioEngine(); if (e) e.pushOneOff(c.full_audio[lang]); }
    }, 950);
  ...
  const cells = [{ label: t(L.edi), value: 6 }, { label: t(L.ketdi), value: 2 }, done ? { label: t(L.qoldi), value: 4 } : { label: t(L.qoldi), unknown: true }];
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <p className="h-sub title fade-up">{t(c.instruction)}</p>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 2vw, 14px)', flexWrap: 'wrap', padding: 'clamp(18px, 3.4vw, 28px)' }}>
          <TableFig pop cells={[{ label: t(L.edi), value: 6 }, { label: t(L.ketdi), value: 2 }, { label: t(L.qoldi), value: 4 }]}/>
        </div>
        <BitSays text={t(c.tip)}/>
      </div>
    </Stage>
...
// s6 — TEST MC: qoldi 8−3=5 (idx0). Variantlar — SolTile (off-by distractorlar).
const Screen6 = (props) => {
  const c = CONTENT.s6;
  const t = useT();
  const L = CONTENT.lab;
...
// s7 — TEST MC: qaysi amal? (keldi -> +) idx0. Variantlar — OpTile.
const Screen7 = (props) => {
  const c = CONTENT.s7;
  const t = useT();
  const L = CONTENT.lab;
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<PQ title={t(c.title)} problem={t(c.problem)}/>}
      figure={(solved) => <TableFig solved={solved} op="add" cells={[{ label: t(L.edi), value: 5 }, { label: t(L.keldi), value: 3 }, { label: t(L.jami), unknown: true, answer: 8 }]}/>}
...
// s8 — TEST MC: noma'lum 'edi' 7−3=4 (idx0). Variantlar — DigitGlyph.
const Screen8 = (props) => {
  const c = CONTENT.s8;
  const t = useT();
  const L = CONTENT.lab;
...
// sg — MINI-O'YIN (3 raund, ballsiz): jadval masalalari (TableFig).
const PROB_GAME = [
  { a: 3, b: 4, op: 'add', ans: 7, opts: [7, 1, 6], qk: 'q_round1' },
  { a: 8, b: 3, op: 'sub', ans: 5, opts: [5, 6, 4], qk: 'q_round2' },
  { a: 5, b: 2, op: 'add', ans: 7, opts: [7, 3, 8], qk: 'q_round3' },
];
const ScreenGame = (props) => {
  ...
  const cells = [
    { label: t(L.edi), value: round.a },
    { label: t(round.op === 'add' ? L.keldi : L.ketdi), value: round.b },
    { label: t(round.op === 'add' ? L.jami : L.qoldi), unknown: true, answer: round.ans }
  ];
  const pick = (i) => {
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <TableFig key={ri} cells={cells} solved={solvedItem}/>
          {!solvedItem && (
...
// s9 — TEST final + FactCard: jami 5+4=9 (idx0).
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  const L = CONTENT.lab;

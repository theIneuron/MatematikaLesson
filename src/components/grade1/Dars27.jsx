...
// ============================================================================
// ░░ 1-SINF · Dars27 — "Вычитание двузначных и из 100" (num-1-27-v1) · Б9 · spec: ETALON_1SINF.md ░░
// Baza: Dars21 (BARQAROR — infra + AppleViz/AppleCrate/cast/saxnalar string-in-string). Dars15 EMAS (boshqa chat o'zgartiradi).
// YADRO: razryad bo'yicha ayirish — o'nlikdan o'nlik, birlikdan birlik (68−25: 6−2=4, 8−5=3 → 43) + 100−N (yuz=10 o'nlik). O'tishsiz, столбиксиз.
// MEXANIKA: SubFig (AppleViz kamayadi + yozuv). s8 — xato topish (Bitning chizib tashlangan javobi). RemoveExplore (tap-remove).
// Syujet: Dars24-26 davomi — uyumdan razryad bo'yicha olib ketamiz (CastScene-bog').
// Misconception'lar: M1 qo'shib yuborish (68−25=93) · M2 bitta razryadni ayirmaslik (48/63) · M3 100 ni noto'g'ri (=6/70).
//
...
const LESSON_META = {
  lessonId: 'num-1-27-v1',
  lessonTitle: { ru: 'Вычитание двузначных и из 100', uz: "Ikki xonali va yuzdan ayirish" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: razryad bo'yicha olamiz
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: 68 dan 25
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // 68-25 razryad bo'yicha
  { id: 's2',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // o'nlikdan o'nlik, birlikdan birlik
  { id: 's3',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 68-25=43
  { id: 's4',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // 100-30 (10 savat)
  { id: 's5',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // 100-N: yuz=10 o'nlik
  { id: 's6',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 100-40=60
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 79-34=45
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // xato topish: 86-53=33
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: 57-24, 100-20, 95-42
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: 95-42=53 + fakt
...
const CONTENT = {
  sIntro: {
    ...
    title: { ru: 'Убираем по разрядам', uz: "Razryad bo'yicha olamiz" },
    body: {
      ru: 'Из кучки яблок часть забирают. Убирать будем по разрядам: десятки от десятков, отдельные яблоки от отдельных.',
      uz: "Olma uyumidan bir qismi olinadi. Razryad bo'yicha olamiz: o'nlikdan o'nlik, yakka olmadan yakka."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! Из большой кучки заберут несколько яблок.',
        'Убирать будем не как попало, а по разрядам.',
        'Десятки от десятков, отдельные яблоки от отдельных.',
        'Научимся вычитать по разрядам. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! Katta uyumdan bir nechta olma olinadi.",
        "Tasodifiy emas, razryad bo'yicha olamiz.",
        "O'nlikdan o'nlik, yakka olmadan yakka olma.",
        "Razryad bo'yicha ayirishni o'rganamiz. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: 'Было 68, забрали 25. Сколько', uz: '68 edi, 25 tasi olindi. Nechta' },
    title_part2_em: { ru: 'осталось', uz: 'qoldi' },
    title_part3: { ru: '?', uz: '?' },
    ...
    audio: {
      intro: {
        ru: 'Было шестьдесят восемь яблок. Забрали двадцать пять. Сколько осталось? Нажми, как думаешь.',
        uz: "Oltmish sakkiz olma edi. Yigirma beshtasi olindi. Nechta qoldi? O'ylab, bosing."
      },
      on_correct: { ru: 'Хорошо. Вычтем десятки и единицы отдельно.', uz: "Yaxshi. O'nlik va birlikni alohida ayiramiz." },
      on_wrong: { ru: 'Хорошо. Вычтем десятки и единицы отдельно.', uz: "Yaxshi. O'nlik va birlikni alohida ayiramiz." }
    }
  },

  s1: {
    eyebrow: { ru: 'Вычитаем', uz: 'Ayiramiz' },
    instruction: { ru: 'Из 68 уберём 25 по разрядам', uz: "68 dan 25 ni razryad bo'yicha olamiz" },
    full_text: { ru: 'Четыре корзины и три яблока — 43.', uz: "To'rt savat va uch olma — 43." },
    full_audio: { ru: 'Шесть десятков без двух — четыре десятка. Восемь без пяти — три единицы. Осталось сорок три.', uz: "Olti o'nlikdan ikki — to'rt o'nlik. Sakkizdan besh — uch birlik. Qirq uch qoldi." },
    audio: {
      ru: [
        'Здесь шесть корзин и восемь яблок, это шестьдесят восемь.',
        'Уберём две корзины и пять яблок. Шесть без двух четыре, восемь без пяти три. Сорок три.'
      ],
      uz: [
        "Bu yerda olti savat va sakkiz olma, bu oltmish sakkiz.",
        "Ikki savat va besh olmani olamiz. Oltidan ikki to'rt, sakkizdan besh uch. Qirq uch."
      ]
    }
  },
  ...
  s2: {
    ...
    title_part1: { ru: 'Десятки от десятков,', uz: 'Razryaddan' },
    title_part2_em: { ru: 'единицы от единиц', uz: "o'z juftini" },
    title_part3: { ru: '', uz: ' ayiramiz' },
    tip: {
      ru: 'Десятки вычитаем из десятков, единицы из единиц. 68 без 25: четыре десятка и три единицы, это 43.',
      uz: "O'nlikdan o'nlik, birlikdan birlik ayiramiz. 68 dan 25: to'rt o'nlik va uch birlik, bu 43."
    },
    audio: {
      ru: 'Десятки вычитаем из десятков, единицы из единиц. Каждый разряд со своим. Четыре десятка и три единицы — это сорок три.',
      uz: "O'nlikdan o'nlik, birlikdan birlik ayiramiz. Har razryad o'z jufti bilan. To'rt o'nlik va uch birlik — bu qirq uch."
    }
  },

  s3: {
    eyebrow: { ru: 'Тренировка · 1 / 5', uz: 'Mashq · 1 / 5' },
    title: { ru: 'Сколько останется?', uz: 'Nechta qoladi?' },
    opt0: { ru: '43', uz: '43' },
    opt1: { ru: '93', uz: '93' },
    opt2: { ru: '48', uz: '48' },
    opt3: { ru: '63', uz: '63' },
    correct_text: { ru: 'Правильно. Четыре десятка и три единицы. Сорок три.', uz: "To'g'ri. To'rt o'nlik va uch birlik. Qirq uch." },
    wrong_1: {
      ru: 'Здесь вычитаем, а не складываем. Шесть без двух четыре, восемь без пяти три. Сорок три.',
      uz: "Bu yerda ayiramiz, qo'shmaymiz. Oltidan ikki to'rt, sakkizdan besh uch. Qirq uch."
    },
    wrong_2: {
      ru: 'Единицы тоже надо вычесть. Восемь без пяти — три. Сорок три.',
      uz: "Birlikni ham ayirish kerak. Sakkizdan besh — uch. Qirq uch."
    },
    wrong_3: {
      ru: 'Десятки тоже надо вычесть. Шесть без двух — четыре. Сорок три.',
      uz: "O'nlikni ham ayirish kerak. Oltidan ikki — to'rt. Qirq uch."
    },
    wrong_default: {
    ...
    audio: {
      intro: { ru: 'Из шестидесяти восьми убрать двадцать пять. Сколько останется? Выбери.', uz: "Oltmish sakkizdan yigirma besh olinsa. Nechta qoladi? Tanlang." },
  ...
  s4: {
    eyebrow: { ru: 'Вычитаем из ста', uz: 'Yuzdan ayiramiz' },
    instruction: { ru: 'Сто — это десять корзин. Уберём три', uz: "Yuz — bu o'n savat. Uchtasini olamiz" },
    full_text: { ru: 'Семь корзин — 70.', uz: "Yetti savat — 70." },
    full_audio: { ru: 'Сто это десять десятков. Десять без трёх — семь десятков. Осталось семьдесят.', uz: "Yuz — bu o'n o'nlik. O'ndan uch — yetti o'nlik. Yetmish qoldi." },
    audio: {
      ru: [
        'Сто — это десять полных корзин.',
        'Уберём три корзины. Десять без трёх — семь десятков. Семьдесят.'
      ],
      uz: [
        "Yuz — bu o'nta to'la savat.",
        "Uch savatni olamiz. O'ndan uch — yetti o'nlik. Yetmish."
      ]
    }
  },
  ...
  s5: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Сто — это', uz: 'Yuz — bu' },
    title_part2_em: { ru: 'десять десятков', uz: "o'n o'nlik" },
    title_part3: { ru: '', uz: '' },
    tip: {
    ...
    audio: {
      ...
      uz: "Yuz — bu o'n o'nlik. Yuzdan ayirish uchun o'nliklarni olamiz. O'ndan uch — yetti. Yetmish."
    }
  },

  s6: {
    eyebrow: { ru: 'Тренировка · 2 / 5', uz: 'Mashq · 2 / 5' },
    title: { ru: 'Сколько останется?', uz: 'Nechta qoladi?' },
    opt0: { ru: '60', uz: '60' },
    opt1: { ru: '6', uz: '6' },
    opt2: { ru: '140', uz: '140' },
    opt3: { ru: '70', uz: '70' },
    correct_text: { ru: 'Правильно. Десять десятков без четырёх — шесть. Шестьдесят.', uz: "To'g'ri. O'n o'nlikdan to'rt — olti. Oltmish." },
    wrong_1: {
      ru: 'Сто — это десять десятков. Десять без четырёх — шесть десятков, шестьдесят.',
      uz: "Yuz — bu o'n o'nlik. O'ndan to'rt — olti o'nlik, oltmish."
    },
    wrong_2: {
      ru: 'Здесь убираем, а не прибавляем. Десять десятков без четырёх — шесть, шестьдесят.',
      uz: "Bu yerda olamiz, qo'shmaymiz. O'n o'nlikdan to'rt — olti, oltmish."
    },
    wrong_3: {
      ru: 'Десять без четырёх — это шесть, не семь. Шестьдесят.',
      uz: "O'ndan to'rt — bu olti, yetti emas. Oltmish."
    },
    wrong_default: {
    ...
    audio: {
      intro: { ru: 'Из ста убрать сорок. Сколько останется? Выбери.', uz: "Yuzdan qirq olinsa. Nechta qoladi? Tanlang." },
      on_correct: { ru: 'Верно. Десять десятков без четырёх — шесть. Шестьдесят.', uz: "To'g'ri. O'n o'nlikdan to'rt — olti. Oltmish." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор. Сто это десять десятков.', uz: "Unchalik emas. Tushuntirishga qarang. Yuz — bu o'n o'nlik." }
    }
  },
  ...
  s8: {
    ...
    opt2: { ru: '36', uz: '36' },
    opt3: { ru: '83', uz: '83' },
    correct_text: { ru: 'Правильно. Восемь без пяти три, шесть без трёх три. Тридцать три.', uz: "To'g'ri. Sakkizdan besh uch, oltidan uch uch. O'ttiz uch." },
    wrong_1: {
      ru: 'Это ответ Бита. Он прибавил единицы вместо вычитания. Шесть без трёх — три. Тридцать три.',
      uz: "Bu Bitning javobi. U birlikni ayirish o'rniga qo'shdi. Oltidan uch — uch. O'ttiz uch."
    },
    wrong_2: {
      ru: 'Единицы тоже надо вычесть. Шесть без трёх — три. Тридцать три.',
      uz: "Birlikni ham ayirish kerak. Oltidan uch — uch. O'ttiz uch."
    },
    wrong_3: {
      ru: 'Десятки тоже надо вычесть. Восемь без пяти — три. Тридцать три.',
      uz: "O'nlikni ham ayirish kerak. Sakkizdan besh — uch. O'ttiz uch."
    },
    wrong_default: {
      ru: 'Вычти десятки из десятков, единицы из единиц.',
      uz: "O'nlikdan o'nlik, birlikdan birlik ayir."
    ...
    audio: {
      intro: { ru: 'Бит вычислял восемьдесят шесть без пятидесяти трёх и ошибся. Найди верный ответ. Выбери.', uz: "Bit sakson oltidan ellik uchni ayirib, xato qildi. To'g'ri javobni top. Tanlang." },
  ...
  sGuest: {
    eyebrow: { ru: 'Всё верно', uz: "Hammasi to'g'ri" },
    title: { ru: 'Вычли по разрядам', uz: "Razryad bo'yicha ayirdik" },
    body: {
      ru: 'Рано, Анвар и Зухра убирали яблоки по разрядам: десятки от десятков, единицы от единиц. Всё сошлось.',
      uz: "Ra'no, Anvar va Zuhra olmalarni razryad bo'yicha oldi: o'nlikdan o'nlik, birlikdan birlik. Hammasi to'g'ri keldi."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, мы вычитали по разрядам.',
        'Десятки от десятков, единицы от единиц. Всё получилось верно.'
      ],
      uz: [
        "Tinglang, biz razryad bo'yicha ayirdik.",
        "O'nlikdan o'nlik, birlikdan birlik. Hammasi to'g'ri chiqdi."
      ]
    }
  },
  ...
  s9: {
    ...
    opt2: { ru: '55', uz: '55' },
    opt3: { ru: '93', uz: '93' },
    correct_text: { ru: 'Правильно. Пять десятков и три единицы. Пятьдесят три.', uz: "To'g'ri. Besh o'nlik va uch birlik. Ellik uch." },
    wrong_1: {
      ru: 'Здесь вычитаем, а не складываем. Девять без четырёх пять, пять без двух три. Пятьдесят три.',
      uz: "Bu yerda ayiramiz, qo'shmaymiz. To'qqizdan to'rt besh, beshdan ikki uch. Ellik uch."
    },
    wrong_2: {
      ru: 'Единицы тоже надо вычесть. Пять без двух — три. Пятьдесят три.',
      uz: "Birlikni ham ayirish kerak. Beshdan ikki — uch. Ellik uch."
    },
    wrong_3: {
      ru: 'Десятки тоже надо вычесть. Девять без четырёх — пять. Пятьдесят три.',
      uz: "O'nlikni ham ayirish kerak. To'qqizdan to'rt — besh. Ellik uch."
    },
    wrong_default: {
      ru: 'Вычти десятки из десятков, единицы из единиц.',
      uz: "O'nlikdan o'nlik, birlikdan birlik ayir."
    ...
    fact_text: {
      ru: 'Дал сто сумов, купил подешевле — сдачу находишь вычитанием. Десятки с десятками, единицы с единицами.',
      uz: "Yuz so'm berib, arzonroq narsa olsangiz, qaytimni ayirib topasiz. O'nlik o'nlik bilan, birlik birlik bilan."
    },
    fact_audio: {
      ru: 'А знаешь, дал сто сумов, купил подешевле. Сдачу находишь вычитанием. Десятки с десятками, единицы с единицами.',
      uz: "Bilasizmi, yuz so'm berib narsa olsangiz, qaytimni ayirib topasiz. O'nlik o'nlik bilan, birlik birlik bilan."
    },
    audio: {
      intro: { ru: 'Последнее задание. Из девяноста пяти убрать сорок два. Сколько останется? Выбери.', uz: "Oxirgi topshiriq. To'qson beshdan qirq ikki olinsa. Nechta qoladi? Tanlang." },
  ...
  s10: {
    ...
    main_2_em: { ru: 'вычитать двузначные числа и из ста', uz: "ikki xonali sonlarni va yuzdan ayira olasiz" },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке научимся решать задачи по шагам.',
      uz: "Keyingi darsda masalalarni bosqichma-bosqich yechishni o'rganamiz."
    },
    audio: {
      ru: [
        'Молодец! Ты вычитал по разрядам и из ста.',
        'Получались верные числа. На следующем уроке начнём решать задачи. До встречи!'
      ],
      uz: [
        "Barakalla! Razryad bo'yicha va yuzdan ayirdingiz.",
        "To'g'ri sonlar chiqdi. Keyingi darsda masala yechishni boshlaymiz. Ko'rishguncha!"
      ]
    }
  }
};
...
// ============================================================
// SCREEN-KOMPONENTLAR — Dars27 (14 ekran)
// ============================================================

const D27_OP = { fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, color: '#5A5A60' };

// SubFig — ayirish: kamayuvchi AppleViz (yechilgach natija) + yozuv a − b = ? . bitAnswer -> Bitning xato javobi (chizib tashlangan).
const SubFig = ({ a, b, solved = false, result, bitAnswer = null }) => {
  const av = solved ? result : a;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.6vw, 18px)' }}>
      <AppleViz tens={Math.floor(av / 10)} ones={av % 10} pop={solved}/>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(6px, 1.6vw, 10px)' }}>
        <DigitGlyph d={a} size="mid"/>
        <span className="mono" style={{ ...D27_OP, fontSize: 'clamp(20px, 3.6vw, 28px)' }}>−</span>
        <DigitGlyph d={b} size="mid"/>
        <span className="mono" style={{ ...D27_OP, fontSize: 'clamp(20px, 3.6vw, 28px)' }}>=</span>
        {bitAnswer != null && !solved
          ? <span className="mono" style={{ fontSize: 'clamp(20px, 3.6vw, 28px)', fontWeight: 800, color: T.ink3, textDecoration: 'line-through' }}>{bitAnswer}</span>
...
// s0 — HOOK (soft): 68 dan 25, nechta qoldi? har javob OK.
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.6vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 2vw, 14px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(3px, 0.8vw, 6px)' }}>
            <div style={{ height: 'clamp(70px, 17vw, 96px)', display: 'flex', alignItems: 'flex-end' }}><RanoSVG mood="pointing"/></div>
            <BasketTop n={14} kind="apple"/>
          </div>
          <span className="mono" style={{ ...D27_OP, fontSize: 'clamp(20px, 3.6vw, 28px)' }}>−</span>
          <DigitGlyph d={25} size="mid"/>
        </div>
        {!picked && (
...
// RemoveExplore — razryad bo'yicha ayirish (s1: 68-25, s4: 100-30).
const RemoveExplore = ({ props, c, a, b, result }) => {
  const lang = useLang();
  const t = useT();
  const audio = useAudio(makeAutoSegments(c, lang));
  const canAct = useCanAnswer(audio);
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 26px)' }}>
          <SubFig a={a} b={b} solved={done} result={result}/>
          {!done && (
...
// s1 — EXPLORATION: 68-25 razryad bo'yicha.
const Screen1 = (props) => (
  <RemoveExplore props={props} c={CONTENT.s1} a={68} b={25} result={43}/>
);

// s2 — RULE: o'nlikdan o'nlik, birlikdan birlik.
const Screen2 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s2;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <SubFig a={68} b={25} solved={true} result={43}/>
        </div>
...
const Screen3 = (props) => {
  ...
  return (
    <QuestionScreen
      ...
      figure={(solved) => <SubFig a={68} b={25} solved={solved} result={43}/>}
      options={[<DigitGlyph d={43} size="mid"/>, <DigitGlyph d={93} size="mid"/>, <DigitGlyph d={48} size="mid"/>, <DigitGlyph d={63} size="mid"/>]}
      correctIdx={0}
      mascot={false}
...
// s4 — EXPLORATION: 100-30 (sto = 10 savat).
const Screen4 = (props) => (
  <RemoveExplore props={props} c={CONTENT.s4} a={100} b={30} result={70}/>
);

// s5 — RULE: 100-N (yuz = 10 o'nlik).
const Screen5 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s5;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <SubFig a={100} b={30} solved={true} result={70}/>
        </div>
        <BitSays text={t(c.tip)}/>
      </div>
    </Stage>
...
const Screen6 = (props) => {
  ...
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <SubFig a={100} b={40} solved={solved} result={60}/>}
      options={[<DigitGlyph d={60} size="mid"/>, <DigitGlyph d={6} size="mid"/>, <DigitGlyph d={140} size="mid"/>, <DigitGlyph d={70} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// s7 — TEST (interaktiv): 79−34 razryad bo'yicha ayirish. Bola "O'nlik ol" (3) va "Birlik ol" (4) bilan
// o'nlikdan o'nlik, birlikdan birlik ayiradi -> 45. "Tekshirish" bilan tasdiqlaydi (erta/chala -> xato yo'li).
const Screen7 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s7;
  ...
  const canAct = useCanAnswer(audio);
  const BASE_T = 7, BASE_O = 9, SUB_T = 3, SUB_O = 4; // 79 − 34
  const [rTens, setRTens] = useState(props.storedAnswer?.solved ? SUB_T : 0);
  const [rOnes, setROnes] = useState(props.storedAnswer?.solved ? SUB_O : 0);
  ...
  const tens = BASE_T - rTens;
  const ones = BASE_O - rOnes;
  const total = tens * 10 + ones;
  const remTen = () => { if (solved || rTens >= SUB_T || !canAct) return; setRTens(rTens + 1); if (status === 'wrong') setStatus('none'); };
  const remOne = () => { if (solved || rOnes >= SUB_O || !canAct) return; setROnes(rOnes + 1); if (status === 'wrong') setStatus('none'); };
  const check = () => {
    if (solved || !canAct) return;
    if (rTens === SUB_T && rOnes === SUB_O) {
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <h2 className="title h-sub fade-up" style={{ textAlign: 'center' }}>{t(c.title)}</h2>
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          ...
          <span className="mono" style={{ fontSize: 'clamp(14px, 1.9vw, 17px)', color: T.ink3 }}>{BASE_T * 10 + BASE_O} − {SUB_T * 10 + SUB_O}</span>
          ...
          {!solved && (
            <div style={{ display: 'flex', gap: 'clamp(8px, 1.8vw, 14px)', flexWrap: 'wrap', justifyContent: 'center' }}>
              <button className="btn-white-accent" disabled={!canAct || rTens >= SUB_T} onClick={remTen}
                style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(16px, 2.4vw, 24px)', fontSize: 'clamp(13px, 1.7vw, 15px)' }}>
                {t(c.btn_tens)}
              </button>
              <button className="btn-white-accent" disabled={!canAct || rOnes >= SUB_O} onClick={remOne}
                style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(16px, 2.4vw, 24px)', fontSize: 'clamp(13px, 1.7vw, 15px)' }}>
                {t(c.btn_ones)}
              </button>
              <button className="btn" disabled={!canAct} onClick={check}
                style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(18px, 2.8vw, 28px)', fontSize: 'clamp(13px, 1.7vw, 15px)' }}>
                {t(c.btn_check)}
              </button>
            </div>
          )}
...
// s8 — TEST MC (xato topish): Bit 86-53=39 (xato), to'g'risi 33 (idx0).
const Screen8 = (props) => {
  const c = CONTENT.s8;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <SubFig a={86} b={53} solved={solved} result={33} bitAnswer={t(c.bit_answer)}/>}
      options={[<DigitGlyph d={33} size="mid"/>, <DigitGlyph d={39} size="mid"/>, <DigitGlyph d={36} size="mid"/>, <DigitGlyph d={83} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// sg — MINI-O'YIN (3 raund, ballsiz): 57-24, 100-20, 95-42.
const SUB_GAME = [
  { a: 57, b: 24, ans: 33, opts: [33, 81, 37] },
  { a: 100, b: 20, ans: 80, opts: [80, 8, 120] },
  { a: 95, b: 42, ans: 53, opts: [53, 137, 93] },
];
const ScreenGame = (props) => {
  ...
  const total = SUB_GAME.length;
  const [ri, setRi] = useState(0);
  const [solvedItem, setSolvedItem] = useState(false);
  const [wrong, setWrong] = useState(() => new Set());
  const [praiseWord, setPraiseWord] = useState('');
  const [encWord, setEncWord] = useState('');
  const round = SUB_GAME[ri];
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <SubFig key={ri} a={round.a} b={round.b} solved={solvedItem} result={round.ans}/>
          {!solvedItem && (
...
// s9 — TEST final + FactCard: 95-42=53 (idx0).
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <SubFig a={95} b={42} solved={solved} result={53}/>}
      options={[<DigitGlyph d={53} size="mid"/>, <DigitGlyph d={137} size="mid"/>, <DigitGlyph d={55} size="mid"/>, <DigitGlyph d={93} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      factOnCorrect={(
...
// ============================================================
export default function SubtractTwoDigitLesson({

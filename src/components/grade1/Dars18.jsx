...
const CONTENT = {
  ...
  // ---- s1 EXPLORATION (DRAG make-ten): 8+5 — bola BESHTADAN IKKITASINI o'zi yashikka suradi
  ...
  s1: {
    ...
    instruction: { ru: 'Восемь яблок и ещё пять. Перетащи два в пустые места — до десяти', uz: "Sakkizta olma va yana beshta. Ikkitasini bo'sh joylarga suring — o'ngacha" },
    ...
    audio: {
      ru: [
        'У нас восемь яблок, и нужно добавить пять. Сначала перетащи два яблока в пустые места, чтобы стало десять.',
        'Ты дополнил до десяти! Остальные три перешли в новый ряд. Десять и три, это тринадцать.'
      ],
      uz: [
        "Bizda sakkizta olma, beshta qo'shish kerak. Avval ikkita olmani bo'sh joylarga suring, o'n bo'lsin.",
  ...
  // ---- sGuest SYUJET KO'PRIK: o'tib qo'shish puxtalandi -> Dars19 (o'tib ayirish) ----
  sGuest: {
    eyebrow: { ru: 'История', uz: 'Hikoya' },
    title: { ru: 'Скоро — вычитание через десяток', uz: "Tez orada — o'nlikdan o'tib ayirish" },
    body: {
      ru: 'Сегодня мы потренировались складывать через десяток. В следующий раз будем вычитать через десяток: сначала спустимся до круглого десятка, а потом уберём остальное.',
      uz: "Bugun o'nlikdan o'tib qo'shishni mashq qildik. Keyingi safar o'nlikdan o'tib ayirishni o'rganamiz: avval to'liq o'nlikkacha tushamiz, keyin qolganini ayiramiz."
    },
    bit_label: { ru: 'Бит', uz: 'Bit' },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    ...
    audio: {
      ru: [
        'Сегодня мы потренировались складывать через десяток.',
        'В следующий раз будем вычитать через десяток сами.',
        'Молодец! Слушай и нажимай кнопку дальше.'
      ],
      uz: [
        "Bugun o'nlikdan o'tib qo'shishni mashq qildik.",
        "Keyingi safar o'nlikdan o'tib ayirishni o'rganamiz.",
...
// s1 — EXPLORATION (DRAG make-ten): yashikda 8 olma + trayда 5; bola IKKITASINI o'zi
// bo'sh joylarga suradi (to'qqiz, o'n!) — strategiyaning kalit harakati. Ikkinchisidan
// keyin qolgan 3 avtomatik yangi qatorga o'tadi (MakeTenFrames done). Dars13 naqshi.
...
const Screen1 = (props) => {
  ...
  const handleDrop = useCallback((tokenId, zoneId) => {
    ...
    setPlaced((prev) => {
      if (prev.has(tokenId) || prev.size >= S1_NEED) return prev;   // yashik to'la — ortiqcha qabul qilinmaydi
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.8vw, 22px)', padding: 'clamp(18px, 3.4vw, 28px)' }}>
          ...
          {done
            ...
            : (
              <>
                {/* Yashik — drop-zona: 8 to'la, 2 joy bo'sh */}
                ...
                {/* Tray — qo'shiladigan 5 olma (faqat 2 tasi yashikka sig'adi) */}
...
const Screen7 = (props) => {
  ...
  const canAct = useCanAnswer(audio);
  const A = 8, B = 7;
  const [added, setAdded] = useState(props.storedAnswer?.solved ? B : 0);
  ...
  const addOne = () => {
    if (solved || added >= B || !canAct) return;
    setAdded(added + 1);
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          ...
          <span className="mono" style={{ fontSize: 'clamp(14px, 1.9vw, 17px)', color: T.ink3 }}>{A} + {B}</span>
          ...
          {!solved && (
            <div style={{ display: 'flex', gap: 'clamp(8px, 1.8vw, 14px)', flexWrap: 'wrap', justifyContent: 'center' }}>
              <button className="btn-white-accent" disabled={!canAct || added >= B} onClick={addOne}

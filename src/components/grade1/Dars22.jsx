...
// ============================================================================
// ░░ 1-SINF · Dars22 — "Чтение, запись и сравнение двузначных чисел" (num-1-22-v1) · Б8 · spec: ETALON_1SINF.md ░░
// Baza: Dars15 (infra + AppleViz + CompareSign + cast + saxnalar string-in-string ko'chirildi).
// YADRO: ikki xonali sonlarni solishtirish — avval o'nliklar, teng bo'lsa birliklar. > < = (CompareSign, Dars04 timsohi).
// MEXANIKA: PairViz (ikki AppleViz + timsoh), rasm->son MC (qaysi katta), belgi-tanlash (s7/sg). Typing/столбик YO'Q.
// Syujet: Dars21 davomi — savatlar sanaldi, kim ko'proq terdi? Razryad bilan adolat (CastScene-bog').
// Misconception'lar: M1 birlikka qarab solishtirish · M2 teng o'nlik=teng son · M3 belgi yo'nalishi · M4 pozitsiya almashtirish.
//
...
const LESSON_META = {
  lessonId: 'num-1-22-v1',
  lessonTitle: { ru: 'Чтение, запись и сравнение двузначных чисел', uz: "Ikki xonali sonlarni o'qish, yozish va solishtirish" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: kim ko'proq terdi
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: ko'z bilan kimda ko'p
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // razryad kartalari: o'qi/yoz 47
  { id: 's2',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // solishtirish o'nlik bo'yicha
  { id: 's3',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // avval o'nliklarni solishtir
  { id: 's4',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qaysi katta 38/53
  { id: 's5',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // teng o'nlik: birlikka qara
  { id: 's6',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // teng o'nlik -> birlik; > < =
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // belgi qo'y 62 _ 60
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qaysi katta 47/51
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: belgi (55=55, 73, 40<80)
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik: adolat bilan
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: qaysi katta 46/64 + fakt
...
const CONTENT = {
  sIntro: {
    ...
    title: { ru: 'Кто собрал больше?', uz: "Kim ko'proq terdi?" },
    body: {
      ru: 'Яблоки разложены по корзинам и сосчитаны. У Рано одно число, у Анвара другое. Кто собрал больше? Сравним честно.',
      uz: "Olmalar savatlarga joylandi va sanaldi. Ra'noda bir son, Anvarda boshqa son. Kim ko'proq terdi? Adolat bilan solishtiramiz."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! Урожай собран и сосчитан.',
        'У Рано одно число яблок, у Анвара другое.',
        'Кто же собрал больше? На глаз не угадать.',
        'Я научу тебя сравнивать числа точно. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! Hosil yig'ildi va sanaldi.",
        "Ra'noda bir son olma, Anvarda boshqa son.",
        "Kim ko'proq terdi? Ko'z bilan topib bo'lmaydi.",
        "Men sizga sonlarni aniq solishtirishni o'rgataman. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: 'Рано и Анвар собрали яблоки. Кто собрал', uz: "Ra'no va Anvar olma terdi. Kim" },
    title_part2_em: { ru: 'больше', uz: "ko'proq" },
    title_part3: { ru: '?', uz: ' terdi?' },
    question: { ru: 'Как думаешь? Нажми ответ.', uz: "Qanday o'ylaysiz? Javobni bosing." },
    opt0: { ru: 'Рано', uz: "Ra'no" },
    opt1: { ru: 'Анвар', uz: 'Anvar' },
    opt2: { ru: 'Трудно сказать', uz: 'Aytish qiyin' },
    audio: {
      intro: {
        ru: 'Рано и Анвар собрали по корзине яблок. Кто собрал больше? Посмотри и нажми, как думаешь.',
        uz: "Ra'no va Anvar bittadan savat olma terdi. Kim ko'proq terdi? Qarang va o'ylab bosing."
      },
      on_correct: { ru: 'Хорошо. На глаз можно ошибиться. Посчитаем десятки и узнаем точно.', uz: "Yaxshi. Ko'z bilan adashish mumkin. O'nliklarni sanab, aniq bilamiz." },
      on_wrong: { ru: 'Хорошо. На глаз можно ошибиться. Посчитаем десятки и узнаем точно.', uz: "Yaxshi. Ko'z bilan adashish mumkin. O'nliklarni sanab, aniq bilamiz." }
    }
  },

  s1: {
    eyebrow: { ru: 'Читаем и записываем', uz: "O'qiymiz va yozamiz" },
    instruction: { ru: 'Четыре десятка и семь единиц. Прочитаем и запишем число', uz: "To'rt o'nlik va yetti birlik. Sonni o'qib, yozamiz" },
    ...
    audio: {
      ru: [
        'Посмотри на корзины и яблоки. Сколько десятков, такая первая цифра.',
        'Четыре десятка — цифра четыре. Семь единиц — цифра семь. Читаем сорок семь.'
      ],
      uz: [
        "Savat va olmalarga qarang. Nechta o'nlik bo'lsa, birinchi raqam shu.",
        "To'rt o'nlik — to'rt raqami. Yetti birlik — yetti raqami. Qirq yetti deb o'qiymiz."
      ]
    }
  },

  s2: {
    eyebrow: { ru: 'Сравниваем', uz: 'Solishtiramiz' },
    instruction: { ru: 'У кого больше? Сначала сосчитаем десятки', uz: "Kimda ko'p? Avval o'nliklarni sanaymiz" },
    full_text: { ru: 'Пять десятков больше трёх. 52 больше 34.', uz: "Besh o'nlik uchtadan ko'p. 52 — 34 dan katta." },
    full_audio: { ru: 'У пятидесяти двух пять десятков, у тридцати четырёх три. Пять больше трёх. Пятьдесят два больше.', uz: "Ellik ikkida besh o'nlik, o'ttiz to'rtda uch. Besh uchdan ko'p. Ellik ikki katta." },
    audio: {
      ru: [
        'Сравним две кучки. Сначала смотрим только десятки.',
        'У одной три десятка, у другой пять. Пять больше. Значит число с пятью десятками больше.'
      ],
      uz: [
        "Ikki uyumni solishtiramiz. Avval faqat o'nliklarga qaraymiz.",
        "Birida uch o'nlik, ikkinchisida besh. Besh ko'proq. Demak besh o'nlikli son katta."
      ]
    }
  },
  ...
  s3: {
    ...
    title_part1: { ru: 'Сначала сравниваем', uz: 'Avval' },
    title_part2_em: { ru: 'десятки', uz: "o'nliklarni" },
    title_part3: { ru: '', uz: ' solishtiramiz' },
    tip: {
      ru: 'У какого числа десятков больше, то число больше. Рот крокодила открывается к большему числу.',
      uz: "Qaysi sonda o'nlik ko'p bo'lsa, o'sha son katta. Timsohning og'zi kattaroq songa ochiladi."
    },
    audio: {
      ru: 'Чтобы сравнить два числа, сначала смотрим десятки. У какого числа десятков больше, то и больше. Рот крокодила всегда открыт к большему числу.',
      uz: "Ikki sonni solishtirish uchun avval o'nliklarga qaraymiz. Qaysi sonda o'nlik ko'p bo'lsa, o'sha katta. Timsohning og'zi doim kattaroq songa ochiq."
    }
  },
  ...
  s4: {
    ...
    opt0: { ru: '38', uz: '38' },
    opt1: { ru: '53', uz: '53' },
    correct_text: { ru: 'Правильно. Пять десятков больше трёх. Пятьдесят три больше.', uz: "To'g'ri. Besh o'nlik uchtadan ko'p. Ellik uch katta." },
    wrong_0: {
      ru: 'Ты сравнил по единицам. Сначала смотрим десятки. У пятидесяти трёх пять десятков, у тридцати восьми три. Пятьдесят три больше.',
      uz: "Birliklarga qarab solishtirdingiz. Avval o'nliklarga qaraymiz. Ellik uchda besh o'nlik, o'ttiz sakkizda uch. Ellik uch katta."
    },
    wrong_default: {
      ru: 'Сначала сосчитай десятки в каждом числе и сравни их.',
      uz: "Avval har sondagi o'nliklarni sanab, ularni solishtiring."
    },
    audio: {
      intro: { ru: 'Какое из двух чисел больше? Сравни и выбери.', uz: "Ikki sondan qaysi biri katta? Solishtirib, tanlang." },
      on_correct: { ru: 'Верно. Пять десятков больше трёх. Пятьдесят три больше.', uz: "To'g'ri. Besh o'nlik uchtadan ko'p. Ellik uch katta." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор и сравни десятки.', uz: "Unchalik emas. Tushuntirishga qarab, o'nliklarni solishtiring." }
    }
  },

  s5: {
    eyebrow: { ru: 'Равные десятки', uz: "Teng o'nliklar" },
    instruction: { ru: 'Десятки одинаковые. Теперь смотрим единицы', uz: "O'nliklar bir xil. Endi birliklarga qaraymiz" },
    full_text: { ru: 'Восемь больше четырёх. 38 больше 34.', uz: "Sakkiz to'rtdan ko'p. 38 — 34 dan katta." },
    full_audio: { ru: 'Десятки равны, по три. Смотрим единицы. Восемь больше четырёх. Тридцать восемь больше.', uz: "O'nliklar teng, uchtadan. Birliklarga qaraymiz. Sakkiz to'rtdan ko'p. O'ttiz sakkiz katta." },
    audio: {
      ru: [
        'Здесь десятков поровну, по три у каждого числа.',
        'Когда десятки равны, смотрим единицы. У одного четыре, у другого восемь. Восемь больше.'
      ],
      uz: [
        "Bu yerda o'nliklar teng, har sonda uchtadan.",
        "O'nliklar teng bo'lsa, birliklarga qaraymiz. Birida to'rt, ikkinchisida sakkiz. Sakkiz ko'proq."
      ]
    }
  },
  ...
  s6: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Десятки равны —', uz: "O'nliklar teng —" },
    title_part2_em: { ru: 'смотрим единицы', uz: 'birliklarga qaraymiz' },
    title_part3: { ru: '', uz: '' },
    tip: {
      ru: 'Если десятков поровну, сравниваем единицы. Есть три знака: больше, меньше и равно.',
      uz: "Agar o'nliklar teng bo'lsa, birliklarni solishtiramiz. Uch belgi bor: katta, kichik va teng."
    },
    audio: {
      ru: 'Когда десятков поровну, смотрим на единицы. У какого числа единиц больше, то и больше. Есть три знака. Больше, меньше и равно.',
      uz: "O'nliklar teng bo'lganda, birliklarga qaraymiz. Qaysi sonda birlik ko'p bo'lsa, o'sha katta. Uch belgi bor. Katta, kichik va teng."
    }
  },

  s7: {
    eyebrow: { ru: 'Тренировка · 2 / 4', uz: 'Mashq · 2 / 4' },
    title: { ru: 'Поставь верный знак между числами', uz: "Sonlar orasiga to'g'ri belgini qo'ying" },
    left_num: { ru: '62', uz: '62' },
    right_num: { ru: '60', uz: '60' },
    correct_text: { ru: 'Правильно. Десятки равны, две единицы больше нуля. Шестьдесят два больше.', uz: "To'g'ri. O'nliklar teng, ikki birlik noldan ko'p. Oltmish ikki katta." },
    wrong_1: {
      ru: 'Десятки равны, но единицы разные. Две единицы больше нуля, значит первое число больше.',
      uz: "O'nliklar teng, lekin birliklar har xil. Ikki birlik noldan ko'p, demak birinchi son katta."
    },
    wrong_2: {
      ru: 'Рот крокодила открывается к большему числу. Первое число больше, значит знак больше.',
      uz: "Timsohning og'zi kattaroq songa ochiladi. Birinchi son katta, demak belgi katta."
    },
    wrong_default: {
      ru: 'Десятки равны. Сравни единицы и выбери знак.',
      uz: "O'nliklar teng. Birliklarni solishtirib, belgini tanlang."
    },
    audio: {
      intro: { ru: 'Поставь между числами верный знак: больше, меньше или равно.', uz: "Sonlar orasiga to'g'ri belgini qo'ying: katta, kichik yoki teng." },
      on_correct: { ru: 'Верно. Десятки равны, две единицы больше нуля. Знак больше.', uz: "To'g'ri. O'nliklar teng, ikki birlik noldan ko'p. Belgi katta." },
      on_wrong: { ru: 'Не совсем. Десятки равны, проверь единицы.', uz: "Unchalik emas. O'nliklar teng, birliklarni tekshiring." }
    }
  },
  ...
  s8: {
    ...
    opt0: { ru: '47', uz: '47' },
    opt1: { ru: '51', uz: '51' },
    correct_text: { ru: 'Правильно. Пять десятков больше четырёх. Пятьдесят один больше.', uz: "To'g'ri. Besh o'nlik to'rttadan ko'p. Ellik bir katta." },
    wrong_0: {
      ru: 'Единицы обманывают. Сначала смотрим десятки. Пять десятков больше четырёх, значит пятьдесят один больше.',
      uz: "Birliklar aldaydi. Avval o'nliklarga qaraymiz. Besh o'nlik to'rttadan ko'p, demak ellik bir katta."
    },
    wrong_default: {
      ru: 'Сначала сосчитай десятки в каждом числе и сравни их.',
      uz: "Avval har sondagi o'nliklarni sanab, ularni solishtiring."
    ...
    audio: {
      intro: { ru: 'Какое из двух чисел больше? Сравни по десяткам.', uz: "Ikki sondan qaysi biri katta? O'nliklar bo'yicha solishtiring." },
      on_correct: { ru: 'Верно. Пять десятков больше четырёх. Пятьдесят один больше.', uz: "To'g'ri. Besh o'nlik to'rttadan ko'p. Ellik bir katta." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор и сравни десятки.', uz: "Unchalik emas. Tushuntirishga qarab, o'nliklarni solishtiring." }
    }
  },
  ...
  sg: {
    ...
    instruction: { ru: 'Поставь верный знак', uz: "To'g'ri belgini qo'ying" },
    correct_text: { ru: 'Верно! Идём дальше.', uz: "To'g'ri! Davom etamiz." },
    done_text: { ru: 'Молодцы! Все знаки на местах.', uz: "Barakalla! Hamma belgi o'z joyida." },
    retry_audio: { ru: 'Ничего страшного. Сравни десятки, потом единицы.', uz: "Hech qisi yo'q. O'nliklarni, keyin birliklarni solishtiring." },
    audio: {
      intro: { ru: 'Поиграем со знаками. Я показываю два числа, а ты ставишь знак больше, меньше или равно. Начинаем.', uz: "Belgilar bilan o'ynaymiz. Men ikki son ko'rsataman, siz katta, kichik yoki teng belgisini qo'yasiz. Boshladik." }
    }
  },

  sGuest: {
    eyebrow: { ru: 'Сравнили честно', uz: 'Adolat bilan solishtirildi' },
    title: { ru: 'Теперь мы знаем точно', uz: 'Endi aniq bilamiz' },
    body: {
      ru: 'Рано, Анвар и Зухра сравнили числа по десяткам и узнали точно, кто собрал больше. Не на глаз, а по разрядам.',
      uz: "Ra'no, Anvar va Zuhra sonlarni o'nliklab solishtirib, kim ko'proq terganini aniq bilishdi. Ko'z bilan emas, razryad bilan."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, мы сравнили числа по десяткам.',
        'Теперь точно знаем, кто собрал больше. Глаз мог обмануть, а разряды — нет.'
      ],
      uz: [
        "Tinglang, biz sonlarni o'nliklab solishtirdik.",
        "Endi kim ko'proq terganini aniq bilamiz. Ko'z aldashi mumkin edi, razryad esa yo'q."
      ]
    }
  },
  ...
  s9: {
    ...
    opt0: { ru: '46', uz: '46' },
    opt1: { ru: '64', uz: '64' },
    correct_text: { ru: 'Правильно. Шесть десятков больше четырёх. Шестьдесят четыре больше.', uz: "To'g'ri. Olti o'nlik to'rttadan ko'p. Oltmish to'rt katta." },
    wrong_0: {
      ru: 'Ты посмотрел на единицы или поменял разряды. У шестидесяти четырёх шесть десятков, это больше. Сначала считай десятки.',
      uz: "Birliklarga qaradingiz yoki razryadlarni almashtirdingiz. Oltmish to'rtda olti o'nlik, bu ko'proq. Avval o'nliklarni sanang."
    },
    wrong_default: {
      ru: 'Сначала сосчитай десятки в каждом числе и сравни их.',
      uz: "Avval har sondagi o'nliklarni sanab, ularni solishtiring."
    ...
    fact_text: {
      ru: 'Сравнивая цены, ты видишь, что дешевле. Широкая сторона знака всегда открыта к большему числу.',
      uz: "Narxlarni solishtirganda qaysi arzonroq ekanini ko'rasiz. Belgining keng tomoni doim kattaroq songa ochiladi."
    },
    fact_audio: {
      ru: 'А знаешь, сравнивая цены, ты видишь, что дешевле. Широкая сторона знака открыта к большему числу.',
      uz: "Bilasizmi, narxlarni solishtirganda qaysi arzonroq ekanini ko'rasiz. Belgining keng tomoni kattaroq songa ochiladi."
    },
    audio: {
      intro: { ru: 'Последнее задание. Какое из двух чисел больше? Сравни и выбери.', uz: "Oxirgi topshiriq. Ikki sondan qaysi biri katta? Solishtirib, tanlang." },
      on_correct: { ru: 'Верно. Шесть десятков больше четырёх. Шестьдесят четыре больше.', uz: "To'g'ri. Olti o'nlik to'rttadan ko'p. Oltmish to'rt katta." },
      on_wrong: { ru: 'Не совсем. Посмотри разбор и сравни десятки.', uz: "Unchalik emas. Tushuntirishga qarab, o'nliklarni solishtiring." }
    }
  },
  ...
  s10: {
    ...
    main_2_em: { ru: 'сравнивать двузначные числа', uz: "ikki xonali sonlarni solishtira olasiz" },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке научимся считать по пять и по десять, вперёд и назад.',
      uz: "Keyingi darsda beshlab va o'nlab, oldinga va orqaga sanashni o'rganamiz."
    },
    audio: {
      ru: [
        'Молодец! Ты сравнил числа по десяткам и единицам.',
        'Теперь ты умеешь сравнивать двузначные числа. На следующем уроке будем считать по пять и по десять. До встречи!'
      ],
      uz: [
        "Barakalla! Sonlarni o'nlik va birlik bo'yicha solishtirdingiz.",
        "Endi siz ikki xonali sonlarni solishtirasiz. Keyingi darsda beshlab va o'nlab sanaymiz. Ko'rishguncha!"
      ]
    }
  }
};
...
// ============================================================
// SCREEN-KOMPONENTLAR — Dars22 (14 ekran)
// ============================================================
...
// PairViz — ikki son AppleViz bilan yonma-yon, oraliqda ixtiyoriy CompareSign (timsoh).
const np = (n) => ({ tens: Math.floor(n / 10), ones: n % 10 });
const PairViz = ({ a, b, sign = null, solved = false, ans = null }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 3vw, 26px)', flexWrap: 'wrap' }}>
      <AppleViz tens={pa.tens} ones={pa.ones} pop={solved} ans={ans != null && ans === a ? a : null}/>
      {sign && <CompareSign dir={sign} big/>}
      <AppleViz tens={pb.tens} ones={pb.ones} pop={solved} ans={ans != null && ans === b ? b : null}/>
    </div>
  );
};

// s0 — HOOK (soft): ikki uyum (34, 52), ko'z bilan kimda ko'p? har javob OK.
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.6vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-end', gap: 'clamp(20px, 7vw, 56px)', flexWrap: 'wrap' }}>
            {[{ svg: <RanoSVG mood="pointing"/>, lab: CONTENT.sIntro.rano_label, n: 12 },
              { svg: <AnvarSVG pose="coming"/>, lab: CONTENT.sIntro.anvar_label, n: 14 }].map((p, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(4px, 1vw, 8px)' }}>
                <div style={{ height: 'clamp(88px, 22vw, 120px)', display: 'flex', alignItems: 'flex-end' }}>{p.svg}</div>
                <BasketTop n={p.n} kind="apple"/>
                <span className="g1-cast-name">{t(p.lab)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {picked && (
          <FeedbackBlock show={true} isCorrect={picked === 'b'} wrongClass="frame-tip">
            <Reaction state={picked === 'b' ? 'correct' : 'wrong'} praise={picked === 'b' ? (t(c.audio.on_correct)) : t(c.audio.on_wrong)}/>
          </FeedbackBlock>
        )}
      </div>
    </Stage>
...
// s1 — EXPLORATION: razryad kartalari, o'qi/yoz (4 o'nlik 7 birlik -> 47).
const Screen1 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s1;
  const audio = useAudio([{ id: 's1_a0', text: c.audio[lang][0], trigger: 'on_mount', waits_for: null }]);
  const canAct = useCanAnswer(audio);
...
// s2 — EXPLORATION: solishtirish o'nlik bo'yicha (34 va 52 -> 52 katta).
const Screen2 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s2;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <p className="h-sub title fade-up">{t(c.instruction)}</p>
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 26px)' }}>
          <PairViz a={34} b={52} sign={done ? 'lt' : null} solved={done}/>
          {!done && (
            <button className="btn" disabled={!canAct} onClick={reveal}
              style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(20px, 3vw, 30px)', fontSize: 'clamp(14px, 1.8vw, 16px)' }}>
              {lang === 'uz' ? 'Solishtiramiz' : 'Сравним'}
            </button>
          )}
        </div>
        {done && (
...
// s3 — RULE: avval o'nliklarni solishtir (timsoh og'zi kattaroq songa).
const Screen3 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s3;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          <DigitGlyph d={52} size="big"/>
          <CompareSign dir="gt" big/>
          <DigitGlyph d={34} size="big"/>
        </div>
...
// s4 — TEST MC: qaysi katta 38/53 (idx1).
const Screen4 = (props) => {
  const c = CONTENT.s4;
  const t = useT();
  return (
    <QuestionScreen
      ...
      figure={(solved) => <PairViz a={38} b={53} solved={solved} ans={solved ? 53 : null}/>}
      options={[<DigitGlyph d={38} size="mid"/>, <DigitGlyph d={53} size="mid"/>]}
      correctIdx={1}
      mascot={false}
...
// s5 — EXPLORATION: teng o'nlik holati (34 va 38 -> 38 katta).
const Screen5 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s5;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <p className="h-sub title fade-up">{t(c.instruction)}</p>
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 26px)' }}>
          <PairViz a={34} b={38} sign={done ? 'lt' : null} solved={done}/>
          {!done && (
            <button className="btn" disabled={!canAct} onClick={reveal}
              style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(20px, 3vw, 30px)', fontSize: 'clamp(14px, 1.8vw, 16px)' }}>
              {lang === 'uz' ? 'Solishtiramiz' : 'Сравним'}
...
// s6 — RULE: teng o'nlikda birlikka qara; uch belgi > < =.
const Screen6 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s6;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          {EX.map((e, i) => (
            <div key={i} className={`fade-up delay-${Math.min(i + 1, 3)}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2.6vw, 18px)' }}>
              <DigitGlyph d={e.a} size="mid"/>
              <CompareSign dir={e.dir}/>
              <DigitGlyph d={e.b} size="mid"/>
            </div>
          ))}
        </div>
        <BitSays text={t(c.tip)}/>
...
// s7 — TEST MC (belgi qo'y): 62 ? 60, belgilar [>, =, <] -> idx0 (>).
const Screen7 = (props) => {
  const c = CONTENT.s7;
  const t = useT();
  return (
    <QuestionScreen
      ...
      question={(
        <div>
          <h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 3vw, 22px)', marginTop: 'clamp(10px, 2vw, 16px)' }}>
            <DigitGlyph d={62} size="big"/>
            <span className="d4-numtile" style={{ color: T.ink3 }} aria-hidden="true">?</span>
            <DigitGlyph d={60} size="big"/>
          </div>
        </div>
      )}
      figure={() => null}
      options={[<CompareSign dir="gt"/>, <CompareSign dir="eq"/>, <CompareSign dir="lt"/>]}
      correctIdx={0}
      optionsCols={3}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
...
// s8 — TEST MC: qaysi katta 47/51 (idx1).
const Screen8 = (props) => {
  const c = CONTENT.s8;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <PairViz a={47} b={51} solved={solved} ans={solved ? 51 : null}/>}
      options={[<DigitGlyph d={47} size="mid"/>, <DigitGlyph d={51} size="mid"/>]}
      correctIdx={1}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
      onNext={props.onNext} onPrev={props.onPrev}
...
// sg — MINI-O'YIN (3 raund, ballsiz): belgi qo'y (55=55, 73>69, 40<80).
const SIGN_ROUNDS = [
  { a: 55, b: 55, opts: ['gt', 'eq', 'lt'], ans: 'eq' },
  { a: 73, b: 69, opts: ['gt', 'eq', 'lt'], ans: 'gt' },
  { a: 40, b: 80, opts: ['gt', 'eq', 'lt'], ans: 'lt' },
];
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 3vw, 22px)' }}>
            <DigitGlyph d={round.a} size="big"/>
            {solvedItem ? <CompareSign dir={round.ans} big/> : <span className="d4-numtile" style={{ color: T.ink3 }} aria-hidden="true">?</span>}
            <DigitGlyph d={round.b} size="big"/>
          </div>
          {!solvedItem && (
            <div className="g1-gameopts" key={ri}>
              {round.opts.map((dir, i) => (
...
// s9 — TEST final + FactCard: qaysi katta 46/64 (idx1).
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <PairViz a={46} b={64} solved={solved} ans={solved ? 64 : null}/>}
      options={[<DigitGlyph d={46} size="mid"/>, <DigitGlyph d={64} size="mid"/>]}
      correctIdx={1}
      mascot={false}
      factOnCorrect={(
...
// ============================================================
export default function CompareTwoDigitLesson({

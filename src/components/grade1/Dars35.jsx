...
// ============================================================================
// ░░ 1-SINF · Dars35 — "Масса — килограмм" (num-1-35-v1) · Б11 (o'lchov) · spec: ETALON_1SINF.md ░░
// Baza: Dars29 (infra + PQ/cast string-in-string). YADRO: massa — og'irroq/yengilroq (tarozi); kilogramm (kg) — massa o'lchovi; kg da sodda sanoq.
// MEXANIKA: BalanceFig (ikki pallali tarozi, og'ir tomon pastga egiladi), KgWeights (1 kg toshlari), PQ (savol matni). Variantlar — MATN ("3 kg") yoki tarozi.
// Matn ovozда to'liq o'qiladi (1-sinf), ekranда qisqa tayanch. Typing YO'Q, tap/tanlash.
// Misconception'lar: M1 katta = og'ir deb o'ylash (hajm ≠ massa) · M2 og'ir tomonni teskari aniqlash · M3 kg va boshqa birlikни almashtirish.
//
...
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: massani o'lchaymiz
  ...
  { id: 's6',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // nechta kg (3)
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qaysi yengil (tarozi)
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // massa qo'shish 2 kg + 3 kg = 5 kg
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: massa aralash
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik -> Dars36
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: massa qo'shish + fakt
...
const CONTENT = {
  lab: {
    melon: { ru: 'Арбуз', uz: 'Tarvuz' },
    apple: { ru: 'Яблоко', uz: 'Olma' },
    grape: { ru: 'Виноград', uz: 'Uzum' },
    heavy: { ru: 'Тяжелее', uz: "Og'irroq" },
    light: { ru: 'Легче', uz: 'Yengilroq' }
  },

  sIntro: {
    eyebrow: { ru: 'История', uz: 'Hikoya' },
    title: { ru: 'Тяжелее или легче', uz: "Og'irroq yoki yengilroq" },
    body: {
      ru: 'Мы измеряли длину. Сегодня узнаем про массу: что тяжелее, а что легче. И познакомимся с килограммом.',
      uz: "Uzunlikni o'lchadik. Bugun massa haqida bilamiz: nima og'irroq, nima yengilroq. Va kilogramm bilan tanishamiz."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! Длину мы измерять умеем.',
        'Сегодня узнаем про массу.',
        'Что-то тяжелее, а что-то легче. Это покажут весы.',
        'А мерка массы — килограмм. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! Uzunlikni o'lchashni bilamiz.",
        "Bugun massa haqida bilamiz.",
        "Biror narsa og'irroq, biror narsa yengilroq. Buni tarozi ko'rsatadi.",
        "Massa o'lchovi esa — kilogramm. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: 'Арбуз и яблоко. Что', uz: 'Tarvuz va olma. Qaysi biri' },
    title_part2_em: { ru: 'тяжелее', uz: "og'irroq" },
    title_part3: { ru: '?', uz: '?' },
    question: { ru: 'Как думаешь? Нажми ответ.', uz: "Qanday o'ylaysiz? Javobni bosing." },
    opt0: { ru: 'Арбуз', uz: 'Tarvuz' },
    opt1: { ru: 'Яблоко', uz: 'Olma' },
    opt2: { ru: 'Трудно сказать', uz: 'Aytish qiyin' },
    audio: {
      intro: {
        ru: 'Арбуз и яблоко. Как думаешь, что тяжелее? Нажми ответ.',
  ...
  s1: {
    ...
    instruction: { ru: 'Положим арбуз и яблоко на весы', uz: "Tarvuz va olmani tarozining pallasiga qo'yamiz" },
    reveal_label: { ru: 'Взвесить', uz: "O'lchash" },
    full_text: { ru: 'Тяжёлая чаша опускается вниз. Арбуз тяжелее.', uz: "Og'ir palla pastga tushadi. Tarvuz og'irroq." },
    full_audio: { ru: 'Смотри. Чаша с арбузом опустилась вниз. Значит арбуз тяжелее яблока.', uz: "Qarang. Tarvuzli palla pastga tushdi. Demak tarvuz olmadan og'irroq." },
    audio: {
      ru: [
        'Положим арбуз на одну чашу, а яблоко на другую.',
        'Та чаша, что тяжелее, опустится вниз.'
      ],
      uz: [
        "Tarvuzni bir pallaga, olmani boshqasiga qo'yamiz.",
        "Qaysi palla og'irroq bo'lsa, pastga tushadi."
      ]
    }
  },
  ...
  s2: {
    ...
    title_part1: { ru: 'Тяжёлое —', uz: "Og'ir —" },
    title_part2_em: { ru: 'вниз, лёгкое — вверх', uz: 'pastga, yengil — tepaga' },
    title_part3: { ru: '', uz: '' },
    tip: {
      ru: 'На весах тяжёлая чаша опускается вниз, а лёгкая поднимается вверх. Если чаши вровень — массы равны.',
      uz: "Tarozida og'ir palla pastga tushadi, yengili tepaga ko'tariladi. Pallalar teng bo'lsa — massalar teng."
    },
    audio: {
  ...
  s3: {
    ...
    title: { ru: 'Что тяжелее?', uz: "Qaysi og'irroq?" },
    problem: { ru: 'Посмотри на весы.', uz: "Tarozi pallasiga qarang." },
    opt0: { ru: 'Арбуз', uz: 'Tarvuz' },
    opt1: { ru: 'Яблоко', uz: 'Olma' },
    correct_text: { ru: 'Правильно. Чаша с арбузом ниже — он тяжелее.', uz: "To'g'ri. Tarvuzli palla pastroqda — u og'irroq." },
    wrong_1: {
      ru: 'Чаша с яблоком выше — значит яблоко легче. Тяжелее арбуз.',
      uz: "Olmali palla yuqorida — demak olma yengilroq. Og'irroq — tarvuz."
    },
    wrong_default: {
      ru: 'Тяжёлая чаша опускается вниз.',
      uz: "Og'ir palla pastga tushadi."
    },
    audio: {
      intro: { ru: 'Посмотри на весы. Что тяжелее, арбуз или яблоко? Выбери.', uz: "Tarozi pallasiga qarang. Qaysi og'irroq, tarvuzmi yoki olma? Tanlang." },
      on_correct: { ru: 'Верно. Арбуз тяжелее.', uz: "To'g'ri. Tarvuz og'irroq." },
      on_wrong: { ru: 'Не совсем. Тяжёлая чаша внизу.', uz: "Unchalik emas. Og'ir palla pastda." }
    }
  },

  s4: {
    eyebrow: { ru: 'Килограмм', uz: 'Kilogramm' },
    instruction: { ru: 'Массу считают в килограммах. Гири бывают разные', uz: "Massani kilogrammda o'lchaymiz. Toshlar har xil bo'ladi" },
    reveal_label: { ru: 'Показать', uz: "Ko'rsatish" },
    full_text: { ru: 'Гири разные: один, два и пять килограммов.', uz: "Toshlar har xil: bir, ikki va besh kilogramm." },
    full_audio: { ru: 'Гири бывают разные. На каждой написан её вес. Один, два, пять килограммов. Число на гире — это её килограммы.', uz: "Toshlar har xil bo'ladi. Har toshda uning og'irligi yozilgan. Bir, ikki, besh kilogramm. Toshdagi son — uning kilogrammi." },
    audio: {
      ru: [
        'Массу измеряют в килограммах. Короткое имя — ка-гэ.',
        'Гири бывают разные. На каждой написано число килограммов.'
      ],
      uz: [
        "Massani kilogrammda o'lchaymiz. Qisqa nomi — ka-ge.",
        "Toshlar har xil bo'ladi. Har birida kilogramm soni yozilgan."
      ]
    }
  },
  ...
  s5: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Складываем гири —', uz: "Toshlarni qo'shsak —" },
    title_part2_em: { ru: 'узнаём массу', uz: 'jami massa' },
    title_part3: { ru: '', uz: '' },
    tip: {
      ru: 'На каждой гире написан её вес в килограммах. Чтобы узнать общую массу, складываем числа на гирях.',
      uz: "Har toshda uning og'irligi kilogrammda yozilgan. Umumiy massani bilish uchun toshlardagi sonlarni qo'shamiz."
    },
    audio: {
      ru: 'Запомни. На гире написан её вес. Чтобы узнать массу, складываем числа на гирях.',
      uz: "Eslab qoling. Toshda uning og'irligi yozilgan. Massani bilish uchun toshlardagi sonlarni qo'shamiz."
    }
  },
  ...
  s6: {
    ...
    wrong_2: {
      ...
      uz: "To'rt emas. Ikki va bir — uch kilogramm."
    },
    wrong_default: {
  ...
  s7: {
    ...
    title: { ru: 'Что легче?', uz: 'Qaysi yengilroq?' },
    problem: { ru: 'Посмотри на весы.', uz: "Tarozi pallasiga qarang." },
    opt0: { ru: 'Виноград', uz: 'Uzum' },
    opt1: { ru: 'Яблоко', uz: 'Olma' },
    correct_text: { ru: 'Правильно. Чаша с виноградом выше — он легче.', uz: "To'g'ri. Uzumli palla yuqorida — u yengilroq." },
    wrong_1: {
      ru: 'Чаша с яблоком ниже — значит яблоко тяжелее. Легче виноград.',
      uz: "Olmali palla pastroqda — demak olma og'irroq. Yengilroq — uzum."
    },
    wrong_default: {
      ru: 'Лёгкая чаша поднимается вверх.',
      uz: "Yengil palla tepaga ko'tariladi."
    },
    audio: {
      intro: { ru: 'Посмотри на весы. Что легче, виноград или яблоко? Выбери.', uz: "Tarozi pallasiga qarang. Qaysi yengilroq, uzummi yoki olma? Tanlang." },
      on_correct: { ru: 'Верно. Виноград легче.', uz: "To'g'ri. Uzum yengilroq." },
      on_wrong: { ru: 'Не совсем. Лёгкая чаша вверху.', uz: "Unchalik emas. Yengil palla tepada." }
    }
  },
  ...
  s8: {
    ...
    correct_text: { ru: 'Правильно. Два и три килограмма — пять килограммов.', uz: "To'g'ri. Ikki va uch kilogramm — besh kilogramm." },
    wrong_1: {
      ru: 'Два и три — это пять, не шесть. Всего пять килограммов.',
      uz: "Ikki va uch — bu besh, olti emas. Jami besh kilogramm."
    },
    wrong_2: {
      ru: 'Дециметр — это длина, а тут масса. Получится пять килограммов.',
      uz: "Detsimetr — bu uzunlik, bu yerda esa massa. Besh kilogramm chiqadi."
    },
    wrong_default: {
  ...
  sg: {
    ...
    r3_o0: { ru: '7 кг', uz: '7 kg' }, r3_o1: { ru: '8 кг', uz: '8 kg' }, r3_o2: { ru: '7 дм', uz: '7 dm' },
    audio: {
      intro: { ru: 'Поиграем с весами и гирями. Я спрашиваю, а ты выбираешь верный ответ. Начинаем.', uz: "Tarozi va toshlar bilan o'ynaymiz. Men so'rayman, siz to'g'ri javobni tanlaysiz. Boshladik." }
    }
  },

  sGuest: {
    eyebrow: { ru: 'Здорово', uz: "Zo'r" },
    title: { ru: 'Узнали про массу', uz: 'Massani bildik' },
    body: {
      ru: 'Рано, Анвар и Зухра научились сравнивать массу и считать в килограммах. Дальше будем читать таблицы и картинки с данными.',
      uz: "Ra'no, Anvar va Zuhra massani solishtirishni va kilogrammda sanashni o'rgandi. Keyin jadval va rasmli ma'lumotlarni o'qiymiz."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, мы научились сравнивать массу.',
        'И считать в килограммах. Дальше будем читать таблицы и картинки с данными.'
      ],
      uz: [
        "Tinglang, biz massani solishtirishni o'rgandik.",
        "Va kilogrammda sanashni. Keyin jadval va rasmli ma'lumotlarni o'qiymiz."
      ]
    }
  },
  ...
  s9: {
    ...
    opt2: { ru: '7 дм', uz: '7 dm' },
    correct_text: { ru: 'Правильно. Три и четыре килограмма — семь килограммов.', uz: "To'g'ri. Uch va to'rt kilogramm — yetti kilogramm." },
    wrong_1: {
      ru: 'Три и четыре — это семь, не восемь. Всего семь килограммов.',
      uz: "Uch va to'rt — bu yetti, sakkiz emas. Jami yetti kilogramm."
    },
    wrong_2: {
      ru: 'Дециметр — это длина, а тут масса. Семь килограммов.',
      uz: "Detsimetr — bu uzunlik, bu yerda esa massa. Yetti kilogramm."
    },
    wrong_default: {
    ...
    fact_text: {
      ru: 'Большое не всегда тяжёлое. Большой шар лёгкий, а маленький камень тяжёлый.',
      uz: "Katta narsa har doim og'ir emas. Katta pufak yengil, kichik tosh esa og'ir."
    },
    fact_audio: {
      ru: 'Большое не всегда тяжёлое. Большой воздушный шар лёгкий, а маленький камень тяжёлый.',
      uz: "Katta narsa har doim og'ir emas. Katta havo shari yengil, kichik tosh esa og'ir."
    },
    audio: {
  ...
  s10: {
    ...
    main_2_em: { ru: 'сравнивать массу', uz: 'massani solishtira olasiz' },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке будем читать таблицы и картинки с данными.',
      uz: "Keyingi darsda jadval va rasmli ma'lumotlarni o'qiymiz."
    },
    audio: {
      ru: [
        'Молодец! Ты научился сравнивать массу: тяжелее и легче.',
        'И считать в килограммах. На следующем уроке будем читать данные. До встречи!'
      ],
      uz: [
        "Barakalla! Massani solishtirishni o'rgandingiz: og'irroq va yengilroq.",
        "Va kilogrammda sanashni. Keyingi darsda ma'lumotlarni o'qiymiz. Ko'rishguncha!"
      ]
    }
  }
};
...
// ThoughtBubble — mavzuga oid o'ylov pufakchasi (ichida tarozi belgisi). .fb klasslarini qayta ishlatadi.
...
// ScaleLandBg — intro foni (bozor): osmon, quyosh, bulut, soyabon, osilgan tarozi (tebranadi), mevali rastalar, pol.
const ScaleLandBg = () => (
  <svg className="g1-yard-bg" viewBox="0 0 400 220" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
    <defs>
      <linearGradient id="d35ySky" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#DCEBFF"/><stop offset="100%" stopColor="#FFF3DC"/></linearGradient>
      <linearGradient id="d35yFloor" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D9BE93"/><stop offset="100%" stopColor="#BE9A63"/></linearGradient>
      <linearGradient id="d35yCounter" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#B98A50"/><stop offset="100%" stopColor="#8A5E2E"/></linearGradient>
      <radialGradient id="d35ySun" cx="40%" cy="36%" r="70%"><stop offset="0%" stopColor="#FFE7A6"/><stop offset="60%" stopColor="#FFC24D"/><stop offset="100%" stopColor="#F39A1E"/></radialGradient>
    </defs>
    <rect width="400" height="220" fill="url(#d35ySky)"/>
    {/* quyosh */}
    <g className="g1-amb-rays" style={{ transformBox: 'fill-box', transformOrigin: 'center' }} stroke="#FFD86B" strokeWidth="3" strokeLinecap="round"><path d="M348 16 v-6 M348 72 v6 M314 44 h-6 M382 44 h6 M325 25 l-4 -4 M371 63 l4 4 M371 25 l4 -4 M325 63 l-4 4"/></g>
    <g className="g1-amb-sun" style={{ transformBox: 'fill-box', transformOrigin: 'center' }}><circle cx="348" cy="44" r="19" fill="url(#d35ySun)"/><ellipse cx="341" cy="37" rx="5.5" ry="4" fill="rgba(255,255,255,0.45)"/></g>
    ...
    {/* bozor soyaboni — chiziqli */}
    <rect x="0" y="70" width="400" height="16" fill="#E0552E"/>
    {Array.from({ length: 13 }).map((_, i) => <rect key={i} x={i * 32 + 16} y="70" width="16" height="16" fill="#F6EFE0"/>)}
    {Array.from({ length: 25 }).map((_, i) => <path key={'s' + i} d={`M${i * 16} 86 a8 8 0 0 0 16 0Z`} fill={i % 2 ? '#E0552E' : '#F6EFE0'}/>)}
    {/* osilgan tarozi — tebranadi */}
    <g className="g1-amb-sway" style={{ transformBox: 'fill-box', transformOrigin: 'top center' }}>
      <line x1="120" y1="96" x2="120" y2="112" stroke="#8A6A3A" strokeWidth="2"/>
      <line x1="104" y1="112" x2="136" y2="112" stroke="#B8954E" strokeWidth="3.4" strokeLinecap="round"/>
      <path d="M100 114 a4 2.5 0 0 0 8 0Z" fill="#D7B87A"/><path d="M132 114 a4 2.5 0 0 0 8 0Z" fill="#D7B87A"/>
      <line x1="120" y1="112" x2="120" y2="124" stroke="#B8954E" strokeWidth="2"/><circle cx="120" cy="128" r="4" fill="#C9A86A"/>
    </g>
    {/* pol */}
    <rect x="0" y="150" width="400" height="70" fill="url(#d35yFloor)"/>
    {/* mevali rastalar (yashiklar) */}
    <g>
      <rect x="24" y="158" width="70" height="26" rx="3" fill="url(#d35yCounter)"/>
      <ellipse cx="40" cy="158" rx="7" ry="6" fill="#E5484D"/><ellipse cx="56" cy="156" rx="7" ry="6" fill="#E5484D"/><ellipse cx="72" cy="158" rx="7" ry="6" fill="#F2A93C"/>
      <rect x="300" y="160" width="72" height="24" rx="3" fill="url(#d35yCounter)"/>
      <ellipse cx="318" cy="160" rx="9" ry="7" fill="#4FA84E"/><ellipse cx="340" cy="159" rx="9" ry="7" fill="#4FA84E"/><ellipse cx="360" cy="161" rx="8" ry="6.5" fill="#3FA34D"/>
    </g>
  </svg>
);

// CastScene — bozor foni + Ra'no + Anvar + Zuhra; pufakchalarda mavzu (tarozi) belgisi.
  ...
  return (
    <div className="g1-yardscene">
      ...
      <div className="g1-yard-cast">
        ...
        {/* o'ylov pufakchalari — bosh USTIDA (yuzni to'smaydi): personajlar TAROZI/og'irlikni o'ylaydi (mavzuga oid) */}
        <div className={`g1-yard-bubble g1-yard-bubble-rano ${step >= 1 ? 'in' : ''}`} aria-hidden="true">
          <ThoughtBubble><ScaleMiniIcon/></ThoughtBubble>
        </div>
        {happy && (
...
// SPARKS / SparkBurst — celebrate paytida otiladigan oltin uchqunlar (etalon Dars32). g1-csp + g1sparkPop.
const SPARKS = [
  { dx: '0px', dy: '-34px', s: 8, d: '0s' },
  { dx: '28px', dy: '-22px', s: 6, d: '0.05s' },
  { dx: '-28px', dy: '-22px', s: 6, d: '0.09s' },
  { dx: '34px', dy: '2px', s: 5, d: '0.13s' },
  { dx: '-34px', dy: '2px', s: 5, d: '0.07s' },
  { dx: '16px', dy: '-32px', s: 4, d: '0.11s' },
];
const SparkBurst = () => (
  <>{SPARKS.map((p, i) => (
...
// PanFruit — pallaga qo'yiladigan realistik meva (og'irligiga qarab o'lchami): melon (og'ir) > uzum > olma.
const PanFruit = ({ kind }) => {
  if (kind === 'melon') return (
    <svg className="g1-bal-fruit g1-bal-fruit-lg" viewBox="0 0 44 44" aria-hidden="true">
      <defs><radialGradient id="d35mel" cx="38%" cy="30%" r="72%"><stop offset="0%" stopColor="#A6E086"/><stop offset="55%" stopColor="#4FA84E"/><stop offset="100%" stopColor="#276B32"/></radialGradient></defs>
      <ellipse cx="22" cy="40.5" rx="15" ry="3" fill="rgba(58,53,48,0.16)"/>
      <circle cx="22" cy="22" r="18" fill="url(#d35mel)"/>
      <g stroke="#296230" strokeWidth="2" fill="none" opacity="0.72"><path d="M22 4 Q13 22 22 40"/><path d="M9 9 Q16 22 9 35"/><path d="M35 9 Q28 22 35 35"/><path d="M14 5.5 Q10 22 14 38.5"/><path d="M30 5.5 Q34 22 30 38.5"/></g>
      <path d="M9 15 A18 18 0 0 1 30 6" fill="none" stroke="rgba(255,255,255,0.35)" strokeWidth="2.4" strokeLinecap="round"/>
      <ellipse cx="15" cy="14" rx="4.6" ry="3" fill="rgba(255,255,255,0.5)"/>
      <circle cx="28" cy="12" r="1.6" fill="rgba(255,255,255,0.45)"/>
    </svg>
  );
  if (kind === 'grape') return (
    <svg className="g1-bal-fruit" viewBox="0 0 40 44" aria-hidden="true">
      <defs><radialGradient id="d35grp" cx="36%" cy="28%" r="72%"><stop offset="0%" stopColor="#BE94E6"/><stop offset="60%" stopColor="#7E52B8"/><stop offset="100%" stopColor="#5A3390"/></radialGradient></defs>
      <ellipse cx="20" cy="41" rx="11" ry="2.6" fill="rgba(58,53,48,0.14)"/>
      <path d="M20 8 q3 -5 7 -5" stroke="#6B4A2A" strokeWidth="2" fill="none"/>
      <path d="M22 6 q5 -2 8 1 q-4 2 -8 -1Z" fill="#4FA84E"/>
      <g fill="url(#d35grp)" stroke="rgba(60,30,100,0.35)" strokeWidth="0.5"><circle cx="14" cy="18" r="6"/><circle cx="26" cy="18" r="6"/><circle cx="20" cy="24" r="6"/><circle cx="12" cy="28" r="6"/><circle cx="28" cy="28" r="6"/><circle cx="20" cy="34" r="6"/></g>
  ...
  return (
    <svg className="g1-bal-fruit" viewBox="0 0 40 44" aria-hidden="true">
      ...
      <ellipse cx="20" cy="40.5" rx="12" ry="2.6" fill="rgba(58,53,48,0.15)"/>
      <rect x="19" y="9" width="2" height="8" rx="1" fill="#7A5230"/>
      <path d="M21 12 q6 -4 9 1 q-6 3 -9 -1Z" fill="#3FA34D"/>
      <path d="M21 12.5 q4 -2 7 0" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="0.8"/>
      <circle cx="20" cy="27" r="13" fill="url(#d35app)"/>
      <path d="M9 22 A13 13 0 0 1 26 15" fill="none" stroke="rgba(255,255,255,0.32)" strokeWidth="2.2" strokeLinecap="round"/>
      <ellipse cx="14.5" cy="21" rx="3.2" ry="4.6" fill="rgba(255,255,255,0.45)"/>
    </svg>
  );
};
// BalanceFig — Dars12 bronza turar tarozi kanoni: base + post + fulcrum + beam (og'ir tomon pastga egiladi, silliq).
// left/right: 'melon'|'apple'|'grape'. down: 'left'|'right'|'level'. anim: 'celebrate' -> to'g'ri javobda sakraydi.
const BalCords = () => (
  <svg className="g1-bal-cords" viewBox="0 0 80 48" preserveAspectRatio="none" aria-hidden="true">
    <line x1="40" y1="4" x2="9" y2="47"/><line x1="40" y1="4" x2="40" y2="47"/><line x1="40" y1="4" x2="71" y2="47"/><circle cx="40" cy="4" r="2.8"/>
  </svg>
);
// winner: to'g'ri javob qaysi pallada ('left'|'right'); answerLabel: celebrate'da chiqadigan javob nomi ("Tarvuz").
const BalanceFig = ({ left, right, down = 'level', anim, winner = null, answerLabel = null }) => {
  const tilt = down === 'left' ? 'left' : down === 'right' ? 'right' : 'eq';
  const cele = anim === 'celebrate';
  // A3 jonlilik: tekis turganda (eq) nur mikro-tebranadi -> tarozi tirik (tushuntirishda qotmaydi).
  const idle = tilt === 'eq' && !cele;
  return (
    <div className={`g1-bal ${cele ? 'g1-bal-cele' : ''} ${idle ? 'g1-bal-idle' : ''} ${cele && winner ? 'g1-bal-win-' + winner : ''}`} aria-hidden="true">
      <span className="g1-bal-base"/>
      <span className="g1-bal-post"/>
      <span className="g1-bal-fulcrum"/>
      <div className={`g1-bal-beam g1-bal-${tilt}`}>
        <span className="g1-bal-arm g1-bal-arm-l"><BalCords/><span className="g1-bal-pan"><PanFruit kind={left}/></span></span>
        <span className="g1-bal-arm g1-bal-arm-r"><BalCords/><span className="g1-bal-pan"><PanFruit kind={right}/></span></span>
      </div>
      {cele && <SparkBurst/>}
      {cele && answerLabel && <span className="g1-bal-ans">{answerLabel}</span>}
    </div>
  );
};
// KgWeight — bitta metall tarozi toshi (hajmli). kg=1|2|5 -> har xil o'lcham + ustidagi son. KATTA render.
const KgWeight = ({ kg = 1, big = false }) => {
  const base = kg >= 5 ? 66 : kg >= 2 ? 54 : 42;
  const w = big ? Math.round(base * 1.24) : base;
  return (
    <svg viewBox="0 0 46 54" width={w} height={w * 54 / 46} aria-hidden="true">
      <defs>
        <linearGradient id="d35kg" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#CFD4DA"/><stop offset="45%" stopColor="#8A9099"/><stop offset="100%" stopColor="#565B63"/></linearGradient>
        <clipPath id="d35kgClip"><path d="M12 16 h22 l3 30 a4 4 0 0 1 -4 3.6 h-20 a4 4 0 0 1 -4 -3.6Z"/></clipPath>
      </defs>
      <ellipse cx="23" cy="51.5" rx="16" ry="3" fill="rgba(58,53,48,0.18)"/>
      <path d="M16 15 a7 7 0 0 1 14 0" fill="none" stroke="#6A7078" strokeWidth="3.6"/>
      <path d="M12 16 h22 l3 30 a4 4 0 0 1 -4 3.6 h-20 a4 4 0 0 1 -4 -3.6Z" fill="url(#d35kg)" stroke="#565B62" strokeWidth="1"/>
      <path d="M14 17 h5 l-2 32 h-1 a3 3 0 0 1 -3 -3Z" fill="rgba(255,255,255,0.26)"/>
      <g clipPath="url(#d35kgClip)"><rect className="g1-kg-gleam" x="-14" y="14" width="9" height="40" fill="rgba(255,255,255,0.55)" transform="skewX(-20)"/></g>
      <circle cx="16.5" cy="16.5" r="1.3" fill="#4C5158"/><circle cx="29.5" cy="16.5" r="1.3" fill="#4C5158"/>
      <text x="23" y="38" fontSize="14" textAnchor="middle" fill="#fff" fontFamily="'JetBrains Mono', monospace" fontWeight="700">{kg}</text>
      <text x="23" y="47" fontSize="6.5" textAnchor="middle" fill="rgba(255,255,255,0.92)" fontFamily="'JetBrains Mono', monospace">kg</text>
    </svg>
  );
};
// KgWeights — toshlar to'plami. weights=[2,1] (har xil kg) yoki n (n ta 1kg, meros). anim='celebrate' + answer -> jami nishoni.
const KgWeights = ({ weights, n, anim, answer = null }) => {
  const list = weights || Array.from({ length: n || 0 }, () => 1);
  return (
    <span className={`g1-kgset ${anim === 'celebrate' ? 'g1-kgset-cele' : ''}`} style={{ position: 'relative', display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 6 }} aria-hidden="true">
      <span style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: 'clamp(7px, 2.2vw, 14px)' }}>
        {list.map((kg, i) => (
          <span key={i} className={anim === 'enter' ? 'g1-kg-in' : ''} style={{ display: 'inline-flex', animationDelay: `${i * 0.12}s` }}><KgWeight kg={kg}/></span>
        ))}
      </span>
      {anim === 'celebrate' && <SparkBurst/>}
      {anim === 'celebrate' && answer && <span className="g1-kg-ans">{answer}</span>}
    </span>
  );
};
// AddKgFig — TUSHUNTIRISH/TEST: massani qo'shish "a kg + b kg = (a+b) kg" (Dars34 AddLenFig massa versiyasi).
// Har son haqiqiy toshlarga bo'linadi (3 kg -> 2 kg + 1 kg). solved=false -> ikki guruh "+" bilan;
// solved -> "+" yig'ilib guruhlar yashil ramkada birlashadi + jami nishoni + SparkBurst.
// play (qoida slaydi) -> mount'da avtomatik false->true (worked demo). reduced-motion -> darrov.
const KG_DENOMS = [5, 2, 1];
const kgDecompose = (v) => {
  const out = [];
  let r = v;
  for (const d of KG_DENOMS) { while (r >= d) { out.push(d); r -= d; } }
  return out.length ? out : [v];
};
const AddKgFig = ({ a, b, solved: solvedProp, answer = null, play = false }) => {
  const reduced = usePrefersReducedMotion();
  const [autoSolved, setAutoSolved] = useState(false);
  useEffect(() => {
    if (!play) return;
    const id = setTimeout(() => setAutoSolved(true), reduced ? 0 : 1100);
    return () => clearTimeout(id);
  }, [play, reduced]);
  const solved = solvedProp !== undefined ? solvedProp : autoSolved;
  const groupA = kgDecompose(a);
  const groupB = kgDecompose(b);
  return (
    <span className={`g1-akg ${solved ? 'is-solved' : ''}`} style={{ position: 'relative', display: 'inline-flex', flexDirection: 'column', alignItems: 'center' }} aria-hidden="true">
      <span className="g1-akg-box">
        <span className="g1-akg-grp">{groupA.map((kg, i) => <span key={'a' + i}><KgWeight kg={kg}/></span>)}</span>
        <span className="g1-akg-op">+</span>
        <span className="g1-akg-grp">{groupB.map((kg, i) => <span key={'b' + i}><KgWeight kg={kg}/></span>)}</span>
      </span>
      {solved && answer && <span className="g1-kg-ans">{answer}</span>}
      {solved && <SparkBurst/>}
    </span>
  );
};
// KgIntroDemo — s4: 1/2/5 kg toshlar (KATTA). play -> har tosh birma-bir ko'tarilib porlaydi (1, 2, 5 sonlariga e'tibor), keyin joylashadi.
const KG_INTRO = [1, 2, 5];
const KgIntroDemo = ({ play }) => {
  const reduced = usePrefersReducedMotion();
  ...
  useEffect(() => {
    ...
    const step = () => {
      setHi(i); i += 1;
      timer = setTimeout(i <= KG_INTRO.length ? step : () => setHi(-1), 850);
    };
    timer = setTimeout(step, 250);
    return () => clearTimeout(timer);
  }, [play, reduced]);
  return (
    <span style={{ display: 'inline-flex', alignItems: 'flex-end', gap: 'clamp(12px, 3.4vw, 22px)' }} aria-hidden="true">
      {KG_INTRO.map((kg, idx) => (
        <span key={idx} className={`g1-kgi ${hi === idx ? 'g1-kgi-hl' : ''}`}><KgWeight kg={kg} big/></span>
      ))}
    </span>
  );
};
// HookSceneScale — s0 sahnasi: bozor rastasi (soyabon + peshtaxta + osilgan mevalar, ambient) + oldida bronza tarozi.
const HookSceneScale = () => (
  <div className="d35-hook" aria-hidden="true">
    <svg className="d35-hook-bg" viewBox="0 0 240 170" preserveAspectRatio="xMidYMid slice">
      <defs>
        <linearGradient id="d35hWall" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#FFF3DC"/><stop offset="100%" stopColor="#F4E3C0"/></linearGradient>
        <linearGradient id="d35hCounter" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#B98A50"/><stop offset="100%" stopColor="#8A5E2E"/></linearGradient>
      </defs>
      <rect x="0" y="0" width="240" height="170" fill="url(#d35hWall)"/>
      {/* quyosh nuri — nafas oladi */}
      <g className="g1-amb-sun" style={{ transformBox: 'fill-box', transformOrigin: 'center' }}><circle cx="206" cy="46" r="22" fill="#FFE7A6" opacity="0.6"/></g>
      {/* soyabon — chiziqli + osilgan bo'laklar */}
      <rect x="0" y="0" width="240" height="20" fill="#E0552E"/>
      {Array.from({ length: 8 }).map((_, i) => <rect key={i} x={i * 30 + 15} y="0" width="15" height="20" fill="#F6EFE0"/>)}
      {Array.from({ length: 16 }).map((_, i) => <path key={'s' + i} d={`M${i * 15} 20 a7.5 8 0 0 0 15 0Z`} fill={i % 2 ? '#E0552E' : '#F6EFE0'}/>)}
      {/* osilgan mevalar — tebranadi */}
      <g className="g1-amb-sway" style={{ transformBox: 'fill-box', transformOrigin: 'top center' }}>
        <line x1="40" y1="30" x2="40" y2="42" stroke="#8A6A3A" strokeWidth="1.4"/><circle cx="40" cy="47" r="6" fill="#E5484D"/>
        <line x1="52" y1="30" x2="52" y2="46" stroke="#8A6A3A" strokeWidth="1.4"/><circle cx="52" cy="51" r="5.5" fill="#F2A93C"/>
      </g>
      {/* peshtaxta */}
      <rect x="0" y="132" width="240" height="38" fill="url(#d35hCounter)"/>
      <rect x="0" y="132" width="240" height="5" fill="rgba(255,255,255,0.28)"/>
      {/* peshtaxtada 1 kg tosh + narx yorlig'i */}
      <g transform="translate(24 108)"><path d="M6 8 a4 4 0 0 1 8 0" fill="none" stroke="#6A7078" strokeWidth="2.4"/><path d="M2 9 h16 l2 18 h-20Z" fill="#8A9099" stroke="#565B62" strokeWidth="0.8"/><text x="10" y="23" fontSize="8" textAnchor="middle" fill="#fff" fontFamily="'JetBrains Mono', monospace" fontWeight="700">1</text></g>
    </svg>
    <div className="d35-hook-front"><BalanceFig left="melon" right="apple" down="level"/></div>
  </div>
);
// ScaleMiniIcon — pufakcha uchun kichik tarozi belgisi (mavzuga oid).
const ScaleMiniIcon = () => (
  <svg viewBox="0 0 48 46" width="100%" style={{ maxWidth: 62 }} aria-hidden="true">
    <defs><linearGradient id="d35mi" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#E8C98A"/><stop offset="100%" stopColor="#A8843E"/></linearGradient></defs>
    <rect x="20" y="12" width="8" height="27" rx="2" fill="url(#d35mi)"/>
    <rect x="11" y="37" width="26" height="5" rx="2.5" fill="#A8843E"/>
    <line x1="7" y1="15" x2="41" y2="15" stroke="#B8954E" strokeWidth="3.6" strokeLinecap="round"/>
    <circle cx="24" cy="15" r="3" fill="#C9A86A"/>
    <path d="M2 17 a5.5 3 0 0 0 11 0Z" fill="#D7B87A" stroke="#A8843E" strokeWidth="0.8"/>
    <path d="M35 17 a5.5 3 0 0 0 11 0Z" fill="#D7B87A" stroke="#A8843E" strokeWidth="0.8"/>
    <circle cx="7.5" cy="16" r="3.4" fill="#E5484D"/>
    <circle cx="40.5" cy="16" r="4.2" fill="#4FA84E"/>
  </svg>
);

// s0 — HOOK (soft): tarvuz va olma — qaysi og'ir? har javob OK.
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.6vw, 18px)' }}>
        ...
        {!picked && (
          <div className="fade-up delay-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'clamp(8px, 2vw, 14px)' }}>
            <button className="g1-tile" disabled={!canAct} onClick={() => pick('a')} style={{ width: '100%', fontSize: 'clamp(15px, 2.4vw, 19px)' }}>{t(c.opt0)}</button>
            <button className="g1-tile" disabled={!canAct} onClick={() => pick('b')} style={{ width: '100%', fontSize: 'clamp(15px, 2.4vw, 19px)' }}>{t(c.opt1)}</button>
            <button className="g1-tile" disabled={!canAct} onClick={() => pick('c')} style={{ width: '100%', fontSize: 'clamp(15px, 2.4vw, 19px)' }}>{t(c.opt2)}</button>
          </div>
        )}
        {picked && (
...
// s1 — EXPLORATION: tarozi — og'ir tomon pastga.
const Screen1 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s1;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(12px, 2.4vw, 18px)' }}>
          <BalanceFig left="melon" right="apple" down={done ? 'left' : 'level'}/>
          {!done && (
...
// s2 — RULE: og'ir pastga, yengil tepaga.
const Screen2 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s2;
...
// s3 — TEST MC: qaysi og'ir (Tarvuz, idx0). Variantlar — matn.
const Screen3 = (props) => {
  const c = CONTENT.s3;
  const t = useT();
  return (
    <QuestionScreen
      ...
      figure={(solved) => <BalanceFig left="melon" right="apple" down="left" anim={solved ? 'celebrate' : undefined} winner="left" answerLabel={t(c.opt0)}/>}
...
// s4 — EXPLORATION: kilogramm — toshlarni sanaymiz.
const Screen4 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s4;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.4vw, 16px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          <KgIntroDemo play={done}/>
          {done && <span className="fade-up eyebrow mono" style={{ color: T.accent, fontSize: 'clamp(14px, 2vw, 17px)' }}>1 · 2 · 5 kg</span>}
          {!done && (
            <button className="btn" disabled={!canAct} onClick={go}
              style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(20px, 3vw, 30px)', fontSize: 'clamp(14px, 1.8vw, 16px)' }}>
              {t(c.reveal_label)}
...
// s5 — RULE: massani kg da sanaymiz.
const Screen5 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s5;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <AddKgFig a={2} b={1} play answer={lang === 'ru' ? '3 кг' : '3 kg'}/>
        </div>
        <BitSays text={t(c.tip)}/>
      </div>
    </Stage>
...
// s6 — TEST MC: nechta kg (3 kg, idx0). Variantlar — matn.
const Screen6 = (props) => {
  const c = CONTENT.s6;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<PQ title={t(c.title)} problem={t(c.problem)}/>}
      figure={(solved) => <AddKgFig a={2} b={1} solved={solved} answer={t(c.opt0)}/>}
...
// s7 — TEST MC: qaysi yengil (Uzum, idx0). Variantlar — matn.
const Screen7 = (props) => {
  const c = CONTENT.s7;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<PQ title={t(c.title)} problem={t(c.problem)}/>}
      figure={(solved) => <BalanceFig left="grape" right="apple" down="right" anim={solved ? 'celebrate' : undefined} winner="left" answerLabel={t(c.opt0)}/>}
      options={[t(c.opt0), t(c.opt1)]}
      correctIdx={0}
      mascot={false}
      storedAnswer={props.storedAnswer} onAnswer={props.onAnswer}
...
// s8 — TEST MC: 2 kg + 3 kg = 5 kg (idx0). Variantlar — matn.
const Screen8 = (props) => {
  const c = CONTENT.s8;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<PQ title={t(c.title)} problem={t(c.problem)}/>}
      figure={(solved) => <AddKgFig a={2} b={3} solved={solved} answer={t(c.opt0)}/>}
      options={[t(c.opt0), t(c.opt1), t(c.opt2)]}
      correctIdx={0}
      optionsCols={3}
      mascot={false}
...
// sg — MINI-O'YIN (3 raund, ballsiz): massa aralash.
const GAME_ROUNDS = [
  { qk: 'q_round1', okeys: ['r1_o0', 'r1_o1'], ans: 0, viz: { balance: { left: 'melon', right: 'grape', down: 'left' } } },
  { qk: 'q_round2', okeys: ['r2_o0', 'r2_o1', 'r2_o2'], ans: 0, viz: { kg: [2, 1, 1] } },
  { qk: 'q_round3', okeys: ['r3_o0', 'r3_o1', 'r3_o2'], ans: 0, viz: { add: { a: 4, b: 3 } } },
];
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 18px)', padding: 'clamp(12px, 2.4vw, 18px)' }}>
          {round.viz && round.viz.balance && <BalanceFig left={round.viz.balance.left} right={round.viz.balance.right} down={round.viz.balance.down}/>}
          {round.viz && round.viz.kg && <KgWeights weights={round.viz.kg} anim={solvedItem ? 'celebrate' : 'enter'} answer={solvedItem ? t(c[round.okeys[round.ans]]) : null}/>}
          {round.viz && round.viz.add && <AddKgFig a={round.viz.add.a} b={round.viz.add.b} solved={solvedItem} answer={solvedItem ? t(c[round.okeys[round.ans]]) : null}/>}
          {!solvedItem && (
            <div key={ri} style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: 'clamp(10px, 2.4vw, 16px)' }}>
              {round.okeys.map((ok, i) => (
                <button key={i} className={`g1-numopt ${wrong.has(i) ? 'g1-numopt-wrong' : ''}`} disabled={wrong.has(i) || !canAns} onClick={() => pick(i)}
                  style={{ width: 'auto', minWidth: 'clamp(78px, 24vw, 110px)', padding: 'clamp(8px, 1.6vw, 12px)', fontWeight: 700, fontSize: 'clamp(14px, 1.9vw, 17px)' }}>
                  {t(c[ok])}
                </button>
              ))}
            </div>
          ...
          {solvedItem && (
            <div className="g1-numopt g1-numopt-ok" style={{ width: 'auto', minWidth: 'clamp(78px, 24vw, 110px)', padding: 'clamp(8px, 1.6vw, 12px)', fontWeight: 700, fontSize: 'clamp(14px, 1.9vw, 17px)', color: T.accent }}>
...
// s9 — TEST final + FactCard: 3 kg + 4 kg = 7 kg (idx0). Variantlar — matn.
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<PQ title={t(c.title)} problem={t(c.problem)}/>}
      figure={(solved) => <AddKgFig a={3} b={4} solved={solved} answer={t(c.opt0)}/>}
      options={[t(c.opt0), t(c.opt1), t(c.opt2)]}
      correctIdx={0}
      optionsCols={3}
      mascot={false}
...
/* === Dars35 — TAROZI (bronza turar tarozi, Dars12 kanoni): nur fulcrum ustida aylanadi, tovoqlar ipda tik osiladi === */
...
.g1-bal-fruit { position: relative; z-index: 1; width: clamp(26px, 7vw, 40px); height: auto; display: block; }
.g1-bal-fruit-lg { width: clamp(38px, 10vw, 56px); }
.g1-bal-cele { animation: g1balCele 0.8s cubic-bezier(0.3,1.3,0.5,1) both; }
@keyframes g1balCele { 0% { transform: translateY(0) scale(1); } 30% { transform: translateY(-10px) scale(1.05); } 60% { transform: translateY(0) scale(0.98); } 100% { transform: translateY(0) scale(1); } }
/* A3 jonlilik — tekis turganda nur mikro-tebranadi (tarozi tirik, tushuntirishda qotmaydi) */
.g1-bal-idle .g1-bal-beam { animation: g1balIdle 4.4s ease-in-out infinite; }
@keyframes g1balIdle { 0%, 100% { transform: translate(-50%, -50%) rotate(-1.4deg); } 50% { transform: translate(-50%, -50%) rotate(1.4deg); } }
/* B metall yaltiroq — 1 kg tosh yuzasidan yorug'lik o'tadi */
.g1-kg-gleam { animation: g1kgGleam 3.6s ease-in-out infinite; }
@keyframes g1kgGleam { 0%, 100% { transform: skewX(-20deg) translateX(0); opacity: 0; } 12% { opacity: 0.6; } 50% { transform: skewX(-20deg) translateX(52px); opacity: 0.6; } 62% { opacity: 0; } }
/* A1 uchqun chaqnashi — celebrate'da tarozi atrofidan otiladi */
.g1-csp { position: absolute; top: 42%; left: 50%; border-radius: 50%; background: radial-gradient(circle at 35% 35%, #FFF4CC, #FFB23C); box-shadow: 0 0 5px rgba(255,190,70,0.85); opacity: 0; pointer-events: none; }
.g1-bal-cele .g1-csp, .g1-kgset-cele .g1-csp { animation: g1sparkPop 0.8s ease-out both; }
...
/* javobga oid: to'g'ri javobda g'olib palla yashil porlaydi + javob nomi nishoni chiqadi */
.g1-bal-win-l .g1-bal-arm-l .g1-bal-pan, .g1-bal-win-r .g1-bal-arm-r .g1-bal-pan { box-shadow: inset 0 2px 4px rgba(255,255,255,0.5), 0 0 0 3px #3E9E52, 0 0 14px 2px rgba(62,158,82,0.55); animation: g1balWinPulse 0.9s ease-in-out 2; }
@keyframes g1balWinPulse { 0%, 100% { box-shadow: inset 0 2px 4px rgba(255,255,255,0.5), 0 0 0 3px #3E9E52, 0 0 10px 1px rgba(62,158,82,0.4); } 50% { box-shadow: inset 0 2px 4px rgba(255,255,255,0.5), 0 0 0 4px #3E9E52, 0 0 18px 4px rgba(62,158,82,0.7); } }
.g1-bal-ans, .g1-kg-ans { position: absolute; left: 50%; top: -4px; transform: translateX(-50%); background: linear-gradient(180deg, #5EC76E, #3E9E52); color: #fff; font-weight: 800; font-family: 'JetBrains Mono', monospace; padding: 3px clamp(11px, 2.6vw, 15px); border-radius: 999px; font-size: clamp(13px, 3vw, 18px); box-shadow: 0 5px 14px -3px rgba(62,158,82,0.55); white-space: nowrap; z-index: 6; animation: g1ansPop 0.55s cubic-bezier(0.34,1.4,0.6,1) both; }
.g1-kg-ans { position: absolute; top: auto; bottom: -6px; }
@keyframes g1ansPop { 0% { opacity: 0; transform: translate(-50%, 8px) scale(0.6); } 70% { transform: translate(-50%, -2px) scale(1.08); } 100% { opacity: 1; transform: translate(-50%, 0) scale(1); } }
/* HookSceneScale — bozor foni + oldida bronza tarozi */
.d35-hook { position: relative; display: inline-flex; align-items: flex-end; justify-content: center; padding: clamp(22px,4.5vw,32px) clamp(14px,3vw,22px) clamp(10px,2vw,14px); border-radius: 14px; overflow: hidden; max-width: 100%; }
.d35-hook-bg { position: absolute; inset: 0; width: 100%; height: 100%; z-index: 0; }
.d35-hook-front { position: relative; z-index: 1; }
.d35-hook-front .g1-bal { width: clamp(196px, 50vw, 276px); height: clamp(158px, 40vw, 216px); margin: 0; }
/* KgWeights — kirish (ketma-ket ko'tarilib paydo) */
.g1-kg-in { animation: g1kgIn 0.5s cubic-bezier(0.34,1.3,0.6,1) both; }
@keyframes g1kgIn { 0% { opacity: 0; transform: translateY(14px) scale(0.8); } 70% { transform: translateY(-2px) scale(1.04); } 100% { opacity: 1; transform: translateY(0) scale(1); } }
/* AddKgFig — massani qo'shish: "a + b" -> javobda "+" yig'ilib toshlar yashil ramkada birlashadi */
.g1-akg-box { display: inline-flex; align-items: flex-end; gap: clamp(6px, 2vw, 12px); padding: clamp(8px, 2vw, 12px) clamp(12px, 3vw, 18px); border-radius: 18px; border: 2.5px solid transparent; transition: border-color 0.4s ease, background 0.4s ease, gap 0.45s cubic-bezier(0.34,1.3,0.6,1); }
.g1-akg-grp { display: inline-flex; align-items: flex-end; gap: clamp(4px, 1.4vw, 8px); }
.g1-akg-op { align-self: center; font-family: 'JetBrains Mono', monospace; font-weight: 800; font-size: clamp(22px, 6vw, 34px); color: #8A8A90; max-width: 44px; overflow: hidden; transition: opacity 0.35s ease, max-width 0.45s ease, transform 0.35s ease, margin 0.45s ease; }
.g1-akg.is-solved .g1-akg-box { border-color: #3E9E52; background: rgba(74,180,90,0.12); gap: clamp(3px, 1.2vw, 7px); animation: g1alfJoin 0.5s cubic-bezier(0.34,1.3,0.6,1); }
.g1-akg.is-solved .g1-akg-op { opacity: 0; max-width: 0; margin: 0 -3px; transform: scale(0.4); }
.g1-akg.is-solved .g1-csp { animation: g1sparkPop 0.8s ease-out both; }
@keyframes g1alfJoin { 0% { transform: scale(0.94); } 60% { transform: scale(1.035); } 100% { transform: scale(1); } }
/* KgIntroDemo — s4: toshlar birma-bir ko'tarilib porlaydi (1, 2, 5) */
.g1-kgi { display: inline-flex; align-items: flex-end; transition: transform 0.4s cubic-bezier(0.34,1.3,0.6,1), filter 0.4s ease; }
.g1-kgi-hl { transform: translateY(-10px) scale(1.1); filter: drop-shadow(0 10px 16px rgba(255,79,40,0.4)); }
/* ambient — sahna hech qachon qotmaydi */
...
.g1-amb-sway { transform-box: fill-box; transform-origin: top center; animation: g1sway 4.8s ease-in-out infinite; }
...
@keyframes g1sway { 0%, 100% { transform: rotate(-3deg); } 50% { transform: rotate(3deg); } }
@media (prefers-reduced-motion: reduce) { .g1-bal-beam, .g1-bal-arm, .g1-bal-cele, .g1-bal-idle .g1-bal-beam, .g1-kg-gleam, .g1-bal-cele .g1-csp, .g1-kgset-cele .g1-csp, .g1-bal-win-l .g1-bal-arm-l .g1-bal-pan, .g1-bal-win-r .g1-bal-arm-r .g1-bal-pan, .g1-bal-ans, .g1-kg-ans, .g1-kg-in, .g1-amb-cloud, .g1-amb-sun, .g1-amb-rays, .g1-amb-sway, .g1-akg-box, .g1-akg-op, .g1-akg.is-solved .g1-akg-box, .g1-akg.is-solved .g1-csp, .g1-kgi { transition: none; animation: none; } .g1-akg.is-solved .g1-akg-op { max-width: 0; opacity: 0; } }

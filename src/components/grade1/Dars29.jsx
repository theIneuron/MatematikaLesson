...
// ============================================================================
// ░░ 1-SINF · Dars29 — "Задача на остаток и неизвестное слагаемое" (num-1-29-v1) · Б5 · spec: ETALON_1SINF.md ░░
// Baza: Dars28 (masala-kit — infra + ProblemViz/SolTile/OpTile/PQ/cast string-in-string).
// YADRO: matnli masala — qoldiqqa masala (olib qo'yilsa → ayirish) va noma'lum qo'shiluvchi (butundan ma'lum qismni ayirib topamiz); amalni MAZMUNDAN tanlash + xato topish.
// MEXANIKA: ProblemViz (ikki olma-guruh), RemoveRow (qoldiq — olinganlar xira), SolTile (yechim yozuvi), OpTile (+/−), PQ (savol+masala matni).
// Matn ovozда to'liq o'qiladi (1-sinf), ekranда qisqa tayanch. Typing YO'Q, son kichik (10 ichida).
// Misconception'lar: M1 qoldiqqa qo'shish tanladi (ayirish o'rniga) · M2 noma'lum qism uchun ikkala sonni qo'shdi · M3 javobda noto'g'ri son (off-by-one).
//
...
const LESSON_META = {
  lessonId: 'num-1-29-v1',
  lessonTitle: { ru: 'Задача на остаток и неизвестное слагаемое', uz: "Qoldiqqa va noma'lum qo'shiluvchiga masala" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: qoldiq/noma'lum qism
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: nechta qoldi
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // qoldiq: olib qo'yildi
  { id: 's2',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // olinsa -> ayiramiz
  { id: 's3',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qaysi amal (−)
  { id: 's4',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // noma'lum qism: qancha qo'shilgan
  { id: 's5',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // butundan ma'lum qismni ayiramiz
  { id: 's6',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // qoldiq yechim 8−3=5
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // noma'lum qism: 9−5=4
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // xato topish: 6−2=4
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: 3 qoldiq masala
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik -> Dars30
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: 9−4=5 + fakt
...
const CONTENT = {
  sIntro: {
    ...
    title: { ru: 'Когда что-то убрали', uz: 'Bir qismi olinganda' },
    body: {
      ru: 'В прошлый раз мы решали задачи на сумму. Сегодня — задачи, где что-то отдают, и где надо узнать, сколько добавили.',
      uz: "O'tgan safar yig'indiga masala yechdik. Bugun — bir qismi beriladigan va qancha qo'shilganini topadigan masalalar."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! В прошлый раз мы складывали.',
        'Сегодня задачи будут другие.',
        'Иногда часть отдают, и спрашивают, сколько осталось.',
        'А иногда надо узнать, сколько добавили. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! O'tgan safar qo'shgan edik.",
        "Bugun masalalar boshqacha bo'ladi.",
        "Ba'zan bir qismi beriladi va nechta qolgani so'raladi.",
        "Ba'zan esa qancha qo'shilganini bilish kerak. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: 'У Зухры было 7 яблок, 2 отдала. Какой тут', uz: 'Zuhrada 7 olma edi, 2 tasini berdi. Bu yerda qanday' },
    title_part2_em: { ru: 'вопрос', uz: 'savol' },
    title_part3: { ru: '?', uz: " bo'lishi mumkin?" },
    question: { ru: 'Как думаешь? Нажми ответ.', uz: "Qanday o'ylaysiz? Javobni bosing." },
    opt0: { ru: 'Сколько осталось?', uz: 'Nechta qoldi?' },
    opt1: { ru: 'Кто старше?', uz: 'Kim kattaroq?' },
    opt2: { ru: 'Трудно сказать', uz: 'Aytish qiyin' },
    audio: {
      intro: {
        ru: 'У Зухры было семь яблок. Два она отдала. Какой вопрос тут можно задать? Нажми, как думаешь.',
        uz: "Zuhrada yetti olma edi. Ikkitasini berdi. Bu yerda qanday savol berish mumkin? O'ylab, bosing."
      },
      on_correct: { ru: 'Хорошо. Спросим, сколько яблок осталось.', uz: "Yaxshi. Nechta olma qoldi, deb so'raymiz." },
      on_wrong: { ru: 'Хорошо. Спросим, сколько яблок осталось.', uz: "Yaxshi. Nechta olma qoldi, deb so'raymiz." }
    }
  },

  s1: {
    eyebrow: { ru: 'Условие и вопрос', uz: 'Shart va savol' },
    instruction: { ru: 'Часть яблок отдали. Найдём условие и вопрос', uz: "Olmalarning bir qismi berildi. Shart va savolni topamiz" },
    q_prompt: { ru: 'Какое предложение — вопрос? Нажми на него.', uz: "Qaysi gap — savol? Uni bosing." },
    audio_intro: { ru: 'В задаче два предложения. Одно говорит, что известно, — это условие. Другое спрашивает — это вопрос. Нажми на вопрос.', uz: "Masalada ikki gap bor. Biri nima ma'lumligini aytadi — bu shart. Ikkinchisi so'raydi — bu savol. Savolni bosing." },
    retry: { ru: 'Это условие — оно говорит, что известно. Вопрос о чём-то спрашивает.', uz: "Bu shart — u nima ma'lumligini aytadi. Savol nimadir so'raydi." },
    shart_label: { ru: 'Условие', uz: 'Shart' },
    savol_label: { ru: 'Вопрос', uz: 'Savol' },
    shart_text: { ru: 'Было 7 яблок, 2 отдали.', uz: "7 olma edi, 2 tasi berildi." },
    savol_text: { ru: 'Сколько яблок осталось?', uz: 'Nechta olma qoldi?' },
    reveal_label: { ru: 'Отдать 2 яблока', uz: 'Ikki olmani berish' },
    full_text: { ru: 'Часть отдали — осталось меньше.', uz: "Bir qismi berildi — kamrog'i qoldi." },
    full_audio: { ru: 'Два яблока отдали, и их стало меньше. Спрашивают, сколько осталось.', uz: "Ikki olma berildi va ular kamaydi. Nechta qolgani so'raladi." },
    audio: {
      ru: [
        'Послушай задачу. Было семь яблок, два отдали. Это условие — то, что известно.',
        'Сколько яблок осталось? Это вопрос — о чём спрашивают.'
      ],
      uz: [
        "Masalani tinglang. Yetti olma edi, ikkitasi berildi. Bu shart — nima ma'lum.",
        "Nechta olma qoldi? Bu savol — nima so'ralyapti."
      ]
    }
  },
  ...
  s2: {
    ...
    title_part1: { ru: 'Отдают, убирают —', uz: 'Beriladi, olinadi —' },
    title_part2_em: { ru: 'вычитаем', uz: 'ayiramiz' },
    title_part3: { ru: '', uz: '' },
    tip: {
      ru: 'Если часть отдают или убирают и спрашивают, сколько осталось — выбираем вычитание. Смотрим на смысл, а не только на слова.',
      uz: "Agar bir qismi berilsa yoki olinsa va nechta qolgani so'ralsa — ayirishni tanlaymiz. Mazmunga qaraymiz, faqat so'zga emas."
    },
    audio: {
      ru: 'Запомни. Если часть отдают или убирают и спрашивают сколько осталось, мы вычитаем. Думаем о смысле, а не только о словах.',
      uz: "Eslab qoling. Agar bir qismi berilsa yoki olinsa va nechta qolgani so'ralsa, biz ayiramiz. So'zga emas, mazmunga o'ylaymiz."
    }
  },

  s3: {
    eyebrow: { ru: 'Тренировка · 1 / 4', uz: 'Mashq · 1 / 4' },
    title: { ru: 'Какое действие подходит?', uz: 'Qaysi amal mos keladi?' },
    problem: { ru: 'У Анвара было 6 яблок, 2 отдал. Сколько осталось?', uz: "Anvarda 6 olma edi, 2 tasini berdi. Nechta qoldi?" },
    correct_text: { ru: 'Правильно. Часть отдали — значит минус.', uz: "To'g'ri. Bir qismi berildi — demak ayirish." },
    wrong_1: {
      ru: 'Здесь яблоки отдают, их становится меньше. Значит вычитаем, ставим минус.',
      uz: "Bu yerda olmalar beriladi, ular kamayadi. Demak ayiramiz, ayirish belgisi."
    },
    wrong_default: {
      ru: 'Часть убирают — выбираем минус.',
      uz: "Bir qismi olinsa — ayirishni tanlaymiz."
    },
    audio: {
      intro: { ru: 'У Анвара было шесть яблок, два он отдал. Какое действие подходит? Выбери.', uz: "Anvarda olti olma edi, ikkitasini berdi. Qaysi amal mos? Tanlang." },
      on_correct: { ru: 'Верно. Отдают — вычитаем, ставим минус.', uz: "To'g'ri. Beriladi — ayiramiz, ayirish belgisi." },
      on_wrong: { ru: 'Не совсем. Подумай, что делают с яблоками.', uz: "Unchalik emas. Olmalar bilan nima qilinayotganini o'ylang." }
    }
  },
  ...
  s4: {
    ...
    audio: {
      ru: [
        'Сначала было четыре яблока. Потом стало семь.',
        'Сколько яблок добавили? Это надо узнать.'
      ],
      uz: [
        "Avval to'rt olma edi. Keyin yetti bo'ldi.",
        "Nechta olma qo'shildi? Buni bilish kerak."
      ]
    }
  },
  ...
  s5: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Из целого убираем часть —', uz: 'Butundan qismni ayiramiz —' },
    title_part2_em: { ru: 'узнаём, сколько добавили', uz: "qancha qo'shilganini bilamiz" },
    title_part3: { ru: '', uz: '' },
    lab_whole: { ru: 'Стало', uz: "Bo'ldi" },
    lab_part: { ru: 'Было', uz: 'Edi' },
    tip: {
      ru: 'Чтобы узнать, сколько добавили, из целого убираем известную часть. Стало семь, было четыре, добавили три.',
      uz: "Qancha qo'shilganini bilish uchun butundan ma'lum qismni ayiramiz. Yetti bo'ldi, to'rt edi, uchta qo'shildi."
    },
    audio: {
      ru: 'Запомни. Чтобы узнать сколько добавили, из целого убираем известную часть. Стало семь, было четыре, добавили три.',
      uz: "Eslab qoling. Qancha qo'shilganini bilish uchun butundan ma'lum qismni ayiramiz. Yetti bo'ldi, to'rt edi, uchta qo'shildi."
    }
  },

  s6: {
    eyebrow: { ru: 'Тренировка · 2 / 4', uz: 'Mashq · 2 / 4' },
    title: { ru: 'Выбери верное решение', uz: "To'g'ri yechimni tanlang" },
    ...
    audio: {
      ...
      on_correct: { ru: 'Верно. Семь без двух пять. Осталось пять.', uz: "To'g'ri. Yettidan ikki besh. Besh qoldi." },
      on_wrong: { ru: 'Не совсем. Яблоки отдали — вычитаем.', uz: "Unchalik emas. Olmalar berildi — ayiramiz." }
    }
  },

  s7: {
    eyebrow: { ru: 'Тренировка · 3 / 4', uz: 'Mashq · 3 / 4' },
    title: { ru: 'Сколько добавили?', uz: "Qancha qo'shilgan?" },
    problem: { ru: 'Было 5 яблок, стало 9.', uz: "5 olma edi, 9 ta bo'ldi." },
    lab_was: { ru: 'Было', uz: 'Edi' },
  ...
  s8: {
    ...
    wrong_1: {
      ru: 'Тут сложили вместо вычитания. Яблоки отдали, значит надо вычитать. Шесть без двух — четыре.',
      uz: "Bu yerda ayirish o'rniga qo'shilgan. Olmalar berildi, demak ayirish kerak. Oltidan ikki — to'rt."
    },
    wrong_2: {
      ru: 'Шесть без двух — это четыре, не три. Верное решение — четыре.',
      uz: "Oltidan ikki — bu to'rt, uch emas. To'g'ri yechim — to'rt."
    },
    wrong_default: {
    ...
    audio: {
      ...
      on_correct: { ru: 'Верно. Шесть без двух четыре. Это правильное решение.', uz: "To'g'ri. Oltidan ikki to'rt. Bu to'g'ri yechim." },
      on_wrong: { ru: 'Не совсем. Яблоки отдали — надо вычитать.', uz: "Unchalik emas. Olmalar berildi — ayirish kerak." }
    }
  },
  ...
  sg: {
    ...
    audio: {
      intro: { ru: 'Поиграем с задачами. Я читаю задачу, а ты выбираешь, сколько осталось. Начинаем.', uz: "Masalalar bilan o'ynaymiz. Men masalani o'qiyman, siz nechta qolganini tanlaysiz. Boshladik." }
    }
  },

  sGuest: {
    eyebrow: { ru: 'Задача решена', uz: 'Masala yechildi' },
    title: { ru: 'Убрали и нашли', uz: 'Ayirib topdik' },
    body: {
      ru: 'Рано, Анвар и Зухра научились решать задачи на остаток и находить, сколько добавили. Везде помогает вычитание.',
      uz: "Ra'no, Anvar va Zuhra qoldiqqa masala yechishni va qancha qo'shilganini topishni o'rgandi. Hamma joyda ayirish yordam beradi."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, мы решили задачи на остаток.',
        'А ещё научились находить, сколько добавили. Везде помогает вычитание.'
      ],
      uz: [
        "Tinglang, biz qoldiqqa masalalar yechdik.",
        "Yana qancha qo'shilganini topishni o'rgandik. Hamma joyda ayirish yordam beradi."
      ]
    }
  },
  ...
  s9: {
    ...
    fact_text: {
      ru: 'Когда часть отдают и спрашивают сколько осталось — это вычитание.',
      uz: "Bir qismi berilsa va nechta qolgani so'ralsa — bu ayirish."
    },
    fact_audio: {
      ru: 'Когда часть отдают и спрашивают сколько осталось, это вычитание.',
      uz: "Bir qismi berilsa va nechta qolgani so'ralsa, bu ayirish."
    ...
    audio: {
      intro: { ru: 'Было шесть яблок, три отдали. Какое решение верное? Выбери.', uz: "Olti olma edi, uchtasi berildi. Qaysi yechim to'g'ri? Tanlang." },
      on_correct: { ru: 'Верно. Шесть без трёх три. Осталось три.', uz: "To'g'ri. Oltidan uch uch. Uch qoldi." },
      on_wrong: { ru: 'Не совсем. Яблоки отдали — вычитаем.', uz: "Unchalik emas. Olmalar berildi — ayiramiz." }
    }
  },
  ...
  s10: {
    ...
    main_2_em: { ru: 'решать задачи на остаток', uz: "qoldiqqa masala yecha olasiz" },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке будем решать составные задачи с таблицей.',
      uz: 'Keyingi darsda jadval bilan tarkibli masala yechamiz.'
    },
    audio: {
      ru: [
        'Молодец! Ты решал задачи на остаток и находил, сколько добавили.',
        'Когда что-то отдают — мы вычитаем. На следующем уроке решим составные задачи с таблицей. До встречи!'
      ],
      uz: [
        "Barakalla! Qoldiqqa masala yechdingiz va qancha qo'shilganini topdingiz.",
        "Bir narsa berilsa — biz ayiramiz. Keyingi darsda jadval bilan tarkibli masala yechamiz. Ko'rishguncha!"
      ]
    }
  }
};
  ...
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(4px, 1.4vw, 8px)' }}>
    <div className="g1-removerow" aria-hidden="true">
      {Array.from({ length: total }).map((_, i) => {
        ...
        return (
          <span key={i} className={`g1-rr-item ${isGone ? 'g1-rr-gone' : ''}`}
            style={isGone ? { transition: 'opacity 0.5s ease, transform 0.5s ease', ...(removed ? { opacity: 0, transform: 'translateY(-16px) scale(0.3)' } : {}) } : undefined}>
            <ObjSvg kind={kind}/>
          </span>
        );
      })}
    ...
    {showNums && (
      <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(6px, 2vw, 12px)' }}>
        <DigitGlyph d={total} size="sm" tone="ink"/>
        {gone > 0 && <span style={{ display: 'inline-flex', opacity: removed ? 0 : 0.4, transition: 'opacity 0.5s ease' }}><DigitGlyph d={gone} size="sm" tone="ink"/></span>}
      </div>
    )}
  </div>
);
...
// ProblemViz — masala vaziyati: ikki olma-guruh yonma-yon (amalni oldindan bermaydi).
const ProblemViz = ({ na, nb, kind = 'apple', anim = 'bob' }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(16px, 6vw, 48px)', flexWrap: 'wrap' }}>
    <span style={{ display: 'inline-flex', maxWidth: 'clamp(90px, 28vw, 160px)' }}><Pips n={na} kind={kind} wrap={true} anim={anim}/></span>
    <span style={{ display: 'inline-flex', maxWidth: 'clamp(90px, 28vw, 160px)' }}><Pips n={nb} kind={kind} wrap={true} anim={anim}/></span>
  </div>
);
...
// OpTile — amal belgisi varianti (+/−).
const OpTile = ({ op }) => (
  <span className="mono" style={{ ...D28_OP, fontSize: 'clamp(26px, 5vw, 38px)', color: '#0E0E10', minWidth: '1.4em', textAlign: 'center' }}>{op === 'add' ? '+' : '−'}</span>
);
...
// s0 — HOOK (soft): Zuhra 7, 2 berdi — savol nima? har javob OK.
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.6vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <RemoveRow total={7} gone={2} removed={picked !== null}/>
        </div>
        {!picked && (
...
// s1 — EXPLORATION (interaktiv): bola masaladagi SAVOLni o'zi topadi (shart vs savol).
const Screen1 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s1;
  const sfx = useSfx();
  const audio = useAudio([{ id: 's1_intro', text: c.audio_intro[lang], trigger: 'on_mount', waits_for: null }]);
  const canAct = useCanAnswer(audio);
  ...
  const tap = (which) => {
    ...
    if (which === 'savol') {
      ...
      setWrong((w) => w + 1); sfx.playWrong();
      if (!audio.muted) { const e = getAudioEngine(); if (e) e.pushOneOff(c.retry[lang]); }
    }
  };
  const navContent = (
    <>
  ...
  const card = (which, label, labelColor, text) => (
    <button onClick={() => tap(which)} disabled={!canAct || solved}
      className={`frame-soft ${wrong && which === 'shart' ? 'g1-nudge' : ''}`}
      style={{ textAlign: 'left', padding: 'clamp(10px, 2vw, 14px)', width: '100%', background: '#fff', cursor: solved ? 'default' : 'pointer', border: solved && which === 'savol' ? `2px solid ${T.success}` : '2px solid transparent' }}>
      {solved && <span className="eyebrow mono" style={{ color: labelColor }}>{which === 'savol' ? '✓ ' : ''}{label}</span>}
      <p style={{ margin: solved ? '4px 0 0' : 0, fontSize: 'clamp(14px, 2vw, 17px)', color: T.ink }}>{text}</p>
    </button>
  );
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <p className="h-sub title fade-up">{solved ? t(c.instruction) : t(c.q_prompt)}</p>
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(16px, 3vw, 24px)' }}>
          {card('shart', t(c.shart_label), T.blue, t(c.shart_text))}
        ...
        {!solved && wrong > 0 && (
          <FeedbackBlock show={true} isCorrect={false} wrongClass="frame-tip">
            <Reaction state="wrong" praise={t(c.retry)}/>
          </FeedbackBlock>
        )}
      </div>
    </Stage>
...
// s2 — RULE: olinsa/berilsa -> ayiramiz.
const Screen2 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s2;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2.6vw, 18px)', flexWrap: 'wrap', padding: 'clamp(18px, 3.4vw, 28px)' }}>
          <span className="g1-pop-in" style={{ display: 'inline-flex', animationDelay: '0.2s' }}><RemoveRow total={6} gone={2}/></span>
          <span className="g1-pop-in" style={{ display: 'inline-flex', animationDelay: '0.42s' }}><OpTile op="sub"/></span>
        </div>
...
// s3 — TEST MC: qaysi amal? − (idx0). Variantlar — OpTile −/+.
const Screen3 = (props) => {
  const c = CONTENT.s3;
  const t = useT();
  return (
    <QuestionScreen
      ...
      options={[<OpTile op="sub"/>, <OpTile op="add"/>]}
      correctIdx={0}
      mascot={false}
...
// s4 — EXPLORATION: noma'lum qism — qancha qo'shilgan (4 + ? = 7).
const Screen4 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s4;
...
// s5 — RULE: butundan ma'lum qismni ayiramiz -> qancha qo'shilgan.
const Screen5 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s5;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(8px, 2.2vw, 16px)', flexWrap: 'wrap', padding: 'clamp(18px, 3.4vw, 28px)' }}>
          <div className="g1-pop-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, animationDelay: '0.2s' }}>
            <span className="eyebrow mono" style={{ color: T.blue }}>{t(c.lab_whole)}</span>
            <span style={{ display: 'inline-flex', maxWidth: 'clamp(70px, 20vw, 110px)' }}><Pips n={7} kind="apple" wrap={true}/></span>
          </div>
          <span className="g1-pop-in" style={{ display: 'inline-flex', animationDelay: '0.36s' }}><OpTile op="sub"/></span>
          <div className="g1-pop-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, animationDelay: '0.52s' }}>
            <span className="eyebrow mono" style={{ color: T.accent }}>{t(c.lab_part)}</span>
            <span style={{ display: 'inline-flex', maxWidth: 'clamp(60px, 16vw, 90px)' }}><Pips n={4} kind="apple" wrap={true}/></span>
          </div>
          <span className="mono g1-pop-in" style={{ ...D28_OP, fontSize: 'clamp(16px, 3vw, 22px)', animationDelay: '0.68s' }}>=</span>
          <span className="g1-pop-in" style={{ display: 'inline-flex', maxWidth: 'clamp(50px, 14vw, 80px)', animationDelay: '0.84s' }}><Pips n={3} kind="apple" wrap={true}/></span>
        </div>
        <BitSays text={t(c.tip)}/>
      </div>
    </Stage>
...
// s6 — TEST MC: qoldiq yechim 7−2=5 (idx0). Variantlar — SolTile.
const Screen6 = (props) => {
  const c = CONTENT.s6;
  const t = useT();
  return (
...
// s7 — TEST MC: noma'lum qism 9−5=4 (idx0). Variantlar — DigitGlyph.
const Screen7 = (props) => {
  const c = CONTENT.s7;
  const t = useT();
  return (
      ...
      figure={(solved) => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 'clamp(12px, 4vw, 28px)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ display: 'inline-flex', maxWidth: 'clamp(70px, 22vw, 120px)' }}><Pips n={5} kind="apple" wrap={true}/></span>
          </div>
          <span className="mono" style={{ ...D28_OP, fontSize: 'clamp(22px, 4vw, 30px)', color: T.ink3 }}>→</span>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
...
// s8 — TEST MC (xato topish): to'g'ri yechim 6−2=4 (idx0). Variantlar — SolTile.
const Screen8 = (props) => {
  const c = CONTENT.s8;
  const t = useT();
  return (
...
// sg — MINI-O'YIN (3 raund, ballsiz): qoldiq masalalar (RemoveRow).
const PROB_GAME = [
  { na: 6, nb: 2, ans: 4, opts: [4, 8, 3], qk: 'q_round1' },
  { na: 9, nb: 4, ans: 5, opts: [5, 6, 4], qk: 'q_round2' },
  { na: 8, nb: 5, ans: 3, opts: [3, 2, 4], qk: 'q_round3' },
];
...
// s9 — TEST final + FactCard: 6−3=3 (idx0).
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  return (

...
// ============================================================================
// ░░ 1-SINF · Dars24 — "Сложение и вычитание круглых десятков (20+30, 80−30)" (num-1-24-v1) · Б9 · spec: ETALON_1SINF.md ░░
// Baza: Dars21 (BARQAROR — infra + AppleCrate/cast/saxnalar string-in-string). Dars15 EMAS (uni boshqa chat o'zgartiradi).
...
const LESSON_META = {
  lessonId: 'num-1-24-v1',
  lessonTitle: { ru: 'Сложение и вычитание круглых десятков', uz: "Yumaloq o'nliklarni qo'shish va ayirish" }
};
const SCREEN_META = [
  { id: 'sIntro', type: 'hook',        template: 'custom',   scored: false, scope: null },            // syujet: o'nliklab sanaymiz
  { id: 's0',  type: 'hook',        template: 'custom',   scored: false, scope: 'hook' },          // jumboq: 2+3 savat
  { id: 's1',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // qo'shish 20+30 (birlashtirish)
  { id: 's2',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // qo'shish qoidasi
  { id: 's3',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 20+30=50
  { id: 's4',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // ayirish 80-30 (olish)
  { id: 's5',  type: 'rule',        template: 'custom',   scored: false, scope: null },            // ayirish qoidasi
  { id: 's6',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 80-30=50
  { id: 's7',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 40+50=90
  { id: 's8',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'module-mikro' },  // 70-40=30
  { id: 'sg',  type: 'exploration', template: 'custom',   scored: false, scope: null },            // o'yin: aralash (3 raund)
  { id: 'sGuest', type: 'hook',     template: 'custom',   scored: false, scope: null },            // syujet ko'prik
  { id: 's9',  type: 'test',        template: 'MCScreen', scored: true,  scope: 'final' },         // final: 30+60=90 + fakt
...
const CONTENT = {
  sIntro: {
    ...
    title: { ru: 'Считаем десятками', uz: "O'nliklab sanaymiz" },
    body: {
      ru: 'Корзины с яблоками нужно объединять и раздавать. Если считать десятками, получится быстро. Научимся складывать и вычитать десятки.',
      uz: "Olmali savatlarni birlashtirib, ulashish kerak. O'nliklab sanasak, tez bo'ladi. O'nliklarni qo'shish va ayirishni o'rganamiz."
    },
    ...
    audio: {
      ru: [
        'Привет, друг! Урожай в корзинах, в каждой десять.',
        'Корзины нужно то объединять, то раздавать.',
        'Если считать десятками, всё получается быстро.',
        'Научимся складывать и вычитать десятки. Слушай до конца и нажимай кнопку дальше.'
      ],
      uz: [
        "Salom, do'stim! Hosil savatlarda, har birida o'nta.",
        "Savatlarni goh birlashtiramiz, goh ulashamiz.",
        "O'nliklab sanasak, hammasi tez chiqadi.",
        "O'nliklarni qo'shish va ayirishni o'rganamiz. Oxirigacha tinglang va davom tugmasini bosing."
      ]
    }
  },
  ...
  s0: {
    ...
    title_part1: { ru: 'Две корзины и ещё три. Сколько яблок', uz: "Ikki savat va yana uch. Nechta olma" },
    title_part2_em: { ru: 'вместе', uz: 'birga' },
    title_part3: { ru: '?', uz: '?' },
    ...
    audio: {
      intro: {
        ru: 'Две корзины и ещё три корзины. В каждой десять яблок. Сколько вместе? Нажми, как думаешь.',
        uz: "Ikki savat va yana uch savat. Har birida o'nta olma. Birga nechta? O'ylab, bosing."
      },
      on_correct: { ru: 'Хорошо. Посчитаем десятки и узнаем точно.', uz: "Yaxshi. O'nliklarni sanab, aniq bilamiz." },
      on_wrong: { ru: 'Хорошо. Посчитаем десятки и узнаем точно.', uz: "Yaxshi. O'nliklarni sanab, aniq bilamiz." }
    }
  },

  s1: {
    eyebrow: { ru: 'Складываем', uz: "Qo'shamiz" },
    instruction: { ru: 'Двадцать и тридцать. Соединим корзины', uz: "Yigirma va o'ttiz. Savatlarni birlashtiramiz" },
    full_text: { ru: 'Пять корзин — пятьдесят. 20 и 30 — это 50.', uz: "Besh savat — ellik. 20 va 30 — bu 50." },
    full_audio: { ru: 'Два десятка и ещё три десятка — это пять десятков. Двадцать и тридцать получается пятьдесят.', uz: "Ikki o'nlik va yana uch o'nlik — bu besh o'nlik. Yigirma va o'ttiz ellik bo'ladi." },
    audio: {
      ru: [
        'Слева две корзины, это двадцать. Справа три, это тридцать.',
        'Соединим их вместе. Два десятка и три десятка — пять десятков, пятьдесят.'
      ],
      uz: [
        "Chapda ikki savat, bu yigirma. O'ngda uch, bu o'ttiz.",
        "Ularni birlashtiramiz. Ikki o'nlik va uch o'nlik — besh o'nlik, ellik."
      ]
    }
  },
  ...
  s2: {
    ...
    title_part1: { ru: 'Складываем', uz: 'Faqat' },
    title_part2_em: { ru: 'десятки', uz: "o'nliklarni" },
    title_part3: { ru: '', uz: " qo'shamiz" },
    tip: {
      ru: 'Круглые десятки складываем как обычные числа, только считаем десятки. 2 десятка и 3 десятка — это 5 десятков, 50. Единиц ноль.',
      uz: "Yumaloq o'nliklarni oddiy sondek qo'shamiz, faqat o'nliklarni sanaymiz. 2 o'nlik va 3 o'nlik — bu 5 o'nlik, 50. Birlik nol."
    },
    audio: {
      ru: 'Чтобы сложить круглые десятки, считаем только десятки. Два и три — это пять десятков. Получается пятьдесят, единиц ноль.',
      uz: "Yumaloq o'nliklarni qo'shish uchun faqat o'nliklarni sanaymiz. Ikki va uch — bu besh o'nlik. Ellik bo'ladi, birlik nol."
    }
  },
  ...
  s4: {
    ...
    instruction: { ru: 'Восемьдесят. Уберём три корзины', uz: "Sakson. Uch savatni olamiz" },
    full_text: { ru: 'Осталось пять корзин — пятьдесят. 80 без 30 — это 50.', uz: "Besh savat qoldi — ellik. 80 dan 30 — bu 50." },
    full_audio: { ru: 'Восемь десятков без трёх десятков — это пять десятков. Осталось пятьдесят.', uz: "Sakkiz o'nlikdan uch o'nlik olsak — besh o'nlik. Ellik qoldi." },
    audio: {
      ru: [
        'Здесь восемь корзин, это восемьдесят.',
        'Уберём три корзины. Восемь десятков без трёх — пять десятков, пятьдесят.'
      ],
      uz: [
        "Bu yerda sakkiz savat, bu sakson.",
        "Uch savatni olamiz. Sakkiz o'nlikdan uch — besh o'nlik, ellik."
      ]
    }
  },
  ...
  s5: {
    eyebrow: { ru: 'Запомним', uz: 'Eslab qolamiz' },
    title_part1: { ru: 'Вычитаем', uz: "O'nlikdan" },
    title_part2_em: { ru: 'десятки', uz: "o'nlik" },
    title_part3: { ru: '', uz: ' ayiramiz' },
    tip: {
      ru: 'Круглые десятки вычитаем по десяткам. 8 десятков без 3 десятков — это 5 десятков, 50.',
      uz: "Yumaloq o'nliklarni o'nliklab ayiramiz. 8 o'nlikdan 3 o'nlik — bu 5 o'nlik, 50."
    },
    audio: {
      ru: 'Чтобы вычесть круглые десятки, считаем десятки. Восемь без трёх — это пять десятков. Получается пятьдесят.',
      uz: "Yumaloq o'nliklarni ayirish uchun o'nliklarni sanaymiz. Sakkiz dan uch — bu besh o'nlik. Ellik bo'ladi."
    }
  },
  ...
  sg: {
    ...
    instruction: { ru: 'Сосчитай по десяткам', uz: "O'nliklab sanang" },
    ...
    retry_audio: { ru: 'Ничего страшного. Сосчитай десятки.', uz: "Hech qisi yo'q. O'nliklarni sanang." },
    audio: {
      intro: { ru: 'Поиграем с десятками. Я показываю пример, а ты считаешь по десяткам. Начинаем.', uz: "O'nliklar bilan o'ynaymiz. Men misol ko'rsataman, siz o'nliklab sanaysiz. Boshladik." }
    }
  },

  sGuest: {
    eyebrow: { ru: 'Всё посчитано', uz: 'Hammasi sanaldi' },
    title: { ru: 'Десятками — быстро', uz: "O'nliklab — tez" },
    body: {
      ru: 'Рано, Анвар и Зухра объединяли и раздавали корзины, считая десятками. Получилось быстро и верно.',
      uz: "Ra'no, Anvar va Zuhra savatlarni o'nliklab sanab, birlashtirib va ulashdi. Tez va to'g'ri bo'ldi."
    },
    rano_label: { ru: 'Рано', uz: "Ra'no" },
    anvar_label: { ru: 'Анвар', uz: 'Anvar' },
    zuhra_label: { ru: 'Зухра', uz: 'Zuhra' },
    audio: {
      ru: [
        'Послушай, мы складывали и вычитали десятками.',
        'Так получилось намного быстрее. Десятки считать удобно.'
      ],
      uz: [
        "Tinglang, biz o'nliklab qo'shdik va ayirdik.",
        "Shunday ancha tez bo'ldi. O'nliklarni sanash qulay."
      ]
    }
  },
  ...
  s9: {
    ...
    wrong_default: {
      ...
      uz: "O'nliklarni sanang: uch va olti."
    },
    fact_badge: { ru: 'А знаешь? · Десятки', uz: "Bilasizmi? · O'nliklar" },
    fact_text: {
      ru: 'Деньги часто складывают круглыми десятками. Тридцать да двадцать посчитать так же легко, как три да два.',
      uz: "Pulni ko'pincha yumaloq o'nliklab qo'shishadi. O'ttiz va yigirma uch va ikkidek oson sanaladi."
    },
    fact_audio: {
      ru: 'А знаешь, деньги часто складывают круглыми десятками. Тридцать да двадцать так же легко, как три да два.',
  ...
  s10: {
    ...
    main_2_em: { ru: 'складывать и вычитать круглые десятки', uz: "yumaloq o'nliklarni qo'shib, ayira olasiz" },
    connections_title: { ru: 'Что дальше', uz: 'Keyin nima' },
    connections_text: {
      ru: 'На следующем уроке будем прибавлять к двузначному числу единицы.',
      uz: "Keyingi darsda ikki xonali songa birlik qo'shamiz."
    },
    audio: {
      ru: [
        'Молодец! Ты складывал и вычитал круглые десятки.',
        'Считать десятками быстро и удобно. На следующем уроке прибавим к двузначному числу единицы. До встречи!'
      ],
      uz: [
        "Barakalla! Yumaloq o'nliklarni qo'shdingiz va ayirdingiz.",
        "O'nliklab sanash tez va qulay. Keyingi darsda ikki xonali songa birlik qo'shamiz. Ko'rishguncha!"
      ]
    }
  }
};
...
// ============================================================
// SCREEN-KOMPONENTLAR — Dars24 (14 ekran)
// ============================================================

const D24_OP = { fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, color: '#5A5A60' };

// CrateRow — n ta o'nlik-savat qatori; faded -> oxirgi `faded` tasi xira (ayirishda olingan).
const CrateRow = ({ n, faded = 0 }) => (
  <span style={{ display: 'inline-flex', gap: 'clamp(4px, 1.2vw, 8px)', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'flex-end' }}>
    {Array.from({ length: n }).map((_, i) => (
      <span key={i} style={i >= n - faded ? { opacity: 0.26, transition: 'opacity 0.3s' } : { transition: 'opacity 0.3s' }}><AppleCrate/></span>
    ))}
  </span>
);

// ExprFig — yumaloq o'nliklar amali: savatlar + yozuv (a op b = ? / natija).
const ExprFig = ({ a, op, b, solved = false, result }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.6vw, 18px)' }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(6px, 2vw, 12px)', flexWrap: 'wrap' }}>
      {op === 'add' ? (
        ...
        <CrateRow n={a / 10} faded={solved ? b / 10 : 0}/>
      )}
    </div>
...
// s0 — HOOK (soft): 2 va 3 savat, nechta birga? har javob OK.
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
...
// s1 — EXPLORATION: qo'shish 20+30 (savatlarni birlashtirish).
const Screen1 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s1;
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 26px)' }}>
          {!done ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2.6vw, 18px)' }}>
              <CrateRow n={5}/>
              <span className="d4-numtile" style={{ color: T.accent }} aria-hidden="true">50</span>
            </div>
          )}
          {!done && (
...
// s2 — RULE: qo'shish (o'nliklarni sanaymiz).
const Screen2 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s2;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <ExprFig a={20} op="add" b={30} solved={true} result={50}/>
        </div>
...
// s4 — EXPLORATION: ayirish 80-30 (savatlarni olish).
const Screen4 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s4;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(16px, 3vw, 26px)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2.6vw, 18px)' }}>
            <CrateRow n={8} faded={done ? 3 : 0}/>
            {done && <span className="d4-numtile" style={{ color: T.accent }} aria-hidden="true">50</span>}
          </div>
          {!done && (
            <button className="btn" disabled={!canAct} onClick={remove}
              style={{ padding: 'clamp(10px, 1.6vw, 13px) clamp(20px, 3vw, 30px)', fontSize: 'clamp(14px, 1.8vw, 16px)' }}>
...
// s5 — RULE: ayirish (o'nliklarni sanaymiz).
const Screen5 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s5;
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(14px, 2.4vw, 18px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', justifyContent: 'center', padding: 'clamp(16px, 3vw, 24px)' }}>
          <ExprFig a={80} op="sub" b={30} solved={true} result={50}/>
        </div>
        <BitSays text={t(c.tip)}/>
      </div>
    </Stage>
...
// s7 — TEST MC: 40+50=90 (idx0).
// s7 — TEST (interaktiv): 40+50. Bola "O'nlik qo'sh" bilan beshta savat (o'nlik) qo'shadi (4 -> 9),
// o'nliklab sanaydi (40..90). "Tekshirish" bilan tasdiqlaydi (erta bossa — xato yo'li).
const Screen7 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s7;
  ...
  const canAct = useCanAnswer(audio);
  const BASE = 4, ADD = 5; // 40 + 50 (o'nliklarda)
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <AppleViz tens={tens} ones={0} pop={solved}/>
          <span className="mono" style={{ fontSize: 'clamp(14px, 1.9vw, 17px)', color: T.ink3 }}>{BASE * 10} + {ADD * 10}</span>
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(14px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <ExprFig key={ri} a={round.a} op={round.op} b={round.b} solved={solvedItem} result={round.ans}/>
          {!solvedItem && (
...
// s9 — TEST final + FactCard: 30+60=90 (idx0).
const Screen9 = (props) => {
  const c = CONTENT.s9;
  const t = useT();
  return (
    <QuestionScreen
      ...
      screenMeta={SCREEN_META[props.screen]} screenContent={c}
      question={<h2 className="title h-sub" style={{ textAlign: 'center' }}>{t(c.title)}</h2>}
      figure={(solved) => <ExprFig a={30} op="add" b={60} solved={solved} result={90}/>}
      options={[<DigitGlyph d={90} size="mid"/>, <DigitGlyph d={9} size="mid"/>, <DigitGlyph d={36} size="mid"/>, <DigitGlyph d={80} size="mid"/>]}
      correctIdx={0}
      mascot={false}
      factOnCorrect={(
...
// ============================================================
export default function RoundTensLesson({

// PK10_06 — ПК10 (Blok 11) · O'lchov birliklari · YANGI SHARTLI (amaliyotda yo'q).
...
const DATA = {
  topic: 'O\'lchov tenglik', kind: 'truefalse', expr: '1 dm = 100 sm', options: ['yes', 'no'], answer: 'no',
  optLabels: { yes: { uz: 'To\'g\'ri', ru: 'Верно' }, no: { uz: 'Noto\'g\'ri', ru: 'Неверно' } },
  eyebrow: { uz: 'Nazorat · O\'lchov', ru: 'Контроль · Измерение' },
  setup: { uz: 'Yozuvni tekshiring.', ru: 'Проверь запись.' },
  ask: { uz: 'Bu yozuv to\'g\'rimi?', ru: 'Верна ли эта запись?' },
  correct: { uz: 'Barakalla! Bir detsimetr — o\'n santimetr, yuz emas.', ru: 'Молодец! Один дециметр — десять сантиметров, а не сто.' },
  hint: { uz: '1 dm necha sm ekanini eslang.', ru: 'Вспомни, сколько см в 1 дм.' },
};

export default function PK10_06(props) {
  const data = DATA;

// Dars10 · Amaliyot 03 — P10 lift pastga «Yuk lifti» · 🟡 · tag: lift_down
// Vertikal son o'qi = yuk lifti. Kabina 9-qavatda, mount'da to'rt qavat pastga (9→5) tushadi.
...
const BADGES = Array.from({ length: TARGET }, (_, i) => i + 1); // 1..5 (qolgan qavatlar)

const T = {
  uz: {
    eyebrow: 'Fabrika · Yuk lifti', title: 'Pastga tushir',
    setup: 'Yuk lifti to\'qqizinchi qavatda turgan edi. U to\'rt qavat pastga tushdi.',
    ask: 'Lift qaysi qavatga tushdi?',
    correct: 'Barakalla! To\'qqizdan to\'rt pastga — besh. Pastga = ayirish!',
    hint: 'Qavatlarni sanang: to\'qqizdan boshlab pastga to\'rt qavat.',
  },
  ru: {
    eyebrow: 'Завод · Грузовой лифт', title: 'Опусти вниз',
    setup: 'Грузовой лифт стоял на девятом этаже. Он опустился на четыре этажа вниз.',
    ask: 'На какой этаж опустился лифт?',
    correct: 'Молодец! От девяти на четыре вниз — пять. Вниз — это вычитание!',
    hint: 'Считай этажи: от девяти вниз на четыре этажа.',
  },
};
...
export default function D10_03(props) {
  ...
  return (
    <div className="pq pq1003">
      <style>{`
        .pq1003{max-width:660px;margin:0 auto;padding:4px 2px 8px;font-family:'Manrope',system-ui,-apple-system,Segoe UI,Roboto,sans-serif;color:#1f2430;}
        .pq1003 .pq-eye{font-size:12px;font-weight:800;letter-spacing:.04em;color:#5a6b86;text-transform:uppercase;}
        .pq1003 .pq-body{font-size:17px;line-height:1.5;margin:4px 0 10px;}
        ...
        .pq1003 .pq-window::before{content:'';position:absolute;left:50%;top:4px;bottom:4px;width:2.5px;margin-left:-1.25px;background:#8f9aac;}
        .pq1003 .pq-window::after{content:'';position:absolute;top:50%;left:4px;right:4px;height:2.5px;margin-top:-1.25px;background:#8f9aac;}
        .pq1003 .pq-glow{position:absolute;top:38px;right:18px;width:30px;height:24px;background:rgba(255,255,255,.5);border-radius:4px;transform:skewX(-16deg);filter:blur(3px);z-index:1;animation:pqGlow 5s ease-in-out infinite;}
        .pq1003 .pq-pipe{position:absolute;top:12px;left:0;right:0;height:12px;background:linear-gradient(#aab4c2,#8894a5);border-bottom:2px solid #6f7a8b;z-index:1;box-shadow:inset 0 2px 0 rgba(255,255,255,.35);}
        .pq1003 .pq-valve{position:absolute;top:8px;left:120px;width:20px;height:20px;border-radius:50%;background:radial-gradient(circle at 40% 40%,#c7d0dc,#8894a5);border:2px solid #6f7a8b;z-index:2;animation:pqGear 14s linear infinite;}
        .pq1003 .pq-valve::before,.pq1003 .pq-valve::after{content:'';position:absolute;left:50%;top:50%;width:16px;height:3px;background:#6f7a8b;transform:translate(-50%,-50%);border-radius:2px;}
        .pq1003 .pq-valve::after{transform:translate(-50%,-50%) rotate(90deg);}
        .pq1003 .pq-steam{position:absolute;top:2px;left:126px;width:9px;height:9px;border-radius:50%;background:rgba(255,255,255,.8);opacity:0;z-index:2;animation:pqSteam 4.4s ease-out infinite;}
        .pq1003 .pq-steam.s2{left:133px;animation-delay:-2.2s;width:7px;height:7px;}
        .pq1003 .pq-gearw{position:absolute;line-height:0;z-index:1;filter:drop-shadow(0 1px 1px rgba(0,0,0,.12));}
        .pq1003 .pq-gear{transform-origin:50% 50%;animation:pqGear 9s linear infinite;}
        .pq1003 .pq-gear.g2{animation-duration:6.5s;animation-direction:reverse;}
        .pq1003 .pq-lamp{position:absolute;top:24px;width:3px;height:12px;background:#8894a5;z-index:2;}
        .pq1003 .pq-lamp::after{content:'';position:absolute;left:50%;top:12px;width:16px;height:9px;margin-left:-8px;border-radius:0 0 9px 9px;background:radial-gradient(circle at 50% 20%,#fff4c4,#f2c94c);box-shadow:0 0 8px 2px rgba(242,201,76,.55);animation:pqFlick 3.2s steps(1) infinite;}
        .pq1003 .pq-lamp.la{left:170px;} .pq1003 .pq-lamp.lb{left:214px;animation-delay:-1.1s;}
        .pq1003 .pq-lamp.lb::after{animation-delay:-1.6s;}

        /* --- Yuk lifti shaxtasi --- */
        .pq1003 .pq-shaft{position:absolute;left:42px;bottom:${SHAFT_BOTTOM}px;width:60px;height:236px;border-radius:6px;background:linear-gradient(#c4ccd8,#b3bccb);border:3px solid #7c8698;overflow:hidden;z-index:2;box-shadow:inset 0 0 8px rgba(0,0,0,.12);}
        .pq1003 .pq-rail{position:absolute;top:2px;bottom:2px;width:3px;background:repeating-linear-gradient(#8994a5 0 6px,#aab4c2 6px 12px);}
        .pq1003 .pq-rail.rl{left:5px;} .pq1003 .pq-rail.rr{right:5px;}
        .pq1003 .pq-tick{position:absolute;left:6px;right:6px;height:2px;background:rgba(90,102,124,.4);}
        .pq1003 .pq-tick.t5{background:rgba(37,99,235,.5);height:2.5px;}
        .pq1003 .pq-cabinw{position:absolute;left:2px;width:56px;bottom:${sfb(TARGET)}px;z-index:3;filter:drop-shadow(0 2px 3px rgba(0,0,0,.22));}
        .pq1003 .pq-cabinw.go{animation:pqDescend 1.05s cubic-bezier(.45,.05,.55,1) both;}
        .pq1003 .pq-cabinw.win .pq-cab{animation:pqCele .55s ease;}
        .pq1003 .pq-cable{position:absolute;left:50%;bottom:34px;width:2.4px;height:250px;margin-left:-1.2px;background:linear-gradient(#6f7a8b,#8894a5);}
        .pq1003 .pq-cargo{position:absolute;left:50%;top:-17px;transform:translateX(-50%);z-index:2;}

        /* --- Qavat raqamlari (shaxtadan chapda) --- */
        .pq1003 .pq-fl{position:absolute;left:12px;width:24px;height:18px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:#8994a5;font-variant-numeric:tabular-nums;z-index:2;}
        .pq1003 .pq-fl.tgt{color:#fff;border-radius:7px;width:22px;height:19px;box-shadow:0 2px 5px rgba(0,0,0,.18);}
        .pq1003 .pq-fl.q{background:linear-gradient(#5f6b86,#4a5568);animation:pqBreath 1.9s ease-in-out infinite;}
        .pq1003 .pq-fl.hit{background:linear-gradient(#2fbf6b,#1a9d52);animation:pqAns .5s cubic-bezier(.3,1.5,.5,1) both;}
        .pq1003 .pq-ground{position:absolute;left:8px;bottom:${scfb(0) - 11}px;font-size:9px;font-weight:800;color:#aab4c2;z-index:2;}

        /* --- Qavat-badge (g'alaba: qolgan 1..5) --- */
        .pq1003 .pq-cnt{position:absolute;left:106px;width:19px;height:19px;border-radius:50%;background:#2563eb;color:#fff;font-size:11px;font-weight:800;display:flex;align-items:center;justify-content:center;z-index:4;animation:pqPop .32s ease both;box-shadow:0 2px 4px rgba(37,99,235,.35);}

        /* --- Boshqaruv paneli + robot --- */
        .pq1003 .pq-panel{position:absolute;left:150px;bottom:96px;width:40px;height:60px;border-radius:8px;background:linear-gradient(#4a5568,#3a424f);border:2.5px solid #2b3140;z-index:3;display:flex;flex-direction:column;align-items:center;padding-top:6px;box-shadow:0 3px 6px rgba(0,0,0,.2);}
        .pq1003 .pq-pind{width:26px;height:22px;border-radius:5px;background:#0e1a24;color:#7fe0ff;font-size:16px;font-weight:900;display:flex;align-items:center;justify-content:center;font-variant-numeric:tabular-nums;box-shadow:inset 0 0 6px rgba(127,224,255,.4);}
        .pq1003 .pq-pind.win{color:#7dffa8;box-shadow:inset 0 0 6px rgba(125,255,168,.5);}
        .pq1003 .pq-pdown{margin-top:5px;font-size:15px;font-weight:900;color:#ff9d5c;line-height:1;animation:pqArrow 1.3s ease-in-out infinite;}
        .pq1003 .pq-pdown.win{color:#7dffa8;animation:none;}
        .pq1003 .pq-pbtn{margin-top:4px;width:9px;height:9px;border-radius:50%;background:#ef8a8a;box-shadow:0 0 5px rgba(239,138,138,.6);animation:pqFlick 2.4s steps(1) infinite;}
        .pq1003 .pq-robot{position:absolute;left:196px;bottom:26px;z-index:3;filter:drop-shadow(0 2px 3px rgba(0,0,0,.2));animation:pqBob 3s ease-in-out infinite;}
        .pq1003 .pq-ant{transform-box:fill-box;transform-origin:50% 50%;animation:pqAnt 1.5s ease-in-out infinite;}
        .pq1003 .pq-reye{transform-box:fill-box;transform-origin:50% 50%;animation:pqBlink 3.6s linear infinite;}
        .pq1003 .pq-led{animation:pqFlick 2.8s steps(1) infinite;}
        .pq1003 .pq-led.l2{animation-delay:-.9s;} .pq1003 .pq-led.l3{animation-delay:-1.7s;}

        /* --- Konveyer (pastda) --- */
        .pq1003 .pq-conv{position:absolute;left:0;right:0;bottom:0;height:24px;background:linear-gradient(#5c6672,#454e5a);border-top:2px solid #6f7a8b;z-index:2;}
        .pq1003 .pq-conv::before{content:'';position:absolute;left:0;right:0;top:3px;height:3px;background:repeating-linear-gradient(90deg,#7c8698 0 10px,#5c6672 10px 20px);}
        .pq1003 .pq-rollw{position:absolute;bottom:2px;line-height:0;z-index:3;}
        .pq1003 .pq-roller{transform-origin:50% 50%;animation:pqRoller 2.2s linear infinite;}
        .pq1003 .pq-convbox{position:absolute;bottom:19px;right:22px;z-index:3;filter:drop-shadow(0 1px 2px rgba(0,0,0,.2));}
        ...
        .pq1003 .pq-opt.sel{border-color:#2563eb;background:#e8eefc;}
        .pq1003 .pq-opt.right{border-color:#1a7f43;background:#e8f7ee;color:#1a7f43;animation:pqCele .5s ease;}
        .pq1003 .pq-opt:disabled{cursor:default;}
        .pq1003 .pq-fb{display:flex;align-items:flex-start;gap:10px;margin-top:16px;padding:14px 16px;border-radius:14px;font-size:16px;font-weight:700;line-height:1.45;animation:pqIn .22s ease both;}
        .pq1003 .pq-fb.ok{background:#e8f7ee;color:#1a7f43;} .pq1003 .pq-fb.no{background:#fdecec;color:#c0392b;}
        ...
        .pq1003 .pq-mote{position:absolute;z-index:0;width:5px;height:5px;border-radius:50%;background:radial-gradient(circle at 40% 40%,#fff,rgba(242,177,52,.5) 55%,rgba(242,177,52,0) 72%);pointer-events:none;opacity:0;animation:pq1003Mote 10.5s ease-in-out infinite;}
        .pq1003 .pq-mote.m2{width:4px;height:4px;animation-duration:12s;animation-delay:-4.5s;}
        .pq1003 .pq-mote.m3{width:6px;height:6px;animation-duration:14s;animation-delay:-8s;}
        @keyframes pq1003Mote{0%{opacity:0;transform:translate(0,8px) scale(.7);}22%{opacity:.5;}50%{opacity:.55;transform:translate(4px,-9px) scale(1);}80%{opacity:.3;}100%{opacity:0;transform:translate(0,-20px) scale(.7);}}
      `}</style>
      ...
      <div className="pq-stage" ref={fitRef}>
        ...
        <div className="pq-scene" style={{ transform: `scale(${scale})`, transformOrigin: 'top left' }}>
          ...
          <span className="pq-mote m3" aria-hidden="true" style={{ left: 306, top: 132 }} />
          {/* Fabrika foni */}

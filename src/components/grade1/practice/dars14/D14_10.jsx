...
// Chapda TAYYOR bir dasta (10, rezinkali) turadi. O'ngda/pastda sochilgan yakka qalamlar.
// Bola yakka qalamlarni birma-bir bosib dasta yoniga qo'shadi. Aynan uchta qo'shib — o'n uch.
// TEEN modeli: 11-15 = 1 DASTA (o'n) + N YAKKA. Dasta alohida, yakka alohida ko'rinadi.
...
// Sochilgan 6 yakka qalam — bola shulardan aynan uchtasini qo'shadi.
// Sahna px (kenglik 372), rot = qiyalik (turgan qalam), c = rang.
const LOOSE = [
  { x: 172, y: 186, r: -16, c: PAL[1] },
  { x: 214, y: 210, r: 14,  c: PAL[2] },
  { x: 256, y: 182, r: -8,  c: PAL[3] },
  { x: 298, y: 212, r: 20,  c: PAL[0] },
  { x: 210, y: 148, r: -22, c: PAL[3] },
  { x: 300, y: 152, r: 10,  c: PAL[1] },
];

const T = {
  uz: {
    eyebrow: "Qalam do'koni · Yasab ol", title: "O'n uchni yasab ol",
    setup: "Chapda tayyor bir dasta — o'nta qalam. O'ngda yakka qalamlar sochilgan.",
    ask: "O'n uch bo'lishi uchun dasta yoniga uchta yakka qalam qo'shing.",
    correct: "Barakalla! Bir dasta va uch yakka — o'n uch. O'zingiz yasadingiz!",
    hint: "Bir dasta o'nta. Yana uchta yakka qo'shsangiz — o'n uch bo'ladi.",
  ...
  ru: {
    eyebrow: 'Магазин карандашей · Собери', title: 'Сделай тринадцать',
    setup: 'Слева готовый пучок — десять карандашей. Справа рассыпаны отдельные карандаши.',
    ask: 'Чтобы получилось тринадцать, добавь к пучку три отдельных карандаша.',
    correct: 'Молодец! Один пучок и три отдельных — тринадцать. Ты сделал сам!',
    hint: 'В пучке десять. Добавь ещё три отдельных — будет тринадцать.',
...
export default function D14_10(props) {
  ...
  const [order, setOrder] = useState([]); // qo'shilgan yakka-qalam indekslari (tartib bilan)
  ...
  return (
    <div className="pq pq1410">
      <style>{`
        ...
        .pq1410 .pq-eye{font-size:12px;font-weight:800;letter-spacing:.04em;color:#c07a1e;text-transform:uppercase;}
        ...
        .pq1410 .pq-stage{display:flex;flex-direction:column;align-items:center;gap:11px;padding:10px 10px 13px;border-radius:22px;background:linear-gradient(#fbe9cf,#f4dcb4);border:2px solid #e9d3a6;}
        .pq1410 .pq-scene{position:relative;width:372px;max-width:100%;height:290px;border-radius:18px;background:linear-gradient(#fdf1d8 0%,#f7e2be 55%,#efd2a4 100%);border:2px solid #e6cfa0;overflow:hidden;}
        .pq1410 .pq-beam{position:absolute;top:-20px;right:44px;width:78px;height:220px;background:linear-gradient(180deg,rgba(255,241,196,.7),rgba(255,241,196,0));transform:rotate(15deg);transform-origin:top center;z-index:1;animation:pqBeam 4.5s ease-in-out infinite;pointer-events:none;}
        .pq1410 .pq-window{position:absolute;top:11px;right:13px;width:54px;height:42px;border-radius:6px;background:linear-gradient(135deg,#dff0fb 0 45%,#c2ddf0 45% 55%,#dff0fb 55%);border:2.5px solid #b58a4e;box-shadow:inset 0 0 0 1px rgba(255,255,255,.45);z-index:1;}
        .pq1410 .pq-window::after{content:'';position:absolute;left:50%;top:3px;bottom:3px;width:2px;background:#b58a4e;transform:translateX(-1px);}
        .pq1410 .pq-window::before{content:'';position:absolute;top:50%;left:3px;right:3px;height:2px;background:#b58a4e;transform:translateY(-1px);}
        .pq1410 .pq-sun{position:absolute;top:18px;left:16px;width:22px;height:22px;border-radius:50%;background:radial-gradient(circle at 40% 38%,#fff6ce,#f9c62f 72%,#f0ab18);box-shadow:0 0 14px 4px rgba(249,198,47,.5);z-index:1;animation:pqSun 3.6s ease-in-out infinite;}
        ...
        /* ish zonasi: [DASTA] + [qo'shish uyalari] — sanoq tagi ustida */
        .pq1410 .pq-work{position:absolute;top:78px;left:50%;transform:translateX(-50%);z-index:4;display:flex;align-items:flex-end;gap:10px;}
        .pq1410 .pq-work.win{animation:pqCele .6s ease;}
        /* tayyor DASTA — 10 qalam, qizil rezinka, qimirlamaydi */
        .pq1410 .pq-bundle{position:relative;display:flex;flex-direction:column;align-items:center;gap:5px;}
        .pq1410 .pq-brow{position:relative;display:flex;gap:1px;padding:7px 8px 8px;border-radius:11px;background:linear-gradient(#f4e2c1,#e9cfa0);border:2px solid #cfa96a;box-shadow:inset 0 2px 0 rgba(255,255,255,.35),0 3px 5px rgba(0,0,0,.12);}
        .pq1410 .pq-bpen{position:relative;width:13px;height:56px;display:block;}
        .pq1410 .pq-band{position:absolute;left:4px;right:4px;top:28px;height:11px;border-radius:3px;background:linear-gradient(#e2635b,#c8443c);border:1.5px solid #a5352e;box-shadow:0 1px 2px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.35);z-index:5;}
        ...
        /* plyus belgisi */
        .pq1410 .pq-plus{font-size:24px;font-weight:900;color:#a06a2e;padding-bottom:22px;opacity:.85;}
        /* qo'shish uyalari — 3 ta bo'sh joy, yakka qalam kelib tushadi */
        .pq1410 .pq-add{position:relative;display:flex;flex-direction:column;align-items:center;gap:5px;}
        .pq1410 .pq-arow{position:relative;display:flex;gap:4px;padding:7px 7px 8px;border-radius:11px;background:rgba(255,253,246,.4);border:2px dashed #cfa96a;}
        .pq1410 .pq-slot{position:relative;width:15px;height:56px;border-radius:3px;display:flex;align-items:flex-end;justify-content:center;box-sizing:border-box;}
        .pq1410 .pq-slot.empty{border:1.5px dashed #cdb488;background:rgba(255,253,246,.55);animation:pqBreath 2.4s ease-in-out infinite;}
        .pq1410 .pq-slot:nth-child(2).empty{animation-delay:-.6s;} .pq1410 .pq-slot:nth-child(3).empty{animation-delay:-1.2s;}
        .pq1410 .pq-apen{position:relative;width:13px;height:54px;display:block;}
        .pq1410 .pq-apen.pop{animation:pqFill .4s cubic-bezier(.3,1.4,.5,1) both;}
        .pq1410 .pq-atag{padding:1px 10px;border-radius:9px;background:#fff;color:#c07a1e;font-size:11.5px;font-weight:900;font-variant-numeric:tabular-nums;box-shadow:0 2px 5px rgba(0,0,0,.14);white-space:nowrap;}
        ...
        /* javon (peshtaxta) */
        .pq1410 .pq-shelf{position:absolute;left:0;right:0;bottom:0;height:30px;background:linear-gradient(#c98a4e,#a86c34 60%,#8f5a2a);border-top:3px solid #d9a463;z-index:1;}
        .pq1410 .pq-shelf::after{content:'';position:absolute;left:0;right:0;top:5px;height:2px;background:rgba(255,255,255,.22);}
        /* sochilgan yakka qalamlar — bosilganda qo'shish uyasiga o'tadi */
        .pq1410 .pq-loose{position:absolute;width:18px;height:54px;padding:0;border:none;background:none;cursor:pointer;line-height:0;z-index:3;transition:transform .16s,opacity .35s;filter:drop-shadow(0 2px 2px rgba(0,0,0,.18));}
        ...
        .pq1410 .pq-taphint{position:absolute;left:50%;bottom:6px;transform:translateX(-50%);z-index:5;font-size:12px;font-weight:800;color:#8a5a1e;background:rgba(255,255,255,.82);padding:3px 12px;border-radius:999px;white-space:nowrap;animation:pqBreathHint 2s ease-in-out infinite;}
      ...
      <div className="pq-stage">
        <div className={'pq-scene' + (still ? ' still' : '')}>
          ...
          <div className={'pq-work' + (ok ? ' win' : '')}>
            {/* TAYYOR DASTA — 10 qalam, doim bog'langan */}
          ...
          {/* sochilgan yakka qalamlar — bosilganda uyaga o'tadi */}
          {LOOSE.map((p, i) => {

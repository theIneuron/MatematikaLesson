// Dars14 · Amaliyot 06 — P13 Zanjir «Dasta va yakka» · 🔴 · tag: teen_bundle_chain
// 4 qator: har birida 1 DASTA (o'nlik, 10 qalam + qizil rezinka) va N ta YAKKA qalam.
// "1 dasta + N = ?" — teen soni tanlanadi (11..15). Bu darsning yuragi: o'n + birlik = teen.
// 14 = "1 va 4" EMAS, balki 1 dasta (o'n) va 4 yakka (birlik). Dasta va yakkalar ALOHIDA ko'rinadi.
...
const TEN = 10;
const ROWS = [
  { ones: 1, ans: 11, opts: [10, 11, 12] },
  { ones: 3, ans: 13, opts: [12, 13, 14] },
  { ones: 5, ans: 15, opts: [14, 15, 13] },
  { ones: 2, ans: 12, opts: [11, 12, 13] },
];
  ...
  return (
    <div className="pq pq1406">
      <style>{`
        ...
        .pq1406 .pq-eye{font-size:12px;font-weight:800;letter-spacing:.04em;color:#c07a1e;text-transform:uppercase;}
        ...
        .pq1406 .pq-scene{position:relative;width:372px;max-width:100%;height:96px;border-radius:18px;background:linear-gradient(#fdf1d8 0%,#f7e2be 55%,#efd2a4 100%);border:2px solid #e6cfa0;overflow:hidden;}
        .pq1406 .pq-beam{position:absolute;top:-20px;right:52px;width:70px;height:150px;background:linear-gradient(180deg,rgba(255,241,196,.7),rgba(255,241,196,0));transform:rotate(16deg);transform-origin:top center;z-index:1;animation:pqBeam 4.5s ease-in-out infinite;pointer-events:none;}
        .pq1406 .pq-window{position:absolute;top:12px;right:14px;width:56px;height:40px;border-radius:6px;background:linear-gradient(135deg,#dff0fb 0 45%,#c2ddf0 45% 55%,#dff0fb 55%);border:2.5px solid #b58a4e;box-shadow:inset 0 0 0 1px rgba(255,255,255,.45);z-index:1;}
        .pq1406 .pq-window::after{content:'';position:absolute;left:50%;top:3px;bottom:3px;width:2px;background:#b58a4e;transform:translateX(-1px);}
        .pq1406 .pq-window::before{content:'';position:absolute;top:50%;left:3px;right:3px;height:2px;background:#b58a4e;transform:translateY(-1px);}
        .pq1406 .pq-sun{position:absolute;top:18px;right:22px;width:20px;height:20px;border-radius:50%;background:radial-gradient(circle at 40% 38%,#fff6ce,#f9c62f 72%,#f0ab18);box-shadow:0 0 14px 4px rgba(249,198,47,.5);z-index:1;animation:pqSun 3.6s ease-in-out infinite;}
        .pq1406 .pq-lamp{position:absolute;left:44px;top:0;width:2px;height:16px;background:#8a5628;z-index:1;}
        .pq1406 .pq-lampshade{position:absolute;left:32px;top:14px;width:26px;height:12px;border-radius:0 0 40% 40%/0 0 100% 100%;background:linear-gradient(#f7d98a,#e0a83f);border:1.5px solid #b98235;z-index:1;box-shadow:0 9px 20px 5px rgba(255,213,110,.42);animation:pqLamp 3.2s ease-in-out infinite;}
        ...
        .pq1406 .pq-shelf{position:absolute;left:0;right:0;bottom:0;height:26px;background:linear-gradient(#c98a4e,#a86c34 60%,#8f5a2a);border-top:3px solid #d9a463;z-index:2;}
        .pq1406 .pq-shelf::after{content:'';position:absolute;left:0;right:0;top:5px;height:2px;background:rgba(255,255,255,.22);}
        .pq1406 .pq-pricetag{position:absolute;right:20px;bottom:32px;width:26px;height:16px;background:#f6c651;border:1.6px solid #cf9a2a;border-radius:3px 8px 8px 3px;z-index:3;transform:rotate(-8deg);transform-origin:left center;animation:pqTag 3s ease-in-out infinite;box-shadow:0 2px 3px rgba(0,0,0,.14);}
        .pq1406 .pq-pricetag::before{content:'';position:absolute;left:3px;top:5px;width:4px;height:4px;border-radius:50%;background:#fff;box-shadow:0 0 0 1px #cf9a2a;}
        .pq1406 .pq-pricetag::after{content:'';position:absolute;left:9px;top:6px;right:4px;height:2px;background:#cf9a2a;border-radius:2px;box-shadow:0 4px 0 #cf9a2a;}

        /* qatorlar */
        .pq1406 .pq-rows{display:flex;flex-direction:column;gap:7px;width:100%;}
        .pq1406 .pq-rw{display:flex;gap:7px;align-items:center;justify-content:center;flex-wrap:wrap;padding:7px 8px;border-radius:14px;border:2.5px solid #ecdcbf;background:#fffaf0;transition:.15s;}
        ...
        .pq1406 .pq-dasta{position:relative;display:flex;gap:1px;padding:2px 3px;filter:drop-shadow(0 2px 2px rgba(0,0,0,.18));}
        .pq1406 .pq-dpen{position:relative;width:7px;height:34px;flex:0 0 auto;}
        .pq1406 .pq-band{position:absolute;left:0;right:0;top:14px;height:8px;border-radius:3px;background:linear-gradient(#e2635b,#c8443c);border:1.4px solid #a5352e;box-shadow:0 1px 2px rgba(0,0,0,.22),inset 0 1px 0 rgba(255,255,255,.32);z-index:3;}
        .pq1406 .pq-dtag{position:absolute;top:-11px;left:50%;transform:translateX(-50%);z-index:4;padding:0 7px;border-radius:8px;background:#2f6f3a;color:#fff;font-size:11px;font-weight:900;line-height:15px;box-shadow:0 2px 3px rgba(0,0,0,.2);}

        /* YAKKA qalamlar (birlik) */
        .pq1406 .pq-singles{display:flex;gap:3px;align-items:flex-end;padding:0 2px;}
        .pq1406 .pq-single{position:relative;width:13px;height:44px;flex:0 0 auto;}
        .pq1406 .pq-cnt{position:absolute;top:-10px;left:50%;transform:translateX(-50%);min-width:16px;height:16px;padding:0 3px;border-radius:50%;background:#2563eb;color:#fff;font-size:9.5px;font-weight:800;display:flex;align-items:center;justify-content:center;box-shadow:0 1px 2px rgba(0,0,0,.25);animation:pqPop .3s ease both;z-index:5;font-variant-numeric:tabular-nums;}

        .pq1406 .pq-op{font-size:20px;font-weight:900;color:#c07a1e;}
        ...
        .pq1406 .pq-chip{margin-left:2px;padding:5px 11px;border-radius:11px;background:#e8f7ee;border:2px solid #1a7f43;color:#1a7f43;font-size:15px;font-weight:900;font-variant-numeric:tabular-nums;white-space:nowrap;animation:pqPop2 .35s ease both;}
        .pq1406 .pq-sgs{display:flex;gap:5px;margin-left:2px;}

// Dars36 · Amaliyot 07 — «Piktogramma va jadval» · Blok 7 ma'lumot · 🔴 · tag: total
// Piktogramma: 3 qator (Olma=4, Nok=6, Banan=3), har rasm = 1 dona. Savol: jami nechta meva? (4+6+3=13)
// Variantlar (son): '10' (qatorni tashlab ketish), '13' TO'G'RI (index 1, chapda emas), '12' (noto'g'ri sanash M1).
// VEDI-DO-VERNOGO: noto'g'rida qulf/retry yo'q; setChecked FAQAT to'g'rida; hint hamma qatorni qo'shishni o'rgatadi.
// ANSWER-LEAK: piktogramma + jadval = DATA (halol ko'rsatiladi); javob = bolaning qo'shishi; to'g'ri variant g'alabagacha yashil emas.
...
const OPTIONS = ['10', '13', '12']; // TO'G'RI '13' (index 1, chapda emas)
const CORRECT = '13';
const DATA = { rows: ROWS.map((r) => ({ key: r.key, n: r.n })), question: 'total', options: OPTIONS, correct: CORRECT, level: '🔴', tag: 'total' };

const T = {
  uz: {
    eyebrow: "Ma'lumot · Piktogramma", title: "Diagrammani o'qing",
    setup: "Har bir rasm — 1 dona meva.",
    ask: "Uchala qatorda jami nechta meva?",
    correct: "Barakalla! 4 va 6 va 3 — jami 13 ta meva.",
    hint: "Uchala qatordagi sonlarni qo'shing: 4 va 6 va 3.",
    head1: "Meva", head2: "Soni",
  },
  ru: {
    eyebrow: "Данные · Пиктограмма", title: "Прочитай диаграмму",
    setup: "Одна картинка — один фрукт.",
    ask: "Сколько всего фруктов в трёх рядах?",
    correct: "Молодец! 4 и 6 и 3 — всего 13 фруктов.",
    hint: "Сложи числа всех трёх рядов: 4 и 6 и 3.",
...
// Eng uzun qator (Nok=6): IX0 96 + 5*33 = 261 + ~9 radius = ~270 < 340 viewBox. 390px kartada sig'adi.
const LX = 8;        // yorliq x
...
export default function D36_07(props) {
  ...
  return (
    <div className={"pq pq3607" + (still ? " still" : "")}>
      <style>{`
        .pq3607.still *{animation:none !important;}
        ...
        .pq3607 .pq-tbl{width:100%;max-width:360px;margin:12px auto 0;border-collapse:collapse;font-size:14px;background:rgba(255,255,255,.9);border-radius:12px;overflow:hidden;}
        .pq3607 .pq-tbl th,.pq3607 .pq-tbl td{border:1px solid #e6d3a8;padding:7px 12px;text-align:left;color:#5a4a22;}
        .pq3607 .pq-tbl th{background:#f2e6c8;font-weight:800;font-size:12px;letter-spacing:.02em;text-transform:uppercase;}
        .pq3607 .pq-tbl td.num{text-align:right;font-weight:800;font-variant-numeric:tabular-nums;}
        .pq3607 .pq-opts{position:relative;z-index:3;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:16px;}
        ...
        .pq3607 .pq-opt:hover:not(:disabled){background:#fffaf0;border-color:#e6c976;}
        .pq3607 .pq-opt:active:not(:disabled){transform:scale(.98);}
        .pq3607 .pq-opt.sel{border-color:#2563eb;box-shadow:0 0 0 3px rgba(37,99,235,.18);color:#1f2430;}
        .pq3607 .pq-opt.right{border-color:#1a7f43;background:#e8f7ee;color:#1a7f43;box-shadow:0 0 0 3px rgba(26,127,67,.18);animation:pq3607cele .5s ease;}
        .pq3607 .pq-opt.dim{opacity:.4;filter:saturate(.6);}
        .pq3607 .pq-opt:disabled{cursor:default;}
        .pq3607 .pq-tick{position:absolute;top:-9px;right:-6px;z-index:7;width:24px;height:24px;border-radius:50%;background:#1a7f43;color:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 3px 7px rgba(0,0,0,.22);animation:pq3607pop .45s ease both;}
        .pq3607.still .pq-tick{animation:none;opacity:1;}
        .pq3607 .pq-spark{position:absolute;z-index:5;color:#ffc93f;opacity:0;line-height:0;pointer-events:none;animation:pq3607tw 1.7s ease-in-out infinite;filter:drop-shadow(0 0 3px rgba(255,201,63,.6));}
        .pq3607 .pq-spark.s2{animation-delay:-.6s;} .pq3607 .pq-spark.s3{animation-delay:-1.15s;}
        .pq3607.still .pq-spark{opacity:1;}
        .pq3607 .pq-fb{display:flex;align-items:flex-start;gap:10px;margin-top:16px;padding:14px 16px;border-radius:14px;font-size:16px;font-weight:700;line-height:1.45;animation:pq3607in .22s ease both;}
        .pq3607 .pq-fb.ok{background:#e8f7ee;color:#1a7f43;} .pq3607 .pq-fb.no{background:#fdecec;color:#c0392b;}
        @keyframes pq3607pop{from{opacity:0;transform:scale(.3);}to{opacity:1;transform:scale(1);}}
        @keyframes pq3607tw{0%,100%{opacity:0;transform:scale(.3) rotate(0);}50%{opacity:1;transform:scale(1.1) rotate(45deg);}}
        @keyframes pq3607cele{0%{transform:scale(1);}30%{transform:scale(1.04);}60%{transform:scale(.99);}100%{transform:scale(1);}}
      ...
      <div className="pq-board">
        ...
        {/* Piktogramma + jadval = DATA (halol ko'rsatiladi); javob = bolaning qo'shishi */}
        <div className="pq-card"><Pictogram lang={lang} /></div>
        <DataTable lang={lang} />

        {/* Sonli variantlar: to'g'ri (13) chapda emas; g'alabagacha yashil emas */}

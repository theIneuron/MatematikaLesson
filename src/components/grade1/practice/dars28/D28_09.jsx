// Dars28 · Amaliyot 09 — Yig'indiga masala (10 ichida) · 🔴 · tag: sum
// SODDA (metodist talabi 2026-07-15): bir xonali, 10 ichida, o'nlik/razryad YO'Q.
// Anvar 5 ta, Ra'no 4 ta olma terdi -> jami 9.
...
const A = 5, B = 4, TARGET = 9;
const DATA = { a: A, b: B, target: TARGET, options: [9, 8, 10], answer: TARGET, level: '🔴', tag: 'sum' };

const T = {
  uz: {
    eyebrow: "Olma bog'i · Masala", title: "Jami nechta?",
    setup: "Anvar 5 ta, Ra'no 4 ta olma terdi.",
    ask: "Jami nechta olma?",
    correct: "Barakalla! Jami — qo'shamiz. 5 + 4 = 9.",
    hint: "Jami — hammasini birga sanang. Qo'shamiz.",
  },
  ru: {
    eyebrow: "Яблоневый сад · Задача", title: "Сколько всего?",
    setup: "Анвар сорвал 5 яблок, Рано — 4.",
    ask: "Сколько всего яблок?",
    correct: "Молодец! Всего — складываем. 5 + 4 = 9.",
...
export default function D28_09(props) {
  ...
  return (
    <div className="pq pq2809" ref={fitRef}>
      <style>{`
        ...
        .pq2809 *,.pq2809 *::before,.pq2809 *::after{box-sizing:border-box;}
        .pq2809 .pq-eye{font-size:12px;font-weight:800;letter-spacing:.04em;color:#c14a3c;text-transform:uppercase;}
        .pq2809 .pq-body{font-size:17px;line-height:1.5;margin:4px 0 12px;}
        .pq2809 .pq-setup{color:#5c6672;font-weight:500;}
        .pq2809 .pq-ask{display:block;margin-top:4px;font-size:21px;font-weight:800;}
        .pq2809 .pq-scene{position:relative;width:340px;height:200px;border-radius:20px;background:linear-gradient(#cfeafc 0%,#e4f4d9 54%,#d3edb6 100%);border:2px solid #bfe0a8;overflow:hidden;}
        .pq2809 .pq-fit{position:relative;margin:0 auto;}
        .pq2809 .pq-sun{position:absolute;right:18px;top:14px;width:30px;height:30px;border-radius:50%;background:radial-gradient(circle at 38% 38%,#fff3c0,#f9c62f 70%,#f0ab18);box-shadow:0 0 18px 5px rgba(249,198,47,.5);z-index:1;animation:pq2809sun 3.6s ease-in-out infinite;}
        .pq2809 .pq-hill{position:absolute;left:0;right:0;bottom:0;height:48px;background:linear-gradient(#bfe39a,#a7d47f);border-top:3px solid #8fc267;z-index:1;}
        .pq2809 .pq-title{position:absolute;top:9px;left:50%;transform:translateX(-50%);z-index:6;padding:4px 15px 5px;border-radius:9px;background:linear-gradient(#c14a3c,#a2382c);border:2.5px solid #83271d;color:#fdeee9;font-size:12px;font-weight:800;white-space:nowrap;box-shadow:0 3px 6px rgba(0,0,0,.16);}
        .pq2809 .pq-arena{position:absolute;left:8px;right:8px;top:40px;bottom:10px;display:flex;align-items:center;justify-content:center;gap:8px;z-index:3;}
        .pq2809 .pq-group{display:flex;flex-direction:column;align-items:center;gap:5px;}
        .pq2809 .pq-apples{display:flex;flex-wrap:wrap;justify-content:center;gap:4px;max-width:120px;}
        .pq2809 .pq-apples.sum{max-width:220px;}
        .pq2809 .pq-obj{line-height:0;}
        .pq2809 .pq-obj.idle{animation:pq2809bob 2.9s ease-in-out infinite;animation-delay:var(--bd,0s);}
        .pq2809 .pq-obj.join{animation:pq2809join .42s ease both;animation-delay:var(--bd,0s);}
        .pq2809 .pq-tag{padding:1px 12px;border-radius:999px;background:#fff;border:2px solid #b6743c;color:#a2582a;font-weight:900;font-size:15px;box-shadow:0 2px 4px rgba(0,0,0,.14);}
        .pq2809 .pq-plus{font-size:30px;font-weight:900;color:#5c6672;align-self:center;}
        .pq2809 .pq-eq{display:flex;justify-content:center;align-items:center;gap:6px;margin-top:14px;flex-wrap:wrap;animation:pq2809in .3s ease both;}
        .pq2809 .pq-eq b{min-width:44px;height:40px;padding:0 8px;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900;border-radius:12px;background:#fdeee9;border:2px solid #eab5aa;color:#c14a3c;font-variant-numeric:tabular-nums;}
        .pq2809 .pq-eq b.res{background:#e8f7ee;border-color:#1a7f43;color:#1a7f43;}
        .pq2809 .pq-eq i{font-style:normal;font-size:22px;font-weight:900;color:#8a94a2;}
        .pq2809 .pq-opts{display:flex;flex-wrap:wrap;gap:12px;justify-content:center;margin-top:18px;}
        .pq2809 .pq-opt{min-width:78px;height:74px;padding:0 12px;font-size:32px;font-weight:800;border-radius:18px;border:2.5px solid #d6dae3;background:#fff;color:#374151;cursor:pointer;font-variant-numeric:tabular-nums;transition:.12s;}
        .pq2809 .pq-opt:hover:not(:disabled){border-color:#eab5aa;transform:translateY(-2px);}
        ...
        @keyframes pq2809join{from{opacity:0;transform:translateY(-8px) scale(.7);}to{opacity:1;transform:translateY(0) scale(1);}}

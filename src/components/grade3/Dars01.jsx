...
const BRIDGES = {
  s1:  { ru: 'Начнём с того, что тебе знакомо с первого класса.', uz: "Sizga birinchi sinfdan tanish narsadan boshlaymiz." },
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        {done && (
          <div ref={doneRef} className="frame-success lm-riseup">
            <Reaction state="correct" praise={scorePraise(firstOkRef.current, S8_TARGETS.length, lang)}/>
          </div>
        )}
      </div>
    </Stage>
          ...
          <>
            ...
            <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
              ...
              <div className="lm-signrow">
                {CMP_SIGNS.map((s) => (
                  <button key={s} className={`lm-signbtn mono ${wrongSet.has(s) ? 'lm-signbtn-wrong' : ''} ${picked === s ? 'lm-signbtn-ok' : ''}`} disabled={!canAct || solvedRound} onClick={() => pick(s)}>{s}</button>
                ))}
              </div>
    ...
    <Stage eyebrow={s12.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(8px, 1.6vw, 12px)', padding: 'clamp(10px, 2vw, 16px)' }}>
          ...
          {solved && <span className="mono lm-riseup" style={{ fontSize: 'clamp(26px, 6vw, 38px)', fontWeight: 800, color: T.success }}>{CASE_ANS}</span>}
        </div>
        {checked && (

// Dars 7 (3-sinf) · Amaliyot 08 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 14-bet 5-mashq — 476 + 378 (ikki marta o'tkazish).
...
function ColStack({ a, op, b, result = null }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      <div style={{ padding: '10px 18px', borderRadius: 14, background: '#FFFFFF', border: '1.5px solid ' + C.ribbonBd, boxShadow: 'inset 0 0 18px rgba(20,90,134,.10)' }}>
...
/* =================== 08 · Ikki marta o'tkazish (476 + 378) · 🔴 · col_add_carry2 =================== */
const D03_A = 476, D03_B = 378, D03_ANS = '854';
const D03_T = {
  uz: {
    eyebrow: "Ikki o'tkazish", setup: "Bu misolda o'tkazish IKKI marta bo'ladi: birlikda ham, o'nlikda ham.",
    ask: '476 + 378 = ? Javobni tering.',
    correct: "To'g'ri! Birliklar: 6 + 8 = 14 — 4 yoziladi, 1 dilda. O'nliklar: 7 + 7 + 1 = 15 — 5 yoziladi, 1 dilda. Yuzliklar: 4 + 3 + 1 = 8. Javob: 854.",
    wrong: "Maslahat: har xonada o'tkazishni tekshiring: 6 + 8 = 14 (1 dilda), keyin 7 + 7 + 1 = 15 (yana 1 dilda). Dildagilarni unutmang!",
    rule: "Har xonada 10 to'lsa — 1 keyingi xonaga o'tadi; o'tkazish bir necha marta bo'lishi mumkin.",
  },
  ru: {
    eyebrow: 'Два перехода', setup: 'В этом примере переход будет ДВА раза: и в единицах, и в десятках.',
    ask: '476 + 378 = ? Набери ответ.',
    correct: 'Верно! Единицы: 6 + 8 = 14 — пишем 4, 1 в уме. Десятки: 7 + 7 + 1 = 15 — пишем 5, 1 в уме. Сотни: 4 + 3 + 1 = 8. Ответ: 854.',
    wrong: 'Подсказка: проверяй переход в каждом разряде: 6 + 8 = 14 (1 в уме), потом 7 + 7 + 1 = 15 (снова 1 в уме). Не теряй то, что в уме!',
    rule: 'Когда в разряде набирается 10 — единица переходит дальше; переход может быть несколько раз.',
  },
};
function D07_08Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D03_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'col_add_carry2', level: '🔴' } });

...
function D07_09Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D09_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'case_two_step', level: '🔴' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
          {t.rows.map(([lbl, num]) => (
            <div key={lbl} style={{ minWidth: 96, padding: '10px 12px', borderRadius: 12, background: '#FFFFFF', border: '1.5px solid ' + C.ribbonBd, textAlign: 'center' }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: C.sink2, textTransform: 'uppercase', marginBottom: 4 }}>{lbl}</div>
              <div style={{ ...S.mono, fontSize: 28, fontWeight: 800, color: C.glow, textShadow: '0 0 10px rgba(20,90,134,.16)' }}>{num}</div>
            </div>
          ))}
        </div>
      </Stage>

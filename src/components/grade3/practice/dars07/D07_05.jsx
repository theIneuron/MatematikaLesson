// Dars 7 (3-sinf) · Amaliyot 05 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 12-bet 5-mashq — 436 + 345 (birlikdan o'tkazish bilan).
...
/* =================== 05 · O'tkazish bilan (436 + 345) · 🟡 · col_add_carry =================== */
const D03_A = 436, D03_B = 345, D03_ANS = '781';
const D03_T = {
  uz: {
    eyebrow: "O'tkazish bilan", setup: "Diqqat: birliklarni qo'shganda o'nlikdan oshib ketadi — o'tkazish kerak.",
    ask: '436 + 345 = ? Javobni tering.',
    correct: "To'g'ri! Birliklar: 6 + 5 = 11 — 1 ni yozamiz, 1 o'nlik dilda. O'nliklar: 3 + 4 + 1 = 8. Yuzliklar: 4 + 3 = 7. Javob: 781.",
    wrong: "Maslahat: birliklar 6 + 5 = 11 — bu bir xonaga sig'maydi: 1 ni yozib, 1 o'nlikni DILDA saqlang va o'nliklarga qo'shing.",
    rule: "O'n birlik to'lsa, 1 o'nlik dilda saqlanadi va keyingi xonaga qo'shiladi.",
  },
  ru: {
    eyebrow: 'Сложение с переходом', setup: 'Внимание: при сложении единиц получится больше десяти — нужен переход.',
    ask: '436 + 345 = ? Набери ответ.',
    correct: 'Верно! Единицы: 6 + 5 = 11 — пишем 1, один десяток в уме. Десятки: 3 + 4 + 1 = 8. Сотни: 4 + 3 = 7. Ответ: 781.',
    wrong: 'Подсказка: единицы 6 + 5 = 11 — это не помещается в один разряд: пиши 1, а один десяток держи В УМЕ и прибавь к десяткам.',
    rule: 'Когда десять единиц набралось, один десяток держим в уме и прибавляем к следующему разряду.',
  },
};
function D07_05Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D03_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'col_add_carry', level: '🟡' } });

...
// Nazariy Dars04 tuzoq-jufti: 519 va 591 — raqamlari bir xil, joylari har xil.
...
function SignBoard({ a, b, sign, setSign, locked, checked, correctSign, fbOk }) {
  const cell = (txt) => (
    <div style={{ minWidth: 96, height: 70, borderRadius: 12, background: '#FFFFFF', border: '1.5px solid ' + C.ribbonBd, boxShadow: 'inset 0 0 14px rgba(20,90,134,.11)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 10px' }}>
      <span style={{ ...S.mono, fontSize: 32, fontWeight: 800, color: C.glow, textShadow: '0 0 12px rgba(20,90,134,.18)' }}>{txt}</span>
    </div>
  );
...
/* =================== 08 · Tuzoq-juft (519 va 591) · 🔴 · compare_trap =================== */
const D01_A = '519', D01_B = '591', D01_SIGN = '<';
const D01_T = {
  uz: {
    eyebrow: "Belgini qo'ying", setup: "Diqqat: ikkala sonda ham 5, 9 va 1 raqamlari bor. Lekin joylari har xil!",
    ask: "519 va 591 orasiga qaysi belgi qo'yiladi?",
    correct: "To'g'ri! 519 < 591: yuzliklar teng (5 = 5), o'nliklar hal qiladi — 1 o'nlik 9 o'nlikdan kichik.",
    wrong: "Maslahat: yuzliklar teng bo'lsa, keyingi razryadga o'ting: o'nliklarni solishtiring. Raqam bir xil bo'lsa ham, JOYI qiymatni belgilaydi.",
    rule: "Yuzliklar teng bo'lsa, o'nliklar hal qiladi: 519 < 591.",
  },
  ru: {
    eyebrow: 'Поставь знак', setup: 'Внимание: в обоих числах есть цифры 5, 9 и 1. Но места у них разные!',
    ask: 'Какой знак ставится между 519 и 591?',
    correct: 'Верно! 519 < 591: сотни равны (5 = 5), решают десятки — 1 десяток меньше 9 десятков.',
    wrong: 'Подсказка: если сотни равны, переходи к следующему разряду: сравни десятки. Одинаковые цифры на разных местах имеют разное значение.',
    rule: 'Если сотни равны, решают десятки: 519 < 591.',
  },
};
function D04_08Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: ['>', '<', '='].map((s, i) => ({ id: String(i), label: s })), studentAnswer: { sign }, correctAnswer: { sign: D01_SIGN }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'compare_trap', level: '🔴' } });

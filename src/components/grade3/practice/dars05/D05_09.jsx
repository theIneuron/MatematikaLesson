// Dars 5 (3-sinf) · Amaliyot 09 — mustaqil topshiriq fayli (grade2 naqshi).
// Ikki qadam: 348 o'nlikka 350 bo'ldi (2-topshiriqda). Endi YUZLIKKA: 300 (48 < 50). Farqni his qilish.
...
/* =================== 09 · Ikki qadam (348) · 🔴 · round_two_step =================== */
const D02_ANS = '300';
const D02_T = {
  uz: {
    eyebrow: 'Ikki qadam', setup: "348 ni yumaloq O'NLIKKA olganda 350 chiqqan edi. Endi xuddi shu 348 ni yumaloq YUZLIKKA olamiz — javob boshqacha chiqadi!",
    ask: "348 ga eng yaqin yumaloq YUZLIKNI raqam-plitada tering.",
    expr: '348 → ?00',
    correct: "To'g'ri! 348 → 300: yuzlik uchun o'nliklarga qaraymiz — 48 < 50. O'nlikka 350, yuzlikka esa 300!",
    wrong: "Maslahat: endi o'nlik emas, YUZLIK kerak: 348 chiziqda 300 bilan 400 orasida. Yarmi — 350. 348 undan oldinmi?",
    rule: "Nishon muhim: o'nlikka — oxirgi raqamga, yuzlikka — o'nliklarga qaraymiz. 348 → 350 (o'nlik), 348 → 300 (yuzlik).",
  },
  ru: {
    eyebrow: 'Два шага', setup: 'Когда 348 брали к круглому ДЕСЯТКУ, получилось 350. Теперь возьмём то же 348 к круглой СОТНЕ — ответ будет другим!',
    ask: 'Набери на панели ближайшую круглую СОТНЮ к 348.',
    expr: '348 → ?00',
    correct: 'Верно! 348 → 300: для сотни смотрим на десятки — 48 < 50. К десятку 350, а к сотне — 300!',
    wrong: 'Подсказка: теперь нужен не десяток, а СОТНЯ: 348 между 300 и 400. Середина — 350. 348 до неё?',
    rule: 'Важна цель: к десятку — смотрим на последнюю цифру, к сотне — на десятки. 348 → 350 (десяток), 348 → 300 (сотня).',
  },
};
function D05_09Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D02_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'round_two_step', level: '🔴' } });

...
// Sonlar darslik poolidan: 348 (79-bet). Yumaloq YUZLIK tanlash — chiziq 300..400.
...
function RoundLine({ lo, hi, value, picked, setPicked, locked, checked, correctIdx, fbOk }) {
  ...
  const endStyle = (i) => {
    const on = picked === i;
    let fill = '#FFFFFF', stroke = C.ribbonBd;
    if (on) { stroke = C.acc; fill = '#E9F3FB'; }
...
/* =================== 04 · Eng yaqin yumaloq yuzlik (348) · 🟡 · round_hundred_line =================== */
// 348 chiziqda 300 bilan 400 orasida; 300 ga yaqinroq (48 < 50).
const D01_LO = 300, D01_HI = 400, D01_VAL = 348, D01_CORRECT = 0;
const D01_T = {
  uz: {
    eyebrow: 'Yumaloq yuzlik', setup: "Endi yumaloq YUZLIK kerak: 348 soni chiziqda 300 bilan 400 orasida turibdi.",
    ask: "348 ga eng yaqin yumaloq yuzlik qaysi? Uni chiziqda bosing.",
    correct: "To'g'ri! 348 → 300: 48 < 50, demak nuqta hali yo'lning yarmiga yetmagan — 300 yaqinroq.",
    wrong: "Maslahat: yo'lning yarmi — 350. 348 undan oldinmi yoki keyinmi? Nuqta qaysi yuzlikka yaqin?",
    rule: "Yumaloq yuzlikni tanlashda o'nliklarga qaraymiz: 48 < 50 → 348 → 300.",
  },
  ru: {
    eyebrow: 'Круглая сотня', setup: 'Теперь нужна круглая СОТНЯ: число 348 стоит на линии между 300 и 400.',
    ask: 'Какая круглая сотня ближе к 348? Нажми её на линии.',
    correct: 'Верно! 348 → 300: 48 < 50, точка ещё не дошла до середины пути — ближе 300.',
    wrong: 'Подсказка: середина пути — 350. 348 до неё или после? К какой сотне точка ближе?',
    rule: 'Для круглой сотни смотрим на десятки: 48 < 50 → 348 → 300.',
  },
};
function D05_04Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [String(D01_LO), String(D01_HI)].map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: picked === 0 ? String(D01_LO) : String(D01_HI) }, correctAnswer: { idx: D01_CORRECT, label: String(D01_HI) }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'round_hundred_line', level: '🟡' } });

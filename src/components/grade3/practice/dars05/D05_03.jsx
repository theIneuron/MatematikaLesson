// Dars 5 (3-sinf) · Amaliyot 03 — mustaqil topshiriq fayli (grade2 naqshi).
// Sonlar darslik poolidan: 267, 854, 523. Har biriga eng yaqin yumaloq o'nlik.
// Mexanika: moslash — son ↔ eng yaqin yumaloq o'nlik. Hammasi-yoki-hech.
...
/* =================== 03 · Moslashtiring (son ↔ yumaloq o'nlik) · 🟢 · match_round =================== */
const D03_TAG = 'match_round', D03_LEVEL = '🟢';
const D03_NUMS = ['267', '854', '523'];
const D03_EXP = { '267': '270', '854': '850', '523': '520' };
const D03_T = {
  uz: {
    eyebrow: 'Moslashtiring', setup: "Har sonni unga eng yaqin yumaloq o'nlikka ulang.",
    ask: "Sonni bosing, keyin eng yaqin yumaloq o'nlikni bosing.",
    correct: "To'g'ri! 267 → 270 (7 > 5), 854 → 850 (4 < 5), 523 → 520 (3 < 5).",
    wrong: "Maslahat: har sonning OXIRGI raqamiga qarang: 5 dan kichik bo'lsa — orqadagi yumaloq o'nlik, 5 yoki katta bo'lsa — keyingisi.",
    rule: "Yumaloq o'nlikni oxirgi raqam hal qiladi: 5 dan kichik — pastga, 5 va katta — yuqoriga.",
  },
  ru: {
    eyebrow: 'Соедини пары', setup: 'Соедини каждое число с ближайшим к нему круглым десятком.',
    ask: 'Нажми число, потом ближайший круглый десяток.',
    correct: 'Верно! 267 → 270 (7 > 5), 854 → 850 (4 < 5), 523 → 520 (3 < 5).',
    wrong: 'Подсказка: смотри на ПОСЛЕДНЮЮ цифру каждого числа: меньше 5 — круглый десяток позади, 5 и больше — следующий.',
...
function D05_03Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { map }, correctAnswer: { map: Object.fromEntries(D03_NUMS.map((_, i) => [i, i])) }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: D03_TAG, level: D03_LEVEL } });

// Dars 2 (3-sinf) · Amaliyot 01 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 11-bet 8-mashq — «Yulduzcha» ko'rik-tanlovidagi 340 soni (shart moslab qayta yozilgan).
...
/* =================== 01 · Sonni o'qing (340) · 🟢 · read_num =================== */
const D01_TAG = 'read_num', D01_LEVEL = '🟢', D01_CORRECT = 0;
const D01_T = {
  uz: {
    eyebrow: "Sonni o'qing", setup: "«Yulduzcha» ko'rik-tanlovida 340 nafar ishtirokchi qatnashdi. Displeyda shu son turibdi.",
    ask: "340 soni qanday o'qiladi?",
    opts: ['uch yuz qirq', "uch yuz to'rt", "o'ttiz to'rt"],
    correct: "To'g'ri! 340 = 3 yuzlik va 4 o'nlik: uch yuz qirq.",
    wrong: "Maslahat: avval yuzliklarni o'qing, keyin o'nliklarni. Oxiridagi 0 birlik yo'qligini bildiradi, u alohida o'qilmaydi.",
    rule: "Son razryadlar bo'yicha o'qiladi: 340 → uch yuz (300) qirq (40).",
  },
  ru: {
    eyebrow: 'Прочитай число', setup: 'В конкурсе «Юлдузча» участвовали 340 человек. Это число на дисплее.',
    ask: 'Как читается число 340?',
    opts: ['триста сорок', 'триста четыре', 'тридцать четыре'],
    correct: 'Верно! 340 = 3 сотни и 4 десятка: триста сорок.',
    wrong: 'Подсказка: сначала читай сотни, потом десятки. Ноль в конце показывает, что единиц нет, — отдельно он не читается.',
    rule: 'Число читается по разрядам: 340 → триста (300) сорок (40).',
  },
};
const D01_ORDER = permFromSeed(3, D01_TAG);
function D02_01Impl(props) {
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage><Neon text="340" /></Stage>
      <p style={S.ask}>{t.ask}</p>
      <div>
        {D01_ORDER.map((i) => (
          <button key={i} type="button" style={optStyle(picked, i, D01_CORRECT, checked, isReview, { fs: 18 })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>
        ))}
      </div>

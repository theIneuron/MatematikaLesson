// Dars 2 (3-sinf) · Amaliyot 02 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 5-bet 8-mashq — Afrosiyob poyezdidagi 215 soni (shart moslab qayta yozilgan).
...
// So'z-karta: displeyda son so'z bilan yozilgan
const WordCard = ({ text }) => (
...
/* =================== 02 · So'zdan songa (215) · 🟢 · word_to_num =================== */
// Chalg'ituvchilar: 205 (o'nlik tushib qolgan), 2105 (razryadlar yonma-yon yopishtirilgan).
const D02_TAG = 'word_to_num', D02_LEVEL = '🟢', D02_CORRECT = 0;
const D02_T = {
  uz: {
    eyebrow: "So'zdan songa", setup: "Afrosiyob tezyurar poyezdi bir vaqtda ikki yuz o'n besh nafar yo'lovchini tashiy oladi.",
    ask: "Bu son raqamlar bilan qanday yoziladi?",
    word: "ikki yuz o'n besh", opts: ['215', '205', '2105'],
    correct: "To'g'ri! Ikki yuz o'n besh = 215: 2 yuzlik, 1 o'nlik, 5 birlik.",
    wrong: "Maslahat: har razryad O'Z katagiga yoziladi — yuzlik, o'nlik, birlik. Sonlar yonma-yon yopishtirilmaydi.",
    rule: "Ikki yuz (200) + o'n (10) + besh (5) = 215 — uch xonali son.",
  },
  ru: {
    eyebrow: 'От слов к числу', setup: 'Скоростной поезд Афросиёб перевозит за раз двести пятнадцать пассажиров.',
    ask: 'Как это число записывается цифрами?',
    word: 'двести пятнадцать', opts: ['215', '205', '2105'],
    correct: 'Верно! Двести пятнадцать = 215: 2 сотни, 1 десяток, 5 единиц.',
    wrong: 'Подсказка: каждый разряд пишется в СВОЮ клетку — сотни, десятки, единицы. Числа не склеиваются рядом.',
    rule: 'Двести (200) + десять (10) + пять (5) = 215 — трёхзначное число.',
  },
};
const D02_ORDER = (shuffleSeed) => {
  const base = permFromSeed(3, String(D02_TAG));
...
function D02_02Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: D02_CORRECT, label: t.opts[D02_CORRECT] }, correct, meta: { tag: D02_TAG, level: D02_LEVEL } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
        {D02_ORDER(shuffleSeed).map((i) => (
          <button key={i} type="button" style={optStyle(picked, i, D02_CORRECT, checked, isReview, { half: true, center: true, fs: 24, mono: true })} disabled={isReview || checked} onClick={() => setPicked(i)}>{t.opts[i]}</button>
        ))}
      </div>

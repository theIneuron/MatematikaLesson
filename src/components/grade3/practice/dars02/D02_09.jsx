// Dars 2 (3-sinf) · Amaliyot 09 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 15-bet kombinatorika 1-mashq — 9, 0, 4 raqamlaridan; eng kichigi 409.
// Tuzoq: 049 yozib bo'lmaydi — nol birinchi o'rinda tursa, son uch xonali bo'lmay qoladi.
...
/* =================== 09 · Eng kichik son (9,0,4 → 409) · 🔴 · digits_min =================== */
const D09_ANS = '409';
const D09_T = {
  uz: {
    eyebrow: 'Kombinatorika', setup: "Yana o'sha kartalar: 9, 0 va 4. Endi teskari vazifa.",
    ask: 'Shu raqamlardan ENG KICHIK uch xonali sonni tering.',
    digits: '9 · 0 · 4',
    correct: "To'g'ri! Eng kichik son — 409: kichik raqam oldinga, lekin nol birinchi turolmaydi, shuning uchun 4 boshlaydi, 0 esa o'rtaga.",
    wrong: "Maslahat: son kichik bo'lishi uchun kichik raqam oldinda turishi kerak. Lekin nol birinchi o'rinda turolmaydi — unda son uch xonali bo'lmay qoladi. Unda qaysi raqam boshlaydi?",
    rule: "Nol sonni boshlay olmaydi: eng kichigi 409 (004 yoki 049 emas).",
  },
  ru: {
    eyebrow: 'Комбинаторика', setup: 'Снова те же карточки: 9, 0 и 4. Теперь обратная задача.',
    ask: 'Набери из этих цифр САМОЕ МАЛЕНЬКОЕ трёхзначное число.',
    digits: '9 · 0 · 4',
    correct: 'Верно! Самое маленькое число — 409: маленькая цифра вперёд, но ноль не может стоять первым, поэтому начинает 4, а 0 — в середину.',
    wrong: 'Подсказка: чтобы число было маленьким, маленькая цифра должна стоять впереди. Но ноль не может быть первым — тогда число перестанет быть трёхзначным. Какая цифра начнёт?',
    rule: 'Ноль не может начинать число: самое маленькое — 409 (не 004 и не 049).',
  },
};
function D02_09Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D09_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'digits_min', level: '🔴' } });

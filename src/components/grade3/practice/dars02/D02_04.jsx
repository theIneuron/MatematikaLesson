// Dars 2 (3-sinf) · Amaliyot 04 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 10-bet 3-mashq — otardagi 410 soni (shart moslab qayta yozilgan).
// Tuzoq: «to'rt yuz o'n» ni 40010 deb yopishtirib yozish (nazariy Dars02 M2 misconception).
...
// Raqam-plita (grade2 NumPad naqshi); tuzoqni ushlash uchun max=5 — bola 40010 deb tera olsin
...
/* =================== 04 · Sonni yozing (410) · 🟡 · write_num =================== */
const D04_ANS = '410';
const D04_T = {
  uz: {
    eyebrow: 'Sonni yozing', setup: "Orif akaning otarida to'rt yuz o'nta qo'y va echki bor. Bu sonni jurnalga raqamlar bilan yozish kerak.",
    ask: "«To'rt yuz o'n» sonini raqam-plitada tering.",
    word: "to'rt yuz o'n",
    correct: "To'g'ri! To'rt yuz o'n = 410: 4 yuzlik, 1 o'nlik, 0 birlik.",
    wrong: "Maslahat: har razryad O'Z katagiga yoziladi. «To'rt yuz» va «o'n» yonma-yon yopishtirilmaydi — uch xonali son hosil bo'lishi kerak.",
    rule: "To'rt yuz (400) + o'n (10) = 410 — uch xonali son, 40010 emas.",
  },
  ru: {
    eyebrow: 'Запиши число', setup: 'В отаре Орифа-ака четыреста десять овец и коз. Это число нужно записать в журнал цифрами.',
    ask: 'Набери число «четыреста десять» на панели.',
    word: 'четыреста десять',
    correct: 'Верно! Четыреста десять = 410: 4 сотни, 1 десяток, 0 единиц.',
    wrong: 'Подсказка: каждый разряд пишется в СВОЮ клетку. «Четыреста» и «десять» не склеиваются рядом — должно получиться трёхзначное число.',
    rule: 'Четыреста (400) + десять (10) = 410 — трёхзначное число, а не 40010.',
  },
};
function D02_04Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D04_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'write_num', level: '🟡' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <NumPad value={value} setValue={setValue} disabled={locked} max={5} tone={checked ? (fb?.correct ? 'ok' : 'no') : 'idle'} />

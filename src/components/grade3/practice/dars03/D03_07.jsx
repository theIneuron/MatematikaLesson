...
function D03_07Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D07_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'strip_addends', level: '🟡' } });
    ...
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'center', ...S.mono, fontSize: 26, fontWeight: 800, color: C.glow, textShadow: '0 0 10px rgba(20,90,134,.16)' }}>{t.expr} = ?</div>
        </div>
      </Stage>

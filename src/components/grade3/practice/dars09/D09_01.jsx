// Dars 9 (3-sinf) · Amaliyot 01 — mustaqil topshiriq fayli (grade5 dars04 formati).
// Manba: darslik 12-bet 3-mashq (uzum savatlari) · grade5 dars04 «Savatlar» formati (sum_to_product).
...
/* =================== 01 · Savatlar: yig'indidan ko'paytmaga · 🟢 · sum_to_product =================== */
// Chalg'ituvchilar: 9 + 8 (qo'shish bilan adashish), 8 × 8 (savat sonini xato sanash).
const D01_TAG = 'sum_to_product', D01_LEVEL = '🟢', D01_CORRECT = 0;
const D01_T = {
  uz: {
    eyebrow: "Savatlar", setup: "Tokzorda 9 ta savat bor, har birida 8 kg uzum. Jami uzumni 8 + 8 + ... deb 9 marta qo'shish uzoq!",
    ask: "Qaysi ko'paytma jami uzumni beradi?",
    opts: ['9 × 8', '9 + 8', '8 × 8'],
    correct: "To'g'ri! 8 ni 9 marta qo'shish — bu 9 × 8 = 72 kg. Ko'paytirish — teng qo'shiluvchilarning qisqa yozuvi.",
    wrong: "Maslahat: nechta savat bor? Har savatda nechadan? Teng qo'shiluvchilar soni × bitta qo'shiluvchi.",
    rule: "Teng qo'shiluvchilar yig'indisi ko'paytma bilan yoziladi: 8+8+... (9 marta) = 9 × 8.",
  },
  ru: {
    eyebrow: 'Корзины', setup: 'В винограднике 9 корзин, в каждой по 8 кг винограда. Складывать 8 + 8 + ... девять раз — долго!',
    ask: 'Какое произведение даст весь виноград?',
    opts: ['9 × 8', '9 + 8', '8 × 8'],
    correct: 'Верно! Сложить 8 девять раз — это 9 × 8 = 72 кг. Умножение — короткая запись равных слагаемых.',
    wrong: 'Подсказка: сколько корзин? По сколько в каждой? Число равных слагаемых × одно слагаемое.',
    rule: 'Сумма равных слагаемых записывается произведением: 8+8+... (9 раз) = 9 × 8.',
  },
};
...
function D10_01Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: D01_CORRECT, label: t.opts[D01_CORRECT] }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: D01_TAG, level: D01_LEVEL } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center', maxWidth: 420, margin: '0 auto' }}>{Array.from({ length: 9 }).map((_, i) => (<div key={i} style={{ minWidth: 48, padding: '8px 6px', borderRadius: 10, background: '#FFFFFF', border: '1.5px solid ' + C.ribbonBd, textAlign: 'center' }}><div style={{ fontSize: 16 }}>🍇</div><div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, fontWeight: 800, color: C.glow }}>8 kg</div></div>))}</div></Stage>

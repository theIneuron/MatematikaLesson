// Dars 8 (3-sinf) · Amaliyot 06 — mustaqil topshiriq fayli (grade2 naqshi).
// Aralash o'qish: XIV = X + IV = 10 + 4 = 14 (ichida chapdan-ayirish bor).
...
/* =================== 06 · Rimchani o'qing (XIV) · 🟡 · rome_read2 =================== */
const D02_ANS = '14';
const D02_T = {
  uz: {
    eyebrow: "Aralash o'qish", setup: "Displeyda XIV yozuvi. Diqqat: ichida chapda turgan I bor!",
    ask: "XIV qaysi son? Raqam-plitada tering.",
    expr: 'XIV',
    correct: "To'g'ri! XIV = X + IV = 10 + 4 = 14. IV da I chapda — ayiriladi (5 − 1 = 4).",
    wrong: "Maslahat: yozuvni bo'laklang: X va IV. IV da kichik I kattadan chapda — demak ayiriladi: 5 − 1. Endi qo'shing.",
    rule: "XIV = X (10) + IV (4) = 14.",
  },
  ru: {
    eyebrow: 'Смешанное чтение', setup: 'На дисплее запись XIV. Внимание: внутри есть I, стоящая слева!',
    ask: 'Какое это число — XIV? Набери на панели.',
    expr: 'XIV',
    correct: 'Верно! XIV = X + IV = 10 + 4 = 14. В IV знак I слева — вычитается (5 − 1 = 4).',
    wrong: 'Подсказка: раздели запись: X и IV. В IV маленькая I слева от большой — значит, вычитается: 5 − 1. Теперь сложи.',
    rule: 'XIV = X (10) + IV (4) = 14.',
  },
};
function D08_06Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D02_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'rome_read2', level: '🟡' } });

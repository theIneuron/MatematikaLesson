// Dars 8 (3-sinf) · Amaliyot 07 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: darslik 88-89-bet qoidalari. Xato: IX = 11 (chapda-ayirish unutilgan).
...
/* =================== 07 · Xatoni toping · 🟡 · find_error_rome =================== */
const D10_TAG = 'find_error_rome', D10_LEVEL = '🟡', D10_CORRECT = 1;
const D10_T = {
  uz: {
    eyebrow: 'Xatoni toping', setup: "Uchta rim yozuvi o'qildi. Bittasi xato o'qilgan.",
    ask: 'Qaysi yozuv XATO? Uni bosing.',
    opts: ['VII = 7', 'IX = 11', 'XX = 20'],
    correct: "To'g'ri topdingiz! IX = 9: I chapda turibdi, demak ayiriladi (10 − 1). 11 esa XI deb yoziladi.",
    wrong: "Maslahat: har yozuvni tekshiring: kichik belgi qaysi tomonda? Chapda bo'lsa ayirilishi kerak — qaysi javobda bu unutilgan?",
    rule: "IX = 9 (chapda — ayiriladi), XI = 11 (o'ngda — qo'shiladi).",
  },
  ru: {
    eyebrow: 'Найди ошибку', setup: 'Три римские записи прочитаны. Одна прочитана неверно.',
    ask: 'Какая запись НЕВЕРНА? Нажми на неё.',
    opts: ['VII = 7', 'IX = 11', 'XX = 20'],
    correct: 'Верно! IX = 9: I стоит слева, значит, вычитается (10 − 1). А 11 записывается как XI.',
    wrong: 'Подсказка: проверь каждую запись: с какой стороны маленький знак? Если слева — он должен вычитаться. Где это забыли?',
    rule: 'IX = 9 (слева — вычитается), XI = 11 (справа — прибавляется).',
  },
};

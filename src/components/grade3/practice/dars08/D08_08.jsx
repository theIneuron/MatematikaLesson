// Dars 8 (3-sinf) · Amaliyot 08 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: darslik 89-bet qoidasi — IX = 10 − 1 = 9 (chapda-ayirish); V — chalg'ituvchi karta.
...
/* =================== 08 · 9 ni rimchada yasang (IX) · 🔴 · rome_build9 =================== */
const D08_DIGITS = ['I', 'X', 'V'];
const D08_ANS = 'IX';
const D08_T = {
  uz: {
    eyebrow: "Rimchada yasang", setup: "Kartalarda I, X va V belgilari bor. 9 sonini yasash uchun faqat IKKITASI kerak.",
    ask: "9 sonini rim raqamlarida yasang (2 ta karta yetadi).",
    tray: 'Kartalar:', slots: '9 =',
    correct: "To'g'ri! IX = 10 − 1 = 9: kichik I kattadan CHAPDA turgani uchun ayiriladi.",
    wrong: "Maslahat: 9 = 10 − 1. Ayirish uchun kichik belgi kattadan CHAPDA turishi kerak. Qaysi ikki belgi kerak, qaysi tartibda?",
    rule: "9 = IX: kichik belgi chapda — ayiriladi. XI esa 11 bo'lar edi.",
  },
  ru: {
    eyebrow: 'Собери по-римски', setup: 'На карточках знаки I, X и V. Чтобы собрать число 9, нужны только ДВА из них.',
    ask: 'Собери число 9 римскими цифрами (хватит 2 карточек).',
    tray: 'Карточки:', slots: '9 =',
    correct: 'Верно! IX = 10 − 1 = 9: маленькая I стоит СЛЕВА от большой, поэтому вычитается.',
    wrong: 'Подсказка: 9 = 10 − 1. Для вычитания маленький знак должен стоять СЛЕВА от большого. Какие два знака нужны и в каком порядке?',
    rule: '9 = IX: маленький знак слева — вычитается. А XI было бы 11.',
  },
};
function D08_08Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { slots, value: made }, correctAnswer: { value: D08_ANS }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: 'rome_build9', level: '🔴' } });

// Dars 6 (3-sinf) · Amaliyot 01 — mustaqil topshiriq fayli (grade2 naqshi).
// Son o'qini o'qish: katta belgi — yuzlik, kichik belgi — o'nlik (nazariy Dars06 qoidasi).
// Strelka 250 da; chalg'ituvchilar: 205 (belgini birlik deb sanash), 240 (bitta belgi adashish).
...
function NumLine({ lo, hi, marks = [], dot = null, dotLabel = null, seg = null }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} 96`} style={{ width: '100%', display: 'block' }} aria-hidden="true">
      {seg && <rect x={xOf(seg[0])} y={LINE_Y - 16} width={xOf(seg[1]) - xOf(seg[0])} height={32} rx={8} fill="rgba(20,90,134,.10)" stroke="rgba(255,184,77,.55)" strokeWidth="1.5" />}
...
/* =================== 01 · Strelka qaysi sonda? (250) · 🟢 · line_read =================== */
const D01_TAG = 'line_read', D01_LEVEL = '🟢', D01_CORRECT = 0;
const D01_T = {
  uz: {
    eyebrow: "Son o'qini o'qing", setup: "Shkalada katta belgilar — yuzliklar, kichik belgilari — o'nliklar. Strelka bitta belgida turibdi.",
    ask: "Strelka qaysi sonni ko'rsatyapti?",
    opts: ['250', '240', '205'],
    correct: "To'g'ri! 200 dan boshlab o'nliklab sanaymiz: 210, 220, 230, 240, 250 — strelka 250 da.",
    wrong: "Maslahat: katta belgi — 200. Undan strelkagacha kichik belgilarni sanang: har kichik belgi — bitta o'nlik.",
    rule: "Son o'qida katta belgi — yuzlik, kichik belgi — o'nlik.",
  },
  ru: {
    eyebrow: 'Читай числовую прямую', setup: 'На шкале большие метки — сотни, маленькие — десятки. Стрелка стоит на одной из меток.',
    ask: 'Какое число показывает стрелка?',
    opts: ['250', '240', '205'],
    correct: 'Верно! Считаем от 200 десятками: 210, 220, 230, 240, 250 — стрелка на 250.',
    wrong: 'Подсказка: большая метка — 200. Считай от неё маленькие метки до стрелки: каждая маленькая метка — один десяток.',
    rule: 'На числовой прямой большая метка — сотня, маленькая — десяток.',
  },
};
...
function D06_01Impl(props) {
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage><NumLine lo={200} hi={300} dot={250} dotLabel={checked && fb?.correct ? '250' : '?'} /></Stage>

// Dars 6 (3-sinf) · Amaliyot 10 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: darslik 50-bet (256) + nazariy seedlar (242, 250) — o'qda chapdan o'ngga joylashuv.
...
/* =================== 10 · O'qda chapdan o'ngga (242, 250, 256) · 🔴 · line_order =================== */
const D07_CARDS = [256, 242, 250]; // aralash tartibda ko'rsatiladi
const D07_ANS = [242, 250, 256];
const D07_T = {
  uz: {
    eyebrow: "O'qdagi tartib", setup: "Uchta son son o'qiga qo'yiladi. O'qda chapda — kichik, o'ngda — katta son turadi.",
    ask: "Kartalarni o'qdagi tartibda bosing: avval eng chapdagisi.",
    slots: "Chapdan o'ngga:", tray: 'Kartalar:',
    correct: "To'g'ri! O'qda chapdan o'ngga: 242, 250, 256 — kichikdan kattaga.",
    wrong: "Maslahat: o'qda chapda eng kichik son turadi. Yuzliklar teng — o'nliklarni solishtiring: 4, 5 va 5. Teng bo'lsa, birlikka o'ting.",
    rule: "Son o'qida chapdan o'ngga sonlar o'sib boradi: 242, 250, 256.",
  },
  ru: {
    eyebrow: 'Порядок на прямой', setup: 'Три числа ставят на числовую прямую. Слева — меньшее, справа — большее.',
    ask: 'Нажимай карточки в порядке прямой: сначала самое левое.',
    slots: 'Слева направо:', tray: 'Карточки:',
    correct: 'Верно! На прямой слева направо: 242, 250, 256 — от меньшего к большему.',
    wrong: 'Подсказка: слева на прямой — самое маленькое число. Сотни равны — сравни десятки: 4, 5 и 5. Если равны — единицы.',
    rule: 'На числовой прямой числа растут слева направо: 242, 250, 256.',
  },
};
function D06_10Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: D07_CARDS.map(String), studentAnswer: { slots }, correctAnswer: { slots: D07_ANS }, correct, meta: { tag: 'line_order', level: '🔴' } });
  }, [slots, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const locked = isReview || checked;
  const inTray = seededOrder(D07_CARDS.length, `line-order:${shuffleSeed}`)
          ...
          {slots.map((v, i) => (
            <React.Fragment key={i}>
              ...
              <button type="button" onClick={() => popSlot(i)} disabled={locked || v == null}
                style={{ minWidth: 90, height: 64, borderRadius: 12, background: v != null ? '#FFFFFF' : 'rgba(255,255,255,.05)', border: v != null ? '1.5px solid ' + C.ribbonBd : '2px dashed ' + C.stageBd, display: 'flex', alignItems: 'center', justifyContent: 'center', ...S.mono, fontSize: 26, fontWeight: 800, color: C.glow, textShadow: v != null ? '0 0 12px rgba(20,90,134,.18)' : 'none', cursor: (locked || v == null) ? 'default' : 'pointer', boxShadow: slotRing(i) }}>
                {v != null ? v : ''}
              </button>
            </React.Fragment>
          ))}

// Dars 1 (3-sinf) · Amaliyot 06 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 4-bet 4-mashq — 903 soni (903 − 3 turkumidan).
// jsx-question kontrakti: onReady / registerCheck / onSubmit. O'z "Tekshirish" tugmasi yo'q — PracticeHost beradi.
// Misconception-tuzoq: nol razryad (903 da 0 o'nlik).
...
// Razryad-katakli neon displey: har raqam o'z katagida, razryad nomi ostida
const PlaceNeon = ({ digits, labels }) => (
  <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
    {digits.map((d, i) => (
      <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
        <div style={{ width: 62, height: 70, borderRadius: 12, background: '#FFFFFF', border: '1.5px solid ' + C.ribbonBd, boxShadow: 'inset 0 0 14px rgba(20,90,134,.11)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontSize: 38, fontWeight: 800, color: C.glow, textShadow: '0 0 12px rgba(20,90,134,.18)' }}>{d}</span>
        </div>
        <span style={{ fontSize: 11, fontWeight: 800, color: C.sink2, textTransform: 'uppercase', letterSpacing: '.03em' }}>{labels[i]}</span>
      </div>
    ))}
  </div>
);
...
/* =================== 06 · Nol razryad (903) · 🟡 · zero_place =================== */
// Tuzoq: bola «903 da 9 o'nlik» yoki «3 o'nlik» deb adashadi. To'g'ri: 0 o'nlik.
const D06_TAG = 'zero_place', D06_LEVEL = '🟡', D06_CORRECT = 0;
const D06_T = {
  uz: {
    eyebrow: 'Diqqat, razryad!', setup: "Displeyda yangi son: 903.",
    ask: "903 sonida nechta o'nlik bor?", opts: ['0', '9', '3'],
    labels: ['yuzlik', "o'nlik", 'birlik'],
    correct: "To'g'ri! 903 da o'nlik yo'q — o'rtada 0 turibdi: 9 yuzlik, 0 o'nlik, 3 birlik.",
    wrong: "Maslahat: o'nliklarni o'rtadagi raqam ko'rsatadi. 903 sonining o'rtasiga qarang.",
    rule: "Nol ham razryadni egallaydi: u joyni saqlab turadi. 903 = 9 yuzlik, 0 o'nlik, 3 birlik.",
  },
  ru: {
    eyebrow: 'Внимание, разряд!', setup: 'На дисплее новое число: 903.',
    ask: 'Сколько десятков в числе 903?', opts: ['0', '9', '3'],
    labels: ['сотни', 'десятки', 'единицы'],
    correct: 'Верно! В 903 нет десятков — в середине стоит 0: 9 сотен, 0 десятков, 3 единицы.',
    wrong: 'Подсказка: десятки показывает средняя цифра. Посмотри на середину числа 903.',
...
function D01_06Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: D06_CORRECT, label: t.opts[D06_CORRECT] }, correct, feedbackText: correct ? t.correct : t.wrong, explanationText: correct ? t.correct : t.wrong, ruleText: t.rule || '', meta: { tag: D06_TAG, level: D06_LEVEL } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        {/* razryad nomlari faqat TO'G'RI javobdan keyin ochiladi — aks holda javob ko'rinib qoladi */}
        <PlaceNeon digits={['9', '0', '3']} labels={checked && fb?.correct ? t.labels : ['', '', '']} />
      </Stage>

...
function D01_09Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { value }, correctAnswer: { value: D09_ANS }, correct, meta: { tag: 'neighbour', level: '🔴' } });
  }, [value, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const locked = isReview || checked;
  const cell = (txt, hot) => (
    <div style={{ minWidth: 86, height: 64, borderRadius: 12, background: '#152342', border: '1.5px solid ' + (hot ? C.glow : C.ribbonBd), boxShadow: hot ? '0 0 14px rgba(255,184,77,.35)' : 'inset 0 0 14px rgba(255,184,77,.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 10px' }}>
      <span style={{ ...S.mono, fontSize: 28, fontWeight: 800, color: hot ? '#FF7A5E' : C.glow, textShadow: '0 0 10px rgba(255,184,77,.7)' }}>{txt}</span>
    </div>
  );

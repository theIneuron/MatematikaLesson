...
const Stepper = ({ label, color, value, set, max, locked, render }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: 8, borderRadius: 12, border: '2px solid ' + C.stageBd, background: C.stile, minWidth: 108, flex: '1 1 30%' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <button type="button" disabled={locked || value <= 0} onClick={() => set(Math.max(0, value - 1))} style={{ width: 34, height: 34, borderRadius: 9, border: 'none', background: value <= 0 ? '#D9E6F0' : color, color: '#243447', fontSize: 20, fontWeight: 800, cursor: (locked || value <= 0) ? 'default' : 'pointer' }}>−</button>
      <span style={{ ...S.mono, fontSize: 22, fontWeight: 800, color: C.sink, minWidth: 22, textAlign: 'center' }}>{value}</span>
      <button type="button" disabled={locked || value >= max} onClick={() => set(Math.min(max, value + 1))} style={{ width: 34, height: 34, borderRadius: 9, border: 'none', background: value >= max ? '#D9E6F0' : color, color: '#243447', fontSize: 20, fontWeight: 800, cursor: (locked || value >= max) ? 'default' : 'pointer' }}>+</button>
    </div>
  </div>
);

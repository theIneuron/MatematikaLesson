// Dars 1 (3-sinf) · Amaliyot 07 — mustaqil topshiriq fayli (grade2 naqshi).
// Manba: 3-sinf darsligi, 4-bet 4-mashq — 854 soni (854 − 50 − 4 turkumidan).
// jsx-question kontrakti: onReady / registerCheck / onSubmit. O'z "Tekshirish" tugmasi yo'q — PracticeHost beradi.
// Joy qiymati: raqamning turgan joyi uning qiymatini belgilaydi (854 dagi 5 = 50).
...
.g3d1-pulse { animation: g3d1pulse 1.6s ease-in-out infinite; display: inline-block; }
@keyframes g3d1pulse { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } }
@media (prefers-reduced-motion: reduce) { * { animation: none !important; transition: none !important; } }`;

/* =================== 07 · Raqamning qiymati (854) · 🟡 · digit_value =================== */
const D07_TAG = 'digit_value', D07_LEVEL = '🟡', D07_CORRECT = 0;
const D07_T = {
  uz: {
    eyebrow: 'Raqamning qiymati', setup: "Displeyda 854 soni turibdi, 5 raqami ajratib yoqilgan.",
    ask: '854 sonidagi 5 raqami nimani bildiradi?',
    opts: ["5 ta o'nlik — 50", '5 ta yuzlik — 500', '5 ta birlik — 5'],
    correct: "To'g'ri! 5 raqami o'nliklar joyida turibdi, demak u 50 ni bildiradi: 854 = 800 + 50 + 4.",
    wrong: "Maslahat: raqamning qiymati uning turgan joyiga bog'liq. 5 raqami 854 da qaysi joyda turibdi — birinchimi, o'rtadami, oxiridami?",
    rule: "Bir xil raqam turli joyda turli qiymat beradi: 854 = 800 + 50 + 4.",
  },
  ru: {
    eyebrow: 'Значение цифры', setup: 'На дисплее число 854, цифра 5 подсвечена.',
    ask: 'Что означает цифра 5 в числе 854?',
    opts: ['5 десятков — 50', '5 сотен — 500', '5 единиц — 5'],
    correct: 'Верно! Цифра 5 стоит на месте десятков, значит она означает 50: 854 = 800 + 50 + 4.',
    wrong: 'Подсказка: значение цифры зависит от её места. Где стоит цифра 5 в числе 854 — первой, в середине или в конце?',
    rule: 'Одна и та же цифра на разных местах даёт разное значение: 854 = 800 + 50 + 4.',
  },
};
...
function D01_07Impl(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: D07_CORRECT, label: t.opts[D07_CORRECT] }, correct, meta: { tag: D07_TAG, level: D07_LEVEL } });
  }, [picked, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const digitStyle = (hot) => ({
    fontFamily: "'JetBrains Mono', ui-monospace, monospace", fontSize: 44, fontWeight: 800, letterSpacing: 6,
    color: hot ? '#FF7A5E' : C.glow, textShadow: hot ? '0 0 16px rgba(255,122,94,.95)' : '0 0 12px rgba(255,184,77,.8)',
  });
  return (
    <div style={S.wrap}>
      ...
      <Stage>
        <div style={{ display: 'flex', justifyContent: 'center' }}>
          <div style={{ padding: '10px 22px', borderRadius: 14, background: '#152342', border: '1.5px solid ' + C.ribbonBd, boxShadow: 'inset 0 0 18px rgba(255,184,77,.22)' }}>
            <span style={digitStyle(false)}>8</span>
            <span className="g3d1-pulse" style={digitStyle(true)}>5</span>
            <span style={digitStyle(false)}>4</span>
          </div>
        </div>
      </Stage>

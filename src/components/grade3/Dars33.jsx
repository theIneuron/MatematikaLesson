...
// s5 saralash chekka yoki ichkari · s6 test kvadrat tomoni 6 · s7 konsol uzun va qisqa yo'l ·
// s8 xatoni top (ichkaridagi kataklar) · s9 Bit tuzog'i (teng tasma, har xil ichkari) ·
// s10 trenajyor 4 ga 6 · s11 trenajyor kvadrat 7 · s12 masala (perimetr, keyin qoldiq) ·
...
const CONTENT = {
  ...
  s0: {
    ...
    audio: {
      intro: {
        ru: [
          'Мы прилетели в квартал кристальной архитектуры. Здесь всё строят и измеряют.',
          'Панель пять метров в длину и три в ширину.',
          'Её надо обвести светящейся лентой по самому краю.',
          'Как думаешь, сколько метров ленты понадобится?'
        ],
        uz: [
          "Biz kristall arxitektura kvartaliga uchib keldik. Bu yerda hamma narsa quriladi va o'lchanadi.",
  ...
  s1: {
    ...
    audio: {
      ru: [
        ...
        'Пять метров, потом три, потом снова пять и снова три. Мы вернулись в ту же точку.',
        'Сложим всё вместе. Пять и три и пять и три, получается шестнадцать метров. Это и есть периметр.'
      ],
      uz: [
  ...
  s2: {
    ...
    audio: {
      ru: [
        ...
        'Идём по верхней стороне пять клеток, потом вниз по правой три.',
        'Возвращаемся. Пять по нижней и три по левой. Всего шестнадцать клеток пути. Это периметр.'
      ],
      uz: [
  ...
  s9: {
    ...
    audio: {
      ru: [
        ...
        'На первую ушло шестнадцать метров ленты, на вторую тоже шестнадцать. Значит панели одинаковые.',
        'Так ли это?'
      ],
      uz: [
      ...
      {Array.from({ length: 3 }).map((_, r) => (
        Array.from({ length: 5 }).map((_, c) => (
          <rect key={`${r}-${c}`} x={c * 34} y={r * 28} width="34" height="28"
            fill={(r + c) % 2 ? '#DCEBF5' : '#EAF4FA'} stroke="#7FA8BF" strokeWidth="0.9" opacity="0.92"/>
        ))
      ))}
      <rect x="0" y="0" width="170" height="84" fill="none" stroke="#FFB92E" strokeWidth="3.4"/>
      <g fill="#FFB92E">
    ...
    <g transform="translate(16 20)">
      {Array.from({ length: 4 }).map((_, r) => (
        Array.from({ length: 4 }).map((_, c) => (
          <rect key={`${r}-${c}`} x={c * 15} y={r * 15} width="15" height="15" fill="#F7F1E4" stroke="#C08A3E" strokeWidth="0.8"/>
        ))
      ))}
      <rect x="0" y="0" width="60" height="60" fill="none" stroke="#C06A2E" strokeWidth="2.4"/>
      <text x="30" y="76" textAnchor="middle" fontSize="9" fontWeight="800" fill="#5A4A2E" fontFamily="'JetBrains Mono', monospace">16 kv.</text>
    </g>
    <g transform="translate(104 50)">
      {Array.from({ length: 7 }).map((_, c) => (
        <rect key={c} x={c * 15} y="0" width="15" height="15" fill="#F7F1E4" stroke="#C08A3E" strokeWidth="0.8"/>
      ))}
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(4px, 1vw, 8px)' }}>
        ...
        <div className="frame fade-up delay-1 d33-hook-scene" style={{ padding: 'clamp(8px, 1.8vw, 14px)', overflow: 'hidden' }}>
        ...
        {picked === null && (
          <div className="frame fade-up delay-1" style={{ padding: 'clamp(6px, 1.2vw, 9px)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <span className="d33-order">
              <span className="mono d33-order-plate">11</span>
              <span className="d33-order-sep mono">:</span>
              <span className="mono d33-order-plate">2</span>
            </span>
            <span className="d33-note">{t(c.order_cap)}</span>
          </div>
        )}
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(6px, 1.4vw, 10px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(6px, 1.4vw, 10px)', padding: 'clamp(10px, 2vw, 15px)' }}>
          <FrameFx/>
          <GridFig w={c.w} h={c.h} mode="perimeter" walk={step >= 2 ? total : step >= 1 ? c.w + c.h : 0} labels={[String(c.w), String(c.h)]}/>
          <div className="d33-gridrow">
            {step >= 1 && (
              <span className="d33-gridcap lm-reveal">
                <span className="d33-expr" style={{ fontSize: 'clamp(11px, 1.7vw, 13px)', color: '#C97F35' }}>{t(c.capA)}</span>
              </span>
            )}
            {step >= 2 && (
              <span className="d33-gridcap lm-reveal">
                <span className="d33-expr" style={{ fontSize: 'clamp(11px, 1.7vw, 13px)', color: '#2E7E9E' }}>{t(c.capB)}</span>
              </span>
            )}
          </div>
          {step >= 2 && <span className="mono d33-final lm-reveal" style={{ animationDelay: '0.25s' }}>{c.res}</span>}
...
// s4 — RASM BO'YICHA: chizmadagi panelning perimetri
const Screen4 = (props) => (
  <MCOne props={props} ck="s4"
    figNode={<GridFig w={CONTENT.s4.fig_w} h={CONTENT.s4.fig_h} mode="perimeter" walk={2 * (CONTENT.s4.fig_w + CONTENT.s4.fig_h)} labels={[String(CONTENT.s4.fig_w), String(CONTENT.s4.fig_h)]}/>}/>
);
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(6px, 1.4vw, 10px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(6px, 1.4vw, 10px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
          <FrameFx/>
          <span className="mono d33-plate">{lines[0]}</span>
          <span className="d33-bad">{lines[1]}</span>
          <span className="d33-note">{t(c.line_cap)}</span>
          <p className="fade-up" style={{ margin: 0, textAlign: 'center', color: T.ink2, fontWeight: 700 }}>{t(c.trap_label)}</p>
          <div className="d33-trap">
        ...
        {done && (
          <div ref={factRef}>
            ...
            <div className="d2-factcard fade-up">
              ...
              <div className="d2-fact-hero"><SamePerimFig/></div>
...
.d33-order { display: inline-flex; gap: clamp(6px, 1.6vw, 12px); align-items: center; padding: clamp(4px, 1vw, 7px) clamp(8px, 1.8vw, 12px);
  border-radius: 10px; background: rgba(255,236,200,.5); box-shadow: inset 0 0 0 1px rgba(190,150,90,.26); }
.d33-order-plate { font-size: clamp(15px, 3vw, 21px); font-weight: 800; color: #3A3530; padding: 3px 10px;
  border-radius: 8px; background: #FFFFFF; box-shadow: 0 2px 8px -4px rgba(58,53,48,0.35); }
.d33-note { font-size: clamp(9.5px, 1.5vw, 12px); font-weight: 700; color: #5A5A60; text-align: center; }
.d33-expr { font-size: clamp(15px, 3vw, 22px); font-weight: 800; color: #3A3530; }
.d33-final { font-size: clamp(18px, 3.6vw, 26px); font-weight: 800; color: #1F7A4D; }
.d33-bad { font-size: clamp(14px, 2.8vw, 20px); font-weight: 800; color: #C0392B; }
.d33-errline { font-size: clamp(13px, 2.5vw, 19px); font-weight: 800; color: #C0392B; padding: 3px 12px;
  border-radius: 9px; background: rgba(192,57,43,.08); box-shadow: inset 0 0 0 1px rgba(192,57,43,.3); text-align: center; }
.d33-steplabel { font-size: clamp(13px, 2.4vw, 18px); font-weight: 800; color: #3A3530; text-align: center; }
.d33-res { font-size: clamp(19px, 3.8vw, 26px); font-weight: 800; color: #1F7A4D; }
.d33-plate { font-size: clamp(19px, 4vw, 28px); font-weight: 800; color: #0E0E10; padding: 4px 14px;
  border-radius: 12px; background: rgba(255,236,200,.55); box-shadow: inset 0 0 0 1px rgba(190,150,90,.28); }
.d33-trap { display: flex; gap: 10px; justify-content: center; }
.d33-bins { display: grid; grid-template-columns: repeat(2, minmax(120px, 1fr)); gap: clamp(8px, 2vw, 16px); width: 100%; max-width: 460px; }

/* --- KATAK TO'R --- */
.d33-grid { display: inline-flex; align-items: flex-start; gap: clamp(5px, 1.2vw, 9px);
...
.d33-grid-part { display: inline-flex; flex-direction: column; gap: 2px; }
.d33-grid-row { display: inline-flex; gap: 2px; }
.d33-cell { display: inline-block; width: clamp(6px, 1.5vw, 10px); height: clamp(6px, 1.5vw, 10px); border-radius: 2px; }
.d33-cell-a { background: #F2A85C; box-shadow: inset 0 0 0 0.5px #C97F35; }
.d33-cell-b { background: #6FD0E4; box-shadow: inset 0 0 0 0.5px #3E8FA8; }
.d33-gridrow { display: flex; flex-wrap: wrap; justify-content: center; align-items: flex-start; gap: clamp(6px, 1.6vw, 12px); }
.d33-gridcap { display: flex; flex-direction: column; align-items: center; gap: 3px; }

/* --- sahna o'lchami: budjet donor bilan bir xil (etalon 629x330 @1440x900) --- */
.d33-hook-scene .lm-scene { width: min(100%, calc(clamp(180px, calc(100dvh - 570px), 372px) * 400 / 210)); }
.d33-final-scene .lm-scene { width: min(100%, calc(clamp(200px, calc(100dvh - 560px), 372px) * 400 / 210)); }

/* --- FACTCARD: zinapoya --- */
.d33-stair { animation: d22stair 3.6s ease-in-out infinite; }
@keyframes d22stair { 0%, 12% { opacity: 0.3; } 34%, 100% { opacity: 1; } }
@media (prefers-reduced-motion: reduce) { .d33-stair { animation: none; opacity: 1; } }

.d33-boxwrap { display: flex; flex-direction: column; align-items: center; gap: clamp(6px, 1.4vw, 10px); }
.d33-boxrow { display: grid; grid-template-columns: repeat(6, auto); gap: clamp(4px, 1vw, 7px); justify-content: center; }
.d33-box { width: clamp(20px, 3.4vw, 27px); height: clamp(17px, 2.9vw, 23px); border-radius: 3px;
  background: #EFE6D6; border: 1.5px solid #D8CDB8; opacity: 0.5; transition: none; }
.d33-box-on { background: linear-gradient(180deg, #FFCB8E 0 26%, #F2A85C 26% 100%); border-color: #C97F35;
...
.d33-rest { display: inline-flex; gap: clamp(4px, 1vw, 7px); align-items: center; padding: clamp(4px, 1vw, 7px) clamp(8px, 1.8vw, 13px);
  border-radius: 999px; background: #FDECE7; border: 1.5px dashed #E0563A; }
.d33-kg { width: clamp(11px, 1.9vw, 15px); height: clamp(11px, 1.9vw, 15px); border-radius: 50%;
  background: #E0563A; border: 1.2px solid #B33F27; }

.d33-fig { display: block; margin: 0 auto; }
.d33-figrow { display: flex; align-items: center; justify-content: center; gap: clamp(10px, 2.4vw, 18px); flex-wrap: wrap; }
.d33-frac { display: inline-flex; flex-direction: column; align-items: center; line-height: 1.05;
  font-family: 'JetBrains Mono', monospace; font-weight: 800; font-size: clamp(20px, 3.4vw, 27px); }
.d33-frac-top { color: #2E7E9E; }
.d33-frac-bar { display: block; width: clamp(24px, 4vw, 32px); height: 2.4px; background: #5D5A52; margin: 3px 0; border-radius: 2px; }
.d33-frac-bot { color: #C97F35; }
.d33-fracname { font-family: 'JetBrains Mono', monospace; font-size: clamp(10px, 1.6vw, 12px); font-weight: 800; letter-spacing: 0.4px; }

/* Yangi uslub yo'q: hamma qoida 24-darsdan ko'chib keldi va nomi almashtirildi. */

.d33-pair { display: inline-flex; align-items: center; justify-content: center; gap: clamp(8px, 2vw, 16px); flex-wrap: nowrap; }
.d33-pair-one { display: inline-flex; flex-direction: column; align-items: center; gap: 4px; }
.d33-pair-cap { font-size: clamp(13px, 2.2vw, 17px); font-weight: 800; color: #5D5A52; }
.d33-pair-sign { font-size: clamp(20px, 3.6vw, 28px); font-weight: 800; color: #C97F35; min-width: clamp(18px, 3vw, 26px); text-align: center; }

...
// ============================================================================
// ░░ 3-SINF · Dars21 — "Yozma ko'paytirish va bo'lish" (num-3-21-v1) · GRADE3 ETALON ░░
// Dars01 ning to'liq sahna, personaj, audio, o'tish va UI dvigateli saqlangan.
// Mavzu: razryadlarni tekislash, ko'paytirishda o'tkazish, bo'lishda xona bo'yicha ishlash
// va teskari amal bilan tekshirish.
...
//
// FREE_NAV=false — Dars01 etalonidagi audio va topshiriq darvozalari saqlangan.
//
...
const LESSON_META = {
  lessonId: 'num-3-21-v1',
  lessonTitle: { ru: 'Урок 21. Письменное умножение и деление', uz: "21-dars. Yozma ko'paytirish va bo'lish" }
};
...
const SCREEN_GUIDES = [
  { tone: 'do', icon: '🎯', label: { ru: 'Сбой терминала', uz: 'Terminal nosozligi' }, steps: [{ ru: 'Оцени ответ', uz: 'Javobni chamalang' }, { ru: 'Выбери способ', uz: 'Usulni tanlang' }] },
  { tone: 'watch', icon: '👀', label: { ru: 'Умножаем столбиком', uz: "Ustunlab ko'paytiramiz" }, steps: [{ ru: 'Начни с единиц', uz: 'Birlikdan boshlang' }, { ru: 'Перенеси десятки', uz: "O'nlikni o'tkazing" }] },
  { tone: 'do', icon: '🧱', label: { ru: 'Собери результат', uz: "Natijani yig'ing" }, steps: [{ ru: 'Сотни', uz: 'Yuzlik' }, { ru: 'Десятки', uz: "O'nlik" }, { ru: 'Единицы', uz: 'Birlik' }] },
  { tone: 'watch', icon: '🔁', label: { ru: 'Три проверки', uz: 'Uchta tekshiruv' }, steps: [{ ru: 'По частям', uz: "Bo'laklab" }, { ru: 'Столбиком', uz: 'Ustunlab' }] },
  { tone: 'watch', icon: '📐', label: { ru: 'Ровные разряды', uz: 'Tekis xonalar' }, steps: [{ ru: 'Единицы под единицами', uz: 'Birlik ostiga birlik' }, { ru: 'Не сдвигай', uz: 'Siljitmang' }] },
  { tone: 'do', icon: '📍', label: { ru: 'Оценка ответа', uz: 'Javobni chamalash' }, steps: [{ ru: 'Найди 200', uz: '200 ni toping' }, { ru: 'Отметь 204', uz: '204 ni belgilang' }] },
  { tone: 'watch', icon: '➗', label: { ru: 'Делим столбиком', uz: "Ustunlab bo'lamiz" }, steps: [{ ru: 'Начни слева', uz: 'Chapdan boshlang' }, { ru: 'Снеси цифру', uz: 'Raqamni tushiring' }] },
  { tone: 'do', icon: '💡', label: { ru: 'Главное правило', uz: 'Asosiy qoida' }, steps: [{ ru: 'Найди единицы', uz: 'Birlikni toping' }, { ru: 'Нажми первую цифру', uz: 'Birinchi raqamni bosing' }] },
  { tone: 'do', icon: '🧩', label: { ru: 'Результаты', uz: 'Natijalar' }, steps: [{ ru: 'Вычисли', uz: 'Hisoblang' }, { ru: 'Собери число', uz: "Sonni yig'ing" }] },
  { tone: 'do', icon: '📦', label: { ru: 'Разряды ответа', uz: 'Javob xonalari' }, steps: [{ ru: 'Возьми цифру', uz: 'Raqamni oling' }, { ru: 'Поставь на место', uz: "O'rniga qo'ying" }] },
  { tone: 'do', icon: '✖️', label: { ru: 'Шаги умножения', uz: "Ko'paytirish qadamlari" }, steps: [{ ru: 'Считай справа', uz: "O'ngdan hisoblang" }, { ru: 'Учти перенос', uz: "O'tkazishni unutmang" }] },
  { tone: 'do', icon: '✅', label: { ru: 'Проверь оценкой', uz: 'Chamalab tekshiring' }, steps: [{ ru: 'Сравни', uz: 'Solishtiring' }, { ru: 'Исправь ошибку', uz: 'Xatoni tuzating' }] },
  { tone: 'do', icon: '🛠️', label: { ru: 'Задача мастерской', uz: 'Ustaxona masalasi' }, steps: [{ ru: 'Найди действие', uz: 'Amalni toping' }, { ru: 'Набери ответ', uz: 'Javobni tering' }] },
  ...
  { tone: 'finish', icon: '🏆', label: { ru: 'Терминал запущен', uz: 'Terminal ishga tushdi' }, steps: [{ ru: 'Повтори алгоритм', uz: 'Algoritmni takrorlang' }, { ru: 'Заверши урок', uz: 'Darsni tugating' }] },
];

const lessonPair = (uz, ru) => ({ uz, ru });
const practiceScreen = (type, title, text, visual, ask, options, correct, hint) => ({
  type,
  title: lessonPair(title[0], title[1]),
  text: lessonPair(text[0], text[1]),
  visual,
  ask: lessonPair(ask[0], ask[1]),
  options: options.map(([uz, ru = uz]) => lessonPair(uz, ru)),
  correct,
  hint: lessonPair(hint[0], hint[1]),
});

// Public metadata is consumed by grade3/practice/theoryDerivedBanks.js.
export const SCREENS = [
  practiceScreen('hook', ['Ustun terminali', 'Терминал столбиком'],
    ["Bit 34 × 6 ni yozma usulda hisoblamoqchi.", 'Бит хочет письменно вычислить 34 × 6.'],
    '34 × 6 = ?', ["Avval nima qilamiz?", 'Что делаем сначала?'],
    [["Xonalarni tekislaymiz", 'Выравниваем разряды'], ["34 + 6 ni topamiz", 'Находим 34 + 6'], ["Raqamlarni aralashtiramiz", 'Перемешиваем цифры']], 0,
    ["Bir xil xonalar bir ustunda turadi.", 'Одинаковые разряды стоят в одном столбце.']),
  practiceScreen('exploration', ['Birlikdan boshlaymiz', 'Начинаем с единиц'],
    ["6 birlik 4 birlik ostida turadi.", '6 единиц стоят под 4 единицами.'],
    ' 34\n× 6\n───', ["6 qayerda turadi?", 'Где стоит 6?'],
    [["4 ostida", 'Под 4'], ["3 ostida", 'Под 3'], ["O'rtada", 'Посередине']], 0,
    ["6 — birlik, u birlik ostida turadi.", '6 — единицы, поэтому стоит под единицами.']),
  practiceScreen('exploration', ["O'tkazish", 'Перенос'],
    ["6 × 4 = 24: 4 ni yozamiz, 2 o'nlikni o'tkazamiz.", '6 × 4 = 24: пишем 4, переносим 2 десятка.'],
    '6 × 4 = 24', ["Birlikka qaysi raqam yoziladi?", 'Какую цифру пишем в единицах?'],
    [['4'], ['2'], ['6']], 0,
    ["24 ning birlik raqami 4.", 'Цифра единиц числа 24 — это 4.']),
  practiceScreen('exploration', ["O'nlikni hisoblaymiz", 'Считаем десятки'],
    ["6 × 3 + 2 = 20 o'nlik.", '6 × 3 + 2 = 20 десятков.'],
    '18 + 2 = 20', ["34 × 6 nechaga teng?", 'Чему равно 34 × 6?'],
    [['184'], ['204'], ['224']], 1,
    ["4 birlik va 20 o'nlik — 204.", '4 единицы и 20 десятков — это 204.']),
  practiceScreen('rule', ["Ko'paytirish qoidasi", 'Правило умножения'],
    ["O'ngdan boshlaymiz va o'nlikni keyingi xonaga o'tkazamiz.", 'Начинаем справа и переносим десятки в следующий разряд.'],
    "birlik → o'nlik → yuzlik", ["Qayerdan boshlaymiz?", 'Откуда начинаем?'],
    [["Birlikdan", 'С единиц'], ["Yuzlikdan", 'С сотен'], ["Istalgan joydan", 'С любого разряда']], 0,
    ["Ko'paytirish o'ngdan boshlanadi.", 'Умножение начинаем справа.']),
  practiceScreen('test', ["O'tkazishsiz", 'Без переноса'],
    ["21 × 3 ni xonalar bo'yicha hisoblang.", 'Вычисли 21 × 3 по разрядам.'],
    '21 × 3', ["Natijani toping.", 'Найди результат.'],
    [['63'], ['33'], ['61']], 0,
    ["3 × 1 = 3, 3 × 2 o'nlik = 6 o'nlik.", '3 × 1 = 3, а 3 × 2 десятка = 6 десятков.']),
  practiceScreen('test', ["Bitta o'tkazish", 'Один перенос'],
    ["27 × 4 da 8 ni yozib, 2 ni o'tkazamiz.", 'В 27 × 4 пишем 8 и переносим 2.'],
    '27 × 4', ["Natija qaysi?", 'Каков результат?'],
    [['88'], ['108'], ['118']], 1,
    ["4 × 2 o'nlikka o'tgan 2 ni qo'shing.", 'К 4 × 2 десяткам прибавь перенесённые 2.']),
  practiceScreen('exploration', ["Yozma bo'lish", 'Письменное деление'],
    ["96 ÷ 4 ni chapdan boshlaymiz.", '96 ÷ 4 начинаем слева.'],
    '96 ÷ 4', ["9 ichida nechta to'liq 4 bor?", 'Сколько полных четвёрок в 9?'],
    [['1'], ['2'], ['3']], 1,
    ["4 × 2 = 8, 4 × 3 esa 9 dan katta.", '4 × 2 = 8, а 4 × 3 больше 9.']),
  practiceScreen('exploration', ["Raqamni tushiramiz", 'Сносим цифру'],
    ["1 o'nlik va 6 birlik — 16.", '1 десяток и 6 единиц — это 16.'],
    "1 o'nlik + 6 birlik = 16", ["16 ÷ 4 nechaga teng?", 'Чему равно 16 ÷ 4?'],
    [['2'], ['4'], ['6']], 1,
    ["4 × 4 = 16.", '4 × 4 = 16.']),
  practiceScreen('rule', ["Bo'lish qoidasi", 'Правило деления'],
    ["Bo'lamiz, ko'paytiramiz, ayiramiz, tushiramiz.", 'Делим, умножаем, вычитаем, сносим.'],
    "bo'l → ko'paytir → ayir → tushir", ["96 ÷ 4 nechaga teng?", 'Чему равно 96 ÷ 4?'],
    [['22'], ['24'], ['26']], 1,
    ["Bo'linmada 2 o'nlik va 4 birlik bor.", 'В частном 2 десятка и 4 единицы.']),
  practiceScreen('test', ["Bo'lish mashqi", 'Тренируем деление'],
    ["84 ni 4 ga bo'ling.", 'Раздели 84 на 4.'],
    '84 ÷ 4', ["Natijani tanlang.", 'Выбери результат.'],
    [['20'], ['21'], ['24']], 1,
    ["8 o'nlik ÷ 4 = 2 o'nlik; 4 ÷ 4 = 1.", '8 десятков ÷ 4 = 2 десятка; 4 ÷ 4 = 1.']),
  practiceScreen('test', ['Xatoni toping', 'Найди ошибку'],
    ["36 × 4 = 124 da o'tkazilgan 2 unutilgan.", 'В 36 × 4 = 124 забыли перенесённые 2.'],
    '36 × 4 = 124 ✗', ["Xato nimada?", 'В чём ошибка?'],
    [["O'tgan 2 qo'shilmagan", 'Не прибавили перенесённые 2'], ["6 noto'g'ri", 'Неверна цифра 6'], ["Bo'lish kerak", 'Нужно делить']], 0,
    ["4 × 3 ga o'tgan 2 ni qo'shing.", 'К 4 × 3 прибавь перенесённые 2.']),
  practiceScreen('case', ['Ustaxona masalasi', 'Задача мастерской'],
    ["4 qurilmaga 24 tadan detal kerak.", 'Для 4 устройств нужно по 24 детали.'],
    '24 × 4', ["Jami nechta detal?", 'Сколько деталей всего?'],
    [['86'], ['96'], ['106']], 1,
    ["4 × 4 = 16; keyin 4 × 2 + 1.", '4 × 4 = 16; затем 4 × 2 + 1.']),
  practiceScreen('test', ['Yakuniy tekshiruv', 'Итоговая проверка'],
    ["156 modul 3 ta tokchaga teng bo'linadi.", '156 модулей делят поровну на 3 полки.'],
    '156 ÷ 3', ["Har tokchada nechta?", 'Сколько на каждой полке?'],
    [['42'], ['52'], ['62']], 1,
    ["15 ÷ 3 = 5 o'nlik, 6 ÷ 3 = 2.", '15 ÷ 3 = 5 десятков, 6 ÷ 3 = 2.']),
  practiceScreen('summary', ['Terminal ishga tushdi', 'Терминал запущен'],
    ["Yozma ko'paytirish va bo'lish algoritmlari bajarildi.", 'Алгоритмы письменного умножения и деления выполнены.'],
    '× ↔ ÷', ["Asosiy tayanch nima?", 'Что главное?'],
    [["Xonalarni tekislash", 'Выравнивание разрядов'], ["Raqamlarni bezash", 'Украшение цифр'], ["Tasodifiy taxmin", 'Случайная догадка']], 0,
    ["Bir xil xonalar bir ustunda turadi.", 'Одинаковые разряды стоят в одном столбце.']),
];
...
const CONTENT = {
  ...
  s1: {
    ...
    audio: {
      ru: [
        'Начнём с того, что тебе уже знакомо с первого класса. В двузначном числе слева десятки, справа единицы. В числе семьдесят два семь десятков и две единицы. Молодец, это ты помнишь.',
  ...
  s4: {
    ...
    audio: {
      ru: [
        ...
        'Триста. Сорок. Пять.',
        'Показали по-разному, а число одно — триста сорок пять. Молодец!'
      ],
      uz: [
  ...
  s6: {
    eyebrow: { ru: 'Главный проспект', uz: "Bosh ko'cha" },
    lead: { ru: 'Где на проспекте дом 470?', uz: "470-uy ko'chaning qayerida?" },
    q: { ru: 'Проспект идёт от дома 300 до дома 800. Где дом 470? Нажми.', uz: "Ko'cha 300-uydan 800-uygacha. 470-uy qayerda? Bosing." },
    q_audio: { ru: 'Проспект идёт от дома триста до дома восемьсот. Как думаешь, где на нём стоит дом четыреста семьдесят? Нажми туда, где считаешь.', uz: "Ko'cha uch yuzinchi uydan sakkiz yuzinchi uygacha. Sizningcha, to'rt yuz yetmishinchi uy qayerda turadi? O'zingiz o'ylagan joyni bosing." },
    ...
    audio: {
      ru: [
        'Покажем дом четыреста семьдесят. Проспект идёт от трёхсот до восьмисот.',
      ...
      uz: [
        "To'rt yuz yetmishinchi uyni ko'rsatamiz. Ko'cha uch yuzdan sakkiz yuzgacha.",
  ...
  sming: {
    eyebrow: { ru: 'Открытие', uz: 'Kashfiyot' },
    lead: { ru: 'А если включить десять панелей?', uz: "Agar o'nta panelni yoqsak-chi?" },
    ...
    audio: {
      ru: [
        ...
        'А теперь вопрос тебе, дорогой ученик. В городе десять панелей. Сколько это огней?',
        'Даю тебе пять секунд на размышление. Смотри на часы.',
      ...
      uz: [
        ...
        "Endi sizga savol, aziz o'quvchi. Shaharda o'nta panel bor. Bu nechta chiroq bo'ladi?",
  ...
  s7: {
    ...
    audio: {
      ru: [
        'Отлично, теперь всё понятно! Пришло время запомнить это как правило, оно нам будет нужно всегда. Слушай внимательно.',
  ...
  s14: {
    ...
    items: [
      ...
      },
      {
        ...
        topic: { ru: 'разряд сотен', uz: "yuzliklar xonasi" },
      ...
      },
      {
        ...
        topic: { ru: 'сравнение чисел', uz: "sonlarni taqqoslash" },
        opt0: { ru: '591', uz: '591' },
        opt1: { ru: '519', uz: '519' },
...
// Dars21 keeps the complete Dars01 engine and replaces only lesson data.
// Long explanations live in audio; visible copy stays short.
const D21_CONTENT = {
  s0: {
    eyebrow: { ru: 'Миссия', uz: 'Missiya' },
    topic: { ru: 'Тема: письменное умножение и деление', uz: "Mavzu: yozma ko'paytirish va bo'lish" },
    lead: { ru: 'Терминал мастерской сбился!', uz: 'Ustaxona terminali adashdi!' },
    q: { ru: 'С чего начать 34 × 6?', uz: "34 × 6 ni nimadan boshlaymiz?" },
    opt0: { ru: 'Сложить 34 и 6', uz: "34 bilan 6 ni qo'shish" },
    opt1: { ru: 'Записать по разрядам', uz: 'Xonalar bo‘yicha yozish' },
    opt2: { ru: 'Не знаю', uz: 'Bilmayman' },
    audio: {
      intro: {
        ru: [
          'Сегодня научимся письменно умножать и делить. Так удобно считать большие числа точно и без спешки.',
          'В мастерской Бита терминал получил пример тридцать четыре умножить на шесть, но перепутал разряды.',
          'Сначала оценим ответ. Тридцать четыре близко к тридцати. Тридцать умножить на шесть — сто восемьдесят. Значит ответ должен быть около двухсот.',
          'Наша миссия — восстановить терминал: ровно записать разряды, выполнить шаги и проверить результат обратным действием.'
        ],
        uz: [
          "Bugun yozma ko'paytirish va bo'lishni o'rganamiz. Bu katta sonlarni aniq va shoshmasdan hisoblashga yordam beradi.",
          "Bit ustaxonasidagi terminal o'ttiz to'rtni oltiga ko'paytirish misolida xonalarni aralashtirib yubordi.",
          "Avval javobni chamalaymiz. O'ttiz to'rt o'ttizga yaqin. O'ttizni oltiga ko'paytirsak, bir yuz sakson bo'ladi. Demak javob ikki yuzga yaqin.",
          "Missiyamiz — terminalni tiklash: xonalarni tekis yozish, qadamlarni bajarish va natijani teskari amal bilan tekshirish."
        ]
      },
      on_correct: { ru: 'Верно. Одинаковые разряды ставим друг под другом.', uz: "To'g'ri. Bir xil xonalarni bir-birining ostiga yozamiz." },
      on_wrong: { ru: 'Сложение здесь не подходит. Нужно умножить 34 шесть раз и сохранить разряды.', uz: "Bu yerda qo'shish mos emas. 34 ni olti marta olish va xonalarni saqlash kerak." },
      on_unknown: { ru: 'Ничего. Сейчас Бит покажет запись по разрядам.', uz: "Hechqisi yo'q. Hozir Bit xonalar bo'yicha yozishni ko'rsatadi." }
    }
  },
  s1: {
    eyebrow: { ru: 'Разбор', uz: 'Tahlil' },
    lead: { ru: '34 × 6 — по разрядам.', uz: "34 × 6 — xonalar bo'yicha." },
    tens_label: { ru: 'десятки', uz: "o'nliklar" },
    ones_label: { ru: 'единицы', uz: 'birliklar' },
    recall_eq: { ru: '34 = 3 десятка и 4 единицы', uz: "34 = 3 o'nlik va 4 birlik" },
    unit_eq: { ru: '34 × 6 = 204', uz: '34 × 6 = 204' },
    audio: {
      ru: [
        'Вспомним смысл. Тридцать четыре — это три десятка и четыре единицы.',
        'Умножаем четыре единицы на шесть. Получаем двадцать четыре единицы.',
        'Двадцать четыре единицы — это два десятка и четыре единицы. Четыре пишем, два десятка переносим.',
        'Три десятка умножаем на шесть, получаем восемнадцать десятков. Добавляем два перенесённых десятка: двадцать десятков, или двести. Вместе с четырьмя единицами получаем двести четыре.'
      ],
      uz: [
        "Ma'nosini eslaymiz. O'ttiz to'rt — uch o'nlik va to'rt birlik.",
        "To'rt birlikni oltiga ko'paytiramiz. Yigirma to'rt birlik chiqadi.",
        "Yigirma to'rt birlik — ikki o'nlik va to'rt birlik. To'rtni yozamiz, ikki o'nlikni o'tkazamiz.",
        "Uch o'nlikni oltiga ko'paytiramiz, o'n sakkiz o'nlik chiqadi. O'tgan ikki o'nlikni qo'shamiz: yigirma o'nlik, ya'ni ikki yuz. To'rt birlik bilan ikki yuz to'rt bo'ladi."
      ]
    }
  },
  s3: {
    eyebrow: { ru: 'Собери ответ', uz: "Javobni yig'ing" },
    lead: { ru: 'Собери результат 34 × 6.', uz: "34 × 6 natijasini yig'ing." },
    hundreds_label: { ru: 'сотни', uz: 'yuzliklar' },
    tens_label: { ru: 'десятки', uz: "o'nliklar" },
    ones_label: { ru: 'единицы', uz: 'birliklar' },
    done_text: { ru: 'Получилось 204: две сотни, ноль десятков и четыре единицы.', uz: "204 chiqdi: ikki yuzlik, nol o'nlik va to'rt birlik." },
    audio: {
      ru: [
        'Теперь собери результат сам. В ответе две сотни.',
        'Отдельных десятков не осталось, поэтому в десятках нужен ноль. Единиц четыре.',
        'Поставь в разряды цифры два, ноль и четыре.'
      ],
      uz: [
        "Endi natijani o'zingiz yig'ing. Javobda ikki yuzlik bor.",
        "Alohida o'nlik qolmadi, shuning uchun o'nlik xonasida nol turadi. Birlik to'rtta.",
        "Xonalarga ikki, nol va to'rt raqamlarini qo'ying."
      ]
    }
  },
  s4: {
    eyebrow: { ru: 'Три проверки', uz: 'Uchta tekshiruv' },
    lead: { ru: 'Один пример — три взгляда.', uz: 'Bitta misol — uchta qarash.' },
    hundreds_label: { ru: 'сотни', uz: 'yuzliklar' },
    tens_label: { ru: 'десятки', uz: "o'nliklar" },
    ones_label: { ru: 'единицы', uz: 'birliklar' },
    m1_label: { ru: 'Способ 1 — по частям', uz: "1-usul — bo'laklab" },
    m1_text: { ru: '30 × 6 и 4 × 6.', uz: '30 × 6 va 4 × 6.' },
    m2_label: { ru: 'Способ 2 — столбиком', uz: '2-usul — ustunlab' },
    m2_text: { ru: 'Пишем 4, переносим 2.', uz: "4 ni yozamiz, 2 ni o'tkazamiz." },
    m3_label: { ru: 'Проверка — делением', uz: "Tekshiruv — bo'lish bilan" },
    m3_text: { ru: 'Обратное действие возвращает 34.', uz: 'Teskari amal 34 ni qaytaradi.' },
    m3_parts: [
      { num: '204', ru: 'делим', uz: "bo'lamiz" },
      { num: '6', ru: 'делитель', uz: "bo'luvchi" },
      { num: '34', ru: 'вернулись', uz: 'qaytdik' }
    ],
    audio: {
      ru: [
        'Разберём один пример тремя способами. Они должны дать один ответ.',
        'Первый способ — по частям.',
        'Тридцать умножить на шесть — сто восемьдесят. Четыре умножить на шесть — двадцать четыре.',
        'Второй способ — столбиком.',
        'Шесть умножить на четыре — двадцать четыре. Пишем четыре, два переносим. Шесть умножить на три и ещё два — двадцать.',
        'Оба способа дали двести четыре.',
        'Теперь проверка обратным действием.',
        'Двести четыре разделить на шесть — тридцать четыре.',
        'Получили исходный множитель. Значит вычисление верно.'
      ],
      uz: [
        "Bitta misolni uch usulda ko'ramiz. Ularning javobi bir xil bo'lishi kerak.",
        "Birinchi usul — bo'laklab.",
        "O'ttizni oltiga ko'paytirsak bir yuz sakson. To'rtni oltiga ko'paytirsak yigirma to'rt.",
        "Ikkinchi usul — ustunlab.",
        "Olti karra to'rt — yigirma to'rt. To'rtni yozamiz, ikkini o'tkazamiz. Olti karra uch va yana ikki — yigirma.",
        "Ikkala usul ham ikki yuz to'rt berdi.",
        "Endi teskari amal bilan tekshiramiz.",
        "Ikki yuz to'rtni oltiga bo'lsak, o'ttiz to'rt chiqadi.",
        "Boshlang'ich ko'paytuvchiga qaytdik. Demak hisob to'g'ri."
      ]
    }
  },
  s5: {
    eyebrow: { ru: 'Выравнивание', uz: 'Tekislash' },
    lead: { ru: 'Место множителя решает.', uz: "Ko'paytuvchining o'rni muhim." },
    hundreds_label: { ru: 'сотни', uz: 'yuzliklar' },
    tens_label: { ru: 'десятки', uz: "o'nliklar" },
    ones_label: { ru: 'единицы', uz: 'birliklar' },
    done_text: { ru: 'В 34 × 6 цифра 6 стоит строго под 4 — единицы под единицами.', uz: "34 × 6 da 6 raqami 4 ning tagida turadi — birlik ostiga birlik." },
    audio: {
      ru: [
        'В письменной записи место цифры меняет её значение.',
        'Шесть под единицами означает умножить на шесть.',
        'Сдвинем шестёрку в десятки — это уже умножение на шестьдесят.',
        'Ещё один сдвиг — умножение на шестьсот.',
        'В нашем примере множитель шесть — единицы. Поэтому пишем его строго под цифрой четыре.'
      ],
      uz: [
        "Yozma hisobda raqamning o'rni uning qiymatini o'zgartiradi.",
        "Birlik ostidagi olti — oltiga ko'paytirish degani.",
        "Oltini o'nlikka siljitsak, endi oltmishga ko'paytirgan bo'lamiz.",
        "Yana bir xona siljitsak — olti yuzga ko'paytirish bo'ladi.",
        "Bizning misolda ko'paytuvchi olti — birlik. Shuning uchun uni to'rtning tagiga yozamiz."
      ]
    }
  },
  s6: {
    eyebrow: { ru: 'Оценка', uz: 'Chamalash' },
    lead: { ru: 'Где примерно ответ 204?', uz: '204 javobi taxminan qayerda?' },
    q: { ru: 'Отметь место на прямой.', uz: "Son o'qida joyni belgilang." },
    q_audio: { ru: 'До точного вычисления мы ожидали ответ около двухсот. Покажи на прямой, где примерно находится двести четыре.', uz: "Aniq hisoblashdan oldin javob ikki yuzga yaqin bo'lishini kutdik. Son o'qida ikki yuz to'rt taxminan qayerdaligini ko'rsating." },
    done_text: { ru: '204 рядом с 200. Оценка подтверждает ответ.', uz: '204 soni 200 ga yaqin. Chamalash javobni tasdiqlaydi.' },
    info_badge: { ru: 'Проверка', uz: 'Tekshiruv' },
    info: { ru: 'Если 34 × 6 получилось около 200, разряд ответа правдоподобен.', uz: "34 × 6 javobi 200 ga yaqin bo'lsa, javob xonasi mantiqan to'g'ri." },
    audio: {
      ru: [
        'Тридцать умножить на шесть — сто восемьдесят.',
        'Четыре умножить на шесть — ещё двадцать четыре.',
        'Сто восемьдесят плюс двадцать четыре — двести четыре. Точка чуть правее двухсот.'
      ],
      uz: [
        "O'ttizni oltiga ko'paytirsak bir yuz sakson.",
        "To'rtni oltiga ko'paytirsak yana yigirma to'rt.",
        "Bir yuz sakson bilan yigirma to'rt — ikki yuz to'rt. Nuqta ikki yuzdan sal o'ngda."
      ]
    }
  },
  sming: {
    eyebrow: { ru: 'Письменное деление', uz: "Yozma bo'lish" },
    lead: { ru: '96 модулей — на 4 полки.', uz: "96 modul — 4 ta tokchaga." },
    ming_q: { ru: '96 ÷ 4 = ?', uz: '96 ÷ 4 = ?' },
    ming_eq: { ru: '96 ÷ 4 = 24', uz: '96 ÷ 4 = 24' },
    ming_word: { ru: 'ПО 24', uz: '24 TADAN' },
    done_text: { ru: 'На каждой полке 24 модуля.', uz: 'Har tokchada 24 ta modul.' },
    audio: {
      ru: [
        'Теперь письменное деление. Девяносто шесть модулей нужно поровну поставить на четыре полки.',
        'Как думаешь, сколько будет на каждой полке?',
        'Даю пять секунд на размышление.',
        'Начинаем слева. В девяти десятках помещается две четвёрки. Два умножить на четыре — восемь. Из девяти вычитаем восемь, остаётся один десяток.',
        'Сносим шесть единиц. Один десяток и шесть единиц — шестнадцать. Шестнадцать разделить на четыре — четыре.',
        'В частном получили две десятки и четыре единицы: двадцать четыре. Проверка: двадцать четыре умножить на четыре — девяносто шесть.'
      ],
      uz: [
        "Endi yozma bo'lish. To'qson olti modulni to'rtta tokchaga teng joylash kerak.",
        "Sizningcha, har tokchada nechtadan bo'ladi?",
        "O'ylash uchun besh soniya beraman.",
        "Chapdan boshlaymiz. To'qqiz o'nlik ichida ikkita to'rtlik bor. Ikki karra to'rt — sakkiz. To'qqizdan sakkizni ayirsak, bir o'nlik qoladi.",
        "Olti birlikni tushiramiz. Bir o'nlik va olti birlik — o'n olti. O'n oltini to'rtga bo'lsak, to'rt.",
        "Bo'linmada ikki o'nlik va to'rt birlik oldik: yigirma to'rt. Tekshiruv: yigirma to'rt karra to'rt — to'qson olti."
      ]
    }
  },
  s7: {
    eyebrow: { ru: 'Правило', uz: 'Qoida' },
    rule: { ru: 'Умножение: справа налево, с переносом. Деление: слева направо — делю, умножаю, вычитаю, сношу.', uz: "Ko'paytirish: o'ngdan chapga, o'tkazish bilan. Bo'lish: chapdan o'ngga — bo'laman, ko'paytiraman, ayiraman, tushiraman." },
    hundreds_label: { ru: 'сотни', uz: 'yuzliklar' },
    tens_label: { ru: 'десятки', uz: "o'nliklar" },
    ones_label: { ru: 'единицы', uz: 'birliklar' },
    check_q: { ru: 'В 34 × 6 нажми разряд, с которого начинаем.', uz: "34 × 6 da qaysi xonadan boshlashni bosing." },
    check_ok: { ru: 'Верно. Умножение начинаем с единиц.', uz: "To'g'ri. Ko'paytirishni birlikdan boshlaymiz." },
    check_no: { ru: 'При умножении начинаем справа — с единиц.', uz: "Ko'paytirishda o'ngdan — birlikdan boshlaymiz." },
    audio: {
      ru: [
        'Запомним два направления.',
        'При письменном умножении выравниваем разряды и начинаем справа, с единиц.',
        'Если получаем десятки, пишем единицы, а десятки переносим в следующий разряд.',
        'При письменном делении начинаем слева, со старшего разряда.',
        'Повторяем четыре действия: делю, умножаю, вычитаю, сношу следующую цифру.',
        'Каждый результат проверяем обратным действием и оценкой.'
      ],
      uz: [
        "Ikki yo'nalishni eslab qolamiz.",
        "Yozma ko'paytirishda xonalarni tekislaymiz va o'ngdan, birlikdan boshlaymiz.",
        "O'nlik hosil bo'lsa, birlikni yozamiz, o'nlikni keyingi xonaga o'tkazamiz.",
        "Yozma bo'lishda chapdan, katta xonadan boshlaymiz.",
        "To'rt amalni takrorlaymiz: bo'laman, ko'paytiraman, ayiraman, keyingi raqamni tushiraman.",
        "Har bir natijani teskari amal va chamalash bilan tekshiramiz."
      ]
    }
  },
  s8: {
    eyebrow: { ru: 'Практика', uz: 'Mashq' },
    q: { ru: 'Вычисли и собери ответ.', uz: "Hisoblang va javobni yig'ing." },
    hundreds_label: { ru: 'сотни', uz: 'yuzliklar' },
    tens_label: { ru: 'десятки', uz: "o'nliklar" },
    ones_label: { ru: 'единицы', uz: 'birliklar' },
    check_label: { ru: 'Проверить', uz: 'Tekshirish' },
    audio: {
      intro: { ru: 'Три примера. Вычисли каждый письменно и собери цифры результата по разрядам.', uz: "Uchta misol. Har birini yozma hisoblang va natija raqamlarini xonalar bo'yicha yig'ing." },
      on_correct: { ru: 'Ответ собран верно.', uz: "Javob to'g'ri yig'ildi." },
      on_wrong: { ru: 'Бит советует ещё раз проверить единицы, перенос и разряд каждой цифры.', uz: "Bit birlikni, o'tkazishni va har bir raqam xonasini yana tekshirishni maslahat beradi." }
    }
  },
  s9: {
    eyebrow: { ru: 'Практика', uz: 'Mashq' },
    q: { ru: 'Разложи цифры ответа по разрядам.', uz: "Javob raqamlarini xonalarga ajrating." },
    hold_hundreds: { ru: 'СОТНИ', uz: 'YUZLIKLAR' },
    hold_tens: { ru: 'ДЕСЯТКИ', uz: "O'NLIKLAR" },
    hold_ones: { ru: 'ЕДИНИЦЫ', uz: 'BIRLIKLAR' },
    audio: {
      intro: { ru: 'Сначала посмотри демонстрацию. Затем сам поставь цифры вычисленного ответа в сотни, десятки и единицы. Две карточки лишние.', uz: "Avval namoyishni ko'ring. Keyin hisoblangan javob raqamlarini yuzlik, o'nlik va birlik xonalariga qo'ying. Ikkita kartochka ortiqcha." },
      on_correct: { ru: 'Разряды ответа стоят верно.', uz: "Javob xonalari to'g'ri joylashdi." },
      on_wrong: { ru: 'Бит просит сначала назвать весь ответ, а затем читать его слева направо по разрядам.', uz: "Bit avval butun javobni aytib, keyin uni chapdan o'ngga xonalar bo'yicha o'qishni so'raydi." }
    }
  },
  s10: {
    eyebrow: { ru: 'Шаги алгоритма', uz: 'Algoritm qadamlari' },
    items: [
      {
        q: { ru: 'В 34 × 6 получили 6 × 4 = 24. Что пишем в единицах?', uz: "34 × 6 da 6 × 4 = 24. Birlikka nimani yozamiz?" },
        hto: [0, 2, 4], ci: 0,
        opts: [{ ru: '4', uz: '4' }, { ru: '2', uz: '2' }, { ru: '24', uz: '24' }, { ru: '6', uz: '6' }],
        hints: [
          null,
          { ru: 'Два — это десятки, их переносим. В единицах числа 24 стоит 4.', uz: "Ikki — o'nlik, uni o'tkazamiz. 24 ning birligida 4 turadi." },
          { ru: 'В один разряд пишут одну цифру. Пишем 4, а 2 переносим.', uz: "Bitta xonaga bitta raqam yoziladi. 4 ni yozib, 2 ni o'tkazamiz." },
          { ru: 'Шесть — множитель. Нужна цифра единиц произведения 24.', uz: "Olti — ko'paytuvchi. 24 ko'paytmaning birlik raqami kerak." }
        ]
      },
      {
        q: { ru: 'Что переносим из числа 24?', uz: "24 sonidan nimani o'tkazamiz?" },
        hto: [0, 2, 4], ci: 1,
        opts: [{ ru: '4 десятка', uz: "4 o'nlik" }, { ru: '2 десятка', uz: "2 o'nlik" }, { ru: '24 десятка', uz: "24 o'nlik" }, { ru: 'Ничего', uz: 'Hech narsa' }],
        hints: [
          { ru: 'Четыре остаётся в единицах. Переносятся 2 десятка.', uz: "To'rt birlikda qoladi. 2 o'nlik o'tkaziladi." },
          null,
          { ru: 'Двадцать четыре состоит из 2 десятков и 4 единиц.', uz: "Yigirma to'rt 2 o'nlik va 4 birlikdan iborat." },
          { ru: 'Перенос нужен: произведение единиц больше девяти.', uz: "O'tkazish kerak: birliklar ko'paytmasi to'qqizdan katta." }
        ]
      },
      {
        q: { ru: 'В 96 ÷ 4 осталось 1 десяток и снесли 6. Что делим дальше?', uz: "96 ÷ 4 da 1 o'nlik qoldi va 6 tushirildi. Keyin nimani bo'lamiz?" },
        hto: [0, 1, 6], ci: 2,
        opts: [{ ru: '6', uz: '6' }, { ru: '10', uz: '10' }, { ru: '16', uz: '16' }, { ru: '96', uz: '96' }],
        hints: [
          { ru: 'Оставшийся десяток нельзя потерять. Вместе с 6 это 16.', uz: "Qolgan o'nlikni yo'qotib bo'lmaydi. 6 bilan birga 16 bo'ladi." },
          { ru: 'К одному десятку добавились 6 единиц: получилось 16.', uz: "Bir o'nlikka 6 birlik qo'shildi: 16 hosil bo'ldi." },
          null,
          { ru: 'Девяносто шесть уже разделили по старшему разряду. Теперь работаем с 16.', uz: "To'qson oltining katta xonasi bo'lindi. Endi 16 bilan ishlaymiz." }
        ]
      }
    ],
    audio: {
      intro: { ru: 'Выбери верный следующий шаг. Ошибку Бит объяснит, и можно сразу попробовать снова.', uz: "Keyingi to'g'ri qadamni tanlang. Xatoni Bit tushuntiradi va darhol yana urinishingiz mumkin." },
      on_correct: { ru: 'Шаг выбран верно.', uz: "Qadam to'g'ri tanlandi." },
      on_wrong: { ru: 'Проверь значение каждого разряда и попробуй ещё.', uz: "Har bir xonaning qiymatini tekshirib, yana urinib ko'ring." }
    }
  },
  s11: {
    eyebrow: { ru: 'Подбор частного', uz: "Bo'linmani tanlash" },
    items: [
      { pair: [8, 9], sign: '<', hint: { ru: 'Две четвёрки — восемь. Это меньше девяти, значит цифра 2 подходит.', uz: "Ikki karra to'rt — sakkiz. Bu to'qqizdan kichik, demak 2 raqami mos." } },
      { pair: [12, 9], sign: '>', hint: { ru: 'Три четвёрки — двенадцать. Это больше девяти, значит цифра 3 слишком велика.', uz: "Uch karra to'rt — o'n ikki. Bu to'qqizdan katta, demak 3 juda katta." } },
      { pair: [16, 16], sign: '=', hint: { ru: 'Четыре четвёрки — ровно шестнадцать.', uz: "To'rt karra to'rt — aynan o'n olti." } }
    ],
    audio: {
      intro: { ru: 'При делении подбираем наибольшее произведение, которое не больше текущей части делимого. Поставь знак.', uz: "Bo'lishda bo'linuvchining hozirgi qismidan katta bo'lmagan eng katta ko'paytmani tanlaymiz. Belgini qo'ying." },
      on_correct: { ru: 'Сравнение верно.', uz: "Taqqoslash to'g'ri." },
      on_wrong: { ru: 'Посчитай произведение и сравни его с текущей частью делимого.', uz: "Ko'paytmani hisoblab, bo'linuvchining hozirgi qismi bilan solishtiring." }
    }
  },
  s12: {
    eyebrow: { ru: 'Задача', uz: 'Masala' },
    lead: { ru: '4 устройства, по 24 детали.', uz: '4 ta qurilma, har biriga 24 tadan detal.' },
    manifest_label: { ru: 'заказ', uz: 'buyurtma' },
    audio: {
      ru: 'Для одного устройства нужно двадцать четыре детали. Устройств четыре. Нужно узнать общее число деталей, поэтому умножаем двадцать четыре на четыре.',
      uz: "Bitta qurilmaga yigirma to'rtta detal kerak. Qurilma to'rtta. Jami detallar sonini topish uchun yigirma to'rtni to'rtga ko'paytiramiz."
    }
  },
  s13: {
    eyebrow: { ru: 'Задача', uz: 'Masala' },
    q: { ru: 'Сколько деталей всего?', uz: 'Jami nechta detal?' },
    wrong_86: { ru: 'Ты записал 6 единиц, но потерял перенесённый десяток. После 4 × 4 = 16 пишем 6 и переносим 1.', uz: "6 birlikni yozdingiz, lekin o'tgan o'nlik yo'qoldi. 4 × 4 = 16 dan keyin 6 ni yozib, 1 ni o'tkazamiz." },
    wrong_106: { ru: 'Перенос прибавляется к десяткам: 4 × 2 + 1 = 9 десятков, а не 10.', uz: "O'tkazma o'nlikka qo'shiladi: 4 × 2 + 1 = 9 o'nlik, 10 emas." },
    wrong_default: { ru: 'Вычисли справа: 4 × 4 = 16, пишем 6, переносим 1. Затем 4 × 2 + 1.', uz: "O'ngdan hisoblang: 4 × 4 = 16, 6 ni yozib, 1 ni o'tkazamiz. Keyin 4 × 2 + 1." },
    audio: {
      intro: { ru: 'Реши письменно двадцать четыре умножить на четыре и набери ответ.', uz: "Yigirma to'rtni to'rtga yozma ko'paytirib, javobni tering." },
      on_correct: { ru: 'Верно, девяносто шесть деталей. Проверка: девяносто шесть разделить на четыре — двадцать четыре.', uz: "To'g'ri, to'qson olti detal. Tekshiruv: to'qson oltini to'rtga bo'lsak, yigirma to'rt." },
      on_wrong: { ru: 'Бит показывает перенос. Попробуй вычислить ещё раз.', uz: "Bit o'tkazishni ko'rsatmoqda. Yana hisoblab ko'ring." }
    }
  },
  s14: {
    eyebrow: { ru: 'Финал', uz: 'Final' },
    intro_line: { ru: 'Пять коротких заданий.', uz: 'Beshta qisqa topshiriq.' },
    items: [
      {
        kind: 'num', ans: 204,
        q: { ru: '34 × 6', uz: '34 × 6' },
        topic: { ru: 'письменное умножение', uz: "yozma ko'paytirish" },
        hint: { ru: '6 × 4 = 24: пишем 4, переносим 2. Затем 6 × 3 + 2.', uz: "6 × 4 = 24: 4 ni yozib, 2 ni o'tkazing. Keyin 6 × 3 + 2." }
      },
      {
        kind: 'num', ans: 25,
        q: { ru: '75 ÷ 3', uz: '75 ÷ 3' },
        topic: { ru: 'письменное деление', uz: "yozma bo'lish" },
        hint: { ru: '7 десятков ÷ 3 = 2 десятка, остаток 1 десяток. Снеси 5 и раздели 15.', uz: "7 o'nlik ÷ 3 = 2 o'nlik, 1 o'nlik qoldiq. 5 ni tushirib, 15 ni bo'ling." }
      },
      {
        kind: 'mc',
        q: { ru: '7 × 4 = 28. Что переносим?', uz: "7 × 4 = 28. Nimani o'tkazamiz?" },
        topic: { ru: 'перенос десятков', uz: "o'nlikni o'tkazish" },
        opt0: { ru: '2 десятка', uz: "2 o'nlik" },
        opt1: { ru: '8 десятков', uz: "8 o'nlik" },
        opt2: { ru: '28 десятков', uz: "28 o'nlik" },
        wrong_1: { ru: 'Восемь — единицы, их пишем. Переносим 2 десятка.', uz: "Sakkiz — birlik, uni yozamiz. 2 o'nlikni o'tkazamiz." },
        wrong_2: { ru: 'Двадцать восемь — это 2 десятка и 8 единиц. Переносим только 2 десятка.', uz: "Yigirma sakkiz — 2 o'nlik va 8 birlik. Faqat 2 o'nlikni o'tkazamiz." }
      },
      {
        kind: 'mc',
        q: { ru: 'В 156 ÷ 3 с чего начинаем?', uz: "156 ÷ 3 da nimadan boshlaymiz?" },
        topic: { ru: 'первое неполное делимое', uz: "birinchi noto'liq bo'linuvchi" },
        opt0: { ru: 'С числа 15', uz: '15 sonidan' },
        opt1: { ru: 'С цифры 1', uz: '1 raqamidan' },
        opt2: { ru: 'С цифры 6', uz: '6 raqamidan' },
        wrong_1: { ru: 'Один меньше делителя 3, поэтому берём первые две цифры: 15.', uz: "Bir bo'luvchi 3 dan kichik, shuning uchun dastlabki ikki raqamni olamiz: 15." },
        wrong_2: { ru: 'Деление начинаем слева, а не с последней цифры.', uz: "Bo'lishni chapdan boshlaymiz, oxirgi raqamdan emas." }
      },
      {
        kind: 'num', ans: 52,
        q: { ru: '156 ÷ 3', uz: '156 ÷ 3' },
        topic: { ru: 'письменное деление', uz: "yozma bo'lish" },
        hint: { ru: '15 ÷ 3 = 5 десятков. Снеси 6: 6 ÷ 3 = 2.', uz: "15 ÷ 3 = 5 o'nlik. 6 ni tushiring: 6 ÷ 3 = 2." }
      }
    ],
    fact_badge: { ru: 'Проверка', uz: 'Tekshiruv' },
    fact_text: { ru: 'Умножение и деление — обратные действия: 204 ÷ 6 возвращает 34.', uz: "Ko'paytirish va bo'lish teskari amallar: 204 ÷ 6 yana 34 ni beradi." },
    fact_audio: { ru: 'Проверяй обратным действием. Двести четыре разделить на шесть — тридцать четыре.', uz: "Teskari amal bilan tekshiring. Ikki yuz to'rtni oltiga bo'lsak, o'ttiz to'rt chiqadi." },
    audio: {
      intro: { ru: 'Финальная проверка. Решай по разрядам. После ошибки Бит объяснит шаг, и можно ответить снова.', uz: "Yakuniy tekshiruv. Xonalar bo'yicha yeching. Xatodan keyin Bit qadamni tushuntiradi va yana javob berish mumkin." },
      on_correct: { ru: 'Верно.', uz: "To'g'ri." },
      on_wrong: { ru: 'Проверь шаг и попробуй ещё раз.', uz: "Qadamni tekshirib, yana urinib ko'ring." }
    }
  },
  s15: {
    eyebrow: { ru: 'Итог', uz: 'Yakun' },
    praise: { ru: 'Отличная работа!', uz: "Ajoyib ish!" },
    mission_done: { ru: 'Письменный терминал запущен!', uz: 'Yozma hisob terminali ishga tushdi!' },
    cando: { ru: 'Ты умеешь умножать и делить столбиком.', uz: "Siz ustunlab ko'paytirish va bo'lishni bilasiz." },
    rule_recap: { ru: 'Умножение — справа. Деление — слева. Проверка — обратным действием.', uz: "Ko'paytirish — o'ngdan. Bo'lish — chapdan. Tekshiruv — teskari amal bilan." },
    conn_label_refs: { ru: 'Опирается на', uz: 'Tayanadi' },
    conn_refs: { ru: 'умножение по частям, деление по частям и обратные действия', uz: "bo'laklab ko'paytirish, bo'laklab bo'lish va teskari amallar" },
    conn_label_next: { ru: 'Дальше', uz: 'Keyingi' },
    conn_next: { ru: 'урок 22: двузначное число умножаем на двузначное', uz: "22-dars: ikki xonali sonni ikki xonali songa ko'paytirish" },
    audio: {
      ru: 'Терминал снова работает. При умножении выравнивай разряды, начинай с единиц и переноси десятки. При делении начинай слева и повторяй: делю, умножаю, вычитаю, сношу. Проверяй ответ обратным действием и оценкой. В следующем уроке умножим двузначное число на двузначное.',
      uz: "Terminal yana ishlayapti. Ko'paytirishda xonalarni tekislang, birlikdan boshlang va o'nlikni o'tkazing. Bo'lishda chapdan boshlang va takrorlang: bo'laman, ko'paytiraman, ayiraman, tushiraman. Javobni teskari amal va chamalash bilan tekshiring. Keyingi darsda ikki xonali sonni ikki xonali songa ko'paytiramiz."
    }
  }
};

Object.entries(D21_CONTENT).forEach(([key, value]) => {
  CONTENT[key] = { ...CONTENT[key], ...value };
});

// slaydlararo ko'priklar (audio-intro boshiga; ekranda ko'rinmaydi). TTS-toza.
const BRIDGES = {
  s1:  { ru: 'Сначала поймём смысл переноса.', uz: "Avval o'tkazish ma'nosini tushunamiz." },
  s2:  { ru: 'Теперь запишем это короче.', uz: 'Endi buni qisqa yozamiz.' },
  s3:  { ru: 'Собери цифры полученного ответа.', uz: "Hosil bo'lgan javob raqamlarini yig'ing." },
  s4:  { ru: 'Сравним способы и проверку.', uz: 'Usullar va tekshiruvni solishtiramiz.' },
  s5:  { ru: 'Проверь, где стоит множитель.', uz: "Ko'paytuvchi qayerda turganini tekshiring." },
  s6:  { ru: 'Оценим, правдоподобен ли ответ.', uz: 'Javob mantiqan to‘g‘riligini chamalaymiz.' },
  sming: { ru: 'Теперь перейдём к письменному делению.', uz: "Endi yozma bo'lishga o'tamiz." },
  s7:  { ru: 'Соберём два алгоритма в правило.', uz: 'Ikki algoritmni qoida qilamiz.' },
  s8:  { ru: 'Пора вычислять самостоятельно.', uz: 'Endi mustaqil hisoblaymiz.' },
  s9:  { ru: 'Следи за разрядами ответа.', uz: 'Javob xonalarini kuzating.' },
  s10: { ru: 'Разберём отдельные шаги.', uz: 'Alohida qadamlarni ko‘ramiz.' },
  s11: { ru: 'Подберём цифру частного сравнением.', uz: "Bo'linma raqamini taqqoslab tanlaymiz." },
  s12: { ru: 'Применим алгоритм в задаче.', uz: 'Algoritmni masalada qo‘llaymiz.' },
  s13: { ru: 'Набери вычисленный ответ.', uz: 'Hisoblangan javobni tering.' },
  s14: { ru: 'Терминал проведёт финальную проверку.', uz: 'Terminal yakuniy tekshiruv o‘tkazadi.' },
  s15: { ru: 'Терминал восстановлен.', uz: 'Terminal tiklandi.' }
};

// s15 payoff (xulosadan oldin aytiladi)
const S15_PAYOFF = {
  ru: 'Миссия выполнена! Письменный терминал снова считает точно. Спасибо за помощь!',
  uz: "Missiya bajarildi! Yozma hisob terminali yana aniq ishlayapti. Yordamingiz uchun rahmat!"
};

// Lumo yo'l-xaritasi yozuvi (lang-lookup)
const READY_LABEL = { ru: 'Письменный терминал', uz: 'Yozma hisob terminali' };

const D21_ETALON_FEEDBACK = `
/* Dars01 feedback flow: the Bit card follows the task and never covers the scene. */
.lesson-root.d21-lesson .feedback-block.visible {
...
.lesson-root.d21-lesson .feedback-block.visible > .frame-tip {
  background: #FBF3D6 !important;
  border: 0 !important;
  border-left: 4px solid #D8A93A !important;
  color: inherit !important;
  box-shadow: 0 6px 16px -6px rgba(180, 138, 30, 0.22) !important;
}
.lesson-root.d21-lesson .feedback-block.visible .g1-bitcard-enc .g1-bitcard-txt {
...
const ColumnAlgorithm = ({ phase = 2, shifted = 0, compact = false }) => {
  const digitStyle = {
    display: 'grid',
    gridTemplateColumns: 'repeat(3, 1.2em)',
    justifyContent: 'end',
    textAlign: 'center',
    fontSize: compact ? 'clamp(18px, 4vw, 26px)' : 'clamp(24px, 5vw, 34px)',
    fontWeight: 800,
    lineHeight: 1.1,
  };
  return (
    <div className="mono" aria-label="34 умножить на 6 равно 204"
      style={{ minWidth: compact ? 132 : 166, padding: compact ? '8px 14px' : '12px 18px', borderRadius: 16, background: '#FFFDF8', border: `2px solid ${T.ink3}`, boxShadow: '0 8px 20px rgba(58,53,48,.08)' }}>
      <div style={{ ...digitStyle, minHeight: '1.1em', color: T.accent }}>
        <span/><span>{phase >= 1 && shifted === 0 ? '2' : ''}</span><span/>
      </div>
      <div style={digitStyle}><span/><span>3</span><span>4</span></div>
      <div style={{ ...digitStyle, transform: `translateX(${-shifted * 1.2}em)`, transition: 'transform .55s cubic-bezier(.34,1.3,.64,1)' }}>
        <span style={{ color: T.accent }}>×</span><span/><span>6</span>
      </div>
      <div style={{ borderTop: `3px solid ${T.ink}`, margin: '5px 0 6px' }}/>
      <div style={{ ...digitStyle, color: phase >= 2 ? T.ink : T.ink2 }}>
        <span>{phase >= 2 ? '2' : ''}</span>
        <span>{phase >= 2 ? '0' : ''}</span>
        <span>{phase >= 1 ? '4' : '?'}</span>
      </div>
...
const DivisionAlgorithm = ({ phase = 3, compact = false }) => (
  <div className="mono" aria-label="96 разделить на 4 равно 24"
    style={{ minWidth: compact ? 170 : 210, padding: compact ? '10px 14px' : '14px 20px', borderRadius: 16, background: '#FFFDF8', border: `2px solid ${T.ink3}`, boxShadow: '0 8px 20px rgba(58,53,48,.08)', fontSize: compact ? 'clamp(17px, 3.8vw, 24px)' : 'clamp(22px, 4.8vw, 31px)', fontWeight: 800 }}>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'start', gap: 8 }}>
      <div style={{ textAlign: 'right' }}>
        <div>96</div>
        {phase >= 1 && <div className="lm-reveal" style={{ color: T.accent }}>−8</div>}
        {phase >= 1 && <div style={{ borderTop: `2px solid ${T.ink}`, paddingTop: 2 }}>{phase >= 2 ? '16' : '1'}</div>}
        {phase >= 2 && <div className="lm-reveal" style={{ color: T.accent }}>−16</div>}
        {phase >= 2 && <div style={{ borderTop: `2px solid ${T.ink}`, paddingTop: 2 }}>0</div>}
      </div>
      <div style={{ borderLeft: `3px solid ${T.ink}`, height: '100%', minHeight: 42 }}/>
      <div>
        <div>4</div>
        <div style={{ borderTop: `3px solid ${T.ink}`, color: phase >= 1 ? T.success : T.ink2 }}>{phase >= 1 ? '24' : '?'}</div>
      </div>
    </div>
  </div>
);
...
const HookScene = ({ gathered = false }) => {
  ...
  return (
    <div className="lm-scene">
      <LumoCityBg fill/>
      <div className="mono" style={{ position: 'absolute', zIndex: 4, top: '8%', left: '50%', transform: 'translateX(-50%)', padding: '6px 16px', borderRadius: 12, background: 'rgba(255,253,248,.94)', border: '2px solid rgba(255,79,40,.35)', boxShadow: '0 7px 18px rgba(20,18,16,.16)', color: gathered ? T.success : T.accent, fontSize: 'clamp(16px, 3vw, 24px)', fontWeight: 800, whiteSpace: 'nowrap' }}>
        {gathered ? '34 × 6 = 204' : '34 × 6 = ?'}
      </div>
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="fade-up delay-1" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
          {opts.map((o, i) => (
              <button key={i} className="option" disabled={!canAct || ok} onClick={() => pick(i)}
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(12px, 2.4vw, 20px)', minHeight: 'clamp(180px, 38vw, 240px)' }}>
          <ColumnAlgorithm phase={reached >= 3 ? 2 : reached >= 1 ? 1 : 0}/>
          <span className="mono lm-eq lm-reveal" style={{ fontSize: 'clamp(14px, 2.5vw, 20px)', fontWeight: 800, color: reached >= 3 ? T.success : T.ink2 }}>
            {reached >= 3 ? t(c.unit_eq) : reached >= 1 ? '24 = 2 · 10 + 4' : t(c.recall_eq)}
          </span>
        </div>
      </div>
    </Stage>
...
// M1: multiplication by place-value parts.
const M1Drop = () => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(8px, 2vw, 14px)' }}>
    <span className="mono lm-reveal" style={{ fontSize: 'clamp(22px, 5vw, 32px)', fontWeight: 800, color: T.ink2 }}>34 × 6</span>
    <div style={{ display: 'flex', gap: 'clamp(8px, 2.4vw, 18px)' }}>
      {[['30 × 6', '180', '#C0392B'], ['4 × 6', '24', T.blue], ['180 + 24', '204', '#1F7A4D']].map(([expr, ans, col], i) => (
        <div key={expr} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          <span className="mono lm-edrop" style={{ animationDelay: `${0.1 + i * 0.7}s`, fontSize: 'clamp(14px, 3vw, 20px)', fontWeight: 800, color: col, border: `2.5px solid ${col}`, borderRadius: 10, minWidth: 'clamp(74px, 18vw, 108px)', textAlign: 'center', padding: '5px 7px', background: T.paper }}>{expr}</span>
          <span className="mono" style={{ fontSize: 'clamp(13px, 2vw, 17px)', color: T.ink, fontWeight: 800 }}>= {ans}</span>
        </div>
      ))}
    </div>
  </div>
);
// M2: the same calculation in a column.
const M2Drop = () => (
  <div className="lm-edrop" style={{ animationDelay: '0.1s' }}><ColumnAlgorithm compact phase={2}/></div>
);
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(12px, 2.4vw, 20px)', minHeight: 'clamp(170px, 36vw, 230px)' }}>
          {!showM1 && <span className="mono" style={{ fontSize: 'clamp(30px, 7vw, 44px)', fontWeight: 800, color: T.ink }}>34 × 6</span>}
          {showM1 && (
...
// s5 — one digit in different columns means ×6, ×60 or ×600.
const S5_LABELS = ['× 6', '× 60', '× 600'];
const S5DigRow = ({ phase }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
    <ColumnAlgorithm compact phase={0} shifted={phase}/>
    <span className="mono lm-reveal" style={{ color: phase === 0 ? T.success : T.accent, fontSize: 'clamp(16px, 3vw, 22px)', fontWeight: 800 }}>{S5_LABELS[phase]}</span>
  </div>
);
const Screen5 = (props) => {
  const lang = useLang();
  ...
  const phase = Math.min(reached, 2);
  const done = audio.muted || reached >= (c.audio[lang].length - 1);
  const canAdv = useAdvanceGate(done, audio);
  const navContent = (
    <>
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        <h1 className="title h-sub fade-up">{t(c.lead)}</h1>
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.4vw, 18px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <S5DigRow phase={phase}/>
          <span className="mono" style={{ color: T.ink2, fontSize: 'clamp(12px, 1.8vw, 15px)', fontWeight: 700 }}>
            {lang === 'ru' ? 'единицы · десятки · сотни' : "birlik · o'nlik · yuzlik"}
          </span>
        </div>
        {done && (
...
const NumberLineAnim = ({ phase, guess = null, onGuess = null }) => {
  ...
  return (
    <svg ref={ref} onClick={handleClick} viewBox={`0 0 ${NL_W} 104`} style={{ width: 'min(360px, 98%)', height: 'auto', cursor: asking ? 'pointer' : 'default' }} aria-hidden={!asking}>
      ...
      {!asking && <line x1={nlx(NL_MIN)} y1={NL_y} x2={nlx(Math.min(pos, 180))} y2={NL_y} stroke={T.accent} strokeWidth="4" strokeLinecap="round" style={{ transition: 'all 0.6s' }}/>}
      {!asking && pos > 180 && <line x1={nlx(180)} y1={NL_y} x2={nlx(pos)} y2={NL_y} stroke={T.blue} strokeWidth="4" strokeLinecap="round" style={{ transition: 'all 0.6s' }}/>}
      ...
      {!asking && (
        <g style={{ opacity: phase >= 1 ? 1 : 0, transition: 'opacity 0.4s' }}>
          <path d={arc(0, 180, 30)} fill="none" stroke={T.accent} strokeWidth="2.5" strokeLinecap="round" strokeDasharray="3 3.5"/>
          <text x={(nlx(0) + nlx(180)) / 2} y={NL_y - 34} textAnchor="middle" fontSize="10" fontWeight="800" fill={T.accent} fontFamily="'JetBrains Mono', monospace">30 × 6 = 180</text>
        </g>
      )}
      {!asking && (
        <g style={{ opacity: phase >= 2 ? 1 : 0, transition: 'opacity .35s' }}>
          <path d={arc(180, 204, 14)} fill="none" stroke={T.blue} strokeWidth="2.2" strokeLinecap="round"/>
          <text x={(nlx(180) + nlx(204)) / 2} y={NL_y - 17} textAnchor="middle" fontSize="9" fontWeight="800" fill={T.blue} fontFamily="'JetBrains Mono', monospace">+24</text>
        </g>
      )}
      {guess !== null && (
        <g>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(12px, 2.4vw, 20px)', minHeight: 'clamp(180px, 38vw, 240px)' }}>
          ...
          {!clockRunning && !all && reached >= 3 && !revealed && (
            <DivisionAlgorithm phase={1}/>
          )}
          {revealed && (
            <>
              <div className="lm-reveal"><DivisionAlgorithm phase={3}/></div>
...
const Screen7 = (props) => {
  ...
  const onCell = (k) => {
    if (!canAct || ok) return;
    setTapped(k);
    if (k === 'o') { sfx.playCorrect(); audio.triggerInternal('answered'); }
  };
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(14px, 2.6vw, 22px)' }}>
          <RazryadTable h="" t={3} o={4} labels={ok ? realLabels : maskLabels} digits onCell={onCell} cellSel={ok ? 'o' : null}/>
...
// s8 — three calculations; the learner builds each result by place value.
const S8_ROUNDS = [
  { expr: '21 × 3', answer: 63 },
  { expr: '27 × 4', answer: 108 },
  { expr: '75 ÷ 3', answer: 25 },
];
const Screen8 = (props) => {
  const lang = useLang();
  ...
  const canAct = useCanAnswer(audio);
  const [round, setRound] = useState(props.storedAnswer ? S8_ROUNDS.length : 0);
  const [h, setH] = useState(0);
  const [tn, setTn] = useState(0);
  const [o, setO] = useState(0);
  ...
  const done = round >= S8_ROUNDS.length;
  const roundData = S8_ROUNDS[Math.min(round, S8_ROUNDS.length - 1)];
  const target = roundData.answer;
  ...
  const step = (k, d) => {
    if (!canAct || (checked && roundOk) || done) return;
    setChecked(false);
    setRoundOk(false);
    const clamp = (v) => Math.max(0, Math.min(9, v + d));
    if (k === 'h') setH(clamp); else if (k === 't') setTn(clamp); else setO(clamp);
  ...
  useEffect(() => {
    if (done && !recorded) {
      setRecorded(true);
      if (!audio.muted) { const e = getAudioEngine(); if (e) e.pushOneOff(scorePraiseAudio(firstOkRef.current, S8_ROUNDS.length, lang)); }
      props.onAnswer({
        stage: SCREEN_META[props.screen].scope, screenIdx: props.screen, question: t(c.q),
        correctAnswer: String(S8_ROUNDS.length), studentAnswer: firstOkRef.current, correct: firstAllRef.current,
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        {true && (
          <>
            <div className="mono fade-up" style={{ textAlign: 'center', color: T.accent, fontWeight: 800 }}>{Math.min(round + 1, S8_ROUNDS.length)} / {S8_ROUNDS.length}</div>
            <h1 className="title h-sub fade-up">{buildLabel}: <span className="mono" style={{ color: T.accent }}>{roundData.expr}</span></h1>
            <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(12px, 2.4vw, 18px)' }}>
              <FrameFx/>
              <RazryadConsole vals={{ h, t: tn, o }} labels={labels} onStep={step} disabled={(checked && roundOk) || done}/>
              <BigNum v={built} accent={done}/>
              <button className="btn-white-accent" disabled={!canAct || (checked && roundOk) || done} onClick={check}>{t(c.check_label)}</button>
            </div>
            {checked && !done && !roundOk && (
              <FeedbackBlock show={true} isCorrect={false} wrongClass="frame-tip">
                <div ref={revealRef}>
                  <Reaction
                    state="wrong"
                    praise={lang === 'ru'
                      ? `Ты собрал ${built}. ${c.audio.on_wrong.ru}`
                      : `Siz ${built} sonini yig'dingiz. ${c.audio.on_wrong.uz}`}
                  />
                </div>
              </FeedbackBlock>
            )}
        ...
        {done && (
          <div ref={doneRef} className="frame-success lm-riseup">
            <Reaction state="correct" praise={scorePraise(firstOkRef.current, S8_ROUNDS.length, lang)}/>
          </div>
        )}
      </div>
    </Stage>
...
// s9 — calculated answers are placed into their exact columns.
const S9_ROUNDS = [
  { expr: '34 × 6', answer: 204 },
  { expr: '27 × 4', answer: 108 },
  { expr: '75 ÷ 3', answer: 25 },
  { expr: '156 ÷ 3', answer: 52 },
];
const S9_NUMS = S9_ROUNDS.map((item) => item.answer);
// Har raundda 2 ta CHALG'ITUVCHI raqam (songa kirmaydi; asl raqamlar bilan takrorlanmaydi).
const S9_DECOYS = [[7, 9], [5, 6], [7, 9], [7, 9]];
...
const DEMO_NUM = '34 × 6 = 204';
const DEMO_DIG = [2, 0, 4];        // 0=yuzlik, 1=o'nlik, 2=birlik
const DEMO_BK = ['h', 't', 'o'];
...
const Screen9 = (props) => {
  ...
  const roundData = S9_ROUNDS[Math.min(round, S9_ROUNDS.length - 1)];
  ...
  const placeInto = (k, e) => {
    if (!canAct || (checked && roundOk) || done || sel === null || bins[k] !== null || flyingIdx !== null) return;
    setChecked(false);
      ...
      <div ref={wrapRef} style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        {!done && phase === 'play' && (
          <>
            ...
            <h1 className="title h-sub fade-up">{sortLabel}: <span className="mono" style={{ color: T.accent }}>{roundData.expr} = {num}</span></h1>
            <div className="lm-digtray fade-up delay-1">
              {S9_ORDERS[Math.min(round, S9_ORDERS.length - 1)].map((i) => [digits[i], i]).map(([d, i]) => !usedIdx.has(i) && flyingIdx !== i && (
                <button key={i} className={`lm-digchip mono ${sel === i ? 'lm-digchip-sel' : ''}`} disabled={!canAct || (checked && roundOk) || flyingIdx !== null} onClick={(e) => { setChecked(false); setSel(i); setSelRect(e.currentTarget.getBoundingClientRect()); }}>{d}</button>
              ))}
              {usedIdx.size === 3 && <span className="lm-digtray-empty mono">{num}</span>}
            </div>
            <div className="lm-bins fade-up delay-1">
              {['h', 't', 'o'].map((k) => (
                <button key={k} className={`lm-bin ${bins[k] !== null ? 'lm-bin-full' : ''} ${sel !== null && bins[k] === null ? 'lm-bin-open' : ''}`} disabled={!canAct || (checked && roundOk) || bins[k] !== null || sel === null || flyingIdx !== null} onClick={(e) => placeInto(k, e)}>
            ...
            {checked && !roundOk && (
              <FeedbackBlock show={true} isCorrect={false} wrongClass="frame-tip">
                <div ref={revealRef}><Reaction state="wrong" praise={c.audio.on_wrong[lang]}/></div>
              </FeedbackBlock>
            )}
          </>
        )}
...
const MCRoundScreen = ({ props, ck, renderFig, cols = 2 }) => {
  ...
  const pick = (i) => {
    ...
    if (i === it.ci) {
      ...
      setHintMsg((it.hints && it.hints[i]) || c.audio.on_wrong);
          ...
          <>
            ...
            <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(12px, 2.4vw, 18px)' }}>
              ...
              <div className="grade3-answer-grid" style={{ '--answer-cols': cols }}>
                {it.opts.map((o, i) => (
                  <button key={i} className="option" disabled={!canAct || okIdx !== null} onClick={() => pick(i)}
              ...
              {hintMsg && (
                <div className="grade3-round-bit">
                  <Reaction state="wrong" praise={t(hintMsg)}/>
                </div>
              )}
            </div>
          </>
...
const Screen11 = (props) => {
  ...
  const pick = (s) => {
    ...
    if (s === it.sign) {
      ...
      const n = new Set(wrongSet); n.delete(s); n.add(s); setWrongSet(n);
      firstAllRef.current = false;
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        {!done && it && (
          <>
            <div className="mono fade-up" style={{ textAlign: 'center', color: T.accent, fontWeight: 800 }}>{idx + 1} / {items.length}</div>
            <h1 className="title h-sub fade-up">{lang === 'ru' ? 'Сравни произведение' : "Ko'paytmani solishtiring"}</h1>
            <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
              ...
              <div className="lm-cmprow">
                ...
                <span className="lm-cmpslot mono">?</span>
                <div className="lm-cmpcell"><BigNum v={it.pair[1]}/></div>
              </div>
              <div className="lm-signrow">
                {CMP_SIGNS.map((s) => (
                  <button key={s} className="lm-signbtn mono" disabled={!canAct || solvedRound} onClick={() => pick(s)}>{s}</button>
                ))}
              </div>
              {wrongSet.size > 0 && !solvedRound && (
                <div className="grade3-round-bit">
                  <Reaction state="wrong" praise={t(it.hint)}/>
                </div>
              )}
            </div>
          </>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(8px, 1.6vw, 12px)', padding: 'clamp(10px, 2vw, 16px)' }}>
          ...
          <div className="lm-report">
            ...
            <div className="lm-report-cols">
              {[[24, lang === 'ru' ? 'в каждом' : 'har birida', '#C0392B', '#FBE9E7'], [4, lang === 'ru' ? 'устройства' : 'qurilma', '#1F7A4D', '#E3F0E8'], ['×', lang === 'ru' ? 'действие' : 'amal', '#019ACB', '#E3F2F8']].map(([n, lbl, col, bg], i) => (
          ...
          {!solved && <p className="fade-up" style={{ margin: 0, textAlign: 'center', color: T.ink2, fontSize: 'clamp(12px, 1.6vw, 14px)', fontWeight: 600 }}>{askLine}</p>}
          ...
          {!solved && <NumPad
            ...
            max={3}
          />}
          {!solved && <button className="btn-white-accent" disabled={!canAct || val === ''} onClick={check}>{lang === 'ru' ? 'Проверить' : 'Tekshir'}</button>}
          {solved && <span className="mono lm-riseup" style={{ fontSize: 'clamp(26px, 6vw, 38px)', fontWeight: 800, color: T.ink }}>{CASE_ANS}</span>}
        </div>
        {checked && !correct && (
          <FeedbackBlock show={true} isCorrect={false} wrongClass="frame-tip">
            <div ref={revealRef}><Reaction state="wrong" praise={fbMsg || s13.wrong_default[lang]}/></div>
          </FeedbackBlock>
        )}
      </div>
    </Stage>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        {!done && it && (
          ...
          <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
            ...
            {it.kind === 'num' ? (
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10 }}>
                  {orders[idx].map((k, i) => (
                    <button key={i} className="option" disabled={!canAct || (picked !== null && orders[idx][picked] === 0)} onClick={() => pick(i)}
            ...
            {it.kind !== 'num' && picked !== null && orders[idx][picked] !== 0 && (
              <div className="grade3-round-bit">
                <Reaction state="wrong" praise={t(it[`wrong_${orders[idx][picked]}`] || it.wrong_1 || it.hint || c.audio.on_wrong)}/>
              </div>
            )}
          </div>
          </>
...
// ============================================================
export default function WrittenAlgorithmsLesson({
  ...
  return (
    <LangContext.Provider value={lang}>
      ...
      <style>{D21_ETALON_FEEDBACK}</style>
      <div className="lesson-root d21-lesson">
        <GradientDefs/>
        <D2Defs/>
        <D2Motes/>
...
/* Raqam katak ICHIDA markazda tursin: demoda chip <span> (display:block, text-align:start
   bo'lib qolardi), o'yin qismida esa <button>. Ikkalasi ham bir xil markazlashsin. */

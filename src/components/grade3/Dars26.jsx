...
// s6 test 1/6 va 1/12 · s7 konsol 4 va 10 · s8 xatoni top (mulohaza) · s9 Bit tuzog'i
// (turli butunlar) · s10 trenajyor katta ulush · s11 trenajyor mayda ulush · s12 masala
// (9 − 3, keyin maxraj) · s13 final 3 topshiriq + FactCard · s14 yakun.
const SCREEN_META = [
...
const CONTENT = {
  ...
  s0: {
    ...
    audio: {
      intro: {
        ru: [
          'Ты уже умеешь записывать доли. Сегодня научишься их сравнивать.',
          'Перед тобой два одинаковых каменных диска.',
          'Первый разделили на три равные части, второй на шесть. С каждого взяли по одному куску.',
          'Как думаешь, чей кусок больше?'
        ],
        uz: [
          "Siz ulushlarni yozishni bilasiz. Bugun ularni taqqoslashni o'rganasiz.",
  ...
  s1: {
    ...
    audio: {
      ru: [
        ...
        'Первый разрезали на три равные части. Одна такая часть большая.',
        'Второй на шесть. Теперь каждая часть заметно меньше. Значит одна третья больше, чем одна шестая.'
      ],
      uz: [
  ...
  s2: {
    ...
    audio: {
      ru: [
        ...
        'Слева диск разрезали на четыре части, справа на восемь. Правый кусок мельче.',
        'Ставим знак. Он открывается к большей доле, значит одна четвёртая больше одной восьмой.'
      ],
      uz: [
  ...
  s7: {
    ...
    cells: [
      { head: { ru: 'первый', uz: 'birinchi' }, label: 'частей', ans: 4, hint: { ru: 'Посчитай части левого диска.', uz: "Chapdagi diskning bo'laklarini sanang." } },
      { head: { ru: 'второй', uz: 'ikkinchi' }, label: 'частей', ans: 10, hint: { ru: 'Посчитай части правого диска.', uz: "O'ngdagi diskning bo'laklarini sanang." } },
      { head: { ru: 'крупнее', uz: 'yirikroq' }, label: 'знаменатель', ans: 4, hint: { ru: 'Крупнее там, где частей меньше.', uz: "Bo'lak kam bo'lgan joyda yirikroq." } }
    ],
    check: '1/4 > 1/10',
  ...
  s9: {
    ...
    audio: {
      ru: [
        ...
        'Здесь половина маленького, здесь четверть большого. Одна вторая всегда больше одной четвёртой.',
        'Так ли это?'
      ],
      uz: [
...
// --- ULUSH KVARTALI (D26): 8-DARSNING QADIMGI CHORAK sahnasi (`AncientHallBg`) qayta
...
const TwoDiskHallBg = () => (
  <svg className="lm-scene-bg" viewBox="0 0 400 230" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
    <defs>
      <linearGradient id="d26wall" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#EADAB4"/><stop offset="100%" stopColor="#CDB689"/></linearGradient>
      <linearGradient id="d26col" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#A8946A"/><stop offset="42%" stopColor="#E8D8B2"/><stop offset="100%" stopColor="#A8946A"/></linearGradient>
      <linearGradient id="d26sky" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#5E4256"/><stop offset="45%" stopColor="#A8705E"/><stop offset="82%" stopColor="#D89A66"/><stop offset="100%" stopColor="#F2C88E"/></linearGradient>
      <linearGradient id="d26floor" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#C9B283"/><stop offset="100%" stopColor="#A38A5E"/></linearGradient>
      <linearGradient id="d26slab" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#E4D3AC"/><stop offset="100%" stopColor="#C6AE7E"/></linearGradient>
      <radialGradient id="d26sun" cx="50%" cy="50%" r="55%"><stop offset="0%" stopColor="#FFE6B0"/><stop offset="55%" stopColor="#EE9A5A"/><stop offset="100%" stopColor="#C0502E" stopOpacity="0"/></radialGradient>
      <radialGradient id="d26moss" cx="50%" cy="35%" r="70%"><stop offset="0%" stopColor="#BFF0C8"/><stop offset="100%" stopColor="#7FD0A0" stopOpacity="0"/></radialGradient>
      <clipPath id="d26arch"><path d="M124 96 L124 70 Q124 40 200 40 Q276 40 276 70 L276 96 Z"/></clipPath>
    </defs>
    {/* --- DEVOR + shift lintel (8-darsdan) --- */}
    <rect x="0" y="0" width="400" height="180" fill="url(#d26wall)"/>
    ...
    {[104, 200, 296].map((cx, i) => (
      <g key={i}>
        ...
        <ellipse cx={cx} cy="34" rx="11" ry="16" fill="url(#d26moss)" opacity="0.5"/>
      </g>
    ))}
    {/* --- ORTDA: RAVOQ -> vayrona mahalla --- */}
    <g clipPath="url(#d26arch)">
      <rect x="120" y="38" width="160" height="60" fill="url(#d26sky)"/>
      ...
      <circle cx="250" cy="88" r="15" fill="url(#d26sun)"/><circle cx="250" cy="88" r="7" fill="#FFD89A"/>
    ...
    <path d="M116 96 L116 70 Q116 32 200 32 Q284 32 284 70 L284 96 L276 96 L276 70 Q276 40 200 40 Q124 40 124 70 L124 96 Z" fill="url(#d26col)" stroke="#8A7550" strokeWidth="1.2"/>
    <g stroke="#8A7550" strokeWidth="0.8" opacity="0.7"><path d="M150 43 l-4 -7"/><path d="M200 36 v-8"/><path d="M250 43 l4 -7"/></g>
    {/* --- RAMKA USTUNLARI --- */}
    {[28, 334].map((x, i) => (
      <g key={i}>
        <rect x={x - 6} y="24" width="54" height="12" rx="3" fill="url(#d26col)" stroke="#8A7550" strokeWidth="1"/>
        <rect x={x} y="36" width="42" height="140" fill="url(#d26col)" stroke="#8A7550" strokeWidth="1"/>
        <g stroke="#9A855C" strokeWidth="1.2" opacity="0.55">{[10, 21, 32].map((dx, k) => <line key={k} x1={x + dx} y1="40" x2={x + dx} y2="172"/>)}</g>
        <rect x={x - 4} y="168" width="50" height="10" rx="2" fill="url(#d26col)" stroke="#8A7550" strokeWidth="1"/>
...
{/* --- MARKAZIY STELA: ikki tosh disk va ular orasida belgi --- */}
    ...
    <text x="200" y="111.5" textAnchor="middle" fontSize="7" letterSpacing="2" fill="#6B5636" fontFamily="'JetBrains Mono', monospace">QAYSI ULUSH KATTA</text>
    {[[156, 3], [244, 6]].map(([cx, n], i) => (
      <g key={i} transform={`translate(${cx} 136)`}>
        ...
        {Array.from({ length: n }).map((_, k) => {
          ...
          return <line key={k} x1="0" y1="0" x2={(17 * Math.cos(a)).toFixed(1)} y2={(17 * Math.sin(a)).toFixed(1)} stroke="#8A7550" strokeWidth="1.1"/>;
        })}
        <path d={`M0 0 L0 -17 A17 17 0 0 1 ${(17 * Math.cos(Math.PI * 2 / n - Math.PI / 2)).toFixed(1)} ${(17 * Math.sin(Math.PI * 2 / n - Math.PI / 2)).toFixed(1)} Z`} fill="#E0A05A"/>
      </g>
    ))}
    <text x="200" y="142" textAnchor="middle" fontSize="17" fontWeight="800" fill="#C06A2E" fontFamily="'JetBrains Mono', monospace">&gt;</text>
    ...
    {[[4, 96], [8, 124], [12, 152]].map(([n, y], i) => (
      <g key={i} transform={`translate(318 ${y})`}>
        <circle cx="0" cy="0" r="10" fill="url(#d26slab)" stroke="#8A7550" strokeWidth="1"/>
        {Array.from({ length: n }).map((_, k) => {
          const a = (Math.PI * 2 * k) / n - Math.PI / 2;
          return <line key={k} x1="0" y1="0" x2={(10 * Math.cos(a)).toFixed(1)} y2={(10 * Math.sin(a)).toFixed(1)} stroke="#8A7550" strokeWidth="0.7"/>;
        })}
        <path d={`M0 0 L0 -10 A10 10 0 0 1 ${(10 * Math.cos(Math.PI * 2 / n - Math.PI / 2)).toFixed(1)} ${(10 * Math.sin(Math.PI * 2 / n - Math.PI / 2)).toFixed(1)} Z`} fill="#E0A05A"/>
      </g>
    ))}
    <circle className="lm-glow" cx="300" cy="86" r="2.4" fill="#BFF0C8"/>
    {/* --- POL: mozaik tosh + perspektiva (8-darsdan) --- */}
    <rect x="0" y="176" width="400" height="54" fill="url(#d26floor)"/>
    ...
    <g transform="translate(58 176)"><rect x="-2" y="-12" width="34" height="11" rx="3" fill="url(#d26col)" stroke="#8A7550" strokeWidth="1" transform="rotate(-6)"/><circle className="lm-glow" cx="0" cy="-8" r="2.6" fill="#BFF0C8"/></g>
...
const TwoDiskHallScene = ({ gathered = false }) => {
...
const ShareFig = ({ shape = 'circle', parts = 4, filled = 0, equal = true, show = true, size = 'md' }) => {
  ...
  if (shape === 'bar') {
    ...
    let x = 0;
    return (
      <svg className="d26-fig" viewBox={`0 0 ${w} ${h}`} style={{ width: `min(${w * 2}px, 78%)`, height: 'auto' }} aria-hidden="true">
        {norm.map((p, i) => {
          const bw = p * w;
          const rx = x;
          x += bw;
  ...
  let a0 = -Math.PI / 2;
  return (
    <svg className="d26-fig" viewBox={`0 0 ${w} ${w}`} style={{ width: size === 'sm' ? 'min(96px, 34%)' : 'min(150px, 46%)', height: 'auto' }} aria-hidden="true">
      {norm.map((p, i) => {
...
// --- IKKI ULUSH YONMA-YON: butun BIR XIL, kesim boshqa. Orada belgi (4-darsdagidek).
const SharePair = ({ left, right, leftLabel = null, rightLabel = null, sign = null }) => (
  <span className="d26-pair">
    <span className="d26-pair-one">
      <ShareFig shape="circle" parts={left} filled={1} size="sm"/>
      {leftLabel && <span className="mono d26-pair-cap">{leftLabel}</span>}
    </span>
    <span className="mono d26-pair-sign">{sign || ''}</span>
    <span className="d26-pair-one">
      <ShareFig shape="circle" parts={right} filled={1} size="sm"/>
      {rightLabel && <span className="mono d26-pair-cap">{rightLabel}</span>}
    </span>
  </span>
);

// --- FACTCARD QAHRAMONI: quyosh soati siferblati, kun teng ulushlarga bo'lingan.
const DialFig = () => (
  <svg viewBox="0 0 220 108" style={{ width: 'min(266px, 84%)', height: 'auto', display: 'block' }} aria-hidden="true">
    <circle cx="110" cy="54" r="42" fill="#F3E7CB" stroke="#C08A3E" strokeWidth="1.6"/>
    {Array.from({ length: 12 }).map((_, k) => {
      const a = (Math.PI * 2 * k) / 12 - Math.PI / 2;
      return <line key={k} x1={110 + 30 * Math.cos(a)} y1={54 + 30 * Math.sin(a)} x2={110 + 42 * Math.cos(a)} y2={54 + 42 * Math.sin(a)} stroke="#8A7550" strokeWidth="1.4"/>;
    })}
    ...
    <text x="110" y="104" textAnchor="middle" fontSize="8" fill="#8A8378" fontFamily="'JetBrains Mono', monospace">kun teng ulushlarga</text>
  </svg>
);
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(10px, 2vw, 14px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
          <FrameFx/>
          {figNode}
          {figLine && <span className="mono d26-errline">{figLine}</span>}
  ...
  return (
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(4px, 1vw, 8px)' }}>
        ...
        <div className="frame fade-up delay-1 d26-hook-scene" style={{ padding: 'clamp(8px, 1.8vw, 14px)', overflow: 'hidden' }}>
          <TwoDiskHallScene gathered={ok}/>
        </div>
        {picked === null && (
          <div className="frame fade-up delay-1" style={{ padding: 'clamp(6px, 1.2vw, 9px)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            <span className="d26-order">
              <span className="mono d26-order-plate">11</span>
              <span className="d26-order-sep mono">:</span>
              <span className="mono d26-order-plate">2</span>
            </span>
            <span className="d26-note">{t(c.order_cap)}</span>
          </div>
        )}
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(6px, 1.4vw, 10px)', padding: 'clamp(12px, 2.4vw, 18px)' }}>
          ...
          {step >= 1 && (
            <span className="lm-reveal" style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
              <span className="mono d26-expr">{c.step1}</span>
              <span className="d26-note">{t(c.step1_cap)}</span>
            </span>
          )}
          {step >= 2 && (
            <span className="lm-reveal" style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
              <span className="mono d26-expr">{c.step2}</span>
              <span className="d26-note">{t(c.step2_cap)}</span>
            </span>
          )}
          {step >= 2 && <span className="mono d26-final lm-reveal" style={{ animationDelay: '0.25s' }}>{c.res}</span>}
          {!done && (
...
// s2 — BELGI: ikki ulush yonma-yon, keyin orasiga belgi qo'yiladi
const Screen2 = (props) => {
  const lang = useLang();
  const t = useT();
  const sfx = useSfx();
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(6px, 1.4vw, 10px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(6px, 1.4vw, 10px)', padding: 'clamp(10px, 2vw, 15px)' }}>
          <FrameFx/>
          <SharePair left={c.left_parts} right={c.right_parts}
            leftLabel={step >= 1 ? c.left_label : null} rightLabel={step >= 1 ? c.right_label : null}
            sign={step >= 2 ? c.sign : null}/>
          <div className="d26-gridrow">
            {step >= 1 && (
              <span className="d26-gridcap lm-reveal">
                <span className="d26-expr" style={{ fontSize: 'clamp(11px, 1.7vw, 13px)', color: '#C97F35' }}>{t(c.capA)}</span>
              </span>
            )}
            {step >= 2 && (
              <span className="d26-gridcap lm-reveal">
                <span className="d26-expr" style={{ fontSize: 'clamp(11px, 1.7vw, 13px)', color: '#2E7E9E' }}>{t(c.capB)}</span>
              </span>
            )}
          </div>
          {step >= 2 && <span className="mono d26-final lm-reveal" style={{ animationDelay: '0.25s' }}>{c.res}</span>}
...
// s4 — RASM BO'YICHA BELGI: ikki disk, javob variantlari — belgi nomlari
const Screen4 = (props) => (
  <MCOne props={props} ck="s4"
    figNode={<SharePair left={CONTENT.s4.fig_left} right={CONTENT.s4.fig_right} leftLabel={`1/${CONTENT.s4.fig_left}`} rightLabel={`1/${CONTENT.s4.fig_right}`}/>}/>
);
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(5px, 1.1vw, 9px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(5px, 1.1vw, 9px)', padding: 'clamp(10px, 2vw, 15px)' }}>
          <FrameFx/>
          <span className="mono d26-expr">{c.swap_line}</span>
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(6px, 1.4vw, 10px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(6px, 1.4vw, 10px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
          <FrameFx/>
          <span className="mono d26-plate">{lines[0]}</span>
          <span className="d26-bad">{lines[1]}</span>
          <span className="d26-note">{t(c.line_cap)}</span>
          <p className="fade-up" style={{ margin: 0, textAlign: 'center', color: T.ink2, fontWeight: 700 }}>{t(c.trap_label)}</p>
          <div className="d26-trap">
        ...
        {done && (
          <div ref={factRef}>
            ...
            <div className="d2-factcard fade-up">
              ...
              <div className="d2-fact-hero"><DialFig/></div>
...
// ============================================================
export default function CompareSharesLesson({
...
.d26-order { display: inline-flex; gap: clamp(6px, 1.6vw, 12px); align-items: center; padding: clamp(4px, 1vw, 7px) clamp(8px, 1.8vw, 12px);
  border-radius: 10px; background: rgba(255,236,200,.5); box-shadow: inset 0 0 0 1px rgba(190,150,90,.26); }
.d26-order-plate { font-size: clamp(15px, 3vw, 21px); font-weight: 800; color: #3A3530; padding: 3px 10px;
  border-radius: 8px; background: #FFFFFF; box-shadow: 0 2px 8px -4px rgba(58,53,48,0.35); }
.d26-note { font-size: clamp(9.5px, 1.5vw, 12px); font-weight: 700; color: #5A5A60; text-align: center; }
.d26-expr { font-size: clamp(15px, 3vw, 22px); font-weight: 800; color: #3A3530; }
.d26-final { font-size: clamp(18px, 3.6vw, 26px); font-weight: 800; color: #1F7A4D; }
.d26-bad { font-size: clamp(14px, 2.8vw, 20px); font-weight: 800; color: #C0392B; }
.d26-errline { font-size: clamp(13px, 2.5vw, 19px); font-weight: 800; color: #C0392B; padding: 3px 12px;
  border-radius: 9px; background: rgba(192,57,43,.08); box-shadow: inset 0 0 0 1px rgba(192,57,43,.3); text-align: center; }
.d26-steplabel { font-size: clamp(13px, 2.4vw, 18px); font-weight: 800; color: #3A3530; text-align: center; }
.d26-res { font-size: clamp(19px, 3.8vw, 26px); font-weight: 800; color: #1F7A4D; }
.d26-plate { font-size: clamp(19px, 4vw, 28px); font-weight: 800; color: #0E0E10; padding: 4px 14px;
  border-radius: 12px; background: rgba(255,236,200,.55); box-shadow: inset 0 0 0 1px rgba(190,150,90,.28); }
.d26-trap { display: flex; gap: 10px; justify-content: center; }
.d26-bins { display: grid; grid-template-columns: repeat(2, minmax(120px, 1fr)); gap: clamp(8px, 2vw, 16px); width: 100%; max-width: 460px; }

/* --- KATAK TO'R --- */
.d26-grid { display: inline-flex; align-items: flex-start; gap: clamp(5px, 1.2vw, 9px);
...
.d26-grid-part { display: inline-flex; flex-direction: column; gap: 2px; }
.d26-grid-row { display: inline-flex; gap: 2px; }
.d26-cell { display: inline-block; width: clamp(6px, 1.5vw, 10px); height: clamp(6px, 1.5vw, 10px); border-radius: 2px; }
.d26-cell-a { background: #F2A85C; box-shadow: inset 0 0 0 0.5px #C97F35; }
.d26-cell-b { background: #6FD0E4; box-shadow: inset 0 0 0 0.5px #3E8FA8; }
.d26-gridrow { display: flex; flex-wrap: wrap; justify-content: center; align-items: flex-start; gap: clamp(6px, 1.6vw, 12px); }
.d26-gridcap { display: flex; flex-direction: column; align-items: center; gap: 3px; }

/* --- sahna o'lchami: budjet donor bilan bir xil (etalon 629x330 @1440x900) --- */
.d26-hook-scene .lm-scene { width: min(100%, calc(clamp(180px, calc(100dvh - 570px), 372px) * 400 / 210)); }
.d26-final-scene .lm-scene { width: min(100%, calc(clamp(200px, calc(100dvh - 560px), 372px) * 400 / 210)); }

/* --- FACTCARD: zinapoya --- */
.d26-stair { animation: d22stair 3.6s ease-in-out infinite; }
@keyframes d22stair { 0%, 12% { opacity: 0.3; } 34%, 100% { opacity: 1; } }
@media (prefers-reduced-motion: reduce) { .d26-stair { animation: none; opacity: 1; } }

.d26-boxwrap { display: flex; flex-direction: column; align-items: center; gap: clamp(6px, 1.4vw, 10px); }
.d26-boxrow { display: grid; grid-template-columns: repeat(6, auto); gap: clamp(4px, 1vw, 7px); justify-content: center; }
.d26-box { width: clamp(20px, 3.4vw, 27px); height: clamp(17px, 2.9vw, 23px); border-radius: 3px;
  background: #EFE6D6; border: 1.5px solid #D8CDB8; opacity: 0.5; transition: none; }
.d26-box-on { background: linear-gradient(180deg, #FFCB8E 0 26%, #F2A85C 26% 100%); border-color: #C97F35;
...
.d26-rest { display: inline-flex; gap: clamp(4px, 1vw, 7px); align-items: center; padding: clamp(4px, 1vw, 7px) clamp(8px, 1.8vw, 13px);
  border-radius: 999px; background: #FDECE7; border: 1.5px dashed #E0563A; }
.d26-kg { width: clamp(11px, 1.9vw, 15px); height: clamp(11px, 1.9vw, 15px); border-radius: 50%;
  background: #E0563A; border: 1.2px solid #B33F27; }

.d26-fig { display: block; margin: 0 auto; }
.d26-figrow { display: flex; align-items: center; justify-content: center; gap: clamp(10px, 2.4vw, 18px); flex-wrap: wrap; }
.d26-frac { display: inline-flex; flex-direction: column; align-items: center; line-height: 1.05;
  font-family: 'JetBrains Mono', monospace; font-weight: 800; font-size: clamp(20px, 3.4vw, 27px); }
.d26-frac-top { color: #2E7E9E; }
.d26-frac-bar { display: block; width: clamp(24px, 4vw, 32px); height: 2.4px; background: #5D5A52; margin: 3px 0; border-radius: 2px; }
.d26-frac-bot { color: #C97F35; }
.d26-fracname { font-family: 'JetBrains Mono', monospace; font-size: clamp(10px, 1.6vw, 12px); font-weight: 800; letter-spacing: 0.4px; }

/* Yangi uslub yo'q: hamma qoida 24-darsdan ko'chib keldi va nomi almashtirildi. */

.d26-pair { display: inline-flex; align-items: center; justify-content: center; gap: clamp(8px, 2vw, 16px); flex-wrap: nowrap; }
.d26-pair-one { display: inline-flex; flex-direction: column; align-items: center; gap: 4px; }
.d26-pair-cap { font-size: clamp(13px, 2.2vw, 17px); font-weight: 800; color: #5D5A52; }
.d26-pair-sign { font-size: clamp(20px, 3.6vw, 28px); font-weight: 800; color: #C97F35; min-width: clamp(18px, 3vw, 26px); text-align: center; }
`;

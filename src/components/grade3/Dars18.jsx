...
// ============================================================================
// DD 3-SINF | Dars18 — "Ikki xonali sonni bir xonaliga bo'lish" (num-3-18) | Б3 «USTAXONA»
// Syujet: Bit sayyorasi LUMO, Ustaxona hududi (SYUJET_3SINF.md Б3 d.20, joy = taqsimot rafi).
//   Modul-bloklar tokchalarga teng taqsimlanadi. Bit — mezbon-gid. FactCard: richag (Arximed) —
//   B3 gravitatsiya fakti Dars19 da berilgani uchun BOSHQA haqiqiy fakt.
// Infra: grade3 Dars13.jsx (yig'indini bo'lish) dan BAYT-ANIQ ko'chirildi. Sahna: USTAXONA
//   interyeri + markaziy asbob TAQSIMOT RAFI (WorkshopRackBg). Birlik vizual = modul-blok (Modul).
// YADRO: 96 ÷ 4 = (80 + 16) ÷ 4 — QULAY bo'laklarga yoyish (90+6 EMAS: 90 to'rtga bo'linmaydi!);
//   80÷4=20, 16÷4=4, 20+4=24. Dars13 bonus g'oyasining (qulay bo'laklar) yadroga chiqishi.
// MEXANIKA: hook 96÷4 (s0), ajratish 90+6 rad -> 80+16 (s1), katta bo'lak (s2), kichik bo'lak (s3),
//   yig'ish (s4), tez zanjir 84÷7 (s5), stway ikki usul + bashorat + bonus, QOIDA (s6),
//   qulay ajratish MC (s7), NumPad (s8), xato yozuv (s9), aralash MC (s10), masala 68÷4 (s11),
//   final panel (s12), yakun (s13).
// Misconception: M1 ko'r-ko'rona xonalab bo'lish (96=90+6, 90:4 yo'q), M2 bo'lakni noto'g'ri tanlash
//   (son o'zgarib qoladi), M3 natijalarni qo'shmaslik (faqat o'nliklar javobi), M4 bo'lish=ayirish.
//
...
// ============================================================
// --- 3-SINF DARS: num-3-18 — Ikki xonali sonni bir xonaliga bo'lish (Б3, 100 ichida) ---
// 8-9 yosh: ovoz yetakchi kanal, concrete-avval (modul-bloklar -> tokchalar -> yozuv).
// Manba: SYUJET_3SINF.md Б3 d.20 + Dars13 mexanikasi (yig'indini bo'lish davomi).
// ============================================================

// Dars13 skeleti saqlangan: 15 ekran, stway ikki-usul ekrani s5 va s6 orasida.
const TOTAL_SCREENS = 15;
const LESSON_META = {
  lessonId: 'num-3-18',
  lessonTitle: { ru: 'Урок 18. Деление двузначного числа', uz: "18-dars. Ikki xonali sonni bo'lish" }
};
// STRUKTURA (15 ekran): s0 hook · s1–s5 kashfiyot · stway ikki usul · s6 qoida · s7–s10 mashq · s11 masala · s12 final · s13 xulosa.
// Syujet: Ustaxona (SYUJET_3SINF.md Б3 d.20) — taqsimot rafi, 96÷4 qulay bo'laklab bo'lish.
const SCREEN_META = [
...
// ============================================================
// CONTENT — 3-sinf Dars18 «Ikki xonali sonni bir xonaliga bo'lish» (num-3-18). RU + UZ to'liq.
// Audio TTS-toza: sonlar so'z bilan, «» va matematik belgilar yo'q, bir segment = bir fikr.
// Rekvizit: modul-bloklar uyumi + taqsimot rafi tokchalari. Ustaxona (Б3 d.20).
// ============================================================
const CONTENT = {
  s0: {
    eyebrow: { ru: 'Миссия', uz: 'Missiya' },
    topic: { ru: 'Тема: делим двузначное число', uz: "Mavzu: ikki xonali sonni bo'lamiz" },
    lead: { ru: 'Модули надо разложить на полки поровну.', uz: "Modullarni tokchalarga teng joylash kerak." },
    q: { ru: '96 модулей и 4 полки. Сколько на каждой?', uz: "96 ta modul va 4 ta tokcha. Har birida nechtadan?" },
    count: 96,
    baskets: 4,
    grid_cap: { ru: 'Модулей 96, полок 4. В таблице умножения числа 96 нет.', uz: "Modul 96 ta, tokcha 4 ta. Ko'paytirish jadvalida 96 yo'q." },
    opt0: { ru: '92', uz: '92' },
    opt1: { ru: '24', uz: '24' },
    opt2: { ru: '20', uz: '20' },
    audio: {
      intro: {
        ru: [
          'Сегодня мы делим двузначное число на однозначное. Одной таблицы для этого не хватит.',
          'В прошлом уроке мы умножали двузначное число по частям. Сегодня идём обратным путём, делим.',
          'Бит привёл друзей к полке распределения. Модулей девяносто шесть, а полок четыре.',
          'Разложить надо поровну. Как думаешь, сколько модулей будет на каждой полке?'
        ],
        uz: [
          "Bugun ikki xonali sonni bir xonaliga bo'lamiz. Buning uchun jadvalning o'zi yetmaydi.",
          "O'tgan darsda ikki xonali sonni bo'laklab ko'paytirdik. Bugun teskari yo'ldan boramiz.",
          "Bit do'stlarni taqsimot rafiga olib keldi. Modul to'qson oltita, tokcha esa to'rtta.",
          "Teng joylash kerak. Sizningcha, har bir tokchada nechta modul bo'ladi?"
        ]
      },
      on_correct: { ru: 'Верная мысль. Девяносто шесть на четыре, это двадцать четыре.', uz: "To'g'ri fikr. To'qson oltini to'rtga bo'lsak, yigirma to'rt chiqadi." },
      on_wrong: { ru: 'Пока нет. Разделить надо все модули до последнего, и не вычитанием, а поровну. Проверим вместе.', uz: "Hozircha yo'q. Oxirgi modulgacha hammasini teng bo'lish kerak, ayirish emas. Birgalikda tekshiramiz." }
    }
  },

  // s1 — KASHFIYOT 1: 90+6 mos emas -> 80+16 qulay bo'laklar
  s1: {
    eyebrow: { ru: 'Открытие', uz: 'Kashfiyot' },
    lead: { ru: 'Разложим модули на удобные части.', uz: "Modullarni qulay bo'laklarga ajratamiz." },
    steps: [
      { expr: '96 = 90 + 6', at: 2, bad: true, tag: { ru: '90 на 4 не делится', uz: "90 to'rtga bo'linmaydi" } },
      { expr: '96 = 80 + 16', at: 3, tag: { ru: 'обе части делятся на 4', uz: "ikkala bo'lak 4 ga bo'linadi" } }
    ],
    done_text: { ru: 'Девяносто шесть, это восемьдесят и ещё шестнадцать. Обе части делятся на четыре.', uz: "To'qson olti, bu sakson va yana o'n olti. Ikkala bo'lak ham to'rtga bo'linadi." },
    audio: {
      ru: [
        'Девяносто шесть в таблице искать бесполезно. Но число можно разложить на части.',
        'Сначала пробуем привычный путь. Десятки, девяносто, единицы, шесть.',
        'Только девяносто на четыре без остатка не делится. Такое разложение нам не подходит.',
        'Разложим иначе. Один десяток отдаём в сторону единиц. Получается восемьдесят и шестнадцать.',
        'Теперь обе части делятся на четыре без остатка. Именно такое разложение нам и нужно.'
      ],
      uz: [
        "To'qson oltini jadvaldan qidirish foydasiz. Lekin sonni bo'laklarga ajratsa bo'ladi.",
        "Avval odatdagi yo'lni sinaymiz. O'nliklar to'qson, birliklar olti.",
        "Faqat to'qsonni to'rtga qoldiqsiz bo'lib bo'lmaydi. Bunday ajratish bizga mos emas.",
        "Boshqacha ajratamiz. Bitta o'nlikni birliklar tomonga beramiz. Sakson va o'n olti chiqadi.",
        "Endi ikkala bo'lak ham to'rtga qoldiqsiz bo'linadi. Aynan shunday ajratish bizga kerak."
      ]
    }
  },
  ...
  s2: {
    ...
    steps: [
      { expr: '8 ÷ 4 = 2', at: 1, tag: { ru: 'таблица', uz: 'jadval' } },
      { expr: '80 ÷ 4 = 20', at: 2, tag: { ru: '2 десятка', uz: "2 ta o'nlik" } }
    ],
    done_text: { ru: 'Из восьмидесяти модулей на каждую полку попадает двадцать.', uz: "Saksonta moduldan har bir tokchaga yigirmatadan tushadi." },
    audio: {
      ru: [
        'Сначала большая часть. Восемьдесят модулей делим на четыре полки.',
        'Восемь разделить на четыре, два. Это мы знаем из таблицы.',
        'Но делили мы не единицы, а десятки. Значит, на каждой полке два десятка, то есть двадцать.',
        'Восемьдесят разделить на четыре, двадцать.'
      ],
      uz: [
        "Avval katta bo'lak. Sakson modulni to'rtta tokchaga bo'lamiz.",
        "Sakkizni to'rtga bo'lsak, ikki. Buni jadvaldan bilamiz.",
        "Lekin biz birliklarni emas, o'nliklarni bo'ldik. Demak, har tokchada ikkita o'nlik, ya'ni yigirma.",
        "Saksonni to'rtga bo'lsak, yigirma."
      ]
    }
  },
  ...
  s3: {
    ...
    steps: [
      { expr: '16 ÷ 4 = 4', at: 2, tag: { ru: 'таблица', uz: 'jadval' } }
    ],
    done_text: { ru: 'Маленькая часть дала каждой полке ещё по четыре модуля.', uz: "Kichik bo'lak har tokchaga yana to'rttadan modul berdi." },
    audio: {
      ru: [
        'Теперь маленькая часть. Отдельно лежат шестнадцать модулей.',
        'Полок четыре, значит шестнадцать разделить на четыре.',
        'Шестнадцать разделить на четыре, четыре. Это чистая таблица.'
      ],
      uz: [
        "Endi kichik bo'lak. Alohida o'n oltita modul yotibdi.",
        "Tokcha to'rtta, demak o'n oltini to'rtga bo'lamiz.",
        "O'n oltini to'rtga bo'lsak, to'rt. Bu sof jadval."
      ]
    }
  },
  ...
  s4: {
    eyebrow: { ru: 'Открытие', uz: 'Kashfiyot' },
    lead: { ru: 'Складываем то, что попало на одну полку.', uz: "Bitta tokchaga tushganini qo'shamiz." },
    steps: [
      { expr: '20 + 4 = 24', at: 2, tag: { ru: 'ответ', uz: 'javob' } }
    ],
    done_text: { ru: 'Девяносто шесть разделить на четыре, двадцать четыре. Делили дважды, а полка одна.', uz: "To'qson oltini to'rtga bo'lsak, yigirma to'rt. Ikki marta bo'ldik, tokcha esa bitta." },
    audio: {
      ru: [
        'Мы делили дважды. Сначала большую часть, потом маленькую.',
        'На одну полку попало двадцать модулей из большой части и четыре из маленькой.',
        'Двадцать плюс четыре, двадцать четыре.',
        'Значит, девяносто шесть разделить на четыре, двадцать четыре. В самом начале это был верный ответ.'
      ],
      uz: [
        "Biz ikki marta bo'ldik. Avval katta bo'lakni, keyin kichigini.",
        "Bitta tokchaga katta bo'lakdan yigirmata, kichik bo'lakdan to'rtta modul tushdi.",
        "Yigirma qo'shuv to'rt, yigirma to'rt.",
        "Demak, to'qson oltini to'rtga bo'lsak, yigirma to'rt. Eng boshida ham to'g'ri javob shu edi."
      ]
    }
  },

  // s5 — KASHFIYOT 5: butun zanjir tez, ikkinchi misol (84÷7 — yana qulay bo'laklar)
  s5: {
    eyebrow: { ru: 'Открытие', uz: 'Kashfiyot' },
    lead: { ru: 'Тот же путь, другое число.', uz: "O'sha yo'l, boshqa son." },
    steps: [
      { expr: '84 = 70 + 14', at: 1, tag: { ru: 'удобные части', uz: "qulay bo'laklar" } },
      { expr: '70 ÷ 7 = 10', at: 2, tag: { ru: 'большая часть', uz: "katta bo'lak" } },
      { expr: '14 ÷ 7 = 2', at: 3, tag: { ru: 'маленькая часть', uz: "kichik bo'lak" } },
      { expr: '10 + 2 = 12', at: 4, tag: { ru: 'собрали', uz: "yig'dik" } }
    ],
    done_text: { ru: 'Четыре шага, как всегда: разложить, разделить обе части, сложить.', uz: "To'rt qadam, har doimgidek: ajratish, ikkala bo'lakni bo'lish, qo'shish." },
    audio: {
      ru: [
        'Пройдём тот же путь ещё раз, быстрее. Восемьдесят четыре разделить на семь.',
        'Первый шаг. Раскладываем. Привычные восемьдесят и четыре не подходят, восемьдесят на семь не делится. Берём семьдесят и четырнадцать.',
        'Второй шаг. Большая часть. Семьдесят разделить на семь, десять.',
        'Третий шаг. Маленькая часть. Четырнадцать разделить на семь, два.',
        'Четвёртый шаг. Складываем. Десять плюс два, двенадцать.',
        'Шаги всегда одни. Разложить на удобные части, разделить обе, сложить.'
      ],
      uz: [
        "O'sha yo'ldan yana bir marta, tezroq o'tamiz. Sakson to'rtni yettiga bo'lamiz.",
        "Birinchi qadam. Ajratamiz. Odatdagi sakson va to'rt mos emas, saksonni yettiga bo'lib bo'lmaydi. Yetmish va o'n to'rtni olamiz.",
        "Ikkinchi qadam. Katta bo'lak. Yetmishni yettiga bo'lsak, o'n.",
        "Uchinchi qadam. Kichik bo'lak. O'n to'rtni yettiga bo'lsak, ikki.",
        "To'rtinchi qadam. Qo'shamiz. O'n qo'shuv ikki, o'n ikki.",
        "Qadamlar doim bir xil. Qulay bo'laklarga ajratish, ikkalasini bo'lish, qo'shish."
      ]
    }
  },

  // stway — IKKI USUL: 1-usul qulay bo'laklar, bashorat, 2-usul ko'paytirish bilan tekshirish, bonus
  stway: {
    eyebrow: { ru: 'Два способа', uz: 'Ikki usul' },
    lead: { ru: 'Раздели, а потом проверь себя.', uz: "Bo'ling, keyin o'zingizni tekshiring." },
    expr: '96 ÷ 4',
    m1_label: { ru: 'Способ 1 — удобные части', uz: "1-usul — qulay bo'laklar" },
    m1_text: { ru: 'Раскладываем число так, чтобы обе части делились.', uz: "Sonni ikkala bo'lak ham bo'linadigan qilib ajratamiz." },
    m1_rows: [
      { pre: '96 = ', acc: '80 + 16', post: '' },
      { t: '80 ÷ 4 + 16 ÷ 4' },
      { t: '20 + 4' }
    ],
    m1_recap: '96 ÷ 4 = 24',
    check_q: { ru: 'Мы получили двадцать четыре. Можно ли проверить ответ умножением?', uz: "Yigirma to'rtni topdik. Javobni ko'paytirish bilan tekshirsa bo'ladimi?" },
    check_opts: ['Да, можно', 'Нет, нельзя'],
    check_opts_uz: ["Ha, bo'ladi", "Yo'q, bo'lmaydi"],
    check_ci: 0,
    check_ok: { ru: 'Верно. Умножение вернуло девяносто шесть, ответ подтверждён.', uz: "To'g'ri. Ko'paytirish to'qson oltini qaytardi, javob tasdiqlandi." },
    check_no: { ru: 'Смотри, умножение обратно делению. Оно может проверить ответ. Попробуй ещё.', uz: "Qarang, ko'paytirish bo'lishning teskarisi. U javobni tekshira oladi. Yana urinib ko'ring." },
    m2_label: { ru: 'Способ 2 — проверка умножением', uz: "2-usul — ko'paytirish bilan tekshirish" },
    m2_text: { ru: 'Умножаем ответ обратно, должно вернуться исходное число.', uz: "Javobni orqaga ko'paytiramiz, asl son qaytishi kerak." },
    m2_parts: [
      { e: '20 × 4', v: '80' },
      { e: '4 × 4', v: '16' }
    ],
    m2_recap: '24 × 4 = 96',
    bonus_label: { ru: 'Бонус — почему не 90 + 6', uz: "Bonus — nega 90 + 6 emas" },
    bonus_text: { ru: 'Каждая часть должна делиться без остатка.', uz: "Har bir bo'lak qoldiqsiz bo'linishi shart." },
    bonus_head: '96 ÷ 4',
    bonus_bad: '90 + 6',
    bonus_bad_why: { ru: '90 не делится', uz: "90 bo'linmaydi" },
    bonus_good: '80 + 16',
    bonus_sub: '20 + 4',
    bonus_ans: '24',
    bonus_recap: '96 ÷ 4 = 24',
    audio: {
      ru: [
        'Девяносто шесть разделить на четыре можно, а потом ещё и проверить себя. Пройдём оба способа.',
        'Первый способ. Раскладываем число на удобные части, восемьдесят и шестнадцать.',
        'Восемьдесят разделить на четыре, двадцать. Шестнадцать разделить на четыре, четыре.',
        'Двадцать плюс четыре, двадцать четыре. Первый способ пройден!'
      ],
      uz: [
        "To'qson oltini to'rtga bo'lish mumkin, keyin esa o'zimizni tekshirsak ham bo'ladi. Ikkala usulni ko'ramiz.",
        "Birinchi usul. Sonni qulay bo'laklarga ajratamiz, sakson va o'n olti.",
        "Saksonni to'rtga bo'lsak, yigirma. O'n oltini to'rtga bo'lsak, to'rt.",
        "Yigirma qo'shuv to'rt, yigirma to'rt. Birinchi usul tayyor!"
      ]
    },
    audio2: {
      ru: [
        'Верно! Второй способ, проверка. Умножим ответ обратно на четыре.',
        'Двадцать четыре на четыре считаем по частям. Двадцать на четыре, восемьдесят. Четыре на четыре, шестнадцать.',
        'Восемьдесят плюс шестнадцать, девяносто шесть. Исходное число вернулось!',
        'Деление и умножение, обратные действия. Умножение всегда может проверить деление.',
        'А в качестве бонуса объясню, почему мы взяли восемьдесят и шестнадцать. Привычные девяносто и шесть не годятся, ведь девяносто на четыре без остатка не делится.',
        'Восемьдесят и шестнадцать делятся оба. Двадцать и четыре, вместе двадцать четыре.',
        'Запомни. Каждая часть должна делиться без остатка. Молодец!'
      ],
      uz: [
        "To'g'ri! Ikkinchi usul, tekshirish. Javobni orqaga to'rtga ko'paytiramiz.",
        "Yigirma to'rtni to'rtga bo'laklab ko'paytiramiz. Yigirmani to'rtga ko'paytirsak, sakson. To'rtni to'rtga ko'paytirsak, o'n olti.",
        "Sakson qo'shuv o'n olti, to'qson olti. Asl son qaytdi!",
        "Bo'lish va ko'paytirish, teskari amallar. Ko'paytirish bo'lishni doim tekshira oladi.",
        "Bonus tariqasida nega sakson va o'n oltini olganimizni aytaman. Odatdagi to'qson va olti mos emas, chunki to'qsonni to'rtga qoldiqsiz bo'lib bo'lmaydi.",
        "Sakson va o'n olti esa ikkalasi ham bo'linadi. Yigirma va to'rt, birga yigirma to'rt.",
  ...
  s6: {
    ...
    rule: { ru: 'Чтобы разделить двузначное число, разложи его на удобные части, которые делятся без остатка. Раздели каждую часть и сложи результаты.', uz: "Ikki xonali sonni bo'lish uchun uni qoldiqsiz bo'linadigan qulay bo'laklarga ajrating. Har bo'lakni bo'ling va natijalarni qo'shing." },
    check_expr: { ru: '(80 + 16) ÷ 4', uz: '(80 + 16) ÷ 4' },
    check_q: { ru: 'Чему равно (80 + 16) ÷ 4? Нажми верную запись.', uz: "(80 + 16) ÷ 4 nimaga teng? To'g'ri yozuvni bosing." },
    check_opts: ['80 ÷ 4 + 16 ÷ 4', '80 ÷ 4 + 16', '80 + 16 ÷ 4'],
    check_ci: 0,
    check_ok: { ru: 'Верно. На четыре делятся обе части.', uz: "To'g'ri. To'rtga ikkala bo'lak ham bo'linadi." },
    check_no: { ru: 'Проверь, по полкам раскладывают все модули, а не половину.', uz: "Tekshiring, tokchalarga barcha modullar taqsimlanadi, yarmi emas." },
    audio: {
      ru: [
        'Правило. Чтобы разделить двузначное число, раскладываем его на удобные части, делим каждую и складываем результаты.',
        'Полок было четыре. Значит, на четыре делится и восемьдесят, и шестнадцать. Обе части.',
        'Если разделить только одну часть, вторая кучка модулей останется неразложенной.',
        'И главное условие. Каждая часть должна делиться без остатка. Девяносто и шесть нам не подошли, а восемьдесят и шестнадцать подошли.'
      ],
      uz: [
        "Qoida. Ikki xonali sonni bo'lish uchun uni qulay bo'laklarga ajratamiz, har birini bo'lamiz va natijalarni qo'shamiz.",
        "Tokcha to'rtta edi. Demak, to'rtga sakson ham, o'n olti ham bo'linadi. Ikkala bo'lak.",
        "Faqat bitta bo'lak bo'linsa, ikkinchi uyum modullar taqsimlanmay qolaveradi.",
        "Va eng muhim shart. Har bir bo'lak qoldiqsiz bo'linishi kerak. To'qson va olti bizga mos kelmadi, sakson va o'n olti esa mos keldi."
      ]
    }
  },
  ...
  s7: {
    ...
    items: [
      { expr: '96 ÷ 4', opts: [{ ru: '80 + 16', uz: '80 + 16' }, { ru: '90 + 6', uz: '90 + 6' }, { ru: '9 + 6', uz: '9 + 6' }], ci: 0,
        hints: [null,
          { ru: 'Девяносто на четыре без остатка не делится. Обе части должны делиться.', uz: "To'qsonni to'rtga qoldiqsiz bo'lib bo'lmaydi. Ikkala bo'lak ham bo'linishi kerak." },
          { ru: 'Девять плюс шесть, пятнадцать, а не девяносто шесть. Число должно остаться прежним.', uz: "To'qqiz qo'shuv olti, o'n besh, to'qson olti emas. Son o'zgarmasligi kerak." }] },
      { expr: '65 ÷ 5', opts: [{ ru: '50 + 15', uz: '50 + 15' }, { ru: '62 + 3', uz: '62 + 3' }, { ru: '6 + 5', uz: '6 + 5' }], ci: 0,
        hints: [null,
          { ru: 'Шестьдесят два на пять не делится. Ищи части, которые делятся обе.', uz: "Oltmish ikkini beshga bo'lib bo'lmaydi. Ikkalasi ham bo'linadigan bo'lak qidiring." },
          { ru: 'Шесть плюс пять, одиннадцать. Нам нужно шестьдесят пять.', uz: "Olti qo'shuv besh, o'n bir. Bizga oltmish besh kerak." }] },
      { expr: '84 ÷ 7', opts: [{ ru: '70 + 14', uz: '70 + 14' }, { ru: '80 + 4', uz: '80 + 4' }, { ru: '8 + 4', uz: '8 + 4' }], ci: 0,
        hints: [null,
          { ru: 'Восемьдесят на семь не делится. Здесь привычные десятки и единицы не подходят, бери семьдесят и четырнадцать.', uz: "Saksonni yettiga bo'lib bo'lmaydi. Bu yerda odatdagi o'nlik va birlik yaramaydi, yetmish va o'n to'rtni oling." },
          { ru: 'Восемь плюс четыре, двенадцать. Число должно остаться восемьдесят четыре.', uz: "Sakkiz qo'shuv to'rt, o'n ikki. Son sakson to'rtligicha qolishi kerak." }] }
    ],
    audio: {
  ...
  s8: {
    ...
    items: [
      { expr: '48 ÷ 2', ans: 24, hint: { ru: 'Сорок на два, двадцать. Восемь на два, четыре. Сложи.', uz: "Qirqni ikkiga bo'lsak, yigirma. Sakkizni ikkiga bo'lsak, to'rt. Qo'shing." } },
      { expr: '65 ÷ 5', ans: 13, hint: { ru: 'Возьми пятьдесят и пятнадцать. Пятьдесят на пять, десять. Пятнадцать на пять, три. Сложи.', uz: "Ellik va o'n beshni oling. Ellikni beshga bo'lsak, o'n. O'n beshni beshga bo'lsak, uch. Qo'shing." } },
      { expr: '72 ÷ 4', ans: 18, hint: { ru: 'Возьми сорок и тридцать два. Сорок на четыре, десять. Тридцать два на четыре, восемь. Сложи.', uz: "Qirq va o'ttiz ikkini oling. Qirqni to'rtga bo'lsak, o'n. O'ttiz ikkini to'rtga bo'lsak, sakkiz. Qo'shing." } }
    ],
    audio: {
  ...
  s9: {
    ...
    items: [
      { rows: ['48 ÷ 2 = 20 + 4', '96 ÷ 4 = 20 + 4', '64 ÷ 4 = 15 + 4'], wrong: 2,
        hint: { ru: 'В третьей строке маленькую часть не разделили. Четыре на четыре, один, а не четыре.', uz: "Uchinchi qatorda kichik bo'lak bo'linmagan. To'rtni to'rtga bo'lsak, bir, to'rt emas." } },
      { rows: ['80 ÷ 4 = 2', '80 ÷ 4 = 20', '8 ÷ 4 = 2'], wrong: 0,
        hint: { ru: 'Восемьдесят, это десятки. Два десятка, это двадцать, а не два.', uz: "Sakson, bu o'nliklar. Ikkita o'nlik, bu yigirma, ikki emas." } },
      { rows: ['96 = 80 + 16', '72 = 40 + 32', '65 = 60 + 15'], wrong: 2,
        hint: { ru: 'Шестьдесят плюс пятнадцать, семьдесят пять. А число было шестьдесят пять.', uz: "Oltmish qo'shuv o'n besh, yetmish besh bo'ladi. Son esa oltmish besh edi." } }
    ],
    audio: {
  ...
  s10: {
    ...
    items: [
      { expr: '84 ÷ 4', opts: [{ ru: '21', uz: '21' }, { ru: '20', uz: '20' }, { ru: '80', uz: '80' }], ci: 0,
        hints: [null,
          { ru: 'Двадцать, это только десятки. Четыре на четыре дают ещё один.', uz: "Yigirma, bu faqat o'nliklar. To'rtni to'rtga bo'lsak, yana bir chiqadi." },
          { ru: 'Это вычитание, а не деление. Восемьдесят четыре раскладывают на четыре полки.', uz: "Bu ayirish, bo'lish emas. Sakson to'rtta to'rtta tokchaga taqsimlanadi." }] },
      { expr: '91 ÷ 7', opts: [{ ru: '13', uz: '13' }, { ru: '12', uz: '12' }, { ru: '84', uz: '84' }], ci: 0,
        hints: [null,
          { ru: 'Это ответ для восьмидесяти четырёх. У нас девяносто один, бери семьдесят и двадцать один.', uz: "Bu sakson to'rt uchun javob. Bizda to'qson bir, yetmish va yigirma birni oling." },
          { ru: 'Это девяносто один минус семь. А нам надо разложить на семь полок.', uz: "Bu to'qson birdan yetti ayirish. Bizga esa yettita tokchaga taqsimlash kerak." }] },
      { expr: '75 ÷ 5', opts: [{ ru: '15', uz: '15' }, { ru: '14', uz: '14' }, { ru: '70', uz: '70' }], ci: 0,
        hints: [null,
          { ru: 'Четырнадцать, это только семьдесят. Пять на пять даёт ещё один.', uz: "O'n to'rt, bu faqat yetmish uchun. Beshni beshga bo'lsak, yana bir chiqadi." },
          { ru: 'Это семьдесят пять минус пять. А нам надо разделить на пять равных частей.', uz: "Bu yetmish beshdan besh ayirish. Bizga esa beshta teng bo'lak kerak." }] }
    ],
    audio: {
  ...
  s11: {
    ...
    lead: { ru: 'Жасур раскладывает модули по полкам.', uz: "Jasur modullarni tokchalarga joylamoqda." },
    q: { ru: 'Привезли 68 модулей, полок 4. Сколько модулей на каждой полке?', uz: "68 ta modul keltirildi, tokcha 4 ta. Har tokchada nechta modul bo'ladi?" },
    ans: 17,
    manifest_label: { ru: 'Полки', uz: 'Tokchalar' },
    exp_display: { ru: '68 ÷ 4', uz: '68 ÷ 4' },
    setup_audio: { ru: 'Жасур отвечает за полку распределения. Привезли шестьдесят восемь модулей, а полок четыре.', uz: "Jasur taqsimot rafi uchun javob beradi. Oltmish sakkizta modul keltirildi, tokcha esa to'rtta." },
    audio: {
      intro: { ru: 'Разложи шестьдесят восемь на сорок и двадцать восемь. Раздели обе части на четыре и сложи. Набери ответ цифрами.', uz: "Oltmish sakkizni qirq va yigirma sakkizga ajrating. Ikkala bo'lakni to'rtga bo'ling va qo'shing. Javobni raqamlab tering." },
      on_correct: { ru: 'Верно. Сорок на четыре, десять, двадцать восемь на четыре, семь, вместе семнадцать.', uz: "To'g'ri. Qirqni to'rtga bo'lsak, o'n, yigirma sakkizni to'rtga bo'lsak, yetti, birgalikda o'n yetti." },
      on_wrong: { ru: 'Пока нет. Сорок на четыре, десять. Двадцать восемь на четыре, семь. Сложи их.', uz: "Hozircha yo'q. Qirqni to'rtga bo'lsak, o'n. Yigirma sakkizni to'rtga bo'lsak, yetti. Ularni qo'shing." }
    }
  },
  ...
  s12: {
    ...
    intro_line: { ru: 'Пять заданий на деление двузначного числа.', uz: "Ikki xonali sonni bo'lishga beshta topshiriq." },
    items: [
      { kind: 'mc', q: { ru: 'Какое разложение подходит для 96 ÷ 4?', uz: "96 ÷ 4 uchun qaysi ajratish mos keladi?" }, topic: { ru: 'удобное разложение', uz: 'qulay ajratish' },
        opt0: { ru: '80 + 16', uz: '80 + 16' }, opt1: { ru: '90 + 6', uz: '90 + 6' }, opt2: { ru: '9 + 6', uz: '9 + 6' },
        wrong_1: { ru: 'Девяносто на четыре без остатка не делится. Здесь десятки и единицы не подходят.', uz: "To'qsonni to'rtga qoldiqsiz bo'lib bo'lmaydi. Bu yerda o'nlik va birlik yaramaydi." },
        wrong_2: { ru: 'Девять плюс шесть, пятнадцать. Число должно остаться девяносто шесть.', uz: "To'qqiz qo'shuv olti, o'n besh. Son to'qson oltiligicha qolishi kerak." } },
      { kind: 'mc', q: { ru: 'Чему равно (60 + 12) ÷ 6?', uz: "(60 + 12) ÷ 6 nimaga teng?" }, topic: { ru: 'деление по частям', uz: "bo'laklab bo'lish" },
        opt0: { ru: '60 ÷ 6 + 12 ÷ 6', uz: '60 ÷ 6 + 12 ÷ 6' }, opt1: { ru: '60 ÷ 6 + 12', uz: '60 ÷ 6 + 12' }, opt2: { ru: '60 + 12 ÷ 6', uz: '60 + 12 ÷ 6' },
        wrong_1: { ru: 'Вторая часть тоже делится на шесть, а не остаётся как есть.', uz: "Ikkinchi bo'lak ham oltiga bo'linadi, o'z holicha qolmaydi." },
        wrong_2: { ru: 'Первая часть тоже делится на шесть. Обе части.', uz: "Birinchi bo'lak ham oltiga bo'linadi. Ikkala bo'lak." } },
      { kind: 'num', q: { ru: 'Набери ответ: 96 ÷ 4', uz: "Javobni tering: 96 ÷ 4" }, topic: { ru: 'деление по частям', uz: "bo'laklab bo'lish" }, ans: 24,
        hint: { ru: 'Восемьдесят на четыре, двадцать. Шестнадцать на четыре, четыре.', uz: "Saksonni to'rtga bo'lsak, yigirma. O'n oltini to'rtga bo'lsak, to'rt." } },
      { kind: 'num', q: { ru: 'Набери ответ: 65 ÷ 5', uz: "Javobni tering: 65 ÷ 5" }, topic: { ru: 'деление по частям', uz: "bo'laklab bo'lish" }, ans: 13,
        hint: { ru: 'Пятьдесят на пять, десять. Пятнадцать на пять, три.', uz: "Ellikni beshga bo'lsak, o'n. O'n beshni beshga bo'lsak, uch." } },
      { kind: 'num', q: { ru: 'Модулей 92, полки 4. Сколько на каждой?', uz: "Modul 92 ta, tokcha 4 ta. Har birida nechtadan?" }, topic: { ru: 'задача на деление', uz: "bo'lishga masala" }, ans: 23,
        hint: { ru: 'Восемьдесят на четыре, двадцать. Двенадцать на четыре, три. Сложи.', uz: "Saksonni to'rtga bo'lsak, yigirma. O'n ikkini to'rtga bo'lsak, uch. Qo'shing." } }
    ],
    fact_badge: { ru: 'Знаешь?', uz: 'Bilasizmi?' },
    fact_text: { ru: 'Длинный рычаг помогает поднять тяжёлый груз малой силой. Это правило открыл древний учёный Архимед. Лом, тачка и качели работают по правилу рычага.', uz: "Uzun richag og'ir yukni oz kuch bilan ko'tarishga yordam beradi. Bu qoidani qadimgi olim Arximed ochgan. Lom, zambilg'altak va arg'imchoq ham richag qoidasi bilan ishlaydi." },
    fact_audio: { ru: 'Длинный рычаг помогает поднять тяжёлый груз малой силой. Это правило открыл древний учёный Архимед. Лом, тачка и качели работают по правилу рычага.', uz: "Uzun richag og'ir yukni oz kuch bilan ko'tarishga yordam beradi. Bu qoidani qadimgi olim Arximed ochgan. Lom, zambilg'altak va arg'imchoq ham richag qoidasi bilan ishlaydi." },
    audio: {
  ...
  s13: {
    ...
    mission_done: { ru: 'Полка распределения заполнена!', uz: "Taqsimot rafi to'ldi!" },
    cando: { ru: 'Теперь ты делишь двузначные числа без таблицы.', uz: "Endi siz ikki xonali sonlarni jadvalsiz bo'lasiz." },
    ...
    conn_refs: { ru: 'деление суммы (урок 13) и умножение по частям (урок 19)', uz: "yig'indini bo'lish (13-dars) va bo'laklab ko'paytirish (19-dars)" },
    conn_label_next: { ru: 'Дальше', uz: 'Keyingi dars' },
    conn_next: { ru: 'деление с остатком', uz: "qoldiqli bo'lish" },
    audio: { ru: 'Полка распределения заполнена. Теперь ты умеешь делить двузначные числа без таблицы. Разложи число на части, которые делятся без остатка, раздели каждую и сложи результаты. В следующем уроке узнаем, что делать, когда поровну не делится.', uz: "Taqsimot rafi to'ldi. Endi siz ikki xonali sonlarni jadvalsiz bo'la olasiz. Sonni qoldiqsiz bo'linadigan bo'laklarga ajrating, har birini bo'ling va natijalarni qo'shing. Keyingi darsda teng bo'linmasa nima qilishni bilib olamiz." }
  }
};

// Ekranlararo ko'priklar
const BRIDGES = {
  stway: { ru: 'Один пример, два способа.', uz: "Bitta misol, ikki usul." },
  s1: { ru: 'Посмотрим на модули внимательнее.', uz: "Modullarga diqqat bilan qaraymiz." },
  s2: { ru: 'Начнём с большой части.', uz: "Katta bo'lakdan boshlaymiz." },
  s3: { ru: 'Теперь маленькая часть.', uz: "Endi kichik bo'lak." },
  s4: { ru: 'Осталось собрать полку.', uz: "Tokchani yig'ish qoldi." },
  ...
  s11: { ru: 'Задача из мастерской Жасура.', uz: "Jasur ustaxonasidan masala." },
  s12: { ru: 'Пять заданий на проверку.', uz: "Tekshiruv uchun beshta topshiriq." }
};

const S13_PAYOFF = { ru: 'Полка распределения заполнена поровну. Мастерская работает дальше.', uz: "Taqsimot rafi teng to'ldirildi. Ustaxona ishida davom etmoqda." };
...
// --- MODUL-BLOK (Dars18 birligi): porlovchi kristall modul (Ustaxona rekviziti).
const Modul = ({ className = '' }) => (
  <svg className={`lm-chiroq ${className}`} viewBox="0 0 16 16" aria-hidden="true">
    <rect x="1.5" y="1.5" width="13" height="13" rx="3.5" fill="#4FB7D8" stroke="#2E8DB0" strokeWidth="1"/>
    <rect x="2.5" y="2.5" width="11" height="5.2" rx="2.6" fill="rgba(255,255,255,0.3)"/>
    <circle cx="8" cy="8.6" r="2.7" fill="url(#lmGlow)"/>
  </svg>
);
...
// ============================================================
// EKRANLAR — Dars18 «Ikki xonali sonni bir xonaliga bo'lish» (Б3 «Ustaxona»)
// ============================================================
...
// --- USTAXONA SAHNASI (D20): interyer + markaziy asbob TAQSIMOT RAFI (modul uyumi -> 4 tokcha).
// Dars19 bilan bir hudud (umumiy ustaxona-interyer ohangi), lekin o'z markaziy asbobi.
const WorkshopRackBg = ({ split = false }) => (
  <svg className="lm-scene-bg" viewBox="0 0 400 230" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
    <defs>
      <radialGradient id="workshoprackbgvig" cx="50%" cy="43%" r="72%"><stop offset="0%" stopColor="#000" stopOpacity="0"/><stop offset="76%" stopColor="#000" stopOpacity="0"/><stop offset="100%" stopColor="#2A2018" stopOpacity="0.28"/></radialGradient>
      ...
      <linearGradient id="g0wall" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#EBDCC0"/><stop offset="100%" stopColor="#D2BC92"/></linearGradient>
    ...
    {/* --- DEVOR (ustaxona ichi, iliq panellar) --- */}
    <rect x="0" y="0" width="400" height="178" fill="url(#g0wall)"/>
    <g stroke="#C2A878" strokeWidth="1.2" opacity="0.5">{[66, 200, 334].map((x, i) => <line key={i} x1={x} y1="8" x2={x} y2="170"/>)}</g>
    <rect x="0" y="8" width="400" height="3" rx="1.5" fill="#C2A878" opacity="0.5"/>
    {/* --- DUMALOQ ILLUMINATOR (Lumo osmoni ko'rinadi) --- */}
    <g>
      <circle cx="66" cy="52" r="26" fill="url(#g0sky)" stroke="#9A855C" strokeWidth="3"/>
      <circle cx="58" cy="44" r="5" fill="#C79AD6"/><ellipse cx="58" cy="44" rx="9" ry="2.4" fill="none" stroke="#E6C8F0" strokeWidth="1.2" opacity="0.85"/>
      <circle cx="78" cy="58" r="7" fill="url(#g0sun)"/><circle cx="78" cy="58" r="3.4" fill="#FFD89A"/>
      <path d="M44 62 Q54 56 64 62 Q72 66 84 62" stroke="#FFFFFF" strokeWidth="2" fill="none" opacity="0.5"/>
      <circle cx="66" cy="52" r="26" fill="none" stroke="#C9B084" strokeWidth="1.4" opacity="0.7"/>
    </g>
    {/* --- ASBOB-TAXTA (chap devor): kalit + bolg'a + tishli g'ildirak --- */}
    <g>
      <rect x="118" y="30" width="66" height="44" rx="6" fill="#D8C49C" stroke="#B49A6E" strokeWidth="1.4"/>
      <g stroke="#8A7350" strokeWidth="2.6" fill="none" strokeLinecap="round">
        <path d="M130 40 l10 12"/><circle cx="128" cy="38" r="3.6"/>
      </g>
      <g><rect x="152" y="36" width="4" height="18" rx="2" fill="#8A7350"/><rect x="147" y="33" width="14" height="7" rx="2.5" fill="#8A7350"/></g>
      <g fill="#8A7350"><circle cx="172" cy="46" r="7"/><circle cx="172" cy="46" r="3" fill="#D8C49C"/>{[0, 60, 120, 180, 240, 300].map((a, k) => <rect key={k} x="170.6" y="36.6" width="2.8" height="4" rx="1" transform={`rotate(${a} 172 46)`}/>)}</g>
      <g fill="#C2A878" opacity="0.8"><circle cx="128" cy="66" r="2"/><circle cx="154" cy="66" r="2"/><circle cx="172" cy="64" r="2"/></g>
    </g>
    {/* --- OSMA CHIROQLAR (sim + porlovchi qalpoq) --- */}
    {[240, 330].map((x, i) => (
      <g key={`lp${i}`}>
        <line x1={x} y1="0" x2={x} y2="26" stroke="#8A7350" strokeWidth="1.6"/>
        <path d={`M${x - 10} 26 h20 l-4 8 h-12 Z`} fill="#C9B084" stroke="#9A855C" strokeWidth="1"/>
        <circle className="lm-glow" style={{ animationDelay: `${i * 0.7}s` }} cx={x} cy="38" r="4.5" fill="url(#lmGlow)"/>
      </g>
    ))}
    {/* --- MARKAZIY ASBOB: TAQSIMOT RAFI (o'ngda) — 4 tokchali raf --- */}
    <g>
      <rect x="252" y="66" width="120" height="110" rx="7" fill="url(#g0bed)" stroke="#6B4E34" strokeWidth="2"/>
      <rect x="258" y="72" width="108" height="98" rx="5" fill="#8F6C4A"/>
      {[0, 1, 2, 3].map((i) => {
        const y = 78 + i * 24;
        return (
          <g key={`sh${i}`}>
            <rect x="260" y={y + 14} width="104" height="5" rx="2.5" fill="#C9A06E" stroke="#6B4E34" strokeWidth="0.8"/>
            <rect x="262" y={y} width="16" height="13" rx="3" fill="#5FBFE0" opacity="0.25"/>
            {/* tokchadagi modullar: split'da to'lib porlaydi */}
            {(split ? [0, 1, 2, 3, 4] : [0, 1]).map((m) => (
              <g key={m}>
                <rect x={284 + m * 15} y={y + 1.5} width="11" height="11" rx="2.6" fill="#4FB7D8" stroke="#2E8DB0" strokeWidth="0.8"/>
                {split && <circle className="lm-glow" style={{ animationDelay: `${(i * 5 + m) * 0.09}s` }} cx={289.5 + m * 15} cy={y + 7} r="2" fill="#FFF4D0"/>}
              </g>
            ))}
            <text x="270" y={y + 10.5} textAnchor="middle" fontSize="8" fontWeight="800" fill="#2E4E60" fontFamily="'JetBrains Mono', monospace">{i + 1}</text>
          </g>
        );
      })}
    </g>
    {/* --- MODUL UYUMI (chapda, palletda) --- */}
    <rect x="20" y="152" width="92" height="12" rx="4" fill="url(#g0bed)"/><rect x="20" y="152" width="92" height="4" rx="2" fill="#B78E64"/>
    {[0, 1, 2].map((r) => Array.from({ length: 6 }).map((_, i) => {
      const x = 28 + i * 14 + (r % 2 ? 7 : 0);
      const y = 138 - r * 12;
      return (
        <g key={`m${r}-${i}`}>
          <rect x={x} y={y} width="11" height="11" rx="2.6" fill="#4FB7D8" stroke="#2E8DB0" strokeWidth="0.9"/>
          <circle className="lm-glow" style={{ animationDelay: `${(r * 6 + i) * 0.07}s` }} cx={x + 5.5} cy={y + 5.5} r="1.9" fill="#FFF4D0"/>
        </g>
      );
    }))}
    {split && <text x="66" y="176" textAnchor="middle" fontSize="9" fontWeight="800" fill="#8A7A5E" fontFamily="'JetBrains Mono', monospace">80 + 16</text>}
    {/* konveyer-strelka: uyumdan rafga */}
    <path d="M118 138 h116" stroke="#B08A5E" strokeWidth="2.4" fill="none" strokeDasharray="7 5"/>
    <path d="M240 138 l-7 -4.2 v8.4 Z" fill="#B08A5E"/>
    {/* tenglama banneri (to'sindan osilgan): javobgacha savol, javobdan keyin yoyilma */}
    <path d="M138 22 h110 v15 l-6 5 l-6 -5 l-6 5 l-6 -5 l-6 5 l-6 -5 l-6 5 l-6 -5 l-6 5 l-6 -5 l-6 5 l-6 -5 l-6 5 l-6 -5 l-6 5 Z" fill="#BFDCEB" stroke="#8FB6CE" strokeWidth="1"/>
    <text x="193" y="34" textAnchor="middle" fontSize="11" fontWeight="800" fill="#2E5E7C" fontFamily="'JetBrains Mono', monospace">{split ? '(80 + 16) ÷ 4' : '96 ÷ 4 = ?'}</text>
    {/* --- POL: ustaxona plitasi + perspektiva --- */}
    ...
    {/* --- OLD PLAN: yashik + porlovchi kristall-quvvat --- */}
    <g transform="translate(20 176)"><rect x="-12" y="-16" width="24" height="16" rx="3" fill="#C98A6A" stroke="#A56B4A" strokeWidth="1.2"/><line x1="-12" y1="-8" x2="12" y2="-8" stroke="#A56B4A" strokeWidth="1"/><circle className="lm-glow" cx="0" cy="-20" r="4" fill="#8FE8C0"/></g>
    <g transform="translate(382 176)"><path d="M-3 0 L-6 -14 L0 -20 L5 -12 L3 0 Z" fill="#BEA0F0" opacity="0.9"/><circle className="lm-glow" cx="0" cy="-12" r="2.6" fill="#FFF7D6"/></g>
    <g><circle className="lm-glow" cx="110" cy="88" r="1.5" fill="#FFE0B0"/><circle className="lm-glow" style={{ animationDelay: '1s' }} cx="222" cy="60" r="1.4" fill="#CFE8FF"/></g>
      {/* REALIZM: vinetka — chekka qorong'ilashuvi (chuqurlik + markazga fokus) */}
    <rect x="0" y="0" width="400" height="230" fill="url(#workshoprackbgvig)" style={{ pointerEvents: 'none' }}/>
  </svg>
);
  ...
  return (
    <div style={{ display: 'inline-grid', gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: gap || 'clamp(3px, 1vw, 7px)', padding: 'clamp(7px, 1.8vw, 13px)', background: '#152342', borderRadius: 12 }}>
      {Array.from({ length: rows * cols }).map((_, i) => (
        <span key={i} className="g1-pop-in" style={{ animationDelay: `${i * 0.012}s`, width: d, height: d, display: 'inline-flex' }}><Chiroq/></span>
      ))}
    </div>
  );
};
...
// s0 — HOOK: ustaxona, 96 modul uyumi + 4 bo'sh tokcha (etalon hook-reveal)
const Screen0 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s0;
...
// bad=true — RAD ETILGAN ajratish (90+6): qizil, ustidan chizilgan (mos-emas ko'rsatkichi).
const ChainStep = ({ expr, tag, last, bad }) => (
  <div className={`lm-chainstep lm-edrop ${last ? 'lm-chainstep-last' : ''}`}>
    <span className="mono lm-chain-expr" style={bad ? { color: '#C0392B', opacity: 0.6, textDecoration: 'line-through' } : undefined}>{expr}</span>
    <span className="lm-chain-tag" style={bad ? { color: '#C0392B' } : undefined}>{bad ? `✗ ${tag}` : tag}</span>
  </div>
);
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 2vw, 14px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'stretch', justifyContent: 'center', gap: 'clamp(8px, 1.8vw, 12px)', padding: 'clamp(14px, 2.6vw, 22px)', minHeight: 'clamp(150px, 32vw, 210px)' }}>
          ...
          {c.steps.map((s, i) => (shown.includes(s)
            ? <ChainStep key={i} expr={s.expr} tag={t(s.tag)} bad={s.bad} last={i === c.steps.length - 1 && c.steps.length > 1}/>
            : null))}
        </div>
        {done && (
...
// Modul-vizual: s1 da 96 ta modul QULAY bo'laklarga AJRALADI — 8 o'nlik quti (80) va
// 1 o'nlik quti + 6 yakka (16). Kashfiyot mag'zi: o'nlikni birliklar tomonga berish.
const TenBox = () => (
  <span className="lm-tenbox">{Array.from({ length: 10 }).map((_, i) => <span key={i} className="lm-lamp-t"><Modul/></span>)}</span>
);
const ShelfSplit = ({ ck, reached }) => {
  if (ck !== 's1') return null;
  const split = reached >= 3;
  return (
    <div className={`lm-shelf ${split ? 'lm-shelf-split' : ''}`}>
      <div className="lm-shelf-part lm-shelf-big">
        <div className="lm-tenrow">{Array.from({ length: 8 }).map((_, i) => <TenBox key={i}/>)}</div>
        {split && <span className="mono lm-shelf-num lm-edrop">80</span>}
      </div>
      {split && <span className="mono lm-shelf-plus lm-edrop">+</span>}
      <div className="lm-shelf-part lm-shelf-small">
        <div className="lm-tenrow"><TenBox/></div>
        <div className="lm-shelf-grid">{Array.from({ length: 6 }).map((_, i) => <span key={i} className="lm-lamp-s"><Modul/></span>)}</div>
        {split && <span className="mono lm-shelf-num lm-edrop">16</span>}
      </div>
    </div>
  );
};

// Hook-vizual: 96 ta modul UYUM bo'lib yotibdi (hech qanday qator/ustun javobni aytmaydi) + 4 bo'sh tokcha.
const BasketSVG = () => (
  <svg viewBox="0 0 40 34" style={{ width: 'clamp(30px, 9vw, 44px)', height: 'auto' }} aria-hidden="true">
    <rect x="3" y="9" width="34" height="23" rx="3" fill="#C9A06E" stroke="#8A6B44" strokeWidth="1.4"/>
    <rect x="3" y="9" width="34" height="6" rx="3" fill="#DDB988" stroke="#8A6B44" strokeWidth="1"/>
    <g stroke="#8A6B44" strokeWidth="1" opacity="0.7"><line x1="13" y1="16" x2="13" y2="30"/><line x1="27" y1="16" x2="27" y2="30"/></g>
    <rect x="16" y="3" width="8" height="6" rx="2" fill="#8A6B44"/>
    <rect x="7" y="19" width="8" height="8" rx="2" fill="#4FB7D8" opacity="0.35"/>
  </svg>
);
  ...
  <div className="lm-heapwrap">
    <div className="lm-heap">
      {Array.from({ length: count }).map((_, i) => (
        <span key={i} className="g1-pop-in lm-lamp-s" style={{ animationDelay: `${i * 0.012}s` }}><Modul/></span>
      ))}
    </div>
...
// s7 — MASHQ: qulay ajratishni tanla (3 raund, MC)
const ExprFig = (it) => (
...
// s8 — MASHQ: bo'laklarga ajratib hisobla (3 raund, NumPad)
const Screen8 = (props) => {
  const lang = useLang();
  const t = useT();
  const c = CONTENT.s8;
...
// s11 — MASALA (case): Jasur taqsimot rafi, NumPad
...
// stway ranglari — tekshirish bo'laklari (20×4 va 4×4) uchun.
const TW_COLS = ['#C0392B', '#1F7A4D'];
// 1-usul (qulay bo'laklar): bosqich qatorlari birma-bir tushadi; ajratish 1-qatorda aksentda.
const TwM1Drop = ({ rows }) => (
  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(4px, 1vw, 8px)' }}>
    {rows.map((r, i) => (
      <span key={i} className="mono lm-edrop" style={{ animationDelay: `${0.1 + i * 0.85}s`, fontSize: 'clamp(18px, 3.6vw, 26px)', fontWeight: 800, color: T.ink }}>
        {r.acc ? <>{r.pre}<span style={{ color: T.accent, background: '#FFF3E9', borderRadius: 6, padding: '0 4px' }}>{r.acc}</span>{r.post}</> : r.t}
      </span>
    ))}
  </div>
);
// Yakuniy natija — yashil pilyulada (yig'ilgandan keyin ham qoladi).
...
// 2-usul (ko'paytirish bilan tekshirish): 20×4 va 4×4 alohida tushadi, ostiga 80 va 16; keyin 80 + 16 qatori.
...
// Bonus (nega 90+6 emas): mos kelmagan ajratish — qizil xira, ustidan chizilgan + sabab-yorliq.
...
// Bonus zanjiri: mos bo'laklar 80 + 16 yashil, ostida 20 + 4, natija pilyulada.
...
// Oqim: 1-usul (qulay bo'laklar) -> BASHORAT (tekshirsa bo'ladimi?) -> 2-usul (24×4=96 teskari amal) -> ★ bonus (nega 90+6 emas).
const TwoWayScreen = (props) => {
  ...
  const showM1w = all || r1 >= 3;    // natija-pilyula
  const showM2d = all || r2 >= 1;    // bo'laklar tushadi
  const showM2w = all || r2 >= 2;    // 30 + 20 va natija-pilyula
  const showBonus  = ok && (all || r2 >= 4);
  const showBonus2 = all || r2 >= 5; // mos bo'laklar zanjiri (30 + 12)
...
/* Ikki tokcha: 96 ta modul 80 lik blok + 16 ga ajraladi */
...
/* O'nlik qutilar (60 = 6 x 10) */
.lm-tenrow { display: grid; grid-template-columns: repeat(3, 1fr); gap: 3px; }
.lm-tenbox { display: grid; grid-template-columns: repeat(5, 1fr); gap: 1px; padding: 2px; border-radius: 4px; background: #1E2E52; }
.lm-lamp-t { width: clamp(5px, 1.6vw, 8px); height: clamp(5px, 1.6vw, 8px); display: inline-flex; }
/* Hook: hosil uyumi + bo'sh savatlar */
.lm-heapwrap { display: flex; flex-direction: column; align-items: center; gap: clamp(8px, 1.8vw, 12px); width: 100%; }
.lm-heap { display: flex; flex-wrap: wrap; justify-content: center; gap: 3px; max-width: 340px; padding: clamp(7px, 1.8vw, 12px); background: #152342; border-radius: 12px; }
.lm-baskets { display: flex; justify-content: center; gap: clamp(10px, 3vw, 20px); }
...
export function Grade3WorkshopEtalonScene({ complete = false }) {
  return (

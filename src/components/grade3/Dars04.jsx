...
const CompareRound = ({ props, ck }) => {
  ...
  const pick = (sgn) => {
    ...
    if (sgn === it.sign) {
      ...
      const n = new Set(wrongSet); n.delete(sgn); n.add(sgn); setWrongSet(n);
          ...
          <>
            ...
            <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.6vw, 20px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
              ...
              <div className="lm-signrow">
                {CMP_SIGNS.map((sgn) => (
                  <button key={sgn} className={`lm-signbtn mono ${wrongSet.has(sgn) ? 'lm-signbtn-wrong' : ''} ${picked === sgn ? 'lm-signbtn-ok' : ''}`} disabled={!canAct || solvedRound} onClick={() => pick(sgn)}>{sgn}</button>
                ))}
              </div>
    ...
    <Stage eyebrow={c.eyebrow} screen={props.screen} totalScreens={TOTAL_SCREENS} navContent={navContent} audioState={audio}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'clamp(12px, 2.2vw, 16px)' }}>
        ...
        <div className="frame fade-up delay-1" style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'clamp(12px, 2.6vw, 18px)', padding: 'clamp(14px, 2.6vw, 20px)' }}>
          ...
          <div className="lm-signrow">
            {CMP_SIGNS.map((sgn) => (
              <button key={sgn} className={`lm-signbtn mono ${wrongSet.has(sgn) ? 'lm-signbtn-wrong' : ''} ${picked === sgn ? 'lm-signbtn-ok' : ''}`} disabled={!canAct || solved} onClick={() => pick(sgn)}>{sgn}</button>
            ))}
          </div>
        </div>
        {solved && (

...
const CONTENT = {
  ...
  s5: {
    ...
    done_text: { ru: 'От 347 до 300 сорок семь шагов, а до 400 больше пятидесяти. Ближе триста.', uz: "347 dan 300 gacha qirq yetti qadam, 400 gacha ellikdan ko'p. Uch yuz yaqinroq." },
    audio: {
      ru: [
  ...
  s7: {
    ...
    items: [
      ...
      {
        num: 445, ci: 0,
        opts: [{ ru: '450', uz: '450' }, { ru: '440', uz: '440' }, { ru: '445', uz: '445' }],
        hints: {
          1: { ru: '445 ровно посередине между 440 и 450. Берём большее, 450.', uz: "445 soni 440 bilan 450 orasida aynan o'rtada. Kattasini olamiz — 450." },
          2: { ru: 'Круглый ответ оканчивается на ноль.', uz: "Yumaloq javob nol bilan tugaydi." }
        }
      }
    ],
...
const Screen0 = (props) => {
  ...
  const [picked, setPicked] = useState(wasSolved ? 1 : null);
  const firstTryRef = useRef(
    props.storedAnswer
      ? (props.storedAnswer.firstTry ?? props.storedAnswer.correct ?? null)
      : null
  );
  ...
  const pick = (i) => {
    ...
    setPicked(i);
    if (isCorrect) {
      const originalOptions = [c.opt0, c.opt1, c.opt2];
      props.onAnswer?.({
        stage: SCREEN_META[props.screen]?.scope ?? null,
        screenIdx: props.screen,
        question: t(c.q),
        options: originalOptions.map((option) => t(option)),
        correctIndex: 1,
        correctAnswer: t(c.opt1),
        studentAnswerIndex: firstIdxRef.current,
        studentAnswer: t(originalOptions[firstIdxRef.current]),

...
const S2 = {
  ...
  props: {
    ...
    // ПОДСТАНОВКА (методист, 2026-08-17). Четыре записи против двух значений
    // a. При тройке считаются ВСЕ ЧЕТЫРЕ, разницы нет никакой. При пятёрке
    // одна остаётся без значения. Ученик видит, что дело не в виде записи, а
    // в одном числе, и это же он потом проверяет сам на экранах практики.
    proof: {
      wide: true,
      ...
      rows: [
        // Три записи, а не четыре: «3a − 4» и «a · a + a» обе без деления и в
        // таблице говорят одно и то же, а вертикали на экране нет. Оставлена
        // пара, которая и создаёт различение: дробь, которая считается всегда,
        // против дроби, которая считается не всегда.
        { v: '3a − 4', left: '5', right: '11' },
        { v: <Frac num="a + 1" den="2"/>, left: '2', right: '3' },
        { v: <Frac num="7" den="a − 5"/>, left: <Frac num="−7" den="2"/>, dead: true },
      ],
    },
    after: L(
...
const S4 = {
  ...
  props: {
    ...
    // ПОДСТАНОВКА ВМЕСТО УТВЕРЖДЕНИЯ (методист, 2026-08-17). Три значения a
    // подставляются в обе записи. При семёрке и единице считаются обе, при
    // нуле у второй значения нет. Вывод теперь ВИДЕН на числах, а не объявлен
    // словами, и это тот же приём, который урок требует на экране 15.
    proof: {
      varLabel: 'a',
      leftLabel: <Frac num="a" den="7"/>,
      rightLabel: <Frac num="7" den="a"/>,
      noValue: L("qiymat yo'q", 'значения нет', 'no value'),
      rows: [
        { v: '7', left: '1', right: '1' },
        { v: '1', left: <Frac num="1" den="7"/>, right: '7' },
        { v: '0', left: '0', dead: true },
      ],
    },

...
/* Сцена НЕ тянется, если на экране есть прибор: оба росли и налезали друг
   на друга (замер 2026-08-16, хук). Тянется прибор, сцена держит свой размер
   и сжимается только при нехватке места. */
.g8-scene-hook { flex: 0 1 auto; }
.g8-frame-fig {
  flex: 1 1 auto;
  min-height: 0;
  display: flex;
...
/* Потолок вернулся: без него сцена съедала всю высоту, прибор не помещался
   и блоки НАЛОЖИЛИСЬ друг на друга — запись поверх подписи, вопрос поверх
   чисел. Сцена важна, но она не единственное на экране. */
.g8-scene-hook svg { height: auto; max-height: min(30vh, 260px); width: auto; }
...
/* ====================================================================
   ОДИНАКОВАЯ ПОСАДКА НА ВСЕХ ЭКРАНАХ (методист, 2026-08-15). На хуке
   содержимое стоит по центру рабочей зоны и от этого выглядит собранным;
   на остальных экранах оно липло к верху, и урок «прыгал» при листании.
   Заголовок остаётся сверху, а рабочая часть центрируется в остатке.
   ==================================================================== */
/* Содержимое прижато КВЕРХУ (методист, 2026-08-16). Центрирование выравнивало
   экраны между собой, но на коротких экранах отрывало заголовок от блока и
   оставляло пустоту сверху. Верх стабильнее: новый блок всегда появляется
   НИЖЕ последнего, и экран не подпрыгивает. */
.g8-stack > .g8-title, .g8-stack > .g8-lead { flex: 0 0 auto; }
.g8-stack { justify-content: flex-start; }

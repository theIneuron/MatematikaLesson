...
function Screen0({ screen, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><SetVisual screen={screen} frame={audio.frame}/><BeatList frames={c.frames} frame={audio.frame}/><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

function TheoryScreen({ screen, onNext, onPrev }) {
  const c = CONTENT['s' + screen]; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><SetVisual screen={screen} frame={audio.frame}/><BeatList frames={c.frames} frame={audio.frame}/></div></Stage>;
}
...
function Screen14({ screen, answers, onPrev, finishLesson }) {
  const t = useT(); const c = CONTENT.s14; const audio = useNarration(c.audio, screen); const reduced = usePrefersReducedMotion(); const unlocked = audio.frame >= 4 || audio.completed || audio.muted || reduced;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={finishLesson} finish><div className="stack"><Heading c={c} bit="happy"/><OverlappingCriteriaDiagram frame={Math.min(audio.frame, 3)} focusZone={audio.frame >= 4 ? 'both' : null}/><BeatList frames={c.frames} frame={audio.frame}/><div className={'finale-bridge ' + (audio.frame >= 4 ? 'show' : '')}><small>{t({ uz: 'KEYINGI DARS', ru: 'СЛЕДУЮЩИЙ УРОК', en: 'NEXT LESSON' })}</small><strong>{t({ uz: "Uzunlik birliklari", ru: 'Единицы длины', en: 'Units of length' })}</strong></div><G4TitleReward unlocked={unlocked} title={{ uz: "Saralash tadqiqotchisi", ru: 'Исследователь сортировки', en: 'Sorting Explorer' }} answers={answers}/></div></Stage>;
}

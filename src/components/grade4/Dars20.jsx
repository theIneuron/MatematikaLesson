...
const ModelChoices = ({ mode = 'addition', picked, solved, correctIndex, onPick, options = [] }) => {
  const t = useT();
...
function Screen0({ screen, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><section className="hook-model"><FractionBar den={8} cyan={2} lime={frame >= 1 ? 3 : 0} label={frame >= 2 ? <span><Frac n="2" d="8"/> + <Frac n="3" d="8"/> = <Frac n="5" d="16"/> ?</span> : null}/></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}
...
function Screen1({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><FractionBar den={frame === 0 ? 1 : 8} cyan={frame >= 2 ? 2 : 0} lime={frame >= 2 ? 3 : 0} divisions={frame >= 1}/><button type="button" className="tiny-action" aria-pressed={unitTapped} onClick={revealUnit}>{t({ uz: "Har biri 1/8", ru: 'Каждая часть: 1/8', en: 'Each part: 1/8' })}</button><FeedbackBlock show={unitTapped} correct>{t(unitFeedback)}</FeedbackBlock><div className={'state-note ' + (frame >= 3 ? 'show' : '')}>{t({ uz: "Ikkalasi ham bir xil sakkizdan ulushlar", ru: 'Обе величины состоят из одинаковых восьмых долей', en: 'Both amounts are made of equal eighths' })}</div></section></div></Stage>;
}
function Screen2({ screen, onNext, onPrev }) {
  const c = CONTENT.s2; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><FractionBar den={8} cyan={Math.min(frame, 2)} label={frame >= 3 ? <Frac n="2" d="8" size="lg"/> : null}/></section></div></Stage>;
}
...
function Screen3({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><FractionBar den={8} cyan={2} lime={frame >= 2 ? 3 : 0}/>{frame >= 1 && <div className="tokens"><i/><i/><i/><span>{t({ uz: "yana 3 ta ulush", ru: 'ещё 3 доли', en: '3 more parts' })}</span></div>}<div className={'formula-card ' + (frame >= 3 ? 'show' : '')}><Frac n="2" d="8"/> + <Frac n="3" d="8"/></div></section></div></Stage>;
}
...
function Screen5({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="rule-card"><div className={'rule-line ' + (frame >= 0 ? 'show' : '')}>(2 + 3) / 8</div><div className={'rule-line accent ' + (frame >= 1 ? 'show' : '')}>5 / 8</div><div className={'state-note ' + (frame >= 2 ? 'show' : '')}>{t({ uz: "8 ta bo'linma o'zgarmadi", ru: '8 делений не изменились', en: 'The 8 divisions did not change' })}</div><div className={'wrong-formula ' + (frame >= 3 ? 'show' : '')}>5 / 16</div></section></div></Stage>;
}

function Screen6({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s6; const audio = useNarration(c.audio, screen); const [marker, setMarker] = useState(4);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><NumberLine den={8} first={2} second={5} frame={audio.frame} marker={marker}/><label className="marker-control"><span>{t({ uz: "Erkin belgi", ru: 'Свободная метка' , en: "Free marker"})}: {marker}/8</span><input className="free-marker" type="range" min="0" max="8" step="1" value={marker} onChange={(event) => setMarker(Number(event.target.value))}/></label><div className={'formula-card ' + (audio.frame >= 3 ? 'show' : '')}><Frac n="2" d="8"/> + <Frac n="3" d="8"/> = <Frac n="5" d="8"/></div></section></div></Stage>;
}
function Screen7({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="whole-card"><FractionBar den={6} cyan={3} lime={frame >= 1 ? 3 : 0} merged={frame >= 2} label={label}/><div className={'formula-card ' + (frame >= 3 ? 'show' : '')}><Frac n="6" d="6"/> = 1</div><div className={'state-note ' + (frame >= 4 ? 'show' : '')}>{t({ uz: "Suratlarni qo'shing, maxrajni saqlang", ru: 'Сложи числители, сохрани знаменатель', en: 'Add the numerators and keep the denominator' })}</div></section></div></Stage>;
}
...
const FinaleReward = ({ answers = [], complete, audio }) => {
  ...
  return <G4FinalTitleReward finalFrameReached={complete} completed={audio?.completed} muted={audio?.muted} title={{ uz: "Kasrlar yig'indisi ustasi", ru: 'Мастер суммы дробей', en: 'Fraction Addition Master' }} firstTry={firstTry} total={scored.length} />;
};

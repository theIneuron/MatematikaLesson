...
function Screen0({ screen, storedAnswer, onAnswer, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext} nextDisabled={picked === null || !narrationReady}><div className="stack"><Heading c={c} bit="think"/><section className="hook-model"><FractionBar den={8} cyan={2} lime={frame >= 1 ? 3 : 0} label={frame >= 2 ? <span><Frac n="2" d="8"/> + <Frac n="3" d="8"/> = <Frac n="5" d="16"/> ?</span> : null}/></section><section className="question" data-g4-role="answer-card"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral disabled={!narrationReady}/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

...
const CONTENT = {
  s0: {
    ...
    audio: { intro: { uz: ["Yuk nazoratida bitta yuk turibdi.", "Yukning birinchi yorlig'ida bir tonna deb yozilgan.", "Ikkinchi yorliqda ming kilogramm yozilgan. Ular tengligini taxmin qiling."], ru: ['На контроле находится один груз.', 'На первой метке груза написана одна тонна.', 'На второй метке написана тысяча килограммов. Предположи, равны ли эти записи.'], en: ["There is one load at the cargo checkpoint.", "The first label on the load says one tonne.", "The second label says one thousand kilograms. Predict whether the two measurements are equal."] } },
  },
  s1: {
...
const BeatList = ({ frames = [], frame, solved = false }) => {
  const t = useT();
  return <div className="beat-list">{frames.map((item, index) => <div key={index} className={'beat ' + (index <= frame || solved ? 'show' : '')}><b>{index + 1}</b><span>{t(item)}</span></div>)}</div>;
};
const MASS_UNITS = [
...
function ChoiceExercise({ screen, storedAnswer, onAnswer, onNext, onPrev }) {
  ...
  const pick = (index) => {
    if (solved) return; attempts.current += 1; const ok = index === c.correctIndex; if (!ok) clean.current = false; setPicked(index); setSolved(ok);
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c} bit={screen === 11 ? 'awkward' : null}/><MassVisual screen={screen} frame={audio.frame} solved={solved}/><BeatList frames={c.frames} frame={audio.frame} solved={solved}/><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} correctIndex={c.correctIndex} solved={solved}/><FeedbackBlock show={picked !== null} correct={solved}>{picked !== null ? t(c.feedback[picked]) : ''}</FeedbackBlock>{solved && <div className="proof">{t(c.proof)}</div>}</section></div></Stage>;
}
function InputExercise({ screen, storedAnswer, onAnswer, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><MassVisual screen={screen} frame={audio.frame} solved={solved}/><BeatList frames={c.frames} frame={audio.frame} solved={solved}/><section className="question"><h2>{t(c.question)}</h2><div className="input-row"><input inputMode="numeric" value={value} onChange={(event) => { setValue(event.target.value.replace(/[^\d]/g, '')); setChecked(false); }} aria-label={t(c.question)}/><span>{t({ uz: 'kg', ru: 'кг', en: "kg" })}</span></div><div className="input-check-row"><button type="button" className="btn-white-accent" onClick={check}>{t({ uz: 'Tekshirish', ru: 'Проверить', en: "Check" })}</button></div><FeedbackBlock show={checked} correct={solved}>{t(solved ? c.inputFeedback.correct : c.inputFeedback.wrong)}</FeedbackBlock>{solved && <div className="proof">{t(c.proof)}</div>}</section></div></Stage>;
}
function Screen0({ screen, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><MassVisual screen={screen} frame={audio.frame}/><BeatList frames={c.frames} frame={audio.frame}/><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}
function TheoryScreen({ screen, onNext, onPrev }) {
  const c = CONTENT['s' + screen]; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><MassVisual screen={screen} frame={audio.frame}/><BeatList frames={c.frames} frame={audio.frame}/></div></Stage>;
}
...
function Screen14({ screen, answers, onPrev, finishLesson }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={finishLesson} finish><div className="stack"><Heading c={c} bit="happy"/><MassVisual screen={screen} frame={audio.frame}/><BeatList frames={c.frames} frame={audio.frame}/><G4TitleReward unlocked={unlocked} title={{ uz: "Massa nazoratchisi", ru: 'Контролёр массы', en: "Mass inspector" }} answers={answers}/></div></Stage>;
}

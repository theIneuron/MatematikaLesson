...
function Task({ task, lang, isLast, onSolved }) {
  ...
  const answerCorrect = () => {
    if (task.kind === 'mc') return Boolean(task.options[picked]?.correct);
  ...
  const wrongText = adaptive(task, task.kind === 'mc' ? task.options[picked] : null, attempts);

...
function Screen0({ screen, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><section className="model-card"><RectangleDiagram a={3} b={2} frame={audio.frame >= 1 ? 3 : -1}/><div className="dimension-note">{t({ uz: "3 sm × 2 sm", ru: '3 см × 2 см', en: '3 cm × 2 cm' })}</div></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={setPicked} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}
...
.stage-content{overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain}.stage-nav{position:relative;z-index:6}

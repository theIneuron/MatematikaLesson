...
const CONTENT = {
  s0: {
    ...
    options: [
      ...
      { uz: "Faqat 2 tomonni qo'shish", ru: 'Сложить только 2 стороны', en: 'Add only 2 sides' },
    ],
...
const Heading = ({ c }) => { const t = useT(); const hook = c === CONTENT.s0; return <div className="heading" data-g4-screen={hook ? 'hook' : undefined} data-g4-role={hook ? 'hook-scene' : undefined}><div><span>{t(c.eyebrow)}</span><h1>{t(c.title)}</h1></div><BitSVG state="happy" className="primary-happy-bit"/>{hook && <span className="sr-only" data-g4-role="answer-card">{t(c.question)}</span>}</div>; };
...
function Screen0({ screen, storedAnswer, onAnswer, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext} activityDone={picked !== null}><div className="stack"><Heading c={c} bit="think"/><section className="model-card"><RectangleDiagram a={3} b={2} frame={audio.frame >= 1 ? 3 : -1}/><div className="dimension-note">{t({ uz: "3 sm × 2 sm", ru: '3 см × 2 см', en: '3 cm × 2 cm' })}</div></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

...
function Task({ task, lang, isLast, onSolved ,
  ...
  const answerCorrect = (
    ...
    || (task.kind === 'place' && picked?.correct === true)
  ...
  const wrongText = (() => {
    ...
    if (task.kind === 'place') return picked?.wrong;
  ...
  const check = () => { if (mode === 'review') return; setChecked(true); setAttempts((old) => old + 1); if (!answerCorrect) setWrongRound((old) => old + 1); };
      ...
      {task.kind === 'match' && <div className="p4-match">
        ...
        <div className="p4-match-cols">
          <div className="p4-match-col">{task.pairs.map((pair) => (
            <button key={pair.id} type="button" className={`p4-match-item ${activeLeft === pair.id ? 'is-active' : ''} ${pairs[pair.id] ? 'is-tied' : ''}${matchToneLeft(task, pairs, pair.id)}`} aria-pressed={activeLeft === pair.id} disabled={solved} onClick={() => { setActiveLeft(pair.id); setChecked(false); }}>
          ...
          <div className="p4-match-col">{rightCards.map((right) => {
            ...
            return <button key={right.id} type="button" className={`p4-match-item p4-match-right ${used ? 'is-used' : ''}${matchToneRight(task, pairs, right.id)}`} aria-pressed={used} disabled={solved || activeLeft === null} onClick={() => connect(right.id)}>{tx(right.text, lang)}</button>;
          })}</div>
        </div>
      </div>}

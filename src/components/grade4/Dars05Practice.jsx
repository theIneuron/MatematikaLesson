      ...
      {task.kind === 'match' && <div className="p4-match">
        ...
        <div className="p4-match-cols">
          <div className="p4-match-col">{task.pairs.map((pair, i) => (
            <button key={pair.id} type="button" className={`p4-match-item ${activeLeft === i ? 'is-active' : ''} ${pairs[i] ? 'is-tied' : ''} ${firstMatchWrong === i ? 'is-no' : ''}${matchToneLeft(task, pairs, i)}`} disabled={solved} onClick={() => { setActiveLeft(i); setChecked(false); }}>
          ...
          <div className="p4-match-col">{rightPairs.map((pair) => (
            <button key={pair.id} type="button" className={`p4-match-item p4-match-right${matchToneRight(task, pairs, pair.id)}`} disabled={solved || activeLeft === null} onClick={() => {

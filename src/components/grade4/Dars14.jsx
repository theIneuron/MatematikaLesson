...
function Screen0({ screen, storedAnswer, onAnswer, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack" data-g4-screen="hook"><Heading c={c}/><h2 className="hook-question-title" data-g4-role="hook-question">{t(c.question)}</h2><div className="hook-scene-shell" data-g4-role="hook-scene"><section className="motion-card hook-motion" data-g4-role="visual-frame"><span className="hook-frame-bit" data-g4-role="hook-bit"><BitSVG state="think"/></span><FixedTrack distance="180 km" chunks={3} progress={progress} labels={['0', '1', '2', '3']} /><div className="hook-facts"><Reveal show={audio.beat >= 0}><span>{t({ uz: "Masofa", ru: 'Расстояние', en: 'Distance' })}</span><b>180 km</b></Reveal><Reveal show={audio.beat >= 1}><span>{t({ uz: "Vaqt", ru: 'Время', en: 'Time' })}</span><b>3 {t({ uz: "soat", ru: 'часа', en: 'hours' })}</b></Reveal><Reveal show={audio.beat >= 2}><span>{t({ uz: "Tezlik", ru: 'Скорость', en: 'Speed' })}</span><b>?</b></Reveal></div></section></div><section className={`question frame-question ${audio.beat >= 2 ? 'ready' : ''}`}><Options values={c.options} picked={picked} onPick={pick} /><Feedback show={picked !== null} correct>{t(c.feedback)}</Feedback></section></div></Stage>;
}
...
.hook-scene-shell{min-width:0;max-width:100%;overflow:hidden;border-radius:24px}[data-g4-role="visual-frame"]{position:relative;isolation:isolate;max-width:100%;overflow:hidden;color:#EAF9FB;background:radial-gradient(circle at 87% 24%,rgba(121,211,218,.16),transparent 24%),radial-gradient(circle at 9% 88%,rgba(149,201,61,.11),transparent 25%),linear-gradient(145deg,rgba(22,143,163,.25),transparent 48%),linear-gradient(135deg,#153B50,#0B2232 72%);box-shadow:0 22px 50px -30px rgba(14,33,44,.75)}[data-g4-role="visual-frame"] :is(img,svg,canvas,video){display:block;max-width:100%;max-height:100%}[data-g4-role="visual-frame"] :is(img,video){width:100%;height:100%;object-fit:contain}.hook-frame-bit{position:absolute;right:8px;bottom:-6px;z-index:4;width:76px;height:96px;display:block;overflow:hidden}.hook-frame-bit>.g1-char,.hook-frame-bit>svg{width:100%;height:100%}.feedback-bit{width:46px;height:58px;display:block;align-self:end;overflow:hidden}.feedback-bit>.g1-char,.feedback-bit>svg{width:100%;height:100%}
...
@media(max-width:639.98px){.hook-scene-shell{border-radius:18px}.hook-frame-bit{right:4px;bottom:-5px;width:62px;height:78px}[data-g4-role="hook-title"]{font-size:25px}}

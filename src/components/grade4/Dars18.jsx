...
const LESSON_META = {
  ...
  frameCounts: FRAME_COUNTS, screens: D18_SCREENS,
};
...
function NumberLine({ denominator, marker, markers = [], frame = 99, interactiveMarker = null, answerState = null }) {
  ...
  return <svg className={`number-line-svg ${answerState ? 'number-line-choice' : ''}`} viewBox="0 0 640 110" role="img"><line x1="35" y1="55" x2="605" y2="55" className="axis"/>{Array.from({ length: denominator + 1 }, (_, index) => { const x = 35 + 570 * index / denominator; return <g key={index}><line x1={x} y1="44" x2={x} y2="66" className="tick"/>{(index === 0 || index === denominator) && <text x={x} y="90" textAnchor="middle">{index === 0 ? '0' : '1'}</text>}</g>; })}{allMarkers.map((item, index) => { const x = 35 + 570 * item.at / denominator; const revealAt = item.revealAt ?? (markers.length ? 0 : 2); const picked = answerState?.picked === index; const right = answerState?.solved && answerState.correctIndex === index; const bad = picked && !right; const activate = () => answerState?.onPick(index); return <g key={`${item.at}-${item.label}`} className={`${frame >= revealAt ? 'marker on' : 'marker'} ${picked ? 'answer-picked' : ''} ${right ? 'answer-right' : ''} ${bad ? 'answer-bad' : ''}`} role={answerState ? 'button' : undefined} tabIndex={answerState ? 0 : undefined} aria-label={answerState ? String(item.label) : undefined} onClick={answerState ? activate : undefined} onKeyDown={answerState ? (event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); activate(); } } : undefined}><circle cx={x} cy="55" r={answerState ? 22 : 13}/><text x={x} y="25" textAnchor="middle">{item.label}</text></g>; })}{interactiveMarker !== null && <g className="marker free-marker on"><circle cx={35 + 570 * interactiveMarker / denominator} cy="55" r="10"/><text x={35 + 570 * interactiveMarker / denominator} y="105" textAnchor="middle">{interactiveMarker}/{denominator}</text></g>}</svg>;
}
...
function VisualPanel({ visual, frame, solved = false, promptReady = false, audio, hintLevel = 0, answerState = null }) {
...
function LessonScreen({ screen, storedAnswer, answers, onAnswer, onPrev, onNext, finishLesson }) {
  ...
  const choose = (index) => { if (solved) return; attempts.current += 1; const ok = index === c.correctIndex; if (!ok) clean.current = false; setPicked(index); setSolved(ok); setHintLevel(ok ? 0 : attempts.current >= 2 ? 2 : 0); playSfx(ok ? 'correct' : 'wrong'); const feedbackAudio = ok ? c.audio.on_correct : (c.feedbackAudio?.[index] ?? c.audio.on_wrong); const spoken = feedbackAudio?.[lang] ?? feedbackAudio?.uz ?? (ok ? "To'g'ri." : 'Yana tekshiring.'); audio.pushOneOff(spoken); onAnswer({ screenIdx: screen, stage: screen === 14 ? 'final' : 'module-mikro', question: t(c.question), options: c.options.map(t), correctIndex: c.correctIndex, correctAnswer: t(c.options[c.correctIndex]), studentAnswerIndex: index, studentAnswer: t(c.options[index]), correct: ok, firstTry: ok && clean.current && attempts.current === 1, attempts: attempts.current, solved: ok }); };
  const chooseNeutral = (index) => { setNeutralPicked(index); const spoken = c.neutralAudio?.[lang] ?? c.neutralAudio?.uz ?? c.neutral?.[lang] ?? c.neutral?.uz; if (spoken) audio.pushOneOff(spoken); };
  ...
  const answerState = directVisualChoice ? { picked, solved, correctIndex: c.correctIndex, onPick: choose, options: c.options } : null;

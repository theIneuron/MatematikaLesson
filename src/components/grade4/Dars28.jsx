...
function HookScreen({ screen, storedAnswer, onAnswer, onPrev, onNext }) {
  const t = useT(); const c = CONTENT.s0; const audio = useNarration(c.audio, screen);
  const [picked, setPicked] = useState(storedAnswer?.studentAnswerIndex ?? null); const [attempts, setAttempts] = useState(storedAnswer?.attempts ?? 0); const [wrongChoices, setWrongChoices] = useState(storedAnswer?.wrongChoices ?? []);
  const answerReady = isAudioReady(audio); const solved = picked === c.correctIndex;
  const order = buildOptionOrder(c.options.length, c.correctIndex, LESSON_META.lessonId, 0);
  const choose = (index) => { if (!answerReady || solved || wrongChoices.includes(index)) return; const ok = index === c.correctIndex; const nextAttempts = attempts + 1; const nextWrongChoices = ok ? wrongChoices : [...wrongChoices, index]; setPicked(index); setAttempts(nextAttempts); setWrongChoices(nextWrongChoices); audio.pushOneOff(t(ok ? c.neutral : HOOK_RETRY)); onAnswer({ stage: SCREEN_META[screen].scope, screenIdx: screen, question: t(c.question), options: c.options.map((option) => t(option)), correctIndex: c.correctIndex, correctAnswer: t(c.options[c.correctIndex]), studentAnswerIndex: index, studentAnswer: t(c.options[index]), correct: ok, firstTry: storedAnswer?.firstTry === false ? false : nextAttempts === 1 && ok, attempts: nextAttempts, wrongChoices: nextWrongChoices }); };
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext} canAdvance={solved}><div className="stack hook-stack" data-g4-screen="hook"><Heading c={c} state={solved ? 'nod' : picked !== null ? 'awkward' : 'think'} showBit hook/><h2 className="hook-question-prompt" data-g4-role="hook-question">{t(c.question)}</h2><section className="model-card hook-card" data-g4-role="hook-scene"><div className="hook-scene-visual" data-g4-role="visual-frame"><TimeVisual scene={c.scene} frame={audio.frame} screen={screen}/><div className="hook-frame-bit" data-g4-role="hook-bit"><BitSVG state="think"/></div></div></section><RevealFrames frames={c.frames} frame={audio.frame}/><section className="question hook-question" data-g4-role="answer-card" aria-live="polite"><h2>{t(c.question)}</h2><div className="options">{order.map((sourceIndex, displayIndex) => { const option = c.options[sourceIndex]; return <button type="button" data-g4-role="answer-card" data-g4-source-index={sourceIndex} data-g4-correct={sourceIndex === c.correctIndex ? 'true' : 'false'} className={'option ' + (solved && sourceIndex === c.correctIndex ? 'right' : wrongChoices.includes(sourceIndex) ? 'bad' : picked === sourceIndex ? 'picked' : '')} disabled={!answerReady || solved || wrongChoices.includes(sourceIndex)} onClick={() => choose(sourceIndex)} key={sourceIndex}><b>{String.fromCharCode(65 + displayIndex)}</b><span>{t(option)}</span></button>; })}</div><div className="feedback-slot hook-feedback-slot">{picked !== null && <div className={'feedback open ' + (solved ? 'correct' : 'wrong')} data-g4-role={solved ? 'feedback-frame bit-answer-comment' : 'feedback-frame'} data-g4-feedback={solved ? 'solution' : 'wrong'}><span className="feedback-bit" data-g4-role="feedback-bit"><BitSVG state={solved ? 'nod' : 'awkward'}/></span><p data-g4-role={solved ? 'bit-answer-comment' : undefined}>{t(solved ? c.neutral : HOOK_RETRY)}{solved && <span><b className="proof-label">{t(SOLUTION_LABEL)}</b>{t(c.options[c.correctIndex])}</span>}</p></div>}</div></section></div></Stage>;
}
...
function QuestionScreen({ screen, storedAnswer, onAnswer, onPrev, onNext }) {
  ...
  const [picked, setPicked] = useState(storedAnswer?.studentAnswerIndex ?? null); const [attempts, setAttempts] = useState(storedAnswer?.attempts ?? 0); const [wrongChoices, setWrongChoices] = useState(storedAnswer?.wrongChoices ?? []);
  const openChoice = SCREEN_META[screen].type === 'strategy'; const revealed = picked !== null; const correct = openChoice ? revealed : picked === c.correctIndex; const canAnswer = isAudioReady(audio);
  const choiceOrdinal = [8, 9, 10, 12, 13].indexOf(screen);
  const order = openChoice ? c.options.map((_, index) => index) : buildOptionOrder(c.options.length, c.correctIndex, LESSON_META.lessonId, choiceOrdinal + 1);
  const baseBitState = screen === 12 ? 'awkward' : screen === 13 ? 'point' : 'focus'; const bitState = revealed ? (correct ? 'nod' : 'awkward') : baseBitState;
  const choose = (index) => { if (!canAnswer || correct || wrongChoices.includes(index)) return; const ok = openChoice || index === c.correctIndex; const nextAttempts = attempts + 1; const nextWrongChoices = ok ? wrongChoices : [...wrongChoices, index]; setPicked(index); setAttempts(nextAttempts); setWrongChoices(nextWrongChoices); playSfx(ok ? 'correct' : 'wrong'); audio.pushOneOff(t(ok ? c.audio.on_correct : c.feedbackAudio[index])); onAnswer({ stage: SCREEN_META[screen].scope, screenIdx: screen, question: t(c.question), options: c.options.map((option) => t(option)), correctIndex: c.correctIndex, correctAnswer: t(c.options[c.correctIndex]), studentAnswerIndex: index, studentAnswer: t(c.options[index]), correct: ok, firstTry: storedAnswer?.firstTry === false ? false : nextAttempts === 1 && ok, attempts: nextAttempts, wrongChoices: nextWrongChoices }); };
  const showProof = !openChoice && (correct || (!correct && wrongChoices.length >= 2));
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext} canAdvance={correct}><div className="stack question-stack"><Heading c={c} state={bitState} showBit/><section className="test-layout"><div className="test-model"><TimeVisual scene={c.scene} frame={audio.frame} screen={screen} solved={revealed}/><RevealFrames frames={c.frames} frame={audio.frame}/></div><div className="question" aria-live="polite"><h2>{t(c.question)}</h2><div className="options">{order.map((sourceIndex, displayIndex) => { const option = c.options[sourceIndex]; const cls = openChoice && picked === sourceIndex ? 'picked' : sourceIndex === c.correctIndex && correct ? 'right' : wrongChoices.includes(sourceIndex) ? 'bad' : ''; return <button type="button" data-g4-branch={openChoice ? undefined : 'choice'} data-g4-source-index={openChoice ? undefined : sourceIndex} data-g4-correct={openChoice ? undefined : sourceIndex === c.correctIndex ? 'true' : 'false'} className={'option ' + cls} disabled={!canAnswer || correct || wrongChoices.includes(sourceIndex)} onClick={() => choose(sourceIndex)} key={sourceIndex}><b>{String.fromCharCode(65 + displayIndex)}</b><span>{t(option)}</span></button>; })}</div><div className="feedback-slot question-feedback-slot">{revealed && <div className="feedback-stack"><div className={'feedback open ' + (correct ? 'correct' : 'wrong')} data-g4-role={correct ? 'feedback-frame bit-answer-comment' : 'feedback-frame'} data-g4-feedback={correct ? 'solution' : 'wrong'}><span className="feedback-bit" data-g4-role="feedback-bit"><BitSVG state={correct ? "nod" : "awkward"}/></span><p data-g4-role={correct ? 'bit-answer-comment' : undefined}>{correct && <b className="proof-label">{t(SOLUTION_LABEL)}</b>}{t(correct ? c.audio.on_correct : c.audio.on_wrong[picked])}</p></div>{showProof && <div className="proof"><b className="proof-label">{t(SOLUTION_LABEL)}</b><span>{t(c.proof)}</span></div>}</div>}</div></div></section></div></Stage>;
}

...
function Screen0({ screen, storedAnswer, onAnswer, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext} nextDisabled={picked === null || !narrationReady}><div className="stack"><Heading c={c} bit="think"/><FrameNotes items={c.frames} frame={audio.frame}/><section className="hook-model"><DecimalStrip filled={3}/><div className="model-label"><Frac n="3" d="10" size="lg"/></div></section><section className="question" data-g4-role="answer-card"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral disabled={!narrationReady}/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

...
function Screen1({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="model-card"><DecimalStrip filled={frame >= 1 ? 3 : 0}/><div className={'representation-row ' + (frame >= 2 ? 'show' : '')}><span><Frac n="3" d="10" size="lg"/></span><b>↔</b><span className={frame >= 3 ? 'decimal-ready' : ''}>{frame >= 3 ? t(bi('0,3', '0,3', '0.3')) : t(bi('0,?', '0,?', '0.?'))}</span></div></section></div></Stage>;
}

function Screen2({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s2; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  const values = [frame >= 2 ? '0' : '', frame >= 3 ? '3' : '', '', ''];
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="model-card"><div className="decimal-code">0<span>{t(bi(',', ',', '.'))}</span>3</div><PlaceTable values={values} active={frame >= 3 ? 1 : frame >= 2 ? 0 : -1}/></section></div></Stage>;
}

function Screen3({ screen, onNext, onPrev }) {
  const c = CONTENT.s3; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  const values = frame >= 3 ? ['0', '0', '7', ''] : ['', '', '', ''];
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="model-card decimal-model-split"><HundredGrid filled={frame >= 1 ? 7 : 0}/><PlaceTable values={values} active={frame >= 3 ? 2 : -1}/></section></div></Stage>;
}

function Screen4({ screen, onNext, onPrev }) {
  const c = CONTENT.s4; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  const values = frame >= 3 ? ['0', '0', '4', '6'] : ['', '', '', ''];
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="model-card decimal-model-split"><ThousandDeck filled={frame >= 1 ? 46 : 0}/><PlaceTable values={values} active={frame >= 3 ? 3 : -1}/></section></div></Stage>;
}
...
function Screen5({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="model-card"><div className="strip-stack"><DecimalStrip filled={10} compact/><DecimalStrip filled={10} compact/><DecimalStrip filled={frame >= 1 ? 3 : 0} compact/></div><div className={'representation-row ' + (frame >= 2 ? 'show' : '')}><span>2</span><b>|</b><span><Frac n="3" d="10"/></span><b>↔</b><span className={frame >= 3 ? 'decimal-ready' : ''}>{frame >= 3 ? t(bi('2,3', '2,3', '2.3')) : t(bi('2,?', '2,?', '2.?'))}</span></div></section></div></Stage>;
}

function Screen6({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s6; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  const values = [frame >= 1 ? '5' : '', frame >= 2 ? '3' : '', frame >= 2 ? '0' : '', frame >= 3 ? '8' : ''];
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="model-card"><div className="decimal-code">5<span>{t(bi(',', ',', '.'))}</span>308</div><PlaceTable values={values} active={frame === 1 ? 0 : frame === 2 ? 2 : frame >= 3 ? 3 : -1}/></section></div></Stage>;
}

function Screen7({ screen, onNext, onPrev }) {
  const c = CONTENT.s7; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="rule-card"><MappingCards frame={audio.frame}/></section></div></Stage>;
}

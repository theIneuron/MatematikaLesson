...
function ConversionVisual({ c, frame, revealed = false }) { const visual = <ConversionVisualContent c={c} frame={frame} revealed={revealed}/>; const sceneName = String(c?.scene ?? ''); return /(^|-)hook($|-)/.test(sceneName) ? visual : <div className="canonical-visual-frame" data-g4-role="visual-frame" style={{ position: 'relative', isolation: 'isolate', minWidth: 0, maxWidth: '100%', height: '100%', overflow: 'hidden' }}>{visual}</div>; }

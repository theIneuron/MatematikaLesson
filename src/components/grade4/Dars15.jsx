...
function Screen0({ screen, storedAnswer, onAnswer, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext} activityDone={picked !== null}><div className="stack"><Heading c={c} bit="think"/><section className="duel"><ResultBars values={[9,14,9,15,13]} name="Anvar"/><ResultBars values={[10,15,11,10,9]} name="Ulug'bek"/><div className="best"><span>15 m</span><b>=</b><span>15 m</span></div></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

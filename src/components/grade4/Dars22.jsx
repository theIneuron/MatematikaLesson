...
const FeedbackBlock = ({ show, correct, children, proof = null }) => {
  ...
  return <div data-g4-feedback={show ? (correct ? 'solution' : 'wrong') : 'reserved'} role={show ? 'status' : undefined} aria-hidden={!show} className={`feedback feedback-slot ${correct ? 'correct' : 'wrong'} ${open ? 'open' : ''}`}><span className="feedback-bit"><BitSVG state={correct ? 'nod' : 'awkward'}/></span><p><span>{show ? children : ''}</span>{show && proof && <strong className="feedback-proof"><small>{t({ uz: 'YECHIM', ru: 'РЕШЕНИЕ', en: 'SOLUTION' })}</small>{proof}</strong>}</p></div>;
};
...
const Options = ({ values, picked, onPick, correctIndex, solved, neutral = false, disabled = false, answerCards = false, wrongChoices = [] }) => {
  const t = useT();
  return <div className="options">{values.map((value, index) => <button type="button" key={index + '-' + t(value)} data-g4-role={answerCards ? 'answer-card' : undefined} data-g4-wrong-choice={wrongChoices.includes(index) || undefined} className={'option ' + (picked === index ? 'picked ' : '') + (!neutral && solved && index === correctIndex ? 'right ' : '') + ((!neutral && picked === index && picked !== correctIndex) || wrongChoices.includes(index) ? 'bad' : '')} disabled={disabled || wrongChoices.includes(index) || (!neutral && solved)} onClick={() => onPick(index)}><b>{String.fromCharCode(65 + index)}</b><span>{t(value)}</span></button>)}</div>;
};
...
function Screen0({ screen, storedAnswer, onAnswer, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext} nextDisabled={!solved || !narrationReady}><div className="stack"><Heading c={c} bit="think"/><FrameNotes items={c.frames} frame={audio.frame}/><section className="hook-model"><GroupBoard total={24} groups={6} selected={audio.frame >= 2 ? 5 : 0}/></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral disabled={!narrationReady || solved} answerCards wrongChoices={wrongChoices}/><FeedbackBlock show={picked !== null} correct={solved} proof={solved ? '24 ÷ 6 × 5 = 20' : null}>{picked !== null ? t(solved ? correctFeedback : wrongFeedback) : ''}</FeedbackBlock></section></div></Stage>;
}

...
function Screen0({ screen, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><FrameNotes items={c.frames} frame={audio.frame}/><section className="hook-model"><GroupBoard total={24} groups={6} selected={audio.frame >= 2 ? 5 : 0}/></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

function Screen1({ screen, onNext, onPrev }) {
  const c = CONTENT.s1; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><FractionBar den={6} filled={audio.frame >= 2 ? 5 : 0} label={<Frac n="5" d="6" size="lg"/>}/></section></div></Stage>;
}
function Screen2({ screen, onNext, onPrev }) {
  const c = CONTENT.s2; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><GroupBoard total={24} groups={6} selected={audio.frame >= 2 ? 1 : 0}/></section></div></Stage>;
}
function Screen3({ screen, onNext, onPrev }) {
  const c = CONTENT.s3; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><GroupBoard total={24} groups={6} selected={audio.frame >= 1 ? 5 : 1}/><div className={'result-chip ' + (audio.frame >= 3 ? 'show' : '')}>20</div></section></div></Stage>;
}
function Screen4({ screen, onNext, onPrev }) {
  const c = CONTENT.s4; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="rule-card"><RuleFlow frame={audio.frame}/><div className={'formula-card ' + (audio.frame >= 3 ? 'show' : '')}>(N ÷ b) × a</div></section></div></Stage>;
}
function Screen5({ screen, onNext, onPrev }) {
  const c = CONTENT.s5; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><ThreeSpanLine frame={audio.frame}/></section></div></Stage>;
}
function Screen6({ screen, onNext, onPrev }) {
  const c = CONTENT.s6; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><GroupBoard total={18} groups={3} selected={2}/><div className={'state-note ' + (audio.frame >= 3 ? 'show' : '')}>12 + 6 = 18 ✓</div></section></div></Stage>;
}
function Screen7({ screen, onNext, onPrev }) {
  const c = CONTENT.s7; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="boundary-grid"><div className="show"><GroupBoard total={28} groups={4} selected={frame >= 1 ? 1 : 0} compact/></div><div className={frame >= 2 ? 'show' : ''}><GroupBoard total={28} groups={4} selected={frame >= 3 ? 4 : 0} compact/></div><div className={'state-note ' + (frame >= 4 ? 'show' : '')}>1/4 → 7 · 4/4 → 28</div></section></div></Stage>;
}
...
.group-board{display:grid;grid-template-columns:repeat(auto-fit,minmax(82px,1fr));gap:9px}.group-cell{min-height:94px;padding:8px;border-radius:15px;display:grid;align-content:space-between;gap:7px;background:#F4F5F1;box-shadow:inset 0 0 0 2px rgba(23,59,82,.12);transition:.4s ease}.group-cell>div{display:flex;flex-wrap:wrap;justify-content:center;gap:4px}.group-cell i{width:12px;height:12px;border-radius:4px;background:${T.cyan};box-shadow:0 3px 7px -5px rgba(23,59,82,.8)}.group-cell small{text-align:center;color:${T.ink2};font:900 10px 'JetBrains Mono',monospace}.group-cell.group-selected{background:${T.accentSoft};box-shadow:inset 0 0 0 3px rgba(255,91,53,.4)}.group-cell.group-selected i{background:${T.accent}}.group-board-compact{grid-template-columns:repeat(auto-fit,minmax(58px,1fr));gap:6px}.group-board-compact .group-cell{min-height:66px;padding:6px}.group-board-compact .group-cell i{width:9px;height:9px}

...
const CONTENT = {
  s0: {
    ...
    options: [
      ...
      { uz: "Hali aniq emas", ru: 'Пока не уверен(а)', en: 'I am not sure yet' },
    ],
...
const FeedbackBlock = ({ show, correct, children, proof = null }) => {
  ...
  if (!show) return null;
  return <div ref={ref} role="status" className={`feedback ${correct ? 'correct' : 'wrong'} ${open ? 'open' : ''}`}><span className="feedback-bit"><BitSVG state={correct ? 'happy' : 'idea'}/></span><p><span>{children}</span>{proof && <strong className="feedback-proof">{proof}</strong>}</p></div>;
};
...
function ChoiceExercise({ screen, storedAnswer, onAnswer, onNext, onPrev, visual = null, renderVisual = null, bit = null, modelChoices = false }) {
  ...
  const contextVisual = modelChoices ? <ModelChoices picked={picked} solved={solved} correctIndex={c.correctIndex} onPick={pick} options={c.options}/> : renderVisual ? renderVisual({ frame: Math.min(audio.frame, 1), solved }) : visual;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c} bit={bit}/>{contextVisual && <div className={'attempt-model ' + (hintLevel > 0 ? 'attempt-highlight' : '')}>{contextVisual}</div>}<section className={'question ' + (hintLevel > 0 ? 'attempt-highlight' : '')}><h2>{t(c.question)}</h2>{hintLevel > 0 && <div className="attempt-cue" role="status">{t({ uz: "Modelda teng bo'linmalarni ajrating: ayirishda maxraj ulush o'lchamini saqlaydi.", ru: 'Выделите равные деления модели: при вычитании знаменатель сохраняет размер доли.', en: 'Identify the equal divisions in the model: during subtraction, the denominator keeps the size of each part unchanged.' })}</div>}{!modelChoices && <Options values={c.options} picked={picked} onPick={pick} correctIndex={c.correctIndex} solved={solved}/>}<FeedbackBlock show={picked !== null} correct={solved} proof={solved && c.proof ? t(c.proof) : null}>{picked !== null ? t(c.feedback[picked]) : ''}</FeedbackBlock></section></div></Stage>;
}
...
function Screen0({ screen, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><section className="hook-model"><FractionTank den={10} filled={7} outlined={frame >= 1 ? 3 : 0} label={frame >= 2 ? <span><Frac n="7" d="10"/> - <Frac n="3" d="10"/> = <Frac n="4" d="7"/> ?</span> : null}/></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

function Screen1({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s1; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  const labels = [
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><FractionTank den={10} filled={7} outlined={frame >= 1 ? 3 : 0}/><div className="state-grid">{labels.map((item, index) => <span className={frame >= index ? 'show' : ''} key={t(item)}>{t(item)}</span>)}</div></section></div></Stage>;
}

function Screen2({ screen, onNext, onPrev }) {
  const c = CONTENT.s2; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><FractionTank den={10} filled={frame >= 2 ? 7 : 0} divisions={frame >= 1} label={frame >= 3 ? <Frac n="7" d="10" size="lg"/> : null}/></section></div></Stage>;
}

function Screen3({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s3; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  const [replay, setReplay] = useState(0); const visualFrame = replay > 0 ? 2 : frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><FractionTank key={replay} den={10} filled={visualFrame >= 2 ? 4 : 7} outlined={visualFrame === 1 ? 3 : 0} removed={visualFrame >= 2 ? 3 : 0}/><button type="button" className="tiny-action" onClick={() => setReplay((value) => value + 1)}>{t({ uz: "Olib tashlashni qayta ko'rsatish", ru: 'Повторить удаление', en: 'Show the removal again' })}</button><div className={'formula-card ' + (frame >= 3 ? 'show' : '')}><Frac n="7" d="10"/> - <Frac n="3" d="10"/></div></section></div></Stage>;
}
function Screen4({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s4; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c} bit="idea"/><section className="model-card"><FractionTank den={10} filled={4}/><div className={'formula-card ' + (frame >= 1 ? 'show' : '')}>7 - 3 = 4</div><div className={'state-note ' + (frame >= 2 ? 'show' : '')}>{t({ uz: "10 ta bo'linma saqlandi", ru: '10 делений сохранились', en: '10 divisions remain' })}</div><div className={'result-chip ' + (frame >= 3 ? 'show' : '')}><Frac n="4" d="10" size="lg"/></div></section></div></Stage>;
}

function Screen5({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s5; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="rule-card"><div className={'rule-line ' + (frame >= 0 ? 'show' : '')}>(7 - 3) / 10</div><div className={'rule-line accent ' + (frame >= 1 ? 'show' : '')}>4 / 10</div><div className={'state-note ' + (frame >= 2 ? 'show' : '')}>{t({ uz: "10 ta bo'linma o'zgarmadi", ru: '10 делений не изменились', en: 'The 10 divisions did not change' })}</div><div className={'wrong-formula ' + (frame >= 3 ? 'show' : '')}>4 / 7</div></section></div></Stage>;
}

function Screen6({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s6; const audio = useNarration(c.audio, screen); const [marker, setMarker] = useState(5);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card"><SubtractionLine den={10} start={7} amount={3} frame={audio.frame} marker={marker}/><label className="marker-control"><span>{t({ uz: "Erkin belgi", ru: 'Свободная метка', en: 'Free marker' })}: {marker}/10</span><input className="free-marker" type="range" min="0" max="10" step="1" value={marker} onChange={(event) => setMarker(Number(event.target.value))}/></label><div className={'formula-card ' + (audio.frame >= 2 ? 'show' : '')}><Frac n="7" d="10"/> - <Frac n="3" d="10"/> = <Frac n="4" d="10"/></div><div className={'state-note ' + (audio.frame >= 3 ? 'show' : '')}>{t({ uz: "Tekshiruv: 4/10 + 3/10 = 7/10", ru: 'Проверка: 4/10 + 3/10 = 7/10', en: 'Check: 4/10 + 3/10 = 7/10' })}</div></section></div></Stage>;
}
function Screen7({ screen, onNext, onPrev }) {
  const t = useT(); const c = CONTENT.s7; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="boundary-grid"><div className={frame >= 0 ? 'show' : ''}><FractionBar den={8} filled={frame >= 1 ? 5 : 8} removed={frame >= 1 ? 3 : 0} label={frame >= 1 ? <span><Frac n="8" d="8"/> - <Frac n="3" d="8"/> = <Frac n="5" d="8"/></span> : <span><Frac n="8" d="8"/> = 1</span>}/></div><div className={frame >= 2 ? 'show' : ''}><FractionBar den={5} filled={frame >= 3 ? 0 : 5} removed={frame >= 3 ? 5 : 0} label={frame >= 3 ? <span><Frac n="0" d="5"/> = 0</span> : <span><Frac n="5" d="5"/> - <Frac n="5" d="5"/></span>}/></div><div className={'state-note ' + (frame >= 4 ? 'show' : '')}>{t({ uz: "Suratlarni ayiring, maxrajni saqlang", ru: 'Вычти числители, сохрани знаменатель', en: 'Subtract the numerators and keep the denominator' })}</div></section></div></Stage>;
}
...
function G4FinalTitleReward({ finalFrameReached, completed = false, muted = false, title, firstTry, total }) {
  ...
  const ariaLabel = { uz: `Unvon: ${localizedTitle}`, ru: `Звание: ${localizedTitle}`, en: `Title: ${localizedTitle}` }[lang];
  return <>
...
const FinaleReward = ({ answers = [], complete, audio }) => {
  ...
  return <G4FinalTitleReward finalFrameReached={complete} completed={audio?.completed} muted={audio?.muted} title={{ uz: "Kasrlar ayirmasi ustasi", ru: 'Мастер разности дробей', en: 'Fraction Difference Master' }} firstTry={firstTry} total={scored.length} />;
};
...
function Screen14({ screen, answers, onPrev, finishLesson }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={finishLesson} finish><div className="stack"><section className="finale-heading"><span>◆ {t({ uz: "YAKUNIY BOSQICH", ru: 'ФИНАЛЬНЫЙ ЭТАП', en: 'FINAL STAGE' })}</span><h1>{t(c.title)}</h1><p>{t({ uz: "Bakdan olingan ulushlarni va qolgan miqdorni to'g'ri bog'ladik.", ru: 'Мы правильно связали убранные доли и оставшееся количество.', en: 'We correctly connected the parts removed from the tank with the amount left.' })}</p></section><section className="finale-main"><div className="finale-payoff"><small>{t({ uz: "BOSHLANG'ICH MISSIYA YECHIMI", ru: 'РЕШЕНИЕ СТАРТОВОЙ МИССИИ', en: 'STARTING MISSION SOLUTION' })}</small><FractionTank den={10} filled={frame >= 2 ? 4 : 7} outlined={frame === 1 ? 3 : 0} removed={frame >= 2 ? 3 : 0}/><div className={'finale-answer ' + (frame >= 2 ? 'show' : '')}><Frac n="7" d="10"/> - <Frac n="3" d="10"/> = <Frac n="4" d="10"/></div></div><div className="finale-takeaways">{takeaways.map((item, index) => <div className={'finale-takeaway ' + (frame >= index ? 'show' : '')} key={t(item.label)}><b>{index + 1}</b><span><small>{t(item.label)}</small><strong>{item.value}</strong></span></div>)}</div></section><section className="finale-bottom"><div className={'finale-bridge ' + (complete ? 'show' : '')}><small>{t({ uz: "KEYINGI MAVZU", ru: 'СЛЕДУЮЩАЯ ТЕМА', en: 'NEXT TOPIC' })}</small><strong>{t({ uz: "Sonning kasr qismini topish", ru: 'Нахождение дробной части числа', en: 'Finding a fraction of a number' })}</strong></div><FinaleReward answers={answers} complete={complete} audio={audio}/></section></div></Stage>;
}

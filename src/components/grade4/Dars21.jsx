...
function Screen0({ screen, storedAnswer, onAnswer, onNext }) {
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext} nextDisabled={picked === null || !narrationReady}><div className="stack"><Heading c={c} bit="think"/><section className="hook-model"><FractionTank den={10} filled={7} outlined={frame >= 1 ? 3 : 0} label={frame >= 2 ? <span><Frac n="7" d="10"/> - <Frac n="3" d="10"/> = <Frac n="4" d="7"/> ?</span> : null}/></section><section className="question" data-g4-role="answer-card"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral disabled={!narrationReady}/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}

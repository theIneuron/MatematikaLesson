...
const CONTENT = {
  s0: {
    eyebrow: bi("Lumo quvvat zaxirasi", "Запас энергии Лумо", "Lumo's energy reserve"),
    title: bi("18 ta blok jamining 3/5 qismi", "18 блоков составляют 3/5 запаса", '18 blocks are 3/5 of the reserve'),
    question: bi("Markazda jami nechta blok bor?", "Сколько блоков всего в центре?", 'How many blocks are there in the centre altogether?'),
    frames: [
      bi("Ma'lum qism: 18 ta blok", "Известная часть: 18 блоков", 'Known part: 18 blocks'),
      bi("Bu butunning 3/5 qismi", "Это 3/5 целого", 'This is 3/5 of the whole'),
      bi("Butun: ?", "Целое: ?", 'Whole: ?'),
    ],
    options: ["24", "30", "45"],
    neutral: bi("Taxmin saqlandi. Endi qismdan butunga yo'lni quramiz.", "Предположение сохранено. Теперь построим путь от части к целому.", 'Your prediction has been recorded. Now we will build the path from the part to the whole.'),
    audio: { intro: {
      uz: ["Markazning faol zaxirasida o'n sakkizta blok ko'rinmoqda.", "Bu bloklar jami zaxiraning beshdan uch qismini tashkil qiladi.", "Jami bloklar soni noma'lum. Taxminingizni belgilashingiz yoki kuzatishingiz mumkin."],
      ru: ["В активном запасе центра видно восемнадцать блоков.", "Они составляют три пятых всего запаса.", "Общее число блоков неизвестно. Можешь отметить предположение или просто наблюдать."],
      en: ['Eighteen blocks can be seen in the active reserve at the centre.', 'They make up three fifths of the entire reserve.', 'The total number of blocks is unknown. You may record a prediction or simply observe.'],
    } },
  },
  s1: {
  ...
  s4: {
    ...
    audio: {
      uz: ["Ma'lum o'n sakkizta blok uchta teng ulushni bildiradi.", "Bitta ulushni topish uchun o'n sakkizni suratdagi uchga bo'lamiz.", "Har bir ulush oltita blokka teng.", "Bu oltita blok butunning beshdan bir qismidir."],
      ru: ["Известные восемнадцать блоков обозначают три равные доли.", "Чтобы найти одну долю, делим восемнадцать на числитель три.", "Каждая доля равна шести блокам.", "Эти шесть блоков составляют одну пятую целого."],
      en: ['The known eighteen blocks represent three equal parts.', 'To find one part, divide eighteen by the numerator, three.', 'Each part equals six blocks.', 'Those six blocks make up one fifth of the whole.'],
    },
  },
  s5: {
    eyebrow: bi("Qismdan butunga", "От части к целому", 'From the part to the whole'),
    ...
    frames: [
      ...
      bi("Butun = 30 ta blok", "Целое = 30 блоков", 'Whole = 30 blocks'),
    ],
    audio: {
      uz: ["Bitta teng ulush oltita blokka teng.", "Maxraj besh, demak butun beshta shunday ulushdan tuzilgan.", "Oltini beshga ko'paytirib, o'ttizni topamiz.", "Shunday qilib, markazda jami o'ttizta blok bor."],
      ru: ["Одна равная доля равна шести блокам.", "Знаменатель пять, значит целое состоит из пяти таких долей.", "Умножаем шесть на пять и получаем тридцать.", "Значит, всего в центре тридцать блоков."],
      en: ['One equal part equals six blocks.', 'The denominator is five, so the whole consists of five such parts.', 'Multiply six by five to get thirty.', 'Therefore, there are thirty blocks in the centre altogether.'],
    },
  },
  s6: {
...
const Options = ({ values, picked, onPick, correctIndex, solved, neutral = false }) => {
  const t = useT();
  return <div className="options">{values.map((value, index) => <button type="button" key={index + '-' + t(value)} className={'option ' + (picked === index ? 'picked ' : '') + (!neutral && solved && index === correctIndex ? 'right ' : '') + (!neutral && picked === index && picked !== correctIndex ? 'bad' : '')} onClick={() => onPick(index)}><b>{String.fromCharCode(65 + index)}</b><span>{t(value)}</span></button>)}</div>;
};
...
function ChoiceExercise({ screen, storedAnswer, onAnswer, onNext, onPrev, visual = null, renderVisual = null, bit = null }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c} bit={bit}/><FrameNotes items={c.frames} frame={audio.frame}/>{contextVisual && <div className={'attempt-model ' + (hintLevel > 0 ? 'attempt-highlight' : '')}>{contextVisual}</div>}<section className={'question ' + (hintLevel > 0 ? 'attempt-highlight' : '')}><h2>{t(c.question)}</h2>{hintLevel > 0 && <div className="attempt-cue" role="status">{t(bi("Avval ma'lum va noma'lum miqdorni ajrating.", "Сначала различи известную и неизвестную величины.", 'First identify the known and unknown quantities.'))}</div>}<Options values={c.options} picked={picked} onPick={pick} correctIndex={c.correctIndex} solved={solved}/><FeedbackBlock show={picked !== null} correct={solved}>{picked !== null ? t(c.feedback[picked]) : ''}</FeedbackBlock>{solved && c.proof && <div className="proof">{t(c.proof)}</div>}</section></div></Stage>;
}
...
function Screen0({ screen, onNext }) {
  ...
  const pick = (index) => { setPicked(index); audio.pushOneOff(t(c.neutral)); };
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><FrameNotes items={c.frames} frame={audio.frame}/><section className="hook-model"><ShareSlots groups={5} known={3} per={6}/></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}
function Screen1({ screen, onNext, onPrev }) {
  const c = CONTENT.s1; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="boundary-grid"><div className={frame >= 0 ? 'show' : ''}><RuleFlow frame={frame}/></div><div className={frame >= 2 ? 'show' : ''}><RuleFlow frame={frame - 2} reverse/></div></section></div></Stage>;
}
function Screen2({ screen, onNext, onPrev }) {
  const c = CONTENT.s2; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><GroupBoard total={40} groups={5} selected={audio.frame >= 3 ? 1 : 0}/></section></div></Stage>;
}
function Screen3({ screen, onNext, onPrev }) {
  const c = CONTENT.s3; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><GroupBoard total={40} groups={5} selected={audio.frame >= 1 ? 3 : 0}/><div className={'result-chip ' + (audio.frame >= 2 ? 'show' : '')}>24</div></section></div></Stage>;
}
function Screen4({ screen, onNext, onPrev }) {
  const c = CONTENT.s4; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><ShareSlots groups={5} known={3} per={6}/></section></div></Stage>;
}
function Screen5({ screen, onNext, onPrev }) {
  const c = CONTENT.s5; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="model-card"><ShareSlots groups={5} known={3} per={6} revealAll={audio.frame >= 1}/><div className={'result-chip ' + (audio.frame >= 3 ? 'show' : '')}>30</div></section></div></Stage>;
}
function Screen6({ screen, onNext, onPrev }) {
  const c = CONTENT.s6; const audio = useNarration(c.audio, screen); const frame = audio.frame;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={frame}/><section className="boundary-grid"><div className="show"><RuleFlow frame={Math.min(frame, 1)}/></div><div className={frame >= 2 ? 'show' : ''}><RuleFlow frame={frame >= 2 ? frame - 2 : -1} reverse/></div></section></div></Stage>;
}
function Screen7({ screen, onNext, onPrev }) {
  const c = CONTENT.s7; const audio = useNarration(c.audio, screen);
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><FrameNotes items={c.frames} frame={audio.frame}/><section className="rule-card"><CycleFlow frame={audio.frame}/><div className={'state-note ' + (audio.frame >= 4 ? 'show' : '')}>49 → 28 → 49 ✓</div></section></div></Stage>;
}
...
const G4TitleReveal = ({ title }) => {
  const lang = useLang(); const t = useT(); const [visible, setVisible] = useState(true);
  useEffect(() => { const timer = window.setTimeout(() => setVisible(false), 3900); return () => window.clearTimeout(timer); }, []);
  if (!visible || typeof document === 'undefined') return null;
  const ariaLabel = { uz: `Unvon olindi: ${t(title)}`, ru: `Звание получено: ${t(title)}`, en: `Title earned: ${t(title)}` }[lang];
...
const G4TitleCard = ({ title, answers = [] }) => {
  const lang = useLang(); const t = useT(); const scored = SCREEN_META.map((item, index) => item.scored ? index : null).filter((index) => index !== null); const firstTry = scored.filter((index) => answers[index]?.firstTry === true).length;
  const labels = { uz: { earned: 'UNVON OLINDI', first: 'birinchi urinishda' }, ru: { earned: 'ЗВАНИЕ ПОЛУЧЕНО', first: 'с первой попытки' }, en: { earned: 'TITLE EARNED', first: 'on the first try' } }[lang];
  return <aside className="g4-title-card-stage" role="status" aria-live="polite" aria-atomic="true"><div className="g4-title-card-confetti" aria-hidden="true">{Array.from({ length: 12 }, (_, index) => <i key={index}/>)}</div><div className="g4-title-card-bit"><BitSVG state="happy"/></div><div className="g4-title-card-medal" aria-hidden="true">★</div><span className="g4-title-card-kicker">{labels.earned}</span><h2>{t(title)}</h2><div className="g4-title-card-score"><strong>{firstTry}/{scored.length}</strong><span>{labels.first}</span></div></aside>;
};
...
function Screen14({ screen, answers, onPrev, finishLesson }) {
  const t = useT(); const c = CONTENT.s14; const audio = useNarration(c.audio, screen); const reduced = usePrefersReducedMotion(); const frame = audio.frame; const complete = frame >= 4; const unlocked = complete || audio.completed || audio.muted || reduced;
  const takeaways = [
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={finishLesson} finish><div className="stack"><section className="finale-heading"><span>◆ {t(bi("YAKUNIY BOSQICH", "ФИНАЛЬНЫЙ ЭТАП", 'FINAL STAGE'))}</span><h1>{t(c.title)}</h1><p>{t(bi("Qismdan butunni tiklab, ikki yo'lni bog'ladik.", "Мы восстановили целое по части и связали два пути.", 'We reconstructed the whole from a part and connected the two paths.'))}</p></section><FrameNotes items={c.frames} frame={frame}/><section className="finale-main"><div className="finale-payoff"><small>{t(bi("BOSHLANG'ICH MISSIYA YECHIMI", "РЕШЕНИЕ СТАРТОВОЙ МИССИИ", 'STARTING MISSION SOLUTION'))}</small><ShareSlots groups={5} known={3} per={6} revealAll={frame >= 2}/><div className={'finale-answer ' + (frame >= 2 ? 'show' : '')}>18 ÷ 3 × 5 = 30</div></div><div className="finale-takeaways">{takeaways.map((item, index) => <div className={'finale-takeaway ' + (frame >= index ? 'show' : '')} key={t(item.label)}><b>{index + 1}</b><span><small>{t(item.label)}</small><strong>{item.value}</strong></span></div>)}</div></section><section className="finale-bottom"><div className={'finale-bridge ' + (complete ? 'show' : '')}><small>{t(bi("KEYINGI MAVZU", "СЛЕДУЮЩАЯ ТЕМА", 'NEXT TOPIC'))}</small><strong>{t(bi("O'ndan, yuzdan va mingdan ulushlar", "Десятые, сотые и тысячные доли", 'Tenths, hundredths and thousandths'))}</strong></div><G4TitleReward unlocked={unlocked} title={bi("Kasrli masalalar ustasi", "Мастер задач с дробями", 'Fraction Problem Master')} answers={answers}/></section></div></Stage>;
}

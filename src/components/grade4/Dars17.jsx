...
const FeedbackBlock = ({ show, correct, children, withBit = true, bitState = null }) => {
  const t = useT();
  ...
  return <div role="status" aria-hidden={!show} data-g4-role={correct ? 'feedback-frame bit-answer-comment' : 'feedback-frame'} data-g4-feedback={correct ? 'solution' : 'wrong'} className={`feedback ${correct ? 'correct' : 'wrong'} ${withBit ? 'with-bit' : ''} ${open ? 'open' : ''}`}>{withBit ? <span className="feedback-bit" data-g4-role="feedback-bit"><BitAnswerComment correct={correct} bitState={bitState}/></span> : <b>{correct ? '✓' : '↻'}</b>}<div className="feedback-content">{show && <><strong>{correct ? t({ uz: 'YECHIM', ru: 'РЕШЕНИЕ', en: 'SOLUTION' }) : t({ uz: "YANA O'YLANG", ru: 'ПРОВЕРЬТЕ СПОСОБ', en: 'CHECK THE METHOD' })}</strong>{children}</>}</div></div>;
};
...
const Stage = ({ screen, audio, onPrev, onNext, finish = false, activityDone, children }) => {
  ...
  const storedActivity = Object.prototype.hasOwnProperty.call(activityState, screen); const activityReady = !meta.active || activityDone === true || storedActivity; const audioReady = !audio || audio.muted || audio.visualOnly || audio.completed; const originalGatePassed = activityReady && audioReady; const canAdvance = canUseGrade4TheoryContinue(originalGatePassed, finish);
...
const Options = ({ values, picked, onPick, correctIndex, solved, neutral = false, order = null }) => { const t = useT(); const sourceOrder = order ?? values.map((_, index) => index); return <div className="options">{sourceOrder.map((sourceIndex, displayIndex) => { const value = values[sourceIndex]; return <button type="button" key={`${sourceIndex}-${t(value)}`} data-g4-role="answer-card" data-g4-source-index={order ? sourceIndex : undefined} data-g4-correct={order ? (sourceIndex === correctIndex ? 'true' : 'false') : undefined} className={`option ${picked === sourceIndex ? 'picked' : ''} ${!neutral && solved && sourceIndex === correctIndex ? 'right' : ''} ${!neutral && picked === sourceIndex && picked !== correctIndex ? 'bad' : ''}`} onClick={() => onPick(sourceIndex)}><b>{String.fromCharCode(65 + displayIndex)}</b><span>{t(value)}</span></button>; })}</div>; };
...
function ScaleChoicePanel({ picked, onPick, solved, correctIndex, order }) {
  const items = [{ intervals: 4 }, { intervals: 5 }, { intervals: 2 }];
  const sourceOrder = order ?? items.map((_, index) => index);
  return <section className="model-card scale-choice-grid scale-choice-list">{sourceOrder.map((sourceIndex, displayIndex) => { const item = items[sourceIndex]; return <button type="button" className={`scale-choice ${picked === sourceIndex ? 'picked' : ''} ${solved && sourceIndex === correctIndex ? 'right' : ''} ${picked === sourceIndex && picked !== correctIndex ? 'bad' : ''}`} key={sourceIndex} data-g4-source-index={order ? sourceIndex : undefined} data-g4-correct={order ? (sourceIndex === correctIndex ? 'true' : 'false') : undefined} aria-pressed={picked === sourceIndex} onClick={() => onPick(sourceIndex)}><b>{String.fromCharCode(65 + displayIndex)}</b><ScaleSVG min={0} max={100} intervals={item.intervals} frame={3} allLabels/></button>; })}</section>;
}
...
function ChoiceExercise({ screen, storedAnswer, onAnswer, onNext, onPrev, visual = null, bit = null, hideOptions = false, visualChoices = false }) {
  ...
  const ordinal = [8, 9, 10, 12, 13].indexOf(screen);
  const order = ordinal >= 0 ? buildOptionOrder(c.options.length, c.correctIndex, LESSON_META.lessonId, ordinal + 2) : null;
  ...
  const visualNode = visualChoices ? <ScaleChoicePanel picked={picked} onPick={pick} solved={solved} correctIndex={c.correctIndex} order={order}/> : typeof visual === 'function' ? visual({ frame: audio.frame, hintLevel, solved, t }) : visual;
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext} activityDone={solved}><div className="stack"><Heading c={c} bit={bit}/>{visualNode && <div className={`exercise-model-wrap ${hintLevel >= 2 && !solved ? 'second-hint' : ''}`}>{visualNode}</div>}<section className="question"><h2>{t(c.question)}</h2>{!hideOptions && <Options values={c.options} picked={picked} onPick={pick} correctIndex={c.correctIndex} solved={solved} order={order}/>}<FeedbackBlock show={picked !== null} correct={solved} withBit>{picked !== null && <><span>{t(c.feedback[picked])}</span>{solved && <strong className="feedback-proof">{t(c.proof)}</strong>}</>}</FeedbackBlock></section></div></Stage>;
}

// Grade 2 Dars07 approved HeroShip. Gradient and animation names are scoped to
// this lesson so multiple lesson previews cannot collide in the same document.
const HeroShip = () => (
  <g className="d17-hero-ship" aria-hidden="true">
    <defs>
      <linearGradient id="d17HsHull" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#8CA0B4"/><stop offset="20%" stopColor="#F0F5F9"/><stop offset="52%" stopColor="#BAC8D5"/><stop offset="100%" stopColor="#6C7A88"/></linearGradient>
      <radialGradient id="d17HsGlass" cx="38%" cy="30%" r="75%"><stop offset="0%" stopColor="#EAFCFF"/><stop offset="45%" stopColor="#5FC0DE"/><stop offset="100%" stopColor="#245C74"/></radialGradient>
      <linearGradient id="d17HsFin" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#B9C7D4"/><stop offset="100%" stopColor="#66727F"/></linearGradient>
      <linearGradient id="d17HsNoz" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#3A4652"/><stop offset="55%" stopColor="#8A97A4"/><stop offset="100%" stopColor="#4A5662"/></linearGradient>
      <linearGradient id="d17HsFlame" x1="0" y1="0" x2="1" y2="0"><stop offset="0%" stopColor="#FFF3C0"/><stop offset="45%" stopColor="#FF9A3C"/><stop offset="100%" stopColor="#fe5b1a" stopOpacity="0"/></linearGradient>
    </defs>
    <g className="d17-hero-flames">
      <path d="M126 -12 Q152 -8 176 -8 Q152 -1 126 -3 Z" fill="url(#d17HsFlame)"/>
      <path d="M126 12 Q152 8 176 8 Q152 1 126 3 Z" fill="url(#d17HsFlame)"/>
      <path d="M126 -10 Q144 -8 158 -8 Q144 -5 126 -6 Z" fill="#FFF6D2" opacity="0.85"/>
      <path d="M126 10 Q144 8 158 8 Q144 5 126 6 Z" fill="#FFF6D2" opacity="0.85"/>
    </g>
    <path d="M68 -15 L92 -29 L96 -12 Z" fill="url(#d17HsFin)" stroke="#5E7183" strokeWidth="1"/>
    <path d="M68 15 L92 29 L96 12 Z" fill="url(#d17HsFin)" stroke="#5E7183" strokeWidth="1"/>
    <path d="M8 -17 Q-14 0 8 17 L100 17 Q111 11 111 0 Q111 -11 100 -17 Z" fill="url(#d17HsHull)" stroke="#5E7183" strokeWidth="1.8"/>
    ...
    <path d="M112 -11 L126 -14 L126 -3 L112 -5 Z" fill="url(#d17HsNoz)" stroke="#3A4652" strokeWidth="0.8"/>
    <path d="M112 11 L126 14 L126 3 L112 5 Z" fill="url(#d17HsNoz)" stroke="#3A4652" strokeWidth="0.8"/>
    <ellipse cx="28" cy="0" rx="11" ry="11" fill="url(#d17HsGlass)" stroke="#5E7183" strokeWidth="1.5"/>
    <ellipse cx="24" cy="-4" rx="4" ry="2.5" fill="#EAFCFF" opacity="0.7"/>
    <rect x="54" y="-7" width="26" height="15" rx="3" fill="#0B1220"/>
    <circle className="d17-hero-neon" cx="63" cy="0.5" r="2.6" fill="#6EF29B"/><circle className="d17-hero-neon" style={{ animationDelay: '.5s' }} cx="71" cy="0.5" r="2.6" fill="#FFC23C"/>
  </g>
);
  ...
  return <div className="bit-flight-scale" role="img" aria-label={t({ uz: "Bit kemasi 0 dan 40 gacha bo'lgan shkalada 30 nuqtada turibdi", ru: 'Корабль Бита находится в точке 30 на шкале от 0 до 40', en: "Bit's ship is at point 30 on a scale from 0 to 40" })}>
    <svg viewBox="0 0 620 150" aria-hidden="true">
      ...
      <g className="flight-ship visible" transform="translate(441 68)">
        <g transform="translate(43 0) scale(-.55 .55)"><HeroShip/></g>
      </g>
    </svg>
  </div>;
};
...
function Screen0({ screen, onNext, hookAnswer, onHookAnswer }) {
  ...
  const order = buildOptionOrder(c.options.length, c.correctIndex, LESSON_META.lessonId, 0);
  ...
  return <Stage screen={screen} audio={audio} onNext={onNext} activityDone={correct}><div className="stack hook-stack" data-g4-screen="hook"><section className="hook-intro"><span className="hook-kicker" data-g4-role="hook-topic">{t(c.eyebrow)}</span><h1 data-g4-role="hook-title">{t(c.title)}</h1><p>{t(c.story)}</p></section><h2 className="hook-question-title" data-g4-role="hook-question">{t(c.question)}</h2><section className="hook-mission-frame" data-g4-role="hook-scene visual-frame"><div className="hook-bit" data-g4-role="hook-bit"><BitSVG state={picked === null ? 'think' : correct ? 'nod' : 'awkward'}/></div><BitFlightScale frame={audio.frame}/></section><section className="hook-question"><div className="hook-answer-frames"><Options values={c.options} picked={picked} onPick={pick} correctIndex={c.correctIndex} solved={correct} order={order}/></div><FeedbackBlock show={picked !== null} correct={correct} withBit bitState={correct ? 'nod' : 'awkward'}>{picked !== null && <><span>{t(c.feedback[picked])}</span>{correct && <strong className="feedback-proof">{t(c.proof)}</strong>}</>}</FeedbackBlock></section></div></Stage>;
}
...
function Screen4({ screen, onNext, onPrev }) {
  ...
  const order = buildOptionOrder(answers.length, 1, LESSON_META.lessonId, 0);
  ...
  ].map((item, index) => <div key={item.text} className={`${item.className} ${audio.frame >= index ? 'show' : ''} ${audio.frame === index ? 'narration-focus' : ''}`}>{item.text}</div>)}</div></section><section className="question compact-question"><h2>{t(c.prompt)}</h2><Options values={answers} picked={choice} onPick={pick} correctIndex={1} neutral order={order}/><FeedbackBlock show={choice !== null} correct={choice === 1} withBit>{choice === 1 ? t({ uz: "To'g'ri. Ikki qo'shni belgi orasidagi bo'linmalar sanaladi.", ru: 'Верно. Считают деления между соседними штрихами.', en: 'Correct. Count the divisions between neighbouring marks.' }) : t({ uz: "Belgilar soni bo'linmalar sonidan bittaga ko'p. Bo'linmalarni sanang.", ru: 'Штрихов на один больше, чем делений. Считайте деления.', en: 'There is one more mark than there are divisions. Count the divisions.' })}</FeedbackBlock></section></div></Stage>;
}
...
.stage-hook{background:radial-gradient(circle at 88% 8%,rgba(22,143,163,.14),transparent 27%),linear-gradient(145deg,#F2FAFA,#E5F5F6)}.stage-hook .stage-header{background:rgba(239,249,249,.9)}.stage-hook .stage-nav{background:rgba(229,245,246,.94)}.stage-hook .stage-content{overflow:hidden}.hook-stack{gap:11px}.hook-intro{width:100%;display:grid;justify-items:start;gap:4px;text-align:left}.hook-kicker{color:${T.cyan};font:900 11px/1.2 'JetBrains Mono',monospace;letter-spacing:.08em;text-transform:uppercase}.hook-intro h1{color:${T.ink};font:650 clamp(26px,4.2vw,36px)/1.08 'Source Serif 4',Georgia,serif;letter-spacing:-.012em}.hook-intro>p{width:min(760px,100%);color:${T.ink2};font-size:clamp(14px,1.8vw,16px);font-weight:700;line-height:1.48}.hook-mission-frame{position:relative;isolation:isolate;width:min(760px,100%);min-height:170px;margin:0 auto;padding:14px 20px;border:0;border-radius:24px;display:grid;grid-template-columns:116px minmax(0,1fr);align-items:center;gap:14px;overflow:hidden;color:#EAF9FB;background:radial-gradient(circle at 87% 24%,rgba(121,211,218,.16),transparent 24%),radial-gradient(circle at 9% 88%,rgba(149,201,61,.11),transparent 25%),linear-gradient(145deg,rgba(22,143,163,.25),transparent 48%),linear-gradient(135deg,#153B50,#0B2232 72%);box-shadow:0 22px 50px -30px rgba(14,33,44,.75)}.hook-mission-frame::after{content:'';position:absolute;inset:1px;z-index:-1;border:1px solid rgba(144,228,235,.12);border-radius:23px;pointer-events:none}.hook-bit{width:108px;height:136px;align-self:center;filter:drop-shadow(0 15px 18px rgba(0,0,0,.28))}.hook-bit .g1-char{width:100%;height:100%;overflow:visible}.bit-flight-scale{min-width:0;height:132px;border-radius:18px;background:rgba(111,213,222,.14)}.bit-flight-scale svg{display:block;width:100%;height:100%;overflow:visible}.flight-line{fill:none;stroke:#9DE3E7;stroke-width:4;stroke-linecap:round;stroke-dasharray:524;stroke-dashoffset:524;transition:stroke-dashoffset .8s ease}.flight-line.drawn{stroke-dashoffset:0}.flight-ticks g{opacity:0;transform:translateY(4px);transition:opacity .65s ease var(--delay),transform .65s ease var(--delay)}.flight-ticks.visible g{opacity:1;transform:none}.flight-ticks path{stroke:#D7F7F8;stroke-width:3;stroke-linecap:round}.flight-ticks text{fill:#EAFBFC;font:900 12px 'JetBrains Mono',monospace}.flight-ship{opacity:0;transform:translate(441px,68px) translateX(-34px);transition:opacity .75s ease,transform 1.05s cubic-bezier(.16,1,.3,1)}.flight-ship.visible{opacity:1;transform:translate(441px,68px)}.d17-hero-flames{transform-box:fill-box;transform-origin:left center;animation:d17-hero-flame .32s ease-in-out 6 alternate}.d17-hero-neon{animation:d17-hero-neon 2.2s ease-in-out 2 alternate}.hook-question{display:grid;gap:9px}.hook-question>h2{color:${T.navy};font:750 clamp(18px,2.4vw,22px)/1.2 'Source Serif 4',Georgia,serif}.hook-answer-frames .options{gap:11px}.hook-answer-frames .option{min-height:54px;border:1px solid rgba(23,59,82,.08);background:linear-gradient(145deg,#FFFFFF,#F7FBFA);box-shadow:0 13px 25px -21px rgba(${T.shadowBase},.62)}.hook-question .feedback{transition:opacity .68s ease,transform .68s cubic-bezier(.16,1,.3,1)}
...
@keyframes page-in{from{opacity:0;transform:translateY(10px)}}@keyframes proof-in{from{opacity:0;transform:translateY(7px)}}@keyframes pop{50%{transform:scale(1.08)}}@keyframes hint-pulse{to{transform:scale(1.012)}}@keyframes antenna{50%{transform:rotate(5deg)}}@keyframes think{50%{transform:rotate(-5deg) translateY(-2px)}}@keyframes happy{to{transform:translateY(-3px)}}@keyframes pulse{to{transform:scale(1.06)}}@keyframes d17-hero-flame{0%,100%{transform:scale(1)}50%{transform:scale(1.08)}}@keyframes d17-hero-neon{0%,100%{opacity:1}50%{opacity:.4}}@keyframes narration-spotlight{0%,100%{box-shadow:none}12%,72%{opacity:1;box-shadow:inset 0 0 0 3px rgba(255,91,53,.55),0 0 20px rgba(255,91,53,.28);transform:scale(1.025)}}@keyframes line-spotlight{12%,72%{stroke:${T.accent};filter:drop-shadow(0 0 7px rgba(255,91,53,.9))}}@keyframes tick-spotlight{12%,72%{stroke:${T.accent};filter:drop-shadow(0 0 6px rgba(255,91,53,.9))}}@keyframes division-spotlight{0%,100%{opacity:0}12%,72%{opacity:.3}}@keyframes pointer-spotlight{12%,72%{filter:drop-shadow(0 0 12px rgba(255,91,53,1))}}@keyframes finaleFall{to{transform:translateY(118px) rotate(180deg)}}
...
.lesson-root{font-family:'Manrope',system-ui,sans-serif}.lesson-root h1{font-family:'Source Serif 4',Georgia,serif}.lesson-root .question h2,.lesson-root .hook-question h2{font-family:'Manrope',system-ui,sans-serif}.screen-count,[class*="formula"],[class*="equation"],[class*="proof-label"]{font-family:'JetBrains Mono',monospace}.lead,.hook-intro>p{font-size:clamp(14px,1.8vw,16px)}
[data-g4-role="hook-title"],[data-g4-role="hook-question"]{width:100%;text-align:left}[data-g4-role="hook-title"]{font:650 clamp(26px,4.2vw,36px)/1.08 'Source Serif 4',Georgia,serif;letter-spacing:-.012em}[data-g4-role="hook-question"]{font:750 clamp(17px,2.5vw,21px)/1.3 'Manrope',system-ui,sans-serif}[data-g4-role="hook-scene"]{position:relative;isolation:isolate;width:min(760px, 100%);min-width:0;min-height:206px;margin-inline:auto;border-radius:24px;overflow:hidden;background:radial-gradient(circle at 87% 24%,rgba(121,211,218,.16),transparent 24%),radial-gradient(circle at 9% 88%,rgba(149,201,61,.11),transparent 25%),linear-gradient(145deg,rgba(22,143,163,.25),transparent 48%),linear-gradient(135deg,#153B50,#0B2232 72%);box-shadow:0 22px 50px -30px rgba(14,33,44,.75)}[data-g4-role="visual-frame"]{position:relative;isolation:isolate;min-width:0;max-width:100%;overflow:hidden}[data-g4-role="visual-frame"] :is(img,svg,canvas,video){display:block;max-width:100%;max-height:100%}[data-g4-role="visual-frame"] :is(img,video){width:100%;height:100%;object-fit:contain}[data-g4-role="hook-bit"]{position:absolute!important;right:42px!important;bottom:-4px!important;width:88px!important;height:110px!important;z-index:4}[data-g4-role="hook-bit"]>.g1-char,[data-g4-role="hook-bit"]>svg{width:100%!important;height:100%!important}
[data-g4-role~="feedback-frame"]{min-height:88px;padding:8px 15px 8px 9px;border-radius:18px;grid-template-columns:62px minmax(0,1fr)}[data-g4-role~="feedback-frame"] [data-g4-role="feedback-bit"]{width:62px;height:76px}[data-g4-feedback="solution"]{min-height:72px;padding:7px 12px 7px 6px;border-radius:15px;background:linear-gradient(135deg,#FFFFFF,#E7F3EC);box-shadow:inset 4px 0 #227A53}[data-g4-feedback="solution"] [data-g4-role="feedback-bit"]{width:51px;height:64px}[data-g4-feedback="wrong"]{background:linear-gradient(135deg,#FFFFFF,#FFF5D9);box-shadow:inset 4px 0 #A96F13}[data-g4-role~="bit-answer-comment"] .feedback-content{font:700 clamp(15px,2vw,18px)/1.35 'Source Serif 4',Georgia,serif}
@media(max-width:639.98px){[data-g4-role="hook-title"]{font-size:25px}[data-g4-role="hook-scene"]{min-height:164px;border-radius:18px}[data-g4-role="hook-bit"]{right:12px!important;bottom:-7px!important;width:68px!important;height:85px!important}[data-g4-role~="feedback-frame"] [data-g4-role="feedback-bit"]{width:54px;height:68px}[data-g4-feedback="solution"] [data-g4-role="feedback-bit"]{width:47px;height:59px}}

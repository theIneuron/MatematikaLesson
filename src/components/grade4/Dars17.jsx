...
const SCREEN_META = [
  ...
  { id: 's1', type: 'diagnostic', template: 'custom', scored: false, scope: null },
...
const CONTENT = {
  s0: {
    eyebrow: { uz: "Transport markazi", ru: 'Транспортный центр', en: 'Transport centre' },
    title: { uz: "Sirli shkala", ru: 'Загадочная шкала', en: 'The mysterious scale' },
    question: { uz: "Ko'rsatkich turgan qiymatni taxmin qiling.", ru: 'Предположите, на каком значении стоит указатель.', en: 'Estimate the value indicated by the pointer.' },
    options: ['30', '35', '40'],
    neutral: { uz: "Taxminingiz saqlandi. Endi shkalani qanday o'qishni aniqlaymiz.", ru: 'Ваше предположение сохранено. Теперь разберёмся, как читать шкалу.', en: 'Your estimate has been saved. Now we will work out how to read the scale.' },
    audio: { intro: {
      uz: ["Lumo City transport markazidagi sensor tekshiruvga tayyor.", "Shkalada yigirma va qirq sonlari ko'rsatilgan.", "Ko'rsatkich turgan qiymatni taxmin qila olasizmi?"],
      ru: ['Начинается проверка датчика транспортного центра Lumo City.', 'На шкале отмечены числа двадцать и сорок.', 'Можете предположить, на каком значении стоит указатель?'],
      en: ['The sensor at the Lumo City transport centre is ready for testing.', 'The numbers twenty and forty are marked on the scale.', 'Can you estimate the value indicated by the pointer?'],
    }, on_pick: { uz: "Taxminingiz saqlandi. Endi shkalani qanday o'qishni aniqlaymiz.", ru: 'Ваше предположение сохранено. Теперь разберёмся, как читать шкалу.', en: 'Your estimate has been saved. Now we will work out how to read the scale.' } },
  },
  s1: {
...
const FeedbackBlock = ({ show, correct, children, withBit = false }) => {
  ...
  if (!show) return null;
  return <div ref={ref} role="status" className={`feedback ${correct ? 'correct' : 'wrong'} ${withBit ? 'with-bit' : ''} ${open ? 'open' : ''}`}>{withBit ? <span className="feedback-bit"><BitSVG state={correct ? 'happy' : 'think'}/></span> : <b>{correct ? '✓' : '↻'}</b>}<div className="feedback-content">{children}</div></div>;
};
...
function ChoiceExercise({ screen, storedAnswer, onAnswer, onNext, onPrev, visual = null, bit = null, hideOptions = false, visualChoices = false }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c} bit={bit}/>{visualNode && <div className={`exercise-model-wrap ${hintLevel >= 2 && !solved ? 'second-hint' : ''}`}>{visualNode}</div>}<section className="question"><h2>{t(c.question)}</h2>{!hideOptions && <Options values={c.options} picked={picked} onPick={pick} correctIndex={c.correctIndex} solved={solved}/>}<FeedbackBlock show={picked !== null} correct={solved} withBit>{picked !== null && <><span>{t(c.feedback[picked])}</span>{solved && <strong className="feedback-proof">{t(c.proof)}</strong>}</>}</FeedbackBlock></section></div></Stage>;
}
...
function Screen0({ screen, onNext }) {
  ...
  const pick = (index) => { setPicked(index); audio.pushOneOff(t(c.audio.on_pick)); };
  return <Stage screen={screen} audio={audio} onNext={onNext}><div className="stack"><Heading c={c} bit="think"/><section className="model-card mission-panel"><span className="city-label">LUMO CITY · {t({ uz: "TRANSPORT MARKAZI", ru: 'ТРАНСПОРТНЫЙ ЦЕНТР', en: 'TRANSPORT CENTRE' })}</span><ScaleSVG min={20} max={40} intervals={4} pointer={35} frame={audio.frame} labels={[20, '', '', '', 40]}/></section><section className="question"><h2>{t(c.question)}</h2><Options values={c.options} picked={picked} onPick={pick} neutral/><FeedbackBlock show={picked !== null} correct withBit>{t(c.neutral)}</FeedbackBlock></section></div></Stage>;
}
...
function Screen2({ screen, onNext, onPrev }) {
  ...
  const [active, setActive] = useState(null); const highlightTimer = useRef(null);
  useEffect(() => () => { if (highlightTimer.current) window.clearTimeout(highlightTimer.current); }, []);
  const pickPart = (index) => {
    if (highlightTimer.current) window.clearTimeout(highlightTimer.current);
    setActive(index); audio.pushOneOff(t(c.tapFeedback[index]));
    highlightTimer.current = window.setTimeout(() => { setActive(null); highlightTimer.current = null; }, 3000);
  };
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card anatomy-card slow-sequence"><ScaleSVG min={20} max={40} intervals={4} pointer={35} frame={audio.frame} activePart={active} slow/><div className="anatomy-grid">{labels.map((label, index) => <button type="button" key={t(label)} aria-pressed={active === index} className={`${audio.frame >= index ? 'show' : ''} ${active === index ? 'active' : ''}`} onClick={() => pickPart(index)}><b>{index + 1}</b><span>{t(label)}</span></button>)}</div><FeedbackBlock show={active !== null} correct>{active !== null ? t(c.tapFeedback[active]) : ''}</FeedbackBlock></section></div></Stage>;
}
...
function Screen3({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c} bit="idea"/><section className="model-card discovery-card slow-sequence"><ScaleSVG min={100} max={200} intervals={4} frame={audio.frame} allLabels slow/><FormulaFlow slow frame={audio.frame} items={['100 → 200', '200 − 100 = 100', t({ uz: "4 ta bo'linma", ru: '4 деления', en: '4 divisions' }), t({ uz: "100 ÷ 4 = 25", ru: '100 ÷ 4 = 25' , en: "100 ÷ 4 = 25"})]}/><div className="division-taps" aria-label={t({ uz: "Bo'linmalar sonini belgilang", ru: 'Отметьте число делений', en: 'Mark the number of divisions' })}>{Array.from({ length: 4 }, (_, index) => <button type="button" key={index} aria-pressed={marked >= index + 1} className={marked >= index + 1 ? 'marked' : ''} onClick={() => markDivisions(index + 1)}><span>{index + 1}</span></button>)}</div><FeedbackBlock show={marked > 0} correct>{marked > 0 ? t(c.tapFeedback[marked - 1]) : ''}</FeedbackBlock></section></div></Stage>;
}
...
function Screen4({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c} bit="happy"/><section className="model-card trap-card"><ScaleSVG min={0} max={40} intervals={4} frame={3} allLabels activePart={scalePart} slow/><div className="count-contrast">{[
...
function Screen5({ screen, onNext, onPrev }) {
  ...
  return <Stage screen={screen} audio={audio} onPrev={onPrev} onNext={onNext}><div className="stack"><Heading c={c}/><section className="model-card reading-card slow-sequence"><ScaleSVG min={20} max={40} intervals={4} pointer={pointer} frame={audio.frame} slow/><FormulaFlow slow frame={audio.frame} items={[t({ uz: "1 bo'linma = 5", ru: 'Цена деления = 5', en: '1 division = 5' }), '20 → 25 → 30 → 35', '20 + 3 × 5', '35']}/><div className="free-slider"><label htmlFor="scale-pointer">{t({ uz: "Ko'rsatkichni erkin suring", ru: 'Свободно перемещайте указатель', en: 'Move the pointer freely' })}</label><input id="scale-pointer" type="range" min="20" max="40" step="5" value={pointer} onChange={(event) => setManualPointer(Number(event.target.value))}/><output htmlFor="scale-pointer" aria-live="polite">{t({ uz: `Tanlangan qiymat: ${pointer}.`, ru: `Выбрано значение: ${pointer}.`, en: `Selected value: ${pointer}.` })}</output></div></section></div></Stage>;
}
...
const FinaleReward = ({ answers = [], complete, audio }) => {
  ...
  return <G4FinalTitleReward finalFrameReached={complete} completed={audio?.completed} muted={audio?.muted} title={FINAL_AWARD} firstTry={firstTry} total={total} />;
};

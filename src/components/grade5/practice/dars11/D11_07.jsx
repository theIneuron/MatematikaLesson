// Dars11 · Amaliyot 07 — Tezlik (kasr) · 🔴 · Bekzod · tag: speed_fraction
// Darslik §31, Mashq 678: 1 daqiqadagi yo'lni kasr ko'rinishida. jsx-question kontrakti. Animatsiyali.
import React, { useState, useEffect, useRef, useCallback } from 'react';

const N = 2, D = 35; // 2 km / 35 daqiqa = 2/35
const DATA = { tag: 'speed_fraction', level: '🔴' };
const T = {
  uz: {
    eyebrow: 'Hayotda', title: 'Bir daqiqada',
    setup: "Halima 35 daqiqada 2 km yurdi. U 1 daqiqada qancha km yuradi? Javobni kasr ko'rinishida yozing.",
    numL: 'Surat', denL: 'Maxraj',
    correct: "To'g'ri. 2 km 35 daqiqaga: 2 : 35 = 2/35 km.",
    wrong: "Hali to'g'ri emas. Yana bir bor o'ylab ko'ring.",
  },
  ru: {
    eyebrow: 'В жизни', title: 'За одну минуту',
    setup: 'Халима прошла 2 км за 35 минут. Сколько км она проходит за 1 минуту? Запишите ответ дробью.',
  ...
  return (
    <div className="pq pq07">
      <style>{`
        ...
        .pq07 .pq-setup { font-size:16px; line-height:1.5; margin:6px 0 20px; color:#374151; }
        .pq07 .pq-frac { display:flex; flex-direction:column; align-items:center; gap:6px; width:110px; margin:0 auto; }
        .pq07 .pq-fin { width:96px; box-sizing:border-box; font-size:26px; font-weight:800; text-align:center; padding:10px; border-radius:12px; border:2px solid #d6dae3; background:#f8fafc; outline:none; font-variant-numeric:tabular-nums; }
        .pq07 .pq-fin:focus { border-color:#5b8def; background:#fff; }
        .pq07 .pq-fin:disabled { opacity:.85; }
        .pq07 .pq-fline { width:104px; height:3px; background:#1f2430; border-radius:2px; }
        .pq07 .pq-flbl { display:flex; justify-content:center; gap:44px; margin-top:8px; font-size:11px; color:#9aa1ad; font-weight:700; text-transform:uppercase; }
        .pq07 .pq-fb { display:flex; align-items:flex-start; gap:10px; margin-top:20px; padding:13px 15px; border-radius:14px; font-size:15px; line-height:1.45; font-weight:600; animation:pqIn .22s ease both; }
        ...
        .pq07 .pq-frac.on { animation:pqPop .5s cubic-bezier(.34,1.56,.64,1); }
      `}</style>

// Dars12 · Amaliyot 01 — Kimniki ko'p · 🟢 · Nodira/Daler · tag: compare_same_denom
// Bir xil maxrajli kasrlarni taqqoslash: maxraj bir xil bo'lsa, surat kattaroq bo'lgani katta.
...
// Kasr — bitta miqdor sifatida (ikkita son emas): surat / chiziq / maxraj
...
// Bar-model: n ta teng bo'lak, chapdan k tasi bo'yalgan.
...
const D12_01_DATA = { correct: 1, tag: 'compare_same_denom', level: '🟢' };
const D12_01_T = {
  uz: {
    eyebrow: 'Taqqoslash', title: "Kimniki ko'p",
    setup: "Ikki bir xil non. Har biri 8 ta teng bo'lakka bo'lingan. Nodira 3 bo'lak yedi, Daler 5 bo'lak.",
    ask: "Kim ko'proq non yedi?",
    opts: ['Nodira', 'Daler', 'Teng', "Aniqlab bo'lmaydi"],
    correct: "To'g'ri. Bo'laklar bir xil kattalikda, Dalerda ular ko'proq.",
    wrongMsg: "Maslahat: maxrajlar bir xil, demak bo'laklar ham bir xil. Kimda bo'lak ko'p?",
  },
  ru: {
    eyebrow: 'Сравнение', title: 'У кого больше',
    setup: 'Два одинаковых хлеба. Каждый разделён на 8 равных частей. Нодира съела 3 части, Далер — 5 частей.',
    ask: 'Кто съел больше хлеба?',
    opts: ['Нодира', 'Далер', 'Поровну', 'Определить нельзя'],
    correct: 'Верно. Части одинакового размера, у Далера их больше.',
    wrongMsg: 'Подсказка: знаменатели одинаковые, значит и части одинаковые. У кого частей больше?',
  },
};

export default function D12_01(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D12_01_T[lang] || D12_01_T.uz;
  ...
  const check = useCallback(() => {
    const correct = picked === D12_01_DATA.correct;
    ...
    onSubmit?.({
      ...
      correctAnswer: { idx: D12_01_DATA.correct, label: t.opts[D12_01_DATA.correct] },
      correct, meta: { tag: D12_01_DATA.tag, level: D12_01_DATA.level },
    });
  ...
  const optStyle = (i) => {
    ...
    if (show) { const ok = i === D12_01_DATA.correct; bg = ok ? '#e8f7ee' : '#fdecec'; bd = ok ? '#1a7f43' : '#c0392b'; col = ok ? '#1a7f43' : '#c0392b'; }
  ...
  return (
    <div className="pq">
      ...
      <div className="pq-row"><span className="pq-tag">Nodira</span><Bar n={8} k={3} /><Frac a={3} b={8} size={16} /></div>
      <div className="pq-row"><span className="pq-tag">Daler</span><Bar n={8} k={5} color="#7c3aed" /><Frac a={5} b={8} size={16} /></div>

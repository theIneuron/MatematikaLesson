// Dars18 · Amaliyot 02 — Ayirmani natijaga ula · 🟡 · tag: sub_match
// Moslash: 5 ta ayirma (maxraj 8) ↔ natijasi eng sodda holda (6/8=3/4, 2/8=1/4 ...).
...
// maxraj 8. Ayirma ↔ natija (eng sodda holda). 5 juft.
const D02_SUBS = { s1: ['7', '2'], s2: ['6', '2'], s3: ['5', '2'], s4: ['7', '1'], s5: ['6', '4'] };
const D02_PAIRS = { s1: '5/8', s2: '1/2', s3: '3/8', s4: '3/4', s5: '1/4' };
const D02_T = {
  uz: {
    eyebrow: 'Moslang', setup: "Malika ayirmalarni yechib, natijalarini alohida yozib qo'ygan edi, lekin qaysi qatorga tegishli ekani chalkashib ketdi. Har ayirmani natijasiga ulang.",
    ask: "Chapdagi ayirmani tanlang, keyin uning natijasini o'ngdan bosing:",
    correct: "To'g'ri. Suratlarni ayirib, kerak bo'lganda eng sodda holga keltirdik: 6/8 = 3/4, 2/8 = 1/4.",
    wrong: "Maslahat: har bir ayirmani o'zingiz hisoblang. Ba'zi natijalarni soddaroq ko'rinishda yozsa ham bo'ladi — mos kelganini qidiring.",
    rule: "Suratlarni ayiring, maxrajni o'zgartirma; natijani eng sodda holga keltiring.",
  },
  ru: {
    eyebrow: 'Соотнесите', setup: 'Малика решила разности и записала результаты отдельно, но перепутала, к какой строке они относятся. Соедини каждую разность с её результатом.',
    ask: 'Выберите разность слева, затем нажмите её результат справа:',
    correct: 'Верно. Вычли числители и, где нужно, привели к простейшему виду: 6/8 = 3/4, 2/8 = 1/4.',
    wrong: 'Подсказка: посчитай каждую разность сам. Некоторые результаты можно записать проще — ищи подходящий.',
    rule: 'Вычитай числители, знаменатель не меняй; результат приведи к простейшему виду.',
  },
};
...
export default function D18_02(props) {
  ...
  const right = ['1/2', '3/8', '5/8', '1/4', '3/4']; // aralash
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { map }, correctAnswer: D02_PAIRS, correct, meta: { tag: 'sub_match', level: '🟡' } });
  }, [map, t, playCorrect, playWrong, onSubmit]);
  useReg(check, registerCheck);
  const allOk = checked && fb?.correct; // qisman to'g'ri = hammasi qizil
  return (
    <div style={S.wrap}>
      <style>{`
      ...
      <div style={{ display: 'flex', gap: 14, justifyContent: 'center', margin: '10px 0' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {left.map((l) => {
            ...
            return <button key={l} type="button" disabled={locked} onClick={() => !done && setPickL(on ? null : l)} style={{ minWidth: 118, height: 58, borderRadius: 12, border: '2px solid ' + bd, background: bg, cursor: locked || done ? 'default' : 'pointer', boxShadow: on ? '0 0 0 4px #ffe7d8' : 'none' }}><span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}><FracStr s={a + '/8'} size={17} /><span style={{ ...S.mono, fontSize: 17, fontWeight: 800, color: '#64748b' }}>−</span><FracStr s={b + '/8'} size={17} />{done ? <span style={{ fontSize: 17, fontWeight: 800, color: '#94a3b8' }}>→</span> : null}</span></button>;
          })}
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {right.map((r) => {

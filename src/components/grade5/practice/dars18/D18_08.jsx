  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ display: 'block' }}>
      ...
      {Array.from({ length: shaded }).map((_, i) => (
        <rect key={'s' + i} x={i * cw + 1} y="1" width={cw - 2} height={h - 2} rx="3" fill={remove.has(i) ? '#fde0e0' : '#93c5fd'} stroke={remove.has(i) ? '#f2b8b8' : '#60a5fa'} strokeWidth="1" strokeDasharray={remove.has(i) ? '3 2' : undefined} />
      ))}
    </svg>
  );
}

    ...
    <div style={S.wrap}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center', margin: '12px 0' }}>
        {D10_ITEMS.map((f) => {
          ...
          return <button key={f} type="button" disabled={locked} onClick={() => toggle(f)} style={{ width: 92, height: 66, borderRadius: 14, border: '2px solid ' + bd, background: bg, cursor: locked ? 'default' : 'pointer', boxShadow: on ? '0 0 0 4px #ffe7d8' : 'none' }}><FracStr s={f} size={22} color={col} /></button>;
        })}
      </div>

// Dars06 · Amaliyot 04 — Manfiy sonlarni taqqoslash · 🟡 · Madina · tag: compare_neg
// 6-sinf «Butun sonlar»: son o'qida o'ngdagi son katta. jsx-question kontrakti. Maslahat yo'q.
import React, { useState, useEffect, useRef, useCallback } from 'react';

const LEFT = -5, RIGHT = -3;
const REL = LEFT > RIGHT ? '>' : (LEFT < RIGHT ? '<' : '=');
const DATA = { tag: 'compare_neg', level: '🟡' };
const T = {
  uz: {
    eyebrow: "Butun sonlar", title: 'Qaysi katta',
    setup: "Ikki manfiy sonni taqqoslang. Son o'qida o'ngdagi son kattaroq.",
    correct: "To'g'ri. -3 son o'qida -5 dan o'ngda, shuning uchun -5 < -3.",
    wrong: "Hali to'g'ri emas. Yana bir bor o'ylab ko'ring.",
    pick: 'Belgini tanlang',
  },
  ru: {
    eyebrow: 'Целые числа', title: 'Что больше',
    setup: 'Сравните два отрицательных числа. На прямой правее — больше.',
    correct: 'Верно. -3 правее -5 на прямой, поэтому -5 < -3.',
  ...
  return (
    <div className="pq pq04">
      <style>{`
        ...
        .pq04 .pq-nums { display:flex; justify-content:center; align-items:center; gap:24px; margin:6px 0 8px; }
        .pq04 .pq-n { font-size:38px; font-weight:800; color:#2563eb; font-variant-numeric:tabular-nums; }
        .pq04 .pq-q { font-size:32px; font-weight:800; color:#9aa1ad; min-width:30px; text-align:center; }
        .pq04 .pq-pick { text-align:center; font-size:13px; color:#9aa1ad; font-weight:600; margin-top:14px; }
        .pq04 .pq-rels { display:flex; justify-content:center; gap:14px; margin:8px 0 4px; }
        ...
        .pq04 .pq-n.L { animation:pqSlideL .5s cubic-bezier(.22,1,.36,1) both; }
        .pq04 .pq-n.R { animation:pqSlideR .5s cubic-bezier(.22,1,.36,1) both; }
        ...
        .pq04 .pq-q.on { animation:pqPop .4s cubic-bezier(.34,1.56,.64,1); }
      `}</style>

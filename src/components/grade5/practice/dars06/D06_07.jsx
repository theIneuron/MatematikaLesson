    ...
    <div style={S.wrap}>
      ...
      <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
        {D07_NUMS.map((n, idx) => {
          ...
          const on = pick === idx;
          return <button key={idx} type="button" disabled={locked} onClick={() => setPick(on ? null : idx)} style={{ width: 60, height: 54, borderRadius: 12, border: '2px solid ' + (on ? '#2563eb' : '#cbd5e1'), background: on ? '#eaf0fe' : '#fff', ...S.mono, fontSize: 21, fontWeight: 800, color: '#1f2430', cursor: locked ? 'default' : 'pointer', boxShadow: on ? '0 0 0 4px #dbeafe' : 'none' }}>{n}</button>;
        })}
      </div>

// Dars08 · Amaliyot 02 — Daraja shaklida · 🟢 · Madina · tag: write_as_power
// 6×6×6×6 → variant. To'g'ri javobdan keyin ko'paytmalar darajaga "siqiladi".
...
const D02_DATA = { correct: 2, base: 6, exp: 4 };
const D02_T = {
  uz: {
    eyebrow: 'Daraja', setup: "Madina 6 ni bir necha marta ko'paytirdi.",
    ask: '6 × 6 × 6 × 6 ni daraja shaklida qanday yoziladi?',
    opts: ['6 × 4', '4⁶', '6⁴', '6⁶'],
    correct: "To'g'ri. Oltini to'rt marta ko'paytirdik — 6⁴. Ko'rsatkich — nechta ko'paytuvchi borligini bildiradi.",
    wrong: "Maslahat: asos — takrorlanuvchi son, ko'rsatkich — nechta ko'paytuvchi borligini bildiradi. Ifodadagi ko'paytuvchilarni sanab ko'ring.",
    rule: "Daraja shaklida: asos — takrorlanuvchi son, ko'rsatkich — takrorlar soni.",
  },
  ru: {
    eyebrow: 'Степень', setup: 'Мадина умножила 6 несколько раз.',
    ask: 'Как записать 6 × 6 × 6 × 6 степенью?',
    opts: ['6 × 4', '4⁶', '6⁴', '6⁶'],
    correct: 'Верно. Умножили шесть четыре раза — 6⁴. Показатель — сколько множителей.',
    wrong: 'Подсказка: основание — повторяющееся число, показатель — сколько множителей. Сосчитайте множители в выражении.',
    rule: 'В степени: основание — повторяющееся число, показатель — число повторов.',
  },
};
export default function D08_02(props) {
  ...
  useEffect(() => { if (initialAnswer?.studentAnswer?.idx != null) { setPicked(initialAnswer.studentAnswer.idx); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); setShrink(!!initialAnswer.correct); } } }, [initialAnswer]);
  useEffect(() => { onReady?.(picked != null && !checked); }, [picked, checked, onReady]);
  const check = useCallback(() => {
    const correct = picked === D02_DATA.correct;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    if (correct) timer.current = setTimeout(() => setShrink(true), 400);
    onSubmit?.({ questionText: t.ask, options: t.opts.map((l, i) => ({ id: String(i), label: l })), studentAnswer: { idx: picked, label: t.opts[picked] }, correctAnswer: { idx: 2, label: '6⁴' }, correct, meta: { tag: 'write_as_power', level: '🟢' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      {/* ko'paytmalar darajaga siqiladi */}

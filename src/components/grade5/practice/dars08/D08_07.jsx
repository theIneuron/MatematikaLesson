        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', alignItems: 'center' }}>
          {[0, 1, 2].map((k) => (
            <React.Fragment key={k}>
              ...
              <span className={step >= k ? 'd8-pop' : undefined} style={{ ...S.mono, fontSize: 21, fontWeight: 800, color: step >= k ? (k === 2 ? '#1a7f43' : '#fe5b1a') : '#e5e7eb', opacity: step >= k ? 1 : 0.3, padding: '5px 10px', borderRadius: 10, background: step >= k ? (k === 2 ? '#e8f7ee' : '#fff4ee') : 'transparent' }}>{nums[k]}</span>
            </React.Fragment>
          ))}
        </div>
      </div>

...
export default function D08_04(props) {
  ...
  const slotColors = ['#fe5b1a', '#7c3aed', '#0f766e', '#c2410c'];
    ...
    <div style={S.wrap}>
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
        {D04_CARDS.map((n, idx) => {
          ...
          const on = pick === idx;
          return <button key={idx} type="button" disabled={locked} onClick={() => setPick(on ? null : idx)} style={{ width: 60, height: 48, borderRadius: 12, border: '2px solid ' + (on ? '#fe5b1a' : '#cbd5e1'), background: on ? '#fff0e8' : '#fff', ...S.mono, fontSize: 19, fontWeight: 800, color: '#1f2430', cursor: locked ? 'default' : 'pointer', boxShadow: on ? '0 0 0 4px #ffe7d8' : 'none' }}>{n}</button>;
        })}
      </div>

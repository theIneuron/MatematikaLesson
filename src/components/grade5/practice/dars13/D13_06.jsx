// Dars13 · Amaliyot 06 — O'sish tartibi · 🟡 · order_asc (sudrash + tap)
// Suratlar teng (uchtadan). 3/10, 3/8, 3/5, 3/4 ni kichikdan kattaga joylashtirish.
// Mexanika: kartani sudrash yoki tap -> bo'sh joyni tap.
...
const D13_06_DATA = { num: 3, bank: [4, 10, 5, 8], correct: [10, 8, 5, 4], tag: 'order_asc', level: '🟡' };
const D13_06_TINT = { 4: '#fae8ff', 5: '#fef9c3', 8: '#dcfce7', 10: '#e0f2fe' };
const D13_06_T = {
  uz: {
    eyebrow: 'Tartib', setup: "Suratlar bir xil — uchtadan. Kasrlarni kichikdan kattaga joylashtiring.",
    ask: "Kartani bo'sh joyga sudrang. Yoki kartani bosing, keyin bo'sh joyni bosing.",
    bank: 'Kartalar',
    correct: "To'g'ri. Suratlar teng bo'lganda tartibni maxraj belgilaydi: maxraj katta — kasr kichik.",
    wrongMsg: "Maslahat: suratlar teng, tartibni maxraj hal qiladi. Maxraj katta bo'lgan kasr kattaroqmi yoki kichikroq?",
  },
  ru: {
    eyebrow: 'Порядок', setup: 'Числители одинаковые — по три. Расставьте дроби от меньшей к большей.',
    ask: 'Перетащите карточку в пустое место. Или нажмите карточку, затем пустое место.',
    bank: 'Карточки',
    correct: 'Верно. При равных числителях порядок задаёт знаменатель: больше знаменатель — меньше дробь.',
    wrongMsg: 'Подсказка: числители равны, порядок задаёт знаменатель. Дробь с бо́льшим знаменателем — больше или меньше?',
  },
};

export default function D13_06(props) {
  const { lang = 'uz', mode = 'answer', initialAnswer = null, playCorrect, playWrong, onReady, registerCheck, onSubmit } = props || {};
  const t = D13_06_T[lang] || D13_06_T.uz;
  const isReview = mode === 'review';
  const [slots, setSlots] = useState([null, null, null, null]);
  const [bank, setBank] = useState(D13_06_DATA.bank);
  ...
  const orderCorrect = slots.join(',') === D13_06_DATA.correct.join(','); // qisman = hammasi qizil
  ...
  const place = useCallback((v, from, i) => {
    setSlots((prev) => {
      ...
      const occ = ns[i];
      ns[i] = v;
      if (typeof from === 'number') ns[from] = occ;
      else if (occ != null) setBank((b) => [...b, occ]);
      return ns;
    });
  ...
  const check = useCallback(() => {
    const correct = slots.join(',') === D13_06_DATA.correct.join(',');
    setFb({ correct }); setChecked(true);
    correct ? playCorrect?.() : playWrong?.();
    onSubmit?.({
      questionText: t.setup,
      options: D13_06_DATA.bank.map((d) => ({ id: String(d), label: '3/' + d })),
      studentAnswer: { order: slots, label: slots.map((d) => '3/' + d).join(' < ') },
      correctAnswer: { order: D13_06_DATA.correct, label: D13_06_DATA.correct.map((d) => '3/' + d).join(' < ') },
      correct, meta: { tag: D13_06_DATA.tag, level: D13_06_DATA.level },
    });
    ...
    <div className="pq">
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', alignItems: 'flex-end', margin: '18px 0 4px' }}>
        {slots.map((v, i) => {
          ...
          if (checked && v != null) { const ok = orderCorrect; bd = ok ? '#1a7f43' : '#c0392b'; bg = ok ? '#e8f7ee' : '#fdecec'; }
          return (

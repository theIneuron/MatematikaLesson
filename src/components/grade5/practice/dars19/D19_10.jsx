// Dars19 · Amaliyot 10 — Masala · 🔴 · tag: add_coprime_story
// 1/4 + 1/3: umumiy maxraj 12. 3/12 + 4/12 = 7/12. Ikki maydon: surat (7) va maxraj (12).
...
const D10_A = 7, D10_D = 12; // 3/12 + 4/12 = 7/12
const D10_T = {
  uz: {
    eyebrow: 'Kitobxon', setup: "Nilufar kitobning 1/4 qismini dushanba, 1/3 qismini seshanba kuni o'qidi.",
    ask: "Nilufar jami kitobning qancha qismini o'qidi? Suratni va maxrajni yozing.",
    l1: 'Surat:', l2: 'Maxraj:',
    correct: "To'g'ri. Umumiy maxraj 12: 1/4 = 3/12, 1/3 = 4/12 → 3/12 + 4/12 = 7/12.",
    wrong: "Maxrajlar 4 va 3 — har xil. Har ikkalasiga ham bo'linadigan qanday son bor? Suratlarni faqat shundan keyin qo'shing.",
    rule: "Umumiy maxrajni top (4 va 3 uchun 12), ekvivalentlarga keltiring, so'ng suratlarni qo'shing.",
  },
  ru: {
    eyebrow: 'Читатель', setup: 'Нилуфар прочитала 1/4 книги в понедельник и 1/3 во вторник.',
    ask: 'Какую часть книги прочитала Нилуфар всего? Впиши числитель и знаменатель.',
    l1: 'Числитель:', l2: 'Знаменатель:',
    correct: 'Верно. Общий знаменатель 12: 1/4 = 3/12, 1/3 = 4/12 → 3/12 + 4/12 = 7/12.',
    wrong: 'Знаменатели 4 и 3 — разные. Какое число делится на оба? Только после этого складывай числители.',
    rule: 'Найди общий знаменатель (для 4 и 3 это 12), приведи к нему, затем сложи числители.',
  },
};
...
export default function D19_10(props) {
  ...
  const check = useCallback(() => {
    ...
    onSubmit?.({ questionText: t.ask, options: [], studentAnswer: { a: parseInt(a, 10), d: parseInt(d, 10) }, correctAnswer: { a: D10_A, d: D10_D }, correct, meta: { tag: 'add_coprime_story', level: '🔴' } });
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, margin: '14px 0 8px' }}>
        <Frac num="1" den="4" size={32} color="#fe5b1a" />
        <span style={{ ...S.mono, fontSize: 24, fontWeight: 800, color: '#94a3b8' }}>+</span>
        <Frac num="1" den="3" size={32} color="#fe5b1a" />
        <span style={{ ...S.mono, fontSize: 24, fontWeight: 800, color: '#94a3b8' }}>=</span>
      ...
      {reveal && <div className="d19-pop" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, ...S.mono, fontSize: 15, fontWeight: 800, color: '#0f766e' }}><Frac num="3" den="12" size={17} color="#0f766e" /><span>+</span><Frac num="4" den="12" size={17} color="#0f766e" /><span>=</span><Frac num="7" den="12" size={17} color="#0f766e" /></div>}

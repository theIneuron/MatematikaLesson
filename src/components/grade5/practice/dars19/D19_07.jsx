// Dars19 · Amaliyot 07 — Masala + qisqartirish · 🔴 · tag: add_story_reduce
// Madina: 1/6 + 1/3 = 1/6 + 2/6 = 3/6, eng sodda holda 1/2. Ikki maydon.
...
function Bar({ parts, shaded, color, w = 210, h = 28 }) {
  const cw = w / parts;
  return (
...
const D07 = { sum: 3, a: 1, b: 2 }; // 3/6 = 1/2
const D07_T = {
  uz: {
    eyebrow: "Bo'yash", setup: "Madina xona devorining 1/6 qismini ertalab, 1/3 qismini tushdan keyin bo'yadi.",
    ask: "Madina jami qancha devor bo'yadi? Yig'indini, so'ng uni eng sodda holda yozing.",
    l1: "Yig'indi:", l2: 'Eng sodda:',
    correct: "To'g'ri. 1/3 = 2/6, so'ng 1/6 + 2/6 = 3/6. Eng sodda holda: 3/6 = 1/2.",
    wrong: "Yig'indini topgach, unga diqqat qiling: surat ham, maxraj ham bitta songa bo'linadimi?",
    rule: "Umumiy maxrajga keltiring, qo'shing, so'ng natijani eng sodda holga keltiring: 3/6 = 1/2.",
  },
  ru: {
    eyebrow: 'Покраска', setup: 'Мадина покрасила 1/6 стены утром и 1/3 после обеда.',
    ask: 'Сколько всего стены покрасила Мадина? Запиши сумму, затем её простейший вид.',
    l1: 'Сумма:', l2: 'Простейший:',
    correct: 'Верно. 1/3 = 2/6, затем 1/6 + 2/6 = 3/6. В простейшем виде: 3/6 = 1/2.',
    wrong: 'Получив сумму, присмотрись: делятся ли и числитель, и знаменатель на одно число?',
    rule: 'Приведи к общему знаменателю, сложи, затем упрости результат: 3/6 = 1/2.',
  },
};
...
export default function D19_07(props) {
  ...
  useEffect(() => { const s = initialAnswer?.studentAnswer; if (s) { if (s.sum != null) setSum(String(s.sum)); if (s.a != null) setA(String(s.a)); if (s.b != null) setB(String(s.b)); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); setReveal(!!initialAnswer.correct); } } }, [initialAnswer]);
  const full = /^\d+$/.test(sum) && /^\d+$/.test(a) && /^\d+$/.test(b);
  useEffect(() => { onReady?.(full && !checked); }, [full, checked, onReady]);
  const check = useCallback(() => {
    const correct = parseInt(sum, 10) === D07.sum && parseInt(a, 10) === D07.a && parseInt(b, 10) === D07.b;
      ...
      <div style={{ display: 'flex', gap: 22, justifyContent: 'center', alignItems: 'flex-start', flexWrap: 'wrap', marginTop: 4 }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            <input value={sum} onChange={(e) => setSum(e.target.value.replace(/[^\d]/g, '').slice(0, 1))} disabled={isReview || checked} inputMode="numeric" placeholder="?" style={cellStyle(bdOf(sum, D07.sum))} />

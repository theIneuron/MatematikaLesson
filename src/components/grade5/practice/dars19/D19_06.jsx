// Dars19 · Amaliyot 06 — Son o'qida qo'sh · 🟡 · tag: add_numberline
// 1/3 + 1/6 oltidan bo'lingan o'qda: 1/3 = 2/6, keyin +1/6 → 3/6. Tushish nuqtasini tanlaydi.
...
const D06_T = {
  uz: {
    eyebrow: "Son o'qida", setup: "O'q oltidan bo'lingan. Chumchuq 0 dan avval 1/3, keyin yana 1/6 masofaga sakraydi.",
    ask: "Chumchuq qayerga tushadi? Kerakli bo'linni bosing:",
    correct: "To'g'ri. 1/3 = 2/6, so'ng 2/6 + 1/6 = 3/6. O'qda 1/3 aynan 2 ta oltidan bo'lakka to'g'ri keladi.",
    wrong: "1/3 sakrash o'qda qayerga borib to'xtaydi? Undan yana bir bo'lak oldinga sakrang.",
    rule: "Qo'shishdan oldin bir xil ulushga keltiring: 1/3 = 2/6, keyin oltidan bo'laklarni qo'shing.",
  },
  ru: {
    eyebrow: 'На оси', setup: 'Ось поделена на шестые. Воробей от 0 сначала прыгает на 1/3, потом ещё на 1/6.',
    ask: 'Куда попадёт воробей? Нажми нужное деление:',
    correct: 'Верно. 1/3 = 2/6, затем 2/6 + 1/6 = 3/6. На оси 1/3 приходится ровно на 2 шестых.',
    wrong: 'Где остановится прыжок 1/3 на оси? Затем прыгни ещё на одну долю вперёд.',
    rule: 'Перед сложением приведи к одинаковым долям: 1/3 = 2/6, затем сложи шестые.',
  },
};
...
export default function D19_06(props) {
  ...
  const [hop, setHop] = useState(0); // 0 → 2 → 3
  const timers = useRef([]);
  useEffect(() => () => timers.current.forEach(clearTimeout), []);
  useEffect(() => { const sa = initialAnswer?.studentAnswer; if (sa?.sel != null) { setSel(sa.sel); if (typeof initialAnswer.correct === 'boolean') { setFb({ correct: initialAnswer.correct }); setChecked(true); if (initialAnswer.correct) setHop(3); } } }, [initialAnswer]);
  useEffect(() => { onReady?.(sel != null && !checked); }, [sel, checked, onReady]);
  const check = useCallback(() => {
    const correct = sel === D06_ANS;
    setFb({ correct }); setChecked(true); correct ? playCorrect?.() : playWrong?.();
    if (correct) { timers.current.push(setTimeout(() => setHop(2), 450)); timers.current.push(setTimeout(() => setHop(3), 1350)); }
  ...
  const W = 320, mL = 22, mR = 300, base = 66, step = (mR - mL) / 6;
  ...
  return (
    <div style={S.wrap}>
      <style>{`
        ...
        @media (prefers-reduced-motion: reduce) { .d19-pop { animation: none !important; } svg circle[style] { transition: none !important; } }
      `}</style>

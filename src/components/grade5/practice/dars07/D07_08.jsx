// Dars07 · Amaliyot 08 — Aziza xatosi (qarama-qarshi) · 🔴 · Sardor · tag: opposite_error
// 5-sinf nazariy Dars07 M1: "minusni olib tashlash" xatosi — musbat songa ham ishora almashtiriladi.
import React, { useState, useEffect, useRef, useCallback } from 'react';

const DATA = { correct: 0, tag: 'opposite_error', level: '🔴' };
const T = {
  uz: {
    eyebrow: 'Xato top', title: 'Aziza xatosi',
    setup: "Aziza qarama-qarshi sonni «minusni olib tashlash yoki qo'yish» deb topdi.",
    ask: 'Qaysi qatorda qarama-qarshi son XATO topilgan?',
    opts: ['4 → 4', '-7 → 7', '5 → -5', '0 → 0'],
    correct: "To'g'ri. 4 ga qarama-qarshi son -4, 4 emas. Musbat songa ham ishora almashtiriladi.",
    wrongMsg: "Hali to'g'ri emas. Yana bir bor o'ylab ko'ring.",
  },
  ru: {
    eyebrow: 'Найди ошибку', title: 'Ошибка Азизы',
    setup: 'Азиза искала противоположное число как «убрать или поставить минус».',
    ask: 'В какой строке противоположное число найдено НЕВЕРНО?',
    opts: ['4 → 4', '-7 → 7', '5 → -5', '0 → 0'],
    correct: 'Верно. Противоположное числу 4 — это -4, а не 4. У положительного числа знак тоже меняется.',
...
export default function D07_08(props) {
  ...
  const optStyle = (i) => {
    ...
    return { display: 'block', width: '100%', textAlign: 'left', padding: '13px 15px', borderRadius: 13, border: '2px solid ' + bd, background: bg, color: col, fontSize: 18, fontWeight: 700, cursor: (isReview || checked) ? 'default' : 'pointer', marginBottom: 9, fontFamily: 'inherit', fontVariantNumeric: 'tabular-nums', animation: anim, transition: 'background .3s, border-color .3s, color .3s' };
  };

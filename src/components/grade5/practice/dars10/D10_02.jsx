      ...
      <div style={{ display: 'flex', justifyContent: 'center', margin: '12px 0' }}>
        <div style={{ display: 'flex', width: 280, height: 44, border: '2.5px solid #38bdf8', borderRadius: 10, overflow: 'hidden' }}>
          {Array.from({ length: D02_DEN }).map((_, i) => {
            ...
            return <div key={i} style={{ flex: 1, borderRight: i < D02_DEN - 1 ? '2px solid ' + (split ? '#38bdf8' : 'transparent') : 'none', background: filled ? '#38bdf8' : '#fff4ee', transition: 'border-color .5s ease, background .6s ease' }} />;
          })}
        </div>
      </div>

// Dars03 · Amaliyot 01 — Ustunda qo'shish · 🟢 · Bekzod · tag: sum_input
// jsx-question kontrakti: onReady/registerCheck/onSubmit. O'z tugmasi yo'q. Faqat react importi.
import React, { useState, useEffect, useRef, useCallback } from 'react';

const TARGET = 28690;
const DATA = { tag: 'sum_input', level: '🟢' };
const T = {
  uz: {
    eyebrow: "Qo'shish", title: "Yig'indi",
  ...
  return (
    <div className="pq pq01">
      <style>{`
        ...
        .pq01 .pq-setup { font-size:16px; line-height:1.5; margin:6px 0 6px; color:#374151; }
        .pq01 .pq-words { font-size:22px; font-weight:800; color:#2563eb; margin:2px 0 18px; }
        .pq01 .pq-label { display:block; font-size:14px; font-weight:600; color:#374151; margin-bottom:6px; }
        .pq01 input.pq-input { width:100%; box-sizing:border-box; font-size:24px; font-weight:800; text-align:center; padding:13px 14px; border-radius:14px; border:2px solid #d6dae3; background:#f8fafc; outline:none; font-variant-numeric:tabular-nums; }
        .pq01 input.pq-input:focus { border-color:#5b8def; background:#fff; }
        .pq01 input.pq-input:disabled { opacity:.85; }
        .pq01 .pq-live { text-align:center; margin:12px 0 2px; }
        .pq01 .pq-live-lbl { font-size:13px; color:#9aa1ad; font-weight:600; }
        .pq01 .pq-live-num { font-size:26px; font-weight:800; font-variant-numeric:tabular-nums; letter-spacing:.02em; }
      ...
      <label className="pq-label" htmlFor="pq01-in">{t.label}</label>
      <input id="pq01-in" className="pq-input" value={val} onChange={(e) => setVal(cleanInt(e.target.value))} inputMode="numeric" pattern="[0-9]*" disabled={isReview || checked} placeholder="0" />

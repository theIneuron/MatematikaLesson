...
const DATA = { number: '5837', markIndex: 1, correct: 0, tag: 'digit_place', level: '🟢' };
const T = {
  uz: {
    eyebrow: 'Atrofimizda', title: 'Xona',
  ...
  ru: {
    eyebrow: 'Вокруг нас', title: 'Разряд',
...
export default function D01_02(props) {
  ...
  const digits = DATA.number.split('');
  return (
    <div className="pq pq02">
      <style>{`
        ...
        .pq02 .pq-num { text-align:center; margin:6px 0 18px; animation:pqReveal .6s cubic-bezier(.22,1,.36,1) both; animation-delay:.1s; }
        .pq02 .pq-d { font-size:40px; font-weight:800; letter-spacing:.06em; font-variant-numeric:tabular-nums; }
        .pq02 .pq-mark { color:#c0392b; }
      ...
      <div className="pq-num">
        {digits.map((d, i) => (<span key={i} className={`pq-d ${i === DATA.markIndex ? 'pq-mark' : ''}`}>{d}</span>))}
      </div>

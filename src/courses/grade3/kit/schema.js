...
export const INTERACTIONS = {
  ...
  // Механики ОБЪЯСНЕНИЯ действующего урока 1 третьего класса: раскрытие с
  // появлением единиц по одной (Screen1) и пауза на размышление с часами
  // (ScreenMing). Это не тесты, а способы объяснять, но имя им нужно: валидатор
  // должен уметь спросить, есть ли они в уроке.
  explain:    ['unitize', 'countdown_reveal', 'staged_reveal'],
...
// ---------------------------------------------------------------------------
// ПОД ВОПРОСОМ, А НЕ ЗАПРЕЩЕНО.
//
// Длинное тире было в списке запрещённых — как ошибка. Измерение показало, что
// оно стоит в 203 сегментах из 2058 (10%) в 14 из 19 эталонных уроков, и во всех
// просмотренных местах это обычная пунктуация: «Сегодня тема урока — сотни,
// десятки и единицы», «Первый способ — разрядная таблица». TTS читает такое тире
// как ПАУЗУ, и здесь пауза уместна.
//
// Единственное основание для запрета — одна двусмысленная строка в чек-листе
// CLAUDE.md («Нет длинных тире — с разъяснением после?»), а сам документ
// audio_rules жил в Notion и утрачен. Держать как ошибку правило, которое ломает
// 10% озвучки эталонных уроков и вред которого не подтверждён, — значит заставить
// переписать то, что работает.
//
// Поэтому: предупреждение, не ошибка. РЕШЕНИЕ МЕТОДИСТА может вернуть запрет —
// тогда 203 сегмента в 14 уроках нужно будет перечитать.
// Настоящий вред давали ────, ✗ и знаки операций: они остались ошибками.
export const WARN_IN_SPEECH = [
  // U+2014 —  U+2013 –
  {
    name: 'em/en dash',
    re: /[—–]/,
    why: 'тире читается как пауза; обычно уместно, но проверь, не заменяет ли оно слово',
  },
];
...
// Русское обращение — «ты» (решение методиста 2026-07-21). Из этого следует
// неочевидное второе правило: русское прошедшее время согласуется с родом, а пол
// ученика платформа не знает. «Ты нашёл» — обращение к мальчику; девочке урок
// говорит не про неё. Поэтому прошедшего времени второго лица в русских текстах
// нет: либо настоящее («ты видишь»), либо безлично («место видно»), либо «мы»
// («сегодня мы увидели»). Про персонажей — можно: Анвар мальчик, и «Анвар не
// поставил ноль» верно. Правило поймало 4 места в первом же написанном уроке,
// поэтому оно машинное, а не «надо помнить».
// ВНИМАНИЕ на регулярные выражения ниже: \b здесь НЕ РАБОТАЕТ. В JavaScript
// граница слова определена через \w = [A-Za-z0-9_], и кириллица в неё не входит,
// поэтому /\bты\s/ не находит «ты» вообще — первая версия правила молча пропускала
// все нарушения. Границы задаются группами (?<![а-яё]) и (?![а-яё]).
const RU_YOU = '(?<![а-яё])ты(?![а-яё])';
export const RU_TEXT_RULES = [
  {
    // Основная форма: прошедшее на -л, -ла, -лся, -лась (нашёл, увидел, справился).
    name: 'gendered past after «ты»',
    re: new RegExp(`${RU_YOU}\\s+(?:[а-яё]+\\s+){0,2}[а-яё]{2,}л(?:ся|а|ась)?(?![а-яё])`, 'i'),
    why: 'пол ученика неизвестен: настоящее время, безличная форма или «мы»',
  },
  {
    // Прошедшее без «л» — список, а не правило: русское прошедшее время
    // регулярным выражением целиком не описывается, а эти формы в детской речи
    // встречаются постоянно.
    name: 'gendered past after «ты» (irregular)',
    re: new RegExp(`${RU_YOU}\\s+(?:[а-яё]+\\s+){0,2}(?:ошибся|ошиблась|смог|смогла|помог|помогла|привык|привыкла|достиг|достигла)(?![а-яё])`, 'i'),
    why: 'пол ученика неизвестен: настоящее время, безличная форма или «мы»',
  },
];

// Узбекское обращение — «siz» (v2 §9.1). Проверяются только местоимения: они
// однозначны. Форму глагола автоматика не проверяет — во 2 классе такая проверка
// давала ложные срабатывания на словах, которые лишь похожи на повелительное
// наклонение («ayir», «qo'sh» — это операции, а не обращение к ученику).
export const UZ_REGISTER_RULES = [
  { name: 'sen instead of siz', re: /\b(sen|senga|sening|seni|senda|sendan)\b/i, why: 'обращение к ученику — siz' },
];
...
export const findUzTextIssues = (text) =>
  [...UZ_TEXT_RULES, ...UZ_REGISTER_RULES]
    .filter(({ re }) => re.test(String(text || '')))
    .map(({ name, re, why }) => ({ name, why, found: re.exec(String(text || ''))?.[0] || '' }));

export const findRuTextIssues = (text) =>
  RU_TEXT_RULES
    .filter(({ re }) => re.test(String(text || '')))
    .map(({ name, re, why }) => ({ name, why, found: re.exec(String(text || ''))?.[0] || '' }));

/**
 * Проверка ЛЮБОГО текста урока по локали — не только озвучки.
 * Апостроф и кириллица в узбекском, регистр обращения, род в русском прошедшем
 * времени: всё это встречается в подписях кнопок и вариантов не реже, чем в
 * репликах. Раньше эти правила существовали только как строки чек-листа, и
 * поэтому нарушались.
 */
export const findTextIssues = (text, loc) =>
  loc === 'uz' ? findUzTextIssues(text) : loc === 'ru' ? findRuTextIssues(text) : [];

...
// Части, которые урок умеет подсвечивать. Ключ приходит из данных (stage.visual.props.highlight).
export const PART_KEYS = ['monitor', 'unit', 'keyboard', 'mouse', 'cpu', 'ram', 'disk'];
...
export default function Computer3D({ highlight = null, open = false, label = null, hint = null }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const shell = box(1.9, 3.9, 3.4, C.case);
      unit.add(shell);
      ...
      const disk = box(0.5, 1.0, 1.4, C.disk, { metalness: 0.35, roughness: 0.4 });
      disk.position.set(0.05, -1.2, 0.5);
      inside.add(disk);
      unit.add(inside);
      ...
      // --- контактная тень: круг под предметами вместо честной тени -------------
      const shadowGeo = new THREE.CircleGeometry(4.6, 32);
      const shadowMat = new THREE.MeshBasicMaterial({ color: 0x8C8578, transparent: true, opacity: 0.13 });
      track(shadowGeo, shadowMat);
      ...
      const dimmable = [];
      root.traverse((o) => {
        if (o.isMesh && o.material && o.material !== side.material) {
      ...
      const state = { highlight: null, open: false, targetSide: 1, userRotY: 0, userRotX: 0 };

      const applyProps = (h, o) => {
        state.highlight = PARTS[h] ? h : null;
        state.open = !!o;
        state.targetSide = state.open ? 0.12 : 1;

        const lit = state.highlight ? new Set(partMeshes(state.highlight)) : null;
        dimmable.forEach(({ mesh, mat: m }) => {
          const isLit = !lit || lit.has(mesh);
          m.opacity = isLit ? 1 : 0.28;
          m.emissiveIntensity = mesh === glass ? 0.45 : (isLit && lit ? 0.25 : 0);
          if (m.emissive && mesh !== glass) {
            m.emissive.set(isLit && lit ? C.accent : 0x000000);
          }
        });
      };
      ...
      const tick = () => {
        ...
        const idle = reduceMotion ? 0 : Math.sin(time * 0.28) * 0.22;
        root.rotation.y = state.userRotY + idle;
        root.rotation.x = state.userRotX;

        // Боковая стенка снимается плавно, а не пропадает кадром.
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div ref={wrapRef} className="inf-canvas-wrap" style={{ aspectRatio: '1.6 / 1' }} aria-hidden="true">

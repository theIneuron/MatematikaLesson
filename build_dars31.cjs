// Dars31.jsx yig'uvchi: infra + base STYLES baytma-bayt Dars30 dan, o'rta qism + MATH-CSS yangidan.
const fs = require('fs');
const path = require('path');

const ROOT = __dirname;
const src = fs.readFileSync(path.join(ROOT, 'src/components/Dars30.jsx'), 'utf8');
...
// --- 1. INFRA: Dars30 boshidan POD UROK doc-comment'gacha (NumInputScreen oxiri + 2 bo'sh qator) ---
const cutMarker = '// ============================================================\n// --- POD UROK:';
const cutIdx = src.indexOf(cutMarker);
if (cutIdx < 0) throw new Error('infra cut marker topilmadi');
let infra = src.slice(0, cutIdx);
const oldComment = '// УРОК: Площадь треугольника — geom_5_03 (Dars30)';
const newComment = '// УРОК: Объём прямоугольного параллелепипеда — geom_5_04 (Dars31)';
if (!infra.includes(oldComment)) throw new Error('УРОК comment topilmadi');
infra = infra.replace(oldComment, newComment);

// --- 2. STYLES: base (geom_5_03 MATH-CSS'gacha) + mening CSS + tail (ambient + reduced-motion) ---
const stylesOpen = 'const STYLES = `';
const sOpenIdx = src.indexOf(stylesOpen);
const baseStart = sOpenIdx + stylesOpen.length;
const geomCssMarker = '/* MATH geom_5_03: TriViz';
...
fs.writeFileSync(path.join(ROOT, 'src/components/Dars31.jsx'), outLf, { encoding: 'utf8' });
const lines = outLf.split('\n').length;
console.log('Dars31.jsx yozildi:', lines, 'qator');

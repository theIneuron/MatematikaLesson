// Assembler for Dars27 (dec_5_04 — O'nli kasrni 10/100/1000 ga ko'paytirish va bo'lish), keep-visible rebuild.
// Infra (byte-exact) sliced from Dars28.jsx (1..before POD UROK marker).
// Base STYLES body extracted from Dars28 and injected into the middle's /*__BASE__*/ placeholder.
// Re-run after editing _dars27_middle.jsx:  node build_dars27.cjs
const fs = require('fs');
...
let mid = fs.readFileSync(path.join(root, 'src/components/_dars27_middle.jsx'), 'utf8').replace(/\r\n/g, '\n');
if (mid.indexOf('/*__BASE__*/') === -1) throw new Error('base-styles placeholder /*__BASE__*/ missing in _dars27_middle.jsx');
mid = mid.replace('/*__BASE__*/', stylesBody);

const out = infra + mid;
fs.writeFileSync(path.join(root, 'src/components/Dars27.jsx'), out, 'utf8');
console.log('Dars27.jsx built:', out.length, 'chars,', out.split('\n').length, 'lines');

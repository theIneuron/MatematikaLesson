...
- ❌ Don't use the browser's built-in speech-to-text (`SpeechRecognition` / `webkitSpeechRecognition`) — support is optional and unreliable across browsers. On voice-mode AI screens, capture audio with `MediaRecorder` via the `VoiceRecorder` pattern and send the WebM blob through `useGrader()` — never transcribe client-side.

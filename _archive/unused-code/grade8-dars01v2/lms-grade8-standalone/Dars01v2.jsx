...
// src/components/grade8/{core.jsx, labkit.jsx, Dars01v2.jsx}. Править нужно ИСХОДНИК,
...
const configureLesson = (next) => { cfg = { ...cfg, ...next } }
...
const Head = ({ eyebrow, title, em, lead }) => {
  const t = useT()
  return (
...
const Frac = ({ num, den, blocked, hot }) => (
...
const Caption = ({ children }) => <p className="g8l-caption">{children}</p>

// Bosish kerak bo'lgan joyni ko'rsatuvchi kursor (maket: animatsion strelka).
const Cursor = () => <span className="g8l-cursor" aria-hidden="true" />
...
const Options = ({ items, picked, wrongs, onPick, cols, disabled, compact }) => {
  const t = useT()
...
const Feedback = ({ kind = 'plain', children }) => (
...
// ============================================================
const LockedList = ({ items, opened, onOpen, render }) => {
  const t = useT()
  return (
...
// ============================================================
function useStaged(total, runKey, ms = 520, onStep) {
...
const Reveal = ({ show, children, className, delay }) => {
  if (!show) return null
  return (
    <div
...
// ============================================================
const Sequence = ({ tasks, helpNote, method, onFirstTry, onDone, onOpen }) => {
  const t = useT()

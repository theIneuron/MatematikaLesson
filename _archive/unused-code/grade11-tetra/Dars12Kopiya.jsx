...
import { Tetrahedron } from './tetra.jsx'
import { FEATURES as TETRA_FEATURES, metrics as tetraMetrics, tetraFmt } from './tetra-model.js'

// NUSXA. Dars 12 ning aynan o'zi, faqat oxiridan oldin bitta ekran
// qo'shilgan: to'g'ri tetraedrning interaktiv modeli (`tetra.jsx`).
// DIQQAT, metodistga: tetraedr STEREOMETRIYA (2-qism, III bob, 113-bet), u
// logarifmik tengsizliklar mavzusiga tegishli emas. Bu ekran shu darsning
// mazmuniga qo'shilmaydi, u alohida turadi.
const LESSON_ID = 'alg_11_12_tetra'
...
function Frame({ meta, right, screen, audio, solved, onPrev, onNext, onFinish, finished, children }) {
  ...
  return (
    // `section` -- ekran O'ZI aytadi: qo'shimcha ekran raqamlarni siljitgan,
    // `sectionOf` esa 15 ta ekran uchun yozilgan.
    <Stage eyebrow={t(meta.eyebrow)} right={right} block={BLOCK} screen={screen} total={TOTAL} audio={audio} nav={nav} section={meta.section}>
...
// ============================================================
// QO'SHIMCHA EKRAN. TO'G'RI TETRAEDR.
//
// Manba: claude.ai/design «3D object modeling», tetrahedron.html.
// Sahifa butunligicha ko'chirilmadi -- sabablari `tetra.jsx` boshida.
// Bu yerda FAQAT ma'lumot: matn, ovoz va savol.
// ============================================================
const TETRA_EDGE = 6

const S16 = {
  section: 'explain',
  eyebrow: L('Stereometriya', 'Стереометрия', 'Solid geometry'),
  title: L("To'g'ri tetraedr", 'Правильный тетраэдр', 'The regular tetrahedron'),
  spin: L('Sichqoncha bilan aylantiring, g\'ildirak masshtabni o\'zgartiradi',
    'Вращай мышью, колесо меняет масштаб',
    'Rotate with the mouse, the wheel changes the scale'),
  edgeLbl: L('Qirra', 'Ребро', 'Edge'),
  modes: [
    { id: 'solid', label: L('To\'liq', 'Сплошные', 'Solid') },
    { id: 'glass', label: L('Yarim shaffof', 'Полупрозрачные', 'Translucent') },
    { id: 'wire', label: L('Karkas', 'Каркас', 'Wireframe') },
  ],
  unfoldOn: L('Yoyish', 'Развернуть', 'Unfold'),
  unfoldOff: L('Yig\'ish', 'Свернуть', 'Fold back'),
  cutOn: L('Kubdan kesish', 'Отсечь от куба', 'Cut from the cube'),
  cutOff: L('Kubni yig\'ish', 'Собрать куб', 'Reassemble the cube'),
  clear: L('Hammasini olib tashlash', 'Снять построения', 'Clear the constructions'),
  secLbl: L('Kesim balandligi', 'Высота сечения', 'Section height'),
  faceLbl: L('Yoq', 'Грань', 'Face'),
  facePrompt: L('Yoqni bosing — yuzasi chiqadi',
    'Нажми на грань — покажется её площадь',
    'Tap a face to see its area'),
  probe: {
    question: L(
      "Qirra oltiga teng. Hajm qancha?",
      'Ребро равно шести. Чему равен объём?',
      'The edge equals six. What is the volume?',
    ),
    items: [
      { id: 'a', label: '25,46', correct: true,
        ok: L(
          "To'g'ri. Hajm qirraning kubini olti karra ikkining kvadrat ildiziga bo'lgandagi natija.",
          'Верно. Объём это куб ребра, делённый на шесть корней из двух.',
          'Correct. The volume is the cube of the edge divided by six root two.',
        ) },
      { id: 'b', label: '36,00',
        hint: L(
          "Siz faqat oltiga bo'ldingiz, maxrajdagi ikkining kvadrat ildizi tushib qoldi.",
          'Здесь деление только на шесть, а корень из двух в знаменателе потерян.',
          'Here the division is only by six, and the root two in the denominator is lost.',
        ) },
      { id: 'c', label: '62,35',
        hint: L(
          "Bu to'la sirt yuzasi, hajm emas. Sirt uchtaning kvadrat ildizini qirra kvadratiga ko'paytiriladi.",
          'Это площадь полной поверхности, а не объём. Поверхность это корень из трёх на квадрат ребра.',
          'That is the total surface area, not the volume. The surface is root three times the edge squared.',
        ) },
      { id: 'd', label: '4,90',
        hint: L(
          "Bu balandlik. Uni «Balandlik DH» tugmasi bilan chizmada ko'ring.",
          'Это высота. Посмотри её на модели кнопкой «Высота DH».',
          'That is the height. Look at it on the model with the button for the height DH.',
        ) },
    ],
  },
  // Kadrni ekranda ushlab turish, ms. MATN O'ZGARSA -- sonni ham to'g'rila.
  holds: [4500, 5000, 8000],
  audio: [
    A('mount',
      "Oldingizda to'g'ri tetraedr. Uning to'rt yog'i ham bir xil teng tomonli uchburchak.",
      'Перед тобой правильный тетраэдр. Все четыре его грани это одинаковые равносторонние треугольники.',
      'In front of you is a regular tetrahedron. All four of its faces are identical equilateral triangles.'),
    A('mount',
      "Pastdagi surgich qirrani o'zgartiradi. Hamma kattaliklar shu zahoti qayta hisoblanadi.",
      'Ползунок меняет ребро. Все величины пересчитываются сразу же.',
      'The slider changes the edge. Every quantity is recomputed at once.'),
    A('mount',
      "Tugmalar balandlikni, apofemani, medianalarni, ichki va tashqi sferani, ikkiyoqli burchakni va kesimni chizadi. Yoqni bossangiz uning yuzasi chiqadi.",
      'Кнопки строят высоту, апофему, медианы, вписанную и описанную сферы, двугранный угол и сечение. Нажми на грань, и покажется её площадь.',
      'The buttons draw the height, the apothem, the medians, the inscribed and circumscribed spheres, the dihedral angle and a section. Tap a face and its area appears.'),
    A('ask',
      "Endi savol. Qirra oltiga teng.",
      'Теперь вопрос. Ребро равно шести.',
      'Now a question. The edge equals six.'),
  ],
}

function Screen16({ screen, onAnswer, ...rest }) {
  const t = useT()
  const segments = useMemo(() => buildAuto(S16.audio, rest.lang, ['ask']), [rest.lang])
  const audio = useAudio(segments)
  const phase = useNarratedSteps(audio, textsOf(S16.audio, rest.lang), S16.holds)
  const [solved, setSolved] = useState(false)
  const [edge, setEdge] = useState(TETRA_EDGE)
  const [mode, setMode] = useState('glass')
  const [secT, setSecT] = useState(0.5)
  ...
  const M = tetraMetrics(edge)
  // Qurilma tugmalari UCHINCHI gapdan keyin ochiladi: ular aynan o'sha
  // gapda aytiladi. Ilgarigi bosish ko'rsatmani uzib qo'yardi.
  const ready = phase >= 2

  const flip = (key) => {
    if (!ready) return
    setFeats((prev) => ({ ...prev, [key]: !prev[key] }))
    if (key === 'cube' && feats.cube) setCutOpen(false)
  }

  return (
    <Frame meta={S16} screen={screen} audio={audio} solved={solved} {...rest}>
      <Cols l={1.5} r={1} align="start">
        <Col>
          <Tetrahedron
            api={api}
            edge={edge}
            ...
            onFace={setFace}
          />
          {/* Yoq haqidagi satr TELEFONDA yashiriladi: u ko'rsatma, model esa
              baribir barmoq bilan boshqariladi. */}
          <span className="g11-hint g11-wrap g11-hide-tight">{face
            ? t(S16.faceLbl) + ' ' + face.label + ':  S₁ = ' + tetraFmt(face.S1)
            : t(S16.facePrompt)}
          </span>
        </Col>
        ...
        <Col>
          ...
          <Panel tone="quiet" pad={9} style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
              <Tag tone="quiet">{t(S16.edgeLbl)}</Tag>
              <span className="g11-expr g11-expr-sm"><Fx>{'a = ' + tetraFmt(edge, 1)}</Fx></span>
            </div>
            <input
              type="range" min="2" max="10" step="0.5" value={edge}
              className="g11-tetra-range"
              onChange={(e) => setEdge(parseFloat(e.target.value))}
            />
            {/* Ko'rinish tugmalari TELEFONDA yashiriladi: yarim shaffof
                rejim boshidan yoqilgan va aynan u kerak. */}
            <div className="g11-tetra-seg g11-hide-tight">
              {S16.modes.map((m) => (
                <button
          ...
          {/* Qurilmalar */}
          <div className="g11-tetra-tg" style={{ opacity: ready ? 1 : 0.34 }}>
            {TETRA_FEATURES.map((f) => (
              <button
                key={f.key} type="button"
                style={{ color: f.color }}
                aria-pressed={!!feats[f.key]}
                disabled={!ready}
                onClick={() => flip(f.key)}
              ><i />
                <span className="g11-tetra-full">{t(f.label)}</span>
                <span className="g11-tetra-short">{t(f.short)}</span>
              </button>
            ))}
          </div>

          {/* Joy OLDINDAN band: kesim surgichi chiqqanda blok sakramaydi.
              Telefonda zaxira olib tashlanadi -- u yerda har piksel qimmat. */}
          <div className="g11-tetra-secslot">
            {feats.section ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {/* Yorliq TELEFONDA yashiriladi: «Kesim» tugmasi yoniq turadi,
                    surgichning nimaligi shundan ko'rinib turibdi. */}
                <span className="g11-hint g11-hide-tight" style={{ whiteSpace: 'nowrap' }}>{t(S16.secLbl)}</span>
                <input
                  type="range" min="0.05" max="0.95" step="0.05" value={secT}
                  className="g11-tetra-range"
                  onChange={(e) => setSecT(parseFloat(e.target.value))}
                />
              </div>
            ) : null}
          </div>

          {/* Uchta tugma bitta qatorda turishi kerak: kub yoqilganda ular
              ikkinchi qatorga o'tib, 393x660 da ekran oshib ketardi. */}
          <div className="g11-tetra-actions" style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            <Btn tone="soft" onClick={() => { if (!api.current) return; api.current.unfold(unfolded ? 0 : 1); setUnfolded(!unfolded) }}>
              {t(unfolded ? S16.unfoldOff : S16.unfoldOn)}
            </Btn>
            {feats.cube ? (
              <Btn tone="soft" onClick={() => { if (!api.current) return; api.current.cut(cutOpen ? 0 : 1); setCutOpen(!cutOpen) }}>
                {t(cutOpen ? S16.cutOff : S16.cutOn)}
              </Btn>
            ) : null}
            <Btn tone="quiet" onClick={() => { setFeats({}); setCutOpen(false); setFace(null); if (api.current) api.current.clear() }}>
              {t(S16.clear)}
            </Btn>
          </div>

          {/* Kattaliklar: qirra o'zgarganda shu zahoti qayta hisoblanadi */}
          {/* Formulalar jadvali TOR ekranda yashiriladi: ma'lumot
              yo'qolmaydi, hamma qiymat modelning o'zida yozilgan. */}
          <Panel tone="quiet" pad={9} className="g11-hide-tight">
            <div className="g11-tetra-fx">
              <span><Fx>{'V = a³/(6√2)'}</Fx></span><b>{tetraFmt(M.V)}</b>
              <span><Fx>{'S = √3·a²'}</Fx></span><b>{tetraFmt(M.S)}</b>
              <span><Fx>{'H = a√6/3'}</Fx></span><b>{tetraFmt(M.H)}</b>
              <span><Fx>{'r = a√6/12'}</Fx></span><b>{tetraFmt(M.r)}</b>
              <span><Fx>{'R = a√6/4'}</Fx></span><b>{tetraFmt(M.R)}</b>
            </div>
          </Panel>
        </Col>
      </Cols>
      ...
      {phase >= 3 ? (
        <div className="g11-in">
          <Probe audio={audio} data={S16.probe} cols={4} fbSlot={46} dense
            onSolved={(r) => { setSolved(true); onAnswer({ ...r, screen, tag: 'tetra_volume' }) }} />
        </div>
      ) : <Slot mh={46} />}
    </Frame>
  )
...
const SCREENS = [
  ...
  Screen9, Screen10, Screen11, Screen12, Screen13, Screen14,
  // Tetraedr YAKUNDAN OLDIN turadi: yakun ekrani oxirgi bo'lib qolishi
  // kerak, aks holda «Darsni yakunlash» tugmasi noto'g'ri ekranga tushadi.
  Screen16, Screen15,
]

export default function Grade11Dars12Kopiya({
  studentName,
  lang: langProp,
  ttsApiBase,
  voiceGender,
  ...
  const finish = useCallback(() => {
    ...
    else console.log('[Grade11 Dars12 nusxa] onFinished', payload)
